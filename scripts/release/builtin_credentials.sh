...
GITHUB_CLIENT_ID="${CC_BUILTIN_GITHUB_CLIENT_ID:-}"
...
if [ -z "$GOOGLE_ID" ] && [ -z "$KLIPY_KEY" ] && [ -z "$GITHUB_CLIENT_ID" ]; then
  echo "::warning::no built-in credentials in the environment — this build ships none (Google Calendar will ask for a client id + secret, the GIF picker stays hidden, GitHub offers only a pasted token)"
  exit 0
fi
...

{
  ...
  echo
  echo "/// The built-in GitHub App client id for the device-flow sign-in."
  echo "const String builtinGitHubClientId = $(dart_literal "$GITHUB_CLIENT_ID");"
} > "$TARGET"
...
echo "      github: $([ -n "$GITHUB_CLIENT_ID" ] && echo present || echo absent)"
