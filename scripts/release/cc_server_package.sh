...
# key) are NOT handled here. They are baked into the server's source constants by
# `builtin_credentials.sh inject` — one release-job step ahead of every build, so
# that this script and the desktop packagers cannot disagree about them. Run it
# yourself before a local release build, or this archive ships without them.
#
