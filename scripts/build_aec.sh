#!/usr/bin/env bash
#
# Builds libaec_ffi — a thin C ABI (native/aec/aec_ffi.cc) over WebRTC's AEC3
...
#
# Usage:
#   scripts/build_aec.sh [DEST_DIR]
#   WAP_REF=<sha> scripts/build_aec.sh ./build/natives
set -euo pipefail
...
DEST="${1:-}"
SHIM="$REPO_ROOT/native/aec/aec_ffi.cc"
...
dests+=("$REPO_ROOT/macos/Frameworks")
[ -n "$DEST" ] && dests+=("$DEST")
