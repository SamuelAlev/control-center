...
void main() {
  ...
  setUp(() {
    space = _FakeChannel();
    rpc = RemoteRpcClient(space)..start();
  ...
  test('speaks the op names and argument keys the server declares', () async {
    ...
    expect(
      space.opsCalled.toSet(),
      containsAll(<String>{'fleet.workerEvents', 'fleet.workerComplete'}),
    );
    for (final call in space.callsTo('fleet.workerEvents')) {
      expect(call, contains('frames'));
    }
    for (final call in space.callsTo('fleet.workerComplete')) {
  ...
  test('every frame decodes with the parser the server uses', () async {
    ...
    final frames = space.eventFrames;
  ...
  test('the last event is Done and the report agrees with it', () async {
    ...
    final report = space.completionReport!;
  ...
  test('a failed send is counted as lost, never silently dropped', () async {
    space.failOp('fleet.workerEvents');
    await runJob(leaseFor());
  ...
  test(
    ...
    () async {
      ...
      final calls = space.callsTo('fleet.workerEvents');
      final frames = space.eventFrames;
      ...
      expect(space.completionReport!.success, isTrue);
    },
  ...
  test('never puts the lease env on the wire', () async {
    ...
    final wire = jsonEncode(space.frames);
    expect(
      wire,
