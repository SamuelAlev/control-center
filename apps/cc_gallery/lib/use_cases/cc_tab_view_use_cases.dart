...
/// Labels that combine an icon with text; the strip inherits the
...
class _IconLabelsDemoState extends State<_IconLabelsDemo> {
  ...
  Widget build(BuildContext context) {
    return CcTabView(
      ...
      tabs: [
        CcTabViewEntry(
          label: _iconLabel(CcIcons.bot, 'Agents'),
          content: _panel(context, 'Agents running in this workspace.'),
        ),
        CcTabViewEntry(
          label: _iconLabel(CcIcons.gitPullRequest, 'Pull requests'),
          content: _panel(context, 'Open pull requests awaiting review.'),
        ),
        CcTabViewEntry(
          label: _iconLabel(CcIcons.workflow, 'Pipelines'),
