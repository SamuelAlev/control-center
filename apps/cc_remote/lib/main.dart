...
/// `fvm flutter build web --release --wasm` and deploy to
...
class _CcRemoteAppState extends ConsumerState<CcRemoteApp>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(remoteUpdateProvider.notifier).start();
    });
  }

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Returning to the PWA tab is the moment a deploy actually matters —
    // re-check the origin's version manifest on focus.
    if (state == AppLifecycleState.resumed && mounted) {
      ref.read(remoteUpdateProvider.notifier).checkForUpdate();
    }
  }
