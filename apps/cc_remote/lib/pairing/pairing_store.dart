...
///
/// This is the stored twin of the v2 `PairingPayload` (cc_domain): the server's
/// full [ConnectionDescriptor] — every known path plus its identity
/// fingerprint — and the device credential (`deviceId` + `psk`). On top of the
/// payload it carries the TOFU [pinnedFingerprint]: seeded from the
/// descriptor's `fp` at pairing time and updated by the connection
/// supervisor's `onFingerprintPinned` after the first verified connect.
class PairingRecord {
  ...
  /// Builds a record from a freshly-scanned pairing [payload]. The TOFU pin is
  /// initialized from the descriptor's fingerprint — the QR was handed over
  /// out-of-band (camera), so it is the trust anchor.
  factory PairingRecord.fromPayload(PairingPayload payload) => PairingRecord(
    descriptor: payload.descriptor,
    deviceId: payload.deviceId,
    psk: payload.psk,
    expiresAt: payload.expiresAt,
    pinnedFingerprint: payload.descriptor.fingerprint,
  );

  /// Decodes the persisted JSON shape (the payload's compact keys plus `pf`,
  /// the TOFU pin). Throws when the descriptor is missing or malformed — the
  /// caller treats that as "not paired".
  factory PairingRecord.fromJson(Map<String, dynamic> json) {
    final d = json['d'];
    if (d is! Map) {
      throw const FormatException('Pairing record has no descriptor');
    }
    return PairingRecord(
      ...
      pinnedFingerprint: json['pf'] as String? ?? '',
    );
  }

  /// How to reach the server: every known path + the published fingerprint.
  /// Refreshed in place (via [copyWith] + `PairingStore.save`) when the server
  /// re-publishes its descriptor over a live connection.
  final ConnectionDescriptor descriptor;

  /// The paired device id (`i`), sent in the auth handshake.
  ...
  /// The TOFU-pinned server identity fingerprint (`pf`). Any later mismatch is
  /// terminal — the user must re-pair; there is no "continue anyway".
  ...
  /// keeps working across reconnects (the PSK is durable).
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
    'v': PairingPayload.currentVersion,
    'd': descriptor.toJson(),
    'i': deviceId,
    'k': psk,
    'x': expiresAt.millisecondsSinceEpoch,
    'pf': pinnedFingerprint,
  };

  @override
  String toString() =>
      'PairingRecord(server: ${descriptor.serverId} '
      '"${descriptor.serverName}", ${descriptor.paths.length} paths, '
      'device: $deviceId, expired: $isExpired)';
}
...
class PairingStore {
  ...
  /// the URL. By then [decodeFragmentOffer] would see an empty hash. Grabbing it
  ...
  /// space onto attacker infrastructure (one-click MITM). Holding it as an
  ...
  /// Persists [record] and requests persistent storage (best-effort). Also
  /// used to refresh the stored descriptor / pinned fingerprint in place.
  ...
  /// Decodes a base64url v2 pairing-payload fragment into a [PairingRecord],
  /// or `null`.
  PairingRecord? _decode(String fragment) {
    try {
      return PairingRecord.fromPayload(PairingPayload.decode(fragment));
    } catch (_) {
      return null;
    }
  }
