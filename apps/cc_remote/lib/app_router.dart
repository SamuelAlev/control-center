...
import 'package:cc_remote/screens/playbooks_screen.dart';
...
///
/// The bottom-tab shell owns the four feature roots (`/tickets`, `/messaging`,
/// `/newsfeed`, `/playbooks`). Detail routes (`/ticket/:id`,
/// `/channels/:channelId`, `/article/:articleId`, `/playbook/:id`) and
/// `/workspaces` are top-level full-screen routes so they cover the tab bar.
/// `/connect` is shown while unpaired; the redirect flips to it (and back) on
/// [RemoteSession] state changes via [RouterRefresh].
  ...
  return GoRouter(
    ...
    routes: <RouteBase>[
      ...
      GoRoute(
        path: '/playbook/:id',
        builder: (context, state) =>
            PlaybookRunScreen(playbookId: state.pathParameters['id']!),
      ),
