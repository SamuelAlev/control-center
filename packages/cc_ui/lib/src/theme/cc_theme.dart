...
extension DesignSystemTokensBuildContext on BuildContext {
  ...
  /// The active [DesignSystemTokens], falling back to the light set when there
  /// is no [CcTheme] ancestor — the non-null form of [designSystem].
  ///
  /// Prefer this at call sites that just need a token and would otherwise
  /// repeat `context.designSystem ?? DesignSystemTokens.light()`. Hold it in a
  /// local (`final t = context.ds;`) when a build method reads several tokens.
