...
class _CcTooltipPanelState extends State<_CcTooltipPanel> {
  ...
  Widget _precisePanel(
    ...
    // Half the caret plus its corner inset — the caret centre must stay within
    // [lo, hi] so the triangle never clips the panel's rounded corners.
    double clampCentre(double target, double extent) {
      const lo = _caretInset + _caretSpan / 2;
      final hi = extent - _caretInset - _caretSpan / 2;
      return target.clamp(lo, hi < lo ? lo : hi);
    }
