...
class _CcProgressBarState extends State<CcProgressBar>
  ...
  Widget build(BuildContext context) {
    ...
    final radius = BorderRadius.all(
      Radius.circular(
        widget.height / 2 < AppRadii.pill ? widget.height / 2 : AppRadii.pill,
      ),
    );
    ...
    Widget bar(Widget fill) {
      return ClipRRect(
        borderRadius: radius,
        child: Container(height: widget.height, color: trackColor, child: fill),
      );
    }
    ...
    if (!_isIndeterminate) {
      ...
      content = LayoutBuilder(
        builder: (context, constraints) {
          ...
          return bar(
            // The indeterminate segment repaints at 60 fps. Without a
            // boundary that repaint propagates to the nearest ancestor one —
            // typically the whole surface hosting the bar — so an
            // indeterminate progress bar re-rasterizes its container every
            // frame for the life of the operation it is reporting.
            RepaintBoundary(
    ...
    // Always expose progress semantics (the label "can be visually
    // hidden, but must be defined"). No live region — a progress indicator is
    // a static node the user navigates to, not a status announcement.
    return Semantics(
