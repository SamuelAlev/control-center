...
class CcResizableController extends ChangeNotifier {
  ...
  /// Whether [dispose] has run. A pointer's hit-test path is captured at
  /// pointer-down and keeps routing moves to render objects that have since
  /// been disposed, so a drag can outlive the tree that owns this controller —
  /// every mutator below is a no-op once that happens rather than an assertion.
  ...
  bool setAvailable(double available) {
    if (_disposed || _available == available) {
      return false;
    }
  ...
  bool resizeBy(int index, double delta) {
    if (_disposed || index < 0 || index + 1 >= _extents.length || delta == 0) {
      return false;
    }
    ...
    final applied = appliedByBefore.abs() < appliedByAfter.abs()
        ? appliedByBefore
        : appliedByAfter;
    if (applied == 0) {
      return false;
...
class _CcResizableState extends State<CcResizable> {
  ...
  void _onDrag(int dividerIndex, double delta) {
    // A pointer that went down on a divider keeps delivering moves through the
    // hit-test path captured at pointer-down, even after this element is gone
    // (the pane closed, the tab switched) — touching the controller then would
    // resurrect a disposed one, and `_controller` would happily hand back the
    // disposed internal instance.
    if (!mounted) {
      return;
    }
...
class _CcResizableDividerState extends State<_CcResizableDivider> {
  ...
  void _onExit() {
    _hoverTimer?.cancel();
    // MouseRegion reports an exit when the region is removed, and a pointer
    // route captured at pointer-down outlives its render object — both arrive
    // after this state is defunct, so every transition checks [mounted].
    if (_hovered && mounted) {
