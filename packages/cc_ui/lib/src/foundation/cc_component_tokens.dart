...
class CcButtonTokens {
  ...
  /// Success — solid green for an affirming verdict (approve/accept).
  ///
  /// `bgSuccessSolid` (green700), not the brighter `success` signal: white
  /// clears 4.5:1 on it in both brightnesses, where green600 is 3.3:1. Mirrors
  /// [CcButtonTokens.destructive] so a verdict pair is one shape in two hues.
  factory CcButtonTokens.success(DesignSystemTokens t) => CcButtonTokens(
    bg: t.bgSuccessSolid,
    bgHover: t.bgSuccessSolidHover,
    bgPressed: t.bgSuccessSolidHover,
    fg: t.textWhite,
    border: _transparent,
    borderHover: _transparent,
  );

  /// Destructive — solid red for delete/remove actions and the negative half
  /// of a verdict pair.
