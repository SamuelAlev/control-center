...
| `CcMotion`                   | Three enter speeds `fast`(80ms)/`moderate`(160ms, alias `normal`)/`slow`(240ms), matching `*Exit` tokens one tier quicker (60/120/160), plus `fade`(80ms). Curves `standard`/`emphasized`. `resolve`/`resolveTravel` drop movement under reduced motion; `resolveFade` keeps the opacity fade. |
| `CcFonts`                    | Family resolution: `CcFonts.ui(...)` (Manrope) and `CcFonts.code(...)` (Fira Code), bundled as host assets.                                                                                                                                                                                        |
...
├── primitives/    # focus_ring, focus_modality (keyboard-only :focus-visible), image_fade
