...
void main() {
  group('CcAvatar', () {
    ...
    testWidgets('announces its semantic label, and image-ness', (tester) async {
      // An image-backed avatar used to announce NOTHING: the initials text only
      // exists on the fallback path, so a screen reader got a bare disc — in a
      // product where attribution (who said it, who ran it) is the point.
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
        ccTestApp(
          const CcAvatar(
            image: AssetImage('nonexistent.png'),
            semanticLabel: 'Samuel Alev',
          ),
        ),
      );
      expect(find.bySemanticsLabel('Samuel Alev'), findsOneWidget);
      handle.dispose();
    });

    testWidgets('falls back to initials as the accessible name', (
      tester,
    ) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(ccTestApp(const CcAvatar(initials: 'SA')));
      expect(find.bySemanticsLabel('SA'), findsOneWidget);
      handle.dispose();
    });

    testWidgets('a decorative avatar is excluded, not announced', (
      tester,
    ) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(
      ...
      // No identity to announce ⇒ nothing is announced, rather than an
      // unlabelled image node for a screen reader to stop on.
      expect(
        tester.getSemantics(find.byType(CcAvatar)).label,
        isEmpty,
        reason: 'a decorative avatar must contribute no accessible name',
      );
      handle.dispose();
    });
  });
