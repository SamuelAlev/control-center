...
void main() {
  group('CcTextArea', () {
    ...
    // Regression: pointer selection must work in prose fields too — a bare
    // RenderEditable places the caret on tap and nothing more, so the system
    ...
    testWidgets('double-clicking selects the word under the pointer', (
      ...
      addTearDown(controller.dispose);
      await tester.pumpWidget(ccTestApp(CcTextArea(controller: controller)));
    ...
    // Regression: a blurred field kept painting its selection highlight, so
    // two fields looked selected at once.
    testWidgets('only the focused area paints its selection', (tester) async {
      final focusNode = FocusNode();
      addTearDown(focusNode.dispose);
      final controller = TextEditingController(text: 'hello world');
      addTearDown(controller.dispose);
      await tester.pumpWidget(
        ccTestApp(CcTextArea(controller: controller, focusNode: focusNode)),
      );
