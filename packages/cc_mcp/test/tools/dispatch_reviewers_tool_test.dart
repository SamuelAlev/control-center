  ...
  group('DispatchReviewersTool', () {
    ...
    group('metadata', () {
      ...
      test('inputSchema declares required fields', () {
        ...
        expect(
          schema['required'],
          containsAll(['space_id', 'workspace_id', 'reviewers']),
        );
      });

      test('inputSchema properties have correct types', () {
        final props = tool.inputSchema['properties'] as Map<String, dynamic>;
        expect((props['space_id'] as Map<String, dynamic>)['type'], 'string');
        expect(
          (props['workspace_id'] as Map<String, dynamic>)['type'],
          'string',
        );
        expect((props['reviewers'] as Map<String, dynamic>)['type'], 'array');
        expect(
    ...
    group('argument validation', () {
      ...
      test('rejects null space_id', () async {
        final result = await tool.call({
          'space_id': null,
          'workspace_id': 'ws-1',
          'reviewers': [
      ...
      test('rejects missing reviewers', () async {
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, contains('reviewers'));
      });
      ...
      test('rejects non-integer concurrency', () async {
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, contains('concurrency'));
      });
    ...
    group('success', () {
      ...
      test('next_step reports unmatched roles', () async {
        service.stubResult({
          'dispatched': [],
          'unmatched': [
            {'role': 'rust-expert'},
        ...
        expect(
          data['next_step'],
          'report each unmatched role so the user can decide on hires',
        );
      });
    });
