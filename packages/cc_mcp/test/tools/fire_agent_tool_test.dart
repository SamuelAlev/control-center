...
void main() {
  group('FireAgentTool', () {
    ...
    test('an agent from a different workspace is simply not found', () async {
      ...
      // at all — the tool never sees the other workspace's agent.
    ...
    test(
      'buildConfirmationRequest returns payload with title "Fire agent"',
      () {
    ...
    test(
      'buildConfirmationRequest with missing agent_id shows unknown in detail',
      () {
        final payload = tool.buildConfirmationRequest({'workspace_id': 'ws-1'});
    ...
    test('returns error when workspace_id is an int', () async {
      final result = await tool.call({'workspace_id': 123, 'agent_id': 'a-1'});
    ...
    test('returns error when agent_id is an int', () async {
      final result = await tool.call({'workspace_id': 'ws-1', 'agent_id': 456});
    ...
    test(
      'empty string workspace_id passes validation (not-found on lookup)',
      () async {
        await repository.upsert(
          Agent(
            id: 'a-1',
        ...
        final result = await tool.call({'workspace_id': '', 'agent_id': 'a-1'});

        // An empty workspace id names no workspace, so the lookup finds
        // nothing — an error, not a crash.
        expect(result.isError, isTrue);
        expect(result.content.first.text, contains('not found'));
      },
    );
    ...
    test('empty string agent_id returns not-found error', () async {
      final result = await tool.call({'workspace_id': 'ws-1', 'agent_id': ''});
    ...
    test('agent is deleted from repository after successful call', () async {
      ...
      final fetched = await repository.getById('ws-1', 'a-1');
    ...
    test('removes only the targeted agent, leaving others intact', () async {
      ...
      expect(result.isError, isFalse);
      expect(await repository.getById('ws-1', 'a-2'), isNull);
      expect(await repository.getById('ws-1', 'a-1'), isNotNull);
      expect(await repository.getById('ws-1', 'a-3'), isNotNull);
