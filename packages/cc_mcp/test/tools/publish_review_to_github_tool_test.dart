  ...
  group('PublishReviewToGithubTool', () {
    ...
    group('metadata', () {
      ...
      test('inputSchema has required fields', () {
        ...
        expect(required, contains('space_id'));
      });
    ...
    group('argument validation', () {
      ...
      test('rejects missing space_id', () async {
      ...
      test('rejects non-string space_id', () async {
        final result = await tool.call({
          'workspace_id': 'ws-1',
          'space_id': true,
        });
    ...
    group('default argument values', () {
      test('selection defaults to consensus when omitted', () async {
        ...
        await tool.call({'workspace_id': 'ws-1', 'space_id': 'ch-1'});
    ...
    group('success', () {
      ...
      test('passes workspace and space through to service', () async {
        service.stubResult(
          const PublishReviewResult(
            reviewId: 10,
            event: 'COMMENT',
        ...
        await tool.call({'workspace_id': 'ws-alpha', 'space_id': 'ch-beta'});

        expect(service.lastWorkspaceId, 'ws-alpha');
        expect(service.lastSpaceId, 'ch-beta');
      });
    ...
    group('errors', () {
      test('WorkspaceMismatchException returns error with message', () async {
        service.stubError(
          const WorkspaceMismatchException('Space belongs to workspace B'),
        );
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, 'Space belongs to workspace B');
      });
      ...
      test('ArgumentError returns error with message', () async {
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, 'Invalid PR number: -1');
      });
      ...
      test('ArgumentError with null message returns fallback', () async {
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, 'Invalid argument');
      });
