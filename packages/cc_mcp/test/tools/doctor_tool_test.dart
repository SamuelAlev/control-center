...
void main() {
  group('DoctorTool', () {
    ...
    test('single OK check with null message renders OK placeholder', () async {
      fakePort.stub(
        const DoctorReport(
          results: [DiagnosticResult(name: 'db', status: DiagnosticStatus.ok)],
        ),
      );
    ...
    test('multiple checks of same status (all OK)', () async {
      ...
      expect(result.isError, isFalse);
      expect(result.content.first.text, contains('[OK] **db**: OK'));
      expect(result.content.first.text, contains('[OK] **disk**: OK'));
      expect(result.content.first.text, contains('[OK] **network**: OK'));
      expect(result.content.first.text, contains('All checks passed'));
    });
    ...
    test(
      'report with all three statuses shows correct icon prefixes',
      () async {
        fakePort.stub(
          const DoctorReport(
            results: [
    ...
    test('allOk false when warnings exist but zero errors', () async {
      fakePort.stub(
        const DoctorReport(
          results: [
            DiagnosticResult(
