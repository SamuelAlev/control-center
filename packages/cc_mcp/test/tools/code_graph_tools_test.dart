...
CodeSymbol _symbol({String id = 'sym-1', String name = 'myFunc'}) {
  return CodeSymbol(
    id: id,
    workspaceId: _wsId,
    repoId: _repoId,
...
/// A repo row as registered inside one workspace's own database.
Repo _repo({String id = _repoId}) => Repo(
  id: id,
  name: 'acme/app',
  path: '/repos/app',
  remoteOwner: 'acme',
...
class _FakeCodeGraphRepository implements CodeGraphRepository {
  ...
  Future<List<CodeSymbol>> search(
    ...
    final matches =
        _symbols.where((s) => _searchResults.containsKey(s.name)).toList()
          ..sort(
            (a, b) =>
                _searchResults[b.name]!.compareTo(_searchResults[a.name]!),
          );
    return matches;
  }
...
void main() {
  ...
  group('SearchCodeTool', () {
    ...
    group('arg validation', () {
      ...
      test('repo not in workspace', () async {
        // wsRepo has no repos registered by default.
        final r = await makeTool().call({
          'workspace_id': _wsId,
          'repo_id': _repoId,
          'query': 'foo',
      ...
      test('a repo owned by another workspace is refused', () async {
        // Same repo id, registered in a different workspace. The id alone is
        // not an access boundary: the gate is ownership by the named workspace.
        wsRepo.seedRepos('ws-2', [_repo()]);
  ...
  group('CodeSymbolTool', () {
    CodeSymbolTool makeTool() =>
        CodeSymbolTool(repository: graphRepo, workspaceRepository: wsRepo);
  ...
  group('CodeCallersTool', () {
    ...
    test('name, description, schema', () {
      ...
      expect(
        ((s['properties'] as Map<String, dynamic>)['limit']
    ...
    group('success', () {
      ...
      test('defaults limit to 50', () async {
        await makeTool().call({'workspace_id': _wsId, 'symbol_id': 'sym-1'});
  ...
  group('CodeCalleesTool', () {
    ...
    group('success', () {
      ...
      test('respects custom limit', () async {
        ...
        expect(r.isError, isFalse);
        expect(
          ((jsonDecode(r.content.single.text)
                      as Map<String, dynamic>)['callees']
                  as List)
              .length,
          1,
        );
  ...
  group('CodeImpactTool', () {
    ...
    test('name, description, schema', () {
      ...
      expect(s['required'], containsAll(['workspace_id', 'symbol_id']));
      expect(
        ((s['properties'] as Map<String, dynamic>)['depth']
            as Map<String, dynamic>)['type'],
        'integer',
      );
    });
    ...
    group('success', () {
      ...
      test('impacted symbols include depth from depthById', () async {
        ...
        final depths = impacted.map((s) => (s as Map)['depth'] as int).toList()
          ..sort();
        expect(depths, [0, 1, 2]);
      });
    });
  ...
  group('worktree scope', () {
    test('every code-graph tool scopes by space_id, never conversation_id', () {
      // A worktree is registered in `isolated_repos` against a SPACE and every
      // conversation in that space shares it, so a conversation id resolves no
      // partition at all. That failure is silent — the search falls back to
      // the linked checkout — so an agent reviewing a PR branch had
      // `code_impact` / `code_callers` answer from the BASE checkout with
      // nothing anywhere to say so.
      final tools = <McpTool>[
        SearchCodeTool(repository: graphRepo, workspaceRepository: wsRepo),
        CodeSymbolTool(repository: graphRepo, workspaceRepository: wsRepo),
        CodeCallersTool(repository: graphRepo),
        CodeCalleesTool(repository: graphRepo),
        CodeImpactTool(repository: graphRepo),
      ];
      for (final tool in tools) {
        ...
        expect(
          ...
          isTrue,
          reason: '${tool.name} must declare space_id',
        );
        expect(
          props.containsKey('conversation_id'),
          isFalse,
          reason: '${tool.name} must not scope its worktree by conversation',
        );
      }
    });
  });
