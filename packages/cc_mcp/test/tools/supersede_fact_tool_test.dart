...
void main() {
  group('SupersedeFactTool', () {
    ...
    test('returns error for missing superseding_fact_id', () async {
      final result = await tool.run({'workspace_id': 'ws-1', 'fact_id': 'f-1'});
    ...
    test(
      'scopes lookup to workspace_id — does not leak across workspaces',
      () async {
        fakeFactRepo.seed([
          MemoryFact(
            id: 'f-1',
    ...
    group('metadata', () {
      ...
      test('inputSchema has type=object and required fields', () {
        ...
        expect(
          schema['required'],
          containsAll(['workspace_id', 'fact_id', 'superseding_fact_id']),
        );
        ...
        expect(
          (properties['fact_id'] as Map<String, dynamic>)['type'],
          'string',
        );
        expect(
          (properties['superseding_fact_id'] as Map<String, dynamic>)['type'],
          'string',
        );
      });
    });
    ...
    group('response shape', () {
      test(
        'success response includes fact_id, superseded_by, status',
        () async {
          fakeFactRepo.seed([
            MemoryFact(
              id: 'f-resp',
