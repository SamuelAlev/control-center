...
void main() {
  group('UpdateMyNotesTool', () {
    ...
    test('returns error for missing workspace_id', () async {
      final result = await tool.run({'agent_id': 'a-1', 'content': 'notes'});
    ...
    test('inputSchema has correct structure', () {
      ...
      expect(
        schema['required'],
        containsAll(['workspace_id', 'agent_id', 'content']),
      );
      expect((schema['required'] as List).length, 3);
      final props = schema['properties'] as Map<String, dynamic>;
      expect(props['workspace_id'], {
        'type': 'string',
        'description': 'The workspace ID.',
      });
      expect(props['agent_id'], {
        'type': 'string',
        'description': 'The agent ID.',
      });
      expect(props['content'], {
        'type': 'string',
        'description': 'New content for the notes (markdown).',
      });
    });
    ...
    test(
      'different workspace_id with same agent_id creates separate memory',
      () async {
        await tool.run({
    ...
    test('updated_at timestamp changes on each update', () async {
      ...
      expect(
        ts2.microsecondsSinceEpoch,
        greaterThan(ts1.microsecondsSinceEpoch),
      );
    });
  });
}
