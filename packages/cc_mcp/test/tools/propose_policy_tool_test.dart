...
class _FakeResolveOrCreateDomainUseCase extends ResolveOrCreateDomainUseCase {
  ...
  group('ProposePolicyTool', () {
    ...
    group('argument validation', () {
      ...
      test('rejects missing domain', () async {
        final result = await tool.run({'workspace_id': 'ws_1', 'rule': 'Do X'});
        expect(result.isError, isTrue);
    ...
    group('errors', () {
      test('returns error when promote returns null', () async {
        ...
        expect(result.isError, isTrue);
        expect(result.content.first.text, contains('Failed to create policy'));
      });
