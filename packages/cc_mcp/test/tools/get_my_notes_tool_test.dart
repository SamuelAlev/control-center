...
import 'package:cc_domain/testing/fake_memory_repositories.dart';
...
void main() {
  group('GetMyNotesTool', () {
    ...
    test('inputSchema has correct structure', () {
      ...
      expect(
        (properties['workspace_id'] as Map<String, dynamic>)['type'],
        'string',
      );
      expect(properties.containsKey('agent_id'), isTrue);
      expect(
        (properties['agent_id'] as Map<String, dynamic>)['type'],
        'string',
      );
    });
    ...
    test('returns error when workspace_id is int', () async {
      final result = await tool.run({'workspace_id': 42, 'agent_id': 'a-1'});
      expect(result.isError, isTrue);
    });

    test('returns error when agent_id is int', () async {
      final result = await tool.run({'workspace_id': 'ws-1', 'agent_id': 7});
      expect(result.isError, isTrue);
    });

    test('returns error when workspace_id is double', () async {
      final result = await tool.run({'workspace_id': 3.14, 'agent_id': 'a-1'});
      expect(result.isError, isTrue);
    });
    ...
    test('returns error when workspace_id is null', () async {
      final result = await tool.run({'workspace_id': null, 'agent_id': 'a-1'});
      expect(result.isError, isTrue);
    });

    test('returns error when agent_id is null', () async {
      final result = await tool.run({'workspace_id': 'ws-1', 'agent_id': null});
      expect(result.isError, isTrue);
    });
    ...
    test(
      'empty string workspace_id is valid and returns null content',
      () async {
        final result = await tool.run({'workspace_id': '', 'agent_id': 'a-1'});
        expect(result.isError, isFalse);
        expect(result.content.first.text, contains('"content":null'));
        expect(result.content.first.text, contains('No working memory found'));
      },
    ...
    test('empty string agent_id is valid and returns null content', () async {
      final result = await tool.run({'workspace_id': 'ws-1', 'agent_id': ''});
      expect(result.isError, isFalse);
      expect(result.content.first.text, contains('"content":null'));
      expect(result.content.first.text, contains('No working memory found'));
    });
    ...
    test('both empty strings returns null content', () async {
      final result = await tool.run({'workspace_id': '', 'agent_id': ''});
      expect(result.isError, isFalse);
      expect(result.content.first.text, contains('"content":null'));
      expect(result.content.first.text, contains('No working memory found'));
    });
