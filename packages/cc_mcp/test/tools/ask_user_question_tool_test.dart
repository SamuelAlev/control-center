  ...
  group('AskUserQuestionTool', () {
    ...
    group('metadata', () {
      ...
      test('has valid inputSchema', () {
        ...
        expect(schema['required'], containsAll(['channel_id', 'question']));
        ...
        expect(
          (props['allow_freeform'] as Map<String, dynamic>)['type'],
          'boolean',
        );
        expect(
          (props['multi_select'] as Map<String, dynamic>)['type'],
          'boolean',
        );
    ...
    group('options parsing', () {
      test('parses valid options with label only', () async {
        port.nextAnswer = const AgentQuestionAnswer(selectedLabels: ['Yes']);
        await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'Proceed?',
      ...
      test('parses options with label, description and value', () async {
        ...
        await tool.call({
          ...
          'question': 'Proceed?',
          'options': [
            {'label': 'Yes', 'description': 'Continue', 'value': 'y'},
          ],
        });
      ...
      test('treats missing options as empty list', () async {
        port.nextAnswer = const AgentQuestionAnswer(freeText: 'my answer');
        await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'What?',
    ...
    group('allow_freeform / options mutual exclusivity', () {
      test(
        'returns error when no options and allow_freeform is false',
        () async {
          final result = await tool.call({
            'workspace_id': 'ws-1',
            'channel_id': 'ch-1',
      ...
      test('succeeds with allow_freeform true and no options', () async {
        port.nextAnswer = const AgentQuestionAnswer(freeText: 'something');
        final result = await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'What?',
    ...
    group('request forwarding', () {
      ...
      test('passes context through', () async {
        ...
        expect(port.lastRequest!.context, 'We need this for the auth flow');
      });
      ...
      test('passes multi_select flag', () async {
        port.nextAnswer = const AgentQuestionAnswer(selectedLabels: ['A', 'B']);
        await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
      ...
      test('defaults multi_select to false', () async {
        port.nextAnswer = const AgentQuestionAnswer(selectedLabels: ['A']);
        await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'Select one',
    ...
    group('edge cases', () {
      test('works with options that have label as only property', () async {
        port.nextAnswer = const AgentQuestionAnswer(selectedLabels: ['Simple']);
        final result = await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'Simple?',
      ...
      test('question text included in success message', () async {
        ...
        expect(result.content.first.text, contains('"Are you sure?"'));
      });

      test('passes empty options list when options is not a List', () async {
        port.nextAnswer = const AgentQuestionAnswer(freeText: 'answer');
        await tool.call({
          'workspace_id': 'ws-1',
          'channel_id': 'ch-1',
          'question': 'What?',
