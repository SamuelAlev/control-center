...
class _FakeIsolatedRepoRepository implements IsolatedRepoRepository {
  ...
  void add(IsolatedRepo r) =>
      _byKey['${r.workspaceId}|${r.spaceId}|${r.repoId}'] = r;
  ...
  Future<IsolatedRepo?> forUnitRepo(
    ...
    String spaceId,
    String repoId,
  ) async => _byKey['$workspaceId|$spaceId|$repoId'];
...
IsolatedRepo _worktree({
  ...
  String repoId = 'r-1',
  // A worktree lives under the SPACE, shared by all its conversations — the
  // path shape is what makes the scope non-negotiable.
  String path = '/data/ws-1/spaces/space-1/repos/app',
  String branch = 'pr/15361',
}) {
  return IsolatedRepo(
    id: 'iso-$repoId',
  ...
  group('ListReposTool', () {
    ...
    group('scoped callers (agents)', () {
      test('local_path is the isolated worktree, never the original', () async {
        ...
        expect(repo['local_path'], '/data/ws-1/spaces/space-1/repos/app');
        expect(repo['is_isolated_copy'], isTrue);
        expect(repo['branch'], 'pr/15361');
      ...
      test(
        'the worktree is resolved by space_id, not the conversation',
        () async {
          // The regression: `isolated_repos` is keyed by space, and every
          // conversation in a space shares one copy. Looking it up by the
          // conversation id matched nothing, so a space that HAD a checkout was
          // reported as having none — which a PR-review lead read as "no working
          // copy is attached to this review" and consolidated without the diff.
          repoRepo.addRepo('ws-1', _repo());
          isolatedRepo.add(_worktree());
          ...
          final result = await tool.call({
            'workspace_id': 'ws-1',
            'space_id': 'space-1',
            // A conversation the worktree registry has never heard of.
      ...
      test(
        'a conversation-only call still hides the original checkout',
        () async {
          // Hiding originals is a rule about WHO is asking, so it must hold even
          // when the call scope carried no space to resolve a worktree with.
          repoRepo.addRepo('ws-1', _repo());
          isolatedRepo.add(_worktree());
      ...
      test('a worktree from another workspace is not resolved', () async {
        repoRepo.addRepo('ws-1', _repo());
        // Same space id and repo id, but registered under ws-2.
      ...
      test('schema declares both scope ids so the call scope injects '
          'them', () {
        final props = tool.inputSchema['properties'] as Map<String, dynamic>;
        expect(props.containsKey('space_id'), isTrue);
