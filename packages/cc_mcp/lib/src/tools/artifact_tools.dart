...
class ListArtifactsTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final limit = McpTool.clampLimit(arguments, defaultLimit);
...
class ArtifactRunContext {
  ...
  const ArtifactRunContext({this.spaceId, this.conversationId, this.error});

  /// The space owning [conversationId], or null when [error] is set. Carried
  /// separately because a side conversation has its own id: a
  /// message posted with the conversation id as the space id would land in the
  /// wrong stream.
...
Future<ArtifactRunContext> resolveArtifactRunContext({
  ...
  // recorded only a space names no conversation row to write into.
  ...
  return ArtifactRunContext(
    // The dispatch path writes `AgentRunLog.spaceId`, so this resolves to the
    // real space. The fallback is for older run rows written before it did —
    // it is wrong for a thread (a conversation id names no space) and exists
    // only so a legacy run degrades instead of throwing.
    spaceId: run.spaceId ?? conversationId,
    conversationId: conversationId,
  );
}
