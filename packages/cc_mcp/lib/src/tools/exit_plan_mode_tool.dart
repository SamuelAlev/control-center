...
class ExitPlanModeTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    // Approved → consume the gate: flip the conversation out of plan mode so
    // the mode guard now permits execution tools.
    if (latest != null && latest.status == ApprovalStatus.approved) {
      await _messaging.setChannelMode(workspaceId, conversationId, Mode.chat);
