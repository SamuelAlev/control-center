...
class SubmitOutputTool extends McpTool {
  ...
  Map<String, dynamic> get inputSchema => {
    ...
    'properties': {
      ...
      'output': {
        'type': 'object',
        'description':
            'Output payload as a JSON object. Required (and '
  ...
  Future<_Outcome> _enforceAndStore(
    ...
    final violations = output == null
        ? <String>[
            'output is required: this run declares an expected output schema',
          ]
    ...
    if (rejections >= _maxContractRejections) {
      await _runLogs.upsert(
        run.copyWith(
          outputJson: null,
          summary:
              'Output failed the expected schema $rejections times:\n$detail',
        ),
      );
    ...
    return _Outcome(
      accepted: false,
      message:
          'Output does not match the expected output schema:\n$detail\n'
