...
abstract class RigUseTool extends McpTool {
  ...
  /// The extra schema this surface contributes beyond the action vocabulary.
  ///
  /// Empty for two of the three: only the browser has anything to choose
  /// before the machine exists.
  Map<String, dynamic> get openSchema => const {};

  /// Reads any surface-specific open options out of an untrusted argument
  /// map, or returns the reason they are invalid.
  ///
  /// Applied only when a rig is actually OPENED — naming an engine while
  /// addressing an existing `rig_id` is answered by that rig's own engine,
  /// which is the honest reading of "act on this machine".
  ({RigBrowserEngine engine, String? error}) parseOpenOptions(
    Map<String, dynamic> arguments,
  ) => (engine: RigBrowserEngine.fallback, error: null);
    ...
    'properties': {
      ...
      'rig_id': {
        ...
        'description':
            ...
            'space\'s rig for this surface.',
      },
      // `space_id`, NOT `conversation_id`: a rig is scoped to the SPACE, whose
      // worktree it syncs, and the human's rig tab is opened with the space
      // id. Declaring the conversation here would have the transport inject
      // the run's conversation uuid instead — a different key, so the agent
      // and the person watching would drive two different machines.
      'space_id': {
        'type': 'string',
        'description':
            'Space the rig belongs to, so repeated calls reuse one machine '
            'instead of booting a new one each time. Filled in automatically.',
      },
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final options = parseOpenOptions(arguments);
    if (options.error != null) {
      return CallResult.error(options.error!);
    }
    ...
    try {
      rig = await _resolveRig(
        ...
        conversationId: _optString(arguments['space_id']),
    ...
    if (conversationId != null && conversationId.isNotEmpty) {
      ...
      for (final candidate in open) {
        if (candidate.conversationId == conversationId &&
            ...
            // The conversation's DEFAULT machine only. A person can open a
            // second machine of the same surface and engine to compare two
            // things side by side; latching onto one of those would have the
            // agent act inside a window someone is reading, and which of them
            // it picked would depend on list order. Addressing one explicitly
            // is what `rig_id` is for.
            candidate.spec.slotId == null &&
            // A conversation can hold one rig per ENGINE, and reusing the
            // wrong one answers "does this work in Firefox?" with Chromium.
            (surface != RigSurface.browser ||
                candidate.spec.browserEngine == engine) &&
...
/// `browser_use` — drive a headless browser inside an enclosed VM.
class BrowserUseTool extends RigUseTool {
  ...
  @override
  String get description =>
      'Drives a headless browser inside an enclosed VM: navigate, reload, go '
      ...
      'take screenshots. Chromium, Firefox and WebKit are all available; pass '
      '"engine" to check a page in a specific one, and open a second rig with '
      'a different engine to compare them side by side. Prefer extract '
  ...
  @override
  Map<String, dynamic> get openSchema => {
    'engine': {
      'type': 'string',
      'enum': [for (final e in RigBrowserEngine.values) e.wire],
      'description':
          'Which browser to drive. Chromium by default. You get the '
          'conversation\'s machine for that engine, so naming a different '
          'engine opens a second machine rather than switching the first — '
          'which is what makes a cross-browser comparison possible. A person '
          'may have opened further machines of the same engine to compare two '
          'things; those are theirs and are reached only by passing their '
          'rig_id. Ignored when rig_id names an existing rig: that rig already '
          'has an engine.',
    },
  };
  ...
  ({RigBrowserEngine engine, String? error}) parseOpenOptions(
    ...
    final raw = arguments['engine'];
    if (raw == null) {
      return (engine: RigBrowserEngine.fallback, error: null);
    }
    if (raw is! String) {
      return (
        engine: RigBrowserEngine.fallback,
        error: 'Invalid argument: engine (expected a string)',
      );
    }
    final parsed = RigBrowserEngine.fromWire(raw);
    if (parsed == null) {
      // Named, not defaulted. A model that asked for Safari and silently got
      // Chromium would report a compatibility result about the wrong engine.
      return (
        engine: RigBrowserEngine.fallback,
        error:
            'Unknown browser engine "$raw". Use '
            '${RigBrowserEngine.values.map((e) => e.wire).join(', ')}.',
      );
    }
    return (engine: parsed, error: null);
  }
          ...
          for (final rig in visible)
            {
              ...
              if (rig.conversationId != null) 'space_id': rig.conversationId,
