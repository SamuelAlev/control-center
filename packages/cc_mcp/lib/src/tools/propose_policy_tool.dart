...
/// MCP tool that stores a policy in shared workspace memory.
class ProposePolicyTool extends McpTool {
  ...
  String get description =>
      ...
      'Pass `repo` when the rule governs ONE codebase; omit it for a rule '
      'that binds the whole workspace.\n\n'
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final sourceFactIds =
        (arguments['source_fact_ids'] as List?)
            ?.map((e) => e.toString())
            .toList() ??
        [];
    final agentRole =
    ...
    try {
      ...
      return CallResult.success(
        jsonEncode({
          'policy_id': policy.id,
          'domain': domain.name,
          'domain_name': MemoryDomainScope.bareName(domain.name),
          'repo': MemoryDomainScope.repoSlugOf(domain.name),
          'status': 'created',
        }),
      );
