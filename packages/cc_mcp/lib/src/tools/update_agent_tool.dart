...
class UpdateAgentTool extends McpTool {
  ...
  Map<String, dynamic> get inputSchema => {
    ...
    'properties': {
      ...
      'model': {'type': 'string', 'description': 'Updated model ID.'},
      'skills': {
        'type': 'array',
        'items': {'type': 'string'},
      ...
      'agent_md_content': {
        'type': 'string',
        'description': "Updated content for the agent's AGENTS.md file.",
      },
    },
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final adapter = arguments.containsKey('adapter')
        ? (rawAdapter is String ? rawAdapter : null)
        : existing.adapterId;
    final model = arguments.containsKey('model')
        ? (rawModel is String ? rawModel : null)
        : existing.modelId;
    final reportsTo = arguments.containsKey('reports_to')
        ? (rawReportsTo is String ? rawReportsTo : null)
        : existing.reportsTo;
    final persona = arguments.containsKey('persona')
    ...
    final agentMdContent = rawAgentMdContent is String
        ? rawAgentMdContent
        : null;

    if (agentMdContent != null) {
      final slug = existing.agentMdPath.split('/').last.replaceAll('.md', '');
      await _filesystem.writeAgentFile(
