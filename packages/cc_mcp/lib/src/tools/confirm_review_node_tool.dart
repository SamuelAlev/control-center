...
/// `confirmedBy`, updates the message metadata and posts a confirmation
...
class ConfirmReviewNodeTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    // Space-wide: the node being confirmed was filed by a PEER, in that peer's
    // own stream — looking only in the standing conversation would make every
    // cross-reviewer confirmation report "not found", which is precisely the
    // thing this tool exists to do.
    final messages = await _repository.getSpaceMessages(workspaceId, spaceId);
    ...
    await _repository.sendMessage(
      ...
      senderType: 'agent',
      messageType: 'system',
      // Link to the review node via metadata (threading was removed).
      metadata: {'reviewNodeId': nodeMessageId},
    );
