...
/// MCP tool to list all RSS feeds. Newsfeed is per-user (global tables, no
...
class ListFeedsTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final feeds = await _repository.watchFeeds(_userId).first;
...
class ListArticlesTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    if (savedOnly) {
      articles = await _repository.watchSavedArticles(_userId).first;
    } else {
      // Fetch a wider window so post-filters (feed/unread) still have room.
      articles = await _repository.watchArticles(_userId, limit: 200).first;
      if (unreadOnly) {
...
class GetArticleTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final article = await _repository.getArticleById(_userId, rawId);
    if (article == null) {
...
class SetArticleReadTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    await _repository.setArticleRead(_userId, rawId, read: read);
    return CallResult.success(
...
class SetArticleSavedTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    await _repository.setArticleSaved(_userId, rawId, saved: saved);
    return CallResult.success(
...
class RefreshFeedsTool extends McpTool {
  ...
  @override
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    await _repository.refreshAll(_userId);
