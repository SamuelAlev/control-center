...
class ProposeFactTool extends McpTool {
  ...
  String get description =>
      ...
      'Pass `repo` when the fact is true of ONE codebase; omit it when the '
      'fact holds workspace-wide. The two are separate scopes: "auth-flow" '
      'for a repo and "auth-flow" for the workspace do not overwrite each '
      'other.\n\n'
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final agentRole =
        AgentRole.tryParse(arguments['agent_role'] as String?) ??
        AgentRole.general;
    ...
    CcMcpLog.d(
      'propose_fact',
      'recording workspace=$workspaceId domain="$domainInput" '
          'repo=${repoSlug ?? '(workspace)'} role=${agentRole.name}',
    );
    ...
    return CallResult.success(
      jsonEncode({
        ...
        // The stored slug plus its parts, so a follow-up call can address the
        // same scope without having to know the prefix format.
        'domain': result.fact.domain,
        'domain_name': MemoryDomainScope.bareName(result.fact.domain),
        'repo': MemoryDomainScope.repoSlugOf(result.fact.domain),
        'topic': topic,
