...
class DismissReviewNodeTool extends McpTool {
  ...
  Future<CallResult> run(Map<String, dynamic> arguments) async {
    ...
    final messages = await _repository.getMessages(workspaceId, channelId);
  ...
  Future<bool> _recordSuppression({
    ...
    try {
      // A suppression fact is only meaningful for a real review channel, so an
      // unassociated channel records nothing.
      final association = await reviewChannels
      ...
      final content = StringBuffer()
        ..writeln(
          'A reviewer dismissed a finding — do not re-flag this '
      ...
      CcMcpLog.e(
        'dismiss_review_node',
        'failed to record suppression fact',
        e,
        st,
      );
      return false;
