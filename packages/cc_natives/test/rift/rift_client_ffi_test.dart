...
void main() {
  ...
  if (RiftFfiBindings.tryLoad(explicitPaths: candidates) == null) {
    // On a developer machine an absent dylib is a broken tree and must FAIL
    // loudly. A CI runner that never ran scripts/natives/build_rift.sh is not
    // a broken tree — it is an un-staged one (the same split
    // `skipServerBootWithoutNatives` makes for the server-boot suites), so
    // skip there instead of failing the job.
    final inCi =
        Platform.environment['CI'] == 'true' ||
        Platform.environment['GITHUB_ACTIONS'] == 'true';
    test(
      'librift_ffi is built',
      () {
        fail(
      ...
      skip: inCi
          ? 'natives not staged on this CI runner (run '
              'scripts/natives/build_rift.sh to run this suite)'
          : false,
    );
    return;
  }
