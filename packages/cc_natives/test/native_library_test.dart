  ...
  group('nativeLibraryCandidates', () {
    test('includes the app-support (data) dir entry for libccpty', () {
      ...
      final expected = p.join(
        dataDir,
        platformLibraryFileName(ptyLibraryBaseName),
      );
      expect(
        candidates,
        contains(expected),
        reason:
            'the PTY loader must search <dataDir>/${platformLibraryFileName(ptyLibraryBaseName)}',
      );
    });
    ...
    test('adds the data-dir entry only when a root is supplied', () {
      ...
      final with_ = nativeLibraryCandidates(
        ptyLibraryBaseName,
        appSupportRoot: dataDir,
      );
    ...
    test('includes the data-dir entry for libcc_watcher', () {
      // Same wiring as libccpty: `cc_server_runtime` points
      // `NativeDirectoryWatcher.libraryResolver` at this call, and losing the
      // data-dir entry silently regresses code-graph watching to
      // package:watcher's full-tree scan per checkout.
      const dataDir = '/srv/cc_server/data';
      final candidates = nativeLibraryCandidates(
      ...
      expect(
        candidates,
        contains(
          p.join(dataDir, platformLibraryFileName(watcherLibraryBaseName)),
        ),
      );
    });
  });
