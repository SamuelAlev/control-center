...
/// — the strongest validation possible without real acoustic hardware.
///
/// Fails (does not skip) when the library hasn't been built locally:
/// libaec_ffi is a REQUIRED native. CI runners do not build natives and skip.
void main() {
  final candidates = devNativeCandidates('aec_ffi', envVar: 'AEC_FFI_DYLIB');

  if (AecFfiBindings.tryLoad(explicitPaths: candidates) == null) {
    test('libaec_ffi is built', () {
      fail(
        'libaec_ffi not loadable — run scripts/natives/build_aec.sh (on '
        'Windows scripts/release/windows_natives.sh). It is a REQUIRED '
        'native.',
      );
    }, skip: skipIfMissingInCi(false, 'libaec_ffi is not built on CI runners'));
    return;
  }

  AecProcessor open() {
    try {
      return AecProcessor.create(explicitPaths: candidates);
    } on AecUnavailable catch (e) {
      fail(
        'libaec_ffi not loadable — run scripts/natives/build_aec.sh (on '
        'Windows scripts/release/windows_natives.sh). It is a REQUIRED '
        'native: $e',
  ...
  test('loads the native lib and processes one block', () {
    final proc = open();
    try {
  ...
  test('cancels a synthetic delayed echo (ERLE proof)', () {
    final proc = open();
    ...
    try {
      ...
      expect(
        cleanedAvg,
        lessThan(nearAvg * 0.5),
        reason:
            'AEC3 did not attenuate the echo (near=$nearAvg '
            'cleaned=$cleanedAvg)',
      );
