...
void main() {
  group('nativeLibraryCandidates', () {
    test('orders env override → app-support → bundled', () {
      ...
      // p.join, not '$dir/$file': the candidates are platform-separator
      // paths, so on Windows the literal '/' expectation never matched.
      expect(candidates.first, p.join('/data/app', fileName));
      ...
      expect(candidates.any((c) => c.contains('macos/Frameworks')), isFalse);
    });
  });
  ...
  group('bundledLibraryCandidates', () {
    test('follows the platform packaging convention', () {
      ...
      // The `dart build cli` bundle `lib/` dir (where the cc_server build
      // hook drops the natives beside libsqlite3) is tried first everywhere.
      expect(
        candidates.first,
        p.join(dartBuildBundleLibDir(), platformLibraryFileName('tree-sitter')),
      );
      if (Platform.isMacOS) {
        expect(candidates.sublist(1), <String>[
        ...
        expect(candidates[1], endsWith('/lib/libtree-sitter.so'));
