...
import 'package:cc_natives/src/inference/embedding/onnx_runtime_loader.dart';
...
class TextEmbedder {
  ...
  static Future<TextEmbedder> load({
    ...
    // Make onnxruntime resolvable on a pure-Dart host BEFORE `OrtEnv` triggers
    // onnxruntime_v2's bare leaf-name open (which a hardened dart-built binary
    // rejects). Best-effort: a no-op on the desktop, where the CocoaPods
    // framework is already loaded. See [ensureOnnxRuntimeLoaded].
