...
class MeetingOfflineVad {
  ...
  ///
  /// **[samples] is CONSUMED**: the buffer is MOVED to the worker rather than
  /// copied (a whole recording is hundreds of megabytes), which detaches it
  /// here. Read what you need from it before calling.
  ///
  ...
  Future<List<Span>> detect({
    ...
    // Moved, not copied — same reasoning as the diarizer: capturing the buffer
    // in the worker closure duplicated hundreds of megabytes for a whole
    // recording. This DETACHES [samples] from the calling isolate.
    final transferable = TransferableTypedData.fromList([samples]);
    final raw = await Isolate.run(
      () => _detectSync(transferable, modelPath, sampleRate, resolved),
    );
...
List<List<int>> _detectSync(
  ...
  final samples = transferredSamples.materialize().asFloat32List();
