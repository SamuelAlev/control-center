...
///
/// [ensureSherpaInitialized] (called inside the worker) wires up the native
/// library for that isolate; the `sherpa_onnx_macos`/`_linux`/`_windows` plugin
/// packages bundle the platform dylib into the process, so it resolves from any
/// isolate — but the binding lookup is per-isolate, hence the dedicated call
/// here rather than relying on the main isolate's init.
class SherpaOnnxTranscriber implements SpeechTranscriber {
  /// Creates a [SherpaOnnxTranscriber].
  ///
  /// [libDir], when given, is the directory holding the sherpa/onnx dylibs; the
  /// decode worker (a separate isolate that can't see the host's preferred-dir
  /// static) loads the native library from there. A pure-Dart host resolves it
  /// with `resolveSherpaLibraryDir`; the Flutter app leaves it null (the plugin
  /// bundles the dylib on the loader path).
  SherpaOnnxTranscriber({required this.paths, this.libDir});

  /// Resolved on-disk paths to the installed model files.
  final VoiceModelPaths paths;

  /// Directory holding the sherpa/onnx dylibs, forwarded to the decode worker;
  /// null = bare loader-path load (the Flutter-app case).
  final String? libDir;
  ...
  Future<void> _spawnWorker(Completer<void> ready) async {
    try {
      ...
      commands.send(<String, Object?>{
        ...
        // Where the worker isolate loads the sherpa/onnx dylibs from (a pure-Dart
        // host has no plugin bundling; null = bare loader-path load).
        'libDir': libDir,
      });
...
void _whisperWorkerMain(SendPort toMain) {
  ...
  commands.listen((Object? message) {
    ...
    switch (msg['type']) {
      case 'init':
        try {
          ensureSherpaInitialized(msg['libDir'] as String?);
          ...
          final model = modelType == 'transducer'
              ? sherpa.OfflineModelConfig(
                  transducer: sherpa.OfflineTransducerModelConfig(
                    encoder: msg['encoder']! as String,
                    decoder: msg['decoder']! as String,
                    joiner: (msg['joiner'] as String?) ?? '',
                  ),
                  tokens: tokens,
                  // Leave model_type empty so sherpa-onnx auto-routes from the
                  // encoder's `model_type` metadata: NeMo Parakeet encoders
                  // (`EncDecRNNTBPEModel` / `EncDecHybridRNNTCTCBPEModel`) take
                  // the NeMo recognizer, while Zipformer/conformer encoders take
                  // the k2 one. Hardcoding `'transducer'` forced the k2 path,
                  // whose decoder init reads `vocab_size` from the *decoder*
                  // ONNX — which NeMo models carry on the *encoder* instead — so
                  // it crashed with `'vocab_size' does not exist in the metadata`
                  // (offline-transducer-model.cc:InitDecoder) and tore down the
                  // recognizer for every Parakeet model.
                  modelType: '',
                  numThreads: 4,
                  provider: 'cpu',
                  debug: false,
              ...
              : sherpa.OfflineModelConfig(
                  whisper: sherpa.OfflineWhisperModelConfig(
                    encoder: msg['encoder']! as String,
                    decoder: msg['decoder']! as String,
                    // Empty language → Whisper auto-detects (multilingual
                    // models); 'en' pins english-only models like base.en.
