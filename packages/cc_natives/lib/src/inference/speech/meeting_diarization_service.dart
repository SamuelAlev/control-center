...
class MeetingDiarizationService implements MeetingDiarizationPort {
  ...
  Future<DiarizationResult> diarize({
    ...
    // Zero-copy hand-off. Capturing `samples` in the worker closure made
    // `Isolate.run` COPY the whole buffer: a 2 h 16 kHz mono recording is
    // ~460 MB of Float32, and it was live three times over at the peak — once
    // in the host, once in the worker, once again in native memory.
    // `TransferableTypedData` moves the buffer instead of copying it, which
    // DETACHES it from this isolate (see the port contract).
