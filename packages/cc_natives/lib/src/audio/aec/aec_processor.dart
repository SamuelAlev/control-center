...
class AecProcessor implements AecEngine {
  ...
  /// Loads the native AEC library and creates an instance.
  ///
  /// Throws [AecUnavailable] when the dylib cannot be loaded (broken install)
  /// or when the engine refuses the requested format. There is no degraded
  /// mode — see the class doc.
  static AecProcessor create({
    ...
    if (bindings == null) {
      throw const AecUnavailable('libaec_ffi could not be loaded');
    }
  ...
  void processReverse(Uint8List block) {
    assert(
      ...
      'reverse block must be $blockBytes bytes',
    );
    if (_disposed) {
      return;
    }
  ...
  Uint8List processCapture(Uint8List block, int streamDelayMs) {
    assert(
      block.length == blockBytes,
      'capture block must be $blockBytes bytes',
    );
    if (_disposed) {
      return block;
    }
