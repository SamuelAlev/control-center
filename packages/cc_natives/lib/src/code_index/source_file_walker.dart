...
class SourceFileWalker {
  ...
  Future<List<HashedSourceFile>> hashPaths(
    ...
    return Isolate.run(() async {
      ...
      for (final relative in relativePaths) {
        // Callers hand POSIX-separated repo-relative paths (the watch service
        // normalizes Windows separators before it reports them); rebuild a
        // platform-native one so what this returns is byte-identical to what
        // [walk] would have produced and keeps matching the stored
        // `code_files.path`.
        final relativePath = p.joinAll(
          relative.replaceAll(r'\', '/').split('/'),
        );
        if (!seen.add(relativePath)) {
          continue;
        }
        if (!walker._isIndexable(p.basename(relativePath))) {
          continue;
        }
        candidates.add(relativePath);
      }
      if (candidates.isEmpty) {
        return const <HashedSourceFile>[];
      }
      final ignored = await walker._gitIgnoredPaths(rootPath, candidates);
      final out = <HashedSourceFile>[];
      for (final relativePath in candidates) {
        final excluded = ignored == null
            ? _isUnderSkippedDir(relativePath)
            : ignored.contains(relativePath);
        if (excluded) {
          continue;
        }
        final absolutePath = p.normalize(p.join(rootPath, relativePath));
        try {
          ...
          out.add(
            HashedSourceFile(
              ...
              contentHash: await walker.hashFile(absolutePath),
            ),
          );
        } on FileSystemException {
          // Vanished or unreadable between the check and the read — same
          // tolerance [walkAndHash] has for a racing checkout.
          continue;
        }
      }
      return out;
  ...
  /// Which of [relativePaths] git would exclude from [walk], in ONE spawn.
  ///
  /// Null when [rootPath] is not a git work tree or git is unavailable, which
  /// is the same signal [_gitListedPaths] gives and sends the caller to the
  /// manual walk's [_skipDirs] instead.
  ///
  /// Deliberately WITHOUT `--no-index`: git does not report a TRACKED file as
  /// ignored even when it matches an ignore rule, which is precisely the
  /// semantics of the `ls-files --cached --others --exclude-standard`
  /// enumeration this mirrors. Adding the flag would make a targeted run drop
  /// files a full run indexes.
  Future<Set<String>?> _gitIgnoredPaths(
    ...
    Process process;
    try {
      process = await Process.start('git', const [
        'check-ignore',
        '-z',
        '--stdin',
      ], workingDirectory: rootPath);
    ...
    // Both pipes are consumed BEFORE stdin is written: git echoes matches as it
    // reads, and a full stdout pipe with nobody draining it would deadlock the
    // write below.
    final stdoutText = process.stdout.transform(utf8.decoder).join();
    final stderrDrained = process.stderr.drain<void>();
    try {
      process.stdin.write(
        relativePaths.map((path) => path.replaceAll(r'\', '/')).join('\u0000'),
      );
    } catch (_) {
      // git can exit 128 ("not a work tree") before reading stdin; the exit
      // code below turns that into a null (manual-walk) answer.
    }
    unawaited(process.stdin.close().catchError((Object _) {}));
    final output = await stdoutText;
    await stderrDrained;
    final exitCode = await process.exitCode;
    // 0 = something matched, 1 = nothing matched. Anything else (128: not a
    // work tree, bad invocation) is not an answer and must not read as "none
  ...
  /// Whether [relativePath] sits under one of the manual walk's skipped
  /// directories — the non-git fallback for [_gitIgnoredPaths].
  static bool _isUnderSkippedDir(String relativePath) {
    for (final segment in p.split(relativePath.replaceAll(r'\', '/'))) {
      if (_skipDirs.contains(segment)) {
        return true;
      }
    }
    return false;
