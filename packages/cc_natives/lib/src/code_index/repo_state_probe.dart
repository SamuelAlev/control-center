...
class RepoStateProbe {
  ...
  Future<RepoStateFingerprint?> probe(String repoPath) async {
    ...
    // Fold a stat of every dirty path into the digest. Porcelain `-z` emits
    // `XY path` records; a rename (`R`/`C` in X) emits the destination in the
    // record and the SOURCE as the following NUL-separated field, with no
    // status prefix. Treating every field as a path (prefixed fields stripped
    // of their 3-char `XY ` header) covers both.
