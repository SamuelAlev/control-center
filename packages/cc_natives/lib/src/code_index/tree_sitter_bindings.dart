...
typedef _NodeTypeNative = Pointer<Char> Function(TSNode);
typedef _NodeCountNative = Uint32 Function(TSNode);
typedef _NodeChildNative = TSNode Function(TSNode, Uint32);
typedef _NodeFlagNative = Bool Function(TSNode);
typedef _NodeFieldNameNative = Pointer<Char> Function(TSNode, Uint32);
...
typedef NodeType = Pointer<Char> Function(TSNode);
typedef NodeCount = int Function(TSNode);
typedef NodeChild = TSNode Function(TSNode, int);
typedef NodeFlag = bool Function(TSNode);
typedef NodeFieldName = Pointer<Char> Function(TSNode, int);
typedef QueryNew =
    Pointer<TSQuery> Function(
      Pointer<TSLanguage>,
      Pointer<Char>,
...
class TreeSitterBindings {
  TreeSitterBindings(DynamicLibrary runtime)
    : parserNew = runtime.lookupFunction<_ParserNewNative, ParserNew>(
        'ts_parser_new',
      ),
      parserDelete = runtime.lookupFunction<_ParserDeleteNative, ParserDelete>(
        'ts_parser_delete',
      ),
      ...
      treeDelete = runtime.lookupFunction<_TreeDeleteNative, TreeDelete>(
        'ts_tree_delete',
      ),
      treeRootNode = runtime.lookupFunction<_TreeRootNodeNative, TreeRootNode>(
        'ts_tree_root_node',
      ),
      nodeStartByte = runtime.lookupFunction<_NodeByteNative, NodeByte>(
        'ts_node_start_byte',
      ),
      nodeEndByte = runtime.lookupFunction<_NodeByteNative, NodeByte>(
        'ts_node_end_byte',
      ),
      nodeStartPoint = runtime.lookupFunction<_NodePointNative, NodePoint>(
        'ts_node_start_point',
      ),
      nodeEndPoint = runtime.lookupFunction<_NodePointNative, NodePoint>(
        'ts_node_end_point',
      ),
      nodeType = runtime.lookupFunction<_NodeTypeNative, NodeType>(
        'ts_node_type',
      ),
      nodeChildCount = runtime.lookupFunction<_NodeCountNative, NodeCount>(
        'ts_node_child_count',
      ),
      nodeChild = runtime.lookupFunction<_NodeChildNative, NodeChild>(
        'ts_node_child',
      ),
      nodeNamedChildCount = runtime.lookupFunction<_NodeCountNative, NodeCount>(
        'ts_node_named_child_count',
      ),
      nodeNamedChild = runtime.lookupFunction<_NodeChildNative, NodeChild>(
        'ts_node_named_child',
      ),
      nodeIsNamed = runtime.lookupFunction<_NodeFlagNative, NodeFlag>(
        'ts_node_is_named',
      ),
      nodeIsMissing = runtime.lookupFunction<_NodeFlagNative, NodeFlag>(
        'ts_node_is_missing',
      ),
      nodeFieldNameForChild = runtime
          .lookupFunction<_NodeFieldNameNative, NodeFieldName>(
            'ts_node_field_name_for_child',
          ),
      queryNew = runtime.lookupFunction<_QueryNewNative, QueryNew>(
        'ts_query_new',
      ),
      queryDelete = runtime.lookupFunction<_QueryDeleteNative, QueryDelete>(
        'ts_query_delete',
      ),
  ...
  final NodeCount nodeChildCount;
  final NodeChild nodeChild;
  final NodeCount nodeNamedChildCount;
  final NodeChild nodeNamedChild;
  final NodeFlag nodeIsNamed;
  final NodeFlag nodeIsMissing;

  /// The field name the parent gave child `i` (`ts_node_field_name_for_child`),
  /// or null when the child is positional.
  ///
  /// Load-bearing for structural matching: `a[b]` and `a(b)` can share a node
  /// type in some grammars, and the field is what distinguishes an argument
  /// from an index.
  final NodeFieldName nodeFieldNameForChild;
