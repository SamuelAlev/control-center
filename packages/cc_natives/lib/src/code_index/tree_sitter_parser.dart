...
class TreeSitterParser {
  ...
  /// `(query address, capture index) → capture name`.
  ///
  /// Capture names are per-QUERY constants — `@name`, `@type`, `@doc` — but
  /// resolving one used to malloc a length cell, call into the native, decode
  /// a string and free the cell FOR EVERY CAPTURE of every match of every
  /// file. Compiled queries are cached and long-lived, so their names can be
  /// too; this removes thousands of allocations per file from the extraction
  /// worker's inner loop.
  final Map<int, Map<int, String>> _captureNames = {};

  /// The name of [captureIndex] in [query], resolved once per query.
  String _captureName(
    TreeSitterBindings bindings,
    Pointer<TSQuery> query,
    int captureIndex,
  ) {
    final perQuery = _captureNames.putIfAbsent(query.address, () => {});
    final cached = perQuery[captureIndex];
    if (cached != null) {
      return cached;
    }
    ...
    try {
      ...
      return perQuery[captureIndex] = name;
    } finally {
      ...
      for (final query in queries) {
        ...
        while (bindings.queryCursorNextMatch(_cursor, matchPtr)) {
          ...
          for (var i = 0; i < match.captureCount; i++) {
            ...
            final name = _captureName(bindings, query, capture.index);
  ...
  /// Parses [source] and copies the whole syntax tree out as [AstNode]s.
  ///
  /// Depth-capped rather than unbounded: a deeply nested expression (a long
  /// chain of `a + b + c + …` is left-recursive in most grammars) would
  /// otherwise recurse as deeply as the file is long, and a stack overflow in
  /// an FFI callback is not a catchable error. Nodes at the cap keep their
  /// span and text and simply report no children, so a matcher below the cap
  /// finds nothing rather than crashing.
  ///
  /// Returns null when the source does not parse to a tree at all. A tree
  /// containing `ERROR` nodes is NOT null — tree-sitter is error-tolerant by
  /// design, and a pattern fragment ("`foo($X)`" with no trailing semicolon)
  /// is exactly the case that produces one while still carrying the structure
  ...
  AstNode _copyNode(
    ...
    if (depthLeft > 0) {
      final count = bindings.nodeChildCount(node);
      for (var i = 0; i < count; i++) {
        final child = bindings.nodeChild(node, i);
        // A MISSING node is one the parser inserted to recover; it spans zero
        // bytes and matching against it would assert about text nobody wrote.
        if (bindings.nodeIsMissing(child)) {
          continue;
        }
        final fieldPtr = bindings.nodeFieldNameForChild(node, i);
        children.add(
          _copyNode(
            bindings,
            child,
            srcBytes,
            fieldPtr == nullptr ? null : fieldPtr.cast<Utf8>().toDartString(),
            depthLeft - 1,
          ),
        );
      }
    ...
    return AstNode(
      type: bindings.nodeType(node).cast<Utf8>().toDartString(),
      isNamed: bindings.nodeIsNamed(node),
      startByte: startByte,
      endByte: endByte,
      startLine: startPoint.row + 1,
      endLine: endPoint.row + 1,
      text: (startByte <= endByte && endByte <= srcBytes.length)
          ? utf8.decode(
              srcBytes.sublist(startByte, endByte),
              allowMalformed: true,
            )
