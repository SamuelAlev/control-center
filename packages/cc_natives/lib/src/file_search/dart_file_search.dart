...
class DartFileSearch implements FileSearch {
  ...
  Stream<List<FileSearchHit>> search({
    ...
    () async {
      ...
      // Offset slices the RANKED list (after the sort), so consecutive pages
      // are disjoint and their union is the un-paged result set.
      controller.add(
        hits.skip(offset).take(limit).toList(growable: false),
      );
...
/// The repo-relative path in the SAME canonical form the native `fff` engine
/// returns (`/`-separated, matching `FffFileSearch.runSearch`'s Rust output),
/// so the Dart fallback and the native engine are interchangeable downstream.
