...
class _SearchCommand extends _FffCommand {
  const _SearchCommand(super.id, this.roots, this.query, this.limit);
...
Future<void> _fffWorkerMain((SendPort, String?) args) async {
  ...
  await for (final message in commands) {
    ...
    switch (message) {
      ...
      case _SearchCommand(:final roots, :final query, :final limit):
        final b = ensureBindings();
        if (b == null) {
          host.send(_FffReply(message.id, error: 'unavailable'));
          break;
        ...
        try {
          host.send(
            _FffReply(message.id, hits: runSearch(b, roots, query, limit)),
          );
        } on Object catch (e) {
          host.send(_FffReply(message.id, error: '$e'));
        }
...
class _FffWorker {
  ...
  Future<List<FileSearchHit>> search(
    ...
    int limit,
      ...
      await _send((id) => _SearchCommand(id, roots, query, limit)) ?? const [];
...
class FffFileSearch implements FileSearch {
  ...
  Stream<List<FileSearchHit>> search({
    ...
    unawaited(_doSearch(ctrl, roots: roots, query: query, limit: limit));
    return ctrl.stream;
  }
  ...
  Future<void> _doSearch(
    ...
    try {
      ...
      ctrl.add(await worker.search(roots, query, limit));
    } catch (e, st) {
