...
class FffFileSearch implements FileSearch {
  ...
  // Lazily resolved native bindings (null until loaded).
  _FffBindings? _bindings;
  Future<_FffBindings>? _bindingsFuture;

  Future<_FffBindings> _ensureBindings() async {
    final inFlight = _bindingsFuture;
    if (inFlight != null) {
      return inFlight;
    }
    final future = _loadBindings();
    _bindingsFuture = future;
    try {
      return await future;
    } catch (_) {
      // Allow a retry on the next call (e.g. the dylib installed later).
      _bindingsFuture = null;
      rethrow;
    }
  }
  ...
  Future<_FffBindings> _loadBindings() async {
    ...
    if (lib == null) {
      ...
      throw const FffUnavailable();
    }
    try {
      return _bindings = _FffBindings(lib);
    } catch (e) {
      _log?.call('FffFileSearch', 'failed to resolve symbols', e);
      throw const FffUnavailable();
    }
  }

  Future<DynamicLibrary?> _openLib() async {
    // Resolve the host-injected app-support root (where build_fff.sh installs
    // the dev dylib next to control_center.db); null when unavailable, in which
    // case only the bundle-relative release paths are tried.
