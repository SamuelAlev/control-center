...
class RiftClient {
  ...
  /// Whether the native library loaded (probed once on the main isolate).
  ///
  /// False means a BROKEN INSTALL on macOS/Linux — `cc_server` refuses to boot
  /// on it — so callers must propagate rather than degrade. Windows is the one
  /// documented exception: rift has no MSVC CoW backend, so `git worktree` is
  /// the backend there (see `RiftRepoIsolationAdapter.missingRiftIsExpected`).
  bool get isAvailable =>
      _available ??= RiftFfiBindings.tryLoad(explicitPaths: dylibPaths) != null;

  /// Name of the marker file rift writes into a managed source, holding the id
  /// of its entry in the registry named by [databasePath].
  static const markerFileName = '.rift';
  ...
  /// Deletes the `.rift` marker at [at] so the next [init] re-adopts the source
  /// as a fresh registry entry. Returns whether a marker was there to remove.
  ///
  /// ONLY for a marker this registry does not recognise
  /// ([RiftException.isStaleMarker]). A marker whose entry still exists is the
  /// link between a source and its managed copies, so clearing it would orphan
  /// them — rift's own verdict, not a guess, is what makes this safe.
  Future<bool> clearMarker({required String at}) async {
    final marker = File(p.join(at, markerFileName));
  ...
  Future<Object?> _send(Map<String, Object?> request) async {
    ...
    throw RiftException(
      code: 'protocol',
      message: 'malformed rift error: $raw',
    );
  }
