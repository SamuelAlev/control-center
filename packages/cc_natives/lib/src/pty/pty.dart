...
/// absent. Null is a PROBE RESULT, not a licence to degrade — `libccpty` is
/// required, so `Pty.start` turns it into a thrown [PtyUnavailable] and
/// `cc_server` refuses to boot without it.
///
...
/// runtime loader.
///
/// API mirrors `flutter_pty`'s `Pty` so consumers (sandboxed terminal
/// sessions) port with no behavioural change.
class Pty {
  ...
  Pty.start(
    ...
    final workingDirectoryPtr = workingDirectory?.toNativeUtf8();

    final options = calloc<PtyOptions>();
    options.ref
      ..rows = rows
      ..cols = columns
      ..executable = executablePtr.cast()
      ..arguments = argv.cast()
      ..environment = envp.cast()
      ..stdout_port = _stdoutPort.sendPort.nativePort
      ..exit_port = _exitPort.sendPort.nativePort
      ..ackRead = ackRead
      ..working_directory = workingDirectoryPtr?.cast() ?? nullptr;

    _handle = bindings.pty_create(options);
    calloc.free(options);

    // `pty_create` copies everything it needs out of `options` (it has to —
    // the struct itself is freed on the line above), so every string and both
    // vectors are ours to release. Without this a long-lived server leaked a
    // few KB of argv/envp per terminal session, forever.
    ...
    calloc.free(envp);
    malloc.free(executablePtr);
    if (workingDirectoryPtr != null) {
      malloc.free(workingDirectoryPtr);
    }
  ...
  void resize(int rows, int cols) =>
      _ensureBindings().pty_resize(_handle, rows, cols);
  ...
  static PtyBindings _ensureBindings() {
    ...
    if (rc != 0) {
      throw StateError(
        'Failed to initialize Dart Native API for libccpty (rc=$rc)',
      );
    }
...
/// Thrown by [Pty.start] when the `libccpty` native is not present. The
/// native ships inside the host bundle and `cc_server` refuses to boot without
/// it (the native preflight), so hitting this at spawn time means a broken
/// install — the failure propagates to the caller, there is no degraded mode.
/// See [NativeLibraryUnavailable].
class PtyUnavailable implements NativeLibraryUnavailable {
  /// Creates the marker exception.
  const PtyUnavailable();

  @override
  String get message =>
      'libccpty could not be loaded (build it with '
      'scripts/natives/build_pty.sh — on Windows '
      'scripts/release/windows_natives.sh — or set \$$ptyLibraryEnvVar)';

  @override
  String toString() => 'PtyUnavailable: $message.';
}
