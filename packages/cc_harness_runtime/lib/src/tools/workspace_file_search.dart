...
String? _linkPrefixFor(String sharedRoot, String workspaceRoot) {
  ...
  for (final child in children) {
    // Any non-file direct child may be the reach-through: a POSIX symlink
    // (`child is Link`), a Windows junction/reparse point (which does not
    // reliably surface from `listSync` as a `Link`), or a bind of the root
    // itself. Resolve and compare identities — a plain subdirectory can only
    // compare equal by BEING the shared root, so this cannot over-match.
    if (FileSystemEntity.typeSync(child.path, followLinks: false) ==
