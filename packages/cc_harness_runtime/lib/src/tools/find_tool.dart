...
class FindTool extends HarnessTool {
  ...
  Future<HarnessToolResult> execute(
    ...
    // The recursive walk plus a `statSync` per match is seconds of blocked
    // event loop on a monorepo workspace, and this tool is embedded in
    // cc_server. Do all of it on a worker isolate.
    final workspaceRoot = context.workingDirectory;
    final sharedRoots = context.sharedRoots.toList(growable: false);
    final (matches, listError) = await Isolate.run(
      () => _walk(base, workspaceRoot, sharedRoots, pattern),
    );
    if (listError != null) {
      return HarnessToolResult.error('Failed to list $rawPath: $listError');
    }
    if (matches.isEmpty) {
  ...
  /// The walk + glob match + mtime stat, run inside a worker isolate.
  static (List<(String, DateTime)>, String?) _walk(
    String base,
