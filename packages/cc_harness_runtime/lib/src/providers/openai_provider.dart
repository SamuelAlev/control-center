...
class OpenAiProvider implements LlmProviderPort {
  ...
  /// The models this endpoint will actually serve, when `/models` over-reports.
  ///
  /// Null means trust the catalog, which is the normal case. It is set for a
  /// host that answers with the account's whole catalogue but serves a subset:
  /// the z.ai GLM Coding Plan lists all ten GLM models and silently answers as
  /// `glm-5.3` for the eight it does not run, so listing them would offer the
  /// operator a choice the endpoint quietly ignores.
  final Set<String>? _servesOnlyModels;
  ...
  Stream<LlmEvent> complete({
    ...
    try {
      ...
      // z.ai's `1113` ("Insufficient balance or no resource package") arrives
      // as a 429 but is the same hard wall: it is what the PAY-AS-YOU-GO host
      // answers a GLM Coding Plan account, which has a subscription and no
      // balance. Retrying is futile and the fix is a different endpoint, so
      // name it — the plan is reachable only from the `zai-coding` provider.
      final planOnBalanceHost = wireCode == '1113';
      final hardQuota =
          wireCode == 'insufficient_quota' ||
          wireCode == 'billing_not_active' ||
          planOnBalanceHost;
      final codingPlanHint = planOnBalanceHost
          ? ' — a GLM Coding Plan is not billed on this endpoint. Connect the '
                'key under "z.ai GLM Coding Plan" in Settings → Providers '
                'instead, or top up the pay-as-you-go balance.'
          : '';
      yield LlmError(
        '$_providerName API error ${e.statusCode}: '
        '${e.body}$reauth$codingPlanHint',
