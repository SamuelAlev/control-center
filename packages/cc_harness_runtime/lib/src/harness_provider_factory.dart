...
class HarnessProviderFactory {
  ...
  LlmProviderPort create({
    ...
    switch (providerId) {
      case 'anthropic':
        // API key only — see [AnthropicProvider]. A stored OAuth credential
        // from before that policy yields a null key and a provider that 401s
        // with the "update the API key" remedy, which is the honest outcome:
        // silently running on it is the thing that is not allowed.
      ...
      // The z.ai open platform, billed per token against the account balance.
      case 'zai':
        return OpenAiProvider(
          apiKey: apiKey,
      ...
      // The GLM Coding Plan. Same key as `zai`, different host: the plan is
      // served ONLY from `/api/coding/paas/v4` and the pay-as-you-go endpoint
      // refuses a plan-only account with `429 {"code":"1113"}`. The plan
      // serves GLM-5.3 and GLM-5.3-Flash — the host accepts any listed id and
      // answers as `glm-5.3` — so the default names what actually replies.
      case 'zai-coding':
        return OpenAiProvider(
          apiKey: apiKey,
          baseUrl: baseUrl ?? 'https://api.z.ai/api/coding/paas/v4',
          defaultModel: model ?? 'glm-5.3',
          providerName: 'z.ai GLM Coding Plan',
          extractThinkTags: true,
          // `/models` is byte-identical on both z.ai hosts — the coding host
          // reports the whole account catalogue and then answers as `glm-5.3`
          // whatever else you ask for. Listing the other eight would offer a
          // choice the endpoint silently overrides.
