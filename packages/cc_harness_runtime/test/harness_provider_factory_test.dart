...
void main() {
  ...
  group('z.ai', () {
    test('the plain lane is the pay-as-you-go open platform', () {
      final provider = factory.create(
        providerId: 'zai',
        credential: const ProviderCredential(
          providerId: 'zai',
      ...
      expect(provider.defaultModel, 'glm-5.2');
    });

    test('the coding plan is a separate provider on its own host', () {
      // Same account key, different wallet. z.ai bills the subscription only
      // from `/api/coding/paas/v4`; the pay-as-you-go host answers a plan-only
      // account `429 {"code":"1113"}`, so the lanes cannot share one entry.
      final provider = factory.create(
      ...
      expect(provider, isA<OpenAiProvider>());
      expect(provider.displayName, 'z.ai GLM Coding Plan');
      expect(provider.defaultModel, 'glm-5.3');
    });

    test('both lanes price against the same models.dev catalog', () {
      expect(harnessProviderMetas['zai']!.catalogProviderId, 'zhipuai');
      expect(harnessProviderMetas['zai-coding']!.catalogProviderId, 'zhipuai');
    });
  });
