...
void main() {
  ...
  group('resolveInsideWorkspace — symlink hardening', () {
    ...
    test('refuses an INTERMEDIATE symlink directory escape', () {
      // The escape the final-component-only check missed. `ws/esc -> /outside`
      // is a link the agent's own bash can create (the link file is
      // legitimately inside the workspace), and `esc/secret.txt` then passed
      // every check: lexically inside the root, final component a regular
      // file — while the OS resolved it to the outside target.
      final outside = Directory.systemTemp.createTempSync('ws_paths_esc_');
      addTearDown(() => outside.deleteSync(recursive: true));
      File(p.join(outside.path, 'secret.txt')).writeAsStringSync('x');
      Link(p.join(root.path, 'esc')).createSync(outside.path);

      expect(resolveInsideWorkspace(root.path, 'esc/secret.txt'), isNull);
      expect(
        resolveInsideWorkspace(root.path, 'esc/nested/deeper.txt'),
        isNull,
      );
    });

    test('refuses a WRITE through an intermediate symlink (target absent)', () {
      // A create/write targets a path that does not exist yet, so the whole
      // path cannot be canonicalized — but the escaping link is in its parent
      // chain, which can.
      final outside = Directory.systemTemp.createTempSync('ws_paths_esc2_');
      addTearDown(() => outside.deleteSync(recursive: true));
      Link(p.join(root.path, 'out')).createSync(outside.path);

      expect(resolveInsideWorkspace(root.path, 'out/new-file.txt'), isNull);
    });

    test('still allows a path whose parent chain stays inside', () {
      Directory(p.join(root.path, 'nested')).createSync();
      expect(
        resolveInsideWorkspace(root.path, 'nested/not-created-yet.txt'),
        isNotNull,
      );
    });
  ...
  group('listWorkspaceTree', () {
    test(
      'expands a trusted direct-child symlink into the shared root',
      () {
  ...
  group('outsideWorkspaceMessage', () {
    test('names the accessible roots and steers to repos/ when shared', () {
      // The message reports the roots NORMALIZED for the platform
      // (`p.normalize(p.absolute(...))`), so the expectations must be built
      // the same way — literal POSIX strings never appear on Windows.
      ...
      expect(message, contains(p.normalize(p.absolute('/conv/agents/dev'))));
      expect(message, contains(p.normalize(p.absolute('/conv/repos'))));
