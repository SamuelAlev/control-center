...
void main() {
  ...
  group('WebSearchTool.execute', () {
    ...
    test('an HTTP error on every backend reports the whole chain', () async {
      // The fake answers every backend identically, so this is the
      // all-backends-down case. The message must name what was tried and must
      // NOT read as "nothing exists" — that is the wrong conclusion for an
      // agent to draw from an infrastructure failure.
      final tool = WebSearchTool(
        client: _FakeHttpClient(statusCode: 429, body: 'blocked'),
      );
      final res = await tool.execute({'query': 'cats'}, _ctx());
      expect(res.isError, isTrue);
      expect(res.content, contains('duckduckgo'));
      expect(res.content, contains('NOT evidence that nothing exists'));
    });
    ...
    test('empty short body is treated as blocked', () async {
      ...
      final res = await tool.execute({'query': 'cats'}, _ctx());
      expect(res.isError, isTrue);
      expect(res.content, contains('blocked or failed'));
    });
    ...
    test('a network failure on every backend is reported, not swallowed',
        () async {
      final tool = WebSearchTool(
      ...
      expect(
        res.content,
        contains('No search backend could be reached'),
        reason: 'unreachable is distinct from blocked, and both are distinct '
            'from "nothing exists"',
      );
    });
  });
