...
void main() {
  group('HarnessOAuthBroker', () {
    ...
    // The default registry is the one production builds. Anthropic's browser
    // login only ever worked by minting a token against Claude Code's own
    // OAuth client, so it is gone — and `supports` returning false is what
    // makes `oauth.begin` refuse it even from a client that still offers the
    // button. Claude Pro/Max belongs to the `claude-code` ADAPTER instead.
    test('the default registry offers no Anthropic browser login', () {
      final broker = HarnessOAuthBroker(
        store: _RecordingStore(),
        deviceProviders: const [],
      );
      expect(broker.supports('anthropic'), isFalse);
