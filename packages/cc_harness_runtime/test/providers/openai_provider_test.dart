...
void main() {
  ...
  group('OpenAiProvider.complete — errors', () {
    ...
    test(
      'HTTP 429 insufficient_quota → NON-retryable (hard quota exhaustion)',
      () async {
        // Billing exhaustion cannot self-heal — retrying the same key after a
        // backoff would fail identically, so the error must read as
        // non-retryable: a fallback chain rotates away immediately and a lone
        // credential fails the run fast instead of burning the retry budget.
        final http = _ScriptedHttp.throws(
          ProviderHttpException(
            429,
        ...
        expect(err.code, 'insufficient_quota');
      },
    );

    test('z.ai 1113 → NON-retryable, and names the coding-plan lane', () async {
      // z.ai returns 1113 on a 429 when a GLM Coding Plan key is pointed at
      // the pay-as-you-go host: the account has a subscription and no balance,
      // so no backoff can help. The fix is a different endpoint, and the error
      // is the only place the operator will read that.
      final http = _ScriptedHttp.throws(
        ProviderHttpException(
          429,
      ...
      final provider = OpenAiProvider(http: http, providerName: 'z.ai');
      final events = await provider
          .complete(messages: [HarnessMessage.user('hi')])
          .toList();
      final err = events.whereType<LlmError>().single;
      expect(err.retryable, isFalse);
      expect(err.code, '1113');
      expect(err.message, contains('z.ai GLM Coding Plan'));
    });
  ...
  group('OpenAiProvider.listModels', () {
    ...
    test('servesOnlyModels drops what the host lists but will not run', () {
      // The z.ai coding host reports the whole account catalogue and then
      // answers as glm-5.3 for everything outside the plan. Offering the rest
      // would be a picker whose choice the endpoint silently overrides.
      final http = _FakeCatalogHttp(
        body: jsonEncode({
          'data': [
            {'id': 'glm-4.6'},
      ...
      final provider = OpenAiProvider(
        http: http,
        servesOnlyModels: const {'glm-5.3', 'glm-5.3-flash'},
      );
      expect(
        provider.listModels().then((m) => m.map((e) => e.id).toList()),
        completion(['glm-5.3', 'glm-5.3-flash']),
      );
    });
