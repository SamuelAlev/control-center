...
class GitHubServiceStatus {
  ...
  /// Parses the Statuspage v2 `summary.json` shape returned by any
  /// Statuspage-hosted status page (githubstatus.com, status.claude.com, …).
  /// Shared by the server (which fetches the JSON) and the thin client (which
  /// receives the raw summary over the `github.serviceStatus` /
  /// `claude.serviceStatus` RPC ops and parses it here), so the parse lives in
  /// one web-safe place.
  ///
  /// The page URL is read from the summary's own `page.url`, so vendor
  /// specifics (the "Visit … for more information" dummy component, the
  /// incident-link fallback) follow the page being parsed.
  factory GitHubServiceStatus.fromSummaryJson(Map<String, dynamic> data) {
    ...
    final indicator = GitHubStatusIndicator.fromApi(
      statusBlock['indicator'] as String?,
    );
    final description =
        (statusBlock['description'] as String?) ?? 'Unknown status';

    final pageUrl = ((data['page'] as Map<String, dynamic>?)?['url'] as String?)
        ?.trim();
    // Old fixtures/snapshots may lack the `page` block — fall back to the
    // githubstatus.com host so its dummy component is still filtered.
    final pageHost = pageUrl == null || pageUrl.isEmpty
        ? 'www.githubstatus.com'
        : Uri.tryParse(pageUrl)?.host ?? 'www.githubstatus.com';

    final rawComponents = (data['components'] as List<dynamic>? ?? const [])
        .whereType<Map<String, dynamic>>()
        .where((c) => c['only_show_if_degraded'] != true)
        // Statuspage injects a "Visit <host> for more information" pseudo-
        // component on some pages — it carries no health signal, drop it.
        .where(
          (c) =>
              (c['name'] as String?) != 'Visit $pageHost for more information',
        )
        .toList();
    ...
    final incidents = (data['incidents'] as List<dynamic>? ?? const [])
        ...
        .map(
          (i) => GitHubStatusIncident(
            ...
            shortlink:
                (i['shortlink'] as String?) ??
                ((pageUrl != null && pageUrl.isNotEmpty)
                    ? pageUrl
