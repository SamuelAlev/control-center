...
class SandboxPolicySpec {
  ...
  /// bind mounts in review/plan/orchestrate modes.
  ///
  /// Both platform materializers consume this, in a specific order:
  /// Linux bwrap emits `--ro-bind` *after* the writable binds (last-mount-wins),
  /// and macOS Seatbelt emits `(deny file-write* …)` *after* the `$HOME`
  /// allowance (last-match-wins). Reads stay open on both. [runDir] is
  /// re-allowed after these on both platforms, because it sits inside a mount.
  ...
  /// Directories the external runner keeps its own state in — see
  /// [SandboxSpec.runnerStateDirs]. Both materializers re-open these LAST,
  /// alongside [runDir], so no later deny or read-only re-mount can close
  /// them.
  final List<String> runnerStateDirs;

  /// Operator-approved exec roots — see [SandboxSpec.execGrantRoots]. macOS
  /// emits these as `(allow process-exec (subpath …))` AFTER the writable-dir
  /// denies, so last-match-wins re-opens exec for exactly these trees. Linux
  ...
  /// spelling the kernel matches on before handing the policy to a platform
  /// emitter. Keeping the resolution there rather than here is deliberate: it
  /// needs `dart:io` and this layer has none.
  SandboxPolicySpec copyWith({
    List<String>? denyRead,
    List<String>? allowWrite,
    List<String>? denyWrite,
    List<String>? readOnlyMounts,
    List<String>? runnerStateDirs,
    List<String>? execGrantRoots,
  }) => SandboxPolicySpec(
    sessionId: sessionId,
    denyRead: denyRead ?? this.denyRead,
    allowWrite: allowWrite ?? this.allowWrite,
    denyWrite: denyWrite ?? this.denyWrite,
    denyExecutables: denyExecutables,
    allowedDomains: allowedDomains,
    deniedDomains: deniedDomains,
    networkOn: networkOn,
    readOnlyMounts: readOnlyMounts ?? this.readOnlyMounts,
    homeDir: homeDir,
    runDir: runDir,
...
class SandboxPolicyResolver {
  ...
  static const List<String> dangerousGitPaths = ['.git/hooks', '.git/config'];
  ...
  SandboxPolicySpec resolve({
    ...
    // the mode's capability profile, the same declaration the tool surface, the
    // guard preset and the prompt project from.
    final worktreeWritable = profileFor(spec.mode).worktreeWritable;
    for (final m in spec.bindMounts) {
      if (worktreeWritable && !m.readOnly) {
    ...
    // The runner's own state dirs are writable in EVERY mode. A read-only mode
    // restricts what the agent may change in the repo; it was never meant to
    // stop the CLI renewing its own credential, and a `CLAUDE_CONFIG_DIR` the
    // CLI cannot write is a CLI that cannot refresh its token.
    for (final dir in spec.runnerStateDirs) {
      if (dir.isNotEmpty) {
        allowWrite.add(dir);
      }
    }
    ...
    return SandboxPolicySpec(
      ...
      // Protected paths (the original repo checkouts) join the secrets denies:
      // on macOS the deny rules are emitted after the `$HOME` write allowance
      // (last-match-wins, so the deny prevails); on Linux they are masked with
      // a /dev/null bind. Every mode — chat included — keeps originals
      // untouchable; agents only ever write in their CoW worktrees.
      denyWrite: [...secretsDenyWrite, ...spec.protectedPaths],
      ...
      // Carried through verbatim: a grant is an operator decision, not
      // something the resolver may widen, narrow or infer. Read-only modes are
      // no exception — a mode restricts what the agent may CHANGE in the repo,
      // and running a linter is not a change.
      execGrantRoots: [
