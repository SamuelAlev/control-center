...
class CommandPolicy {
  ...
  CommandDecision _evaluateSingle(String command) {
    ...
    for (final rule in allow) {
      if (_matchesRule(actualTokens, rule)) {
        return CommandDecision.allow;
      }
    }
...
CommandPolicy commandPolicyForMode(Mode mode) {
  switch (mode) {
    case Mode.chat:
      return const CommandPolicy(deny: defaultDeny, prompt: defaultPrompt);
    case Mode.review:
    case Mode.plan:
