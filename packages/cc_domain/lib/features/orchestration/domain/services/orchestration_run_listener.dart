...
class OrchestrationRunListener {
  ...
  void start() {
    _subs
      ..add(
        _eventBus.on<PipelineRunFailed>().listen(
          (e) =>
              _onTerminal(e.pipelineRunId, failed: true, error: e.errorMessage),
        ),
      )
      ..add(
        _eventBus.on<PipelineRunCancelled>().listen(
  ...
  Future<void> _onTerminal(
    ...
    try {
      ...
      CcDomainLog.error(
        'OrchestrationRunListener: terminal handling failed',
        e,
        st,
      );
    }
