...
class OrchestrationMaterializer {
  ...
  /// hired). [spaceId], [parentTicketId] and [projectId] are threaded onto
  ...
  PipelineDefinition buildDefinition(
      ...
      steps.add(
        PipelineStepDefinition(
          ...
          config: _agentConfig(
            ...
            projectId: projectId,
            // Investigating, not building: read-only.
            mode: Mode.review,
          ),
          x: x,
          y: 0,
        ...
        steps.add(
          PipelineStepDefinition(
            ...
            config: _agentConfig(
              ...
              prompt:
                  '${p.discussion.prompt}\n\nGoal: ${p.goal}\n\n'
                  'Post your approach for the team in the shared space, then '
                  'call submit_output with your structured position.',
              outputKey: 'discussion_${role.roleKey}',
              outputSchema: _discussionSchema,
              spaceId: spaceId,
              parentTicketId: parentTicketId,
              projectId: projectId,
              // Stating an approach, not building: read-only.
              mode: Mode.review,
            ),
            x: x,
            y: dy,
    ...
    // ── Sub-ticket work DAG ──────────────────────────────────────────────
    // Partial approval (PRD 17 §4): an unapproved node's work step hangs off
    // an injected approval gate that suspends until a later approval resumes
    // it. The whole plan always materializes — deferral is a suspended step,
    // never a missing one — and the mapping is a pure function of the
    // orchestration's approved set, so determinism holds.
    final subTicketIds = <String>[];
    var sy = 0.0;
    for (final t in p.subTickets) {
      final id = 'sub_${t.key}';
      subTicketIds.add(id);
      var upstream = t.dependsOn.isEmpty
          ? rootGate
          : [for (final dep in t.dependsOn) 'sub_$dep'];
      if (!orchestration.isNodeApproved(t.key)) {
        final gateId = 'gate_${t.key}';
        steps.add(
          PipelineStepDefinition(
            id: gateId,
            kind: StepKind.listen,
            bodyKey: BuiltInBodyKeys.orchestrationAwaitApproval,
            triggers: [StepTrigger(sourceStepIds: upstream)],
            config: PipelineNodeConfig(
              label: 'Await approval: ${t.title}',
              extras: {
      ...
      steps.add(
        PipelineStepDefinition(
          ...
          config: _agentConfig(
            ...
            parentTicketId: parentTicketId,
            projectId: projectId,
            // The work node. Approving a plan is the operator saying "do it", so
            // this step is read-write — `review` would hand it Claude's read-only
            // permission mode and it could not change anything.
            mode: Mode.chat,
          ),
          x: x,
          y: sy,
    ...
    steps.add(
      PipelineStepDefinition(
        ...
        config: PipelineNodeConfig(
          ...
          extras: {
            'orchestration_id': orchestration.id,
            'sub_ticket_keys': [for (final t in p.subTickets) t.key],
          },
        ),
        x: x,
        y: 0,
    ...
    steps.add(
      PipelineStepDefinition(
        ...
        bodyKey: BuiltInBodyKeys.promptAgent,
        triggers: const [
          StepTrigger(sourceStepIds: [markPhaseId]),
        ],
        config: _agentConfig(
          ...
          parentTicketId: parentTicketId,
          projectId: projectId,
          // Produces the deliverable (and may write it out): read-write.
          mode: Mode.chat,
        ),
        x: x,
        y: 0,
    ...
    steps.add(
      PipelineStepDefinition(
        ...
        triggers: const [
          StepTrigger(sourceStepIds: [synthesisId]),
        ],
        config: PipelineNodeConfig(
          label: 'Deliver',
          extras: {'orchestration_id': orchestration.id},
        ),
        x: x,
        y: 0,
      ),
    ...
    steps.add(
      PipelineStepDefinition(
        ...
        triggers: const [
          StepTrigger(sourceStepIds: [persistId]),
        ],
    ...
    return PipelineDefinition(
      ...
      description:
          'Generated from orchestration ${orchestration.id} — ${p.goal}',
      steps: steps,
    );
  }
  ...
  PipelineNodeConfig _agentConfig({
    ...
    return PipelineNodeConfig(
      ...
      extras: {
        // The KEY matters: the message-step bodies read `mode`
        // (PipelineNodeConfig.modeName). This used to be written as
        // `conversationMode`, which nothing read — so every generated node
        // silently fell back to read-only `review` and an approved plan's work
        // steps could not write a single file.
        'mode': mode.name,
        'spaceId': spaceId,
  ...
  String _subTicketPrompt(OrchestrationProposal p, ProposedSubTicket t) {
    ...
    if (p.discussion.enabled) {
      buf
        ..writeln()
        ..writeln(
          'Team positions from the discussion round are available in '
          'the shared space.',
        );
    }
    for (final dep in t.dependsOn) {
      buf
        ..writeln()
        ..writeln(
          'Result of upstream task "$dep" (truncate as needed; use '
  ...
  String _synthesisPrompt(OrchestrationProposal p) {
    final buf = StringBuffer()
      ...
      ..writeln()
      ..writeln(
        'Sub-task results (a failed task shows `{"failed": true}` — '
