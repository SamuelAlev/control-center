...
class Orchestration {
  ...
  Orchestration({
    ...
    this.approvedNodeKeys,
    this.errorMessage,
    this.completedAt,
  }) {
    if (id.isEmpty) {
      throw ArgumentError('Orchestration id must not be empty');
    }
    if (workspaceId.isEmpty) {
      throw ArgumentError('Orchestration workspaceId must not be empty');
    }
    if (revision < 1) {
  ...
  /// Approved sub-ticket keys (PRD 17 §4 partial approval). Null means every
  /// node is approved (the whole-plan approve path); when set, unapproved
  /// nodes materialize behind suspended approval gates.
  final List<String>? approvedNodeKeys;

  /// Whether [nodeKey] may execute.
  bool isNodeApproved(String nodeKey) =>
      approvedNodeKeys == null || approvedNodeKeys!.contains(nodeKey);
  ...
  Orchestration copyWith({
    ...
    bool clearApprovedNodeKeys = false,
    String? errorMessage,
    DateTime? updatedAt,
    ...
    approvedNodeKeys: clearApprovedNodeKeys
        ? null
        : (approvedNodeKeys ?? this.approvedNodeKeys),
