...
class ProposedRole {
  ...
  ProposedRole copyWith({
    ...
    existingAgentId: clearExistingAgentId
        ? null
        : (existingAgentId ?? this.existingAgentId),
...
class ProposedHire {
  ...
  factory ProposedHire.fromJson(Map<String, dynamic> json) => ProposedHire(
    ...
    title: json['title'] as String? ?? '',
    skills: (json['skills'] as List?)?.whereType<String>().toList() ?? const [],
...
class ProposedSubTicket {
  ...
  /// Last computed estimate (PRD 17 §3), persisted with the proposal so an
  /// approval records what the operator saw. Null until estimated.
  final PlanNodeEstimate? estimate;

  /// Why this node exists (PRD 17 §7) — evidence refs, not prose.
  ...
  /// Returns an edited copy (Plan Studio editor). [key] is immutable — it is
  /// the node's identity across revisions.
  ProposedSubTicket copyWith({
    String? title,
    String? roleKey,
    String? description,
...
/// How execution responds when a step drifts outside its declared scope
/// (PRD 17 §6): cost past its estimate band or files outside its
/// provenance-declared blast radius.
enum PlanDriftPolicy {
  /// Mark the node with a divergence marker and keep going (default).
  annotate,

  /// Mark the node AND pause its dispatched run so the operator decides.
  stopAndAsk;

  /// Parses a stored name, defaulting to [annotate].
  static PlanDriftPolicy fromName(String? name) =>
      PlanDriftPolicy.values.firstWhere(
        (p) => p.name == name,
        orElse: () => PlanDriftPolicy.annotate,
      );
}
...
class OrchestrationProposal {
  ...
  const OrchestrationProposal({
    ...
    this.driftPolicy = PlanDriftPolicy.annotate,
  });
  ...
  /// Divergence response during execution (PRD 17 §6).
  final PlanDriftPolicy driftPolicy;
  ...
  factory OrchestrationProposal.fromJson(Map<String, dynamic> json) =>
      OrchestrationProposal(
        ...
        budget: json['budget'] is Map
            ...
            : const BudgetSpec(),
        driftPolicy: PlanDriftPolicy.fromName(json['driftPolicy'] as String?),
      );

  /// Parses from a JSON string.
  factory OrchestrationProposal.fromJsonString(String raw) =>
      OrchestrationProposal.fromJson(jsonDecode(raw) as Map<String, dynamic>);
  ...
  OrchestrationProposal copyWith({
    ...
    driftPolicy: driftPolicy ?? this.driftPolicy,
  );
}
