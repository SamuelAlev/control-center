...
Map<String, dynamic> _toolResultToJson(HarnessToolResult result) => {
  ...
  if (result.images.isNotEmpty)
    'images': [for (final i in result.images) i.toJson()],
};
...
/// Reads the optional `images` array written by [_toolResultToJson] and the
/// tool-result block. Absent on every recording made before tool results could
/// carry images, so a missing key is an empty list, never an error.
List<HarnessImageBlock> _imagesFromJson(Object? raw) {
  if (raw is! List) {
    return const [];
  }
  return [
    for (final e in raw.whereType<Map<String, dynamic>>())
      HarnessImageBlock(
        ...
        mediaType: e['media_type'] as String? ?? 'image/png',
      ),
  ];
}
