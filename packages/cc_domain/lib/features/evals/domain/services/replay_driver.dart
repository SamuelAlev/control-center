...
String loopEventSignature(AgentLoopEvent event) {
  switch (event) {
    ...
    case LoopToolsActivated(:final names, :final trigger):
      // Part of the signature, not skipped: which tools a run had to go find
      // is a behavioural difference between two replays, and an eval that
      // ignored it could not tell "found the tool" from "never looked".
      return 'tools_activated:$trigger:${names.join(',')}';
    case LoopCompaction(
      :final summarized,
