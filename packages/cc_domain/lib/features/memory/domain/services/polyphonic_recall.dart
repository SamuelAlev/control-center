...
/// The four independently-rankable recall voices.
enum RecallVoice {
...
class RecallCandidate<T> {
  ...
  /// Context relevance multiplier applied to the final score. `1.0` is neutral.
  ///
  /// Used to favour memories scoped to the repo the caller is working in
  /// ([kRepoRecallBoost]) without ever excluding the rest: a boost reorders,
  /// it never filters, so a workspace-wide fact and another repo's fact both
  /// stay reachable.
  final double boost;
}

/// Score multiplier for a memory scoped to the caller's current repo.
///
/// Deliberately modest. Repo affinity is one signal among four voices plus
/// decay; a large multiplier would let a stale, barely-matching fact from the
/// right repo outrank a fresh, strongly-matching one from the workspace.
const double kRepoRecallBoost = 1.35;
...
/// fact's Weibull decay (so a stale `request` sinks below a fresh fact), a
/// mild importance factor and the candidate's context [RecallCandidate.boost]
/// (repo affinity), and finally [mmrRerank]ed for topical diversity.
List<RankedRecall<T>> fusePolyphonicRecall<T>({
  ...
  for (final entry in rrf.entries) {
    ...
    final finalScore =
        entry.value * decayFactor * importanceFactor * candidate.boost;
    scored.add(
      MmrItem<RankedRecall<T>>(
        value: RankedRecall<T>(
  ...
  final reranked = mmrRerank<RankedRecall<T>>(
    scored,
    lambda: mmrLambda,
    topK: topK,
  );
