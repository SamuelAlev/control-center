...
class ExtractionResult {
  ...
  List<ExtractedItem> toTypedItems() => <ExtractedItem>[
    ...
    for (final t in timelines)
      ExtractedItem(content: t, type: MemoryType.event),
  ];
}
...
final List<_HeuristicRule> _heuristicRules = <_HeuristicRule>[
  ...
  _h(
    r"\bi (?:hate|dislike|do not like|don't like)\s+([^,.!?;]+)",
    'The user dislikes {}',
    MemoryType.preference,
    'preference',
  ),
  _h(
    r'\b(?:i|you)\s+(?:always|never)\s+([^,.!?;]+)',
    'Instruction: {}',
