...
/// Heuristic memory-type rules
/// (`TYPE_PATTERNS`). Order matters only as a tie-break
...
final List<_TypeRule> _rules = <_TypeRule>[
  ...
  _rule(
    r'\b(often|frequently|sometimes|rarely)\s+\w+',
    MemoryType.observation,
    0.6,
  ),
  _rule(
  ...
  _rule(
    r'\b(PR|pull\s+request|issue|ticket)\s+#?\d+',
    MemoryType.artifact,
    0.9,
  ),
  _rule(
...
const Map<MemoryType, List<String>>
_confidenceBoosters = <MemoryType, List<String>>{
  MemoryType.fact: [
    'verified',
    'confirmed',
    'official',
...
TypeMatch classifyMemory(String content) {
  ...
  return wordCount < 5
      ? const TypeMatch(
          ...
          confidence: 0.3,
          matchedPattern: 'default_short',
        )
      : const TypeMatch(
          memoryType: MemoryType.context,
          confidence: 0.3,
          matchedPattern: 'default_long',
        );
}
