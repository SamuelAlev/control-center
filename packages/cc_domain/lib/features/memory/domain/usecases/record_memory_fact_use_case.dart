...
class RecordMemoryFactUseCase {
  ...
  ///
  /// [conflictRepository] and [edgeRepository] are optional: when
  /// absent, conflict rows / episodic edges are simply not
  ...
  Future<RecordFactResult?> record({
    ...
    // [repoSlug] scopes the fact to one repo; omitted, the fact is
    // workspace-wide. Dedup, conflict detection and supersession below all key
    // off `resolved.name`, so the same statement recorded for two repos stays
    // two independent facts rather than one overwriting the other.
    ...
    final sameDomain = activeForTopic
        .where((f) => f.domain == resolved.name)
        .toList();
