...
/// Promotes a set of facts into a formal memory policy after enforcing
...
class PromoteFactsToPolicyUseCase {
  ...
  /// Promotes a group of facts into a single [MemoryPolicy].
  ///
  /// Returns the new policy, or `null` if the author lacks permission.
  ...
  /// Auto-promotes fact groups (≥3 facts per domain) into policies.
  ///
  /// Returns the list of newly created policies.
  Future<List<MemoryPolicy>> autoPromote(String workspaceId) async {
    ...
    for (final entry in domainGroups.entries) {
      if (entry.value.length >= 3) {
        ...
        final rule = factsForDomain.map((f) => '- ${f.content}').join('\n');
        ...
        final alreadyPromoted = existing.any(
          (p) =>
              p.sourceFactIds.length == sourceIds.length &&
        ...
        if (!alreadyPromoted) {
          final policy = MemoryPolicy(
            ...
            domain: entry.key,
            rule:
                '# Auto-promoted from ${factsForDomain.length} facts\n\n$rule',
