...
abstract interface class HarnessProviderRepository {
  ...
  ///
  /// [models] pre-registers models (bare id → metadata) for endpoints that
  /// cannot enumerate their own; each is stored as a manual model override.
  ...
  /// Stores a per-model metadata override for [providerId]: context window,
  /// output ceiling and/or modalities, winning over the endpoint's live report
  /// and the models.dev catalog. [modelId] is the bare provider-native id (the
  /// qualified `provider/model` form is also accepted). An override with every
  /// field cleared behaves as [removeModelOverride].
  ...
  /// Clears the stored override for [providerId]'s [modelId]; a manual model
  /// leaves the list, a live-reported model returns to endpoint/catalog
  /// metadata.
