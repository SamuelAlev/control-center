...
class PipelineNodeConfig {
  ...
  /// conversation (the channel clone/code-index scope).
  ///
  ...
  /// `['{{repoId}}']` clones only the repo the run is processing).
  ///
  /// Read by the bodies that create conversations (`pipeline.promptAgent`,
  /// `team.dispatch`, `flow.forEach`, `human.gate`). Ignored when the node
  /// reuses an existing conversation via [channelId] — that conversation
  /// keeps the repo selection it was created with.
  ...
  ///
  /// Read by the message-step bodies. Note the KEY: a node that writes any
  /// other spelling is silently ignored, which is how generated orchestration
  /// steps once ended up read-only.
  String? get modeName {
