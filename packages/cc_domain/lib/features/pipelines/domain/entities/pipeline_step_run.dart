...
class PipelineStepRun {
  ...
  /// The previous firings of this row, oldest first, archived each time the
  /// row is re-opened for another go (Retry button / crash-resume). The
  /// current firing is the row itself, so the attempt an operator is watching
  /// is number `priorAttempts.length + 1`.
  final List<PipelineStepAttempt> priorAttempts;
  ...
  @override
  int get hashCode =>
      Object.hash(id, pipelineRunId, stepId, status, branchIndex);
}
