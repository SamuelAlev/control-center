...
class BuiltInBodyKeys {
  ...
  /// Removes stale isolated worktrees. With a `ticketId` or PR
  /// (`repoFullName` + `prNumber`) in the trigger payload it tears down that
...
Map<String, List<BuiltInTriggerSeed>> builtInTriggerSeeds() => {
  ...
  'pr_merged_cleanup': const [
    ...
    BuiltInTriggerSeed.event('ChannelDeleted'),
...
PipelineDefinition _prReviewSeed({
  ...
  PipelineStepDefinition reviewer({
    ...
    return PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        // Clone only the repo under review into the reviewer conversation,
        // not every workspace repo.
        repoIds: const ['{{repoId}}'],
  ...
  final steps = <PipelineStepDefinition>[
    ...
    // publishing to GitHub is now user-gated through `publish_review_to_github`
    // (the "Publish to GitHub" button), so the pipeline must not post directly
    // or the review would double-publish. The pipeline ends after consolidating
    // findings into the channel; reviewer agents record findings as review
    // nodes, the CEO finalizes the verdict and the user publishes.
    PipelineStepDefinition(
...
/// directory has vanished, worktrees whose CHANNEL no longer exists (deleting a
/// conversation fires `ChannelDeleted` → release, but an event missed while the
...
/// `.mcp.json` — left behind by a deleted channel.
///
...
PipelineDefinition _repoCleanupSeed({required String workspaceId}) {
  ...
  final steps = <PipelineStepDefinition>[
    PipelineStepDefinition(
      ...
      config: const PipelineNodeConfig(
        label: 'Remove stale worktrees + deleted conversations',
        outputKey: 'cleanupResult',
  ...
  return PipelineDefinition(
    ...
    description:
        'Removes stale isolated worktrees and the conversation folders they '
        'live in. Fires on PR merged/closed and ticket done/cancelled (cleans '
  ...
  PipelineStepDefinition specialist({
    ...
    return PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        // Clone only the repo under review into the specialist
        // conversation, not every workspace repo.
        repoIds: const ['{{repoId}}'],
        outputKey: '${stepId}_findings',
        label: label,
        prompt: prompt,
...
PipelineDefinition _ticketToPrSeed({
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        // Clone only the ticket's repo into the coder conversation, not
        // every workspace repo. Falls back to all repos on trigger paths
        // whose payload carries no repoId (e.g. TicketAssigned).
        repoIds: const ['{{repoId}}'],
    ...
    PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        timeoutMs: 900000,
        prompt:
            'You are a reviewer. The change for "{{ticketTitle}}" is '
  ...
  return PipelineDefinition(
    ...
    inputs: [
      ...
      PipelineInput(key: 'ticketTitle', label: 'Ticket title', required: true),
      PipelineInput(
        key: 'ticketBody',
...
PipelineDefinition _prTriageSeed({
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      bodyKey: BuiltInBodyKeys.promptAgent,
      triggers: const [
        StepTrigger(sourceStepIds: [cloneId]),
      ],
      config: PipelineNodeConfig(
        agentId: agentIds.qa,
        label: 'Classify PR',
        ...
        timeoutMs: 300000,
        prompt:
            'The PR branch is checked out at `{{repoLocalPath}}` '
    ...
    branchReview(
      ...
      label: 'Docs check',
      prompt:
          'Docs-only PR at `{{repoLocalPath}}` (#{{prNumber}} — '
    ...
    branchReview(
      ...
      label: 'Security review',
      prompt:
          'Security-sensitive PR at `{{repoLocalPath}}` (#{{prNumber}} — '
    ...
    branchReview(
      ...
      prompt:
          'PR at `{{repoLocalPath}}` (#{{prNumber}} — {{prTitle}}). '
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        timeoutMs: 900000,
        prompt:
            'Review PR #{{prNumber}} — {{prTitle}} at `{{repoLocalPath}}`. '
    ...
    PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        prompt:
            'Review findings for PR #{{prNumber}} before merge:\n\n'
...
PipelineDefinition _meetingSummarySeed({
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      config: const PipelineNodeConfig(
        ...
        inputKeys: ['meetingId', 'diarizationSpans'],
      ),
      x: 0,
      y: -40,
    ),
    ...
    PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        prompt:
            ...
            r'<instructions>{{$trigger.summaryInstructions}}</instructions>'
            '\n\n'
            '<user_notes>\n{{userNotes}}\n</user_notes>\n\n'
            '<transcript>\n{{transcript}}\n</transcript>',
      ),
  ...
  return PipelineDefinition(
    ...
    isBuiltIn: true,
    inputs: [
      PipelineInput(key: 'meetingId', label: 'Meeting ID', required: true),
  ...
  PipelineStepDefinition auditNode({
    ...
    return PipelineStepDefinition(
      ...
      config: PipelineNodeConfig(
        ...
        timeoutMs: 900000,
        prompt:
            'The repo is cloned at `{{repoLocalPath}}`. Audit the '
...
PipelineDefinition _helloSeed({required String workspaceId}) {
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      triggers: const [
        StepTrigger(sourceStepIds: ['greet']),
      ],
    ...
    PipelineStepDefinition(
      ...
      triggers: const [
        StepTrigger(sourceStepIds: ['world']),
      ],
    ),
  ];
    ...
    steps: List.unmodifiable([
      PipelineStepDefinition(
        ...
        config: const PipelineNodeConfig(
          ...
          inputKeys: ['workspaceId', 'skill_slug'],
          outputKey: 'skillScanSummary',
          timeoutMs: 600000,
        ),
        x: 0,
        y: 0,
    ...
    steps.add(
      PipelineStepDefinition(
        ...
        config: PipelineNodeConfig(
          ...
          // Clone only the repo being indexed into the analyzer
          // conversation, not every workspace repo.
          repoIds: const ['{{repoId}}'],
          outputKey: 'analysis',
          prompt:
              'The repository (id: {{repoId}}) was just (re-)indexed into the '
              'code graph for workspace {{workspaceId}}. This pipeline runs on '
              ...
              'list_policies) for workspace {{workspaceId}} to load the '
              ...
              'pass workspace_id: {{workspaceId}} and repo_id: {{repoId}} so '
              ...
              'workspace {{workspaceId}}. Focus on lasting, high-level '
