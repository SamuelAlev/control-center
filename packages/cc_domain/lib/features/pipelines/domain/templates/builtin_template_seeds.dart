import 'package:cc_domain/core/domain/events/pr_events.dart' show ExternalPrDetected;
...
/// run or the weekly schedule sweeps every orphaned worktree in the workspace.
///
...
PipelineDefinition _repoCleanupSeed({required String workspaceId}) {
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      triggers: const [StepTrigger(sourceStepIds: [cleanupId])],
    ),
  ];
  ...
  return PipelineDefinition(
    ...
    description:
        'Removes stale isolated worktrees. Fires on PR merged/closed and '
        'ticket done/cancelled (cleans up that unit), or on a manual run / '
        'weekly schedule (sweeps every orphaned worktree in the workspace).',
    isBuiltIn: true,
    isEnabled: false,
    steps: List.unmodifiable(steps),
  );
...
PipelineDefinition _meetingSummarySeed({
  ...
  final steps = <PipelineStepDefinition>[
    ...
    PipelineStepDefinition(
      ...
      triggers: const [StepTrigger(sourceStepIds: [identifySpeakersId])],
      config: PipelineNodeConfig(
        agentId: agentIds.ceo,
        ...
        // which is read-only and would block the mutating `complete_ticket`
        // this step's contract requires. Summarizing a meeting is a
        // content-generation task, not a read-only review.
        extras: const {'conversationMode': 'chat'},
        prompt: 'You are augmenting meeting notes for the meeting titled '
            ...
            '`complete_ticket` tool — a single JSON object with these keys '
