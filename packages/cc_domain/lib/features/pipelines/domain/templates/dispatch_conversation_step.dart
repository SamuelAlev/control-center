...
/// authored it.
///
/// **Repo scope.** [repoIds] (a node config's `repoIds`) scopes which
/// workspace repos the fresh hidden conversation clones — and therefore
/// code-indexes — at provisioning time. Empty (the default) provisions every
/// workspace repo as before. A reused configured room keeps the repo
/// selection it was created with; the scope only applies at creation.
///
...
Future<StepResult> dispatchConversationStep({
  ...
  if (reuseConfigured) {
    ...
    final channel = await messagingPort.createChannel(
      ...
      repoIds: _resolveScopedRepoIds(repoIds, ctx),
    );
...
/// completely is dropped, because a seeded template's `['{{repoId}}']` must
/// survive trigger paths whose payload carries no repo (e.g. a scheduled
/// sweep). When nothing resolves the step falls back to the legacy
/// every-repo provisioning and logs a warning rather than failing the run —
/// an over-provisioned conversation keeps the agent's work correct, while a
/// hard failure would brick the whole template on that trigger path.
List<String> _resolveScopedRepoIds(
  ...
  if (dropped.isNotEmpty) {
    CcDomainLog.warning(
      'dispatchConversationStep: repo scope entries did not resolve and were '
      'ignored: ${dropped.join(', ')}',
    );
    if (resolved.isEmpty) {
      CcDomainLog.warning(
        'dispatchConversationStep: repo scope is empty after resolution — '
        'falling back to provisioning every workspace repo',
      );
    }
  }
  return resolved;
