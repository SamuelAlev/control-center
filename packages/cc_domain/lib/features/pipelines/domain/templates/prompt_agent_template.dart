...
///    room named by `extras['channelId']` when the node carries one (generated
...
void registerPromptAgentBody(
  ...
  registry.registerBody(BuiltInBodyKeys.promptAgent, (ctx) async {
    ...
    final agent = await agentRepository.getById(ctx.workspaceId, agentId);
    if (agent == null) {
    ...
    if (!renderResult.isComplete) {
      ...
      CcDomainLog.warning(
        'promptAgent: Step "${ctx.stepId}" prompt has unresolved placeholders: '
    ...
    return dispatchConversationStep(
      ...
      mode: _resolveMode(stepConfig),
      channelId: stepConfig.channelId,
      repoIds: stepConfig.repoIds,
    );
  });
}
...
Mode _resolveMode(PipelineNodeConfig config) {
  return switch (config.modeName) {
    // `chat` is the plain read-write task mode. A content-generation step
    // (e.g. meeting summarization) MUST opt into it: `review`/`plan` map to
    // Claude's read-only `--permission-mode plan`, which is wrong for a step
    // that has to produce and submit output.
    'chat' => Mode.chat,
    'review' => Mode.review,
    'plan' => Mode.plan,
    // Reviewers (the built-in default) get review mode; they set no override.
    _ => Mode.review,
  };
}
