  ...
  registry.registerBody(BuiltInBodyKeys.teamDispatch, (ctx) async {
    ...
    if (config == null) {
      return StepResult.failed(
        'teamDispatch: step "${ctx.stepId}" missing config',
      );
    }
    final teamId = config.teamId;
    if (teamId == null || teamId.isEmpty) {
      return StepResult.failed(
        'teamDispatch: step "${ctx.stepId}" missing teamId',
      );
    }
    if (config.prompt == null || config.prompt!.isEmpty) {
      return StepResult.failed(
        'teamDispatch: step "${ctx.stepId}" missing prompt',
      );
    }

    final members = await teamRepository.membersOf(workspaceId, teamId);
    if (members.isEmpty) {
    ...
    if (mode == 'manager') {
      ...
      final agent = await agentRepository.getById(workspaceId, leader.agentId);
      if (agent == null) {
        return StepResult.failed(
    ...
    for (final member in members) {
      final agent = await agentRepository.getById(workspaceId, member.agentId);
      if (agent == null) {
