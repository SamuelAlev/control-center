...
///  * `channelId` present (a conversation was deleted) → tears down that
///    conversation's worktrees AND its folder — the per-agent overlays and
///    their token-bearing `.mcp.json`
///    ([RepoWorkspaceProvisionerPort.releaseConversation]).
///  * none of them (a manual run or the scheduled sweep) → sweeps the
///    workspace: worktrees whose directory vanished, worktrees whose channel is
///    gone and orphan conversation folders
...
void registerCleanupReposBody(
  ...
  registry.registerBody(BuiltInBodyKeys.cleanupRepos, (ctx) async {
    ...
    // `ChannelDeleted` carries the channel it removed.
    final channelId = ctx.optional<String>('channelId');

    final hasTicket = ticketId != null && ticketId.isNotEmpty;
    final hasPr =
        repoFullName != null && repoFullName.isNotEmpty && prNumber != null;
    final hasChannel =
        !hasTicket && !hasPr && channelId != null && channelId.isNotEmpty;
    ...
    try {
      if (hasTicket) {
        ...
        await provisioner.releaseConversation(
          workspaceId: ctx.workspaceId,
          channelId: channelId,
        );
        summary = 'Released worktrees + folder for conversation $channelId';
      } else {
