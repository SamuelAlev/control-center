...
/// contract, then suspends. The approver seeks the user's decision in-space
  ...
  registry.registerBody(BuiltInBodyKeys.humanGate, (ctx) async {
    ...
    if (config == null) {
      return StepResult.failed(
        'humanGate: step "${ctx.stepId}" missing config',
      );
    }
    ...
    final approver = await agentRepository.getById(workspaceId, approverId);
    if (approver == null) {
