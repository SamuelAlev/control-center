...
import 'package:cc_domain/features/pipelines/domain/entities/pipeline_node_config.dart' show StepRetryPolicy;
...
import 'package:cc_domain/features/pipelines/domain/entities/step_result.dart' show StepResult;
...
class PipelineEngine implements PipelineEnginePort {
  ...
  Future<PipelineRun?> start(
    ...
    if (!definition.isEnabled) {
      CcDomainLog.info('PipelineEngine: Rejected disabled template: $templateId',
      );
      return null;
    }
    ...
    try {
      ...
      if (existing != null) {
        CcDomainLog.info('PipelineEngine: Skipping duplicate run for $templateId $dedupKey (active=${existing.id})',
        );
        return null;
      }
  ...
  Future<void> resumeAll() async {
    ...
    CcDomainLog.info('PipelineEngine: Resuming ${runs.length} in-flight pipeline(s)',
    );
    ...
    for (final run in runs) {
      ...
      if (definition == null) {
        CcDomainLog.warning('PipelineEngine: Cannot resume run ${run.id}: template ${run.templateId} '
          'missing for workspace $workspaceId',
        );
        continue;
      }
      if (definition.version != run.templateVersion) {
        CcDomainLog.warning('PipelineEngine: Run ${run.id} pinned template ${run.templateId} v'
  ...
  Future<void> cancel(String pipelineRunId) async {
    ...
    eventBus.publish(PipelineRunCancelled(
      pipelineRunId: run.id,
    ...
    for (final sr in steps) {
      if (sr.status == PipelineStepStatus.running ||
        ...
        try {
          ...
          CcDomainLog.error('PipelineEngine: cancel: kill callback threw for ${sr.id}',
              e, st);
        }
  ...
  Future<void> resumeStep({
    ...
    final suspended = suspendedRuns
        .reduce((a, b) => b.startedAt.isAfter(a.startedAt) ? b : a);
    ...
    if (outputKey != null && outputKey.isNotEmpty && stepDef != null) {
      try {
        ...
        final withOutput = runs
            .where((r) => r.outputJson != null)
            .toList()
        ...
        if (schema != null && schema['type'] == 'object') {
          // Single-object contract: exactly one run should carry a payload.
          if (withOutput.isEmpty) {
            // Every awaited run terminated without output — the agent
            // failed, was force-failed by the output contract, or the run
            // ended without a final submit_output. Surface that reason (and
            // record it on the step) instead of a schema-shape error.
            if (definition != null) {
              final reasons = runs
                  .map((r) => r.summary)
                  .whereType<String>()
              ...
              final reason = reasons.isEmpty
                  ? 'the agent returned no structured output payload'
                  : reasons.join('; ');
  ...
  Future<PipelineDefinition> _requireTemplate(
    ...
    for (final step in def.steps) {
      ...
      if (!bodies.hasBody(step.bodyKey)) {
        CcDomainLog.warning('PipelineEngine: Template "$templateId" step "${step.id}" references unregistered '
  ...
  Future<void> _runStep({
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        CcDomainLog.error('PipelineEngine: Step ${stepDef.id} attempt $attempt/$maxAttempts failed',
          e,
          st,
        );
        if (attempt < maxAttempts) {
  ...
  Future<void> _handleStepFailure({
    ...
    await repository.updateStepRun(
      ...
      status: PipelineStepStatus.completed,
      outputJson: outputJson ??
          jsonEncode({'error': error, 'continuedOnFail': true}),
      finishedAt: DateTime.now(),
    );
