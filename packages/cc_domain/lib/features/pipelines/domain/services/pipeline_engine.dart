...
class PipelineEngine implements PipelineEnginePort {
  ...
  Future<void> resumeAll() async {
    ...
    for (final staleRun in runs) {
      ...
      final run = staleRun.copyWith(lastResumedAt: DateTime.now());
  ...
  Future<void> resumeStep({
    ...
    if (outputKey != null && outputKey.isNotEmpty && stepDef != null) {
      try {
        ...
        final withOutput = runs.where((r) => r.outputJson != null).toList()
  ...
  Future<void> retry(String workspaceId, String pipelineRunId) async {
    ...
    final reset = PipelineRun(
      ...
      // Reset the wall-clock start so the run's duration reflects only this
      // retry attempt. Preserving the original `startedAt` made the duration
      // span the idle gap between failures (days → absurd values), since
      // `finishedAt` is stamped at the new completion moment. Preserved
      // completed step runs keep their own timestamps for the waterfall.
      startedAt: DateTime.now(),
      ...
      // the idle gap between failure and retry is excluded.
