...
import 'package:cc_domain/features/pipelines/domain/templates/builtin_template_seeds.dart'
    show BuiltInBodyKeys, kPipelineSpaceStateKey;

/// Run-context identifiers that `PipelineContext.renderState` injects into
/// every `{{placeholder}}` render, so they always resolve regardless of
/// upstream output and must not be flagged as undeclared consumed keys.
const Set<String> _alwaysAvailableStateKeys = {
  'workspace_id',
  'pipeline_run_id',
  'step_id',
};
...
class PipelineValidator {
  ...
  /// The set of body keys registered in the [PipelineBodyRegistry]. When
  /// non-empty, a step whose `bodyKey` is not in this set is a save-time error
  /// (a typo'd body otherwise only fails at run time). Empty disables the check
  /// (e.g. unit tests without a populated registry).
  final Set<String> knownBodyKeys;

  /// Validates each node's `outputSchema` *document* is well-formed (a
  /// malformed schema otherwise silently validates nothing at run time).
  ...
  List<PipelineIssue> validate(PipelineDefinition def) {
    ...
    if (triggers.isEmpty) {
      issues.add(
        const PipelineIssue(
          ...
          message:
              'Pipeline has no trigger step; every pipeline must start with '
              'a trigger.',
        ),
      );
    ...
    // ── Agent steps need a room, and never open one ─────────────────────
    //
    // A room is a repo checkout. Letting an agent step open one makes a
    // checkout a side effect of dispatching, and parallel steps each reach
    // that point before any has published a room — so a fan-out of N
    // reviewers provisions the workspace N times. The room is one decision the
    // template makes in a node the author can see, and every agent step points
    // at it.
    //
    // Caught here rather than at dispatch: a template that cannot run should
    // fail when it is saved, not halfway through a run that has already
    // dispatched its earlier steps.
    final roomProviders = <String>{
      for (final s in def.steps)
        if (_opensARoom(s))
          if (s.config.outputKey case final String key) key,
    };
    for (final s in def.steps) {
      if (!_dispatchesAgents(s)) {
        continue;
      }
      final room = s.config.spaceId?.trim() ?? '';
      if (room.isEmpty) {
        issues.add(
          PipelineIssue(
            severity: PipelineIssueSeverity.error,
            stepId: s.id,
            message:
                'Step "${s.id}" dispatches agents but names no conversation. '
                "Set extras['spaceId'] — usually "
                '`{{$kPipelineSpaceStateKey}}`, the room a '
                '`messaging.createSpace` node opens for the run.',
          ),
        );
        continue;
      }
      if (s.config.repoIds.isNotEmpty) {
        issues.add(
          PipelineIssue(
            severity: PipelineIssueSeverity.error,
            stepId: s.id,
            message:
                'Step "${s.id}" declares a repo scope, but a conversation IS '
                'the checkout and this step joins one rather than opening it. '
                'Move the repos onto the `messaging.createSpace` node that '
                'opens the room.',
          ),
        );
      }
      // A `{{key}}` reference has to be produced by a node that actually opens
      // a room; a literal id points at a space outside this template and is
      // checked at runtime instead (the operator may have deleted it since).
      final ref = renderer.placeholders(room).firstOrNull;
      if (ref != null && !roomProviders.contains(renderer.stateKeyOf(ref))) {
        issues.add(
          PipelineIssue(
            severity: PipelineIssueSeverity.error,
            stepId: s.id,
            message:
                'Step "${s.id}" works in `$room`, but no step in this pipeline '
                'opens that conversation. Add a `messaging.createSpace` node '
                'upstream whose output key is `${renderer.stateKeyOf(ref)}`.',
          ),
        );
      }
    }
          ...
          if (!ids.contains(src)) {
            issues.add(
              PipelineIssue(
                ...
                stepId: s.id,
                message:
                    'Step "${s.id}" lists upstream "$src" which does not '
                    'exist.',
              ),
            );
            ...
            if (source != null && source.kind != StepKind.router) {
              issues.add(
                PipelineIssue(
                  ...
                  stepId: s.id,
                  message:
                      'Step "${s.id}" has a routed edge from "$src", but '
                      '"$src" is not a router.',
                ),
              );
            }
    ...
    for (final r in def.steps.where((s) => s.kind == StepKind.router)) {
      final hasRoutedEdge = def.steps.any(
        (s) => s.triggers.any(
          (t) => t.routeKey != null && t.sourceStepIds.contains(r.id),
        ),
      );
      if (!hasRoutedEdge) {
        issues.add(
          PipelineIssue(
            ...
            stepId: r.id,
            message:
                'Router "${r.id}" has no routed outgoing edges; downstream '
          ...
          if (cases.isEmpty && !hasDefault) {
            issues.add(
              PipelineIssue(
                ...
                stepId: r.id,
                message:
                    'Router "${r.id}" is a switch with no cases and no '
        ...
        if (!anyReducer) {
          issues.add(
            PipelineIssue(
              ...
              message:
                  'Output key "${entry.key}" is written by '
    ...
    // Body keys must resolve to a registered step body (when the registry's
    // keys are known to the validator).
    if (knownBodyKeys.isNotEmpty) {
      // Structural kinds (trigger/terminal/join/router/forEach) are executed by
      // the engine directly and carry a sentinel bodyKey; only work-performing
      // `listen` steps look up a registered body.
      for (final s in def.steps.where((s) => s.kind == StepKind.listen)) {
        if (!knownBodyKeys.contains(s.bodyKey)) {
          issues.add(
            PipelineIssue(
              severity: PipelineIssueSeverity.error,
              stepId: s.id,
              message: 'Step "${s.id}" uses unknown body "${s.bodyKey}".',
            ),
          );
        }
      }
    ...
    // Output-schema documents must themselves be well-formed.
    final validator = schemaValidator;
    if (validator != null) {
      for (final s in def.steps) {
        final schema = s.config.outputSchema;
        if (schema == null) {
          continue;
        }
        for (final problem in validator.validateSchema(schema)) {
          issues.add(
            PipelineIssue(
              severity: PipelineIssueSeverity.error,
              stepId: s.id,
              message: 'Step "${s.id}" output schema is malformed: $problem',
            ),
          );
        }
      }
    ...
    for (final s in def.steps) {
      ...
      for (final field in [s.config.prompt, s.config.script]) {
        ...
        for (final ref in renderer.placeholders(field)) {
          if (renderer.isTriggerScoped(ref)) {
            continue;
          }
      ...
      for (final key in consumed) {
        // Run-context identifiers are injected into every render by
        // `PipelineContext.renderState`, so they always resolve regardless of
        // upstream output — never flag them as undeclared.
        if (_alwaysAvailableStateKeys.contains(key)) {
          continue;
        }
        if (!producedBy.containsKey(key)) {
          issues.add(
            PipelineIssue(
              ...
              stepId: s.id,
              message:
                  'Step "${s.id}" reads "$key" which no upstream step '
...
List<String> _predicateIssues(Map<String, dynamic> node) {
  ...
  switch (type) {
    case 'fileExists':
      ...
      final hasPath =
          (paths is List &&
              paths.any((p) => p is String && p.trim().isNotEmpty)) ||
          (paths is String && paths.trim().isNotEmpty) ||
          (node['path'] is String &&
              (node['path'] as String).trim().isNotEmpty);
      return hasPath
          ? const []
          : const ['has a file-exists check with no paths.'];
    case 'comparison':
    ...
    case 'not':
      final of = node['of'];
      final child = of is Map
          ? of
          : (of is List && of.isNotEmpty ? of.first : null);
      if (child is! Map) {
        return const ['has a "not" condition with no child.'];
      }
...
/// Whether [s] dispatches agents into a conversation — the steps that must name
/// a room. Keyed on the body, because that is what actually dispatches.
bool _dispatchesAgents(PipelineStepDefinition s) => const {
...
/// Whether [s] opens a conversation and publishes its id under its output key.
bool _opensARoom(PipelineStepDefinition s) =>
    s.bodyKey == BuiltInBodyKeys.createSpace;
