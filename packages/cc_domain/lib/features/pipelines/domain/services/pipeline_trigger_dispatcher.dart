...
class PipelineTriggerDispatcher {
  ...
  /// The domain event bus to listen on.
  final DomainEventBus eventBus;

  /// The pipeline engine used to start runs.
  final PipelineEngine engine;

  /// Repository for looking up enabled triggers.
  ...
  Future<void> _onEvent(DomainEvent event) async {
    ...
    try {
      ...
      final scopeWorkspaceId = payload['workspace_id'] as String?;
      for (final trigger in triggers) {
        ...
        if (run != null) {
          CcDomainLog.info(
            'PipelineTrigger: Started ${trigger.templateId} for $typeName in workspace ${trigger.workspaceId}',
          );
        }
      }
    } on Object catch (e, st) {
      CcDomainLog.error(
        'PipelineTrigger: Failed to dispatch for $typeName',
        e,
        st,
      );
    }
