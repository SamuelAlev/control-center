...
abstract class CalendarRepository {
  ...
  /// Watches the synced calendar sources for one account in [workspaceId]
  /// (the calendars the user owns/subscribed-to on [accountId] — primary
  /// first). Persisted host-side by the sync sweep; thin clients read this
  /// watch instead of calling the provider directly (they hold no OAuth
  /// tokens). Scoped to `(workspaceId, accountId)`.
  ...
  /// [sources] (deleting any the provider no longer lists). Host-side only —
  /// runs during the sync sweep, never reached from a thin client.
  ...
  /// Removes the events whose provider ids are in [externalIds] from one
  /// calendar — the incremental-sync deletion path (the provider reports
  ...
  /// The stored incremental-sync token for one calendar, or null when no
  /// token is held (the next sync must be a full one). Host-side only.
  ...
  /// Stores (non-null [token]) or clears (null [token]) one calendar's
  /// incremental-sync token, with the full-sync stamp that minted it.
  /// Host-side only.
  ...
  /// meeting isn't linked). The meeting and event themselves are untouched.
  ...
  /// for every meeting whose title is NOT user-customized. The "keep in sync"
  /// half of calendar↔meeting linking; runs after each calendar sync.
