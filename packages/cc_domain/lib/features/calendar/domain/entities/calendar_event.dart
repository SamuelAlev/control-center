...
class CalendarAttendee {
  ...
  /// A human-friendly label: the provider-supplied [displayName] when present,
  /// otherwise a name derived from the [email] (e.g. `ada.lovelace@x.com` →
  /// "Ada Lovelace"), falling back to the raw email when nothing usable can be
  /// derived. Google rarely populates per-attendee display names, so most
  /// invitees rely on the derived form.
  String get displayLabel {
    final name = displayName?.trim();
    if (name != null && name.isNotEmpty) {
      return name;
    }
    return humanizeEmailName(email);
  }
...
/// Derives a human-friendly name from an [email] address, used when no display
/// name is available — e.g. `ada.lovelace@example.com` → "Ada Lovelace".
///
/// The local part (before `@`) is stripped of any `+tag` sub-address, split on
/// `.`, `_` and `-` and each segment is title-cased. Returns the original
/// [email] unchanged when the local part yields nothing usable, so opaque or
/// role addresses still render as the raw address.
String humanizeEmailName(String email) {
...
/// date-only [startTime] at local midnight ON THE READING DEVICE (they cross
/// the wire as a bare `YYYY-MM-DD` civil date, so the day never shifts with
/// the reader's timezone); timed events carry UTC instants — render with
/// `toLocal()`.
@immutable
class CalendarEvent {
...
/// One of a connected account's calendars — the user's primary calendar, a
/// shared "Team" calendar, a "Holidays" subscription, etc. — i.e. one row of
/// the sidebar's per-account calendar list.
///
/// Synced read-only from the provider by the host (which owns the OAuth
/// tokens) during the calendar sync sweep and read by thin clients over the
/// `calendar.watchSources` RPC. `backgroundColor` is the provider's `#rrggbb`
/// hex (parsed to a UI color in the presentation layer — the domain is
/// Flutter-free). `writable` is true for calendars the user owns or can edit,
/// so RSVP is only offered where it is meaningful.
@immutable
