...
///
/// `self: true` returns only the local user (for the "me" space); `self:
/// false` returns the other invitees (for the "them" space). Each name is the
...
List<String> eventAttendeeNames(CalendarEvent? event, {required bool self}) {
  if (event == null) {
    return const [];
  }
  final names = <String>[];
