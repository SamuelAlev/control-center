...
class OrphanRunReaper {
  ...
  Future<void> reap() async {
    ...
    CcDomainLog.info('$_tag: Inspecting ${activeRuns.length} active run(s) for orphans');
  ...
  Future<void> _reapRun(AgentRunLog run) async {
    final pid = run.pid;
    if (pid == null) {
      CcDomainLog.warning('$_tag: Run ${run.id} has null pid — marking as dead');
    ...
    if (!processAlive) {
      CcDomainLog.info('$_tag: Run ${run.id} pid $pid is dead (process not found)');
  ...
  Future<void> _failBackingTicket(AgentRunLog run) async {
    ...
    try {
      ...
      CcDomainLog.error('$_tag: Failed to fail backing ticket $ticketId', e, st);
    }
  }

  Future<void> _maybeScheduleRecovery(AgentRunLog run) async {
    if (run.retry.attempt >= _maxRetries) {
      CcDomainLog.info('$_tag: Run ${run.id} exhausted retries ($_maxRetries) — skipping recovery',
      );
      return;
    }
    ...
    if (budget != null) {
      try {
        ...
        if (block != null) {
          CcDomainLog.warning('$_tag: Run ${run.id} recovery blocked by budget: ${block.reason}',
          );
          return;
        }
      } on Object catch (e, st) {
        CcDomainLog.error('$_tag: Budget check failed for run ${run.id}', e, st);
        return;
      }
    }

    CcDomainLog.info('$_tag: Run ${run.id} eligible for recovery '
