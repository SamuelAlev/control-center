...
class AgentReadinessChecker {
  ...
  Future<AgentReadinessResult> check(
    ...
    // belong to.
    //
    // Reached only by callers that already HOLD an agent — [checkFromId] reads
    // it out of [workspaceId]'s own database, so a foreign id resolves to "not
    // found" long before this. That makes the branch below the guard for an
    // agent obtained some other way (a cached entity, a wire payload), where the
    // id pairing has not been proven by the read itself.
  ...
  /// Looks up an agent by [agentId] within [workspaceId] and checks its
  /// readiness. [workspaceId] selects the workspace the agent is read from, so
  /// an id belonging to another workspace resolves to "not found".
