...
class HeartbeatRecoveryService {
  ...
  Future<void> evaluateAndSchedule(AgentRunLog run) async {
    if (run.errorFamily == RunErrorFamily.transientUpstream &&
      ...
      CcDomainLog.info(
        'HeartbeatRecovery: Scheduling retry ${run.retry.attempt + 1} '
    ...
    if (run.liveness == RunLiveness.stalled) {
      if (!shouldContinue(run)) {
        CcDomainLog.warning(
          'HeartbeatRecovery: Run ${run.id} exceeded max continuation '
      ...
      CcDomainLog.info(
        'HeartbeatRecovery: Scheduling continuation wake for '
    ...
    if (run.liveness == RunLiveness.dead) {
      CcDomainLog.warning(
        'HeartbeatRecovery: Run ${run.id} classified as dead — '
        'failing ticket',
      );
  ...
  Future<void> _scheduleRetry(AgentRunLog run, Duration delay) async {
    try {
      ...
      CcDomainLog.error(
        'HeartbeatRecovery: Failed to schedule retry for ${run.id}',
        e,
        st,
      );
    }
  ...
  Future<void> _scheduleContinuation(AgentRunLog run) async {
    try {
      ...
      CcDomainLog.error(
        'HeartbeatRecovery: Failed to schedule continuation for ${run.id}',
        e,
        st,
      );
    }
  ...
  Future<void> _failBackingTicket(AgentRunLog run, String reason) async {
    ...
    try {
      ...
      CcDomainLog.error(
        'HeartbeatRecovery: Failed to fail ticket $ticketId',
        e,
        st,
      );
    }
