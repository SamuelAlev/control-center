...
class ModeToolGuard {
  ...
  /// escape its mode by omitting `channel_id`. [workspaceId] scopes both
  /// lookups, so a channel or agent id from another workspace resolves to no
  ...
  Future<Mode?> resolveMode({
    ...
    final resolvedChannelId = await _resolveConversationId(
      workspaceId,
      channelId,
      agentId,
    );
    ...
    return _resolver.resolveForConversation(workspaceId, resolvedChannelId);
  }
  ...
  String refusalMessage(String toolName, Mode mode) => switch (mode) {
    ...
    Mode.review =>
      'Tool `$toolName` is not available in a review-mode conversation. '
          'Review channels are restricted to commentary, suggestions, '
