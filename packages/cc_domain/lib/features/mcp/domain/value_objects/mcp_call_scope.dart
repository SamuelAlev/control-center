...
/// it; `agent_id` / `conversation_id` / `channel_id` are conveniences injected
/// only when the caller omitted them.
class McpCallScope {
  /// Creates a call scope. Empty strings are treated as absent.
  const McpCallScope({this.workspaceId, this.agentId, this.conversationId});
  ...
  /// The conversation (channel) the agent runs in, injected into empty
  /// `conversation_id` / `channel_id` arguments.
  ...
  Map<String, dynamic> apply(
    ...
    fillIfEmpty('channel_id', conversationId);
    return scoped;
  }
}
