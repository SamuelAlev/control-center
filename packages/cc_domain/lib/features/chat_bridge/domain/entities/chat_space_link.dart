...
/// Maps one external chat conversation to one Control Center space.
///
/// The external side is a `(provider, team, space, thread)` tuple: a bot
/// `@mention` in a public space anchors on that message's thread, while a DM
...
/// externalThreadId)` is unique — one CC space per external thread — and
/// [ccSpaceId] is the reverse lookup the outbound relay uses to find where an
/// agent turn should stream.
class ChatSpaceLink {
  /// Creates a [ChatSpaceLink].
  ChatSpaceLink({
    required this.id,
    ...
    if (ccSpaceId.isEmpty) {
      throw ArgumentError.value(ccSpaceId, 'ccSpaceId', 'must not be empty');
    }
  }
  ...
  /// Provider-side conversation id (Slack space `C…`, group `G…` or DM `D…`).
  ...
  /// The Control Center space this external thread drives.
  ...
  ChatSpaceLink copyWith({DateTime? lastActivityAt}) => ChatSpaceLink(
    id: id,
