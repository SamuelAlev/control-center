/// The autonomy a principal may act with, ordered least→most (PRD 22 §3,
/// aligning with PRD 16 §12 / PRD 24 policy). A delegate can never exceed its
/// delegator's effective level — privilege cannot be laundered through
/// delegation.
enum AutonomyLevel {
  /// Observe / propose only; no effects without a human.
  observeOnly('observe_only'),

  /// Act, but effects require human approval.
  actWithApproval('act_with_approval'),

  /// Act freely within policy.
  ...
  /// Parses from a wire string (defaults to the safest level).
  static AutonomyLevel fromWire(String value) =>
      AutonomyLevel.values.firstWhere(
        (a) => a.wire == value,
        orElse: () => AutonomyLevel.observeOnly,
      );
