...
class TranscriptBlock {
  ...
  String get firstContent => events.isNotEmpty ? events.first.content : '';
    ...
    while (i < events.length) {
      ...
      if (event.type == AgentProcessEventType.toolCall ||
        ...
        if (group.length >= _groupThreshold) {
          ...
          for (final e in group) {
            blocks.add(
              TranscriptBlock(type: TranscriptBlockType.tool, events: [e]),
            );
          }
        }
        continue;
      ...
      blocks.add(
        TranscriptBlock(type: TranscriptBlockType.message, events: [event]),
      );
      i++;
    }
