...
class PrInboxData {
  ...
  PrInboxData({required this.sections})
    : assert(
        PrInboxSection.values.every(sections.containsKey),
...
class ClassifyPrInboxUseCase {
  ...
  /// (the operator's merged/closed PR history) for [currentUserLogin].
  ///
  ...
  /// GitHub teams (org → slugs, lower-cased) so a still-pending team request
  /// counts as Needs your review. [mergedWindow] bounds how far back
  /// [PrInboxSection.mergingAndMerged] reaches.
  PrInboxData execute({
