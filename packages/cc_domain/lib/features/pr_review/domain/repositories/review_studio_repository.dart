...
/// `review_spaces` stores). Server-side (association-first, cached GitHub
...
abstract class ReviewRunSnapshotRepository {
  ...
  /// Titles of findings a human dismissed, newest first, across every review
  /// in [workspaceId].
  ///
  /// This is the workspace's record of what it does not want to be told again,
  /// and it is the only durable one: a dismissal is the clearest feedback a
  /// reviewer ever gets, and until now nothing read it back. Titles rather
  /// than whole bodies because the title is the claim — the body is the
  /// argument for it, and it is the part that gets reworded between passes.
  ///
  /// CROSS-PR BY DESIGN, workspace-scoped: a pattern the team rejected on one
  /// pull request is a pattern they will reject on the next.
