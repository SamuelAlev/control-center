...
sealed class PrReviewer {
  ...
  /// Returns this reviewer with [isCodeOwner] forced on.
  ///
  /// Forges report code ownership only while the review request is still
  /// outstanding and drop the flag once the owner actually reviews, so the
  /// caller remembers every identity it has seen owning a PR and re-applies it.
  /// Without that, the shield disappears at exactly the moment it was earned.
  PrReviewer asCodeOwner();
}
...
class PrUserReviewer extends PrReviewer {
  ...
  @override
  PrUserReviewer asCodeOwner() =>
      PrUserReviewer(user: user, isCodeOwner: true, state: state);
...
class PrTeamReviewer extends PrReviewer {
  ...
  /// Team logo URL. Empty when the forge omitted it.
  ...
  @override
  PrTeamReviewer asCodeOwner() => PrTeamReviewer(
    name: name,
    slug: slug,
    isCodeOwner: true,
    state: state,
  ...
  @override
  int get hashCode =>
      Object.hash(slug, name, avatarUrl, isCodeOwner, state, reviewedBy);
}
...
class PrReviewerCandidate {
  ...
  /// Avatar URL for a user, or the team logo URL for a team. Null/empty
  /// falls back to initials (users) or the team glyph (teams).
  final String? avatarUrl;
