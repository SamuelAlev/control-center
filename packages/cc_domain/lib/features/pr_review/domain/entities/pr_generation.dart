...
class PrGeneration {
  ...
  PrGeneration({
    ...
    if (id.isEmpty) {
      throw ArgumentError('PrGeneration id must not be empty');
    }
  }
  ...
  PrGeneration markPublished() {
    // Thrown, not asserted: publishing is what opens a PR against a real
    // forge, and `assert` is stripped from the shipped server binary — so in
    // production this transition was unguarded, which is the one place it
    // most needed to be guarded.
    if (status is! Draft) {
      throw ArgumentError(
