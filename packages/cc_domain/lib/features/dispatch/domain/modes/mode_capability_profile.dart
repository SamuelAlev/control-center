...
class ModeCapabilityProfile {
  ...
  ///
  /// Only `ask_user` exists: it renders a form in the run's conversation and
  /// blocks on the human's answer. The set used to name `ask_user_question` and
  /// `request_confirmation` as well — two MCP tool classes that were deleted
  /// rather than wired — so pinning them pinned nothing, and the ONE name that
  /// needed to survive every filter was absent from the list meant to protect
  /// it. A pinned name that matches no tool is inert, which is why this drifted
  /// without failing anything.
  static const Set<String> interactionVerbs = {'ask_user'};
  ...
  /// Which of this mode's admitted tools ride every request, and which wait to
  /// be asked for.
  ///
  /// [enabled] is the field kill switch (`--tool-deferral=off`): false makes
  /// every admitted tool resident, i.e. byte-identical requests to the
  /// pre-deferral behaviour.
  ///
  /// The mode's pinned verbs are unioned in unconditionally. A run must never
  /// have to DISCOVER the call that delivers its deliverable, and the same goes
  /// for the interaction verbs, which are how it asks a human anything.
  ToolResidencySpec toToolResidencySpec({bool enabled = true}) =>
      ToolResidencySpec(
        enabled: enabled,
        residentNames: {
          ...ModeToolPolicy.residentNamesFor(mode),
          ...pinnedVerbs,
        },
      );
...
const Set<ActionClass> _readOnlyDeniedClasses = {
  ...
  // Booting and driving a machine is not reading, whatever the machine is
  // disposable. A review/plan-mode agent that needs to see a page asks for a
  // mode that can.
