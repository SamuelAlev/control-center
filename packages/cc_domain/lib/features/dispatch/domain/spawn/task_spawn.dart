...
///
/// Batch spawn validation: the batch
...
class TaskItem {
  ...
  TaskItem({
    ...
    if (id.trim().isEmpty) {
      throw ArgumentError('TaskItem.id must not be empty');
    }
    if (assignment.trim().isEmpty) {
      throw ArgumentError('TaskItem.assignment must not be empty');
    }
  }
...
class BatchSpawn extends SpawnRequest {
  ...
  const BatchSpawn({required this.context, required this.tasks});
