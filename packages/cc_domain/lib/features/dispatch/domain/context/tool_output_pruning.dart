...
/// Tunables for the conversation-budget pruner. Defaults keep the two most-recent turns pristine, protect the newest 40k
...
class ConversationPruner {
  ...
  PrunePlan plan(
    ...
    if (refs.isEmpty) {
      return const PrunePlan(
        updatedSegmentsByMessageId: {},
        reclaimedTokens: 0,
      );
    }
    ...
    for (final ref in refs) {
      ...
      if (survivor != null &&
          survivor != '${ref.messageId}#${ref.segmentIndex}') {
    ...
    if (gate == null || gate.permitsPrune) {
      ...
      if (budgetReclaim >= policy.minimumBenefitTokens) {
        for (final ref in budgetCandidates) {
          ref.segment = ref.segment.copyWith(
            outputs: prunedResultMarker,
            prunedAt: now,
          );
          changed.add(ref.messageId);
          reclaimed += ref.tokens;
  ...
  Set<String> _recentTurnMessageIds(List<Message> messages) {
