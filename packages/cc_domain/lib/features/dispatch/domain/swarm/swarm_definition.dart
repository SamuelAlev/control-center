...
class SwarmAgent {
  ...
  SwarmAgent({
    ...
    if (name.isEmpty) {
      throw ArgumentError('SwarmAgent.name must not be empty');
    }
    if (role.isEmpty) {
      throw ArgumentError('SwarmAgent.role must not be empty');
    }
    if (task.isEmpty) {
      throw ArgumentError('SwarmAgent.task must not be empty');
    }
  }
...
class SwarmDefinition {
  ...
  SwarmDefinition({
    ...
    if (name.isEmpty) {
      throw ArgumentError('SwarmDefinition.name must not be empty');
    }
    if (workspace.isEmpty) {
      throw ArgumentError('SwarmDefinition.workspace must not be empty');
    }
    if (agentOrder.length != agents.length) {
      throw ArgumentError('agentOrder must list every agent exactly once');
    }
  }
...
bool _shallowAgentsEqual(Map<String, SwarmAgent> a, Map<String, SwarmAgent> b) {
  if (identical(a, b)) {
    return true;
  }
  if (a.length != b.length) {
