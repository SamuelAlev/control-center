...
/// `submit_output` tool will reject a non-conforming payload with the exact
...
String renderOutputContract(
  ...
  final buf = StringBuffer()
    ...
    ..writeln(
      'When you finish, your `submit_output` `output` payload MUST be a '
  ...
  if (mode == OutputContractMode.strict) {
    buf.writeln(
      'The `submit_output` tool rejects a non-conforming payload and returns '
      'the exact list of violations — read them, fix the payload and call '
      '`submit_output` again. Do NOT wrap the payload in `{"result": ...}` '
