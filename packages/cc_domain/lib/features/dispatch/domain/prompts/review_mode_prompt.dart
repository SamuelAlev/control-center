...
tools:
  ...
  - list_pull_requests      — PR catalog / metadata
  ...
  - send_channel_message    — narrate progress / discuss a finding in the review channel
...
2. Ask the disagreeing agents to clarify via `send_channel_message` (reference
...
1. Fetch the PR description, diff, and commits first. Prefer the checked-out
   ...
   `bash` (`git diff`, `git log`, `git show`) for the patch. Fall back to
   `list_pull_requests` for metadata when you need the catalog view.
...
$briefSection
$repoSection
Start by inspecting the checked-out branch (or `list_pull_requests` for
catalog metadata), then proceed with the method described in your system
prompt.
''';
}
