...
/// the integration decides what dispose means (stop the dispatch, kill the
/// process, …). [dispatchId] lets the manager mirror the live dispatch into the
...
typedef PersistedSubagentReviverFactory =
    Future<AgentReviver?> Function(AgentRef ref);
...
class AgentLifecycleManager {
  ...
  Future<AgentSessionController> _revive(
    String id,
    AgentReviver reviver,
  ) async {
  ...
  void _armTimer(String id, _AdoptedAgent adopted) {
    ...
    adopted.timer?.cancel();
    adopted.timer = Timer(Duration(milliseconds: adopted.idleTtlMs), () {
      adopted.timer = null;
      unawaited(park(id));
    });
  }
