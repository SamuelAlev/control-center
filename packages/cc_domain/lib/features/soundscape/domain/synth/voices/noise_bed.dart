...
/// parameter in `[0, 1]`, then shaped by a per-channel low-pass. The two
/// channels draw independent noise streams so the bed is stereo-decorrelated.
///
...
class NoiseBed implements SoundscapeVoice {
  ...
  // Paul Kellet 7-state pink-filter state (per channel).
  final Float64List _pinkL = Float64List(7);
  final Float64List _pinkR = Float64List(7);

  // Leaky-integrator brown-noise state (per channel).
  double _brownL = 0.0;
  double _brownR = 0.0;
  ...
  void renderAdd(Float32List outStereo, int frames, double gain) {
    ...
    for (var i = 0; i < frames; i++) {
      ...
      // Right channel (independent noise stream).
