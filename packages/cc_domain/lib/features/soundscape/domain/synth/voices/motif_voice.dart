...
class MotifVoice implements SoundscapeVoice {
  ...
  }) : assert(partialGains.isNotEmpty, 'motif voice needs partials'),
       _sampleRate = sampleRate,
       _partialGains = List<double>.unmodifiable(partialGains),
       _norm = 1.0 / partialGains.fold<double>(0.0, (sum, g) => sum + g.abs()),
       _vibratoInc = vibratoRateHz.clamp(0.0, 12.0) / sampleRate,
       // Small-angle pitch mod: 2^(c/1200) - 1 ≈ c·ln2/1200 for the few
