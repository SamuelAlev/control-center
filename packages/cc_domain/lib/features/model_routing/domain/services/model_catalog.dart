...
class ModelCatalog {
  ...
  ///
  /// When the harness provider id differs from models.dev's (`kimi-code` vs
  /// `kimi-for-coding`, `zai` vs `zhipuai`), retries under
  /// [HarnessProviderMeta.modelsDevProviderId] so context, output ceiling and
  /// modalities still enrich.
  ModelInfo? resolve(String qualifiedId) {
    final ref = ModelRef.parse(qualifiedId);
    final direct = modelGet(ref.providerId, ref.modelId);
    if (direct != null) {
      return direct;
    }
    final alias = harnessProviderMetas[ref.providerId]?.modelsDevProviderId;
    if (alias == null || alias == ref.providerId) {
      return null;
    }
    return modelGet(alias, ref.modelId);
  }
