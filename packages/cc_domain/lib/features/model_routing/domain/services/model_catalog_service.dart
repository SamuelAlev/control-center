...
/// and refreshes in the background. The server fetches models.dev and caches
/// it under its data dir, then exposes the document over `models.catalog`.
/// Thin clients read that op rather than dialing models.dev or shipping a
/// source snapshot.
///
...
class ModelCatalogService {
  ...
  void startBackgroundRefresh() {
    _timer ??= Timer.periodic(_backgroundRefreshInterval, (_) {
      // Best-effort; failures fall back to the on-disk cache inside the source.
      unawaited(refresh(force: true));
    });
    // Kick a TTL-aware refresh without blocking the caller (and without
    // holding the ready banner on a first-run fetch).
