...
class SoftClaim {
  ...
  /// The claimed entity id (for `worktree`: the conversation/channel id).
...
class ParticipantPresence {
  ...
  /// The channel they are typing in right now, when any.
  final String? typingInChannelId;

  /// Live agent state (null for humans).
  final AgentLiveStatus? agent;

  /// The channel this participant is presenting (spotlight, PRD 16 §5).
  ...
  Map<String, dynamic> toWire() => {
    ...
    'a': availability.name,
    'l': ?locus?.toWire(),
    'ty': ?typingInChannelId,
    'ag': ?agent?.toWire(),
    'sp': ?spotlightChannelId,
