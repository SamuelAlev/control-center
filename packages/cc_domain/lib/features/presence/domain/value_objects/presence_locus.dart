...
sealed class PresenceLocus {
  ...
  static PresenceLocus? fromWire(Map<String, dynamic> wire) {
    switch (wire['t']) {
      case 'sp':
        return SpaceLocus(
          spaceId: wire['c'] as String? ?? '',
          conversationId: wire['cv'] as String?,
        );
      case 'file':
        return FileLocus(
...
/// Viewing / typing in a space, optionally down to the conversation.
class SpaceLocus extends PresenceLocus {
  /// Creates a [SpaceLocus].
  const SpaceLocus({required this.spaceId, this.conversationId});

  /// The space id — what `/workspaces/:w/spaces/:spaceId` names.
  final String spaceId;

  /// The conversation within [spaceId], when the participant is in a specific
  /// one. A space holds parallel conversations and a conversation id is NEVER
  /// a space id, so this cannot be folded into [spaceId]: following someone
  /// needs the space to route to AND the conversation to focus.
  ...
  @override
  int get hashCode => Object.hash('sp', spaceId, conversationId);
}
