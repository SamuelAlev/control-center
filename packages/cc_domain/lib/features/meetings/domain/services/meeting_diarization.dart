...
abstract interface class MeetingDiarizationPort {
  ...
  ///
  /// **[samples] is CONSUMED.** A full meeting is hundreds of megabytes of
  /// Float32 and an implementation is expected to move that buffer to its
  /// worker rather than copy it, which detaches it here. Read anything you
  /// need from [samples] BEFORE calling, and do not touch it afterwards.
...
/// testable.
int? assignSpeakerByOverlap(List<DiarizedSpan> spans, int startMs, int endMs) {
  var bestSpeaker = -1;
  var bestOverlap = 0;
...
List<MeetingSegment> _mergeTurns(
  List<MeetingSegment> segments,
  int mergeGapMs,
) {
  if (segments.length <= 1) {
  ...
  for (final seg in segments) {
    if (out.isNotEmpty) {
      ...
      final contiguous =
          ...
          (tinyFragment ? mergeGapMs * 2 : mergeGapMs);
      if (sameSpeaker && contiguous) {
        ...
        final dup =
            tokenContainment(prev.text, seg.text) >= 0.67 ||
