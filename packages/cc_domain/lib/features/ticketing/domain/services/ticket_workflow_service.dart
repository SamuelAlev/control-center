...
import 'package:cc_domain/features/ticketing/domain/ports/ticket_workflow_port.dart';
...
class TicketWorkflowService implements TicketWorkflowPort {
  ...
  /// already in the delegation chain, then runs the structural guards
  /// ([DelegationGuards.checkDepth] + [DelegationGuards.checkCycle]). On any
  /// refusal it throws a [DelegationRefusedException] carrying the guard's
  /// reason verbatim (loud denial, never a silent no-op). On allow it creates
  /// the child through the ordinary [createTicket] path.
  ///
  /// The autonomy-ceiling and budget-envelope guards are structural too, but
  /// need the delegator's effective autonomy + remaining budget, which are not
  /// cheaply available in the pure ticket domain — see the TODO below.
  Future<Ticket> delegateGuarded({
    ...
    // TODO(prd22): thread the delegator's effective autonomy level + remaining
    // budget envelope through so `checkAutonomyCeiling` + `checkBudgetEnvelope`
    // are enforced here as well (depth + cycle are the deterministic
    // structural guards enforced today).
