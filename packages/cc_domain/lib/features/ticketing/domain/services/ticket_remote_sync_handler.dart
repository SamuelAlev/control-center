...
class TicketRemoteSyncHandler {
  ...
  Future<void> _onCreated(TicketCreated event) async {
    final ticket = await repository.getById(event.workspaceId, event.ticketId);
  ...
  Future<void> _onStatusChanged(TicketStatusChanged event) async {
    ...
    try {
      ...
      CcDomainLog.error(
        'TicketRemoteSyncHandler: remote status push failed',
        e,
        st,
      );
    }
  ...
  Future<void> _onAssigned(TicketAssigned event) async {
    ...
    try {
      await providerPort.assign(ticket.externalKey!, ticket.assignedAgentId!);
    } on Object catch (e, st) {
