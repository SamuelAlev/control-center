...
class StrandedTicketReconciler {
  ...
  Future<void> reconcile() async {
    ...
    CcDomainLog.info(
      '$_tag: Reconciling tickets across ${agents.length} agent(s)',
    );
    ...
    for (final agent in agents) {
      try {
        ...
        CcDomainLog.error(
          '$_tag: Failed to reconcile agent ${agent.id}',
          e,
          st,
        );
      }
    ...
    CcDomainLog.info(
      '$_tag: Reconciliation finished (${seenTickets.length} ticket(s) inspected)',
    );
  }
  ...
  Future<void> _reconcileTicket(
    ...
    if (agent == null) {
      CcDomainLog.warning(
        '$_tag: Ticket ${ticket.id} assigned to non-existent agent '
    ...
    final ticketRuns = allRuns.where((r) => r.ticketId == ticket.id).toList()
      ..sort((a, b) => b.startedAt.compareTo(a.startedAt));

    if (ticketRuns.isEmpty) {
      CcDomainLog.info(
        '$_tag: Ticket ${ticket.id} has no runs — eligible for re-dispatch',
      );
    ...
    if (ticket.status == TicketStatus.inProgress) {
      ...
      if (retryCount >= _maxRetries) {
        CcDomainLog.warning(
          '$_tag: Ticket ${ticket.id} in-progress with exhausted retries '
  ...
  Future<void> _handleFailedRun(
    ...
    if (latestRun.retry.attempt >= _maxRetries) {
      CcDomainLog.warning(
        '$_tag: Ticket ${ticket.id} previous runs failed with exhausted retries '
    ...
    if (budget != null) {
      try {
        ...
        if (block != null) {
          CcDomainLog.warning(
            '$_tag: Ticket ${ticket.id} re-dispatch blocked by budget: ${block.reason}',
          );
          await _escalateToBlocked(ticket, 'Budget blocked: ${block.reason}');
          return;
        }
      } on Object catch (e, st) {
        CcDomainLog.error(
          '$_tag: Budget check failed for ticket ${ticket.id}',
          e,
          st,
        );
        return;
    ...
    CcDomainLog.info(
      '$_tag: Ticket ${ticket.id} eligible for re-dispatch '
  ...
  Future<void> _markForRedispatch(Ticket ticket, Agent agent) async {
    ...
    final updated = ticket.copyWith(status: TicketStatus.open, updatedAt: now);
    await _ticketRepo.update(updated);
    CcDomainLog.info(
      '$_tag: Ticket ${ticket.id} reset to open for re-dispatch (agent ${agent.id})',
    );
  }
  ...
  Future<void> _escalateToBlocked(Ticket ticket, String reason) async {
    if (!ticket.status.canTransitionTo(TicketStatus.blocked)) {
      CcDomainLog.warning(
        '$_tag: Cannot transition ticket ${ticket.id} from ${ticket.status} to blocked',
      );
      return;
    }
    ...
    try {
      ...
      CcDomainLog.info(
        '$_tag: Ticket ${ticket.id} escalated to blocked ($reason)',
      );
    } on Object catch (e, st) {
      CcDomainLog.error(
        '$_tag: Failed to escalate ticket ${ticket.id} to blocked',
        e,
        st,
      );
    }
