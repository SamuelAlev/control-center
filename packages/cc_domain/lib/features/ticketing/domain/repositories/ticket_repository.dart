...
abstract interface class TicketRepository {
  ...
  ///
  /// The id resolves only within [workspaceId]: a ticket owned by another
  /// workspace is simply not found.
  Future<Ticket?> getById(String workspaceId, String id);

  /// Fetches a ticket by its provider + external key, or null.
  ///
  /// The only lookup with no workspace id, because the caller is an inbound
  /// provider webhook that carries nothing else. Implementations resolve the
  /// owning workspace from `provider` + [externalKey] and must not search
  /// workspaces they were not pointed at.
  ...
  /// Adds a collaborator (idempotent on (ticketId, agentId)), scoped to
  /// [workspaceId].
  ...
  /// Removes a collaborator, scoped to [workspaceId].
  ...
  /// Watches the collaborators of a ticket, scoped to [workspaceId].
  Stream<List<TicketCollaborator>> watchCollaborators(
    String workspaceId,
    String ticketId,
  );

  /// Reads the collaborators of a ticket, scoped to [workspaceId].
