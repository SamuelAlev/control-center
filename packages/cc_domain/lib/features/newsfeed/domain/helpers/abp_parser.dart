...
/// Simple line-splitter for ABP filter rules that avoids importing dart:io.
///
/// Named `AbpLineSplitter` (not `LineSplitter`) so it does NOT shadow
/// `dart:convert`'s `LineSplitter` when this file is re-exported through a
/// barrel — the shadow previously broke any file that imported both
/// `dart:convert` and a barrel re-exporting this (the dispatch process source
/// lost its `StreamTransformer` typing and failed to compile).
class AbpLineSplitter {
  /// Creates a const [AbpLineSplitter].
  const AbpLineSplitter();
