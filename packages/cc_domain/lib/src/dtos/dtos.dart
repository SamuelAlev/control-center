...
/// Channel wire DTO: `{id, name, workspace_id, mode?, pipeline_run_id?,
...
class ChannelDto {
  ...
  /// Provisioning status (`ChannelProvisioningStatus.toDbValue()` string);
  /// null ⇒ `ready` (legacy rows / older servers).
  final String? provisioningStatus;

  /// Granular in-flight provisioning step
  /// (`ChannelProvisioningStep.toDbValue()` JSON string); null ⇒ no step
  ...
  /// How the channel came to exist (`ChannelOrigin.wire` string); null ⇒ `user`
  /// (legacy rows / older servers). Lets clients section agent-DM channels.
  final String? origin;
  final DateTime? createdAt;
  final DateTime? updatedAt;
...
/// metadata, channel_id?, conversation_id?, compacted?, created_at?}`.
class MessageDto {
  MessageDto({
    required this.id,
  ...
  /// The conversation (stream) inside the channel; null on lossy surfaces (the
  /// UI then treats it as the channel's `main` conversation, == channel id).
