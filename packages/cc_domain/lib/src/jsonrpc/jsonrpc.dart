...
class JsonRpcRequest {
  ...
  ///
  /// Throws a [FormatException] — not the constructor's [ArgumentError] — when
  /// `method` is missing. The distinction is load-bearing at the call sites:
  /// an ArgumentError from the CONSTRUCTOR means our own code built a nonsense
  /// request (a programming error), while a bad frame off the wire is DATA,
  /// and the transports already answer a FormatException with the correct
  /// `-32700 Parse error` / 400 rather than a 500 with a diagnostic id.
  factory JsonRpcRequest.fromJson(Map<String, dynamic> json) {
    final method = json['method'] as String? ?? '';
    if (method.isEmpty) {
      throw const FormatException('JSON-RPC request has no `method`');
    }
  ...
  JsonRpcRequest({
    ...
    if (method.isEmpty) {
      throw ArgumentError('JsonRpcRequest method must not be empty');
    }
  }
...
class JsonRpcResponse {
  ...
  JsonRpcResponse({this.jsonrpc = '2.0', required this.result, this.id}) {
    if (result.isEmpty) {
      throw ArgumentError('JsonRpcResponse result must not be empty');
    }
  }
...
class JsonRpcError {
  ...
  /// A missing `message` throws a [FormatException] (see
  /// [JsonRpcRequest.fromJson] for why that type and not [ArgumentError]).
  factory JsonRpcError.fromJson(Map<String, dynamic> json) {
    final message = json['message'] as String? ?? '';
    if (message.isEmpty) {
      throw const FormatException('JSON-RPC error frame has no `message`');
    }
    return JsonRpcError(
      jsonrpc: json['jsonrpc'] as String? ?? '2.0',
  ...
  JsonRpcError({
    ...
    if (message.isEmpty) {
      throw ArgumentError('JsonRpcError message must not be empty');
    }
  }
...
class JsonRpcNotification {
  ...
  /// A missing `method` throws a [FormatException] (see
  /// [JsonRpcRequest.fromJson] for why that type and not [ArgumentError]).
  factory JsonRpcNotification.fromJson(Map<String, dynamic> json) {
    final method = json['method'] as String? ?? '';
    if (method.isEmpty) {
      throw const FormatException('JSON-RPC notification has no `method`');
    }
  ...
  JsonRpcNotification({
    ...
    if (method.isEmpty) {
      throw ArgumentError('JsonRpcNotification method must not be empty');
    }
  }
