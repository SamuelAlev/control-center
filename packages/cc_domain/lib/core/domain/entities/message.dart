...
/// Rendering type of a space message.
...
enum MessageType {
  ...
  /// messages. Its [Message.content] is the anchored summary text; the
  ...
  /// A mid-run steering nudge queued for a live agent run. Written by a human
  /// while the agent works; delivered by injection into the run at a turn
  /// boundary (or converted to a plain [text] message when the run ends
  /// first). The lifecycle rides in metadata — see `Message.steerState`. A
  /// queued one renders in the composer's steering queue strip, not the trail;
  /// an injected one renders in the trail with a "steered" badge.
  steering,
}

/// Who sent a space message.
enum SenderType {
  /// Human user.
  user,
...
/// A message inside a messaging space.
class Message {
  ...
  Message({
    ...
    if (spaceId.isEmpty) {
      throw ArgumentError('Message spaceId must not be empty');
    }
    if (conversationId.isEmpty) {
      throw ArgumentError('Message conversationId must not be empty');
    }
  }
  ...
  /// The conversation (stream) inside the space this message belongs to.
  /// A space holds flat, co-equal conversations, each with its own uuid; they
  /// share the space's worktree but keep their own message history and agent
  /// sessions. A conversation with an `anchorMessageId` is a thread.
  ...
  bool get isUser => senderType == SenderType.user;

  /// Whether this is a system message.
  bool get isSystem => messageType == MessageType.system;

  /// Whether this is a ticket card.
  bool get isTicket => messageType == MessageType.ticketCard;

  /// Whether this is a complete agent turn with a structured transcript.
  bool get isAgentTurn => messageType == MessageType.agentTurn;

  /// Whether this is a review node.
  bool get isReviewNode => messageType == MessageType.reviewNode;

  /// Whether this is a hire proposal awaiting approval.
  bool get isHireProposal => messageType == MessageType.hireProposal;

  /// Whether this is an editorial review summary.
  bool get isReviewSummary => messageType == MessageType.reviewSummary;

  /// Whether this is a plan message.
  bool get isPlan => messageType == MessageType.plan;

  /// Whether this is an agent question rendered as an interactive form.
  bool get isUserQuestion => messageType == MessageType.userQuestion;
  ...
  bool get isArtifact => messageType == MessageType.artifact;

  /// Whether this is a first-class compaction summary.
  bool get isCompaction => messageType == MessageType.compaction;

  /// Whether this is a mid-run steering message.
  bool get isSteering => messageType == MessageType.steering;

  /// Lifecycle of a [MessageType.steering] message, from metadata
  /// (`metadata['steerState']`): `'queued'` while it waits to be injected
  /// (rendered in the steering queue strip), `'injected'` once a run has
  /// taken it (rendered in the trail), and null for every other message.
  String? get steerState {
    if (!isSteering) {
      return null;
    }
    return metadata?['steerState'] as String?;
  }

  /// Whether this steering message is still waiting to be delivered.
  bool get isSteeringQueued => steerState == 'queued';

  /// Whether a run has injected this steering message.
  bool get isSteeringInjected => steerState == 'injected';

  /// Position of a queued steering message in the user-ordered queue
  /// (`metadata['steerOrder']`, ascending). Falls back to `createdAt`
  /// milliseconds so rows written before any reorder still sort stably.
  int get steerOrder =>
      metadata?['steerOrder'] as int? ?? createdAt.millisecondsSinceEpoch;

  /// The id of the run that injected this steering message, when known.
  String? get steerRunId => metadata?['steerRunId'] as String?;

  /// When the run injected this steering message (epoch ms), when known.
  int? get steerInjectedAt => metadata?['steerInjectedAt'] as int?;

  /// Whether a queued steering message was converted into a normal user
  /// message when its run ended undelivered (`metadata['convertedFromSteering']`).
  bool get convertedFromSteering => metadata?['convertedFromSteering'] == true;

  /// Whether this message stands in for older context — either a first-class
  /// [MessageType.compaction] message or a legacy compacted system
  ...
  /// Attachments decoded from `metadata['attachments']`, in send order.
  ///
  /// Tolerant: the list is free-form JSON a client wrote, so a malformed entry
  /// is skipped rather than throwing inside a transcript build — one bad row
  /// would blank the whole conversation.
  List<MessageAttachment> get attachments =>
      MessageAttachment.attachmentsFromMetadata(metadata);
