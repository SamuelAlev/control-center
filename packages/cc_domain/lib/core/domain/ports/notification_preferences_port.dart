...
abstract interface class NotificationPreferencesPort {
  ...
  /// Repositories whose notifications are suppressed, as lowercased
  /// `owner/name`.
  ///
  /// A mute is a fact about one person's attention, not about a workspace, so
  /// it rides the per-user preference lane alongside the category toggles
  /// rather than becoming a server row. It is applied when a stored or pushed
  /// frame is rendered, which is what makes it cover the bell history and the
  /// toast with one check — and why un-muting brings the suppressed history
  /// back rather than having discarded it.
  Future<Set<String>> getMutedRepos();

  /// Mutes or unmutes [repoFullName] (`owner/name`, case-insensitive).
  Future<void> setRepoMuted(String repoFullName, {required bool muted});
