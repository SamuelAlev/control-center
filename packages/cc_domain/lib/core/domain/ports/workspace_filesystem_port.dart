...
///
/// `dart:io`-free: directory accessors return absolute path strings;
/// [readSkillFile] returns content or null. Web binds `RpcWorkspaceFilesystemPort`
/// (`cc_data`); VM uses `WorkspaceFilesystemService` (`cc_infra`, re-exports
/// this symbol). Returned paths are server-side opaque tokens for clients.
abstract interface class WorkspaceFilesystemPort {
  /// Workspace dir path.
  Future<String> workspaceDir(String workspaceId);

  /// Spaces container dir path (parent of all per-space folders).
  Future<String> spacesDir(String workspaceId);

  /// Per-SPACE working dir path. Used as the sandbox terminal's `pwd`.
  ///
  /// Keyed by the space, never by a conversation: the repo worktrees under it
  /// are what every conversation in the space works in, so two conversations
  /// in one space share one checkout (and one clone) rather than each paying
  /// for their own.
  Future<String> spaceDir(String workspaceId, String spaceId);

  /// Creates [spaceDir] if it doesn't exist and returns its path.
  Future<String> ensureSpaceDir(String workspaceId, String spaceId);
  ...
  ///
  /// Desktop/server only: a web client has no readable [sourcePath] (browsers
  /// don't expose real filesystem paths), so it must use [persistLogoBytes]
  /// instead.
  Future<String?> persistLogo(String workspaceId, String sourcePath);

  /// Writes [bytes] into the workspace's own directory as `logo<extension>`
  /// and returns the absolute path of the copy. The bytes-based counterpart to
  /// [persistLogo]: this is the only way to persist a logo from a web client,
  /// where the picked file has no readable server-side path.
  ///
  /// [extension] should include the leading dot (e.g. `.png`); an empty value
  /// writes a bare `logo` file. Returns the persisted path, or null when
  /// [bytes] is empty.
  ...
  Future<String> prCloneDir(String workspaceId, String owner, String repo);
