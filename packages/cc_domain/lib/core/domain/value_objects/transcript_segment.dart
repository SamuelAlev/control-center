...
enum TurnOutcome {
  ...
  /// The loop hit its turn ceiling (`LoopDoneReason.maxTurns`): the process
  /// exited cleanly but the task was left unfinished mid-flight. Distinct from
  /// [completed] so a space can say "stopped at the turn limit" instead of
  /// looking like the agent chose to stop.
  maxTurns,
}
...
/// `Message.metadata['segments']` as a JSON list (discriminated by the
...
sealed class TranscriptSegment {
  ...
  static TranscriptSegment fromJson(Map<String, dynamic> json) {
    ...
    switch (json['type'] as String?) {
      case 'tool':
        final prunedAtRaw = (json['prunedAt'] as num?)?.toInt();
        return ToolSegment(
          ...
          prunedAt: prunedAtRaw == null
              ? null
              : DateTime.fromMillisecondsSinceEpoch(prunedAtRaw),
          images: [
            for (final raw in (json['images'] as List?) ?? const [])
              if (raw is Map)
                ?ToolImageRef.fromJson(raw.cast<String, dynamic>()),
          ],
        );
      case 'text':
        return TextSegment(
...
class ReasoningSegment extends TranscriptSegment {
  ...
  ReasoningSegment copyWith({String? text, int? durationMs}) =>
      ReasoningSegment(
        text: text ?? this.text,
        startedAt: startedAt,
...
class ToolSegment extends TranscriptSegment {
  ...
  /// When this tool's output was pruned/elided to reclaim context, or null if
  /// it still carries its full result. When set, [outputs] has been replaced by
  /// a short marker (see the context-pruning utilities) — the segment is kept
  /// so the call/result pairing and the transcript order stay intact, but the
  /// fat body no longer consumes context.
  ...
  /// through every message query and into the FTS index for content no search
  /// ever matches. The reference resolves through the host's blob endpoint.
  ///
  /// Empty for the overwhelming majority of tool calls, so it serializes only
  /// when non-empty and costs an existing transcript nothing.
  final List<ToolImageRef> images;

  /// Whether this tool's output has been pruned/elided.
  bool get isPruned => prunedAt != null;
  ...
  ToolSegment copyWith({
    ...
    startedAt: startedAt,
    durationMs: durationMs ?? this.durationMs,
    prunedAt: prunedAt ?? this.prunedAt,
  ...
  Map<String, dynamic> toJson() => {
    ...
    if (durationMs != null) 'durationMs': durationMs,
    if (prunedAt != null) 'prunedAt': prunedAt!.millisecondsSinceEpoch,
...
/// A reference to one image a tool returned, resolved through the host's blob
/// endpoint rather than carried inline.
class ToolImageRef {
  /// Creates a [ToolImageRef].
  const ToolImageRef({
    required this.ref,
  ...
  /// Parses one entry produced by [toJson]. Returns null for a malformed entry
  /// so one bad image cannot cost the whole transcript segment.
  static ToolImageRef? fromJson(Map<String, dynamic> json) {
    ...
    return ToolImageRef(
      ref: ref,
      mediaType: json['mediaType'] as String? ?? 'image/png',
  ...
  /// `blob:sha256:<hex>` — the stored content's address.
  final String ref;

  /// MIME type of the stored bytes.
  final String mediaType;

  /// Size in bytes, when known. Shown in the UI so a large frame is legible as
  /// a cost rather than a mystery.
  final int? bytes;

  /// Pixel width, when the producer reported it.
  final int? width;

  /// Pixel height, when the producer reported it.
  final int? height;

  /// Serializes for transcript persistence.
  Map<String, dynamic> toJson() => {
    'ref': ref,
    'mediaType': mediaType,
  ...
  @override
  int get hashCode => Object.hash(ref, mediaType, bytes, width, height);
}
...
class ErrorSegment extends TranscriptSegment {
  ...
  /// Where the code came from (e.g. `anthropic`, `relay`).
  final String? source;
