...
/// (`messaging.watchChannelTurns`) to ship per-segment updates from the
...
/// One event on the live turn relay (`messaging.watchChannelTurns`), decoded
/// from a wire frame by [channelTurnEventFromWire].
sealed class ChannelTurnEvent {
  const ChannelTurnEvent();
}

/// The first frame of a relay subscription: every turn currently streaming in
/// the channel, each with its full segment snapshot so a late subscriber (or a
/// reconnect) adopts the in-flight state with no lost prefix.
class TurnRelaySeed extends ChannelTurnEvent {
...
class TurnRelayUpdates extends ChannelTurnEvent {
...
/// Decodes a relay frame into a [ChannelTurnEvent], or null for unknown
/// frames (tolerant, so protocol additions degrade to a skip).
ChannelTurnEvent? channelTurnEventFromWire(Map<String, dynamic> frame) {
...
///
/// Run-scoped rather than channel-scoped, so no id rides each frame: the
/// subscription already names the run. Deliberately not reusing
/// [ChannelTurnEvent] — that would force a run id into a `message_id` field and
