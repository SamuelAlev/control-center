...
class DomainEventAuditBridge {
  ...
  void start() {
    _subs
      ..add(
        _eventBus.on<TicketAssigned>().listen((e) {
          _logger.log(
      ...
      ..add(
        _eventBus.on<TicketStatusChanged>().listen((e) {
      ...
      ..add(
        _eventBus.on<AgentRunCompleted>().listen((e) {
      ...
      ..add(
        _eventBus.on<OrchestrationApproved>().listen(
          (e) => _orch(e, 'approved'),
        ),
      )
      ..add(
        _eventBus.on<OrchestrationExecutionStarted>().listen(
          (e) => _orch(e, 'execution_started'),
        ),
      )
      ..add(
        _eventBus.on<OrchestrationCompleted>().listen(
          (e) => _orch(e, 'completed'),
        ),
      )
      ..add(
        _eventBus.on<OrchestrationFailed>().listen((e) => _orch(e, 'failed')),
      )
      ..add(
        _eventBus.on<OrchestrationCancelled>().listen(
          (e) => _orch(e, 'cancelled'),
        ),
      );
  }
