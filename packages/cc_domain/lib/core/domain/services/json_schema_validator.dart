...
class JsonSchemaValidator implements SchemaValidatorPort {
  ...
  void _validateSchemaDoc(Object? schema, String path, List<String> out) {
    if (schema is! Map) {
    ...
    switch (type) {
      ...
      case 'array':
        ...
        if (minItems != null && list.length < minItems) {
          out.add(
            '$path: expected at least $minItems items, got ${list.length}',
          );
        }
        final maxItems = (schema['maxItems'] as num?)?.toInt();
        if (maxItems != null && list.length > maxItems) {
          out.add(
            '$path: expected at most $maxItems items, got ${list.length}',
          );
        }
        final items = schema['items'];
        if (items is Map) {
