...
void main() {
  group('JsonRpcRequest', () {
    ...
    test('rejects an empty method', () {
      expect(() => JsonRpcRequest(method: ''), throwsA(isA<ArgumentError>()));
    });
  ...
  group('JsonRpcResponse', () {
    ...
    test('rejects an empty result', () {
      expect(() => JsonRpcResponse(result: {}), throwsA(isA<ArgumentError>()));
    });
  ...
  group('a malformed frame is a FormatException, not an ArgumentError', () {
    // The type is load-bearing at the call sites: the transports answer a
    // FormatException with `-32700 Parse error` / 400, and everything else
    // with a 500 and a diagnostic id. A bad frame off the wire is DATA, not a
    // programming error, so it must be the former.
    test('a request with no method', () {
      expect(
        () => JsonRpcRequest.fromJson({'jsonrpc': '2.0', 'id': 1}),
        throwsFormatException,
      );
      expect(
        () => JsonRpcRequest.fromJson({'method': '', 'id': 1}),
        throwsFormatException,
      );
    });

    test('a notification with no method', () {
      expect(
        () => JsonRpcNotification.fromJson({'jsonrpc': '2.0'}),
        throwsFormatException,
      );
    });

    test('an error frame with no message', () {
      expect(() => JsonRpcError.fromJson({'code': -1}), throwsFormatException);
    });

    test('building one in code is still an ArgumentError', () {
      // Our own code constructing a method-less request is a bug in us, and it
      // should not be reported to a peer as their parse error.
      expect(() => JsonRpcRequest(method: ''), throwsArgumentError);
      expect(() => JsonRpcError(code: -1, message: ''), throwsArgumentError);
    });
  });
}
