...
/// MessageMention codec and the equality contract for [Message].
void main() {
  ...
  group('Message type predicates', () {
    ...
    test('isUser tracks senderType', () {
      ...
      expect(make(MessageType.text).isUser, isFalse);
    });

    test('each messageType flag maps to its enum value', () {
      expect(make(MessageType.system).isSystem, isTrue);
      expect(make(MessageType.ticketCard).isTicket, isTrue);
      expect(make(MessageType.agentTurn).isAgentTurn, isTrue);
      expect(make(MessageType.reviewNode).isReviewNode, isTrue);
      expect(make(MessageType.hireProposal).isHireProposal, isTrue);
      expect(make(MessageType.reviewSummary).isReviewSummary, isTrue);
      expect(make(MessageType.plan).isPlan, isTrue);
      expect(make(MessageType.userQuestion).isUserQuestion, isTrue);
      expect(
        make(MessageType.orchestrationProposal).isOrchestrationProposal,
        isTrue,
      );
      expect(make(MessageType.artifact).isArtifact, isTrue);
      expect(make(MessageType.compaction).isCompaction, isTrue);
    });

    test('a text message is none of the type flags', () {
      final message = make(MessageType.text);
  ...
  group('Message metadata-derived accessors', () {
    test('isContextSummary covers both first-class and legacy compaction', () {
      expect(
        make(MessageType.compaction).isContextSummary,
        isTrue,
      ...
      // summary.
      expect(make(MessageType.system).isContextSummary, isFalse);
    });
    ...
    test('compaction accessors read metadata keys with fallbacks', () {
      ...
      final bare = make(MessageType.compaction);
    ...
    test('question / plan / thread / edit / delete flags read metadata', () {
      ...
      expect(make(MessageType.userQuestion).isQuestionAnswered, isFalse);
      ...
      expect(make(MessageType.plan).planStatus, 'pending');
      ...
      expect(make(MessageType.text).isStreamingComplete, isFalse);

      expect(
        make(MessageType.text, meta: const {'editedAt': 1}).isEdited,
        isTrue,
      );
      expect(make(MessageType.text).isEdited, isFalse);

      expect(
        make(MessageType.text, meta: const {'deletedAt': 1}).isDeleted,
        isTrue,
      );
      expect(make(MessageType.text).isDeleted, isFalse);
    });
    ...
    test('isReverted mirrors the reverted flag', () {
      ...
      expect(make(MessageType.text).isReverted, isFalse);
    });
  });

  group('Message metadata mutators', () {
    final base = Message(
      id: 'm1',
      spaceId: 'ch1',
      conversationId: 'ch1',
  ...
  group('Message equality', () {
    ...
    test('a differing field breaks equality', () {
      ...
      expect(make(spaceId: 'a') == make(spaceId: 'b'), isFalse);
      expect(
    ...
    test('a non-Message is never equal', () {
  ...
  group('Message.copyWith', () {
    ...
    test('a single-field copyWith preserves every other field', () {
      ...
      expect(next.id, 'm1');
      expect(next.spaceId, 'ch1');
      expect(next.conversationId, 'ch1');
      expect(next.senderId, 'u1');
      expect(next.senderType, SenderType.user);
      expect(next.messageType, MessageType.text);
...
Message make(MessageType type, {Map<String, dynamic>? meta}) =>
    Message(
      id: 'm1',
      spaceId: 'ch1',
      conversationId: 'ch1',
