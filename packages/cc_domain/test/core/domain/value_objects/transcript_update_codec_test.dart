...
void main() {
  ...
  group('channelTurnEventFromWire', () {
    test('decodes a seed frame with turns and their snapshots', () {
      ...
      final event = channelTurnEventFromWire(frame);
    ...
    test('seed frame with a non-list turns payload degrades to empty', () {
      final event = channelTurnEventFromWire(const {'kind': 'seed'});
      expect(event, isA<TurnRelaySeed>());
    ...
    test('updates frame without a message id yields null', () {
      expect(
        channelTurnEventFromWire(const {'kind': 'updates', 'updates': []}),
        isNull,
      );
    });

    test('unknown frame kinds yield null', () {
      expect(channelTurnEventFromWire(const {'kind': 'wat'}), isNull);
      expect(channelTurnEventFromWire(const {}), isNull);
    });
  });
