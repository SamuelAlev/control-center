...
void main() {
  ...
  group('ModelFuzzySearch', () {
    ...
    test('fuzzy subsequence match ranks the obvious hit first', () {
      final hits = ModelFuzzySearch.search('opus', ids, limit: 3);
      expect(hits.first, 'anthropic/claude-opus-4-5');
    });

    test('matches across separators', () {
      final hits = ModelFuzzySearch.search('gpt5nano', ids);
      expect(hits.first, 'openai/gpt-5-nano');
    });

    test('empty query returns the head', () {
      expect(ModelFuzzySearch.search('', ids, limit: 2), ids.take(2));
    });
  });
}
