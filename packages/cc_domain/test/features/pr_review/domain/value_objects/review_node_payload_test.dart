...
/// Coverage for the typed view over a `reviewNode` space-message metadata
...
void main() {
  ...
  group('ReviewNodePayload', () {
    ...
    group('triage axes', () {
      ...
      test('a payload written before the axes existed still parses', () {
        // The whole point of making them optional: an old finding must keep
        // rendering rather than dropping out of the review.
        final p = ReviewNodePayload.fromMetadata(legacyMeta);
        expect(p, isNotNull);
        expect(p!.category, isNull);
        expect(p.severity, isNull);
        expect(p.effort, isNull);
        expect(p.fixDiff, isNull);
        expect(p.aiPrompt, isNull);
      });

      test('effectiveSeverity falls back to the priority mapping', () {
        final p = ReviewNodePayload.fromMetadata(legacyMeta)!;
        expect(p.effectiveSeverity, ReviewFindingSeverity.major);
      });

      test('effectiveSeverity prefers the stored severity', () {
        final p = ReviewNodePayload.fromMetadata({
          ...legacyMeta,
          'severity': 'trivial',
        })!;
        expect(p.effectiveSeverity, ReviewFindingSeverity.trivial);
      });

      test('round-trips the axes through metadata', () {
        final p = ReviewNodePayload.fromMetadata({
          ...legacyMeta,
          ...
          'effort': 'heavy_lift',
          'fixDiff': '- old\n+ new',
          'aiPrompt': 'Await the future.',
        })!;
        expect(p.category, ReviewFindingCategory.dataIntegrity);
        expect(p.severity, ReviewFindingSeverity.critical);
        expect(p.effort, ReviewFindingEffort.heavyLift);
        expect(p.fixDiff, '- old\n+ new');
        expect(p.aiPrompt, 'Await the future.');

        final round = ReviewNodePayload.fromMetadata(p.toMetadata());
        expect(round, p);
      });

      test('drops unrecognized axis values instead of failing the parse', () {
        // A mislabelled finding is still a finding; bouncing it would lose
        // the expensive part of a review pass.
        final p = ReviewNodePayload.fromMetadata({
          ...legacyMeta,
          'category': 'vibes',
      ...
      test('treats a blank fix or prompt as absent', () {
        final p = ReviewNodePayload.fromMetadata({
          ...legacyMeta,
          'fixDiff': '   ',
          'aiPrompt': '',
      ...
      test('omits the axis keys from metadata when unset', () {
        final meta = ReviewNodePayload.fromMetadata(legacyMeta)!.toMetadata();
      ...
      test('severity maps onto the verdict-gating priority', () {
        expect(
          ReviewFindingSeverity.critical.toPriority(),
          ReviewNodePriority.p0,
        );
        expect(ReviewFindingSeverity.major.toPriority(), ReviewNodePriority.p1);
        expect(ReviewFindingSeverity.minor.toPriority(), ReviewNodePriority.p2);
        expect(
          ReviewFindingSeverity.trivial.toPriority(),
          ReviewNodePriority.p3,
        );
        expect(ReviewFindingSeverity.info.toPriority(), ReviewNodePriority.p3);
      });

      test('atLeast orders most-severe-first', () {
        expect(
          ReviewFindingSeverity.critical.atLeast(ReviewFindingSeverity.minor),
          isTrue,
        );
        expect(
          ReviewFindingSeverity.trivial.atLeast(ReviewFindingSeverity.minor),
          isFalse,
        );
        expect(
          ReviewFindingSeverity.minor.atLeast(ReviewFindingSeverity.minor),
          isTrue,
        );
      });

      test('the agent-prompt guard preamble is non-empty', () {
        // Rendered by the consumers rather than accepted from the filing
        // agent — a payload that could supply its own guard could omit it.
        expect(kAiAgentPromptGuardPreamble.trim(), isNotEmpty);
      });
    });
  });
}
