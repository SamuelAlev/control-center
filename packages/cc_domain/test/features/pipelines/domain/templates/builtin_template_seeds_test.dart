...
void main() {
  ...
  group('builtInTemplateSeeds', () {
    ...
    test('index_code dispatches the librarian when one is provided', () {
      ...
      // The analyzer's conversation clones only the repo being indexed, not
      // every workspace repo (heavy IO/FS) — repoId is always in the payload
      // (RepoAdded event / required repo input).
      expect(index.step('analyze')?.config.repoIds, ['{{repoId}}']);
    });
