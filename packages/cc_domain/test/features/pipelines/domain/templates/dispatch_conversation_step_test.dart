...
class _FakeMessagingPort implements MessagingPort {
  ...
  /// The repo scope each created channel was given (empty = all repos).
...
void main() {
  ...
  group('dispatchConversationStep — repo scope', () {
    test('scopes the hidden conversation to the configured repo ids', () async {
      final messaging = _FakeMessagingPort();
      ...
      await dispatchConversationStep(
        ...
        agentIds: ['agent-1'],
        prompt: 'Do the work',
        repoIds: const ['repo-a', 'repo-b'],
      );

      expect(messaging.createdWithRepoIds.single, ['repo-a', 'repo-b']);
    });

    test('renders {{placeholders}} in repo ids against pipeline state', () async {
      final messaging = _FakeMessagingPort();
      ...
      expect(messaging.createdWithRepoIds.single, ['repo-1']);
    });

    test(
      'falls back to all repos when the scope cannot be resolved',
      () async {
        // A seeded template's `['{{repoId}}']` on a trigger path whose payload
        // has no repo must degrade to the legacy every-repo provisioning
        ...
        await dispatchConversationStep(
          ...
          agentIds: ['agent-1'],
          prompt: 'Do the work',
          repoIds: const ['{{missingKey}}'],
        );

        expect(messaging.createdChannels, hasLength(1));
        expect(messaging.createdWithRepoIds.single, isEmpty);
      },
    );

    test('a reused conversation keeps its own repo selection', () async {
      final messaging = _FakeMessagingPort(existingChannels: {'chan-plan'});
      ...
      await dispatchConversationStep(
        ...
        prompt: 'Do the work',
        channelId: 'chan-plan',
        repoIds: const ['repo-a'],
      );

      expect(
        messaging.createdWithRepoIds,
        isEmpty,
        reason: 'the scope only applies when the step creates the channel',
      );
    });
  });
}
