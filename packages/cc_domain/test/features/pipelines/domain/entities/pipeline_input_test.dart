...
void main() {
  ...
  group('PipelineInput construction', () {
    test('defaults label to key when no label is supplied', () {
      final input = PipelineInput(key: 'repo_id');
      expect(input.key, 'repo_id');
      expect(input.label, 'repo_id');
    ...
    test('defaults label to key when an empty label is supplied', () {
      final input = PipelineInput(key: 'repo_id', label: '');
      expect(input.label, 'repo_id');
    });
    ...
    test('throws an ArgumentError when the key is empty', () {
