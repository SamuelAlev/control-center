...
void main() {
  ...
  group('ConversationGoal construction', () {
    ...
    test('rejects a blank title', () {
      expect(() => goal(title: '   '), throwsA(isA<ArgumentError>()));
    });

    test('rejects an empty workspaceId', () {
      expect(() => goal(workspaceId: ''), throwsA(isA<ArgumentError>()));
    });

    test('rejects an empty spaceId', () {
      expect(() => goal(spaceId: ''), throwsA(isA<ArgumentError>()));
    });
  ...
  group('ConversationGoal.copyWith', () {
    test('returns a copy with the given field replaced', () {
      ...
      expect(updated.spaceId, g.spaceId);
  ...
  group('ConversationGoal equality', () {
    ...
    test('is unequal when the space differs', () {
      expect(goal(spaceId: 'c-1'), isNot(goal(spaceId: 'c-2')));
    });
