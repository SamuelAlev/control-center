...
Message _user(String id, {int minute = 0}) => Message(
  id: id,
  spaceId: 'c',
  conversationId: 'c',
  senderId: 'user',
  ...
  group('budget pruning', () {
    test('protects newest output then prunes the rest past the minimum', () {
      ...
      for (var i = 0; i < 12; i++) {
        ...
        messages.add(
          _turn('t$i', [_tool('bash', 'X' * 38000)], minute: minute++),
        );
      }
      ...
      final plan = pruner.plan(messages, now: now, gate: CachePruneGate.always);
