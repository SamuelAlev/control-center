...
Message _turn(String answer, {List<String> tools = const []}) {
...
Message _summary(String text) => Message(
  id: 's${_seq++}',
  spaceId: 'c',
  conversationId: 'c',
  senderId: 'system',
...
void main() {
  ...
  test('100-turn conversation compacts to summary + last N turns', () {
    ...
    expect(plan!.messagesToCompact.first.content, contains('snake_case'));
  ...
  test('re-anchors after a prior compaction: only newer region folds', () {
    final messages = <Message>[];
    messages.add(
      _summary('## Conversation summary (compacted)\nearlier stuff'),
    );
    for (var i = 0; i < 20; i++) {
      messages.add(_user('q$i'));
      messages.add(_turn('a$i'));
