...
void main() {
  ...
  group(
    'scope precedence (channel > agent > workspace > preset > default)',
    () {
      test('channel rule wins over agent, workspace and mode preset', () {
        final rules = [
          _rule(
        ...
        // channel rule is found first in the scope chain and decides.
        ...
        expect(r.source, 'channel');
        expect(r.rule, isNotNull);
      });

      test('agent rule wins when no channel rule matches', () {
        final rules = [
          _rule(
            scopeType: ActionScopeType.agent,
            scopeId: 'a1',
  ...
  group('scope chain without a channel', () {
    test('resolveClass(gitPush, channelId: null, agentId: a1) with an agent '
