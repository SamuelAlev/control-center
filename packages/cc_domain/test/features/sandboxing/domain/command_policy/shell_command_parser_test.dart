...
void main() {
  group('parseShellCommand', () {
    ...
    test('pipe splits commands', () {
      expect(parseShellCommand('git log | grep foo'), ['git log', 'grep foo']);
    });
  ...
  group('normalizeCommandTokens', () {
    test('strips leading path from executable', () {
      expect(normalizeCommandTokens('/usr/bin/git push'), ['git', 'push']);
    });

    test('collapses extra spaces', () {
      expect(normalizeCommandTokens('git   push'), ['git', 'push']);
    });
  });
