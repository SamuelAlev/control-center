...
class SubagentSpawnRequest {
  ...
  /// The raw `subagent_type` string the model supplied.
  ///
  /// Carried alongside [type] rather than replacing it: a FILE-defined agent
  /// has a name the enum does not know, but it still resolves to one of the
  /// built-in types for its tier ceiling. The host uses this to look the
  /// definition up; every safety check keeps using [type].
