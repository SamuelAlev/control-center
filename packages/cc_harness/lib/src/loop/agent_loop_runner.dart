...
class AgentLoopRunner implements AgentLoop {
  ...
  Stream<AgentLoopEvent> run({
    ...
    final toolSchemas = tools.map((t) => t.toSchema()).toList();
    ...
    // Context management: overhead (system + tool schemas) is constant across
    // the run, so estimate it once for the compaction budget.
    ...
    final overheadTokens = compactor == null
        ? 0
        : _overheadTokens(config.systemPrompt, toolSchemas) + config.maxTokens;
    ...
    for (var turn = 0; maxTurns == null || turn < maxTurns; turn++) {
      ...
      if (pauseGate != null && pauseGate.isPaused) {
        ...
        await pauseGate.waitWhilePaused();
      ...
      // turn) — where a `checkpoint` marks and a `rewind` truncates to.
      ...
      while (!completed) {
        ...
        await for (final event in provider.complete(
          ...
          switch (event) {
            ...
            case LlmToolUseDelta(:final id, :final name, :final argumentsJson):
              pending.add(_PendingTool(id, name, _decodeArgs(argumentsJson)));
            case LlmUsage(
              :final inputTokens,
              :final outputTokens,
        ...
        if (lastError != null) {
          ...
          // turn instead of failing the run (parity with goose/kilo/oh-my-pi).
          if (compactor != null &&
      ...
      if (textBuffer.isNotEmpty) {
        blocks.add(HarnessTextBlock(textBuffer));
      }
      ...
      // executes concurrently (parity with oh-my-pi's "shared" concurrency);
      // write/exec/control tools stay strictly sequential ("exclusive"). Only
      // batched when no lifecycle hook is configured, since a hook's per-tool
      // veto/observation must run in order. Result blocks are always appended in
      // the model's original tool-call order so pairing is unambiguous.
      ...
      while (pi < pending.length) {
        ...
        if (hooks == null && _isParallelSafe(toolsByName[pending[pi].name])) {
          ...
          final results = await Future.wait([
            for (final b in batch)
              _safeExecute(toolsByName[b.name]!, b.args, ctx, toolCallId: b.id),
          ]);
        ...
        if (t.name == 'checkpoint') {
        ...
        } else if (hooks != null && !await hooks.preToolUse(t.name, t.args)) {
          ...
          if (config.approvalCallback != null) {
            ...
            result = decision.allowed
                ? await _safeExecute(tool, t.args, ctx, toolCallId: t.id)
            ...
            result = await _safeExecute(tool, t.args, ctx, toolCallId: t.id);
          } else {
      ...
      // Cheap reclaim: elide uneventful older tool results (no summary/cache
      // rewrite). Full compaction is the heavier fallback at the loop top.
      compactor?.pruneToolResults(history);
      ...
      if (advisor != null && turnsSinceAdvice >= advisorEveryTurns) {
        turnsSinceAdvice = 0;
        final note = await advisor.review(history);
        if (note != null) {
    ...
    // the channel, so a follow-up "continue" started from nothing. Give the
  ...
  Future<HarnessToolResult> _safeExecute(
    ...
    try {
      return await tool.execute(args, context.withToolCallId(toolCallId));
    } on Object catch (e) {
  ...
  /// Estimated tokens for the constant per-request overhead (system prompt +
  /// tool schemas) so compaction budgets against real request size.
...
class _PendingTool {
  _PendingTool(this.id, this.name, this.args);
