...
/// on `ChannelMessage`, a different shape). Pure Dart: the LLM-backed summarizer
...
abstract interface class HarnessCompactor {
  ...
  int pruneToolResults(List<HarnessMessage> history);
}
...
class DefaultHarnessCompactor implements HarnessCompactor {
    ...
    for (var i = 0; i < history.length; i++) {
      ...
      for (final b in m.content) {
        if (b is HarnessToolResultBlock &&
            ...
            elision.isUseless(
              toolName: _toolNameFor(history, b.toolUseId),
              outputs: b.content,
              isError: b.isError,
            )) {
          rebuilt.add(
  ...
  /// Finds the tool name that produced [toolUseId] by scanning assistant turns.
  String _toolNameFor(List<HarnessMessage> history, String toolUseId) {
    for (final m in history) {
      if (m.role != HarnessRole.assistant) {
        continue;
      }
      for (final b in m.content) {
        if (b is HarnessToolUseBlock && b.id == toolUseId) {
          return b.name;
        }
      }
    }
