...
import 'package:cc_harness/src/provider/provider_model_override.dart';
...
class ProviderCredential {
  ...
  factory ProviderCredential.fromJson(Map<String, dynamic> json) {
    ...
    return ProviderCredential(
      ...
      modelOverrides: {
        for (final e
            in (json['modelOverrides'] as Map?)?.entries ??
                const Iterable<MapEntry<dynamic, dynamic>>.empty())
          if (e.key is String && e.value is Map)
            e.key as String: ProviderModelOverride.fromJson(
  ...
  /// Per-model metadata overrides, keyed by the provider-native (bare) model
  /// id. Overrides win over the endpoint's live report and the models.dev
  /// catalog; entries flagged [ProviderModelOverride.manual] are models the
  /// user registered by hand. Provider-scoped state kept on the credential row
  /// for storage convenience — merged across every credential of the provider
  /// on read.
  final Map<String, ProviderModelOverride> modelOverrides;
  ...
  ProviderCredential copyWith({
    ...
    Map<String, ProviderModelOverride>? modelOverrides,
  }) => ProviderCredential(
  ...
  Map<String, dynamic> toJson() => {
    ...
    if (modelOverrides.isNotEmpty)
      'modelOverrides': {
        for (final e in modelOverrides.entries) e.key: e.value.toJson(),
      },
  };
}
