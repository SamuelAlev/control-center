...
void main() {
  ...
  group('harnessProviderMetas', () {
    ...
    test('the GLM Coding Plan is its own entry, keyed to the same catalog', () {
      // Same account key as `zai`, different host and different wallet: the
      // pay-as-you-go endpoint answers a plan-only account 1113, so the two
      // lanes cannot be one tile.
      final meta = harnessProviderMetas['zai-coding']!;
      expect(meta.supportsApiKey, isTrue);
      expect(meta.supportsOAuth, isFalse);
      expect(meta.catalogProviderId, 'zhipuai');
      expect(harnessSupportedProviderIds, contains('zai-coding'));
    });
  });
}
