...
void main() {
  ...
  group('harness_providers', () {
    ...
    test('supportsOAuth / supportsApiKey flags', () {
      ...
      expect(harnessProviderMetas['openai']!.supportsApiKey, isTrue);
      // Anthropic is API-key-only: its browser login existed only by minting a
      // token against Claude Code's own OAuth client. Claude Pro/Max is
      // reachable through the `claude-code` adapter instead.
      expect(harnessProviderMetas['anthropic']!.supportsOAuth, isFalse);
