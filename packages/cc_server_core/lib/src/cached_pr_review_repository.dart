...
class CachedPrReviewRepository implements PrReviewRepository {
  ...
  Stream<T> _swrPass<T>(
    ...
    // SWR: a "hit" here means the pass could serve something immediately. That
    // is the number worth watching for this cache: it is what decides whether a
    // PR page paints instantly or waits on the forge. It no longer implies the
    // network ran — a first pass inside the freshness window below serves the
    // hit and stops.
    if (hadCache) {
      _swrStats.hit();
    } else {
    ...
    // A subscription that mounts moments after the last successful
    // revalidation serves the cache and stops there. The stream still enters
    // its reactive phase below, so a real change still arrives — this only
    // suppresses the redundant ask, and only on the first pass.
    if (cachedModel != null &&
        isFirstPass &&
        _isRevalidatedRecently(kind, key)) {
      return;
    }
    ...
    try {
      ...
      // Stamped only when the row was actually rewritten. An oversized payload
      // is refused by [_putBounded] and the OLD row survives; stamping there
      // would vouch for a row nobody refreshed and let the next subscriber
      // serve it while skipping the fetch that would have replaced it.
      if (await _putBounded(kind, key, freshEncoded)) {
        _stampRevalidated(kind, key);
      }
  ...
  Future<bool> _putBounded(String kind, String key, String encoded) async {
  ...
  static const Map<String, Duration> _revalidateWindows = {
    ...
    _Kind.prReviewComments: Duration(seconds: 20),
    _Kind.prIssueComments: Duration(seconds: 20),
    _Kind.prTimelineEvents: Duration(seconds: 30),
    _Kind.prReviewerState: Duration(seconds: 20),
    // Checks move fastest of anything here and are the one surface an operator
    // stares at waiting for a change, so the window is short enough to feel
    // live even if a signal is missed.
    _Kind.prCheckRuns: Duration(seconds: 10),
    _Kind.prCommitStatuses: Duration(seconds: 10),
    // A commit list only changes on a push, which always signals.
    _Kind.prCommits: Duration(seconds: 120),
    // Keyed by an immutable SHA — the content cannot change under the key.
    _Kind.prCommitFiles: Duration(minutes: 30),
  };

  /// When each `(kind, key)` last completed a successful revalidation.
  ///
  /// In-process and per-repository-instance, deliberately: it is a de-duplication
  /// window, not a durability claim, and the repository instances are already
  /// cached per (workspace, repo, acting user). A restart simply revalidates
  /// once more.
  final Map<String, DateTime> _lastRevalidated = {};

  bool _isRevalidatedRecently(String kind, String key) {
    final window = _revalidateWindows[kind];
    if (window == null) {
      return false;
    }
    final at = _lastRevalidated['$kind|$key'];
    if (at == null) {
      return false;
    }
  ...
  /// Stamps `(kind, key)` as just revalidated.
  ///
  /// Called ONLY where the cache row was actually rewritten. Stamping anywhere
  /// else would let a probe's in-memory success vouch for a cache row nobody
  /// refreshed, and the next subscriber would serve that stale row and skip the
  /// fetch that would have fixed it.
  void _stampRevalidated(String kind, String key) {
    if (!_revalidateWindows.containsKey(kind)) {
      return;
    }
    _lastRevalidated['$kind|$key'] = DateTime.now();
  }

  /// Drops the freshness stamps for [key] across every PR-scoped kind.
  ///
  /// An explicit invalidation must be authoritative: the caller deleted the
  /// cache rows precisely because it wants the network hit, and a surviving
  /// stamp would let the next subscriber skip it.
  void _clearRevalidateStamps(String key) {
    _lastRevalidated.removeWhere((k, _) => k.endsWith('|$key'));
  }
  ...
  Stream<PullRequest?> watchPullRequest(int prNumber) {
    return _swr<PullRequest?>(
      ...
      fetch: (token) => _fetchPrDetail(prNumber, token),
  ...
  /// The canonical `GET /pulls/{n}` (plus reaction enrichment).
  ///
  /// A named method rather than a closure inside [watchPullRequest] because
  /// [_probePrDetail] runs it through the SAME single-flight lane. Sharing the
  /// lane is only safe if both callers produce the identical, fully-enriched
  /// value — whoever leads writes what every waiter (including the detail SWR,
  /// which caches it) receives. A probe leading with a cheaper un-enriched
  /// fetch would poison the cached row with empty reactions.
  Future<PullRequest?> _fetchPrDetail(
    int prNumber,
    CancelToken? cancelToken,
  ) async {
    final pr = await _client.getPullRequest(prNumber, cancelToken: cancelToken);
    if (pr == null) {
      return null;
    }
  ...
  Future<PullRequest?> _probePrDetail(
    ...
    return _singleFlightFetch<PullRequest?>(
      _Kind.prDetail,
      _prKey(prNumber),
      (token) => _fetchPrDetail(prNumber, token),
      cancelToken,
    );
  }
  ...
  Future<List<ReactionGroup>> _reactionGroupsWithUser(
    ...
    // One forge round trip PER REACTED COMMENT, and it ran again on every
    // revalidation pass — a PR with 30 reacted comments paid 30 requests each
    // time. The counts in [base] come from the comment payload itself, so they
    // are a free change detector for the thing this call resolves (who reacted,
    // and whether one of them is me): a toggle by anyone moves a count.
    final memoKey = '${target.name}|$targetId';
    final signature = [
      for (final g in base) '${g.content}:${g.count}',
    ].join(',');
    final memo = _reactorMemo[memoKey];
    if (memo != null &&
        memo.signature == signature &&
        DateTime.now().difference(memo.at) < _reactorMemoTtl) {
      return memo.groups;
    }
    try {
      ...
      _rememberReactors(memoKey, signature, resolved);
      return resolved;
    } catch (e) {
  ...
  /// Resolved reactor lists, keyed by target and invalidated by the reaction
  /// COUNTS the comment payload already carries.
  ///
  /// TTL-bounded on top of the signature check because two opposite toggles
  /// between passes (one person adds 👍 while another removes one) leave the
  /// count unchanged, which would otherwise pin a stale hover-card roster for
  /// as long as the repository instance lived.
  static const Duration _reactorMemoTtl = Duration(minutes: 5);
  static const int _maxReactorMemoEntries = 512;
  final Map<
    String,
    ({String signature, DateTime at, List<ReactionGroup> groups})
  >
  _reactorMemo = {};

  void _rememberReactors(
    String key,
    String signature,
    ...
    _reactorMemo
      ..remove(key)
      ..[key] = (signature: signature, at: DateTime.now(), groups: groups);
    while (_reactorMemo.length > _maxReactorMemoEntries) {
      _reactorMemo.remove(_reactorMemo.keys.first);
    }
  }
  ...
  Stream<String> watchDiff(int prNumber) {
    return _swr<String>(
      ...
      skipRevalidate: (_, cancelToken) async {
        ...
        final currentPr = await _probePrDetail(prNumber, cancelToken);
  ...
  Stream<PrFilesLoad> _filesLoadPass(
    ...
    try {
      currentPr = await _probePrDetail(prNumber, cancelToken);
    } catch (e) {
      CcInfraLog.error(
  ...
  Stream<List<PrReviewSubmission>> watchReviews(int prNumber) {
    return _swr<List<PrReviewSubmission>>(
      ...
      fetch: (token) async => _enrichReviewReactions(
        await _client.listReviews(prNumber, cancelToken: token),
        prNumber: prNumber,
        cancelToken: token,
      ),
      encode: (fresh) => _encodeJson(
  ...
  /// Stamps review-summary reactions onto the REST-sourced submissions.
  ///
  /// A separate call because REST cannot see them at all: GitHub only exposes
  /// reactions on the GraphQL review node, so without this join a 👍 left on a
  /// review summary renders nowhere in the client. One round trip per PR, and
  /// best-effort by design — on failure the reviews render without their
  /// reactions rather than the whole timeline failing.
  Future<List<PrReviewSubmission>> _enrichReviewReactions(
    List<PrReviewSubmission> reviews, {
    required int prNumber,
    Object? cancelToken,
  }) async {
    if (reviews.isEmpty || !_client.capabilities.reactions) {
      return reviews;
    }
    final List<PrReviewReactions> perReview;
    try {
      perReview = await _client.listReviewReactions(
        prNumber,
        cancelToken: cancelToken,
      );
    } catch (e) {
      CcInfraLog.error(
        'Review Reactions: enrichment failed for #$prNumber: $e',
        e,
      );
      return reviews;
    }
    if (perReview.isEmpty) {
      return reviews;
    }
    final byId = <int, List<ForgeReaction>>{
      for (final r in perReview)
        if (r.reactions.isNotEmpty) r.reviewId: r.reactions,
    };
    if (byId.isEmpty) {
      return reviews;
    }
    // The viewer flag rides the same usernames the groups already carry, so
    // marking them costs no extra round trip (unlike the per-target reaction
    // listing the comment paths pay for).
    final currentUserLogin = await _currentLogin(cancelToken);
    return [
      for (final review in reviews)
        if (byId[review.id] case final reactions?)
          review.copyWith(
            reactions: [
              for (final g in reactionGroupsFromPerUser(reactions))
                if (currentUserLogin == null)
                  g
                else
                  g.copyWith(
                    userReacted: g.usernames.contains(currentUserLogin),
                  ),
            ],
          )
        else
    ...
    return _swr<List<PrCodeReviewComment>>(
      ...
      fetch: (token) async {
        ...
        return _enrichCommentReactions(
          await _enrichCommentThreadState(
            comments,
            prNumber: prNumber,
            cancelToken: token,
          ),
  ...
  /// Stamps each comment with the conversation state of the thread it belongs
  /// to, so a resolved conversation renders resolved.
  ///
  /// A separate call because forges model it separately (GitHub's REST comment
  /// payload has no resolution field at all — it lives on the GraphQL thread).
  /// One extra round trip per revalidation, not one per comment: the thread
  /// query carries its comments' numeric ids, which is what keys this join.
  ///
  /// Best-effort by design: if the thread query fails the comments still render
  /// — as unresolved, which is the safe direction. Hiding a conversation
  /// because a side query succeeded-with-garbage would lose review feedback;
  /// showing an already-settled one costs a click.
  Future<List<PrCodeReviewComment>> _enrichCommentThreadState(
    List<PrCodeReviewComment> comments, {
    required int prNumber,
    Object? cancelToken,
  }) async {
    if (comments.isEmpty || !_client.capabilities.commentThreadResolution) {
      return comments;
    }
    final List<PrReviewThreadState> threads;
    try {
      threads = await _client.listReviewThreadStates(
        prNumber,
        cancelToken: cancelToken,
      );
    } catch (e) {
      // Logged, not swallowed silently: a scope or schema problem here would
      // otherwise render every conversation on every PR as open, forever, with
      // nothing anywhere saying why.
      CcInfraLog.error(
        'Review threads: state lookup failed for #$prNumber: $e',
        e,
      );
      return comments;
    }
    return withReviewThreadState(comments, threads);
  }
  ...
  Future<void> invalidatePullRequest(int prNumber) async {
    ...
    // Read the head SHA BEFORE the detail row is deleted: check runs and commit
    // statuses are keyed by SHA, not by PR number, so clearing only the
    // PR-keyed stamps would leave the one surface an operator actually watches
    // — the checks rail — serving its pre-refresh answer for the length of its
    // window, right after they pressed refresh.
    //
    // Guarded: this decodes a cache row, and invalidation is precisely the
    // operation that must still work when one is unreadable. A corrupt row
    // costs a stale stamp, never a refresh that refuses to run.
    String? headSha;
    try {
      headSha = await _cachedHeadSha(prNumber);
    } on Object {
      headSha = null;
    }
    ...
    _clearRevalidateStamps(key);
    if (headSha != null && headSha.isNotEmpty) {
      _clearRevalidateStamps(_shaKey(headSha));
    }
  ...
  Future<void> replyToReviewComment({
    ...
    // Bust the comment list so the reply shows up now rather than whenever the
    // next SWR revalidation happens to run — a reply that disappears for half a
    // minute reads as a post that failed, and gets written twice.
  ...
  /// Toggles a reaction on a review submission's summary.
  @override
  Future<void> toggleReviewReaction({
    ...
    await _toggleReaction(
      target: ForgeReactionTarget.review,
      targetId: '$reviewId',
      prNumber: prNumber,
      content: content,
      add: add,
      invalidates: const [_Kind.prReviews],
    );
  }
  ...
  Future<void> setReviewThreadResolved({
    ...
    await _client.setReviewThreadResolved(
      prNumber: prNumber,
      threadId: threadId,
      resolved: resolved,
    );
    // Resolution rides on the comment list (it is what carries `isResolved`),
    // so bust that kind — not the whole PR — and every open viewer re-reads.
    await _invalidatePrKinds(prNumber, const [_Kind.prReviewComments]);
  }

  /// Submits a review (approve, request changes, or comment), carrying any
  /// queued inline comments so the whole review lands as one event.
  @override
  Future<void> submitReview({
    ...
    // Snapshot the reviewer rail as it stands BEFORE the write, so
    // [_settleReviewerState] can tell "the forge caught up" from "the forge
    // handed us the same pre-review state again".
    final railBefore = await _cache.read(
      _workspaceId,
    ...
    await _client.submitReview(
      ...
      body: body,
      comments: [for (final c in comments) c.toWire()],
    );
    if (comments.isNotEmpty) {
      await _invalidatePrKinds(prNumber, const [_Kind.prReviewComments]);
    }
    await invalidatePullRequest(prNumber);
    unawaited(_settleReviewerState(prNumber, railBefore));
  ...
  /// Backoff for [_settleReviewerState]. Three tries over ~8s covers the lag
  /// observed in practice without turning one review into an open-ended poll.
  static const _reviewerSettleBackoff = <Duration>[
  ...
  Future<void> _settleReviewerState(int prNumber, String? before) async {
    ...
    for (final delay in _reviewerSettleBackoff) {
      await Future<void>.delayed(delay);
      // This runs unawaited, outliving the RPC that started it: the workspace
      // database or the signal bus can be torn down under it (server shutdown,
      // a workspace closing, a test's tearDown). Nothing here is worth more
      // than the refresh button it replaces, and an unhandled error in the
      // server's root isolate is fatal — so give up quietly.
      try {
  ...
  ///
  /// Gated on the forge's `draftToggle` capability rather than left to the
  /// adapter: a forge without draft state has nothing to toggle, and failing
  /// here names the capability instead of surfacing a vendor API error.
  @override
  Future<void> setPullRequestDraft({
    required int prNumber,
    required bool draft,
  }) async {
    if (!_client.capabilities.draftToggle) {
      throw ForgeUnsupportedError(_client.capabilities.forge, 'draftToggle');
    }
    await _client.setPullRequestDraft(prNumber: prNumber, draft: draft);
    await invalidatePullRequest(prNumber);
    _emitStatusChanged(draft ? 'draft' : 'ready_for_review', prNumber);
  }
