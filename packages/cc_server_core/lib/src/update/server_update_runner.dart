...
typedef UpdateHttpDownload =
    Future<int> Function(Uri uri, File destination);
...
class ServerUpdateRunner {
  ...
  Future<int> run({
    ...
    if (archiveFile.existsSync() &&
        _sha256OfFile(archiveFile) == expectedSha) {
  ...
  Future<int> _applyWindows({
    ...
    for (final entry in Directory('${bundle.path}/bin').existsSync()
