...
///  * the run's conversation → a [ChannelLocus], so "where is this agent
///    working" is a roster fact.
///
...
class AgentPresenceSynthesizer {
  ...
  Set<String> _blockedConversations = const {};
  ...
  Future<void> start() async {
    ...
    _confSub = confirmations?.pending.listen((pending) {
      _blockedConversations = {
        for (final p in pending)
          if (p.request.conversationId.isNotEmpty) p.request.conversationId,
      };
      unawaited(_publish());
    });
    ...
    for (final entry in representative.entries) {
      ...
      final state = !_isLive(run)
          ...
          : (run.conversationId != null &&
                _blockedConversations.contains(run.conversationId))
      ...
      hub.publishAgent(
        ...
        locus: run.conversationId == null
            ? null
            : ChannelLocus(channelId: run.conversationId!),
      );
      next.add(entry.key);
    }
