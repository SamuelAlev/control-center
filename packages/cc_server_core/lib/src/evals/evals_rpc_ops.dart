...
List<RepoOp> buildEvalsOps({
  ...
  return [
    RepoOp(
      ...
      handler: (ctx) async {
        final suites = await repository.suites(ctx.workspaceId!);
        return {'suites': suites.map(evalSuiteToWire).toList()};
      },
    ),
    RepoOp(
    ...
    RepoOp(
      ...
      requiredArgs: const ['suite_id'],
      handler: (ctx) async {
        final runs = await repository.runsForSuite(
          ctx.workspaceId!,
          ctx.args['suite_id'] as String,
        );
        return {'runs': runs.map(evalRunToWire).toList()};
      },
    ),
    RepoOp(
      name: 'evals.runSuite',
    ...
    RepoOp(
      name: 'evals.recordings',
      kind: RepoOpKind.read,
      handler: (ctx) async {
        final recs = await repository.recordings(
          ctx.workspaceId!,
          agentId: ctx.args['agent_id'] as String?,
        );
        return {'recordings': recs.map(recordingToWire).toList()};
      },
    ),
    RepoOp(
      name: 'evals.goldens',
      ...
      handler: (ctx) async {
        ...
        final goldens = agentId == null
            ? await repository.goldens(ctx.workspaceId!)
            : await repository.goldensForAgent(ctx.workspaceId!, agentId);
        return {'goldens': goldens.map(goldenToWire).toList()};
      },
    ),
    RepoOp(
    ...
    RepoOp(
      name: 'evals.deleteGolden',
      kind: RepoOpKind.destructive,
      requiredArgs: const ['golden_id'],
      handler: (ctx) async {
        await repository.deleteGolden(
          ctx.workspaceId!,
          ctx.args['golden_id'] as String,
        );
        return {'ok': true};
      },
    ),
    RepoOp(
      name: 'evals.configVersions',
      kind: RepoOpKind.read,
      requiredArgs: const ['agent_id'],
      handler: (ctx) async {
        final versions = await repository.configVersionsForAgent(
          ctx.workspaceId!,
          ctx.args['agent_id'] as String,
        );
        return {'versions': versions.map(configVersionToWire).toList()};
      },
    ),
    RepoOp(
      ...
      handler: (ctx) async {
        ...
        return {'reliability': score.toJson()};
      },
    ),
  ];
}
