...
class SelectableVoiceModelControl implements SelectableModelControl {
  ...
  ModelStatusSnapshot _last = const ModelStatusSnapshot(
  ...
  Stream<ModelStatusSnapshot> watch() async* {
    // Await the inner disk probe before the first yield. Yielding the
    // construction-time `unknown` snapshot synchronously races the
    // `sub/subscribe` ack: the client registers the subscription id only
    // after the ack, so a same-burst snapshot is dropped and the settings
    // StreamProvider spins in loading forever. Same contract as
    // [ManagedModelControl.watch].
    _last = await _inner.status();
    yield _last;
    yield* _out.stream;
  }
