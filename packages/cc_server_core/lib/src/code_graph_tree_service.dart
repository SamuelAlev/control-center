...
/// `index_code` walks) plus one partition per conversation/PR worktree (built
/// and kept current by the `CodeGraphWatchService`). A conversation-scoped
/// call therefore resolves BOTH the searched partition and the audit tree to
/// the conversation's isolated repo copy; a non-conversation call uses the
/// linked checkout for both.
///
...
/// workspace-scoped, so a foreign repo or channel simply does not resolve and
...
class CodeGraphTreeService implements CodeGraphTreePort {
  ...
  Future<String?> checkoutIdFor({
    ...
    try {
      final worktrees = await _isolated.forChannel(workspaceId, conversationId);
  ...
  Future<CodeGraphPathAudit?> audit({
    ...
    // conversation's worktree when [checkoutId] selects its partition, else
    // the linked checkout.
  ...
  /// The conversation's isolated copy of [repoId], or [fallback] when the call
  /// is not conversation-scoped / the conversation has no copy of this repo.
