...
Future<CcServer> runCcServer({
  ...
  final oidcService = OidcService(
    config: OidcConfig.fromEnvironment(Platform.environment),
    users: userRepository,
    members: membershipRepository,
    workspaces: DaoWorkspaceRepository(
      globalDb.workspaceRegistryDao,
  ...
  final samlService = SamlService(
    config: SamlConfig.fromEnvironment(Platform.environment),
    users: userRepository,
    members: membershipRepository,
    workspaces: DaoWorkspaceRepository(
      globalDb.workspaceRegistryDao,
  ...
  await ssoSettings.loadAndApply(env: Platform.environment);
  // Server-admin gate for the `sso.*` ops: the caller must OWN at least one
  // live workspace (the bootstrap owner qualifies by construction). SSO
  // decides who may authenticate to the whole server, so it never rides
  // mere pairing.
  Future<bool> isServerAdmin(String userId) async {
    final rows = await globalDb.workspaceRegistryDao.getAll();
    return rows.any((w) => w.deletedAt == null && w.ownerUserId == userId);
  }
  ...
  // token (Settings > environment > `gh` CLI for GitHub only), so a workspace
  // holding repos on several forges authenticates to each independently and one
  // missing credential never blanks another forge's PRs.
  final forgeCredentials = ForgeCredentials(
    secrets: secrets,
    env: (name) => Platform.environment[name],
    ghCliProbe: () async => ghStatus.isAuthenticated
        ? (token: ghStatus.token, username: ghStatus.username)
        : null,
  );
  ...
  if (connectedForges.isEmpty) {
    CcHostLog.warning(
      ...
      '`gh auth login` / GITLAB_TOKEN / BITBUCKET_API_TOKEN is available.',
    );
  } else {
    CcHostLog.info(
  ...
  ForgePrClient forgePrClientForRepo(Repo repo) => forgePrClientBuilder(
    repo.forge,
    forgeDioFactory.of(repo.forge),
  ...
  // The operator's merged history, asked of each forge under that forge's own
  // identity — the same human is `octocat` on GitHub and something else on
  // GitLab, so there is no single login to search by.
  ...
  final serverForgeRegistry = buildForgeProviderRegistry(
    workspaceDbs: workspaceDbs,
    dioFactory: forgeDioFactory,
  ...
  eventBus.on<ChannelCreated>().listen((e) {
    ...
    if (workspaceId.isEmpty) {
      ...
      // into an arbitrary workspace. (`ChannelCreated.workspaceId` is non-null
  ...
  final terminalSessions = TerminalSessionService(
    ...
    vmShell:
        ({
          ...
          try {
            final existing = rigService.execRigFor(workspaceId, conversationId);
            ...
            final rig =
                ...
                await rigService.open(
                  ...
                  spec: RigSpec.exec(
                    ...
                    // The terminal is the operator's playground — the same
                    // stance `terminalSandboxConfig` already takes for the
                    // host shell. Capability gating exists to bound AGENTS;
                    // a person typing in their own terminal expects
                    // `git push` to work, and refusing it here would make the
                    // enclosed terminal strictly worse than the host one for
                    // no security gain (they can push from the host anyway).
                  ...
                  openedBy: UserPrincipal(ownerUserId),
                );
  ...
  // (unified with `review_channels.prExternalId`, migration 46). Association-first
  ...
  final dispatchReviewersService = DispatchReviewersService(
    ...
    guidelineLookup: (workspaceId) async {
      final facts = await memoryFactRepository.getActiveByWorkspace(
        workspaceId,
      );
      return [
        for (final f in facts)
          if (f.domain == SystemMemoryDomains.reviewGuidelines)
            ReviewGuideline(
  ...
  // The deterministic finalize shared by the `finalize_review` MCP tool and
  // the review hub — one verdict pipeline, two entry points.
  ...
  // The one canonical AI-review flow (the Review Hub): ensure the PR channel,
  // compute the deterministic review context (cohorts + contract/visual axes),
  // fan out reviewers into THE PR channel with the area map in their briefs,
  // await their runs, author the structured walkthrough with a CEO editorial
  // pass, finalize deterministically and honor the workspace auto-publish
  // opt-in. `start` returns immediately; the orchestration narrates progress
  // into the channel as system messages.
  final reviewHubService = ReviewHubService(
    resolvePr:
        ({
          required String workspaceId,
          required String owner,
          ...
          return ReviewHubPrContext(
            externalId: pr.externalId.isNotEmpty
                ? pr.externalId
    ...
    ensureChannel:
        ({
          ...
          final ensured = await ensurePrChannel(
            workspaceId: workspaceId,
            repoFullName: repoFullName,
            prNumber: prNumber,
            prExternalId: prExternalId,
          ...
          return ensured['channel_id'] as String;
        },
    computeCohorts:
        ({
          required String workspaceId,
          ...
          final result = await computeReviewStudioFn(
            workspaceId: workspaceId,
            owner: owner,
            repo: repo,
            prNumber: prNumber,
          ...
          final raw = result['cohorts'] as List? ?? const [];
          return [
            for (final c in raw.whereType<Map>())
              ReviewCohort.fromJson(c.cast<String, dynamic>()),
          ];
        },
    fetchPatches:
        ({
    ...
    runSnapshots: reviewRunSnapshotRepository,
    cohortRepository: reviewCohortRepository,
    dispatchReviewers: dispatchReviewersService,
  ...
  // The agent-facing entry to the same canonical flow the UI's "Ask AI"
  // action uses (registered here because it needs the hub service, which
  // needs the channel/compute closures defined above).
  mcpRegistry.register(StartAiReviewTool(start: reviewHubService.start));
  ...
  final catalog = buildRemoteRpcCatalog(
    ...
    extraOps: [
      ...
      ...buildSsoOps(settings: ssoSettings, isServerAdmin: isServerAdmin),
    ],
    extraWatchQueries: [
      ...fleetWatchQueries,
    ...
    // PRD 23 §1: skills registry browse/preview/install ops; §2/§6: the
    // installed-skills scan/status/save ops + quarantine detach enforcement.
    ...
    buildForgePrClient: forgePrClientForRepo,
    ...
    eventBus: eventBus,
    // PR review over RPC: when the server has a `gh` token (see [serverForgeRegistry]
    ...
    forgeProviderRegistry: serverForgeRegistry,
        ...
        : (repos) async {
            final nodes = await serverGitHubClient.graphql
                .searchReviewRequestedPullRequests(
                  ...
                  repos: [
                    for (final r in repos)
                      (owner: r.remoteOwner, name: r.remoteName),
                  ],
                );
            final byFullName = {
    ...
    // The operator's merged PR history, asked of each repo's own forge under
    // that forge's viewer identity. Fails soft per repo so one inaccessible
    // repo — or one unauthenticated forge — never sinks the rest.
  ...
  unawaited(() async {
    try {
      ...
      for (final ws in all) {
        await workspaceSeeder.reseedTemplates(ws.id);
      }
    } on Object catch (e) {
  ...
  // and the ticket could never be picked up again. (The channel-provisioning
  ...
  final githubNotificationPoller = ghToken.isEmpty
      ...
      : GitHubNotificationPollingService(
          ...
          // repo list on each notification-poll lookup (O(workspaces) queries
          // per repo per ~60s tick). Repo↔workspace links change rarely, so
          // the index is rebuilt on a TTL rather than per lookup.
