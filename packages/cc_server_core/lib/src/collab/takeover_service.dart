...
///     dispatches into the channel are refused while it stands.
...
///     the channel, queued as steering for every paused run (the agent
...
class TakeoverService {
  ...
  /// The Caches kind holding take-over markers (key = channel id).
  ...
  /// is always read and written in the workspace that owns the channel.
  CacheDao _cache(String workspaceId) => _dbs.of(workspaceId).cacheDao;

  /// The active take-over marker for a channel, or null.
  ...
  /// Whether a take-over stands on [channelId] (dispatch gating).
  Future<bool> isActive(String workspaceId, String channelId) async =>
      await status(workspaceId, channelId) != null;

  /// Begins a take-over: pauses/stops the channel's active runs and writes
  ...
  Future<Map<String, dynamic>> begin({
    ...
    final existing = await status(workspaceId, channelId);
    if (existing != null) {
      throw StateError(
    ...
    await _cache(
      workspaceId,
    ).put(workspaceId, cacheKind, channelId, jsonEncode(marker));
  ...
  Future<Map<String, dynamic>> handBack({
    ...
    final marker = await status(workspaceId, channelId);
    if (marker == null) {
      throw StateError('No take-over stands on this conversation.');
    }
    final summary = await _diffSummary(workspaceId, channelId);
    ...
    await _cache(workspaceId).deleteEntry(workspaceId, cacheKind, channelId);
    return {'summary': summary, 'resumed_run_ids': paused};
  }

  Future<String> _diffSummary(String workspaceId, String channelId) async {
    try {
      final files = await _conversationChanges(workspaceId, channelId);
      if (files.isEmpty) {
