...
McpToolRegistry buildServerMcpRegistry({
  ...
  // The newsfeed is per-user; MCP tools have no session user, so agents
  // ride the SERVER OWNER's feed list.
  ...
  // Resolves the memory tools' optional `repo` argument (an id, an
  // `owner/name`, or a scope slug) to one canonical repo slug. Reads through
  // the same workspace-scoped repo repository, so naming a repo the workspace
  // does not have is an error rather than a silent workspace-wide write.
  final memoryRepoScope = MemoryRepoScopeResolver(repoRepository);
  ...
  final registry = McpToolRegistry([
    // Newsfeed (per-user; RSS fetched + persisted server-side; agents ride
    // the server owner's feeds).
    ListFeedsTool(repository: newsfeedRepository, userId: newsfeedOwnerUserId),
    ListArticlesTool(
      repository: newsfeedRepository,
      userId: newsfeedOwnerUserId,
    ),
    GetArticleTool(repository: newsfeedRepository, userId: newsfeedOwnerUserId),
    SetArticleReadTool(
      repository: newsfeedRepository,
      userId: newsfeedOwnerUserId,
    ),
    ...
    // Messaging (read + write). The write tool registers as `send_message`; it
    // was long referenced by the mode-guard allow-lists and the review-mode
    // prompt but never registered — agents were instructed to call a tool that
    // did not exist.
    ...
    ProposeFactTool(recordFact: recordFactUseCase, repoScope: memoryRepoScope),
    ProposePolicyTool(
    ...
    // the two things a work product lacked: a typed `artifact` space message
...
/// DAO-backed [SpaceNotesPort] — adapts [SpaceExtrasDao] to the domain
/// port so MCP tools can read/write the shared space notes doc without
/// reaching into cc_persistence.
class _DaoSpaceNotesPort implements SpaceNotesPort {
  _DaoSpaceNotesPort(this._dbs);
  final WorkspaceDatabaseManager _dbs;

  SpaceExtrasDao _dao(String workspaceId) =>
      _dbs.of(workspaceId).spaceExtrasDao;

  @override
  Future<({String contentMarkdown, String updatedBy, DateTime updatedAt})?>
  getNote(String workspaceId, String spaceId) async {
    final row = await _dao(workspaceId).noteForSpace(workspaceId, spaceId);
    if (row == null) {
      return null;
    }
    return (
