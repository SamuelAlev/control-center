...
class MediaCache {
  ...
  Future<void> _write(String key, MediaFetchBuffered fetched) async {
    ...
    await _metaFile(
      key,
    ).writeAsString(jsonEncode(meta.toJson()), flush: false);
  }
    ...
    await _metaFile(key)
        .writeAsString(
          jsonEncode(
            meta
                .copyWithExpiresAt(
                  _clock().add(ttl).millisecondsSinceEpoch,
                )
                .toJson(),
          ),
          flush: false,
  ...
  /// Updates the body file's mtime for LRU ordering (best-effort).
  void _touch(String key) {
    unawaited(_bodyFile(key).setLastModified(_clock()).catchError((_) {}));
  }
