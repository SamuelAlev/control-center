  ...
  return [
    RepoOp(
      ...
      handler: (ctx) async {
        ...
        final status = ctx.args['status'] as String?;
        // No filter = every status, newest first. A status filter is answered
        // by the indexed per-status read rather than by filtering the union.
        final jobs = status == null || status.isEmpty
            ? await _allJobs(fleetRepository, ws)
            : await fleetRepository.jobsByStatus(
                ws,
                JobStatus.fromWire(status),
              );
