...
class GitHubOpenPrFetchAdapter implements OpenPrFetchPort {
  ...
  Future<OpenPrFetchResult> fetchGroups(List<Repo> repos) async {
    ...
    for (var i = 0; i < repos.length; i++) {
      ...
      // Present in the batch = GitHub answered for this repo. Recorded BEFORE
      // the empty-group skip below, so a repo with a genuinely empty queue
...
class OpenPrPollingService {
  ...
  Future<void> _runSweep(
    ...
    try {
      ...
      if (listChanged) {
        final result = await _port.fetchGroups(repos);
        ...
        if (result.resolvedRepoIds.isEmpty) {
          CcHostLog.warning(
            'open_pr_poll: GitHub resolved 0 of ${repos.length} repos for '
        ...
        if (resolved.length < repos.length) {
          CcHostLog.warning(
            'open_pr_poll: GitHub resolved ${resolved.length} of '
            '${repos.length} repos for $workspaceId — the rest keep their '
            'previous entries',
          );
        }
  ...
  Map<String, dynamic> _snapshotFromGroups(
    ...
    return {'authenticated': true, 'repos': repos};
  }
