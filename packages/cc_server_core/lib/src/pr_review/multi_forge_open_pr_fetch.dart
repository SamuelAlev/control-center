...
class MultiForgeMergedHistory {
  ...
  const MultiForgeMergedHistory({
    required ForgePrClientForRepo clientFor,
    required Future<String> Function(ForgeHost forge) viewerLoginFor,
  ...
  /// The operator's recently merged pull requests across [repos], grouped by
  /// repo. Repos whose forge cannot answer are simply absent.
  Future<List<OpenPrGroup>> mergedByViewer(List<Repo> repos) async {
    final groups = <OpenPrGroup>[];

    await Future.wait(
      repos.map((repo) async {
        try {
          final login = await _viewerLoginFor(repo.forge);
          if (login.isEmpty) {
            return;
          }
          final prs = await _clientFor(repo).listMergedByAuthor(login);
          if (prs.isNotEmpty) {
