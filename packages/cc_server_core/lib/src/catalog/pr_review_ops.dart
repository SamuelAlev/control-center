...
List<RepoOp> buildPrReviewOps({
  ...
  required ({String owner, String repo}) Function(Map<String, dynamic> args)
  requireRepoCoords,
  ...
  previewSwr,
  required Future<void> Function(String workspaceId, String spaceId)
  assertSpaceOwned,
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      final draft = await repo.getDraft((ctx.args['pr_number'] as num).toInt());
      return {'draft': draft};
    },
  ),
  RepoOp(
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      return {'candidates': candidates.map(prReviewerCandidateToWire).toList()};
    },
  ),
  RepoOp(
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      await repo.invalidatePullRequest((ctx.args['pr_number'] as num).toInt());
      return {'ok': true};
    },
  ),
  RepoOp(
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      for (final m in messages) {
        if (m.messageType != MessageType.reviewNode || !wanted.contains(m.id)) {
          continue;
        }
  ...
  RepoOp(
    ...
    kind: RepoOpKind.mutate,
    requiredArgs: ['owner', 'repo', 'pr_number', 'review_id', 'content', 'add'],
    handler: (ctx) async {
      final c = requireRepoCoords(ctx.args);
      final repo = await resolvePrReviewRepository(
        ctx.workspaceId!,
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      return {'labels': labels.map(prLabelToWire).toList()};
    },
  ),
  RepoOp(
    name: 'pr_review.addLabels',
    kind: RepoOpKind.mutate,
    requiredArgs: ['owner', 'repo', 'pr_number', 'names'],
    handler: (ctx) async {
      final c = requireRepoCoords(ctx.args);
      final repo = await resolvePrReviewRepository(
        ctx.workspaceId!,
      ...
      await repo.addLabels(
        prNumber: (ctx.args['pr_number'] as num).toInt(),
        names: ((ctx.args['names'] as List?) ?? const [])
            .map((e) => e.toString())
            .toList(),
      );
      return {'ok': true};
  ...
  // Fetched as the CALLER. The no-caller client is the GitHub App, and
  // GitHub answers 404 (not 403) for a private repo that installation
  // cannot see — the chip then falls back to a plain link even though the
  // person reading the markdown can open the PR. The SWR row is keyed by
  // the caller too: a title resolved with one member's credential must not
  // be served to another member whose token cannot see the repo.
  // `null` means the ref can't be resolved (the chip falls back to a link).
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      final preview = await previewSwr(
        ...
        kind: 'prPreview',
        key: '${ctx.userId}|${c.owner}/${c.repo}#$number',
        fetch: () => fetchPrPreview(
          c.owner,
          c.repo,
          number,
  ...
  RepoOp(
    ...
    handler: (ctx) async {
      ...
      final preview = await previewSwr(
        workspaceId: ctx.workspaceId!,
        kind: 'commitPreview',
        key: '${ctx.userId}|${c.owner}/${c.repo}@$sha',
        fetch: () => fetchCommitPreview(
          c.owner,
          c.repo,
          sha,
          actingUserId: ctx.userId,
