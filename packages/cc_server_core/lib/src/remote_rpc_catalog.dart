...
RemoteRpcCatalog buildRemoteRpcCatalog({
  ...
  ForgeProviderRegistry? forgeProviderRegistry,
  ...
  // The operator's merged PR history across a workspace's repos, resolved per
  // forge under that forge's own viewer identity. Null → empty.
  Future<List<({Repo repo, List<PullRequest> prs, bool hasMore})>> Function(
    List<Repo> repos,
  )?
  fetchMergedHistory,
  ...
  // `credentials.*ForgeToken` ops. SECURITY: the same rule as `githubCli` —
  // connection status crosses the wire, tokens never do. Absent when null →
  // those ops are not declared and the client shows every forge disconnected.
  ForgeCredentialPort? forgeCredentials,
  // Builds the API client for one repo, on that repo's own forge. Backs the
  // compose-PR reads (branches, default branch, comparison, templates) so the
  // composer works the same on every forge. Null → those ops degrade to empty.
  ForgePrClient Function(Repo repo)? buildForgePrClient,
  ...
  reviewCohortImpact,
  // Starts the canonical AI review flow (Review Hub): ensure the PR channel,
  // compute the deterministic review context, fan out reviewers into the
  // channel, author the walkthrough, finalize. Returns immediately — progress
  // streams through the channel + association status.
  Future<Map<String, dynamic>> Function({
    required String workspaceId,
    required String owner,
    required String repo,
  ...
  reviewHubStart,
  // Asks one focused question about ONE review area, answered by a single
  // read-only agent turn in the PR channel. Manual and one-shot.
  Future<Map<String, dynamic>> Function({
    required String workspaceId,
    required String owner,
    required String repo,
  ...
  /// whole workspace's channel list through a throwaway drift watch and
  /// scanning it in Dart made each of those O(all channels) rows + entity
  ...
  Future<bool> channelInWorkspace(String workspaceId, String channelId) async {
    final channel = await messagingRepository.getChannelById(
      workspaceId,
      channelId,
    );
    return channel != null;
  ...
  /// [requireWorkspaceForgeRepo], or null when its forge has no client wired.
  ForgePrClient? forgeClientFor(Repo repo) =>
      repo.hasForgeRemote ? buildForgePrClient?.call(repo) : null;
  ...
  Future<PrReviewRepository> resolvePrReviewRepository(
    ...
    // others.
    final created = forgeProviderRegistry.resolve(
      ForgeProviderContext(repo: match, workspaceId: workspaceId),
    );
  ...
  final ops = RepoOpRegistry([
    ...
    // Which forges are connected, and as whom. Never carries a token: this
    // crosses to every client including remote ones, and the credential stays
    // on the server (the same rule `githubCliStatusToWire` has always applied,
    ...
    if (forgeCredentials != null)
      RepoOp(
        ...
        handler: (ctx) async => {
          'connections': [
            for (final c in await forgeCredentials.connections()) c.toJson(),
          ],
        },
      ),
    // Re-probes one forge and refreshes its cached viewer identity. Backs the
    // "test connection" affordance, so it deliberately hits the network rather
    // than reporting the cached verdict.
    if (forgeCredentials != null)
      RepoOp(
        ...
        handler: (ctx) async {
          ...
          return (await forgeCredentials.testConnection(forge)).toJson();
        },
      ),
    // Stores or clears one forge's credential. Takes effect on the next
    // request — the HTTP clients read the token per call, so there is no
    // restart and no stale interceptor holding the old value.
    if (forgeCredentials != null)
      RepoOp(
        ...
        workspaceScoped: false,
        handler: (ctx) async {
          // Server-wide credential: every workspace on this server
          // authenticates to the forge with it, so only the operator may
          // change it (mirrors `mcp.*` / `models.*` / `sso.*`). Without this
          // any paired user — including a guest of one workspace, or one with
          // no memberships at all — could overwrite or wipe it.
          ...
          await forgeCredentials.setToken(forge, token);
          return (await forgeCredentials.testConnection(forge)).toJson();
        },
      ),
    if (forgeCredentials != null)
      RepoOp(
        ...
        workspaceScoped: false,
        handler: (ctx) async {
          requireServerAdmin(ctx); // Server-wide credential — operator only.
          final forge = ForgeHost.fromWire(ctx.args['forge'] as String?);
          if (!forge.isSupported) {
            throw const NotFoundException('Missing or invalid argument: forge');
          }
          await forgeCredentials.clearToken(forge);
          return {'ok': true};
        },
      ),
    ...
    if (rigPorts != null)
      ...[
        RepoOp(
          ...
          requiredArgs: ['rig_id'],
          handler: (ctx) async {
            final snapshot = rigPorts.portsFor(
              ctx.workspaceId!,
              ctx.args['rig_id'] as String,
            );
            // Absent, not an error: a terminal that has not opened a port yet
            // and a rig in another workspace read the same to the panel.
            return snapshot ?? const {'ports': <dynamic>[]};
          },
        ),
        RepoOp(
    ...
    //
    // `assertChannelOwned` gates the CHANNEL, and `conversation_id` is now
    ...
    if (serverSettingsRepository != null)
      RepoOp(
        ...
        requiredCapability: SessionCapability.fullClient,
        handler: (ctx) async {
          // The values are host config — launch argv/env overrides can carry
          // host credentials — so even the READ is operator-only.
          requireServerAdmin(ctx);
    ...
    RepoOp(
      ...
      handler: (ctx) async {
        final prId = ctx.args['pr_id'] as String;
        await assertPrLifecycleOwned(ctx.workspaceId!, prId);
        final result = await prLifecycleRepository.publishToForge(
          workspaceId: ctx.workspaceId!,
          prId: prId,
          owner: ctx.args['owner'] as String,
    ...
    RepoOp(
      ...
      handler: (ctx) async {
        ...
        final ghRepos = fetch == null
            ? const <Repo>[]
            : await linkedForgeRepos(ctx.workspaceId!);
    ...
    RepoOp(
      ...
      handler: (ctx) async {
        ...
        final groups = await history(repos);
        return {
          'repos': [
            for (final g in groups)
              {
    ...
    RepoOp(
      ...
      handler: (ctx) async {
        ...
        final client = forgeClientFor(matched);
        if (client == null) {
          return {'branches': <String>[]};
        }
        final branches = await client.listBranches();
        // The operator's own branches lead the picker — that is almost always
        // what they are about to open a PR from. Resolved per forge, because
        // the adapter cannot know who "me" is: the same human is a different
        // account on each.
        final me = (await forgeCredentials?.viewerLogin(matched.forge) ?? '')
            .toLowerCase();
        final mine = <String>[];
        final others = <String>[];
    ...
    // Asks one question about one review area. Dispatches a single read-only
    // agent turn whose answer lands in the PR channel.
    if (reviewHubAskArea != null)
      RepoOp(
        name: 'review_hub.askArea',
        kind: RepoOpKind.mutate,
    ...
    // Starts the canonical AI review (the Review Hub "Ask AI" action). Manual
    // by design — no event trigger. Fires the orchestration in the background;
    // the response only reports the channel + status.
  ...
  final watch = WatchQueryRegistry([
    ...
    if (pendingConfirmationRegistry != null)
      WatchQuery(
        ...
        //
        // `conversation_id` is an OPTIONAL narrowing on top of that, not a
        // second authorization check. The phone renders approvals inside one
        // thread and used to subscribe host-globally and filter in Dart — so
              ...
              'pending': [
                for (final p in list)
                  if (conversationId == null ||
                      p.request.conversationId == conversationId)
