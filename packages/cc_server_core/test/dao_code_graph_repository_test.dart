...
void main() {
  ...
  group('pruneUnresolvedEdges', () {
    test('deletes unresolved edges and keeps bound ones', () async {
      ...
      final bound = CodeEdge(
        id: codeEdgeId(wsId, repoId, source.id, target.id, 'calls'),
        workspaceId: wsId,
        repoId: repoId,
        sourceSymbolId: source.id,
        sourceFilePath: filePath,
      ...
      // Unresolved: `react` has no definition in the tree, exactly the row
      // the old behavior retained forever.
      final external = CodeEdge(
        id: codeEdgeId(wsId, repoId, source.id, 'react', 'imports'),
        workspaceId: wsId,
        repoId: repoId,
        sourceSymbolId: source.id,
        sourceFilePath: filePath,
      ...
      await ingest(symbols: [source, target], edges: [bound, external]);

      final pruned = await repo.pruneUnresolvedEdges(wsId, repoId);
      expect(pruned, 1);
      expect(await repo.countUnresolvedEdges(wsId, repoId), 0);
      // The bound edge still serves graph queries.
      expect((await repo.callees(wsId, source.id)).map((s) => s.id), [
        target.id,
      ]);
    });

    test('is scoped to one checkout partition', () async {
      // The worktree partition's rows reference an isolated_repos row
      // (checkout_id is FK'd to it).
      await db
      ...
      final s = symbol(name: 'main', qualifiedName: 'main');
      Future<void> ingestIn(String? checkoutId) => repo.ingestFile(
        workspaceId: wsId,
        repoId: repoId,
      ...
      // Pruning the linked partition must not touch the worktree's rows —
      // each partition's lifecycle is its own.
      expect(await repo.pruneUnresolvedEdges(wsId, repoId), 1);
      expect(await repo.countUnresolvedEdges(wsId, repoId), 0);
      expect(
        await repo.countUnresolvedEdges(wsId, repoId, checkoutId: 'wt1'),
        1,
        reason: 'a base run must never prune a worktree partition',
      );
    });
  });
