...
void main() {
  ...
  test('honors the persisted initial selection', () async {
    ...
    expect(
      ...
      VoiceModelInfo.whisperBaseEn.id,
    );
  });
  ...
  test(
    'selecting the already-active model is a no-op (no re-persist)',
    () async {
      final persisted = <String>[];
      final control = SelectableVoiceModelControl(
        paths: paths,
  ...
  test('watch first emission is the probed status, not unknown', () async {
    final control = SelectableVoiceModelControl(paths: paths);
    addTearDown(control.dispose);

    ModelStatusSnapshot? sync;
    final sub = control.watch().listen((s) => sync ??= s);
    // A synchronous first yield races the `sub/subscribe` ack and is dropped
    // by the client — this must stay async.
    expect(sync, isNull);

    final first = await control.watch().first;
    await sub.cancel();

    expect(first.status, ModelLifecycleStatus.notInstalled);
    expect(first.status, isNot(ModelLifecycleStatus.unknown));
  });
}
