...
void main() {
  ...
  test('a channel link bounces to the desktop route', () async {
    final resp = await request(
      '${ChatDeepLinks.pathPrefix}/workspaces/ws-1/channels/chan-1',
    );
    ...
    expect(
      body,
      contains('control-center://workspaces/ws-1/channels/chan-1'),
    );
  });
  ...
  test('the page reads nothing: an unknown workspace still bounces', () async {
    ...
    final resp = await request(
      '${ChatDeepLinks.pathPrefix}/workspaces/ws-nope/channels/chan-nope',
    );
  ...
  test('anything but a plain id is a 404, not a redirect', () async {
    for (final path in [
      ...
      '/open/workspaces/ws-1/channels/..%2F..%2Fetc',
      // A scheme smuggled into the id — the reason the shape check exists.
      '/open/workspaces/ws-1/channels/https:%2F%2Fevil.example',
      // An unknown kind, a missing segment, and a segment too many.
      '/open/workspaces/ws-1/secrets/x',
      '/open/workspaces/ws-1/channels',
      '/open/workspaces/ws-1/channels/chan-1/extra',
      '/open/ws-1/channels/chan-1',
      '/open/',
    ]) {
