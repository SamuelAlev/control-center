...
void main() {
  ...
  group('SamlService', () {
    ...
    test('rejects a replay inside the clock-skew tail', () async {
      ...
      final service = _makeService(
        _enabledConfig(allowIdpInitiated: true),
        fakes: _Fakes(),
        backend: backend,
      );
    ...
    test('autoMember none provisions the account but grants nothing', () async {
      ...
      final backend = _FakeBackend();
      final service = _makeService(
        _enabledConfig(autoMemberMode: SsoAutoMemberMode.none),
        fakes: fakes,
        backend: backend,
      );
      service.beginLogin(origin: _origin);
  ...
  group('SamlConfig clock skew', () {
    test('clamps an out-of-range skew to [0, 600]s', () {
      // A negative value would reject valid assertions; an unbounded one
      // silently widens the replay-acceptance window the native enforces.
      expect(SamlConfig.clampClockSkew(999999), const Duration(seconds: 600));
      expect(SamlConfig.clampClockSkew(-30), Duration.zero);
    });
  });
}

/// An enabled config with the knobs a test wants to vary.
SamlConfig _enabledConfig({
  String spEntityId = '',
  ...
  clockSkew: clockSkew ?? const Duration(seconds: 90),
);
