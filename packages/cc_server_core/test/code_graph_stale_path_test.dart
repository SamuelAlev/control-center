...
/// isolation rule that keeps one space's branch from erasing the index.
void main() {
  ...
  late Directory worktree; // the space's isolated copy
  ...
  setUp(() {
    ...
    // but not in this space's copy; `gone.dart` exists in neither.
  ...
  Future<Map<String, dynamic>> run(SearchCodeTool tool, {String? space}) async {
  ...
  test('hides symbols whose file is gone from the space copy', () async {
    final json = await run(searchTool(), space: spaceId);
    expect(
  ...
  test(
    ...
    () async {
      await run(searchTool(), space: spaceId);
      // `gone.dart` is absent from the checkout the index was built from → dead
      // row, pruned. `branch_only.dart` is missing from this space's copy
      // but still in the checkout → hidden from this caller, kept in the index,
      // or one space's branch would erase the shared graph.
  ...
  test('without a space, verifies against the linked checkout', () async {
  ...
  test('serves everything unverified when no tree port is wired', () async {
    final json = await run(searchTool(verify: false), space: spaceId);
