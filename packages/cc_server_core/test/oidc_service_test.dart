...
void main() {
  ...
  group('OidcConfig.fromEnvironment', () {
    ...
    test('parses the default role, groups claim and auto-member', () {
      ...
      expect(config.autoMemberMode, SsoAutoMemberMode.all);
    ...
    test('allowJit defaults on and honors an explicit opt-out', () {
      ...
      expect(
        OidcConfig.fromEnvironment(const {
          'CC_OIDC_ISSUER': 'https://idp.test',
          'CC_OIDC_CLIENT_ID': 'c',
          'CC_OIDC_ALLOW_JIT': 'false',
        }).allowJit,
        isFalse,
      );
    });

    test('issuer scheme gate: https and loopback http only', () {
      expect(OidcConfig.isIssuerAllowed('https://idp.test'), isTrue);
      expect(OidcConfig.isIssuerAllowed('http://localhost:8080'), isTrue);
      expect(OidcConfig.isIssuerAllowed('http://127.0.0.1:8080'), isTrue);
      expect(OidcConfig.isIssuerAllowed('http://idp.test'), isFalse);
      expect(OidcConfig.isIssuerAllowed('not-a-url'), isFalse);
    });
  });
  ...
  group('OidcService.beginLogin', () {
    ...
    test('refuses a plaintext (non-loopback) issuer before any HTTP', () async {
      ...
      final service = _buildService(
        ...
        workspaces: _FakeWorkspaceRepository(),
        members: _FakeMembershipRepository(),
        env: const {'CC_OIDC_ISSUER': 'http://idp.test'},
      );
      await expectLater(
        service.beginLogin(redirectUri: Uri.parse('https://app/cb')),
        throwsA(isA<AuthException>()),
    ...
    test('accepts a loopback http issuer (local development)', () async {
      final service = _buildService(
        ...
        workspaces: _FakeWorkspaceRepository(),
        members: _FakeMembershipRepository(),
        env: const {'CC_OIDC_ISSUER': 'http://localhost:8080'},
      );
      final url = await service.beginLogin(
    ...
    test(
      ...
      () async {
        ...
        final service = _buildService(
          ...
          workspaces: _FakeWorkspaceRepository(),
          members: _FakeMembershipRepository(),
          env: const {'CC_OIDC_CLIENT_SECRET': 'topsecret'},
        );
        beginUrl = await service.beginLogin(
          redirectUri: Uri.parse('https://app/cb'),
        );
    ...
    test(
      ...
      () async {
        ...
        final service = _buildService(
          ...
          workspaces: _FakeWorkspaceRepository(),
          members: _FakeMembershipRepository(),
          env: const {'CC_OIDC_ALLOW_JIT': 'false'},
        );
        beginUrl = await service.beginLogin(
          redirectUri: Uri.parse('https://app/cb'),
        );
