...
void main() {
  ...
  group('buildEvalsOps', () {
    ...
    test('the catalog declares every eval op', () {
      ...
      expect(
        ...
        containsAll([
          ...
          'evals.blessGolden',
          'evals.deleteGolden',
          'evals.configVersions',
          'evals.reliability',
        ]),
      );
    });

    test('evals.suites lists suites for the workspace', () async {
      repo.suitesByWsId['ws-1:s-1'] = _suite();
      repo.suitesByWsId['ws-2:s-2'] = _suite(id: 's-2', workspaceId: 'ws-2');
      final result = await op(
        buildEvalsOps(repository: repo),
        'evals.suites',
      ).handler(_ctx({'workspace_id': 'ws-1'}));
      final suites = result['suites'] as List;
      expect(suites, hasLength(1));
      expect((suites.first as Map)['id'], 's-1');
    });
    ...
    test('evals.runs lists runs for a suite', () async {
      ...
      repo.runsByWsId['ws-1:r-2'] = EvalRun(
        id: 'r-2',
        workspaceId: 'ws-1',
        suiteId: 's-other',
        configHash: 'h',
      ...
      expect((runs.first as Map)['id'], 'r-1');
    });
    ...
    test('evals.recordings lists recordings, filtered by agent', () async {
      repo.recordingsByWsId['ws-1:rec-1'] = SessionRecording(
        id: 'rec-1',
        workspaceId: 'ws-1',
        runLogId: 'l-1',
        agentId: 'a-1',
      ...
      repo.recordingsByWsId['ws-1:rec-2'] = SessionRecording(
        id: 'rec-2',
        workspaceId: 'ws-1',
        runLogId: 'l-2',
        agentId: 'a-2',
      ...
      final recs = result['recordings'] as List;
      expect(recs, hasLength(1));
      expect((recs.first as Map)['id'], 'rec-1');
    });

    group('evals.goldens', () {
      test('lists every golden when no agent is given', () async {
        repo.goldensByWsId['ws-1:g-1'] = GoldenSession(
          id: 'g-1',
          workspaceId: 'ws-1',
          agentId: 'a-1',
          recordingId: 'rec-1',
        ...
        repo.goldensByWsId['ws-1:g-2'] = GoldenSession(
          id: 'g-2',
          workspaceId: 'ws-1',
          agentId: 'a-2',
          recordingId: 'rec-2',
        ...
        expect(result['goldens'] as List, hasLength(2));
      });

      test('filters to one agent when agent_id is supplied', () async {
        repo.goldensByWsId['ws-1:g-1'] = GoldenSession(
          id: 'g-1',
          workspaceId: 'ws-1',
          agentId: 'a-1',
        ...
        final result = await op(
          buildEvalsOps(repository: repo),
          'evals.goldens',
        ).handler(_ctx({'workspace_id': 'ws-1', 'agent_id': 'a-2'}));
        final goldens = result['goldens'] as List;
        expect(goldens, hasLength(1));
        expect((goldens.first as Map)['id'], 'g-2');
      });
    });
    ...
    test('evals.deleteGolden removes the golden', () async {
      repo.goldensByWsId['ws-1:g-1'] = GoldenSession(
        id: 'g-1',
        workspaceId: 'ws-1',
        agentId: 'a-1',
      ...
      final result = await op(
        buildEvalsOps(repository: repo),
        'evals.deleteGolden',
      ).handler(_ctx({'workspace_id': 'ws-1', 'golden_id': 'g-1'}));
      expect(result['ok'], isTrue);
      expect(repo.deletedGolden, isTrue);
      expect(repo.goldensByWsId, isEmpty);
    });

    test('evals.configVersions lists versions for an agent', () async {
      ...
      expect((versions.first as Map)['id'], 'v-1');
    });
  ...
  group('buildEvalsWatchQueries', () {
    ...
    test('evals.watchRecordings emits a recordings snapshot', () async {
      repo.recordingsByWsId['ws-1:rec-1'] = SessionRecording(
        id: 'rec-1',
        workspaceId: 'ws-1',
        runLogId: 'l-1',
      ...
      expect(snapshot['recordings'] as List, hasLength(1));
    });

    test('evals.watchGoldens emits a goldens snapshot', () async {
      repo.goldensByWsId['ws-1:g-1'] = GoldenSession(
        id: 'g-1',
        workspaceId: 'ws-1',
        agentId: 'a-1',
      ...
      expect(snapshot['goldens'] as List, hasLength(1));
    });
