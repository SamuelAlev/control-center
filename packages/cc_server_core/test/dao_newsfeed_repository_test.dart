...
class FakeRssFetcherService extends RssFetcherService {
  ...
  Future<ParsedFeed> fetchAndParseFeed({
    ...
    return _feeds[feedId] ?? const ParsedFeed(articles: []);
  }
}
...
void main() {
  ...
  group('feed icon resolution', () {
    test('persists the channel image the feed advertises (TechCrunch case)', () async {
      final fetcher = FakeRssFetcherService();
      ...
      final stored = await dao.getFeedByUrl(feed.url);
      expect(
        stored!.iconUrl,
        'https://techcrunch.example.com/wp-content/uploads/2015/02/cropped-cropped-favicon-gradient.png?w=32',
      );
      // The channel image wins; the site is never fetched for an icon.
    ...
    test(
      'uses the feed URL as the site when the channel has no link',
      () async {
        final fetcher = FakeRssFetcherService();
        final icons = FakeSiteIconResolver();
        final refreshRepo = DaoNewsfeedRepository(
    ...
    test('a refreshed channel image replaces a rebranded icon', () async {
      final fetcher = FakeRssFetcherService();
      final icons = FakeSiteIconResolver();
      final refreshRepo = DaoNewsfeedRepository(
        dao,
    ...
    test(
      ...
      () async {
        ...
        var stored = await dao.getFeedByUrl(feed.url);
        ...
        await refreshRepo.refreshFeed(feed.id);
        stored = await dao.getFeedByUrl(feed.url);
        expect(stored!.iconUrl, 'https://news.example.com/favicon.png');
      },
    );

    test(
      'an SVG channel image is not stored; the site resolver takes over',
      () async {
        final fetcher = FakeRssFetcherService();
        final icons = FakeSiteIconResolver();
        final refreshRepo = DaoNewsfeedRepository(
  ...
  group('workspace scoping', () {
    ...
    test('repository operates without workspaceId parameter', () async {
      ...
      await repo.setFeedEnabled(feed.id, enabled: false);
      final disabledFeed = (await repo.watchFeeds().first).singleWhere(
        (f) => f.id == feed.id,
      );
    ...
    test(
      'feeds are globally visible regardless of workspace context',
      () async {
        final feed = await repo.addFeed(
          name: 'Global Feed',
    ...
    test(
      'articles are globally visible regardless of workspace context',
      () async {
        final feed = await repo.addFeed(
    ...
    test('deleteFeed cascade is not gated by workspace', () async {
      ...
      expect(
        (await repo.watchFeeds().first).where((f) => f.id == feed.id),
        isEmpty,
      );
    });
    ...
    test('markAllRead operates globally across all feeds', () async {
      ...
      await dao.upsertArticleFromFeed(
        _articleCompanion(
          ...
          guid: 'mg1',
          title: 'Global 1',
          link: 'https://mark-global.example.com/1',
        ),
      );
      await dao.upsertArticleFromFeed(
        _articleCompanion(
          ...
          guid: 'mg2',
          title: 'Global 2',
          link: 'https://mark-global.example.com/2',
        ),
      );
