...
void main() {
  test('connects, bridges tools and reports status', () async {
    ...
    final manager = ConnectionManager(transportFactory: (_) async => transport);
  ...
  test('reconnects and restores tools after an unexpected drop', () async {
    var built = 0;
    late FakeServerTransport current;
    final manager = ConnectionManager(
      reconnectBackoff: const [Duration(milliseconds: 1)],
      transportFactory: (_) async {
        built++;
        return current = FakeServerTransport(tools: ['echo']);
      },
    );
    addTearDown(manager.shutdown);
    ...
    // Drop the live space → the manager should dial a fresh transport on
    // backoff and come back online with its tools intact.
    current.drop();
    await Future<void>.delayed(const Duration(milliseconds: 100));

    expect(built, greaterThanOrEqualTo(2));
    expect(manager.statuses.single.lifecycle, McpServerLifecycle.connected);
    expect(manager.tools.map((t) => t.name), ['mcp__srv__echo']);
  });

  test('manual reconnect recovers after the breaker opens', () async {
    var flapping = true;
    final manager = ConnectionManager(
      reconnectBurstLimit: 2,
      reconnectBurstWindow: const Duration(seconds: 30),
      reconnectBackoff: const [Duration(milliseconds: 1)],
      transportFactory: (_) async {
        final t = FakeServerTransport(tools: ['echo']);
        if (flapping) {
    ...
    // Let the flap loop trip the breaker.
    await Future<void>.delayed(const Duration(milliseconds: 300));
    expect(manager.statuses.single.lifecycle, McpServerLifecycle.circuitOpen);

    // Stop flapping and manually reconnect → the breaker window resets and the
    // connection re-establishes.
    flapping = false;
    await manager.reconnect('srv', manual: true);
    await Future<void>.delayed(const Duration(milliseconds: 20));
    expect(manager.statuses.single.lifecycle, McpServerLifecycle.connected);
    expect(manager.tools.map((t) => t.name), ['mcp__srv__echo']);
  });
