  ...
  group('discover() merges and lets workspace win', () {
    test('workspace config overrides same-named user config', () async {
      ...
      final discovery = McpConfigDiscovery(
        homeDir: '/home',
        workspaceDir: '/ws',
        // The discovery layer asks with p.join-built (platform-separator)
        // paths while this fixture map is keyed POSIX-style; normalize so the
        // fake answers on Windows too.
        readFile: (path) async => files[path.replaceAll('\\', '/')],
      );
