...
import 'package:cc_mcp_client/src/tool_list_cache.dart';
...
class ConnectionManager {
  ...
  ConnectionManager({
    ...
    this.startupGate = const Duration(milliseconds: 250),
    McpToolListCache? toolCache,
  ...
  /// How long [connectAll] waits for connects to settle before returning and
  /// letting the stragglers finish in the background.
  ///
  /// This is a gate, not a timeout: a server still dialling when the gate
  /// closes is NOT cancelled and NOT marked failed — it keeps connecting and
  /// registers its tools through [onToolsChanged] when it lands. What the gate
  /// buys is that the caller's latency is bounded by the gate rather than by
  /// the slowest server on the machine.
  final Duration startupGate;

  /// Remembers each server's last-known tool list, so a server that is still
  /// connecting when the gate closes can advertise its tools immediately (the
  /// first CALL then waits for the connection). Defaults to a cache that never
  /// hits, which leaves the gate's timeout half working on its own.
  final McpToolListCache _toolCache;
  ...
  Future<void> connectAll(List<McpServerConfig> configs) async {
    ...
    // Return once every connect has settled OR the startup gate closes,
    // whichever comes first. Awaiting all of them made the caller's latency
    // the SLOWEST server's latency: one wedged stdio server that takes ten
    // seconds to answer `initialize` held up every other server's tools for
    // ten seconds, even though they were ready in milliseconds.
    //
    // The stragglers are deliberately not cancelled. `connect()` emits
    // `_emitToolsChanged()` on its own completion, so a late server still
    // registers its tools — it just does so after this future resolves.
    // `connect()` swallows its own failures, but guard anyway: an error
    // reaching `Future.any` would make `connectAll` throw and abort the boot
    // step, which is exactly the fragility this gate exists to remove.
    final settled = Future.wait(tasks).then<void>((_) {}).catchError((_) {});
    await Future.any([
      settled,
      Future<void>.delayed(startupGate),
    ]);
  }
  ...
  Future<void> connect(McpServerConfig config) async {
    ...
    // Advertise the last-known tools while the dial is in flight, so a run
    // starting inside the connect window still sees this server. They are
    // marked deferred: `_invoke` waits for [conn.pendingConnect] before
    // calling, so a cached tool never fails merely for being early.
    final fingerprint = mcpToolCacheFingerprint(config);
    if (conn.tools.isEmpty) {
      final cached = _toolCache.read(config.name, fingerprint);
      if (cached != null && cached.isNotEmpty) {
        conn
          ...
          ..toolsAreDeferred = true;
        _log(
          'info',
          'server "${config.name}" is still connecting; offering '
              '${cached.length} cached tool(s) in the meantime',
        );
        _emitToolsChanged();
      }
    }
    ...
    conn.pendingConnect = completer.future;
    try {
      ...
      conn.toolsAreDeferred = false;
      conn.lifecycle = McpServerLifecycle.connected;
      _toolCache.write(config.name, fingerprint, [
        for (final t in conn.tools) t.remoteTool,
      ]);
      _wireNotifications(conn);
      _watchClose(conn);
      _log(
        'info',
        'connected to "${config.name}" (${conn.tools.length} tools)',
      );
    } on Object catch (e) {
      ...
      // A server that failed must stop advertising its cached tools: handing
      // the model a menu of calls that cannot succeed is worse than showing
      // it nothing. `teardown()` clears the list; the cache entry goes too,
      // so the next boot does not re-offer them.
      await conn.teardown();
      conn.toolsAreDeferred = false;
      _toolCache.evict(config.name);
    } finally {
  ...
  void _watchClose(_ServerConnection conn) {
    ...
    unawaited(
      client.done.then((_) {
        if (_shuttingDown || conn.lifecycle == McpServerLifecycle.disabled) {
          return;
        }
  ...
  Future<Map<String, dynamic>> _invoke(
    ...
    // A tool offered from cache while the server was still dialling: wait for
    // that dial here, on the one call that actually needs it, rather than
    // having made every session wait for it at startup. Bounded by the
    // server's own timeout so a wedged connect surfaces as a tool error
    // instead of hanging the turn.
    final pending = conn.pendingConnect;
    if (pending != null && conn.client == null) {
      await pending.timeout(
        conn.config.timeout,
        onTimeout: () {
          throw McpTransportException(
            'server "$serverName" is still connecting',
          );
        },
      );
    }
...
class _ServerConnection {
  ...
  /// Completes when the in-flight [ConnectionManager.connect] settles, or null
  /// when no dial is in flight. `_invoke` awaits it so a tool offered from
  /// cache waits for the connection instead of failing "not connected".
  Future<void>? pendingConnect;

  /// Whether [tools] came from the cache rather than a live `tools/list`.
  /// Paired with a `connecting` lifecycle this is what distinguishes "no tools
  /// yet" from "last-known tools, call will wait".
  bool toolsAreDeferred = false;
