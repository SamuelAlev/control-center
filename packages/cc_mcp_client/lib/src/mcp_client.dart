...
class McpClient {
  ...
  Future<Map<String, dynamic>> _request(
    ...
    try {
      ...
      // The frame never left — don't orphan the completer in `_pending`.
      _pending.remove(id);
      rethrow;
    }
    // A non-positive timeout used to mean INFINITE, so a `Duration.zero` config
    // typo hung every call to that server forever. Treat it as "use the
    // default" instead: an MCP call always has a deadline.
    final configured = timeout ?? const Duration(seconds: 30);
    final effective = configured.inMilliseconds <= 0
  ...
  void _onFrame(Map<String, dynamic> frame) {
    ...
    if (frame.containsKey('method')) {
      ...
      // Server→client *requests* (sampling/elicitation) are not supported.
      // Silence is the wrong answer: JSON-RPC requires a response, and a server
      // that waits for one blocks its own turn until its timeout. Answer with
      // `-32601 Method not found`, which is exactly "capability absent".
      if (id != null) {
        unawaited(
          transport
              .send({
                ...
                'error': {
                  ...
                  'message': 'method not found: $method '
                      '(this client serves no server→client requests)',
                },
              })
              .catchError((Object _) {
                // The link is gone; the server will notice by other means.
              }),
        );
      }
      return;
