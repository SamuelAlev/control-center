...
/// maps) in both directions and reports when the underlying space dies. All
...
abstract interface class McpTransport {
  /// Opens the space (spawns the process / dials the URL). Throws
  ...
  /// Completes when the space closes for any reason (process exit, stream
  /// EOF, network drop). Never completes with an error.
  Future<void> get done;

  /// Tears the space down. Idempotent.
  Future<void> close();
}
