...
class BackgroundProcessManager {
  ...
  Future<void> _terminate(_ManagedProcess managed) async {
    ...
    // One shared ladder (reap tree → SIGTERM → 3s → SIGKILL): this is where it
    // was first got right, and the MCP stdio transport now calls the same
    // helper instead of carrying a bare SIGTERM.
    await terminateProcessTree(managed.process);
  }
