...
///
/// String values match the upstream clients so they
...
enum McpServerLifecycle {
  ...
  static McpServerLifecycle fromWire(String? raw) => switch (raw) {
    ...
    'needs_client_registration' => McpServerLifecycle.needsClientRegistration,
...
class McpRemoteTool {
  ...
  /// Round-trips through [McpRemoteTool.fromJson]. Used by the tool-list cache
  /// so a still-connecting server can still advertise its last-known tools.
      ...
      McpRemotePrompt(
        ...
        arguments:
            (json['arguments'] as List?)
                ?.whereType<Map>()
                .map(
                  (a) => McpPromptArgument.fromJson(a.cast<String, dynamic>()),
                )
                .toList() ??
            const [],
      );
...
class McpServerCapabilities {
  ...
  factory McpServerCapabilities.fromJson(Map<String, dynamic> json) {
    ...
    bool listChanged(Object? cap) => cap is Map && cap['listChanged'] == true;
