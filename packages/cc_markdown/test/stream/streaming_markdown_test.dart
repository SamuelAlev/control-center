...
void main() {
  ...
  group('style & builders environment change', () {
    ...
    testWidgets('a new callback/builder prop invalidates the memo', (
      tester,
    ) async {
      // Regression: only `style`/`builders` were part of the memo key, but a
      // memoized sealed block also captures `onTapLink`, `onTapImage`,
      // `imageBuilder`, `codeBuilder` and `selectable` through its render
      // context. A parent re-passing any of them kept the OLD one alive on
      // every sealed block until the stream happened to mutate — a tap going
      // to a dead handler, or a code block rendered by a builder that has
      // since been replaced.
      //
      // `codeBuilder` is the one whose staleness is directly VISIBLE, so it
      ...
      await tester.pumpWidget(
        _host(
          CcStreamingMarkdown(
            controller: controller,
            codeBuilder: (code, language, {required cache}) =>
                const Text('FIRST BUILDER'),
          ),
        ),
      );
      ...
      await tester.pumpWidget(
        _host(
          CcStreamingMarkdown(
            controller: controller,
            codeBuilder: (code, language, {required cache}) =>
                const Text('SECOND BUILDER'),
          ),
        ),
      );
      await tester.pump();

      expect(find.text('SECOND BUILDER'), findsOneWidget);
      expect(
        find.text('FIRST BUILDER'),
        findsNothing,
        reason: 'the memoized block must not keep the replaced builder',
      );
    });
