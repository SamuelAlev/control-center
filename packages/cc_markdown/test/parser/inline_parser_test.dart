...
void main() {
  ...
  group('parseInline', () {
    test('recognizes the inline constructs a table cell may carry', () {
      final nodes = parser.parseInline('`x` is *a* **b** [c](https://d.e)');
      expect(nodes.whereType<CcInlineCode>().single.code, 'x');
      expect(nodes.whereType<CcEmphasis>(), hasLength(1));
      expect(nodes.whereType<CcStrong>(), hasLength(1));
      expect(nodes.whereType<CcLink>().single.url, 'https://d.e');
    });

    test('leaves block markers as literal text', () {
      // The point of the entry point: a cell is not a block context, so a
      // leading marker is a character the operator typed, not a construct.
      // Block-parsing and digging the paragraph out would lose both of these.
      expect(textOf(parser.parseInline('- not a list')), '- not a list');
      expect(textOf(parser.parseInline('# not a heading')), '# not a heading');
    });

    test('empty input yields no nodes', () {
      expect(parser.parseInline(''), isEmpty);
    });
  });
}
