...
class CcMermaidTextPainterRuler extends CcMermaidTextRuler {
  ...
  /// Measured painters, in least-recently-used order (a `Map` keeps insertion
  /// order, and a hit re-inserts).
  final Map<String, TextPainter> _cache = {};

  /// Cap on retained painters.
  ...
  TextPainter _painter(String text, CcMermaidTextRole role, {Color? color}) {
    ...
    if (cached != null) {
      // Re-insert to mark recency: `Map` preserves insertion order, so
      // removing and re-adding moves the entry to the end and makes
      // `_cache.keys.first` the least-recently-USED one.
      _cache
        ..remove(key)
        ..[key] = cached;
      return cached;
    ...
    // ruler outlives a single frame. Evict the least-recently-used entry
    // rather than clearing everything — a wholesale clear at the cap meant a
    // re-measure storm of the ENTIRE visible diagram every time one new label
    // pushed it over, which is exactly what heavy diagram scrolling does.
