...
final class _BlockParser {
  ...
  List<_Raw> parseRaw(List<String> lines, {int depth = 0}) {
    ...
    while (i < lines.length) {
      ...
      // Details. Depth-guarded like blockquotes and lists: `_parseDetails`
      // recurses through `parseRaw(..., depth: depth + 1)`, so N nested
      // `<details>` blocks recursed N deep and a ~100k-line document died with
      // a StackOverflowError — an `Error`, which no `catch` in this engine's
      // "never throws" contract survives. cc_markdown renders LLM output, so
      // that input is reachable from model text.
      if (options.details &&
          depth < options.maxBlockDepth &&
          _detailsOpen.hasMatch(line)) {
        closeParagraph();
