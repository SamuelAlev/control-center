  ...
  Widget build(BuildContext context) {
    ...
    return CustomPaint(
      foregroundPainter: _CcLinkUnderlinePainter(
        textKey: _textKey,
        // A defensive copy: `_linkRanges` is a mutable field cleared on every
        // build, and the painter compares it against the previous delegate's.
        ranges: List<(int, int)>.of(_linkRanges),
...
class _CcLinkUnderlinePainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _CcLinkUnderlinePainter oldDelegate) =>
      // Compared rather than an unconditional `true`. The paint depends on
      // exactly these four inputs (the paragraph is looked up through
      // [textKey], so a same-key paragraph whose layout changed already
      // triggers a repaint through its own render object). An unconditional
      // repaint would also defeat any future span-identity memoization above.
