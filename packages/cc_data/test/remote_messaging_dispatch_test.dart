...
/// The workspace that owns every space these tests address. A workspace id
/// selects the database file server-side, so a space-addressed op is only
...
/// (space lifecycle + agent dispatch + steering) over an in-process JSON-RPC
...
void main() {
  ...
  group('RemoteMessagingDispatch space lifecycle', () {
    test(
      ...
      () async {
        ...
        // conversation (see the conversation_messages rebuild in
        // app_database.dart).
    ...
    test('addAgentToSpace sends dispatch.addAgentToSpace', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.addAgentToSpace(_ws, 'c-1', 'a-1');
      final call = host.lastCall('dispatch.addAgentToSpace')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    ...
    test('deleteSpace sends messaging.deleteSpace', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.deleteSpace(_ws, 'c-1');
      final call = host.lastCall('messaging.deleteSpace')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    });

    test('archiveSpace sends messaging.archiveSpace', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.archiveSpace(_ws, 'c-1');
      final call = host.lastCall('messaging.archiveSpace')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    });

    test('unarchiveSpace sends messaging.unarchiveSpace', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.unarchiveSpace(_ws, 'c-1');
      final call = host.lastCall('messaging.unarchiveSpace')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    });

    test('updateSpaceName sends messaging.updateSpaceName', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.updateSpaceName(_ws, 'c-1', 'New name');
      final call = host.lastCall('messaging.updateSpaceName')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
      expect(call.args['name'], 'New name');
    });

    test('getSpaceRepos decodes the selection', () async {
      host.callResults['messaging.getSpaceRepos'] = {
        'repo_ids': ['r-1', 'r-2'],
      };
      final dispatch = RemoteMessagingDispatch(client);
      final selection = await dispatch.getSpaceRepos(_ws, 'c-1');
      expect(selection, ['r-1', 'r-2']);
      final call = host.lastCall('messaging.getSpaceRepos')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    });

    test('getSpaceRepos decodes the all-repos default as null', () async {
      host.callResults['messaging.getSpaceRepos'] = {'repo_ids': null};
      final dispatch = RemoteMessagingDispatch(client);
      expect(await dispatch.getSpaceRepos(_ws, 'c-1'), isNull);
    });

    test('setSpaceRepos sends the selection, null meaning all repos', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.setSpaceRepos(_ws, 'c-1', ['r-1']);
      var call = host.lastCall('messaging.setSpaceRepos')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
      expect(call.args['repo_ids'], ['r-1']);

      await dispatch.setSpaceRepos(_ws, 'c-1', null);
      call = host.lastCall('messaging.setSpaceRepos')!;
      expect(call.args.containsKey('repo_ids'), isFalse);
    });

    test('clearSpaceMessages sends messaging.clearSpaceMessages', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.clearSpaceMessages(_ws, 'c-1');
      final call = host.lastCall('messaging.clearSpaceMessages')!;
      expect(call.args['workspace_id'], _ws);
      expect(call.args['space_id'], 'c-1');
    });

    test('createSpace decodes the returned SpaceDto', () async {
      host.callResults['messaging.createSpace'] = {
        'space': {
          'id': 'c-9',
          'name': 'Team',
          'workspace_id': _ws,
    ...
    test('createSpace forwards the pipelineRunId when given', () async {
      host.callResults['messaging.createSpace'] = {
        'space': {'id': 'c-9', 'name': 'P', 'workspace_id': ''},
      };
      final dispatch = RemoteMessagingDispatch(client);
  ...
  group('RemoteMessagingDispatch agent dispatch', () {
    ...
    // The regression this guards: the composer uploaded every attachment and
    // built the metadata, and this call dropped it on the floor. The message
    // landed with no attachments, the transcript showed none, and the agent was
    // handed a sentence naming screenshots it had never been given.
    test('sendAndDispatch carries the attachment metadata', () async {
      final dispatch = RemoteMessagingDispatch(client);
      await dispatch.sendAndDispatch(
        _ws,
        'c-1',
      ...
      final call = host.lastCall('dispatch.sendAndDispatch')!;
      final attachments =
          (call.args['metadata'] as Map)['attachments'] as List;
      expect((attachments.single as Map)['name'], 'shot.png');
    });
  ...
  group('RpcMessagingPort full delegation', () {
    test('createSpace returns a Space decoded from the dto', () async {
      host.callResults['messaging.createSpace'] = {
        'space': {
          'id': 'c-9',
          'name': 'Team',
      ...
      final space = await port.createSpace(_ws, 'Team', ['a-1']);
      expect(space.id, 'c-9');
      expect(space.name, 'Team');
      expect(space.workspaceId, _ws);
      expect(space.mode, Mode.chat);
      expect(host.lastCall('messaging.createSpace')!.args['workspace_id'], _ws);
    });
    ...
    test(
      ...
      () async {
        ...
        await port.addAgentToSpace(_ws, 'c-1', 'a-1');
        await port.removeParticipant(_ws, 'c-1', 'a-1');
        await port.deleteSpace(_ws, 'c-1');
        await port.clearSpaceMessages(_ws, 'c-1');
        final send = host.lastCall('dispatch.sendUserMessage')!;
        expect(send.args['content'], 'hi');
        expect(send.args['workspace_id'], _ws);
        expect(
          host.lastCall('dispatch.addAgentToSpace')!.args['agent_id'],
          'a-1',
        );
        expect(
        ...
        expect(host.lastCall('messaging.deleteSpace')!.args['space_id'], 'c-1');
        final clear = host.lastCall('messaging.clearSpaceMessages')!;
        expect(clear.args['space_id'], 'c-1');
    ...
    test(
      ...
      () async {
        ...
        await port.refinePlan(workspaceId: _ws, spaceId: 'c-1', feedback: 'f');
