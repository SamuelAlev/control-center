...
void main() {
    ...
    () async {
      ...
      host.push({
        ...
        'changes': [
          ...
          {
            'tbl': 'conversation_messages',
            'pk': 'm1',
            'op': 'upsert',
            'row': {'id': 'm1', 'space_id': 'c1'},
          },
        ],
      });
      await _settle();
      host.push({
        ...
        'seq': 4,
        'changes': [
          {'tbl': 'spaces', 'pk': 'c1', 'op': 'delete'},
        ],
      });
      await _settle();
      final spaces = <List<Map<String, dynamic>>>[];
      final messages = <List<Map<String, dynamic>>>[];
      final s1 = store.watchRows('spaces').listen(spaces.add);
      final s2 = store.watchRows('conversation_messages').listen(messages.add);
      await _settle();
      expect(spaces.last, isEmpty);
  ...
  test('kill-switch paths: a pruned pull or an unknown wire version drops the '
    ...
    final (server2, space2) = InProcessRpcChannel.pair();
    _SyncHost(server2);
    final client2 = RemoteRpcClient(space2)..start();
