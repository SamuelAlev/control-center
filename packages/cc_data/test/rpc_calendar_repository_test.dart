...
void main() {
  ...
  group('RpcCalendarRepository events', () {
    ...
    test(
      'watchEventsInRange parses an all-day bare date as LOCAL midnight',
      () async {
        // All-day events cross the wire as a bare `YYYY-MM-DD` civil date
        // (see calendarEventToWire): the reader's own local midnight is what
        // keeps the event on the same day in every timezone.
        host.snapshotFor('calendar.watchEventsInRange', {
          'events': [
            {
              'id': 'ev-ad',
              'account_id': 'acc-1',
              'external_event_id': 'ext-ad',
              'calendar_id': 'primary',
        ...
        expect(e.startTime, DateTime(2026, 8, 26));
        expect(e.startTime.isUtc, isFalse);
        expect(e.endTime, DateTime(2026, 8, 27));
      },
    );
