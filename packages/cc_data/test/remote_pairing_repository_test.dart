...
class _PairingHost {
  ...
  void _onFrame(Map<String, dynamic> frame) {
    ...
    // Scripted error wins over everything else.
    ...
    // Scripted canned data wins over the default per-op payload.
    final scripted = callResults[op];
    if (scripted != null) {
      _data(id, op, scripted);
      return;
    }
    switch (op) {
      case 'pairing.mint':
...
void main() {
  ...
  test(
    'mint sends label + platform and parses the minted credential',
    () async {
  ...
  test(
    'rename sends device_id + label and parses the updated device',
    () async {
      final repo = RemotePairingRepository(client);
      final d = await repo.rename('dev1', 'Work laptop');
      expect(host.calls.single.args, {
        'device_id': 'dev1',
        'label': 'Work laptop',
      });
  ...
  group('RemotePairingRepository edge cases', () {
    test('list forwards the status filter', () async {
      final repo = RemotePairingRepository(client);
      await repo.list(status: 'revoked');
      expect(host.calls.single.args['status'], 'revoked');
    });

    test('list returns empty on opUnknown', () async {
      host.errorCodes['pairing.list'] = RpcErrorCodes.opUnknown;
      final repo = RemotePairingRepository(client);
      expect(await repo.list(), isEmpty);
    });

    test('list rethrows non-opUnknown errors', () async {
      host.errorCodes['pairing.list'] = RpcErrorCodes.internalError;
      final repo = RemotePairingRepository(client);
      expect(repo.list, throwsA(isA<RemoteRpcException>()));
    });

    test('list skips non-Map rows', () async {
      host.callResults['pairing.list'] = {
        'devices': ['junk', 42, null],
      };
      final repo = RemotePairingRepository(client);
      expect(await repo.list(), isEmpty);
    });
    ...
    test(
      'mint falls back to default values when optional fields are absent',
      () async {
        host.callResults['pairing.mint'] = {'device_id': 'd2', 'psk': 'p'};
        final repo = RemotePairingRepository(client);
        final mint = await repo.mint(label: 'L');
        expect(mint.serverUrl, '');
        expect(mint.signalingUrl, '');
        expect(mint.room, '');
        expect(mint.platform, 'web');
        expect(mint.descriptor, isNull);
        expect(mint.isDirectlyReachable, isFalse);
        expect(mint.canRelay, isFalse);
      },
    );

    test('mint detects relay reachability via signaling + room', () async {
      host.callResults['pairing.mint'] = {
        'device_id': 'd3',
        'psk': 'p',
        'signaling_url': 'wss://broker',
        'room': 'r1',
      };
      final repo = RemotePairingRepository(client);
      final mint = await repo.mint(label: 'L');
      expect(mint.canRelay, isTrue);
    });

    test('mint with non-Map descriptor treats it as null', () async {
      host.callResults['pairing.mint'] = {
        'device_id': 'd4',
        'psk': 'p',
        'descriptor': 'broken',
    ...
    test(
      'device maps invalid paired_at to epoch and empty last_seen_at to null',
      () async {
        host.callResults['pairing.list'] = {
          'devices': [
            {'device_id': 'd5', 'paired_at': 'not-a-date'},
          ],
        };
        final repo = RemotePairingRepository(client);
        final d = (await repo.list()).single;
        expect(d.pairedAt, DateTime.fromMillisecondsSinceEpoch(0));
