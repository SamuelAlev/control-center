...
void main() {
  ...
  group('RemotePrLifecycleRepository', () {
    ...
    test('updateDraft forwards the workspace + the optional fields', () async {
      ...
      expect(call.args['github_pr_number'], 42);
      expect(call.args['github_pr_url'], 'https://x');
    });

    test(
      'createOnGitHub forwards the publish args and returns the result map',
      () async {
        host.callResults['pr_lifecycle.createOnGitHub'] = {
          'result': {'number': 7, 'html_url': 'u'},
        };
        final repo = RemotePrLifecycleRepository(client);
        final result = await repo.createOnGitHub(
          workspaceId: 'ws-1',
          prId: 'pr-1',
          owner: 'o',
          repo: 'r',
        ...
        final call = host.lastCall('pr_lifecycle.createOnGitHub')!;
        expect(call.args['workspace_id'], 'ws-1');
        expect(call.args['pr_id'], 'pr-1');
    ...
    test('createOnGitHub returns an empty map when result is absent', () async {
      host.callResults['pr_lifecycle.createOnGitHub'] = const {};
      final repo = RemotePrLifecycleRepository(client);
      expect(
        await repo.createOnGitHub(
          workspaceId: 'ws-1',
          prId: 'pr-1',
          owner: 'o',
          repo: 'r',
  ...
  group('RpcPrLifecycleRepository', () {
    ...
    test('createOnGitHub delegates and returns the result map', () async {
      host.callResults['pr_lifecycle.createOnGitHub'] = {
        'result': {'number': 9},
      };
      final repo = RpcPrLifecycleRepository(client);
      ...
      expect(
        host.lastCall('pr_lifecycle.createOnGitHub')!.args['workspace_id'],
        'ws-1',
      );
    });
  });
