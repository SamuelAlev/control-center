...
void main() {
  ...
  test(
    ...
    () async {
      host.snapshotFor('workProduct.watchForSpace', {
        'work_products': [wireProduct()],
      });
      final products = await RpcWorkProductRepository(
        client,
      ).watchForSpace('chan-1', conversationId: 'conv-2').first;
      expect(products.single.id, 'wp-1');
      expect(host.lastSubscribe!.query, 'workProduct.watchForSpace');
      expect(host.lastSubscribe!.args['space_id'], 'chan-1');
      expect(host.lastSubscribe!.args['conversation_id'], 'conv-2');
    },
  );

  test('watchForChannel omits conversation_id when not supplied', () async {
    host.snapshotFor('workProduct.watchForSpace', const {
      'work_products': <Object>[],
    });
    await RpcWorkProductRepository(client).watchForSpace('chan-1').first;
