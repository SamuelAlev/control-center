...
void main() {
  ...
  group('RpcTicketRepository', () {
    ...
    test('forAgent / childrenOf ask the server, not the whole workspace', () async {
      // These used to pull EVERY ticket in the workspace and `where` them in
      // Dart, so a 2,000-ticket workspace shipped 2,000 wire rows to answer
      // "what is this agent working on?". They are scoped server ops now, and
      // what matters is that the scope reaches the host — a client-side filter
      // over a full pull would pass an assertion on the RESULT just as well.
      host.callResults['tickets.listForAgent'] = {
        'tickets': [
          {'ticket_id': 't1', 'title': 'A', 'assignee': 'a1'},
        ],
      };
      host.callResults['tickets.listChildren'] = {
        'tickets': [
          {'ticket_id': 't1', 'title': 'A', 'parent_ticket_id': 'p1'},
          {'ticket_id': 't2', 'title': 'B', 'parent_ticket_id': 'p1'},
        ],
      };
      final repo = RpcTicketRepository(client);

      expect((await repo.forAgent('ws1', 'a1')).single.id, 't1');
      expect(host.lastCall('tickets.listForAgent')?.args, {
        'workspace_id': 'ws1',
        'agent_id': 'a1',
      });

      expect((await repo.childrenOf('ws1', 'p1')).length, 2);
      expect(host.lastCall('tickets.listChildren')?.args, {
        'workspace_id': 'ws1',
        'parent_ticket_id': 'p1',
      });

      expect(
        host.subs.where((s) => s.query == 'tickets.watchForWorkspace'),
        isEmpty,
        reason: 'a scoped read must not open a whole-workspace subscription',
      );
    });
