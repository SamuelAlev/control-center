...
void main() {
  ...
  group('RpcWorkspaceFilesystemPort path accessors', () {
    test(
      ...
      () async {
        ...
        host.callResults['fs.conversationsDir'] = {'path': '/srv/ws/convos'};
        ...
        expect(await port.conversationsDir('ws'), '/srv/ws/convos');
        expect(await port.skillsDir('ws'), '/srv/ws/skills');
        expect(await port.agentsDir('ws'), '/srv/ws/agents');
      },
    );

    test('conversationDir passes the conversation_id', () async {
      host.callResults['fs.conversationDir'] = {'path': '/srv/ws/convos/c-1'};
      final port = RpcWorkspaceFilesystemPort(client);
      expect(await port.conversationDir('ws', 'c-1'), '/srv/ws/convos/c-1');
      expect(
        host.lastCall('fs.conversationDir')!.args['conversation_id'],
        'c-1',
      );
    });
  ...
  group('RpcWorkspaceFilesystemPort mutations', () {
    test('ensureConversationDir returns the ensured path', () async {
      host.callResults['fs.ensureConversationDir'] = {
        'path': '/srv/ws/convos/c-1',
      };
      final port = RpcWorkspaceFilesystemPort(client);
      expect(
        await port.ensureConversationDir('ws', 'c-1'),
        '/srv/ws/convos/c-1',
      );
    });
  ...
  group('RpcWorkspaceFilesystemPort workspace scoping', () {
    test('every op targets the workspace it was handed, not the client '
      ...
      await port.conversationsDir(target);
      await port.conversationDir(target, 'c-1');
      await port.ensureConversationDir(target, 'c-1');
    ...
    test('no fs.* call site omits workspace_id', () {
      ...
      final callSites = RegExp(
        r"_client\.call\(\s*'(fs\.[A-Za-z]+)'\s*,\s*(const\s*)?\{([^}]*)\}",
        multiLine: true,
