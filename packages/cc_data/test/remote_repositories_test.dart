...
class _FakeHost {
  ...
  final List<Map<String, dynamic>> subscribeArgs = [];
  ...
  final List<Map<String, dynamic>> sentReviewSpaces = [];
  ...
  void _repoCall(dynamic id, Map<String, dynamic> params) {
    ...
    switch (op) {
      ...
      case 'messaging.getParticipants':
        _replyData(id, op, {
          'participants': [
            _participantJson(
              'p1',
              args['space_id'] as String? ?? 'c1',
              'user-1',
              participantType: 'user',
            ),
          ],
        });
      case 'messaging.setSpaceMode':
      case 'messaging.addParticipant':
      case 'messaging.updateMessage':
      case 'messaging.deleteSpace':
      case 'messaging.archiveSpace':
      case 'messaging.unarchiveSpace':
      case 'messaging.updateSpaceName':
      case 'messaging.setSpaceRepos':
      case 'messaging.clearSpaceMessages':
      case 'messaging.removeParticipant':
        sentMessagingMutations.add({'op': op, ...args});
        _replyData(id, op, {'ok': true});
      case 'messaging.revertConversationTo':
        sentMessagingMutations.add({'op': op, ...args});
        _replyData(id, op, {
          'affected_message_ids': ['m2', 'm3'],
          'filesystem_restored': true,
        });
      case 'messaging.unrevertConversation':
        sentMessagingMutations.add({'op': op, ...args});
        _replyData(id, op, {
          'affected_message_ids': ['m2', 'm3'],
        });
      case 'messaging.createSpace':
        sentMessagingMutations.add({'op': op, ...args});
        _replyData(id, op, {
          'space': _spaceJson('grp-1', args['name'] as String),
        });
      ...
      case 'newsfeed.getArticle':
        // A scoped point lookup now, not a scan of every article across every
        // subscribed feed. The server resolves it with an indexed join through
        // the user's feeds, so an id that is not theirs is simply not found.
        final articleId = args['article_id'] as String?;
        if (articleId == 'a1') {
          _replyData(id, op, {'article': _articleJson('a1', 'Headline')});
        } else {
          _replyError(id, RpcErrorCodes.notFound, 'Article not found');
        }
      ...
      case 'memory_fact.getByWorkspace':
        _replyData(id, op, {
          'facts': [
            _factJson('mf1', 'preferences'),
            _factJson('mf2', 'codebase'),
          ],
        });
        ...
        _replyData(id, op, {
          'facts': [
            _factJson(
              'mf1',
              'preferences',
              author: args['agent_id'] as String?,
            ),
          ],
        });
      ...
      case 'memory_policy.getActiveByWorkspace':
        _replyData(id, op, {
          'policies': [
            _policyJson('p1', args['domain'] as String? ?? 'coding'),
          ],
        });
      ...
      case 'memory_policy.delete':
        sentPolicies.add(args);
        _replyData(id, op, {'ok': true});
      case 'review_space.create':
        sentReviewSpaces.add(args);
        _replyData(id, op, {
          'association': _reviewSpaceJson(
            'rc-new',
            args['space_id'] as String,
            prExternalId: args['pr_external_id'] as String,
            prNumber: (args['pr_number'] as num).toInt(),
            repoFullName: args['repo_full_name'] as String,
          ),
        });
      case 'review_space.updateStatus':
        sentReviewSpaces.add(args);
        _replyData(id, op, {'ok': true});
        ...
        _replyData(id, op, {
          'logs': [
            _runLogJson(
              ...
              'a1',
              pipelineRunId: args['pipeline_run_id'] as String?,
              pipelineStepId: args['pipeline_step_id'] as String?,
            ),
          ],
        });
      ...
      case 'isolated_repo.forSpace':
        _replyData(id, op, {
          'repos': [
            _isolatedRepoJson('ir1', spaceId: args['space_id'] as String),
          ],
        });
      case 'isolated_repo.forTicket':
        _replyData(id, op, {
          'repos': [
            _isolatedRepoJson('ir1', ticketId: args['ticket_id'] as String),
          ],
        });
      case 'isolated_repo.forSpaceAcrossWorkspaces':
        _replyData(id, op, {
          'repos': [
            _isolatedRepoJson('ir1', spaceId: args['space_id'] as String),
          ],
      ...
      case 'project.getForWorkspace':
        _replyData(id, op, {
          'projects': [
            _projectJson('p1', 'First'),
            _projectJson('p2', 'Second'),
          ],
        });
      case 'project.insert':
      ...
      case 'ticket_link.getForTicket':
        _replyData(id, op, {
          'links': [_ticketLinkJson('tl1', args['ticket_id'] as String, 't2')],
        });
      ...
      case 'pipeline_template.forWorkspace':
        _replyData(id, op, {
          'templates': [
            _pipelineTemplateJson('pt1', 'PR review', isBuiltIn: true),
          ],
        });
      ...
      case 'pr_review.getJobRunDetail':
        sentPrReviewOps.add({'op': op, ...args});
        if ((args['job_id'] as num?)?.toInt() == 404) {
          _replyData(id, op, {'job': null});
        } else {
          _replyData(id, op, {
            'job': {
              'job_id': 101,
              'status': 'completed',
              'conclusion': 'success',
              'html_url': 'https://github.com/acme/cc/actions/runs/7/job/101',
              'steps': [
                {
                  'number': 1,
                  'name': 'Set up job',
                  ...
                  'completed_at': '2026-01-01T10:00:10.000Z',
                },
                {
                  'number': 2,
                  'name': 'Run tests',
      ...
      case 'pr_review.getWorkflowGraph':
        sentPrReviewOps.add({'op': op, ...args});
        if ((args['run_id'] as num?)?.toInt() == 404) {
          _replyData(id, op, {'graph': null});
        } else {
          _replyData(id, op, {
            'graph': {
              'name': 'CI',
  ...
  void _subscribe(dynamic id, Map<String, dynamic> params) {
    ...
    subscribeArgs.add(
      params['args'] is Map
          ? (params['args'] as Map).cast<String, dynamic>()
          : <String, dynamic>{},
    );
    const subId = 'sub-1';
    ...
    final data = switch (query) {
      ...
      'messaging.watchMessages' => {
        'messages': [
          _messageJson(
            'm1',
            params['args'] is Map
                ? (params['args'] as Map)['space_id'] as String? ?? 'c1'
                : 'c1',
          ),
        ],
      },
      ...
      'space_read.watchUserLastReadAt' => _spaceReadJson(
        params['args'] is Map
            ? (params['args'] as Map)['space_id'] as String? ?? 'c1'
            : 'c1',
      ),
      ...
      'review_space.watchByWorkspace' => {
        'associations': [_reviewSpaceJson('rc1', 'c1')],
      },
      'review_space.watchByPr' => {
        'association': _reviewSpaceJson('rc1', 'c1', prExternalId: 'pr-node-1'),
      },
      'review_space.watchBySpace' => {
        'association': _reviewSpaceJson('rc1', 'c1'),
      },
      ...
      'isolated_repo.watchForWorkspace' => {
        'repos': [_isolatedRepoJson('ir1', spaceId: 'c1')],
      },
      ...
      'pipeline_run.watchRun' => {'run': _pipelineRunJson('pr1', 'tmpl-1')},
        ...
        'events': [
          {
            ...
            'actor': {'login': 'octocat', 'avatar_url': 'https://a/o.png'},
  ...
  Map<String, dynamic> _isolatedRepoJson(
    ...
    'space_id': spaceId,
    'repo_id': repoId,
    'path': '/ws/ws1/conversations/$spaceId/repos/$repoId',
    'branch': 'agent/$spaceId',
    'backend': 'rift',
  ...
  Map<String, dynamic> _spaceJson(String id, String name) => {
    'id': id,
    'name': name,
    'workspace_id': 'ws1',
    'mode': 'chat',
  ...
  Map<String, dynamic> _messageJson(
    ...
    'message_type': 'text',
    'space_id': spaceId,
    'conversation_id': conversationId ?? spaceId,
    'compacted': false,
    'created_at': '2026-01-01T00:00:00.000Z',
  };
  ...
  Map<String, dynamic> _spaceReadJson(String spaceId) => {
    'space_id': spaceId,
    'last_read_at': '2026-01-03T00:00:00.000Z',
  };
  ...
  Map<String, dynamic> _factJson(String id, String topic, {String? author}) => {
    'id': id,
    'workspace_id': 'ws1',
    'domain': 'memory',
    'topic': topic,
  ...
  Map<String, dynamic> _reviewSpaceJson(
    String id,
    String spaceId, {
  ...
  Map<String, dynamic> _teamMemberJson(
    ...
    String agentId, {
    String role = 'member',
  }) => {'team_id': teamId, 'agent_id': agentId, 'role': role};
...
void main() {
  ...
  test('RemoteTicketRepository round-trips list/get/assign', () async {
    ...
    final one = await repo.get('ws1', 't1');
    expect(one.id, 't1');
  ...
  test('RemoteMessagingRepository lists spaces + sends', () async {
    final repo = RemoteMessagingRepository(client);
    final spaces = await repo.listSpaces('ws1');
    expect(spaces.single.name, 'general');

    final id = await repo.sendMessage(
      workspaceId: 'ws1',
      spaceId: 'c1',
      content: 'hi',
    );
    ...
    // A workspace id selects the database file server-side, so the send names
    // it rather than leaning on the client's ambient active workspace.
    expect(host.sentMessages.single['workspace_id'], 'ws1');
  });
  ...
  test('RpcNewsfeedRepository maps watch + getById to domain entities', () async {
    ...
    // A not-found is null, not a throw: the caller (a deep link into an
    // article that has since been swept) renders "gone", it does not crash.
    expect(await repo.getArticleById('nope'), isNull);
  ...
  test('RpcTicketRepository maps watch + getById to domain entities', () async {
    ...
    final one = await repo.getById('ws1', 't1');
    expect(one, isNotNull);
    expect(one!.title, 'First');
  });

  test(
    'RpcTicketRepository insert/update/delete reach the host losslessly',
    () async {
      final repo = RpcTicketRepository(client);
      final ticket = Ticket(
        id: 't9',
  ...
  test('RpcTicketRepository round-trips collaborators over RPC', () async {
    ...
    expect(added['workspace_id'], 'ws1');
    expect(added['id'], 'tc9');
    expect(added['ticket_id'], 't1');
    expect(added['principal_id'], 'a1');

    final fetched = await repo.getCollaborators('ws1', 't1');
    expect(fetched.single.id, 'tc1');
    expect(fetched.single.principalId, 'a1');

    final live = await repo.watchCollaborators('ws1', 't1').first;
    expect(live.single.ticketId, 't1');

    await repo.removeCollaborator('ws1', 't1', 'a1');
    expect(
      host.sentTicketCollaborators.last['op'],
      'tickets.removeCollaborator',
    );
    expect(host.sentTicketCollaborators.last['workspace_id'], 'ws1');
  });
  ...
  test(
    'RpcAgentRepository maps watch/getById/findByName to entities',
    () async {
      ...
      final one = await repo.getById('ws1', 'a1');
      expect(one, isNotNull);
      expect(one!.name, 'Ada');
  ...
  test('RpcAgentRepository upsert + delete reach the host', () async {
    ...
    expect((host.sentAgents.last['agent'] as Map)['id'], 'a9');

    await repo.delete('ws1', 'a9');
    expect(host.sentAgents.last['agent_id'], 'a9');
    expect(host.sentAgents.last['workspace_id'], 'ws1');
  });

  test('RpcRepoRepository maps watch + getById; upsert returns id', () async {
    final repo = RpcRepoRepository(client);

    // Repos are workspace-scoped: the same checkout registered in two
    // workspaces is two rows with two ids, so every op names its workspace.
    final live = await repo.watchAll('ws1').first;
    expect(live.single.id, 'r1');
    expect(live.single.remoteOwner, 'acme');

    final one = await repo.getById('ws1', 'r1');
    expect(one, isNotNull);
    expect(one!.name, 'cc');
    ...
    expect(host.sentRepos.last['workspace_id'], 'ws1');
  });

  test('RpcSpaceReadRepository marks read + maps watch to DateTime', () async {
    final repo = RpcSpaceReadRepository(client);

    await repo.markSpaceRead('w1', 'c1', 'user-1');
    expect(host.sentReadCursors.single['space_id'], 'c1');
    ...
    // The subscription must carry the caller-validated workspace explicitly
    // (not rely on the client's ambient active workspace) so a workspace switch
    // can't pair this space with a different workspace server-side.
    final spaceReadSub = host.subscribeArgs.singleWhere(
      (a) => a['space_id'] == 'c1',
    );
    expect(spaceReadSub['workspace_id'], 'w1');
  });

  test(
    'RpcMemoryDomainRepository maps watch/get/findByName; upsert reaches host',
    () async {
      ...
      expect(
        host.sentMemoryDomains.last['domain'],
        isA<Map<dynamic, dynamic>>(),
      );
      expect((host.sentMemoryDomains.last['domain'] as Map)['id'], 'd9');
    },
  );

  test(
    'RpcMemoryAccessGrantRepository maps watch/getByWorkspace + writes reach the host',
    () async {
      ...
      expect(
        ((host.sentGrants.last['grants'] as List).first
            as Map)['memory_domain'],
        'design',
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(
        host.sentWorkingMemories.last['memory'],
        isA<Map<dynamic, dynamic>>(),
      );
      expect((host.sentWorkingMemories.last['memory'] as Map)['id'], 'wm9');
    },
  );

  test(
    'RpcReviewSpaceRepository round-trips create/updateStatus/watch',
    () async {
      final repo = RpcReviewSpaceRepository(client);

      final byWorkspace = await repo.watchByWorkspace('ws1').first;
      expect(byWorkspace.single.id, 'rc1');
      expect(byWorkspace.single.spaceId, 'c1');
      expect(byWorkspace.single.workspaceId, 'ws1');
      expect(byWorkspace.single.status, ReviewSpaceStatus.requested);

      final byPr = await repo.watchByPr('ws1', 'pr-node-1').first;
      expect(byPr, isNotNull);
      expect(byPr!.prExternalId, 'pr-node-1');

      final bySpace = await repo.watchBySpace('ws1', 'c1').first;
      expect(bySpace, isNotNull);
      expect(bySpace!.spaceId, 'c1');
      ...
      expect(host.sentReviewSpaces.last['pr_external_id'], 'pr-node-9');

      await repo.updateStatus('ws1', 'rc-new', ReviewSpaceStatus.completed);
      expect(host.sentReviewSpaces.last['status'], 'completed');
      expect(host.sentReviewSpaces.last['workspace_id'], 'ws1');
    },
  );

  test(
    'RpcMemoryPolicyRepository round-trips watch/get/active + upsert/delete',
    () async {
  ...
  test('RpcAgentRunLogRepository round-trips reads/watches/upsert', () async {
    ...
    final one = await repo.getById('ws1', 'rl1');
    expect(one, isNotNull);
    expect(one!.agentId, 'a1');

    final activeRun = await repo.activeRunForAgent('ws1', 'a1');
    ...
    expect(byStep.single.pipelineStepId, 'step-1');
  ...
  test(
    ...
    () async {
      ...
      expect(live.single.steps.last.triggers.single.sourceStepIds, ['trigger']);
  ...
  test(
    ...
    () async {
      ...
      expect(live.single.workspaceId, 'ws1');
      expect(live.single.spaceId, 'c1');
      expect(live.single.backend, RepoIsolationBackend.rift);

      final unit = await repo.forUnitRepo('ws1', 'c1', 'r1');
      expect(unit, isNotNull);
      expect(unit!.spaceId, 'c1');
      expect(unit.repoId, 'r1');

      final bySpace = await repo.forSpace('ws1', 'c1');
      expect(bySpace.single.spaceId, 'c1');
      ...
      final acrossCh = await repo.forSpaceAcrossWorkspaces('c1');
      ...
      expect(
        (host.sentIsolatedRepos.last['repo'] as Map)['backend'],
        'gitWorktree',
      );

      await repo.deleteById('ws1', 'ir9');
      expect(host.sentIsolatedRepos.last['id'], 'ir9');
      expect(host.sentIsolatedRepos.last['workspace_id'], 'ws1');
    },
  );

  test(
    'RpcMemoryFactRepository round-trips reads/watch/upsert/delete',
    () async {
  ...
  test(
    'RpcProjectRepository round-trips reads/watch + insert/update/delete',
    () async {
  ...
  test('RpcPipelineRunRepository round-trips runs/step-runs/watches', () async {
    ...
    final step = await repo.getStepRunById('ws1', 'sr1');
    ...
    expect(host.sentPipelineStepRuns.last['workspace_id'], 'ws1');
    ...
    await repo.deleteStepRun('ws1', 'sr9');
    expect(host.sentPipelineStepRuns.last['workspace_id'], 'ws1');
    expect(host.sentPipelineStepRuns.last['step_run_id'], 'sr9');
  });

  test(
    'RpcPipelineTriggerRepository round-trips reads/watch/mutations',
    () async {
      ...
      expect(
        (host.sentTriggers.last['trigger'] as Map)['event_type'],
        'PrMerged',
      );
      expect(
      ...
      await repo.markFired('ws1', 'pt9', DateTime.utc(2026, 2));
      expect(host.sentTriggers.last['workspace_id'], 'ws1');
  ...
  test('RpcTeamRepository round-trips teams/members + watches', () async {
    ...
    final one = await repo.getTeam('ws1', 'tm1');
    expect(one, isNotNull);
    expect(one!.name, 'Platform');

    final liveMembers = await repo.watchMembersOf('ws1', 'tm1').first;
    ...
    final members = await repo.membersOf('ws1', 'tm1');
    ...
    await repo.deleteTeam('ws1', 'tm9');
    expect(host.sentTeams.last['workspace_id'], 'ws1');
  ...
  test(
    'RpcOrchestrationRepository round-trips reads/watches/insert/update',
    () async {
      ...
      expect(
        (host.sentOrchestrations.last['orchestration'] as Map)['id'],
        'orc9',
      );
  ...
  test('RpcMessagingRepository maps spaces/messages/participants', () async {
    final repo = RpcMessagingRepository(client);

    final spaces = await repo.watchSpaces().first;
    expect(spaces.single.id, 'c1');
    expect(spaces.single.name, 'general');
    expect(spaces.single.workspaceId, 'ws1');
    expect(spaces.single.mode, Mode.chat);

    // watchSpacesByWorkspace resolves to the bound-workspace space stream.
    final scoped = await repo.watchSpacesByWorkspace('ws1').first;
    expect(scoped.single.id, 'c1');

    final messages = await repo.watchMessages('ws1', 'c1', 'c1').first;
    expect(messages.single.id, 'm1');
    expect(messages.single.spaceId, 'c1');
    expect(messages.single.senderType, SenderType.user);

    final window = await repo
        .watchMessagesWindow('ws1', 'c1', 'c1', limit: 50)
        .first;
    ...
    expect(participants.single.principalId, 'user-1');
    expect(participants.single.isUser, isTrue);

    // A space id is a uuid, never an access boundary: every space-keyed
    // subscription names the workspace whose database file holds the rows.
    final spaceSubs = host.subscribeArgs
        .where((a) => a.containsKey('space_id'))
        .toList();
    expect(spaceSubs, hasLength(3));
    expect(spaceSubs.map((a) => a['workspace_id']), everyElement('ws1'));

    final byId = await repo.getMessageById('ws1', 'm9');
    expect(byId, isNotNull);
    expect(byId!.id, 'm9');

    expect(await repo.spaceExists('ws1', 'c1'), isTrue);

    final got = await repo.getMessages('ws1', 'c1');
    expect(got.single.spaceId, 'c1');

    final gotParticipants = await repo.getParticipants('ws1', 'c1');
    expect(gotParticipants.single.id, 'p1');
  });

  test('workspace-keyed watches carry their own workspace_id', () async {
    // The ambient id is whatever the last workspace SWITCH set; a caller keyed
    // to a workspace is asking about THAT workspace. Regression: these watches
    // dropped their argument and let the client's ambient injection answer, so
    // a `family` provider keyed by workspace A could be handed workspace B's
    // rows the moment it re-subscribed across a switch — and then feed A's
    // space ids to B-scoped reads, which the server rejects on every retry.
    client.activeWorkspaceId = 'ws2';

    await RpcMessagingRepository(client).watchSpacesByWorkspace('ws1').first;
    await RpcMessagingRepository(client).watchSpaceActivity('ws1').first;
    await RpcTicketRepository(client).watchForWorkspace('ws1').first;

    expect(host.subscribeArgs, hasLength(3));
    expect(
      host.subscribeArgs.map((a) => a['workspace_id']),
      everyElement('ws1'),
    );
  });
  ...
  test('RpcMessagingRepository mutations reach the host', () async {
    ...
    expect(host.sentMessages.last['workspace_id'], 'ws1');
    ...
    await repo.updateMessage(
      'ws1',
      'm1',
      metadata: const {'status': 'dismissed'},
    );
    expect(host.sentMessagingMutations.last['op'], 'messaging.updateMessage');
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');
    expect(host.sentMessagingMutations.last['message_id'], 'm1');

    await repo.setSpaceMode('ws1', 'c1', Mode.review);
    expect(host.sentMessagingMutations.last['op'], 'messaging.setSpaceMode');
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');
    expect(host.sentMessagingMutations.last['mode'], 'review');

    await repo.addParticipant('ws1', 'c1', 'a7');
    expect(host.sentMessagingMutations.last['op'], 'messaging.addParticipant');
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');
    expect(host.sentMessagingMutations.last['agent_id'], 'a7');
  });

  test('RpcMessagingRepository revert/unrevert reach the host', () async {
    final repo = RpcMessagingRepository(client);

    final reverted = await repo.revertConversationTo('ws1', 'c1', 'm1');
    expect(reverted, ['m2', 'm3']);
    expect(
      host.sentMessagingMutations.last['op'],
      'messaging.revertConversationTo',
    );
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');
    expect(host.sentMessagingMutations.last['space_id'], 'c1');
    expect(host.sentMessagingMutations.last['message_id'], 'm1');
    expect(host.sentMessagingMutations.last['inclusive'], isFalse);

    final restored = await repo.unrevertConversation('ws1', 'c1');
    expect(restored, ['m2', 'm3']);
    expect(
      host.sentMessagingMutations.last['op'],
  ...
  test(
    'RemoteMessagingRepository revert surfaces filesystem_restored',
    () async {
      final remote = RemoteMessagingRepository(client);
      final outcome = await remote.revertConversationTo('ws1', 'c1', 'm1');
      expect(outcome.affected, ['m2', 'm3']);
      expect(outcome.filesystemRestored, isTrue);
    },
  );

  test(
    'RpcMessagingRepository host-owned ops throw UnsupportedError',
    () async {
      final repo = RpcMessagingRepository(client);
      expect(
        () => repo.createSpace('ws1', 'g', const ['a1']),
        throwsUnsupportedError,
      );
      expect(() => repo.deleteSpace('ws1', 'c1'), throwsUnsupportedError);
      expect(
        () => repo.clearSpaceMessages('ws1', 'c1'),
        throwsUnsupportedError,
      );
      expect(
        () => repo.removeParticipant('ws1', 'c1', 'a1'),
        throwsUnsupportedError,
      );
      expect(
        () => repo.markCompacted('ws1', const ['m1']),
        throwsUnsupportedError,
      );
      expect(
        () => repo.getMessagesWithEmbedding('ws1', 'c1'),
        throwsUnsupportedError,
      );
      expect(
        () => repo.getMessagesWithoutEmbedding('ws1'),
        throwsUnsupportedError,
      );
      expect(
        () => repo.updateSpaceName('ws1', 'c1', 'x'),
        throwsUnsupportedError,
      );
    },
  );

  test('RpcMessagingPort space lifecycle reaches the host over messaging.* '
      'ops (works on a headless server)', () async {
    final port = RpcMessagingPort(client);

    final group = await port.createSpace('ws1', 'Team', const ['a1', 'a2']);
    expect(group.id, 'grp-1');
    expect(group.name, 'Team');
    expect(host.sentMessagingMutations.last['op'], 'messaging.createSpace');
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');

    await port.clearSpaceMessages('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.clearSpaceMessages',
      'workspace_id': 'ws1',
      'space_id': 'c1',
    });

    await port.removeParticipant('ws1', 'c1', 'a1');
    expect(host.sentMessagingMutations.last, {
    ...
    await port.deleteSpace('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.deleteSpace',
      'workspace_id': 'ws1',
      'space_id': 'c1',
    });

    await port.archiveSpace('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.archiveSpace',
      'workspace_id': 'ws1',
      'space_id': 'c1',
    });

    await port.unarchiveSpace('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.unarchiveSpace',
      'workspace_id': 'ws1',
      'space_id': 'c1',
    });

    await port.updateSpaceName('ws1', 'c1', 'Renamed');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.updateSpaceName',
      'workspace_id': 'ws1',
      'space_id': 'c1',
      'name': 'Renamed',
    });

    await port.setSpaceRepos('ws1', 'c1', ['r-1']);
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.setSpaceRepos',
      'workspace_id': 'ws1',
      'space_id': 'c1',
      'repo_ids': ['r-1'],
    });
  ...
  test('RpcWorkspaceRepository maps watchAll + repo links', () async {
    ...
    expect(repos.single.remoteOwner, 'acme');
  ...
  test('RpcWorkspaceRepository upsert/delete/link reach the host', () async {
    ...
    // every repo op (no session binding to infer it from).
    ...
    // Drag-to-reorder in "manage workspaces": the host owns the stored
    // positions, so the client ships the whole displayed sequence.
    await repo.reorderWorkspaces(const ['ws2', 'ws1']);
    expect(host.sentWorkspaces.last['op'], 'workspace.reorder');
    expect(host.sentWorkspaces.last['workspace_ids'], ['ws2', 'ws1']);
  });
  ...
  test('RpcPrReviewRepository maps PR watches to entities', () async {
    ...
    expect(reviews.single.id, 42);
    expect(reviews.single.state, PrReviewSubmissionState.approved);
    expect(reviews.single.author?.login, 'octocat');
    expect(
      reviews.single.submittedAt,
      DateTime.parse('2026-07-01T12:00:00.000Z'),
    );

    final timelineEvents = await repo.watchTimelineEvents(7).first;
    expect(timelineEvents.single.kind, PrTimelineEventKind.reviewRequested);
    expect(timelineEvents.single.actor?.login, 'octocat');
    expect(timelineEvents.single.reviewerName, 'platform');
    expect(timelineEvents.single.reviewerIsTeam, isTrue);
    ...
    expect(reviewComments.single.reviewId, 42);
    ...
    expect(checks.single.jobId, 101);
    expect(checks.single.workflowRunId, 7);
  ...
  test(
    'RpcPrReviewRepository fetches job run detail + workflow graph',
    () async {
      final repo = RpcPrReviewRepository(
        client,
        workspaceId: 'ws1',
      ...
      final detail = await repo.getJobRunDetail(101);
      expect(host.sentPrReviewOps.last['op'], 'pr_review.getJobRunDetail');
      expect(host.sentPrReviewOps.last['job_id'], 101);
      expect(detail, isNotNull);
      expect(detail!.jobId, 101);
      expect(detail.status, CheckRunStatus.completed);
      expect(detail.conclusion, CheckRunConclusion.success);
      expect(detail.steps, hasLength(2));
      expect(detail.steps.first.name, 'Set up job');
      expect(detail.steps.first.status, CheckRunStatus.completed);
      expect(detail.steps.first.startedAt, isNotNull);
      expect(detail.logs, 'full log text');
      expect(detail.logsTruncated, isTrue);
      expect(detail.isComplete, isTrue);

      expect(await repo.getJobRunDetail(404), isNull);

      final graph = await repo.getWorkflowGraph(7);
      expect(host.sentPrReviewOps.last['op'], 'pr_review.getWorkflowGraph');
      expect(host.sentPrReviewOps.last['run_id'], 7);
      expect(graph, isNotNull);
      expect(graph!.name, 'CI');
      expect(graph.jobs, hasLength(2));
      expect(graph.jobs.first.id, 'build');
      expect(graph.jobs.last.needs, ['build']);

      expect(await repo.getWorkflowGraph(404), isNull);
    },
  );
  ...
  test('RpcForgeProviderFactory builds an RpcPrReviewRepository', () async {
    final factory = RpcForgeProviderFactory(client);
    expect(factory.forge, ForgeHost.github);
