...
class _FakeHost {
  ...
  void _subscribe(dynamic id, Map<String, dynamic> params) {
    ...
    final data = switch (query) {
      ...
      'review_channel.watchByPr' => {
        'association': _reviewChannelJson('rc1', 'c1', prExternalId: 'pr-node-1'),
      },
  ...
  Map<String, dynamic> _messageJson(
    ...
    'message_type': 'text',
    'channel_id': channelId,
    'conversation_id': conversationId ?? channelId,
    'compacted': false,
    'created_at': '2026-01-01T00:00:00.000Z',
  };
...
void main() {
  ...
  test('RemoteMessagingRepository lists channels + sends', () async {
    ...
    final channels = await repo.listChannels('ws1');
  ...
  test('RpcChannelReadRepository marks read + maps watch to DateTime', () async {
    ...
    await repo.markChannelRead('w1', 'c1', 'user-1');
    ...
    // can't pair this channel with a different workspace server-side.
    final channelReadSub = host.subscribeArgs.singleWhere(
      (a) => a['channel_id'] == 'c1',
    );
    expect(channelReadSub['workspace_id'], 'w1');
  });
  ...
  test(
    'RpcReviewChannelRepository round-trips create/updateStatus/watch',
    () async {
      ...
      final byChannel = await repo.watchByChannel('ws1', 'c1').first;
      ...
      expect(host.sentReviewChannels.last['pr_external_id'], 'pr-node-9');

      await repo.updateStatus('ws1', 'rc-new', ReviewChannelStatus.completed);
      expect(host.sentReviewChannels.last['status'], 'completed');
      expect(host.sentReviewChannels.last['workspace_id'], 'ws1');
    },
  );
  ...
  test('RpcMessagingRepository maps channels/messages/participants', () async {
    ...
    expect(channels.single.mode, Mode.chat);
    ...
    // A channel id is a uuid, never an access boundary: every channel-keyed
    // subscription names the workspace whose database file holds the rows.
    final channelSubs = host.subscribeArgs
        .where((a) => a.containsKey('channel_id'))
        .toList();
    expect(channelSubs, hasLength(3));
    expect(channelSubs.map((a) => a['workspace_id']), everyElement('ws1'));

    final byId = await repo.getMessageById('ws1', 'm9');
    expect(byId, isNotNull);
    expect(byId!.id, 'm9');

    expect(await repo.channelExists('ws1', 'c1'), isTrue);
  ...
  test('workspace-keyed watches carry their own workspace_id', () async {
    ...
    // channel ids to B-scoped reads, which the server rejects on every retry.
    client.activeWorkspaceId = 'ws2';

    await RpcMessagingRepository(client).watchChannelsByWorkspace('ws1').first;
    await RpcMessagingRepository(client).watchChannelActivity('ws1').first;
  ...
  test('RpcMessagingRepository mutations reach the host', () async {
    ...
    await repo.setChannelMode('ws1', 'c1', Mode.review);
  ...
  test(
    ...
    () async {
      final repo = RpcMessagingRepository(client);
      expect(
        () => repo.createChannel('ws1', 'g', const ['a1']),
        throwsUnsupportedError,
      );
      expect(() => repo.deleteChannel('ws1', 'c1'), throwsUnsupportedError);
      expect(
        () => repo.clearChannelMessages('ws1', 'c1'),
        throwsUnsupportedError,
      );
      expect(
      ...
      expect(
        () => repo.updateChannelName('ws1', 'c1', 'x'),
        throwsUnsupportedError,
      );
    },
  );
  ...
  test('RpcMessagingPort channel lifecycle reaches the host over messaging.* '
    ...
    final group = await port.createChannel('ws1', 'Team', const ['a1', 'a2']);
    expect(group.id, 'grp-1');
    expect(group.name, 'Team');
    expect(host.sentMessagingMutations.last['op'], 'messaging.createChannel');
    expect(host.sentMessagingMutations.last['workspace_id'], 'ws1');

    await port.clearChannelMessages('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.clearChannelMessages',
      'workspace_id': 'ws1',
      'channel_id': 'c1',
    });
    ...
    await port.deleteChannel('ws1', 'c1');
    expect(host.sentMessagingMutations.last, {
      'op': 'messaging.deleteChannel',
      'workspace_id': 'ws1',
      'channel_id': 'c1',
    });
  });
