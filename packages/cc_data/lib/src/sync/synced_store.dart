...
class SyncedStore {
  ...
  /// the rest is pure resident memory. `conversation_messages` is the expensive
  ...
  void _applyChanges(List<dynamic> changes) {
    ...
    for (final raw in changes) {
      ...
      if (!_mirrors(tbl)) {
        ...
        // server ships every adopted table's changes, so
        // `conversation_messages` — the whole growing content of every message
        // in the workspace — accumulated in memory during agent streaming for
        // readers that do not exist. Deletes still cascade below for the
        // tables that ARE mirrored.
        continue;
      }
      final table = _rows.putIfAbsent(tbl, () => {});
      if (change['op'] == 'delete') {
        table.remove(pk);
        // Cascade: removing a space removes its child rows locally, the
        ...
        if (tbl == 'spaces') {
          for (final childTbl in const [
            ...
            child.removeWhere((_, row) => row['space_id'] == pk);
