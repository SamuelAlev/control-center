...
///
/// `SyncedStore` emits a fresh whole-table list on every delta frame, and each repository
/// pipeline mapped every row through `Dto.fromJson` + an entity constructor on every one —
/// so a single field change on a single ticket cost N full row decodes (a ticket alone
/// parses seven `DateTime`s) and N entity builds, on the UI isolate.
/// A busy agent writing 20 tickets a minute into a 2,000-ticket workspace burned that
/// continuously.
