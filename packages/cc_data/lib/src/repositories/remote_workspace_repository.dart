...
///
/// The workspace switcher's list uses the session-control method
/// `session/list_workspaces` (there is no `set_workspace` — the client points
/// `RemoteRpcClient.activeWorkspaceId` at the route's workspace instead). The full
...
class RemoteWorkspaceRepository {
  ...
  /// Re-sequences the workspace list to match [orderedIds] (drag-to-reorder).
  /// The host owns the stored positions; this sends the full displayed order.
  Future<void> reorderWorkspaces(List<String> orderedIds) =>
      _client.call('workspace.reorder', {'workspace_ids': orderedIds});
