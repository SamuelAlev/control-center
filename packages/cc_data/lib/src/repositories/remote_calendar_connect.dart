...
/// supplied client id + secret (or asks for the host's own app), surfaces the
/// code + URL to approve, polls until the host reports the result and can
/// disconnect an account. Mirrors the `calendar.connectInfo` /
...
class RemoteCalendarConnect {
  ...
  /// Whether the host has a Google app of its own, i.e. whether the dialog can
  /// offer to connect without any Google Cloud setup.
  ///
  /// The host's client id is deliberately not part of the answer — the client
  /// has no use for it and the pair stays server-side.
  Future<bool> builtinAvailable() async {
    final d = await _client.call('calendar.connectInfo', const {});
    return d['builtin_available'] == true;
  }

  /// Begins a device-code flow.
  ///
  /// With [useBuiltin] the host authorizes with its own OAuth client and
  /// [clientId]/[clientSecret] are ignored; otherwise they are the user's own
  /// Google Cloud client.
  Future<CalendarConnectBeginDto> begin({
    ...
    final d = await _client.call('calendar.beginConnect', {
      if (useBuiltin) 'use_builtin': true,
      'client_id': clientId,
    ...
    return CalendarConnectBeginDto(
      ...
      interval: Duration(
        seconds: (d['interval_seconds'] as num?)?.toInt() ?? 5,
      ),
      expiresIn: Duration(
