...
class RpcWorkspaceFilesystemPort implements WorkspaceFilesystemPort {
  ...
  @override
  Future<String> conversationsDir(String workspaceId) async => _path(
    await _client.call('fs.conversationsDir', {'workspace_id': workspaceId}),
  );
  ...
  Future<List<String>> listAgentSlugs(String workspaceId) async {
    final data = await _client.call('fs.listAgentSlugs', {
      'workspace_id': workspaceId,
    });
  ...
  Future<List<String>> listSkillSlugs(String workspaceId) async {
    final data = await _client.call('fs.listSkillSlugs', {
      'workspace_id': workspaceId,
    });
    return ((data['slugs'] as List?) ?? const [])
        .map((s) => s.toString())
