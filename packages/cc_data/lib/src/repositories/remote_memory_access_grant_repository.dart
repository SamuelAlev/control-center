...
class RemoteMemoryAccessGrantRepository {
  ...
  Future<List<MemoryAccessGrantDto>> getByWorkspace() async {
    final data = await _client.call(
      'memory_access_grant.getByWorkspace',
      const {},
    );
    return _grants(data);
  }
  ...
  Future<void> upsertAll(List<MemoryAccessGrantDto> grants) => _client.call(
  ...
  /// change.
  Stream<List<MemoryAccessGrantDto>> watch() => _client
      .subscribe('memory_access_grant.watchByWorkspace', const {})
      .map(_grants);
