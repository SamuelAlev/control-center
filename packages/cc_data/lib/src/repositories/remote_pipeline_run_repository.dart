...
class RemotePipelineRunRepository {
  ...
  /// How many runs of [templateId] currently hold one of its
  /// `maxParallelRuns` slots (everything non-terminal except `queued`).
  Future<int> activeRunCountForTemplate({
    required String workspaceId,
    required String templateId,
    Set<String> excludeTriggerEventTypes = const {},
  }) async {
    final data = await _client.call('pipeline_run.activeRunCountForTemplate', {
      'workspace_id': workspaceId,
      'template_id': templateId,
      if (excludeTriggerEventTypes.isNotEmpty)
        'exclude_trigger_event_types': excludeTriggerEventTypes.toList(),
    });
    return (data['count'] as num?)?.toInt() ?? 0;
  }

  /// The oldest `queued` run of [templateId], or null when none is waiting.
  Future<PipelineRunDto?> nextQueuedRunForTemplate({
    required String workspaceId,
    required String templateId,
  }) async {
    final data = await _client.call('pipeline_run.nextQueuedRunForTemplate', {
      'workspace_id': workspaceId,
      'template_id': templateId,
    });
    final run = data['run'];
  ...
  /// Re-opens a step run in [workspaceId] for another attempt.
  Future<void> restartStepRun(
    String workspaceId,
    String stepRunId, {
    required String startedAt,
  }) => _client.call('pipeline_run.restartStepRun', {
    'workspace_id': workspaceId,
    'step_run_id': stepRunId,
