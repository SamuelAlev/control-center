...
NotificationItemState? _itemStateFromWire(Map<String, dynamic> w) {
  final workspaceId = w['workspace_id'] as String?;
  final userId = w['user_id'] as String?;
  final itemId = w['item_id'] as String?;
  if (workspaceId == null || userId == null || itemId == null) {
    return null;
  }
  final read = w['read_at'];
  final dismissed = w['dismissed_at'];
  return NotificationItemState(
    workspaceId: workspaceId,
    userId: userId,
    itemId: itemId,
    readAt: read is String ? DateTime.tryParse(read) : null,
    dismissedAt: dismissed is String ? DateTime.tryParse(dismissed) : null,
  );
}
...
class RpcNotificationFeedRepository implements NotificationFeedRepository {
  ...
  Stream<List<NotificationItemState>> watchItemStates(
    ...
    String userId,
      ...
      .subscribe('notifications.watchItemStates', {
        'workspace_id': workspaceId,
      })
      .map(
        (data) => _maps(
          data['states'],
        ).map(_itemStateFromWire).whereType<NotificationItemState>().toList(),
      );
  ...
  Future<void> setItemRead(
    ...
    String itemId, {
    required bool read,
  }) => _client.call('notifications.setItemRead', {
    'workspace_id': workspaceId,
    'item_id': itemId,
    'read': read,
  });
  ...
  Future<void> dismissItem(
    ...
    String userId,
    String itemId,
  ) => _client.call('notifications.dismissItem', {
    'workspace_id': workspaceId,
    'item_id': itemId,
  });
}
