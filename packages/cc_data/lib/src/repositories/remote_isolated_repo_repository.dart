...
/// ([forChannelAcrossWorkspaces] / [forTicketAcrossWorkspaces]) are declared
/// exemptions and pass only their id — they run after the channel (and its
...
class RemoteIsolatedRepoRepository {
  ...
  Future<List<IsolatedRepoDto>> forChannel(
    String workspaceId,
    String channelId,
  ) async {
  ...
  Future<List<IsolatedRepoDto>> forChannelAcrossWorkspaces(
    ...
    final data = await _client.call(
      'isolated_repo.forChannelAcrossWorkspaces',
      {'channel_id': channelId},
    );
    return _repos(data);
  }
