...
class RpcMessagingRepository
  ...
  static ChannelMessage _messageFromDto(
    ...
    return ChannelMessage(
      ...
      senderId: d.senderId,
      senderType:
          _channelSenderTypeByName[d.senderType] ?? ChannelSenderType.user,
      content: d.content,
      messageType:
          _channelMessageTypeByName[d.messageType] ?? ChannelMessageType.text,
  ...
  Stream<List<Channel>> _watchAdoptedChannels(
    ...
    // Map instance, so its Channel is reused rather than re-decoded.
    final cache = RowEntityCache<Channel>();
    yield* store
        .watchRows('channels')
        .map(
          (rows) =>
              cache
                  .map(rows, (r) => channelFromDto(ChannelDto.fromJson(r)))
  ...
  Stream<List<ChannelParticipant>> _watchAdoptedParticipants(
    ...
    // The participants table is shared by every channel, so this pipeline
    // filters it down per emission; the memo means the surviving rows are not
    // re-decoded as well.
    final cache = RowEntityCache<ChannelParticipant>();
    yield* store
  ...
  Future<ChannelMessage?> getMessageById(
    ...
    // The ONLY `_messageFromDto` call site with no `fallbackChannelId`, so it
    // is the only one where the mapper's `?? ''` fallback can produce an empty
    // channelId — and `ChannelMessage` now REFUSES that in release, where it
    // used to be a stripped assert. A message that cannot be placed in a
    // channel is, for every caller of this method, not found; manufacturing
    // one and letting the domain throw would surface an out-of-window
    // permalink lookup as a crash instead of a miss.
    //
    // Unreachable on today's wire (`messageToWire` always emits `channel_id`),
...
final Map<String, ChannelMessageType> _channelMessageTypeByName =
    ChannelMessageType.values.asNameMap();
final Map<String, ChannelSenderType> _channelSenderTypeByName =
    ChannelSenderType.values.asNameMap();
