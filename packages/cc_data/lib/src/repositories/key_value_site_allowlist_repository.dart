...
/// [KeyValueStore]-backed [SiteAllowlistRepository].
///
/// Stores the allowlist as a JSON-encoded array under
/// `newsfeed.adblock.allowed_domains`. Pushes updates through a broadcast
/// [Stream] so widgets can react without polling. The thin client binds this
/// to its on-device preferences store; the preference itself is a local UI
/// setting for the in-app webview ad-blocker, not server state.
class KeyValueSiteAllowlistRepository implements SiteAllowlistRepository {
  /// Creates a new [KeyValueSiteAllowlistRepository].
  KeyValueSiteAllowlistRepository(this._prefs)
