...
/// host" state.
///
/// [start] and [killStep] name their workspace on the wire; [cancel] / [retry]
/// carry only a run id, which routes to its own workspace server-side.
class RpcPipelineEnginePort implements PipelineEnginePort {
  ...
  @override
  Future<void> cancel(String workspaceId, String pipelineRunId) =>
      _remote.cancel(workspaceId, pipelineRunId);

  @override
  Future<void> retry(String workspaceId, String pipelineRunId) =>
      _remote.retry(workspaceId, pipelineRunId);

  @override
  Future<void> killStep(String workspaceId, String stepRunId) =>
      _remote.killStep(workspaceId, stepRunId);
}
