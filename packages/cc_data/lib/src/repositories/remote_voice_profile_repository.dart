...
/// name one — a CLIENT-side default, not a server-held session binding (the
/// server is stateless and its dispatcher refuses a scoped op whose args carry
/// no `workspace_id`).
...
class RemoteVoiceProfileRepository {
  ...
  Future<List<VoiceProfileDto>> getByWorkspace() async {
    final data = await _client.readOr('voice_profile.getByWorkspace', const {}, const {});
  ...
  Future<VoiceProfileDto?> getByName(String displayName) async {
    final data = await _client.readOr('voice_profile.getByName', {
  ...
  /// change.
  Stream<List<VoiceProfileDto>> watch() => _client
      .subscribe('voice_profile.watchForWorkspace', const {})
      .map(_profiles);
