...
///
/// `PullRequests` is workspace-scoped, so every call names its workspace on the
/// wire and an id-keyed row from another workspace simply does not resolve. Each
/// emitted row carries the host-stamped authoritative `workspaceId` the client
/// rebuilds the entity from. Publishing ([publishToForge]) runs server-side
/// against the host-resident GitHub token.
