...
class RemoteReviewStudioRepository {
  ...
  /// Starts the canonical AI review (the "Ask AI" action). Returns
  /// `{status, channel_id, pr_external_id}`; progress streams through the
  /// review channel and the association status.
  Future<Map<String, dynamic>> startReview({
    required String owner,
    required String repo,
    required int prNumber,
  }) => _client.call('review_hub.start', _prArgs(owner, repo, prNumber));

  /// Asks one question about one review area. The answer arrives as a message
  /// in the PR channel, which the hub is already watching — there is nothing
  /// to poll here.
  ...
  }) => _client.call('review_hub.askArea', {
    ..._prArgs(owner, repo, prNumber),
    'cohort_key': cohortKey,
    'question': question,
  });
