...
class RpcOpenPrListRepository implements OpenPrListRepository {
  ...
  Future<WorkspaceOpenPrs> listOpenForWorkspace(String workspaceId) async {
    final data = await _client.readOr('pr.listOpenForWorkspace', {
  ...
  Stream<List<InaccessibleRepo>> watchRepoAccessForWorkspace(
    ...
    return _client
        .subscribe('pr.watchRepoAccessForWorkspace', {
          'workspace_id': workspaceId,
        })
        .map(_inaccessibleOf);
  }
  ...
  static WorkspaceOpenPrs _workspaceOpenPrsOf(Map<String, dynamic> data) {
    ...
    return WorkspaceOpenPrs(
      ...
      inaccessibleRepos: _inaccessibleOf(data),
    );
  }

  static List<InaccessibleRepo> _inaccessibleOf(Map<String, dynamic> data) => [
    for (final raw in (data['inaccessible_repos'] as List?) ?? const [])
      if (raw is Map)
        InaccessibleRepo(
          repoId: (raw['repo_id'] as String?) ?? '',
  ...
  Future<Set<String>> reviewedByKeysForWorkspace(String workspaceId) async {
    final data = await _client.readOr('pr.searchReviewedByForWorkspace', {
      'workspace_id': workspaceId,
    }, const {});
    return {
