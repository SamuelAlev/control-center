...
class RemotePipelineTemplateRepository {
  ...
  Future<List<PipelineTemplateDto>> forWorkspace() async {
    final data = await _client.call('pipeline_template.forWorkspace', const {});
    return _templates(data);
  }

  /// A single template by id (scoped by the `workspace_id` the request carries), or
  ...
  /// version bumping).
  Future<void> upsert(PipelineTemplateDto template) =>
      _client.call('pipeline_template.upsert', {'template': template.toJson()});
