...
class RemoteTicketRepository {
  ...
  /// Tickets assigned to [agentId] within [workspaceId].
  ///
  /// A scoped read rather than [list] + a client-side `where`: the server
  /// answers it with an indexed query, so the wire carries the agent's tickets
  /// instead of the workspace's.
  Future<List<TicketDto>> listForAgent(
    String workspaceId,
    String agentId,
  ) async {
    final data = await _client.call('tickets.listForAgent', {
  ...
  /// Direct children of [parentTicketId] within [workspaceId]. Scoped for the
  /// same reason as [listForAgent].
  Future<List<TicketDto>> listChildren(
    String workspaceId,
    String parentTicketId,
  ) async {
    final data = await _client.call('tickets.listChildren', {
      'workspace_id': workspaceId,
