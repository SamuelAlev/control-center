...
class RpcMemoryFactRepository implements MemoryFactRepository {
  ...
  static MemoryFact _fromDto(MemoryFactDto d) => MemoryFact(
    ...
    authoredByRole: d.authoredByRole == null
        ? null
        : _agentRoleByName[d.authoredByRole],
  ...
  Future<List<MemoryFact>> recallPolyphonic(
    ...
    // degrades to the host's FTS5 search and caps to [topK]. The repo boost is
    // applied client-side over that shortlist — approximate, but it keeps the
    // ordering the same shape as the host's rather than dropping the signal.
    final dtos = await _remote.search(query);
    return sortByRepoAffinity<MemoryFact>(
      dtos.map(_fromDto),
      boostRepoSlug,
