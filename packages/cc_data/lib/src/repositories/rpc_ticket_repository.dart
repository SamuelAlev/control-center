...
class RpcTicketRepository implements TicketRepository {
  ...
  Stream<List<Ticket>> _watchAdopted(
    ...
    yield* store.watchRows('tickets').map(_rowsToSortedTickets);
  }

  static List<Ticket> _rowsToSortedTickets(List<Map<String, dynamic>> rows) =>
      rows.map((row) => _fromDto(TicketDto.fromJson(row))).toList()
