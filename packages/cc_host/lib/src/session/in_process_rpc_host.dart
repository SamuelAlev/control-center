...
class InProcessRpcHost {
  ...
  /// [deviceId] tags the session for auditing (defaults to `in-process`) and
  /// [userId] is the acting principal (the embedding app resolves its local
  /// user — typically the server owner — before wiring the host).
  factory InProcessRpcHost({
    ...
    final session = RemoteRpcSession(
      ...
      // The gates apply in-process too: the embedding app may be serving a
      // signed-in user who is not a member of every registered workspace and
      // the subscription/tools chokepoints must not depend on the transport.
