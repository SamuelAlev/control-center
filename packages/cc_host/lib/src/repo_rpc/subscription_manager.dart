...
class SubscriptionManager {
  ...
  Map<String, dynamic> subscribe({
    ...
    // materialising a ghost `workspace.db` on disk.
    final gate = workspaceExists;
    final targetWorkspace = workspaceId;
    if (targetWorkspace != null && gate != null) {
      _subs[subId] = _Subscription.pending();
      unawaited(() async {
        if (!await gate(targetWorkspace)) {
          CcHostLog.warning(
...
class _Subscription {
  ...
  /// existence gate is still resolving); cancellation is a no-op until the
  /// real stream replaces this entry.
  _Subscription.pending() : _sub = null;
