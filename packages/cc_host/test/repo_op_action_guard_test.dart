...
void main() {
  ...
  group('repo-op ActionGuard effect net (PRD 24)', () {
    ...
    test(
      'a prompt decision runs without a confirmation — the operator asked',
      () async {
        ...
        expect(
          ...
          0,
          reason:
              'repo/call is the human clicking; approving your own click is '
              'not a control',
        );
      },
    );

    test('a prompt decision with no approver still runs', () async {
      final d = dispatcher(
        rules: [_wsRule(ActionClass.fileDelete, ActionDecision.prompt)],
      );
      final r = await call(d, 'thing.deleteFile');
      expect(
        isError(r),
        isFalse,
        reason: 'fail-closed guards an agent with nobody watching, not a click',
      );
    });

    test(
      'fileDelete defaults to prompt with no rule → the click runs it',
      () async {
    ...
    test('a deny rule still refuses an operator-initiated op', () async {
      final port = _RecordingPort(answer: true);
      final d = dispatcher(
        rules: [_wsRule(ActionClass.fileDelete, ActionDecision.deny)],
        port: port,
      ...
      final r = await call(d, 'thing.deleteFile');
      expect(isError(r), isTrue, reason: 'deny is not waived for a human');
      expect(port.prompts, 0, reason: 'a deny never offers an override');
    });
  });
}
