...
void main() {
  group('InProcessRpcHost', () {
    ...
    setUp(() {
      ...
      repoOps = RepoOpDispatcher(
        registry: RepoOpRegistry([
          ...
          // One workspace-scoped mutation: lets the audit sink observe what
          // ip the in-process session reports (there is no socket, so null).
          RepoOp(
            name: 'probe.mutate',
    ...
    test(
      'initialize advertises the repo-RPC catalog + subscriptions',
      () async {
    ...
    test(
      'the seeded binding flows through repo/call as the workspace',
      () async {
    ...
    test(
      ...
      () async {        // Clear the active workspace so the client injects no `workspace_id`
    ...
    test(
      'a mutation from the in-process session audits a null ip (no socket)',
      () async {
        await host.client.call('probe.mutate', const {});
        // The fire-and-forget append lands on the microtask queue.
        await Future<void>.delayed(Duration.zero);
        expect(auditIps, hasLength(1));
        expect(auditIps.single, isNull);
      },
    );
  });
}
