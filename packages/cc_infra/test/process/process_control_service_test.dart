...
void main() {
  ...
  group('ProcessControlService.isPidAlive — degradation', () {
    test('a pid it cannot inspect still answers from existence', () async {
      // `ps -o stat=` prints an EMPTY state field in restricted environments
      // (an entitlement-limited macOS shell does exactly this). Treating that
      // as "not alive" made every live process read as dead, so the state read
      // is advisory and `kill -0` is the primary answer. Our OWN pid is the
      // one process guaranteed to be alive here.
