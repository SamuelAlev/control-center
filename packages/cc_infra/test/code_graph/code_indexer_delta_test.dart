...
void main() {
  ...
  test(
    'every run prunes unresolvable edges for its own partition after resolving',
    () async {
      // The prune is what keeps external references (`react`, `vitest`, …)
      // from accumulating forever: nothing reads an unresolved edge, so the
      // only thing retaining one buys is a bigger unresolved read on the next
      // run. It must fire on EVERY completed run — a quiet save included —
      // and be scoped to the run's partition, never another checkout's rows.
      await indexerOver({'a.dart': 'h1'}).indexRepo(
        workspaceId: 'ws1',
        repoId: 'repo1',
        repoPath: '/linked',
      );
      expect(repo.pruned, hasLength(1));
      expect(repo.pruned.single.workspaceId, 'ws1');
      expect(repo.pruned.single.repoId, 'repo1');
      expect(repo.pruned.single.checkoutId, isNull);
      ...
      expect(repo.pruned.single.checkoutId, 'wt1');
    },
  );
}
...
class _FakeRepo implements CodeGraphRepository {
  ...
  /// Every (partition) the indexer asked to prune, in call order.
  final pruned = <({String workspaceId, String repoId, String? checkoutId})>[];
