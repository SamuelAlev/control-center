...
void main() {
  ...
  group('access', () {
    test('a linked writing member drives an agent', () async {
      ...
      expect(h.channelLinks.rows, hasLength(1));
    ...
    test('an unlinked chat account is refused with instructions', () async {
      ...
      expect(h.channelLinks.rows, isEmpty);
  ...
  group('inbound routing', () {
    ...
    test('a thread reply continues the bridged channel', () async {
      h.link('U1', 'user-1');
      await h.bridge.start();
      ...
      expect(h.messaging.dispatched, hasLength(2));
      // Same Control Center channel: one thread is one conversation.
      expect(
        h.messaging.dispatched.map((d) => d.channelId).toSet(),
        hasLength(1),
      );
      expect(h.channelLinks.rows, hasLength(1));
    });

    test('an unbridged plain channel message is ignored', () async {
      h.link('U1', 'user-1');
      await h.bridge.start();
    ...
    test('a DM needs no mention and is one continuous conversation', () async {
      ...
      expect(h.channelLinks.rows.single.externalThreadId, isNull);
      ...
      expect(h.channelLinks.rows.single.ccChannelId, isNotEmpty);
    });
    ...
    test(
      'a stale link whose channel was deleted is rebuilt in place',
      () async {
        h.link('U1', 'user-1');
        h.channelLinks.rows.add(
          ChatChannelLink(
            id: 'link-1',
            workspaceId: 'ws-1',
        ...
        expect(h.channelLinks.rows, hasLength(1));
        expect(h.channelLinks.rows.single.id, 'link-1');
        expect(h.channelLinks.rows.single.ccChannelId, isNot('gone'));
        expect(h.messaging.dispatched, hasLength(1));
      },
    );

    test(
      'the channel is named after the request and the conversation',
      () async {
        h.link('U1', 'user-1');
  ...
  group('outbound streaming', () {
    test('streams the agent reply and closes the stream', () async {
      ...
      final channelId = h.messaging.dispatched.single.channelId;
      ...
      h.registry.register('msg-1', channelId: channelId);
    ...
    test('a provider that refuses streaming falls back to one message', () async {
      ...
      h.registry.register('msg-2', channelId: channelId);
    ...
    test('the stream is opened for the member who asked', () async {
      ...
      // Slack will not stream into a channel without knowing who the reply is
    ...
    test('a turn started in Control Center streams to the linker', () async {
      ...
      h.link('U1', 'user-1');
      h.channelLinks.rows.add(_storedLink);
      await h.bridge.start();

      h.registry.register('msg-1', channelId: 'chan-9');
    ...
    test('a turn in an unbridged channel is not relayed', () async {
      await h.bridge.start();
      h.registry.register('msg-1', channelId: 'other-channel');
  ...
  group('task cards', () {
    test('the card reports the turn without quoting it', () async {
      ...
      expect(card.id, 'setup-$channelId');
      ...
      expect(
        card.link?.url,
        'https://cc.example/open/workspaces/ws-1/channels/$channelId',
      );
  ...
  group('workspace setup', () {
    ...
    test('the card reports the setup before the agent can answer', () async {
      h.link('U1', 'user-1');
      await h.bridge.start();
      // A first mention creates the channel, and creating it clones the repos —
      // minutes in which the agent has said nothing at all.
      h.messaging.whileDispatching = (channelId) async {
        h.provisioning(
          channelId,
          step: const ChannelProvisioningStep(
    ...
    test('each step replaces the last, and readiness is a step too', () async {
      ...
      h.messaging.whileDispatching = (channelId) async {
        ...
        await Future<void>.delayed(const Duration(milliseconds: 30));
        h.provisioning(channelId, status: ChannelProvisioningStatus.ready);
        await Future<void>.delayed(const Duration(milliseconds: 30));
        h.registry.register('msg-1', channelId: channelId);
      };
      ...
      expect(h.adapter.cards.map((c) => c.narration), [
        ...
        // warm channel has, and worth saying rather than blanking the card.
        'Starting the agent…',
      ]);
    ...
    test('a failed setup says so on the card', () async {
      ...
      await h.bridge.start();
      h.messaging.whileDispatching = (channelId) async {
        h.provisioning(channelId, status: ChannelProvisioningStatus.failed);
        await Future<void>.delayed(const Duration(milliseconds: 30));
      };
      await h.mention('U1', text: 'fix the flaky test');
    ...
    test('setup nobody is waiting for is not announced', () async {
      ...
      // A boot-time reconciler re-provisioning a stored link, or a channel
  ...
  group('outbound mirror', () {
    test('a message typed in Control Center reaches the chat thread', () async {
      ...
      h.messages.rows['m-2'] = ChannelMessage(
        id: 'm-2',
        channelId: channelId,
    ...
    test('a message that came from the chat app does not echo back', () async {
      ...
      h.messages.rows['m-1'] = ChannelMessage(
        id: 'm-1',
        channelId: channelId,
        conversationId: 'conv-1',
        senderId: 'user-1',
    ...
    test('another workspace’s message is not mirrored', () async {
      ...
      h.publish(channelId, 'm-3', workspaceId: 'ws-2');
      await pumpEventQueue();
...
/// A link already in the database when the bridge arms: a channel bridged in an
/// earlier run, which nobody has asked anything through yet this one.
final _storedLink = ChatChannelLink(
  id: 'cl-1',
  workspaceId: 'ws-1',
  provider: ChatProvider.slack,
  externalTeamId: 'T1',
...
class _Harness {
  ...
  final channelLinks = _FakeChannelLinks();
  ...
  /// Announces where a channel's workspace provisioning stands, the way
  /// `ChannelProvisioningService` does while it clones and sets up.
  void provisioning(
    String channelId, {
    ChannelProvisioningStatus status = ChannelProvisioningStatus.provisioning,
  ...
  Future<Channel> _createChannel({
    ...
    messaging.channels.add(channel.id);
    messages.channels[channel.id] = channel;
    return channel;
  }
...
class _Dispatched {
  _Dispatched(this.channelId, this.content, this.senderUserId, this.metadata);
...
class _FakeMessagingPort implements MessagingPort {
  ...
  /// channel that is provisioning the workspace, and the real call only returns
  /// once the run is registered. Tests that care about setup put that here.
  Future<void> Function(String channelId)? whileDispatching;
  ...
  Future<void> sendAndDispatch(
    ...
    dispatched.add(_Dispatched(channelId, content, senderUserId, metadata));
    await whileDispatching?.call(channelId);
  }
...
class _FakeMessages implements MessagingRepository {
  final Map<String, ChannelMessage> rows = {};
...
class _FakeChannelLinks implements ChatChannelLinkRepository {
  final List<ChatChannelLink> rows = [];

  @override
  Future<void> upsert(ChatChannelLink link) async {
    rows
      ..removeWhere(
        (r) =>
  ...
  Future<ChatChannelLink?> forCcChannel(
    ...
    String ccChannelId,
      ...
      .where(
        (r) => r.workspaceId == workspaceId && r.ccChannelId == ccChannelId,
      )
      .firstOrNull;
