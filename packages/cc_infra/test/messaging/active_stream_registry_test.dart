...
/// space relay index and the registration broadcast stream.
...
void main() {
  ...
  group('ActiveStreamRegistry space relay', () {
    test('activeIn returns the message ids in a space', () {
    ...
    test(
      'spaceUpdates relays updates tagged with their message id',
      () async {
        registry.register('m1', spaceId: 'c1');
        final events = <SpaceTurnUpdate>[];
        final sub = registry.spaceUpdates('c1').listen(events.add);
        final seg = TextSegment(text: 'a', startedAt: DateTime(2026, 1, 1));
        registry.apply('m1', SegmentOpened(0, seg));
