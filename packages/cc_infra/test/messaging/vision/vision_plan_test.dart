...
void main() {
  group('planArchive', () {
    test('empty source yields no slices', () {
      final plan = planArchive(sourceText: '', shape: _tinyShape, maxFrames: 8);
    ...
    test('foveates with HQ edges and an LQ middle when overflowing', () {
      ...
      expect(
        plan.slices.sublist(0, firstLq).every((s) => s.highQuality),
        isTrue,
      );
      expect(
        plan.slices.sublist(lastLq + 1).every((s) => s.highQuality),
        isTrue,
      );
    ...
    test('drops the OLDEST middle chars, keeping the newest', () {
      ...
      final middleText = plan.slices
          .where((s) => !s.highQuality)
          .map((s) => s.text)
          .join();
    ...
    test('all-HQ slices reconstruct the source exactly with a real shape', () {
      ...
      final plan = planArchive(sourceText: source, shape: shape, maxFrames: 8);
      expect(plan.truncatedRegions, 0);
