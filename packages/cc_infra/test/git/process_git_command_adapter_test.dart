...
void main() {
  ...
  group('ProcessGitCommandAdapter.run', () {
    ...
    test('a cancelled command is killed and reports failure', () async {
      // Clones and fetches are minutes of network work. Cancelling has to KILL
      // the process — a token the adapter merely reads once would leave the
      // caller's "stop" waiting exactly as long as no stop at all.
      final source = CancellationTokenSource();
      // `git wait-for-my-editor`: a command that blocks until we kill it.
      final pending = adapter.run(
        [
          '-c',
          'core.editor=sh -c "while :; do sleep 1; done"',
          'commit',
          '--amend',
        ],
        workdir: repo.path,
      ...
      final res = await pending.timeout(const Duration(seconds: 5));
      expect(res.isSuccess, isFalse);
      expect(res.stderr, contains('cancelled'));
    });

    test('an already-cancelled token never starts the command', () async {
