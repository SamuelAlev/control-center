...
import 'package:cc_infra/src/rigs/rig_browser_defaults.dart'
    show kBrowserRigHomePath, kBrowserRigHomeUrl;
...
void main() {
  ...
  group('create argv containment', () {
    test('loopback-only outbound is always present and bare --net never is',
        () {
    ...
    test('a blocked delete names its path only inside the smolvm VM store',
      ...
      expect(
        smolvmBlockedDeletePath(
          ...
          '/Users/x/Library/Caches/smolvm/vms/8d988d1d: Permission denied '
          '(os error 13)',
        ),
        '/Users/x/Library/Caches/smolvm/vms/8d988d1d',
      );
      expect(
        smolvmBlockedDeletePath(
          'delete machine data: /etc: Permission denied',
        ),
        isNull,
      );
    ...
    test('a browser machine installs nothing at boot', () {
      final args = buildSmolvmCreateArgs(
        plan(
          spec: RigSpec(surface: RigSurface.browser, conversationId: 'c1'),
          devtoolsHostPort: 9222,
    ...
    test('the exec image and browser image are digest-pinned', () {
    ...
    test('the boot home page is local and needs no egress', () {
      ...
      expect(
        script,
        isNot(contains('http')),
        reason: 'A boot that depends on reaching an external site over the '
    ...
    test('the dev TLS pin rides the workload only when material exists', () {
      ...
      expect(
        ...
        reason: 'A host with no TLS material must not carry the flag at all — '
    ...
    test('parses rows with labels and pids', () {
      final machines = parseSmolvmMachineList(
        jsonEncode([
          ...
          {'name': 'ccrig-r2', 'state': 'created', 'labels': <String, String>{}},
        ]),
      );
  ...
  group('sweep', () {
    test('deletes only machines carrying this server\'s labels', () async {
      ...
      await Directory('${dataDir.path}/rigs/smolvm/dead-rig')
  ...
  group('launch failure', () {
    test('a failed create leaves neither machine record nor runtime dir',
        () async {
      ...
      await expectLater(
        backend.launch(rigId: 'r9', spec: RigSpec.exec(conversationId: 'c1')),
        throwsA(isA<RigToolException>()),
      );
      expect(
        calls.where((c) => c.contains('delete') && c.contains('ccrig-r9')),
        isNotEmpty,
        reason: 'A half-created machine in smolvm\'s global store is a leak '
