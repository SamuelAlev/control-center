...
// The whole file drives a `_LocalShellTransport` — a REAL `sh -c` on this
// host — because the point is proving the shell scripts the guest runs. A rig
// guest is always a Linux VM; the Windows hosts this suite would run on have
// no POSIX `sh` contract worth testing against (the GitHub runner's PATH
// `bash` is the WSL stub with no distro installed), so the file stays
// POSIX-only.
...
void main() {
  ...
  group('get', () {
    // `get`'s probe runs `stat -c "%s %F" -- <path>` — GNU coreutils syntax.
    // A rig guest is always GNU/Linux, so the SCRIPT is right; the local
    // `sh` this test uses as its transport stand-in is only GNU on Linux.
    // BSD stat (macOS) exits non-zero on `-c`, so the probe returns null and
    // the assertions below would fail for a reason that is not the code's.
    final gnuStatSkip =
        Platform.isMacOS ? 'get() probes with GNU stat -c; macOS stat is BSD' : false;

    test(
      'reads a regular file back out',
      () async {
    ...
    test(
      'reads binary content unchanged',
      () async {
