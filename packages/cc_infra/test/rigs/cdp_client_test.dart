...
class _FakeCdpSocket implements CdpSocket {
  ...
  void fail(int id, String message, {int code = -32000}) =>
      push({'id': id, 'error': {'code': code, 'message': message}});
}
...
void main() {
  group('command correlation', () {
    ...
    test('sending after close fails fast', () async {
      ...
      final client = CdpClient.over(socket);
      await client.close();
      await expectLater(client.send('Page.enable'), throwsA(isA<CdpException>()));
    });
  });
  ...
  group('typing', () {
    test('text goes in one insertText, not a key event per character', () async {
  ...
  group('pressKey', () {
    ...
    test('a named key dispatches a keydown/keyup pair with its vk code', () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchKeyEvent'] = const {};
      final client = CdpClient.over(socket);
      ...
      for (final frame in socket.sent) {
        ...
        expect(
          ...
          isFalse,
          reason: 'An arrow inserts no character; sending `text` would type '
              'one.',
        );
      }
      await client.close();
    ...
    test('function keys are covered', () async {
      ...
      expect((socket.sent.first['params'] as Map)['windowsVirtualKeyCode'], 116);
    ...
    test('a named key with modifiers carries the bitmask and no text', () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchKeyEvent'] = const {};
      final client = CdpClient.over(socket);
    ...
    test('shift+arrow extends the selection (the modifier reaches the page)',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchKeyEvent'] = const {};
      final client = CdpClient.over(socket);
      expect(await client.pressKey('ArrowLeft', modifiers: ['shift']), isTrue);
      expect((socket.sent.first['params'] as Map)['modifiers'], 8);
      await client.close();
    });

    test('a letter chord dispatches a real key event, not insertText', () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchKeyEvent'] = const {};
      final client = CdpClient.over(socket);
  ...
  group('pointer primitives', () {
    test('hover moves report no buttons; a mid-drag move reports the held one',
        () async {
      final socket = _FakeCdpSocket()
      ...
      expect(
        params['buttons'],
        1,
        reason: 'Chromium extends a selection only under a move that still '
    ...
    test('mouseDown carries the button mask and the click count', () async {
      ...
      expect(
        params['clickCount'],
        2,
        reason: 'A double click exists ONLY as clickCount 2 on the press — '
  ...
  group('reload and navigation state', () {
    test('reload forwards the cache flag', () async {
      ...
      expect(
        (socket.sent.single['params'] as Map)['ignoreCache'],
        isTrue,
      );
      await client.close();
    });
    ...
    test('the first entry cannot go back, the last cannot go forward',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Page.getNavigationHistory'] = const {
          'currentIndex': 0,
    ...
    test('a malformed history answer throws rather than reporting "no page"',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Page.getNavigationHistory'] = const {};
      final client = CdpClient.over(socket);
  ...
  group('domSnapshot', () {
    ...
    test('a selector that matches nothing returns null, not the page', () async {
      final socket = _FakeCdpSocket()
        ..autoReply['DOM.getDocument'] = documentReply()
  ...
  group('screencast', () {
    test('a requested fps maps onto an every-Nth-frame throttle', () async {
      ...
      final socket = _FakeCdpSocket()..autoReply['Page.startScreencast'] = const {};
      final client = CdpClient.over(socket);
  ...
  group('reconnect', () {
    test('a drop re-attaches and re-applies domains, viewport and screencast',
        () async {
      ...
      final cast =
          second.sent.firstWhere(
              ...
              as Map;
      expect(cast['quality'], 60);
      await client.close();
    });

    test('a viewer holding the frame stream keeps working across a re-attach',
        () async {
      ...
      expect(ended, isFalse, reason: 'The viewer must see a gap, not an end.');
      expect(
  ...
  group('targets', () {
    ...
    test('the newest survivor is attached when the current target dies',
        () async {
