...
void main() {
  ...
  group('exec rig lookup', () {
    ...
    test('one member never inherits another member\'s exec rig', () {
      // One rig per conversation is the feature, but it cannot span members.
      // A rig's credential grant is bound to whoever opened it, so reusing
      // Alice's machine for Bob would let Bob mint on Alice's forge access —
      // and would drop him inside a VM where her agent may be running with its
      // token in the environment.
      seed(
        id: 'exec-alice',
        conversationId: 'c1',
        exec: true,
        openedByUserId: 'user-alice',
      );

      expect(
        service.execRigFor('ws1', 'c1', openedByUserId: 'user-alice')?.id,
        'exec-alice',
      );
      expect(
        service.execRigFor('ws1', 'c1', openedByUserId: 'user-bob'),
        isNull,
        reason: 'Bob must get his own machine, not Alice\'s',
      );
      expect(
        service.execRigFor('ws1', 'c1'),
        isNotNull,
        reason:
            'naming nobody keeps the any-opener behaviour a solo server sees',
      );
    });
  });

  group('slots', () {
    test('a slot is part of the reuse key, in both directions', () async {
      // A conversation can hold more than one machine of the same surface and
      // engine — two browsers to compare two builds. Reuse that ignored the
      // slot would hand BOTH opens whichever machine is first in the live map,
      // so the second tab would show the first machine and closing it would
      // shut down the one still on screen.
      seed(id: 'primary', conversationId: 'c1');
      seed(id: 'second', conversationId: 'c1', slotId: 's2');

      final primary = await service.open(
        workspaceId: 'ws1',
      ...
      expect(
        second.id,
        'second',
        reason: 'The slot picks WHICH of the conversation\'s machines.',
      );
    });

    test('a slot never falls back to the default machine', () async {
      // The dangerous direction: with only the default machine up, an open for
      // a slot must BOOT rather than quietly join it — an agent drives the
      // default one, and joining it would put two drivers on one machine.
      seed(id: 'primary', conversationId: 'c1');
      ...
      expect(
        opened.id,
        isNot('primary'),
        reason: 'It must be a machine of its own, not the default one.',
      );
      expect(opened.spec.slotId, 's2');
      expect(
        repository.rigs['primary']!.status.phase.isTerminal,
        isFalse,
        reason: 'And the default machine is untouched by the second open.',
      );
    });
  });
