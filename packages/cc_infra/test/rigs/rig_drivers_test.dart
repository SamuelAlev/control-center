...
({MobileRigDriver driver, FakeSpawner spawner}) _mobile({
  ...
  spawner = FakeSpawner((process) {
    ...
    if (args.contains('getprop')) {
      process.complete(exitCode: deviceReady ? 0 : 1, stdout: deviceReady ? '1\n' : '',
...
void main() {
  group('rigDriverFailure', () {
    ...
    test('each transport gets its own diagnosis', () {
      ...
      expect(
        rigDriverFailure('screenshot', const GuestAgentException('no route'))
            .text,
    ...
    test('a programming error is shaped as OUR bug, not a dead channel', () {
      // The model must be able to tell "the control channel is down" (stop
  ...
  group('BrowserRigDriver', () {
    ...
    test('extract with an unresolvable selector is an error, not the page',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['DOM.getDocument'] = {
    ...
    test('an unknown key name is refused rather than reported pressed',
        () async {
      final socket = _FakeCdpSocket();
      final driver = _driver(socket);
    ...
    test('a drag is press, stepped moves with the button held, release',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchMouseEvent'] = const {};
      final driver = _driver(socket);
      ...
      expect(
        draggedMoves,
        greaterThan(2),
        reason: 'A single jump reads as a teleport to most drag handlers; '
            'the selection never starts.',
      );
      await driver.dispose();
    });

    test('a release with no coordinate uses the last pointer position',
        () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Input.dispatchMouseEvent'] = const {};
      final driver = _driver(socket);
    ...
    test('two quick presses at one spot become a double click', () async {
      ...
      expect(
        pressCounts,
        [1, 2],
        reason: 'Chromium only derives a word selection from clickCount 2; '
            'two count-1 presses are two clicks.',
      );
      expect(
        releaseCounts,
        [1, 2],
        reason: 'The DOM click/dblclick event detail comes from the RELEASE, '
    ...
    test('main-frame navigations publish the URL; subframes do not', () async {
      ...
      expect(
        urls,
        ['https://a.test'],
        reason: 'An iframe loading is not the page navigating — the address '
    ...
    test('seedCurrentUrl publishes the page the driver attached to', () async {
      ...
      expect(
        urls,
        ['https://home.test'],
        reason: 'The home page loaded before this driver existed, so no '
    ...
    test('stop_loading stops the load and clears the flag', () async {
      final socket = _FakeCdpSocket()
        ..autoReply['Page.stopLoading'] = const {};
      final driver = _driver(socket);
      socket.push({
        'method': 'Page.frameNavigated',
        'params': {
  ...
  group('MobileRigDriver stills', () {
    test('a still is downscaled to the agent ceiling and encoded as JPEG',
        () async {
    ...
    test('the cached size is refreshed before the frame is taken', () async {
      ...
      expect(
        m.spawner.started.any((p) => p.args.contains('wm')),
        isTrue,
      );
    });
  });
  ...
  group('MobileRigDriver watch lane', () {
    test('the lane declares MJPEG — what leaves the host, not what adb makes',
        () {
    ...
    test('a host with no ffmpeg fails loudly and by name', () async {
      ...
      final m = _mobile(hasFfmpeg: false);
      await expectLater(
        m.driver.openWatchStream(RigWatchRequest(size: RigDisplaySize(720, 1280))),
        throwsA(
