...
void main() {
  group('MeetingRecordingService concurrency', () {
    ...
    test('two simultaneous recordings stay isolated', () async {
      ...
      final idA = await service.start(
        workspaceId: ws,
        title: 'A',
        mode: 'remote',
      );
      final idB = await service.start(
        workspaceId: ws,
        title: 'B',
        mode: 'remote',
      );
...
class _InMemoryMeetingRepo implements MeetingRepository {
  ...
  Future<void> appendSegment(MeetingSegment segment) async {
    (_segments[_k(segment.workspaceId, segment.meetingId)] ??= []).add(segment);
  }
  ...
  Future<List<MeetingSegment>> getSegments(
    String workspaceId,
    String meetingId,
  ) async => List.of(_segments[_k(workspaceId, meetingId)] ?? const []);
