...
/// Pins every directory layout (workspaces, spaces, agents, skills,
...
void main() {
  ...
  group('directory layout', () {
    ...
    test('spacesDir nests under the workspace', () async {
      final dir = await fs.spacesDir('ws-1');
      expect(dir, p.join(tempRoot.path, 'ws-1', 'spaces'));
    });

    test('spaceDir nests under spaces', () async {
      final dir = await fs.spaceDir('ws-1', 'c-1');
      expect(dir, p.join(tempRoot.path, 'ws-1', 'spaces', 'c-1'));
    });

    test('ensureSpaceDir creates the directory', () async {
      final path = await fs.ensureSpaceDir('ws-1', 'c-1');
      expect(Directory(path).existsSync(), isTrue);
    });

    test('ensureSpaceDir is idempotent', () async {
      await fs.ensureSpaceDir('ws-1', 'c-1');
      await fs.ensureSpaceDir('ws-1', 'c-1'); // no throw
      expect(
        Directory(
          p.join(tempRoot.path, 'ws-1', 'spaces', 'c-1'),
        ).existsSync(),
        isTrue,
      );
    });
  ...
  group('syncAgentSkillLinks quarantine filter', () {
    Set<String> linkNames() {
      final dir = Directory(
        p.join(tempRoot.path, 'ws-1', 'agents', 'qa', '.agents', 'skills'),
      );
    ...
    test('a refused slug never gains a link', () async {
      final gated = WorkspaceFilesystemService(
        paths,
        linkFilter: (ws, slug) async => slug != 'quarantined',
      );
      await gated.ensureWorkspaceDirs('ws-1');
      await gated.writeSkillFile('ws-1', 'git', 'a');
      await gated.writeSkillFile('ws-1', 'quarantined', 'b');

      await gated.syncAgentSkillLinks('ws-1', 'qa', ['git', 'quarantined']);
      expect(linkNames(), {'git'});
    });

    test('a refused slug loses an existing link on the next sync', () async {
      // No filter yet: the skill links normally.
      await fs.ensureWorkspaceDirs('ws-1');
      await fs.writeSkillFile('ws-1', 'git', 'a');
      await fs.writeSkillFile('ws-1', 'quarantined', 'b');
      await fs.syncAgentSkillLinks('ws-1', 'qa', ['git', 'quarantined']);
      expect(linkNames(), {'git', 'quarantined'});

      // The verdict flips to quarantine (filter attached post-construction,
      // exactly like the runtime wires it over SkillBundleService): the same
      // attachment list now reconciles the link away.
      fs.linkFilter = (ws, slug) async => slug != 'quarantined';
      await fs.syncAgentSkillLinks('ws-1', 'qa', ['git', 'quarantined']);
      expect(linkNames(), {'git'});
    });

    test('the filter is consulted per workspace', () async {
      final seen = <String>[];
      final gated = WorkspaceFilesystemService(
        paths,
        linkFilter: (ws, slug) async {
          seen.add('$ws:$slug');
          return true;
        },
      );
      await gated.ensureWorkspaceDirs('ws-1');
      await gated.writeSkillFile('ws-1', 'git', 'a');
      await gated.syncAgentSkillLinks('ws-1', 'qa', ['git']);
      expect(linkNames(), {'git'});
      expect(seen, ['ws-1:git']);
    });
  });
