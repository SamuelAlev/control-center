...
void main() {
  ...
  group('WorkspaceSeeder.reseedTemplates', () {
    ...
    test('upgrades a stale built-in row that predates a seed change', () async {
      ...
      // carries: the analyzer node with no repo scope, so its conversation
      // clones every workspace repo instead of the one being indexed.
      ...
      expect(templates.byId['ws|index_code']!.step('analyze')!.config.repoIds, [
        '{{repoId}}',
