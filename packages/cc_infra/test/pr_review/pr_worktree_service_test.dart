...
class _FakeRegistry implements IsolatedRepoRepository {
  ...
  Future<IsolatedRepo?> forUnitRepo(
    ...
    String channelId,
    String repoId,
  ) async => byUnitKey['$workspaceId|$channelId|$repoId'];

  @override
  Future<List<IsolatedRepo>> forChannelAcrossWorkspaces(
    String channelId,
  ) async => byChannel[channelId] ?? const [];
  ...
  Future<void> upsert(IsolatedRepo row) async {
    byUnitKey['${row.workspaceId}|${row.channelId}|${row.repoId}'] = row;
    byChannel.putIfAbsent(row.channelId, () => []).add(row);
  }
...
void main() {
  group('PrWorktreeService.ensureWorktree', () {
    test('provisions a fresh worktree and registers it', () async {
      ...
      expect(path, '/ws/pr_worktrees/acme__widget/pr-42');
  ...
  group('PrWorktreeService.release', () {
    ...
    test(
      ...
      () async {
        ...
        final registry = _FakeRegistry()
          ..byChannel['pr:acme/widget#1'] = [
            IsolatedRepo(
              id: 'x',
              workspaceId: 'ws',
              channelId: 'pr:acme/widget#1',
              repoId: 'r',
              path: '/p/x',
              branch: 'b',
              backend: RepoIsolationBackend.rift,
