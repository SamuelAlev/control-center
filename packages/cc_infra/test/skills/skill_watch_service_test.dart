...
void main() {
  ...
  Future<SkillWatchService> arm({required _FakeWatcherFactory factory}) async {
  ...
  test('skills-lock.json changes never publish (no feedback loop)', () async {
    ...
    factory
        .watcherFor(skillsDir)
        .emit(
          DirectoryChangeBatch(paths: [pJoin(skillsDir, 'skills-lock.json')]),
        );
    await Future<void>.delayed(const Duration(milliseconds: 120));
  ...
  test('a vanished root drops its watch without crashing', () async {
    ...
    factory
        .watcherFor(skillsDir)
        .emit(const DirectoryChangeBatch(rootGone: true));
...
/// p.join, not '/' interpolation: the watcher keys are the service's own
/// platform-separator paths (`WorkspaceFilesystemService.skillsDir` builds
/// them with p.join), and a mixed-separator key never matches on Windows.
