...
void main() {
  ...
  group('SkillBundleService.reVerify', () {
    ...
    test('re-scans a drifted entry even with a current rules version', () async {
      final scanner = _FakeScanner();
      final dir = Directory(
  ...
  group('SkillBundleService.scanInstalled', () {
    ...
    test('scans every on-disk file (lock excluded) and returns the verdict', () async {
      final scanner = _FakeScanner();
      final dir = Directory(
        p.join(_workspaceRoot(temp), 'ws', 'skills', 'multi'),
      );
      ...
      expect(scanner.scanned.single.files.keys, unorderedEquals(<String>['SKILL.md', 'NOTES.md']));
  ...
  group('SkillBundleService.saveLocal', () {
    ...
    test('quarantine without override throws and writes nothing', () async {
      ...
      expect(
        Directory(p.join(_workspaceRoot(temp), 'ws', 'skills', 'evil'))
            .existsSync(),
        isFalse,
      );
      expect((await svc.readLock('ws')).skills, isEmpty);
    ...
    test('override writes and records the quarantine verdict', () async {
      ...
      expect(entry.scanVerdict, SkillScanVerdict.quarantine);
      expect(
        Directory(p.join(_workspaceRoot(temp), 'ws', 'skills', 'risky'))
            .existsSync(),
        isTrue,
      );
    });
  ...
  group('SkillBundleService.listInstalledStatus', () {
    ...
    test('drifted skill reports drift and the verdict for the NEW bytes', () async {
      final cache = _FakeScanCache();
      final dir = Directory(p.join(_workspaceRoot(temp), 'ws', 'skills', 'd'));
      await dir.create(recursive: true);
      await File(p.join(dir.path, 'SKILL.md')).writeAsString('# edited');
      final svc = _service(
        temp,
  ...
  group('SkillBundleService publishes SkillUpdated', () {
    ...
    test('installFromRegistry publishes origin registry', () async {
      final events = <SkillUpdated>[];
      final svc = _service(
        temp,
        scanner: _FakeScanner(),
      ...
      await svc.installFromRegistry(workspaceId: 'ws', slug: 'sh-skill');
      await Future<void>.delayed(Duration.zero);
      expect(events.single.origin, SkillUpdateOrigin.registry);
      expect(events.single.slug, 'sh-skill');
    });
