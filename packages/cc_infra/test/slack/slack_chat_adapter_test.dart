...
void main() {
  ...
  group('inbound normalization', () {
    ...
    test('the duplicate `message` copy of a mention is dropped', () async {
      // Slack delivers a channel mention twice, in two envelopes with the same
    ...
    test('a DM is recognized by channel type and by id shape', () async {
      await h.envelope(
        _event('message', user: 'U1', text: 'hi', channelType: 'im'),
        id: 'Ev1',
      );
      await h.envelope(
        _event('message', user: 'U1', text: 'hi again', channel: 'D100'),
        id: 'Ev2',
      );
  ...
  group('outbound translation', () {
    ...
    test('a stream names the member it is answering', () async {
      // Slack refuses to open a stream in a channel without both ids
