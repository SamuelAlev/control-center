...
void main() {
  group('sandbox enforcement (real process)', () {
    late Directory root;
    late Directory work;
    ...
    Future<ProcessResult> runSandboxed(String script) async {
      final spec = SandboxSpec(
        ...
        bindMounts: [SandboxBindMount(hostPath: work.path, guestPath: work.path)],
    ...
    test('writes to a protected checkout are denied', () async {
      ...
      expect(
        ...
        'original',
        reason: 'SandboxSpec.protectedPaths must survive as a deny-write rule',
      );
    });

    test('a protected path reached through a symlink is still denied', () async {
      ...
      final result = await runSandboxed('echo pwned > $resolved/tracked.txt');
