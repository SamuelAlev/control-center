...
void main() {
  group('EnvCredentialsRepository.loadCredentials', () {
    ...
    test(
      'defaults ticketing provider to local when CC_TICKETING_PROVIDER unset',
      () async {
    ...
    test('reads ticketing config', () async {
      final repo = EnvCredentialsRepository(
        environment: {
          'CC_TICKETING_API_KEY': 'tk',
          'CC_TICKETING_PROVIDER': 'linear',
        },
      );
      final creds = await repo.loadCredentials();
      expect(creds.ticketingApiKey, 'tk');
