...
/// (Pi) transport forwards `--mcp-config`. Only `exec`/`events`/`backend` are
/// exercised — the rest route through `noSuchMethod` and never fire because the
/// test supplies `onResolveHandle` directly.
...
void main() {
  test(
    'structured (Pi) dispatch writes <cwd>/.mcp.json and passes --mcp-config',
    () async {
      ...
      final deps = SandboxDispatchDeps(
        ...
        mcpConfigPathResolver:
            (cwd, {workspaceId, agentId, conversationId}) async {
      ...
      // And the spawned argv points the adapter at it via --mcp-config.
      expect(sandbox.execArgs, hasLength(1));
      final argv = sandbox.execArgs.single;
      final flagIndex = argv.indexOf('--mcp-config');
      expect(
        flagIndex,
        greaterThanOrEqualTo(0),
        reason: 'structured argv must include --mcp-config',
      );
      expect(argv[flagIndex + 1], '${tempDir.path}/.mcp.json');
    },
  );
}
