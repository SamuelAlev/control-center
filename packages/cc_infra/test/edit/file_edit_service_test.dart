...
void main() {
  group('FileEditService', () {
    ...
    test(
      'atomic multi-section: one unrecoverable section writes neither file',
      () async {
  ...
  group('TreeSitterBlockResolver', () {
    test('throws TreeSitterUnavailable when the loader is unavailable', () {
      // A loader pointed at bogus paths never reports availability — the
      // natives ship in the host bundle, so this is a broken install and must
      // fail loudly instead of degrading to blind span guesses.
      final loader = TreeSitterLoader(
        runtimePath: '/nonexistent/libtree-sitter.dylib',
        grammarPaths: const {'dart': '/nonexistent/libtree-sitter-dart.dylib'},
      );
