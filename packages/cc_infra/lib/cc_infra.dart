/// Server-side VM-only infrastructure adapters for Control Center.
///
/// Pure `dart:io` implementations of `cc_domain` ports (git / process / GitHub
/// CLI), JSON-schema validation and agent-adapter / ACP-model detection — the
/// concrete work a server performs. Kept SEPARATE from the `cc_host` RPC kernel
/// and free of Flutter / `cc_natives`, so both packages link into the
/// Flutter-free `dart build cli` server binary (the north star). The desktop
/// imports the same adapters during the thin-client migration.
library;

export 'src/agents/agent_discovery_service.dart';
export 'src/blobs/blob_store.dart';
export 'src/calendar/calendar_sync_service.dart';
export 'src/calendar/google_device_auth_client.dart';
export 'src/chat/chat_bridge_service.dart';
export 'src/chat/chat_deep_links.dart';
export 'src/chat/chat_link_code_store.dart';
export 'src/chat/chat_provider_adapter.dart';
export 'src/chat/chat_text.dart';
export 'src/claude_accounts/claude_account_store.dart';
export 'src/claude_accounts/credential_cooldown_store.dart';
export 'src/code_graph/code_extractor.dart';
export 'src/code_graph/code_graph_watch_service.dart';
export 'src/code_graph/code_indexer.dart';
export 'src/code_graph/extraction_isolate.dart';
export 'src/context/glyph_font.dart';
export 'src/context/snapcompact.dart';
export 'src/context/snapcompact_compactor.dart';
export 'src/context/text_raster.dart';
export 'src/dap/dap_adapter_registry.dart';
export 'src/dap/dap_client.dart';
export 'src/dap/debug_session.dart';
export 'src/detection/acp_models_repository_impl.dart';
export 'src/detection/acp_models_service.dart';
export 'src/detection/adapter_detection_repository.dart';
export 'src/detection/adapter_detection_service.dart';
export 'src/dictation/dictation_service.dart';
export 'src/dispatch/acp/acp_client.dart';
export 'src/dispatch/adapter_one_shot_runner.dart';
export 'src/dispatch/agent_dispatch_service.dart';
export 'src/dispatch/agent_registry_impl.dart';
export 'src/dispatch/backend_registry.dart';
export 'src/dispatch/backends/acp_backend.dart';
export 'src/dispatch/backends/cli_backends.dart';
export 'src/dispatch/backends/harness_backend.dart';
export 'src/dispatch/dispatch_session.dart';
export 'src/dispatch/goal_supervisor.dart';
export 'src/dispatch/guided_goal_service.dart';
export 'src/dispatch/persona_loader.dart';
export 'src/dispatch/sandboxed_agent_dispatch_adapter.dart';
export 'src/edit/file_edit_service.dart';
export 'src/edit/tree_sitter_block_resolver.dart';
export 'src/embedding/embedding_model_manager.dart';
export 'src/embedding/embedding_service.dart';
export 'src/eval/eval_kernel.dart';
export 'src/eval/magics_transform.dart';
export 'src/eval/rig_kernel_launcher.dart';
export 'src/fleet/fleet_scheduler_service.dart';
export 'src/fleet/local_job_executor.dart';
export 'src/fonts/fontsource_catalog_service.dart';
export 'src/geoip/geoip_lookup.dart';
export 'src/git/git_diff_z_parser.dart';
export 'src/git/git_repo_inspector.dart';
export 'src/git/github_api_pr_diff_source.dart';
export 'src/git/github_pr_search_adapter.dart';
export 'src/git/github_profile_activity_adapter.dart';
export 'src/git/github_viewer_activity_polling_service.dart';
export 'src/git/pr_polling_service.dart';
export 'src/git/process_git_command_adapter.dart';
export 'src/git/process_git_snapshot_adapter.dart';
export 'src/git/process_session_diff_adapter.dart';
export 'src/git/review_publisher_service.dart';
export 'src/git/service_status_service.dart';
export 'src/git/viewer_comment_fetch_port.dart';
export 'src/git/viewer_github_identity_cache.dart';
export 'src/harness/ast_parser_provider.dart';
export 'src/harness/ast_tools.dart';
export 'src/harness/cc_natives_file_search_port.dart';
export 'src/harness/debug_tool.dart';
export 'src/harness/eval_tool.dart';
export 'src/harness/file_transcript_store.dart';
export 'src/harness/harness_command_runner.dart';
export 'src/harness/harness_system_prompt.dart';
export 'src/harness/harness_tool_registry_builder.dart';
export 'src/harness/harness_tool_search_tool.dart';
export 'src/harness/harness_tool_surface.dart';
export 'src/harness/lsp_rename_tool.dart';
export 'src/harness/lsp_tool.dart';
export 'src/harness/mcp_tool_bridge.dart';
export 'src/harness/tools/apply_patch_tool.dart';
export 'src/ide/code_server_service.dart';
export 'src/ide/native_editor_launcher.dart';
export 'src/ide/reveal_in_file_manager.dart';
export 'src/log/cc_infra_log.dart';
export 'src/log/rotating_file_log_sink.dart';
export 'src/lsp/diagnostics_ledger.dart';
export 'src/lsp/lsp_client.dart';
export 'src/lsp/lsp_server_registry.dart';
export 'src/lsp/lsp_supervisor.dart';
export 'src/meetings/calendar_meeting_signal_collector.dart';
export 'src/meetings/meeting_audio_loader.dart';
export 'src/meetings/meeting_echo_filter.dart';
export 'src/meetings/meeting_recording_session.dart';
export 'src/meetings/meeting_summary_reconciler.dart';
export 'src/meetings/meeting_transcription_service.dart';
export 'src/meetings/process_meeting_signal_collector.dart';
export 'src/messaging/active_stream_registry.dart';
export 'src/messaging/agent_question_service.dart';
export 'src/messaging/agent_responder_resolver.dart';
export 'src/messaging/agent_stream_processor.dart';
export 'src/messaging/agent_working_directory.dart';
export 'src/messaging/conversation_checkpoint_coordinator.dart';
export 'src/messaging/conversation_checkpoint_service.dart';
export 'src/messaging/conversation_compaction_service.dart';
export 'src/messaging/conversation_side_channel_service.dart';
export 'src/messaging/conversation_title_service.dart';
export 'src/messaging/json_content_extractor.dart';
export 'src/messaging/messaging_service.dart';
export 'src/messaging/prompt_attachments.dart';
export 'src/messaging/run_transcript_recorder.dart';
export 'src/messaging/run_transcript_relay.dart';
export 'src/messaging/space_turn_relay.dart';
export 'src/messaging/steering_queue_service.dart';
export 'src/messaging/transcript_folder.dart';
export 'src/model_routing/file_models_dev_source.dart';
export 'src/model_routing/in_memory_models_dev_source.dart';
export 'src/model_routing/model_catalog_service.dart';
export 'src/network/app_network.dart';
export 'src/network/bitbucket/bitbucket_api_client.dart';
export 'src/network/bitbucket/bitbucket_forge_pr_client.dart';
export 'src/network/bitbucket/bitbucket_pr_mapper.dart';
export 'src/network/dedup_interceptor.dart';
export 'src/network/dns_wire.dart';
export 'src/network/error_mapper.dart';
export 'src/network/forge_dio_factory.dart';
export 'src/network/github/github_app_client.dart';
export 'src/network/github/github_forge_pr_client.dart';
export 'src/network/github_api_client.dart';
export 'src/network/github_content_client.dart';
export 'src/network/github_graphql_client.dart';
export 'src/network/github_pr_client.dart';
export 'src/network/gitlab/gitlab_api_client.dart';
export 'src/network/gitlab/gitlab_forge_pr_client.dart';
export 'src/network/gitlab/gitlab_pr_mapper.dart';
export 'src/network/google_calendar_api_client.dart';
export 'src/network/klipy_api_client.dart';
export 'src/network/mdns_responder.dart';
export 'src/network/models/date_parser.dart';
export 'src/network/models/github_check_run.dart';
export 'src/network/models/github_commit.dart';
export 'src/network/models/github_commit_status.dart';
export 'src/network/models/github_issue_comment.dart';
export 'src/network/models/github_job_run.dart';
export 'src/network/models/github_label.dart';
export 'src/network/models/github_pr_review_state.dart';
export 'src/network/models/github_pr_stack.dart';
export 'src/network/models/github_pull_request.dart';
export 'src/network/models/github_pull_request_file.dart';
export 'src/network/models/github_reaction.dart';
export 'src/network/models/github_review.dart';
export 'src/network/models/github_review_comment.dart';
export 'src/network/models/github_team.dart';
export 'src/network/models/github_timeline_event.dart';
export 'src/network/models/github_user_profile.dart';
export 'src/network/models/github_viewer_activity.dart';
export 'src/network/models/github_workflow_run.dart';
export 'src/network/models/google_calendar_event.dart';
export 'src/network/models/google_calendar_list_entry.dart';
export 'src/network/network_constants.dart';
export 'src/network/pr_review_mapper.dart';
export 'src/network/retry_interceptor.dart';
export 'src/network/slack_api_client.dart';
export 'src/network/tailscale_cli.dart';
export 'src/network/tunnel_manager.dart';
export 'src/network/weather_api_client.dart';
export 'src/network/workflow_graph_parser.dart';
export 'src/newsfeed/abp_parser.dart';
export 'src/newsfeed/filter_list_service.dart';
export 'src/newsfeed/rss_fetcher_service.dart';
export 'src/newsfeed/scriptlet_library.dart';
export 'src/newsfeed/site_icon_resolver.dart';
export 'src/pipelines/bash_script_template.dart';
export 'src/pipelines/condition_template.dart';
export 'src/pipelines/pr_review_pipeline_template.dart';
export 'src/pipelines/register_meeting_bodies.dart';
export 'src/ports/workspace_filesystem_port.dart';
export 'src/pr_review/api_contract_diff_service.dart';
export 'src/pr_review/dispatch_reviewers_service.dart';
export 'src/pr_review/image_differ.dart';
export 'src/pr_review/local_git_pr_diff_source.dart';
export 'src/pr_review/pr_clone_manager.dart';
export 'src/pr_review/pr_worktree_service.dart';
export 'src/pr_review/review_dependency_service.dart';
export 'src/pr_review/review_finalizer.dart';
export 'src/pr_review/review_finding_status_service.dart';
export 'src/pr_review/visual_diff_service.dart';
export 'src/process/binary_resolver.dart';
export 'src/process/cc_server_process.dart';
export 'src/process/process_control_service.dart';
export 'src/process/process_detection_service.dart';
export 'src/repos/filesystem_directory_browser.dart';
export 'src/repos/repo_script_service.dart';
export 'src/repos/repo_workspace_provisioner.dart';
export 'src/repos/rift_repo_isolation_adapter.dart';
export 'src/repos/worktree_gc_listener.dart';
export 'src/rigs/adb_client.dart';
export 'src/rigs/bidi_client.dart';
export 'src/rigs/browser_engine_attach.dart';
export 'src/rigs/browser_engine_client.dart';
export 'src/rigs/cdp_client.dart';
export 'src/rigs/guest_agent_client.dart';
export 'src/rigs/guest_credential_service.dart';
export 'src/rigs/host_shell_listeners.dart';
export 'src/rigs/host_ffmpeg.dart';
export 'src/rigs/ios_automation_store.dart';
export 'src/rigs/ios_rig_driver.dart';
export 'src/rigs/ios_simulator_backend.dart';
export 'src/rigs/qemu_argv.dart';
export 'src/rigs/qemu_enclosure_backend.dart';
export 'src/rigs/qemu_keymap.dart';
export 'src/rigs/qmp_client.dart';
export 'src/rigs/reverse_mux.dart';
export 'src/rigs/rig_browser_defaults.dart';
export 'src/rigs/rig_drivers.dart';
export 'src/rigs/rig_exec_defaults.dart';
export 'src/rigs/rig_image_store.dart';
export 'src/rigs/rig_machine.dart';
export 'src/rigs/rig_port_service.dart';
export 'src/rigs/rig_ports.dart';
export 'src/rigs/rig_service.dart';
export 'src/rigs/smolvm_enclosure_backend.dart';
export 'src/rigs/wda_client.dart';
export 'src/rigs/webdriver_client.dart';
export 'src/rigs/worktree_sync.dart';
export 'src/sandboxing/claude_stream_json.dart';
export 'src/sandboxing/domain_matcher.dart';
export 'src/sandboxing/env_credential_broker.dart';
export 'src/sandboxing/env_credentials_repository.dart';
export 'src/sandboxing/env_sanitizer.dart';
export 'src/sandboxing/github_app_token_minter.dart';
export 'src/sandboxing/github_fine_grained_broker.dart';
export 'src/sandboxing/http_proxy.dart';
export 'src/sandboxing/linux_sandbox.dart';
export 'src/sandboxing/macos_sandbox.dart';
export 'src/sandboxing/native_sandbox_adapter.dart';
export 'src/sandboxing/no_sandbox_adapter.dart';
export 'src/sandboxing/run_log_writer.dart';
export 'src/sandboxing/sandbox_backend_detector.dart';
export 'src/sandboxing/sandbox_config.dart';
export 'src/sandboxing/sandbox_config_builder.dart';
export 'src/sandboxing/sandbox_manager.dart';
export 'src/sandboxing/socks_proxy.dart';
export 'src/sandboxing/terminal_session_service.dart';
export 'src/sandboxing/violation_monitor.dart';
export 'src/skills/github_skill_source_adapter.dart';
export 'src/skills/repo_skill_catalog.dart';
export 'src/skills/skill_bundle_service.dart';
export 'src/skills/skill_llm_review_runner.dart';
export 'src/skills/skill_scanner_adapter.dart';
export 'src/skills/skill_watch_service.dart';
export 'src/slack/slack_chat_adapter.dart';
export 'src/slack/slack_manifest_service.dart';
export 'src/slack/slack_socket.dart';
export 'src/slack/slack_socket_mode_client.dart';
export 'src/slack/slack_task_card.dart';
export 'src/slack/slack_text.dart';
export 'src/soundscape/hls_segmenter.dart';
export 'src/soundscape/soundscape_context_builder.dart';
export 'src/soundscape/soundscape_hub.dart';
export 'src/soundscape/soundscape_session.dart';
export 'src/speech/diarization_model_manager.dart';
export 'src/speech/voice_model_manager.dart';
export 'src/teams/messaging_team_leader_dispatch.dart';
export 'src/tickets/linear/linear_graphql_client.dart';
export 'src/tickets/linear/linear_issue_dto.dart';
export 'src/tickets/linear/linear_ticket_adapter.dart';
export 'src/tickets/sync/clickup_ticket_sync_adapter.dart';
export 'src/tickets/sync/github_issues_ticket_sync_adapter.dart';
export 'src/tickets/sync/jira_ticket_sync_adapter.dart';
export 'src/tickets/sync/linear_ticket_sync_adapter.dart';
export 'src/usage/claude_usage_cache.dart';
export 'src/usage/claude_usage_strategy.dart';
export 'src/usage/codex_usage_strategy.dart';
export 'src/usage/cursor_usage_strategy.dart';
export 'src/usage/kimi_usage_strategy.dart';
export 'src/usage/subscription_usage_service.dart';
export 'src/usage/subscription_usage_strategy.dart';
export 'src/usage/zai_usage_strategy.dart';
export 'src/usecases/approve_orchestration_use_case.dart';
export 'src/usecases/hire_agent_use_case.dart';
// NOTE: agent_detection_service is intentionally NOT exported here — it defines
// its own `DetectionStatus`, which would collide with cc_domain's
// `DetectionStatus` for any barrel consumer. Import it by its src/ path.
export 'src/util/agents_md_parser.dart';
export 'src/util/cc_paths.dart';
export 'src/util/command_redaction.dart';
export 'src/util/isolate_json.dart';
export 'src/util/json_schema_validator.dart';
export 'src/util/wav_io.dart';
export 'src/weather/server_weather_service.dart';
export 'src/workspaces/create_ceo_agent.dart';
export 'src/workspaces/create_specialist_agents.dart';
export 'src/workspaces/workspace_filesystem_service.dart';
export 'src/workspaces/workspace_seeder.dart';
