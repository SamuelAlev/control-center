...
class SkillBundleService implements SkillBundlePort {
  ...
  Future<List<String>> reVerify(String workspaceId) async {
    ...
    for (final entry in lock.skills.values) {
      ...
      // Only verdict-stale entries need work; a current-version entry's cached
      // verdict already reflects the live rules.
      if (rv == null || rv >= kSkillRulesVersion) {
        continue;
      }
      try {
        final content = await _fs.readSkillFile(workspaceId, entry.slug);
        if (content == null) {
        ...
        final result = await scanner.scan(
          SkillBundle(slug: entry.slug, files: {'SKILL.md': content}),
          workspaceId: workspaceId,
