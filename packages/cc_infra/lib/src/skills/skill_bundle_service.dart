...
class SkillBundleService implements SkillBundlePort {
  ...
  Future<String?> computeSkillHash(String workspaceId, String slug) async {
    ...
    await for (final entity in dir.list(recursive: true, followLinks: false)) {
      ...
      // POSIX-canonical on every platform: the hash travels in a lockfile
      ...
      entries.add((relPosix, sha256.convert(bytes).toString()));
    }
    if (entries.isEmpty) {
      return null;
    }
  ...
  Future<SkillScanResult> previewFiles({
    ...
    // are byte-identical to what an install would write, so the later
  ...
  Future<SkillLockEntry?> uninstall({
    ...
    await _fs.deleteSkillDir(workspaceId, slug);
    if (entry != null) {
      await writeLock(workspaceId, lock.without(slug));
    }
    return entry;
  }

  /// Replaces the skill's on-disk directory with exactly [files] (relative
  /// path → content): delete-then-write so a file dropped upstream between
  /// versions never lingers to pollute the rolled-up hash.
  Future<void> _replaceSkillDir(
    String workspaceId,
    String slug,
    ...
    for (final e in files.entries) {
      // Bundle paths are POSIX-relative; split explicitly so a path never
      // lands on disk as a single weirdly-named file on any platform.
      final file = File(p.joinAll([dirPath, ...e.key.split('/')]));
      await file.parent.create(recursive: true);
