...
class GitHubPrReviewState {
  ...
  /// The PR's lifecycle state: GitHub's enum string `OPEN` / `CLOSED` /
  /// `MERGED` (empty when the PR could not be resolved).
  final String prState;

  /// Whether the PR is still a draft. Drafts are not reviewable yet —
  /// GitHub itself withholds review-request notifications until the PR is
  /// marked ready, even when CODEOWNERS already listed the viewer.
  final bool isDraft;
}
