...
class GitHubPullRequest {
  ...
  factory GitHubPullRequest.fromJson(Map<String, dynamic> json) {
    ...
    return GitHubPullRequest(
      ...
      // GitHub's REST payloads call the forge id `node_id`; `external_id` is
      // OUR name for it, and only [toJson] ever writes that key. Reading only
      // `external_id` meant every PR fetched from the API carried an EMPTY
      // forge id — so a review space keyed on it was keyed on '', the PR page
      // looked its review up by '' and found either nothing or another PR's
      // row, and the review tab reported "no review yet" for a PR that had one.
      // Ours first so a re-decoded [toJson] payload round-trips unchanged.
      externalId:
          json['external_id'] as String? ?? json['node_id'] as String? ?? '',
