...
class GitLabForgePrClient implements ForgePrClient {
  ...
  /// GitLab models a resolvable discussion, but this adapter does not read or
  /// write it yet (`commentThreadResolution` is false for GitLab), so nothing
  /// upstream calls this and it reports "no thread state known" rather than
  /// guessing every conversation is open.
  @override
  ...
  Future<void> submitReview({
    ...
    final token = _token(cancelToken);
    // GitLab has no single call that posts inline notes AND casts a verdict, so
    // a batched review lands as its comments first and the verdict after — the
    // author reads the reasoning before they are told the outcome. They are
    // posted sequentially on purpose: a burst of discussion creates on one MR is
    // what GitLab rate-limits first.
  ...
  /// Moves a merge request between draft and ready-for-review.
  ///
  /// GitLab has no draft field: the `Draft: ` title prefix *is* the state, so
  /// this reads the current title and writes it back with the prefix applied or
  /// stripped. Returns without a write when the merge request is already in the
  /// requested state, so a redundant call does not churn the MR's activity feed
  /// with a title change.
  @override
  Future<void> setPullRequestDraft({
    ...
    await _client.updateMergeRequest(
      ...
      prNumber,
      title: draft
          ? applyGitLabDraftPrefix(mr.title)
          : stripGitLabDraftPrefix(mr.title),
      cancelToken: token,
    );
  }
  ...
  /// Always empty: GitLab has no review objects to react to. The submissions
  /// [listReviews] synthesizes from approvals carry no forge identity, so
  /// there is nothing on the forge side to list — an empty answer is the
  /// truthful one, not a degraded one.
  @override
  ...
  Future<void> toggleReaction({
    ...
    if (target == ForgeReactionTarget.review) {
      throw const ForgeUnsupportedError(ForgeHost.gitlab, 'review reactions');
    }
    final token = _token(cancelToken);
