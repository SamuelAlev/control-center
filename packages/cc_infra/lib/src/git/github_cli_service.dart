...
class ProcessGitHubCliService implements GitHubCliPort {
  ...
  /// Runs `gh` with a hard timeout.
  ///
  /// This is called on the boot path and `gh auth status` / `auth token` reach
  /// a credential helper (macOS keychain, `pass`, a network-backed keyring) that
  /// can block indefinitely — a locked keychain will sit there forever. Without
  /// a bound, that is a server that never finishes booting and never says why.
  /// A timeout degrades to "no gh auth", which the caller already handles.
  Future<ProcessResult> _run(String binary, List<String> args) async {
    try {
      return await Process.run(binary, args).timeout(_timeout);
    } on TimeoutException {
      return ProcessResult(
        0,
        1,
        '',
        'gh ${args.join(' ')} timed out after ${_timeout.inSeconds}s',
      );
  ...
  /// Ceiling on a single `gh` invocation (see [_run]).
