...
/// A PR search adapter backed by GitHub's GraphQL `search`.
///
...
class GitHubPrSearchAdapter {
  ...
  /// Resolves [query] across [repos] in one chunked GraphQL `search`,
  /// returning the matches grouped per repo (empty when the query is inactive
  /// or no repo is in scope).
  Future<List<RepoPullRequests>> search({
    ...
    final nodes = await _client.graphql.searchPullRequestNodes(
      ...
      repos: repos
          .map((r) => (owner: r.remoteOwner, name: r.remoteName))
