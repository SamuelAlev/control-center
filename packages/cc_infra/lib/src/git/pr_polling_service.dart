...
class PrPollingService {
  ...
  /// Guards against overlapping polls. `Timer.periodic` fires on a wall clock
  /// and does not wait for the async body, so a sweep slower than [_interval]
  /// would stack a second one on top of it and double the GitHub calls — the
  /// exact failure the sibling notification poller avoids with this flag.
  bool _polling = false;
  ...
  Future<void> _pollRepo(String owner, String name) async {
    ...
    try {
      final page = await _githubClient.pr.listOpenPullRequestsPage(owner, name);
      ...
      CcInfraLog.warning('pr_polling: Failed to poll $repoKey: $e');
    }
  }
