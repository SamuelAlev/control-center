...
class AgentProcessDataSource implements AgentDispatchPort {
  ...
  /// Hard cap on the coalesce buffer. The 50ms window normally keeps it tiny,
  /// but a process that dumps megabytes in one burst (a tool result echoing a
  /// large file) would otherwise accumulate it all in one in-memory event.
  static const int _coalesceMaxChars = 64 * 1024;
  ...
  void _bufferEvent(AgentProcessEventType type, String content) {
    ...
    if (_bufferedContent.length >= _coalesceMaxChars) {
      _flushBufferedEvent();
      return;
    }
  ...
  DispatchHandle start({
    ...
    // Accepted for interface parity; slash commands are a built-in-harness
    // concept and this datasource spawns external CLIs.
    String? userText,
    String? modelId,
    String? agentId,
    // Accepted for interface parity; the per-run git identity + co-author
    // trailer are applied by the sandboxed adapter, which owns env assembly.
    String? agentName,
    String? workspaceId,
    ...
    Mode? mode,
    // This direct (non-sandboxed) datasource has no silence monitor; the
    // override is accepted for interface parity and applied by the sandboxed
    // adapter, which owns the silence timer.
    ...
    // Accepted for interface parity; this datasource spawns external CLIs
    // and cannot price their usage — mid-run cost caps are enforced by the
    // sandboxed (built-in harness) adapter only.
    int? costCapCents,
  }) {
    ...
    final dispatchId =
        runLogId ?? 'ds-${DateTime.now().millisecondsSinceEpoch}';
    _spawnProcess(cliName, prompt, modelId: modelId);

    return DispatchHandle(dispatchId: dispatchId, events: _controller!.stream);
  }
  ...
  Future<bool> steerDispatch(
    ...
    String message, {
    bool followUp = false,
  }) async => false; // External-CLI processes have no mid-run steering channel.

  @override
  Future<bool> pauseDispatch(String dispatchId) async => false; // External-CLI processes have no safe turn boundary to pause at.

  @override
  Future<bool> resumeDispatch(String dispatchId) async => false;
  ...
  Future<void> _spawnProcess(
    ...
    if (cliName != 'pi') {
      _addError(
        ArgumentError('Unsupported CLI "$cliName" — only "pi" is supported.'),
      );
      _closeController();
      return;
    }
  ...
  Future<void> _spawnPi(String prompt, {String? modelId}) async {
    try {
      // Resolve the agent binary to its absolute path: a bundled `.app` / `.desktop`
      // launch inherits a minimal PATH (no Homebrew/Nix), so the bare name
      // fails with "command not found". Same probe Settings → Adapters
          ...
          .listen(
            (line) {
              try {
                ...
                // Keep the line's content (don't drop output) but emit a
                // grep-able marker so an NDJSON parse drift is visible in the
                // run log instead of silently masquerading as agent text.
                _addEvent(
                  DebugEvent(
                    content:
                        '[ndjson-parse-error] unparseable line treated as '
  ...
  void handlePiEvent(Map<String, dynamic> json) {
    ...
    switch (type) {
        ...
        switch (eventType) {
          ...
          case 'error':
            _emitEvent(
              ErrorEvent(
                content: content,
                code: json['code'] as String? ?? json['errorType'] as String?,
                source: 'pi',
              ),
            );
          case 'tool_call':
          ...
          default:
            _emitEvent(
              DebugEvent(
                content:
                    '[ndjson-parse-error] unknown pi eventType '
                    '"$eventType"',
              ),
            );
        }
      ...
      default:
        _emitEvent(
          DebugEvent(
            content: '[ndjson-parse-error] unknown pi event type "$type"',
          ),
        );
    }
  }
