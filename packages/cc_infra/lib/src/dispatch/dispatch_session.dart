...
class DispatchSession {
  ...
  Future<void> _runStructuredCli({
    ...
    if (handle.state == SandboxState.error) {
      ...
      try {
        ...
        CcInfraLog.warning('dispatch $dispatchId: destroy after launch '
    ...
    if (store != null) {
      try {
        for (final cred in await store.credentialsFor(primaryProviderId)) {
          ...
          // `providers.saveGenerationDefaults` before any key existed) is not a
          // rotation target — failing over to it would 401 with no account to
          // blame. Custom-provider definitions (dialect-carrying) are exempt:
          // their none-method credential IS the endpoint.
  ...
  Map<String, String> _mergedEnv({
    ...
    final memberToken = _memberGitHubToken;
    if (memberToken != null) {
      merged['GH_TOKEN'] = memberToken;
      merged['GITHUB_TOKEN'] = memberToken;
    }
    return merged;
  }
  ...
  Future<void> _runClaudeCli({
    ...
    _claudeParser = ClaudeStreamJsonParser(
      ClaudeStreamJsonCallbacks(
        ...
        onError: (message) =>
            addEvent(ErrorEvent(content: redactSecrets('[claude] $message'))),
      ),
    );
  ...
  Future<void> _killActiveProcess(String reason) async {
    ...
    if (childPid != null) {
      try {
        ...
        CcInfraLog.warning('dispatch $dispatchId: kill pid $childPid failed: $e');
      }
    }
    ...
    silenceTimer = Timer.periodic(silenceCheckInterval, (_) {
      ...
      if (last != null && DateTime.now().difference(last) >= threshold) {
        ...
        unawaited(() async {
          ...
          await _closeRunLog(
            error: 'silent for ${threshold.inMinutes} min',
          );
          _failRun(
