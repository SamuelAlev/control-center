...
/// Posts a system message into the goal's space. Wired by the runtime to
...
class GoalSupervisor implements GoalSupervisionPort {
  ...
  Future<AgentGoalRun?> startGoal({
    ...
    // A token that looked like a limit and was not is reported rather than
    // silently starting an UNBOUNDED autonomous loop — the failure mode there
    ...
    final goal = AgentGoalRun(
      ...
      kind: kind,
      // A leading duration (`/loop 30m …`) is a deadline expressed the way
      // people actually type it; `--days` remains for longer horizons.
      deadlineAt: switch ((flags.days, flags.timeBudget)) {
        (final int days, _) => now.add(Duration(days: days)),
        (_, final Duration budget) => now.add(budget),
        _ => null,
      },
...
class _GoalFlags {
  ...
  /// Wall-clock budget from a leading duration (`/loop 30m …`), or null.
  ///
  /// Distinct from [days]: a duration bound is what someone types when they
  /// mean "work on this for half an hour", and expressing that as a fractional
  /// day loses the intent and the precision.
  final Duration? timeBudget;

  /// Set when a leading token LOOKED like a limit and failed to parse, so the
  /// caller can report the mistake instead of silently running unbounded.
  final String? error;
  ...
  static _GoalFlags parse(String text) {
    // A LEADING count or duration is the natural form (`/loop 10 fix tests`,
    // `/loop 30m refactor`) and is what people type before they discover the
    // `--flag` spellings. Parsed first so it can coexist with them.
    final leading = parseLoopArgs(text);
    if (leading.isError) {
      return _GoalFlags(objective: text.trim(), error: leading.error);
    }
    var remaining = text;
    int? leadingRuns;
    Duration? leadingTime;
    if (leading.limit != null) {
      remaining = leading.prompt ?? '';
      switch (leading.limit!) {
        case LoopIterationLimit(:final iterations):
          leadingRuns = iterations;
        case LoopDurationLimit(:final duration):
          leadingTime = duration;
      }
    }
    ...
    return _GoalFlags(
      ...
      // An explicit `--max` wins over a leading count: the longer spelling is
      // the more deliberate one.
      maxRuns: maxRuns ?? leadingRuns,
      budgetCents: budgetCents,
      days: days,
      timeBudget: leadingTime,
    );
  }
