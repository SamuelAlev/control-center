...
class RunLogWriter {
  ...
  /// Returns a copy of [metadata] with any secret-shaped values scrubbed, by
  /// running the regex redactor over its JSON encoding. Returns null unchanged.
  Map<String, dynamic>? _redactMetadata(Map<String, dynamic>? metadata) {
    if (metadata == null || metadata.isEmpty) {
      return metadata;
    }
    try {
      return jsonDecode(redactSecrets(jsonEncode(metadata)))
          as Map<String, dynamic>;
    } catch (_) {
      // Non-encodable metadata — drop it rather than risk persisting a secret.
      return null;
    }
  }
