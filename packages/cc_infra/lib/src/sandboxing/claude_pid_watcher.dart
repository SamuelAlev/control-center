...
class ClaudePidWatcher {
  ...
  void _poll() {
    // H3 hardening: verify pid liveness independently before trusting the
    // agent-supplied session file. A dead pid means the session file may be
    // stale or forged — do not act on its status fields.
    if (!_isPidAlive()) {
      if (_lastStatus != 'dead') {
        _lastStatus = 'dead';
        onStatusChange('dead', null, PidFileData(
          pid: pid,
          sessionId: '',
          cwd: '',
          kind: '',
  ...
  /// Returns true when [pid] is an alive process. Uses signal 0 (existence
  /// check) — does not actually send a signal.
  bool _isPidAlive() {
    try {
