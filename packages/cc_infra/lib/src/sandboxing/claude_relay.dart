...
import 'package:cc_infra/src/sandboxing/anthropic_proxy.dart';
...
// Pure-Dart PTY (vendored flutter_pty native, built as libccpty) — Flutter-free
// so the relay links into the `dart build cli` cc_server binary. API-identical
// to the old flutter_pty `Pty`, so the spawn/output/exitCode/write/kill calls
// below are unchanged.
...
class ClaudeRelay {
  ...
  static List<String> buildClaudeArgs({
    ...
    if (mcpConfigPath != null && mcpConfigPath.isNotEmpty) {
      ...
      // behind a separate approval prompt that the relay (which only
      // auto-confirms the folder-trust prompt) never answers, and
      // `--dangerously-skip-permissions` does not lift that gate. Without this
      // the agent sees zero `mcp__*` tools and cannot call `complete_ticket`.
      // `--strict-mcp-config` makes Claude use ONLY this config, avoiding a
      // duplicate of the same server picked up from project discovery.
  ...
  Future<int> run({
    ...
    // When set, the PTY spawns the pre-wrapped sandbox command instead of
    // assembling claudePath + args + prompt. The prompt is already baked
    // into the wrapped inner command.
    ...
    try {
      ...
      final env = <String, String>{
        ...const EnvSanitizer().hardenPlatform({}),
        ...environment,
        'ANTHROPIC_BASE_URL': proxy.baseUrl,
      };

      final exec = wrappedExecutable ?? claudePath;
      final execArgs = wrappedArgv ?? [...args, prompt];
