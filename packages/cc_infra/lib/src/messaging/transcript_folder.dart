...
/// registry (relayed to thin clients via `messaging.watchSpaceTurns` and
...
/// the same folding serve a space turn (`AgentStreamProcessor`, keyed by
...
class TranscriptFolder {
  ...
  void _applyToolResult(ToolResultEvent event) {
    ...
    final closed = existing.copyWith(
      ...
      // Screenshots ride the final result only; a partial never carries them.
      images: event.images.isEmpty ? null : event.images,
    );
