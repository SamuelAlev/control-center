...
class ConversationCheckpointService {
  ...
  /// Reverts the conversation to [messageId] within [workspaceId]. When
  /// [inclusive] the target message is reverted too. When [worktreePath] is
  /// provided and the relevant turn carries a git snapshot, the filesystem is
  /// rolled back as well: keeping the target → restore to its post-state;
  /// reverting it → its pre-state.
  ...
  /// the filesystem is not re-applied (the user can re-run the agent to
  /// regenerate changes).
  Future<RevertOutcome> unrevert({
    required String workspaceId,
    required String channelId,
  }) async {
    final restored = await _repo.unrevertConversation(workspaceId, channelId);
    return RevertOutcome(
