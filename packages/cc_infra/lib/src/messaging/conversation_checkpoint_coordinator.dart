...
/// channel's agent participant's working directory) and runs the
...
class ConversationCheckpointCoordinator {
  ...
  /// Reverts [channelId] to [messageId] (undo) within [workspaceId]. Resolves
  /// the conversation's
  ...
  Future<RevertOutcome> revertTo({
    ...
    final worktreePath = await _resolveWorktree(workspaceId, channelId);
  ...
  Future<RevertOutcome> unrevert({
    required String workspaceId,
    required String channelId,
  }) => _service.unrevert(workspaceId: workspaceId, channelId: channelId);
  ...
  /// only.
  Future<String?> _resolveWorktree(String workspaceId, String channelId) async {
