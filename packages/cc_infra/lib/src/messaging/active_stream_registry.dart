...
/// A single relayed turn event on a space stream: which in-flight message
...
class ActiveStreamRegistry {
  ...
  // Space index for the live turn relay.
  final Map<String, Set<String>> _bySpace = {};
  final Map<String, String> _spaceOf = {};
  final Map<String, StreamController<SpaceTurnUpdate>> _spaceStreams = {};
  ...
  /// Message ids of the turns currently streaming in [spaceId].
  Iterable<String> activeIn(String spaceId) =>
      _bySpace[spaceId] ?? const <String>{};

  /// Broadcast of every active turn's updates in [spaceId], for the relay.
  ///
  /// The stream survives individual turns (a space can stream several turns
  /// over a subscription's lifetime) and is torn down when the last listener
  /// cancels and no turn is active.
  Stream<SpaceTurnUpdate> spaceUpdates(String spaceId) =>
      _spaceController(spaceId).stream;

  StreamController<SpaceTurnUpdate> _spaceController(String spaceId) {
    return _spaceStreams.putIfAbsent(spaceId, () {
      late final StreamController<SpaceTurnUpdate> controller;
      controller = StreamController<SpaceTurnUpdate>.broadcast(
        onCancel: () {
          // Last listener gone: drop the controller unless turns are active
          // (they would still need to broadcast to a future subscriber).
          if (!controller.hasListener && (_bySpace[spaceId]?.isEmpty ?? true)) {
            _spaceStreams.remove(spaceId);
            controller.close();
          }
        },
      );
  ...
  /// Opens a turn for [messageId]. [spaceId] indexes the turn for the
  /// space relay; the thin client may omit it (it folds per space anyway).
  void register(String messageId, {String? spaceId}) {
    ...
    if (spaceId != null) {
      _spaceOf[messageId] = spaceId;
      (_bySpace[spaceId] ??= <String>{}).add(messageId);
    }
  ...
  void seed(
    ...
    register(messageId, spaceId: spaceId);
  ...
  void apply(String messageId, TranscriptUpdate update) {
    ...
    final spaceId = _spaceOf[messageId];
    ...
    if (spaceId != null) {
      if (_spaceStreams[spaceId] case final controller?
  ...
  Future<void> unregister(String messageId) async {
    ...
    final spaceId = _spaceOf.remove(messageId);
    if (spaceId != null) {
      final set = _bySpace[spaceId];
      set?.remove(messageId);
      if (set != null && set.isEmpty) {
        _bySpace.remove(spaceId);
        final space = _spaceStreams[spaceId];
        if (space != null && !space.hasListener) {
          _spaceStreams.remove(spaceId);
          await space.close();
        }
      }
