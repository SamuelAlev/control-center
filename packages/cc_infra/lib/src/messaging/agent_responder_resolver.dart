...
class AgentResponderResolver {
  ...
  ///
  /// Prefers the agent that last sent a message in the space; if none,
  /// picks the lead hint or the first top-level agent. Returns `null` when
  /// no agents are available. For a single-agent space the fallback chain
  /// naturally resolves to that sole agent.
