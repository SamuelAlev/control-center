...
/// The server half of the live turn relay (`messaging.watchChannelTurns`).
///
/// Emits wire frames for one channel from [registry]:
...
Stream<Map<String, dynamic>> watchChannelTurnFrames(
  ...
  void enqueue(ChannelTurnUpdate event) {
  ...
  out = StreamController<Map<String, dynamic>>(
    onListen: () {
      ...
      sub = registry.channelUpdates(channelId).listen(enqueue);
      out.add({
        'kind': 'seed',
        'turns': [
          for (final messageId in registry.activeIn(channelId).toList())
            {
