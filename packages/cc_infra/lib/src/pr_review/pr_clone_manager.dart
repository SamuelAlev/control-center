...
class PrCloneManager {
  ...
  /// CoW backend for seeding from a local checkout. Null when the caller has no
  /// rift wired (then a network clone is the only option). An *unloadable*
  /// non-null client is a broken install and throws — see [_tryRiftCopy].
  final RiftClient? _rift;

  /// Whether an unloadable `librift_ffi` is a supported platform gap (Windows,
  /// which ships no rift) rather than a broken install.
  ...
  Map<String, String> get _authEnv {
    ...
    return {
      ..._baseEnv,
      'GIT_CONFIG_PARAMETERS': existing.isEmpty ? param : '$existing $param',
    };
  }
  ...
  Stream<PrCloneProgress> ensureCloneAndFetch({
    ...
    try {
      final cloneDir = Directory(
        await _filesystem.prCloneDir(_workspaceId, _owner, _repo),
      );
  ...
  /// false when there is nothing to copy from or CoW is genuinely unusable here
  /// (non-CoW filesystem, an operational rift error, or Windows, which ships no
  /// rift). The token is never involved — the copy inherits the source's clean
  /// `origin`; the subsequent [_doFetch] pulls PR refs over HTTPS.
  ///
  /// Throws when a wired rift client cannot load its dylib on a platform that
  /// ships it: that is a broken install, and quietly network-cloning forever
  /// would hide it.
  Future<bool> _tryRiftCopy(Directory cloneDir) async {
    final src = _localCheckoutPath;
    final rift = _rift;
    // No local checkout / no rift wired: a network clone is the correct answer,
    // not a degradation.
    if (src == null ||
        src.isEmpty ||
        rift == null ||
    ...
    if (!rift.isAvailable) {
      if (_missingRiftIsExpected) {
        return false; // Windows: no CoW backend exists, network clone it is.
      }
      throw const RiftException(
        code: 'unavailable',
        message:
            'rift native library is not loaded — build it with '
            'scripts/natives/build_rift.sh, or rebuild the host bundle with '
            'the natives staged.',
      );
    }
    try {
      ...
      if (e.isUnavailable && !_missingRiftIsExpected) {
        // The worker isolate could not load the dylib the main isolate just
        // probed successfully: a broken install, so surface it.
        rethrow;
      }
      CcInfraLog.warning(
