...
/// (who may act, which space it drives, whether the thread is bridged) and
...
final class ChatMessageEvent extends ChatInboundEvent {
  ...
  /// treats as one continuous conversation rather than one space per thread.
  final bool isDm;
}
...
///
/// Slack requires it to open a stream in a *space* (`recipient_user_id` +
...
class ChatRecipient {
  ...
  /// shared (Slack Connect) space this is the *sender's* team, which is not
...
/// Slack it is a space plus a message `ts`) and the core only ever passes the
/// handle back.
...
abstract interface class ChatProviderAdapter {
  ...
  /// space without it. Null for a turn nobody started from chat (an agent
  /// posting into a bridged space on its own), which a provider that requires
  ...
  /// Titles a thread after the Control Center space it drives. Only called
  ...
  ///
  /// Providers usually offer a pre-authorized reply space that works even
