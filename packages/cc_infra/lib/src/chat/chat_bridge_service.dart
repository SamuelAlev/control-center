...
/// Creates the Control Center space that mirrors an external chat
/// conversation.
///
...
typedef ChatSpaceFactory =
    Future<Space> Function({
...
class ChatBridgeService {
  ...
  ChatBridgeService({
    ...
    required this._createSpace,
    required this._spaceLinks,
    required this._userLinks,
    required this._users,
    required this._members,
    required this._linkCodes,
    this._eventBus,
    this._createTicket,
    this._deepLinks,
    String Function()? newId,
    DateTime Function()? clock,
    this._flushInterval = const Duration(seconds: 1),
    this._onStreamingAvailability,
  ...
  final ChatSpaceFactory _createSpace;
  ...
  /// Where a reply for a Control Center space goes, refreshed on every inbound
  /// message so a DM's reply lands in the thread that asked.
  final Map<String, _ChatTarget> _targets = {};

  /// One subscription per bridged Control Center space.
  final Map<String, StreamSubscription<SpaceTurnUpdate>> _spaceSubs = {};

  /// One relay per in-flight agent turn.
  final Map<String, _TurnRelay> _relays = {};

  /// A card per space whose workspace is still being built, keyed by Control
  /// Center space id. A fresh space clones its repos before the agent can
  ...
  /// Spaces with a chat request in flight. Setup is only worth reporting for
  /// one: provisioning also runs where nobody in chat is waiting (a space
  ...
  StreamSubscription<SpaceProvisioningChanged>? _setupSub;
  ...
  /// Arms the bridge: relays for already-linked spaces, the inbound event
  ...
  Future<void> start() async {
    ...
    _started = true;
    try {
      for (final link in await _spaceLinks.forWorkspace(
        workspaceId,
        provider: provider,
      )) {
        _targets[link.ccSpaceId] = _ChatTarget(
    ...
    // ignore: cancel_subscriptions
    _setupSub = _eventBus?.on<SpaceProvisioningChanged>().listen(_onSetup);
  ...
  Future<void> stop() async {
    ...
    _setupSub = null;
    for (final sub in _spaceSubs.values) {
      await sub.cancel();
    }
    _spaceSubs.clear();
  ...
  Future<void> _onMessage(ChatMessageEvent event) async {
    ...
    if (!event.viaMention && !event.isDm) {
      ...
      final bridged = await _spaceLinks.forExternalThread(
        workspaceId,
        provider: provider,
    ...
    final link = await _linkFor(
      ...
      // Control Center space. A shared space anchors per thread.
    ...
    _targets[link.ccSpaceId] = _ChatTarget(
      ...
      // Who the streamed reply is for. Slack will not open a stream in a space
      // without it and the asker's team is not always the app's own (a shared
      // space), so it comes from the message rather than from the connection.
    ...
    _awaitingRequest.add(link.ccSpaceId);
    ...
    await _openRequestCard(link.ccSpaceId);
    // SpaceCreated starts provisioning during `_linkFor`, so the first
    ...
    await _catchUpSetup(link.ccSpaceId);
    try {
      ...
      _awaitingRequest.remove(link.ccSpaceId);
      await _closeUnclaimedSetupCard(link.ccSpaceId);
      rethrow;
    }
  ...
  Future<ChatSpaceLink> _linkFor({
    ...
    final existing = await _spaceLinks.forExternalThread(
      workspaceId,
      provider: provider,
      externalChannelId: conversationId,
    ...
    if (existing != null &&
        await _messaging.spaceExists(workspaceId, existing.ccSpaceId)) {
      await _spaceLinks.upsert(existing.copyWith(lastActivityAt: _clock()));
      return existing;
    }

    final name = await _spaceName(
      conversationId: conversationId,
      seedText: seedText,
      isDm: isDm,
    );
    final created = await _createSpace(
      workspaceId: workspaceId,
      name: name,
      createdByUserId: userId,
    );
    final link = ChatSpaceLink(
      ...
      externalChannelId: conversationId,
      externalThreadId: threadId,
      ccSpaceId: created.id,
      createdByUserId: userId,
      createdAt: _clock(),
      lastActivityAt: _clock(),
    );
    await _spaceLinks.upsert(link);
    ...
    CcInfraLog.info(
      ...
      '${threadId == null ? '' : '/$threadId'} → space ${created.id}',
    );
    return link;
  }

  Future<String> _spaceName({
    required String conversationId,
    required String seedText,
    required bool isDm,
  }) async {
  ...
  /// Answers a command, preferring the adapter's own reply space (which works
  ...
  Future<void> _reply(ChatCommandEvent event, _Answer answer) async {
    ...
    // provider's reply space is gone and the text already says everything the
    // card would.
    await _ephemeral(
  ...
  /// command whose own reply space is gone.
  ///
  ...
  /// into the space everyone is reading.
  ...
  void _watchSpace(String ccSpaceId) {
    _spaceSubs.putIfAbsent(
      ccSpaceId,
      () => _registry
          .spaceUpdates(ccSpaceId)
          .listen((update) => _onTurnUpdate(ccSpaceId, update)),
    );
  }

  void _onTurnUpdate(String ccSpaceId, SpaceTurnUpdate event) {
    final target = _targets[ccSpaceId];
    if (target == null) {
      return;
    }
    _awaitingRequest.remove(ccSpaceId);
    final relay = _relays.putIfAbsent(
      event.messageId,
      // A card already reporting this space's setup IS this turn's card: the
      ...
      // in a bot DM several threads share one Control Center space and the
      // answer belongs to the thread that asked.
      () =>
          _setupRelays.remove(ccSpaceId) ??
          _TurnRelay(
            target: target,
            ccSpaceId: ccSpaceId,
  ...
  Future<void> _openRequestCard(String ccSpaceId) async {
  ...
  /// Reports a space's workspace setup on the card the request already opened.
  ///
  ...
  void _onSetup(SpaceProvisioningChanged event) {
    ...
    final target = _targets[event.spaceId];
    if (target == null || !_awaitingRequest.contains(event.spaceId)) {
      return;
    }
    // A card is already reporting a turn in this space and its own narration
    ...
    if (_relays.values.any((relay) => relay.ccSpaceId == event.spaceId)) {
      return;
    }
    // A mention already opened a card; `ready` on a warm space that somehow
    // never did is left quiet rather than flashing "Starting the agent…".
    if (event.status == SpaceProvisioningStatus.ready &&
        !_setupRelays.containsKey(event.spaceId)) {
      return;
    }
    final relay = _setupRelays.putIfAbsent(
      event.spaceId,
      () => _TurnRelay(
        ...
        cardId: 'setup-${event.spaceId}',
        title: target.requestTitle ?? _genericCardTitle,
        link: _cardLink(event.spaceId),
      ),
    );
    relay.setSetupNote(_setupNote(event));
    // Stopped counts the same as failed here: no turn is coming, so the card
    // must be closed rather than left spinning.
    if (event.status.isStopped) {
      _awaitingRequest.remove(event.spaceId);
      unawaited(_closeUnclaimedSetupCard(event.spaceId));
      return;
    }
    if (relay.hasCardUpdate) {
      relay.scheduleFlush(
  ...
  /// Paints a setup card from the space row when events raced the inbound
  /// handler: provisioning starts in `createSpace`, the first announce can
  /// land before `_awaitingRequest` contains the id.
  Future<void> _catchUpSetup(String ccSpaceId) async {
    try {
      final space = await _messages.getSpaceById(workspaceId, ccSpaceId);
      if (space == null ||
          space.provisioningStatus == SpaceProvisioningStatus.ready) {
        return;
      }
      _onSetup(
        SpaceProvisioningChanged(
          workspaceId: workspaceId,
          spaceId: ccSpaceId,
          status: space.provisioningStatus,
          step: space.provisioningStep,
          occurredAt: _clock(),
        ),
      );
    } on Object catch (e) {
      _warn('catching up on setup for $ccSpaceId failed: $e');
    }
  }

  /// What the card says while a space's workspace is being built.
  ///
  ...
  static String? _setupNote(SpaceProvisioningChanged event) =>
      switch (event.status) {
        SpaceProvisioningStatus.provisioning => switch (event.step) {
          ...
          SpaceProvisioningStep(
            ...
            :final subject,
              ...
              when subject.isNotEmpty =>
            'Running the setup script for $subject…',
          SpaceProvisioningStep(
        ...
        // wait a warm space has — so the card says what it is waiting for.
        SpaceProvisioningStatus.ready => 'Starting the agent…',
        SpaceProvisioningStatus.failed =>
          'Workspace setup failed — details are in Control Center.',
        SpaceProvisioningStatus.cancelled =>
          'Workspace setup was stopped — details are in Control Center.',
      };
  ...
  Future<void> _closeUnclaimedSetupCard(String ccSpaceId) async {
    if (_registry.activeIn(ccSpaceId).isNotEmpty) {
      return;
    }
    final relay = _setupRelays.remove(ccSpaceId);
    if (relay == null) {
      return;
    }
    relay.cancelFlush();
  ...
  Future<ChatRecipient?> _recipientFor(ChatSpaceLink link) async {
    ...
    try {
      ...
      // fail over it.
      _warn('resolving the reader of ${link.ccSpaceId} failed: $e');
      return null;
    }
  }
  ...
  ChatTaskLink? _cardLink(String ccSpaceId) {
    final url = _deepLinks?.space(workspaceId, ccSpaceId);
    return url == null
        ? null
        : ChatTaskLink(label: 'View in Control Center', url: url);
  }
  ...
  Future<void> _mirrorOut(MessageReceived event) async {
    ...
    try {
      ...
      if (message == null || message.senderType != SenderType.user) {
        return;
      }
...
/// Where in the chat product a Control Center space's traffic goes.
class _ChatTarget {
  const _ChatTarget({
  ...
  /// The last thing asked here, as a card title. Null for a space armed from a
  /// stored link at boot — nobody has asked anything through it yet this run.
  final String? requestTitle;

  /// Who asked, in the provider's id space. Null for a space armed from a
  /// stored link at boot; a provider that needs a recipient to stream (Slack, in
  /// a space) then posts the reply whole instead.
...
class _TurnRelay {
  ...
  /// The Control Center space this relay reports on.
  final String ccSpaceId;
