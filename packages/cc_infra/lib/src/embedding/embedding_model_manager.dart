...
class EmbeddingModelManager {
  ...
  Future<EmbeddingModelPaths?> resolve() async {
    ...
    if (modelFile.existsSync() && vocabFile.existsSync()) {
      return EmbeddingModelPaths(model: modelFile.path, vocab: vocabFile.path);
    }
    return null;
  }
  ...
  Future<EmbeddingModelPaths> install({
    ...
    // Transfer into `.part` siblings and rename into place only once BOTH
    // downloads have finished. [resolve] is an existence check with no checksum
    // or size test, so a file sitting at its final path has to be complete by
    // construction. Writing straight there meant a process killed mid-transfer
    // left a truncated file that every later boot reported as installed — and a
    // kill is the ordinary case here, not an exotic one: the first-boot fetch
    // runs for minutes after the ready banner, quitting the app during it kills
    // the server, and no `catch` runs for a SIGKILL. A rename within one
    // directory is atomic, so the window shrinks to two adjacent renames.
    final modelPart = '$modelPath.part';
    final vocabPart = '$vocabPath.part';
    ...
    try {
      ...
      await File(vocabPart).rename(vocabPath);
    } catch (e) {
      // Clean up partial files so the next attempt starts fresh. Only the
      // `.part` files can be incomplete; anything already renamed is whole.
      for (final path in [modelPart, vocabPart]) {
        final f = File(path);
        if (f.existsSync()) {
          await f.delete();
        }
