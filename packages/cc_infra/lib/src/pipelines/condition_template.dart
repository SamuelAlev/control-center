  ...
  registry.registerBody(BuiltInBodyKeys.condition, (ctx) async {
    ...
    if (config == null) {
      return StepResult.failed(
        'condition: step "${ctx.stepId}" missing config',
      );
    }
    ...
    if (switchKey is String && switchKey.isNotEmpty) {
      final raw =
          (ctx.state[switchKey] ?? ctx.triggerPayload?[switchKey])
              ?.toString() ??
          '';
...
Future<bool> _fileExists(
  ...
  final baseKey = node['baseKey'] as String? ?? 'repo_local_path';
...
class _PredicateContext {
  ...
  /// Resolves (and ensures) the per-run workspace dir for [pipelineRunId].
  /// Injected so this template stays Flutter-free (no app-storage lookup).
  final Future<String> Function(String pipelineRunId) runDirPath;
  ...
  Future<String> runDir() async {
    return _runDirPath ??= await runDirPath(pipelineRunId);
  }
}
