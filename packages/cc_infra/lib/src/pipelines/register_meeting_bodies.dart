...
void registerMeetingBodies(
  ...
  Future<(Meeting, MeetingOutcome)?> load(PipelineContext ctx) async {
    final meetingId = ctx.optional<String>('meeting_id');
    ...
    return (meeting, MeetingOutcome.parse(ctx.state['meeting_outcome']));
  }
  ...
  registry.registerBody(BuiltInBodyKeys.meetingDiarize, (ctx) async {
    final meetingId = ctx.optional<String>('meeting_id');
    if (meetingId == null || meetingId.isEmpty) {
      return StepResult.failed('meeting.diarize: missing meeting_id');
    }
    final meeting = await meetingRepository.getById(ctx.workspaceId, meetingId);
    if (meeting == null) {
    ...
    try {
      ...
      if (audioDir == null || audioDir.isEmpty) {
        ...
        return StepResult.ok(mutatedState: const {'meeting_diarized': false});
      }
      if (modelPaths == null) {
        _logI(
          'meeting.diarize',
      ...
      await meetingRepository.replaceSpeakers(ctx.workspaceId, meetingId, [
      ...
      final relabeled = await meetingRepository.getSegments(
        ctx.workspaceId,
        meetingId,
      );
      ...
      final transcript = formatMeetingTranscript(
        relabeled,
      ...
      return StepResult.ok(
        mutatedState: {
          ...
          'meeting_diarized': true,
          'meeting_speaker_count': assigned.length,
          ...
          'diarization_spans': encodeDiarizedSpans(spans),
        },
      );
    } catch (e, s) {
  ...
  registry.registerBody(BuiltInBodyKeys.meetingIdentifySpeakers, (ctx) async {
    final meetingId = ctx.optional<String>('meeting_id');
    if (meetingId == null || meetingId.isEmpty) {
      return StepResult.failed('meeting.identifySpeakers: missing meeting_id');
    }
    try {
      ...
      if (speakers.isEmpty) {
        return StepResult.ok(
          mutatedState: const {'meeting_speakers_recognized': 0},
        );
      }
      ...
      final updatedSpeakers = await meetingRepository.getSpeakers(
        ctx.workspaceId,
        meetingId,
      );
      final segments = await meetingRepository.getSegments(
      ...
      return StepResult.ok(
        mutatedState: {
          'transcript': transcript,
          'meeting_speakers_recognized': recognized,
        },
      );
    } catch (e, s) {
  ...
  registry.registerBody(BuiltInBodyKeys.meetingUpdateTranscript, (ctx) async {
    final meetingId = ctx.optional<String>('meeting_id');
    if (meetingId == null || meetingId.isEmpty) {
      return StepResult.failed('meeting.updateTranscript: missing meeting_id');
    }
    final diarized = ctx.state['meeting_diarized'] == true;
    final spansJson = ctx.optional<String>('diarization_spans');
    if (!diarized || spansJson == null || spansJson.isEmpty) {
      return StepResult.ok(
        mutatedState: const {'meeting_transcript_updated': false},
      );
    }
    try {
      ...
      return StepResult.ok(
        mutatedState: {
          'meeting_transcript_updated': true,
          'meeting_transcript_turns': separated.length,
        },
      );
    } catch (e, s) {
  ...
  // meeting.assemblePlayback — folds the retained per-space WAVs (me.wav +
  ...
  registry.registerBody(BuiltInBodyKeys.meetingAssemblePlayback, (ctx) async {
    final meetingId = ctx.optional<String>('meeting_id');
    if (meetingId == null || meetingId.isEmpty) {
      return StepResult.failed('meeting.assemblePlayback: missing meeting_id');
    }
    final meeting = await meetingRepository.getById(ctx.workspaceId, meetingId);
    if (meeting == null) {
      return StepResult.failed(
    ...
    if (dir == null || dir.isEmpty) {
      return StepResult.ok(mutatedState: const {'playback_assembled': false});
    }
    try {
      ...
      final sampleRate = me.samples.isNotEmpty
          ? me.sampleRate
          : them.sampleRate;
      await writeMonoWav(
        p.join(dir, 'mixed.wav'),
        mixed,
      ...
      return StepResult.ok(mutatedState: const {'playback_assembled': true});
    } catch (e, s) {
  ...
  registry.registerBody(BuiltInBodyKeys.meetingSaveNotes, (ctx) async {
    ...
    return StepResult.ok(mutatedState: const {'meeting_notes_saved': true});
  });
  ...
  registry.registerBody(BuiltInBodyKeys.meetingAddActionItems, (ctx) async {
    ...
    if (!outcome.isStructured) {
      return StepResult.ok(
        mutatedState: const {'meeting_action_items_skipped': true},
      );
    }
    final now = DateTime.now();
    ...
    return StepResult.ok(
      mutatedState: {'meeting_action_item_count': items.length},
    );
  });
  ...
  registry.registerBody(BuiltInBodyKeys.meetingAddDecisions, (ctx) async {
    ...
    if (!outcome.isStructured) {
      return StepResult.ok(
        mutatedState: const {'meeting_decisions_skipped': true},
      );
    }
    final now = DateTime.now();
    ...
    return StepResult.ok(
      mutatedState: {'meeting_decision_count': decisions.length},
    );
  });
}
