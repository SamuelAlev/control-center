...
/// [SubscriptionStatus.unconfigured] snapshot, a plan with nothing left yields
/// a [SubscriptionStatus.exhausted] one, a failed fetch yields a
/// [SubscriptionStatus.error] one and none of them throws — so one provider
...
class SubscriptionUsageService {
  ...
  Future<SubscriptionUsage> fetchClaudeForConfigDir(String configDir) =>
      _fetchClaude(configDir: configDir);

  Future<SubscriptionUsage> _fetchClaude({String? configDir}) async {
    const id = 'claude';
    const name = 'Claude';
    try {
      final token = await _readClaudeToken(configDir: configDir);
      ...
      return _ok(id, name, windows, spend: spend);
    } catch (e) {
  ...
  Future<String?> _readClaudeToken({String? configDir}) async {
    final home = _homeDir;
  ...
  Future<SubscriptionUsage> _fetchKimi({
    ...
    try {
      ...
      final exhausted = _exhaustionReason(e);
      if (exhausted != null) {
        return _exhausted(id, name, exhausted);
      }
      return _error(id, name, _short(e));
    } catch (e) {
      return _error(id, name, _short(e));
  ...
  SubscriptionUsage _exhausted(String id, String name, String reason) =>
      SubscriptionUsage(
        providerId: id,
        displayName: name,
