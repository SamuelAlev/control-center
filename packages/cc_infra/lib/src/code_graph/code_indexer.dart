...
class DefaultCodeIndexer implements CodeIndexer {
  ...
  Future<CodeIndexResult> indexRepo({
    ...
    // What this partition already knows, so unchanged files cost a stat rather
    // than a full read + SHA-256 — the difference between an unchanged checkout
    // costing a directory walk and costing every byte of every file.
    //
    // Enumeration + hashing itself runs on its own isolate; inline it would
    // stall the server's event loop for the whole run.
    final knownStates = await _repository.fileStates(
      workspaceId,
      repoId,
      checkoutId: checkoutId,
    );
    final files = await _walker.walkAndHash(
      repoPath,
      known: {
