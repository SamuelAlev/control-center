...
class CodeExtractor {
  ...
  ///
  /// Throws [TreeSitterUnavailable] when the natives can't load — a broken
  /// install must fail the caller, not extract "nothing" silently.
  ExtractionResult extract({
    ...
    final matches = parser.parseMatches(
  ...
  List<CodeEdge> _buildEdges(
    ...
    final fileNodeId = codeFileNodeId(
      workspaceId,
      repoId,
      filePath,
      checkoutId: checkoutId,
    ...
    for (final match in matches) {
      for (final capture in match) {
        switch (capture.name) {
          ...
          case 'import.uri':
            final uri = _cleanUri(capture.text);
            // An import of an external package can never bind to a symbol in
            // this repo, and an unresolved edge is invisible to every graph
            // query — persisting it is pure write amplification (row + 8 index
            // entries, re-scanned by the resolver on every run). Measured on a
            // real index, `react`/`vitest`-style package imports were 30% of
            // ALL edge rows.
            if (_isExternalModuleSpecifier(uri, languageId)) {
              continue;
            }
            addEdge(
              source: fileNodeId,
