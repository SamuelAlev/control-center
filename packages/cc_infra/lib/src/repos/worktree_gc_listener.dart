  ...
  void start() {
    ...
    _subs.add(
      _eventBus.on<ChannelDeleted>().listen((e) {
        // `ChannelDeleted.workspaceId` is required and non-null now, so the
        // cross-workspace fallback this used to take when it was absent
        // (`releaseConversationAnyWorkspace`, a scan of every workspace file)
        ...
        if (workspaceId.isEmpty) {
          CcInfraLog.warning(
            'worktree gc: ChannelDeleted for ${e.channelId} carries an empty '
