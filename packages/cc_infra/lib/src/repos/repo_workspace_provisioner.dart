...
class RepoWorkspaceProvisioner implements RepoWorkspaceProvisionerPort {
  ...
  RepoWorkspaceProvisioner({
    ...
    required Future<String> Function() mcpConfigPath,
  ...
  /// Resolves the absolute path to the source `.mcp.json` to symlink into each
  /// provisioned conversation workspace. Injected (rather than reading the
  /// app-support dir directly) so this service stays Flutter-free.
  final Future<String> Function() _mcpConfigPath;
  ...
  Future<String> ensureConversationWorkspace({
    ...
    try {
      ...
      await _ensureConfigLinks(convRoot, agentConfigDir);
  ...
  Future<void> _ensureConfigLinks(
    ...
    final mcpPath = await _mcpConfigPath();
    if (File(mcpPath).existsSync()) {
      await _ensureSymlink(p.join(convRootPath, '.mcp.json'), mcpPath);
    }
