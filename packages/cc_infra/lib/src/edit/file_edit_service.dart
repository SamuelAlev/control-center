...
class FileEditResult {
  ...
  const FileEditResult.failure({required this.failures, required this.heldBack})
    : applied = false,
      changes = const [];
  ...
  String get summary {
    if (applied) {
      ...
      final parts = changes.map((c) {
        ...
        final warn = c.warnings.isEmpty ? '' : ' (${c.warnings.join('; ')})';
...
class FileEditService implements SnapshotStore {
  ...
  /// anchored to.
  ///
  /// Bounded on both axes — a long agent session touches many files, each
  /// repeatedly, and every entry is a full copy of the file's text. Only the
  /// [_maxHashesPerPath] most recent versions per path and the
  /// [_maxCachedPaths] most recently touched paths are retained (LRU). An
  /// edit anchored to an evicted version fails its hash check and recovers
  ...
  void _cache(String path, String content) {
    ...
    // Re-insert to mark the path most-recently-used.
    final byHash = _snapshotCache.remove(key) ?? {};
    _snapshotCache[key] = byHash;
    while (_snapshotCache.length > _maxCachedPaths) {
      _snapshotCache.remove(_snapshotCache.keys.first);
    }
    final hash = computeContentHash(normalized);
    byHash.remove(hash);
    byHash[hash] = normalized;
    while (byHash.length > _maxHashesPerPath) {
      byHash.remove(byHash.keys.first);
    }
  }
