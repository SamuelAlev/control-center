...
class ApproveOrchestrationUseCase {
  ...
  ///
  /// [approvedNodeKeys] narrows approval to a subtree (PRD 17 §4): the whole
  /// plan still materializes, but unapproved nodes sit behind suspended
  /// approval gates until [approveNodes] releases them. Null approves every
  /// node. The set must be dependency-closed — an approved node whose
  /// upstream is deferred could never start, so that shape is rejected.
  Future<void> approve({
    ...
    if (o.status == OrchestrationStatus.proposed) {
      if (approvedNodeKeys != null) {
        _assertValidNodeSet(o, approvedNodeKeys);
      }
      o = o.copyWith(
        status: OrchestrationStatus.approved,
        approvedRevision: o.revision,
        approvedNodeKeys: approvedNodeKeys?.toList(),
    ...
    try {
      ...
      CcInfraLog.error('ApproveOrchestration: materialization failed', e, st);
      await _fail(workspaceId, o, 'Materialization failed: $e');
      rethrow;
    }
  }

  /// Approves additional nodes of an already-executing partial approval
  /// (PRD 17 §4): widens the stored approved set, then resumes each newly
  /// approved node's suspended gate step. Approval is always an explicit act
  /// — completing an approved subtree never auto-starts deferred siblings.
  Future<void> approveNodes({
    required String workspaceId,
    required String orchestrationId,
    required Set<String> nodeKeys,
  }) async {
    final o = await _orchestrations.getById(workspaceId, orchestrationId);
    if (o == null) {
      throw StateError('Orchestration $orchestrationId not found');
    }
    final current = o.approvedNodeKeys;
    if (current == null) {
      // Everything was already approved — nothing to widen.
      return;
    }
    final union = {...current, ...nodeKeys};
    _assertValidNodeSet(o, union);
    final newlyApproved = nodeKeys.difference(current.toSet());
    if (newlyApproved.isEmpty) {
      return;
    }
    final allKeys = o.proposal.subTickets.map((t) => t.key).toSet();
    await _orchestrations.update(
      o.copyWith(
        approvedNodeKeys: union.length == allKeys.length
            ? null
            : union.toList(),
        clearApprovedNodeKeys: union.length == allKeys.length,
        updatedAt: DateTime.now(),
      ),
    );
    // Release the gates. The gate body re-reads the approved set on any
    // crash-recovery re-run, so both paths agree.
    final runId = o.pipelineRunId;
    if (runId != null) {
      for (final key in newlyApproved) {
        await _engine.resumeStep(pipelineRunId: runId, stepId: 'gate_$key');
      }
    }
  }

  /// Rejects an approved-node set that is not dependency-closed or names
  /// unknown nodes.
  void _assertValidNodeSet(Orchestration o, Set<String> nodeKeys) {
    final byKey = {for (final t in o.proposal.subTickets) t.key: t};
    for (final key in nodeKeys) {
      final ticket = byKey[key];
      if (ticket == null) {
        throw ArgumentError('Unknown plan node: $key');
      }
      for (final dep in ticket.dependsOn) {
        if (!nodeKeys.contains(dep)) {
          throw ArgumentError(
            'Node $key depends on $dep, which is not in the approved set — '
            'approve the dependency first (approval must be '
            'dependency-closed).',
          );
        }
      }
  ...
  Future<Orchestration> _materialize(
    String workspaceId,
    Orchestration o,
  ) async {
