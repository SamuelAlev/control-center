...
/// on [close]. Used to retain a meeting's per-space audio for the offline
/// diarization step.
///
...
class WavStreamWriter {
  WavStreamWriter._(this._raf, this.sampleRate, this.spaces);
  ...
  /// Space count of the written audio.
  final int spaces;
  ...
  static Future<WavStreamWriter> create(
    ...
    final writer = WavStreamWriter._(raf, sampleRate, spaces);
  ...
  Uint8List _header(int dataBytes) {
    final byteRate = sampleRate * spaces * 2;
    final blockAlign = spaces * 2;
...
/// the file is multi-space it averages spaces to mono. Returns empty data
...
Future<WavData> readWavToFloat32(String path) async {
  ...
  while (pos + 8 <= bytes.length) {
    ...
    if (chunkId == 'fmt ' && body + 16 <= bytes.length) {
      spaces = view.getUint16(body + 2, Endian.little);
  ...
  if (dataOffset < 0 || bitsPerSample != 16 || spaces < 1) {
  ...
  final frames = totalSamples ~/ spaces;
  final out = Float32List(frames);
  for (var f = 0; f < frames; f++) {
    var sum = 0.0;
    for (var c = 0; c < spaces; c++) {
      final s = view.getInt16(dataOffset + (f * spaces + c) * 2, Endian.little);
      sum += s / 32768.0;
    }
    out[f] = sum / spaces;
  }
...
/// Writes [samples] (normalized `[-1, 1]`) to [path] as a mono 16-bit PCM WAV
/// in a single pass. Used to materialize the mixed playback track from a
/// meeting's retained per-space WAVs. Out-of-range samples are clamped.
Future<void> writeMonoWav(
  String path,
  Float32List samples, {
  int sampleRate = 16000,
}) async {
  ...
  final body = ByteData(44 + dataBytes);
  // RIFF / WAVE header.
  _putAscii(body, 0, 'RIFF');
  body.setUint32(4, 36 + dataBytes, Endian.little);
  _putAscii(body, 8, 'WAVE');
  _putAscii(body, 12, 'fmt ');
  body.setUint32(16, 16, Endian.little); // fmt chunk size
  body.setUint16(20, 1, Endian.little); // PCM
  body.setUint16(22, 1, Endian.little); // mono
  body.setUint32(24, sampleRate, Endian.little);
  body.setUint32(28, sampleRate * 2, Endian.little); // byte rate
  body.setUint16(32, 2, Endian.little); // block align
  body.setUint16(34, 16, Endian.little); // bits per sample
  _putAscii(body, 36, 'data');
  body.setUint32(40, dataBytes, Endian.little);
  for (var i = 0; i < samples.length; i++) {
    ...
    body.setInt16(44 + i * 2, (s * 32767).round(), Endian.little);
  }
  await file.writeAsBytes(body.buffer.asUint8List(), flush: true);
}

void _putAscii(ByteData data, int offset, String s) {
