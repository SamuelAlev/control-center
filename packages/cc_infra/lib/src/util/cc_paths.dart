...
class CcPaths {
  ...
  /// `<root>/review_snapshots/<prNodeId>/<componentKey>/`. Content-keyed PNG
  /// renders (base/head/diff) live here and are served via `/proxy/media`.
  Future<Directory> reviewSnapshotDir(String prNodeId, String componentKey) =>
      _ensure(
        p.join(
          appSupportRoot,
          'review_snapshots',
          _safeSegment(prNodeId),
