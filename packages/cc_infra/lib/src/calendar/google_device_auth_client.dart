...
class GoogleServerCredentials {
  ...
  static GoogleServerCredentials? fromJson(Map<String, dynamic> json) {
    ...
    return GoogleServerCredentials(
      ...
      usesBuiltinClient: json['usesBuiltinClient'] as bool? ?? false,
    );
  }
  ...
  /// client). Lives only in the server's credential store and is empty when
  /// [usesBuiltinClient] — see [asBuiltinClient].
  final String clientSecret;

  /// Whether this account was connected with the SERVER's own OAuth client
  /// rather than a user-supplied one, in which case [clientId]/[clientSecret]
  /// are empty and the refresh path resolves the pair from the server's current
  /// configuration.
  ///
  /// A marker rather than a stored copy for two reasons: the credential file
  /// never holds the built-in secret and rotating that secret (by shipping a
  /// new build) takes effect on the next refresh instead of stranding every
  /// account on whatever was current when it connected.
  ...
  /// The form to persist when the server's own OAuth client authorized this
  /// account: the literal pair is dropped in favour of the marker.
  GoogleServerCredentials asBuiltinClient() => GoogleServerCredentials(
    accessToken: accessToken,
    refreshToken: refreshToken,
    expiresAt: expiresAt,
...
class GoogleDeviceAuthClient {
  ...
  Future<GoogleDeviceCode> requestDeviceCode({
    ...
    try {
      ...
      return _deviceCodeFrom(_asMap(response.data));
    } on DioException catch (e) {
      throw _mapDeviceError(e, 'request a device code');
    }
  }

  /// Parses a device-code response, whichever transport produced it.
  GoogleDeviceCode _deviceCodeFrom(Map<String, dynamic> body) {
  ...
  static Map<String, dynamic> _asMap(Object? data) {
    ...
    if (data is String && data.isNotEmpty) {
      try {
        ...
        // A malformed token-endpoint body is abnormal and previously masked
        // sync-stops; fall back to empty but record why.
        CcInfraLog.warning('google_device_auth: unparseable response body: $e');
      }
    }
