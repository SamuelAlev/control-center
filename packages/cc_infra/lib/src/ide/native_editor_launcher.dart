...
///
/// Wired on `cc_server` so `ide.detectEditors` / `ide.openPrInEditor` run on
/// the machine that owns the checkout. Detection never spawns subprocesses —
/// it only checks well-known absolute install locations (and `PATH`
/// directories, when present) for existence — so it is fast and works under
/// the stripped `PATH` of a packaged desktop app. Launching uses the
/// platform's native opener (`open` on macOS, `explorer` on Windows,
/// `xdg-open` on Linux) or the editor's resolved binary directly.
  ...
  List<_CatalogEntry> _windowsCatalog() {
    ...
    return [
      ...
      exe(
        'cursor',
        'Cursor',
        join([localAppData], r'Programs\cursor\Cursor.exe'),
      ),
      exe('zed', 'Zed', [
      ...
      exe('intellij', 'IntelliJ IDEA', [
        ...join([
          localAppData,
        ], r'Programs\IntelliJ IDEA Ultimate\bin\idea64.exe'),
        ...join([
          localAppData,
