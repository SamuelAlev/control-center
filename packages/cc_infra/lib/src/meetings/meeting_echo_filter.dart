...
typedef OnAccepted =
    Future<void> Function(MeetingSpeaker speaker, TranscribedWindow window);
...
class MeetingEchoFilter {
  ...
  /// (protects backspaces like "okay" / "yeah").
  final int minTokens;
  ...
  void _acceptThem(EchoCandidate candidate, Set<String> tokens) {
    ...
    _commit(candidate);
    final buffered = _BufferedWindow(
      candidate.window,
      candidate.emitMs,
      tokens,
    );
  ...
  Future<void> drain() async {
    ...
    for (final p in pending) {
      ...
      try {
        ...
        CcInfraLog.warning('MeetingEchoFilter: drain commit failed: $e');
      }
    }
    _recentThem.clear();
  }
  ...
  void _commit(EchoCandidate candidate) {
    unawaited(
      Future<void>(
        () => _onAccepted(candidate.speaker, candidate.window),
      ).catchError((Object e, StackTrace s) {
        CcInfraLog.warning('MeetingEchoFilter: persist failed: $e');
      }),
    );
  }
  ...
  void _logDrop(EchoCandidate me, _BufferedWindow them, double sim) {
    CcInfraLog.info(
      'MeetingCapture: dropped echo of them (sim ${sim.toStringAsFixed(2)}): '
