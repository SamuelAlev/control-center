...
class MeetingRecordingService {
  ...
  /// Invoked after the LAST live session drains and closes. The host uses it
  /// to release the loaded ASR model (hundreds of MB) between recordings; the
  /// transcriber reloads lazily when the next session's first window decodes.
...
class _RecordingSession {
  ...
  void start() {
    ...
    _meTx = _transcription
        .transcribe(_me.stream)
        .listen(
    ...
    _themTx = _transcription
        .transcribe(_them.stream)
        .listen(
