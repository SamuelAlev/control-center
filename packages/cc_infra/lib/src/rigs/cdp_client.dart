...
class CdpException implements Exception {
  ...
  const CdpException(this.message, {this.code});

  /// What Chromium said.
  final String message;

  /// Its numeric error code, when it supplied one.
  final int? code;
...
class CdpClient {
  ...
  Future<void> setViewport({
    ...
    final viewport = _CdpViewport(width: width, height: height, mobile: mobile);
    _viewport = viewport;
...
''';
  ...
  void _handleDrop(String reason) {
    if (_state != CdpConnectionState.connected) {
      ...
      // itself through more than one channel and only the first is news.
      return;
    }
...
class CdpScreencastFrame {
  /// Creates a [CdpScreencastFrame].
  const CdpScreencastFrame({required this.bytes, required this.sessionId});

  /// The JPEG bytes, already base64-decoded.
  final Uint8List bytes;

  /// Chromium's frame session id, or -1 when the frame carried none. Acking it
  /// is what releases the next frame, so it doubles as the backpressure token.
  final int sessionId;
}
