...
///
/// Owns the process, the overlay and the control channels. It deliberately
...
class QemuEnclosureBackend {
  ...
  ///
  /// Teardown steps are independent by design: a channel that will not close
  ...
  /// mode, and the directories gate a control channel that can stop the VM.
  Future<void> _chmod(String mode, String path) async {
    if (Platform.isWindows) {
      // There is no POSIX mode to set, and no rig boots here anyway: the
      // control channel is a unix socket. Silently skipping is honest here in
