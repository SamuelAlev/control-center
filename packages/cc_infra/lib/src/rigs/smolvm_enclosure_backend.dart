...
String smolvmPackFileName(String image) =>
    'pack-${sha256.convert(utf8.encode(image)).toString().substring(0, 16)}'
    '.smolmachine';
...
List<String> buildSmolvmBrowserWorkload(
  ...
  // The welcome page travels base64'd so nothing in its HTML is at the mercy
  // of shell quoting, then is decoded to a guest file the browser loads over
  // file:// — no egress, no DNS, always renders. The old default pointed at
  // an external CDN-fronted site, which the deny-by-default egress gate
  // silently refused (rotated CDN IP), leaving a "Ready" rig showing a blank
  // white page.
  final homeB64 = base64Encode(utf8.encode(kBrowserRigHomeHtml));
  // One string, built outside the argv list: adjacent string literals inside
  // a list literal are a lint here.
  final script =
      'echo $homeB64 | base64 -d > $kBrowserRigHomePath; '
      ...
      // channel into the rig, handed out for a check we do not need.
      '--no-sandbox '
...
List<String> buildSmolvmCreateArgs(SmolvmLaunchPlan plan) {
  ...
  return [
    ...
    if (isExec) ...['--storage', '8', '--overlay', '16'],
    ...
    // Exec guests bootstrap git/curl on first start. Browser guests boot a
    // baked image: installing anything at runtime would re-introduce the
    // first-start race the pinned image exists to remove.
    if (isExec) ...['--init', kSmolvmExecInit],
    if (!isExec) ...[
      '--',
      ...buildSmolvmBrowserWorkload(
        plan.spec.display,
...
class SmolvmMachine implements RigMachine {
  ...
  /// Host loopback port forwarded to the guest's DevTools relay, when this is
  /// a browser rig.
...
class SmolvmEnclosureBackend {
  ...
  Future<RigBackendCapabilities> _probeUncached() async {
    ...
    return RigBackendCapabilities(
      ...
      note:
          ...
          'boot, digest-pinned OCI images that pull on first use.',
      version: version,
    );
  }
  ...
  Future<SmolvmMachine> launch({
    ...
    final defaultImage = isExec ? kSmolvmExecImage : kSmolvmBrowserImage;
    ...
    final packPath = packPathFor(image);
    ...
    try {
      ...
      if (!File(packPath).existsSync()) {
        unawaited(_buildPack(binary, image, warmExec: isExec));
      }
      ...
      if (isExec) {
        ...
        await _awaitDevtools(devtoolsPort!, onProgress: onProgress);
      }
  ...
  String packPathFor(String image) =>
      p.join(_packsDir, smolvmPackFileName(image));
  ...
  Future<void> _buildPack(
    String binary,
    String image, {
    required bool warmExec,
  }) async {
    final destination = packPathFor(image);
    if (!_packsBuilding.add(image)) {
      return;
    }
    try {
      ...
      if (warmExec) {
        final template = await _buildWarmExecTemplate(binary, image);
        if (template == null) {
          return;
        }
        try {
      ...
      CcInfraLog.info(
        ...
        '${warmExec ? ' and the first-start package install' : ''}',
      );
    } on Object catch (e) {
      CcInfraLog.warning('rig/smolvm: pack build for $image failed: $e');
    } finally {
      _packsBuilding.remove(image);
    }
  }

  /// Boots a pristine template of the exec image, lets its init warm the
  /// overlay (git/curl/socat), verifies the warm actually took, and returns
  /// the STOPPED machine's name ready to snapshot — or null on any failure.
  ///
  ...
  Future<String?> _buildWarmExecTemplate(String binary, String image) async {
    ...
    try {
      ...
      if (check.exitCode != 0) {
        CcInfraLog.warning(
          'rig/smolvm: exec template never warmed (are the apt mirrors '
          'reachable?); keeping the plain-boot path',
        );
        await _deleteQuietly(binary, template);
        return null;
      }
      ...
      CcInfraLog.warning('rig/smolvm: exec template build failed: $e');
      await _deleteQuietly(binary, template);
      return null;
    }
  }
  ...
  /// helper all ride the same channel.
  ...
  /// Waits until the browser's DevTools endpoint answers on the forwarded
  /// host port.
  ///
  /// The first start pulls the image before the workload launches, so this
  /// is the long pole of a cold browser rig and the timeout is sized for it.
  /// The guest's socat relay accepts before Chromium listens and drops each
  /// such connection on its own, which reads here as "not up yet".
  Future<void> _awaitDevtools(
    int port, {
    void Function(String step)? onProgress,
    Duration timeout = const Duration(seconds: 180),
  }) async {
    final deadline = DateTime.now().add(timeout);
    onProgress?.call('Waiting for the browser');
    final client = HttpClient()..connectionTimeout = const Duration(seconds: 3);
    try {
      while (DateTime.now().isBefore(deadline)) {
        try {
          // Per-ATTEMPT deadline, not just a connection timeout. The guest's
          // socat relay accepts before Chromium listens, so a stalled relay
          // gives a connected socket that never answers — `connectionTimeout`
          // has already been satisfied by then and the loop would wait on
          // that one request forever, never reaching its own deadline.
          final response = await client
              .get('127.0.0.1', port, '/json/version')
    ...
    throw RigLaunchException(
      'The browser did not come up within ${timeout.inSeconds}s.',
    );
  }
