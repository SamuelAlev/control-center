...
///
/// Exposed so a watchdog can tell "QEMU is gone" from "the control channel
...
class QmpClient {
  ...
  /// same way every other command does when the channel is gone — which is
  ...
  Future<void> clickWithModifiers(
    ...
    try {
      ...
      try {
        ...
        // report a broken control channel as the reason the click did not
        // land. The channel is already gone, so the modifier is moot.
  ...
  Future<void> _scrollHorizontalWithShift(
    ...
    try {
      ...
      try {
        ...
        // is the actionable one, and a stuck shift on a dead channel is moot.
  ...
  void _handleDeath(String reason) {
    if (_state != QmpConnectionState.connected) {
      ...
      // through several channels (done, onDone, onError) and only the first
      // one is news.
      return;
    }
