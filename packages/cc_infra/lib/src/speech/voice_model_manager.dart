...
class VoiceModelManager {
  ...
  Future<VoiceModelPaths> install({
    ...
    // Extract into a STAGING directory and rename the unpacked tree into place,
    // rather than writing the archive's entries straight into `root`. Here
    // extraction is the last step, so its per-entry writes are what [resolve]
    // observes: a process killed part-way through could leave every file it
    // checks present with the last one truncated, and every later boot would
    // report the model installed. Renaming one directory is atomic, so the
    // model's own folder either does not exist or holds a complete extraction.
    onProgress?.call(0.86, 'extracting');
    final staging = Directory(p.join(root.path, '.${model.id}.extract'));
    if (staging.existsSync()) {
      await staging.delete(recursive: true);
    }
    await _extract(tmpArchive, staging, onProgress);

    if (tmpArchive.existsSync()) {
      await tmpArchive.delete();
    }

    final unpacked = Directory(p.join(staging.path, model.unpackedDirName));
    if (unpacked.existsSync()) {
      final destination = await _modelDir();
      // Anything already at the destination failed the `resolve` above, so it
      // is an incomplete leftover; a non-empty directory would refuse a rename.
      if (destination.existsSync()) {
        await destination.delete(recursive: true);
      }
      await unpacked.rename(destination.path);
    }
    if (staging.existsSync()) {
      await staging.delete(recursive: true);
    }
