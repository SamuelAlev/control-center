...
class DiarizationModelInfo {
  ...
  static const pyannoteWespeaker = DiarizationModelInfo(
    ...
    segmentationModelFile: 'sherpa-onnx-pyannote-segmentation-3-0/model.onnx',
...
class DiarizationModelManager {
  ...
  Future<DiarizationModelPaths> install({
    ...
    final dir = await _modelDir();
    final tmpArchive = File(p.join(dir.path, 'segmentation.tar.bz2.part'));
    final embeddingPath = p.join(dir.path, model.embeddingFile);
    // The embedding model is the LAST artifact [resolve] looks for, so it is
    // what decides whether this install counts as finished. Transfer it into a
    // `.part` sibling and rename it into place: written straight to its final
    // path, a process killed mid-download left a truncated `.onnx` that every
    // later boot accepted as installed (existence is the only check). The
    // segmentation half needs no such guard — it is extracted BEFORE this
    // download, so a kill during extraction leaves no embedding file, `resolve`
    // returns null and the next attempt re-extracts over it.
    final embeddingPart = File('$embeddingPath.part');
    ...
    try {
      await _dio.download(
        ...
        onReceiveProgress: (received, total) {
          final expected = total > 0 ? total : model.segmentationArchiveBytes;
      ...
      await embeddingPart.rename(embeddingPath);
    } catch (e) {
      // Clean up partials so the next attempt starts fresh.
      for (final f in [tmpArchive, embeddingPart]) {
        if (f.existsSync()) {
          await f.delete();
        }
      }
