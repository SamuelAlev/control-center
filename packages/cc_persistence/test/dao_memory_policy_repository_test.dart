...
void main() {
  ...
  setUp(() async {
    ...
    repo = DaoMemoryPolicyRepository(dbs);
  });
  ...
  group('getByWorkspace', () {
    test('filters by workspace', () async {
      await repo.upsert(
        makePolicy(id: 'p-1', workspaceId: 'ws-1', domain: 'a'),
      );
      await repo.upsert(
        makePolicy(id: 'p-2', workspaceId: 'ws-1', domain: 'b'),
      );
      await repo.upsert(
        makePolicy(id: 'p-3', workspaceId: 'ws-2', domain: 'c'),
      );
  ...
  group('getActiveByWorkspace', () {
    ...
    test('filters by domain when provided', () async {
      ...
      final codebase = await repo.getActiveByWorkspace(
        'ws-1',
        domain: 'codebase',
      );
