  ...
  final steps = [
    ...
    PipelineStepDefinition(
      ...
      config: const PipelineNodeConfig(
        agentId: 'librarian',
        // A step that dispatches agents must name the conversation it works
        // in; a literal room id satisfies validation (a template reference
        // would need a `messaging.createSpace` provider in this fixture).
        extras: {'spaceId': 'space-1'},
      ),
      triggers: [
    ...
    PipelineStepDefinition(
      ...
      bodyKey: '_terminal',
      triggers: [
        const StepTrigger(sourceStepIds: ['step1']),
      ],
    ),
  ];
...
void main() {
  ...
  group('upsert', () {
    ...
    test('throws validation error on invalid non-built-in template', () async {
      ...
      expect(() => repo.upsert(invalidDef), throwsA(isA<Exception>()));
    });
