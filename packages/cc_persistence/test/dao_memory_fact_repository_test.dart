...
void main() {
  ...
  setUp(() async {
    ...
    repo = DaoMemoryFactRepository(dbs);
  });
  ...
  group('CRUD', () {
    ...
    test(
      'getByWorkspace returns all facts in workspace, newest first',
      () async {
    ...
    test(
      'delete is idempotent — deleting nonexistent fact does not throw',
      () async {
  ...
  group('workspace scoping', () {
    ...
    test(
      'watchByWorkspace emits only facts from the requested workspace',
      () async {
        await repo.upsert(makeFact(id: 'f-1', workspaceId: 'ws-1'));
        await repo.upsert(makeFact(id: 'f-2', workspaceId: 'ws-2'));
  ...
  group('search', () {
    test('FTS search finds facts by topic or content', () async {
      await repo.upsert(
        makeFact(id: 'f-1', topic: 'deploy', content: 'deployment runbook'),
      );
      await repo.upsert(
        makeFact(id: 'f-2', topic: 'testing', content: 'unit test strategy'),
      );
    ...
    test('FTS search returns empty for no match', () async {
      await repo.upsert(
        makeFact(id: 'f-1', topic: 'deploy', content: 'runbook'),
      );
    ...
    test('hybrid search with query embedding uses vector path', () async {
      await repo.upsert(
        makeFact(
          id: 'f-1',
          topic: 'testing',
          content: 'integration test strategy',
        ),
      );
  ...
  group('filtering', () {
    test(
      'getActiveByTopic returns only non-superseded facts for topic',
      () async {
        await repo.upsert(makeFact(id: 'f-1', topic: 'deploy'));
        await repo.upsert(
          makeFact(id: 'f-2', topic: 'deploy', supersededBy: 'f-x'),
        );
    ...
    test('getActiveByTopic scopes to workspace', () async {
      await repo.upsert(
        makeFact(id: 'f-1', workspaceId: 'ws-1', topic: 'deploy'),
      );
      await repo.upsert(
        makeFact(id: 'f-2', workspaceId: 'ws-2', topic: 'deploy'),
      );
    ...
    test('getByAuthor scopes to workspace', () async {
      await repo.upsert(
        makeFact(id: 'f-1', workspaceId: 'ws-1', authoredByAgentId: 'agent-a'),
      );
      await repo.upsert(
        makeFact(id: 'f-2', workspaceId: 'ws-2', authoredByAgentId: 'agent-a'),
      );
  ...
  group('edge cases', () {
    ...
    test('sourceObservationIds with values round-trips correctly', () async {
      await repo.upsert(
        makeFact(id: 'f-obs', sourceObservationIds: ['obs-a', 'obs-b']),
      );
