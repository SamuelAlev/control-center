...
void main() {
  ...
  group('createChannel', () {
    ...
    test('records a scoped repo selection for provisioning', () async {
      ...
      expect(
        await db.channelRepoDao.repoIdsForChannel('ws-1', channel.id),
        ['r-1'],
      );
    });

    test('refuses a repo id that is not registered in the workspace', () async {
      // A foreign (or stale) repo id must never land in channel_repos: the
      // provisioner would clone a repo this workspace does not own.
      await expectLater(
        repo.createChannel('ws-1', 'Foreign', [], repoIds: const ['r-x']),
