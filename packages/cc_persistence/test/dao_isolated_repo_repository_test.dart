...
void main() {
  ...
  group('DaoIsolatedRepoRepository', () {
    ...
    test(
      'forChannel returns matching repos',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'forChannel returns empty for non-matching workspace',
      timeout: const Timeout.factor(2),
      () async {
        await repository.upsert(
          _makeIsolatedRepo(
    ...
    test(
      'forChannelAcrossWorkspaces returns repos across all workspaces',
      timeout: const Timeout.factor(2),
      () async {
        await repository.upsert(
          _makeIsolatedRepo(
        ...
        final results = await repository.forChannelAcrossWorkspaces(
          'shared-ch',
        );
        expect(results.length, 2);
      },
