...
void main() {
  ...
  setUp(() async {
    ...
    await seedTestWorkspace(global, dbs, 'w1');
    await seedTestWorkspace(global, dbs, 'w2');
    repo = DaoVoiceProfileRepository(dbs);
  });
  ...
  group('DaoVoiceProfileRepository.enroll', () {
    test(
      'creates a new profile on first enrollment (round-trips embedding)',
      () async {
        await repo.enroll(
          workspaceId: 'w1',
          displayName: 'Alex',
    ...
    test(
      'blends a second sample into the centroid and increments the count',
      () async {
        await repo.enroll(
          workspaceId: 'w1',
          displayName: 'Alex',
    ...
    test(
      'is workspace-scoped: an enrollment never leaks to another workspace',
      () async {
        await repo.enroll(
          workspaceId: 'w1',
          displayName: 'Alex',
  ...
  group('DaoVoiceProfileRepository.unenroll', () {
    ...
    test(
      'backs a sample out of a two-sample centroid and decrements the count',
      () async {
        await repo.enroll(
          workspaceId: 'w1',
          displayName: 'Alex',
    ...
    test(
      'is workspace-scoped: cannot un-enroll a foreign workspace profile',
      () async {
        await repo.enroll(
          workspaceId: 'w1',
          displayName: 'Alex',
