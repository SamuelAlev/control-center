...
void main() {
  ...
  setUp(() async {
    ...
    // Seed workspaces + channels the todos FK-reference.
  ...
  group('TodoDao workspace + conversation isolation', () {
    test('watchForConversation returns only same (ws, conv) rows', () async {
    ...
    test('replaceAll clears + reinserts scoped to (ws, conv)', () async {
      await insertTodo('t-1', 'w-1', 'c-1');
