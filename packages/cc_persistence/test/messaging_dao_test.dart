...
void main() {
  ...
  group('MessagingDao — channels', () {
    test('insert and watch channels', () async {
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-2', name: 'random'),
      );
    ...
    test('updateChannelUpdatedAt sets timestamp', () async {
      ...
      final channels = await db.messagingDao.watchChannels().first;
      expect(channels.first.id, 'ch-ts');
    });
    ...
    test('updateChannelName changes name', () async {
      ...
      final channels = await db.messagingDao.watchChannels().first;
      expect(channels.first.name, 'new-name');
    });
    ...
    test(
      ...
      () async {
        ...
        await db.messagingDao.updateChannelProvisioningStep(
          'ch-prov',
          '{"kind":"repo","subject":"api"}',
        );

        var row = (await db.messagingDao.watchChannels().first).first;
        expect(row.provisioningStep, '{"kind":"repo","subject":"api"}');

        await db.messagingDao.updateChannelProvisioningStatus(
          'ch-prov',
          'ready',
        );

        row = (await db.messagingDao.watchChannels().first).first;
        expect(row.provisioningStatus, 'ready');
        expect(row.provisioningStep, isNull);
      },
    );

    test('channelsByProvisioningStatus finds stranded channels', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(
          id: 'ch-stuck',
          name: 'stuck',
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-ok', name: 'ok'),
      );

      final stuck = await db.messagingDao.channelsByProvisioningStatus(
        'provisioning',
      );
    ...
    test(
      'deleteChannelCascade removes channel, messages, participants',
      () async {
  ...
  group('MessagingDao — participants', () {
    test('insert and watch participants', () async {
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-p', name: 'p'),
      );
      await db.messagingDao.insertParticipant(
        ChannelParticipantsTableCompanion.insert(
          id: 'cp-a',
    ...
    test('insertParticipant ignores duplicates', () async {
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-dup', name: 'dup'),
      );
      await db.messagingDao.insertParticipant(
        ChannelParticipantsTableCompanion.insert(
          id: 'cp-d',
    ...
    test('getParticipants returns all participants for channel', () async {
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-gp', name: 'gp'),
      );
      await db.messagingDao.insertParticipant(
        ChannelParticipantsTableCompanion.insert(
          id: 'cp-1',
    ...
    test('removeParticipant deletes participant', () async {
      ...
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-rm', name: 'rm'),
      );
      await db.messagingDao.insertParticipant(
        ChannelParticipantsTableCompanion.insert(
          id: 'cp-x',
  ...
  group('MessagingDao — messages', () {
    test('insert and watch messages', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-msg', name: 'msg'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-1',
    ...
    test('getMessages returns future list', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-get', name: 'get'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-a',
    ...
    test('markCompacted sets compacted flag', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-comp', name: 'comp'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-c1',
    ...
    test('clearChannelMessages removes all messages', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-clear', name: 'clear'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-x',
    ...
    test('updateMessage updates content', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-upd', name: 'upd'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-u',
    ...
    test('updateMessage updates metadata', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-meta', name: 'meta'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-m',
    ...
    test('updateMessage partially updates content only', () async {
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-part', name: 'part'),
      );
      await db.messagingDao.insertMessage(
        ChannelMessagesTableCompanion.insert(
          id: 'msg-p',
    ...
    test('messages inserted in the same second keep insertion order '
      ...
      // correct.
      await db.messagingDao.insertChannel(
        ChannelsTableCompanion.insert(id: 'ch-order', name: 'order'),
      );
