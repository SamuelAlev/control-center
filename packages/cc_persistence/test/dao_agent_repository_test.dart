...
void main() {
  ...
  setUp(() async {
    ...
    // `watchAll` merges one stream per REGISTERED workspace, so `ws-1` (the
    // workspace every agent in this file belongs to) has to be in the registry.
    await seedTestWorkspace(global, dbs, 'ws-1');
    dao = dbs.of('ws-1').agentDao;
    repo = DaoAgentRepository(dbs);
  });
  ...
  group('getById', () {
    test('returns agent when found', () async {
      ...
      final result = await repo.getById('ws-1', 'found');
  ...
  group('delete', () {
    test('deletes agent by id', () async {
      ...
      await repo.delete('ws-1', 'to-delete');
