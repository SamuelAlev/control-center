...
/// The feed owner most tests act as (the FK target).
const _user = 'user-1';

/// A second user for isolation assertions.
const _otherUser = 'user-2';

Future<void> _insertUser(GlobalDatabase db, String id) =>
    db.into(db.usersTable).insert(
      UsersTableCompanion.insert(id: id, handle: id, displayName: id),
    );
...
void main() {
  ...
  group('RssDao - Feeds', () {
    test('watchFeeds returns empty when no feeds', () async {
      final feeds = await db.rssDao.watchFeeds(_user).first;
    ...
    test('getEnabledFeeds returns only enabled feeds', () async {
      ...
      final enabled = await db.rssDao.getEnabledFeeds(_user);
    ...
    test('getFeedByUrl finds matching feed', () async {
      ...
      final found = await db.rssDao.getFeedByUrl(_user, 'https://example.com/rss');
      expect(found, isNotNull);
      expect(found!.name, 'My Feed');
    });

    test('getFeedByUrl returns null for unknown URL', () async {
      final found = await db.rssDao.getFeedByUrl(
        _user,
    ...
    test('getFeedById only finds the owning users feed', () async {
      await db.rssDao.upsertFeed(
        RssFeedsTableCompanion.insert(
          id: 'feed-1',
          userId: _user,
      ...
      expect(await db.rssDao.getFeedById(_user, 'feed-1'), isNotNull);
      expect(
        await db.rssDao.getFeedById(_otherUser, 'feed-1'),
        isNull,
        reason: 'a foreign feed id is indistinguishable from a missing one',
      );
    });
    ...
    test('updateFeedFetchResult updates lastFetchedAt', () async {
      ...
      await db.rssDao.updateFeedFetchResult(feedId: 'feed-1', fetchedAt: now);

      final feed = await db.rssDao.getFeedByUrl(_user, 'https://example.com/rss');
    ...
    test('setFeedEnabled toggles enabled state', () async {
      ...
      await db.rssDao.setFeedEnabled(_user, 'feed-1', enabled: false);
      final feed = await db.rssDao.getFeedByUrl(_user, 'https://example.com/rss');
      expect(feed!.enabled, isFalse);

      await db.rssDao.setFeedEnabled(_user, 'feed-1', enabled: true);
      final enabled = await db.rssDao.getEnabledFeeds(_user);
      expect(enabled.length, 1);
    });
    ...
    test('deleteFeed removes feed', () async {
      ...
      await db.rssDao.deleteFeed(_user, 'feed-1');
    ...
    test('deleteFeed returns affected row count', () async {
      ...
      final count = await db.rssDao.deleteFeed(_user, 'feed-1');
  ...
  group('RssDao - Articles', () {
    ...
    test('watchAllArticles returns empty when no articles', () async {
      final articles = await db.rssDao.watchAllArticles(_user).first;
      expect(articles, isEmpty);
    });

    test('watchSavedArticles returns empty when no saved', () async {
      final saved = await db.rssDao.watchSavedArticles(_user).first;
    ...
    test(
      'upsertArticlesFromFeed persists a batch in one transaction',
      () async {
        await db.rssDao.upsertArticlesFromFeed([
          RssArticlesTableCompanion.insert(
            id: 'art-1',
            feedId: feedId,
            title: 'One',
    ...
    test('upsertArticlesFromFeed revalidates existing rows by guid', () async {
      await db.rssDao.upsertArticleFromFeed(
        RssArticlesTableCompanion.insert(
          id: 'art-1',
          feedId: feedId,
      ...
      // Batch with a new id but the same guid must update, not duplicate.
      await db.rssDao.upsertArticlesFromFeed([
        RssArticlesTableCompanion.insert(
          id: 'art-2',
          feedId: feedId,
    ...
    test(
      'upsertArticleFromFeed refreshes content but keeps saved/read',
      () async {
        final entry = RssArticlesTableCompanion.insert(
          id: 'art-1',
          feedId: feedId,
        ...
        await db.rssDao.setArticleSaved(_user, 'art-1', saved: true);
        await db.rssDao.setArticleRead(_user, 'art-1', read: true);
        ...
        final article = await db.rssDao.getArticleById(_user, 'art-1');
    ...
    test('getArticleById returns null for unknown id', () async {
      final article = await db.rssDao.getArticleById(_user, 'nonexistent');
    ...
    test('setArticleSaved toggles saved state', () async {
      ...
      await db.rssDao.setArticleSaved(_user, 'art-1', saved: true);
      final savedArticles = await db.rssDao.watchSavedArticles(_user).first;
      expect(savedArticles.length, 1);

      await db.rssDao.setArticleSaved(_user, 'art-1', saved: false);
      final noSaved = await db.rssDao.watchSavedArticles(_user).first;
    ...
    test('markAllRead marks all articles as read', () async {
      ...
      await db.rssDao.markAllRead(_user);
  ...
  group('RssDao - user isolation', () {
    test('feeds, articles and flags never cross users', () async {
      // Both users subscribe to the same URL independently.
      await db.rssDao.upsertFeed(
        RssFeedsTableCompanion.insert(
          id: 'mine',
          userId: _user,
      ...
      expect(await db.rssDao.watchFeeds(_user).first, hasLength(1));
      expect(await db.rssDao.getFeedByUrl(_otherUser, 'https://shared.example.com/rss'), isNotNull);

      // Their article is invisible to me…
      expect(await db.rssDao.watchAllArticles(_user).first, isEmpty);
      expect(await db.rssDao.getArticleById(_user, 'their-art'), isNull);

      // …and my flag writes cannot reach it.
      await db.rssDao.setArticleSaved(_user, 'their-art', saved: true);
      expect(
        (await db.rssDao.getArticleById(_otherUser, 'their-art'))!.saved,
        isFalse,
      );

      // markAllRead is scoped by owner.
      await db.rssDao.markAllRead(_user);
      expect(
        (await db.rssDao.getArticleById(_otherUser, 'their-art'))!.read,
        isFalse,
      );

      // Deleting my copy leaves theirs alone.
      await db.rssDao.deleteFeed(_user, 'mine');
      expect(await db.rssDao.watchFeeds(_otherUser).first, hasLength(1));
    });
  });
}
