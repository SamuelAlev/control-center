...
void main() {
  ...
  group('CalendarDao.upsertEvents', () {
    ...
    test('preserves alertedAt across re-sync', () async {
      ...
      await db.calendarDao.markAlerted(
        'ws-A',
        'local-1',
        DateTime.utc(2026, 6, 11, 9, 55),
      );
      ...
      expect(
        rows.single.alertedAt!.isAtSameMomentAs(
          DateTime.utc(2026, 6, 11, 9, 55),
        ),
        isTrue,
      );
  ...
  group('CalendarDao.deleteEventsMissingFrom', () {
    ...
    test(
      'removes events not in the keep set, retains those that are',
      () async {
        final t = DateTime.utc(2026, 6, 11, 10);
        await db.calendarDao.upsertEvents([
          event(
        ...
        expect(removed, 1);
        final rows = await db.calendarDao
            .watchEventsInRange('ws-A', winFrom, winTo)
            .first;
    ...
    test(
      'does not touch other calendars or events outside the window',
      () async {
        ...
        final rows = await db.calendarDao
            ...
            .first;
        expect(rows.map((e) => e.externalEventId).toSet(), {
          'ext-team',
          'ext-later',
        });
      },
    ...
    test('is workspace-scoped', () async {
      ...
      final bRows = await db.calendarDao
          .watchEventsInRange('ws-B', winFrom, winTo)
          .first;
  ...
  group('CalendarDao workspace isolation', () {
    test(
      'watchEventsInRange never returns another workspace\'s events',
      () async {
        final t = DateTime.utc(2026, 6, 11, 10);
        await db.calendarDao.upsertEvents([
          event(
        ...
        final aRows = await db.calendarDao
            .watchEventsInRange(
              'ws-A',
              DateTime.utc(2026, 6, 1),
              DateTime.utc(2026, 7, 1),
    ...
    test('filters by window, alertedAt null and excludes all-day', () async {
      await db.calendarDao.upsertEvents([
        ...
        event(
          ...
          accountId: 'acc-A',
          externalEventId: '2',
          start: DateTime.utc(2026, 6, 11, 23),
        ),
  ...
  group('CalendarDao accounts + links', () {
    ...
    test('upsertAccount allows several accounts per workspace', () async {
      ...
      expect(accounts.map((a) => a.accountEmail).toSet(), {
        'a@x.com',
        'b@x.com',
      });
    });
  ...
  group('CalendarDao sources', () {
    CalendarSourcesTableCompanion source({
      required String id,
      required String workspaceId,
      required String accountId,
    ...
    }) => CalendarSourcesTableCompanion.insert(
      id: id,
      workspaceId: workspaceId,
      accountId: accountId,
      calendarId: calendarId,
    ...
    test(
      'replaceSources is a scoped replace-all for (workspace, account)',
      () async {
        await db.calendarDao.replaceSources('ws-A', 'acc-A', [
          source(
            id: 's1',
            workspaceId: 'ws-A',
            accountId: 'acc-A',
        ...
        var rows = await db.calendarDao.watchSources('ws-A', 'acc-A').first;
        expect(rows.map((s) => s.calendarId), [
          'primary',
          'team@group',
        ]); // primary first

        // A re-sync drops the now-absent calendar and keeps only the new set.
        await db.calendarDao.replaceSources('ws-A', 'acc-A', [
          source(
            id: 's3',
            workspaceId: 'ws-A',
        ...
        rows = await db.calendarDao.watchSources('ws-A', 'acc-A').first;
        expect(rows.map((s) => s.calendarId), ['family']);
      },
    );

    test('watchSources is scoped to (workspace, account)', () async {
      // Account ids are globally unique in production (they embed the
      // workspace), so each (workspace, account) pair uses a distinct id.
      await db.calendarDao.replaceSources('ws-A', 'acc-A', [
        source(
          id: 's1',
          workspaceId: 'ws-A',
      ...
      // Same workspace, different account — tests account scoping.
      await db.calendarDao.replaceSources('ws-A', 'acc-B', [
        source(
          id: 's2',
          workspaceId: 'ws-A',
          accountId: 'acc-B',
      ...
      // Different workspace, different account — must never leak into ws-A.
      await db.calendarDao.replaceSources('ws-B', 'acc-C', [
        source(
          id: 's3',
          workspaceId: 'ws-B',
          accountId: 'acc-C',
      ...
      expect(
        (await db.calendarDao.watchSources('ws-A', 'acc-A').first).map(
          (s) => s.summary,
        ),
        ['A primary'],
      );
      expect(
        (await db.calendarDao.watchSources('ws-A', 'acc-B').first).map(
          (s) => s.summary,
        ),
        ['B primary'],
      );
      // ws-B's source never streams into a ws-A watch.
      expect(
        (await db.calendarDao.watchSources('ws-A', 'acc-A').first).map(
          (s) => s.summary,
        ),
        isNot(contains('other workspace')),
      );
    });
  });
  ...
  group('CalendarDao.markNeedsReauth', () {
    ...
    test(
      'returns true on the null→set transition, false when already set',
      () async {
    ...
    test('is a no-op for a foreign workspace (isolation)', () async {
      ...
      expect(
        await db.calendarDao.markNeedsReauth('ws-B', 'acc-A', at),
        isFalse,
      );
    ...
    test('returns false for an absent account', () async {
      expect(await db.calendarDao.markNeedsReauth('ws-A', 'nope', at), isFalse);
    });

    test(
      'setLastSyncedAt clears the flag (successful sync = healthy)',
      () async {
        await seedAccount('ws-A', 'acc-A');
        await db.calendarDao.markNeedsReauth('ws-A', 'acc-A', at);
  ...
  group('CalendarDao.syncLinkedMeetingTitles', () {
    ...
    test(
      'is workspace-scoped (a foreign workspace is never touched)',
      () async {
        await db.calendarDao.upsertEvents([
          event(
            id: 'e1',
