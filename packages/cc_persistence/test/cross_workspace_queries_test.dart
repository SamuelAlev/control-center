...
void main() {
  ...
  group('fanOut', () {
    ...
    test('skips soft-deleted workspaces unless asked for them', () async {
      // A READ fan-out serves operator-facing all-workspace views, and opening
      // a removed workspace's file pays a cold open to contribute rows the
      // operator asked to stop seeing. Maintenance is the opposite case — the
      // file is still on disk, so retention and backup must still reach it —
      // hence the opt-in rather than a blanket rule.
      for (final id in const ['w-1', 'w-2', 'w-3']) {
        await global.workspaceRegistryDao.deleteWorkspace(id);
      }
      expect(await cross.fanOut((db) async => db.workspaceId), isEmpty);
      expect(
        await cross.fanOut((db) async => db.workspaceId, includeDeleted: true),
        hasLength(3),
      );
  ...
  group('open-file retention', () {
    // A workspace file is expensive to hold: a background isolate and an 8 MB
    // page cache each. The rule is that a workspace someone is USING stays
    // open and one a fan-out merely visited does not.

    /// Registers [id] in the registry WITHOUT materialising its database, so
    /// the fan-out is what opens it.
    Future<void> register(String id) =>
        global.workspaceRegistryDao.upsertWorkspace(
    ...
    test('a fan-out closes the files it had to open', () async {
      await dbs.closeAll();
      await register('w-4');
      expect(dbs.openIds, isEmpty);

      await cross.fanOut((db) async => db.workspaceId);

      expect(
        dbs.openIds,
        isEmpty,
        reason:
            'rendering one all-workspace list must not leave every workspace '
            'database resident for the process lifetime',
      );
    });

    test('a workspace already in use survives a fan-out', () async {
      await dbs.closeAll();
      // Someone is working in w-2.
      dbs.of('w-2');
      expect(dbs.openIds, contains('w-2'));
      ...
      expect(
        ...
        <String>['w-2'],
        reason:
            'closing a workspace out from under its user would kill every '
            'live drift subscription on it',
      );
    });

    test('a request arriving mid-fan-out claims the workspace', () async {
      // The race the conditional close exists for: the fan-out opened the
      // file, and while it is reading someone starts working in that
      // workspace. Closing it afterwards would break them, so the claim wins.
      await dbs.closeAll();
      await cross.fanOut((db) async {
        if (db.workspaceId == 'w-3') {
          dbs.of('w-3');
        }
        return db.workspaceId;
      ...
      expect(dbs.openIds, <String>['w-3']);
    });

    test(
      'concurrent fan-outs close the file once, after the last one',
      () async {
        await dbs.closeAll();
        final gate = Completer<void>();
        final slow = cross.fanOut((db) async {
          await gate.future;
          return db.workspaceId;
        });
        final quick = cross.fanOut((db) async => db.workspaceId);
        await quick;
        // The slow fan-out is still reading — its files must still be open.
        expect(dbs.openIds, isNotEmpty);
        gate.complete();
        await slow;
        expect(dbs.openIds, isEmpty);
      },
    ...
    test('a live merged stream keeps its workspaces open', () async {
      await dbs.closeAll();
      final sub = cross
          .mergeStreams((db) => db.agentDao.watchAll())
          .listen((_) {});
      addTearDown(sub.cancel);
      await pumpEventQueue();
      ...
      expect(
        dbs.openIds,
        unorderedEquals(<String>['w-1', 'w-2', 'w-3']),
        reason:
            'a merged live view holds one drift subscription per workspace; '
            'closing the connection under it kills the subscription',
      );
    });
  });
}
