    ...
    test('upsert overwrites existing repo', () async {
      ...
      await db.repoDao.upsertRepo(
        ReposTableCompanion.insert(
          ...
          name: 'original',
          path: '/path/orig',
          remoteOwner: const Value('old'),
          remoteName: const Value('old-repo'),
        ),
      );
      await db.repoDao.upsertRepo(
        ReposTableCompanion.insert(
          ...
          name: 'updated',
          path: '/path/updated',
          remoteOwner: const Value('new'),
          remoteName: const Value('new-repo'),
        ),
      );

      final repo = await db.repoDao.getById(id);
      expect(repo!.name, 'updated');
      expect(repo.remoteOwner, 'new');
    });
