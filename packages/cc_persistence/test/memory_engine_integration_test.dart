...
void main() {
  ...
  setUp(() async {
    ...
    factRepo = DaoMemoryFactRepository(dbs);
    conflictRepo = DaoMemoryConflictRepository(dbs);
    edgeRepo = DaoEpisodicEdgeRepository(dbs);
    beliefRepo = DaoMemoryBeliefRepository(dbs);
    workingRepo = DaoWorkingMemoryItemRepository(dbs);
    final resolveDomain = ResolveOrCreateDomainUseCase(
      domainRepository: DaoMemoryDomainRepository(dbs),
      grantRepository: DaoMemoryAccessGrantRepository(dbs),
    );
  ...
  test(
    'AC: contradictory fact is detected and the old one superseded',
    () async {
  ...
  test(
    'AC: 4-voice recall surfaces graph-connected, time-relevant facts',
    () async {
      ...
      expect(
        edges.any(
          (e) => {
            e.sourceFactId,
            e.targetFactId,
          }.containsAll({f1!.fact.id, f2!.fact.id}),
        ),
        isTrue,
      );
  ...
  test(
    'AC: extraction yields facts from conversation without propose_fact',
    () async {
      ...
      final count = await extract.extractAndRecord(
        workspaceId: ws,
        text: 'my name is Sam and I work at UseCtrl. I prefer dark mode.',
      );
  ...
  test('SHMR emits a corroborated belief from two agreeing agents', () async {
    ...
    expect(
      beliefs.first.provenanceAgentIds,
      containsAll(['agent-1', 'agent-2']),
    );
  });

  test(
    'consolidation rolls working items into durable facts and evicts',
    () async {
