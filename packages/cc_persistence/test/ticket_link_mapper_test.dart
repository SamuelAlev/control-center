...
void main() {
  ...
  Future<TicketLinksTableData> insertLink({
    ...
    final links = await db.ticketLinkDao.getForTicket(
      workspaceId,
      sourceTicketId,
    );
  ...
  group('TicketLinkMapper', () {
    test(
      'fromRowOrNull maps all fields correctly',
    ...
    test(
      'fromRowOrNull returns null for unknown type',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'fromRowOrNull maps all TicketLinkType values',
      timeout: const Timeout.factor(2),
      () async {
        const typeMap = {
        ...
        for (final entry in typeMap.entries) {
          ...
          expect(
            link,
            isNotNull,
            reason: 'fromRowOrNull returned null for ${entry.key}',
          );
    ...
    test(
      'toCompanion round-trips through fromRowOrNull',
      timeout: const Timeout.factor(2),
      () async {
        ...
        final links = await db.ticketLinkDao.getForTicket('ws-1', 'ticket-src');
    ...
    test(
      'toCompanion serializes blocks type correctly',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'toCompanion serializes relatesTo type correctly',
      timeout: const Timeout.factor(2),
      () async {
        final link = TicketLink(
          id: 'l2',
    ...
    test(
      'toCompanion serializes duplicateOf type correctly',
      timeout: const Timeout.factor(2),
      () async {
        final link = TicketLink(
          id: 'l3',
    ...
    test(
      'fromRowOrNull for null type returns null',
      timeout: const Timeout.factor(2),
      () async {
