...
void main() {
  ...
  test(
    'getForTicket returns links on either endpoint, scoped to workspace',
    () async {
      await db.ticketLinkDao.insert(link('l-1', 'w-1', 't-1', 't-2', 'blocks'));
      await db.ticketLinkDao.insert(
        link('l-2', 'w-1', 't-3', 't-1', 'relates_to'),
      );
  ...
  test(
    'deleteByEndpoints removes the matching edge, scoped to workspace',
    () async {
      await db.ticketLinkDao.insert(link('l-1', 'w-1', 't-1', 't-2', 'blocks'));
