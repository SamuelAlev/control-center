...
/// orchestrations, code graph, channel surfaces, memory policies, plan/playbook
...
void main() {
  ...
  group('Channel-surface generated tables', () {
    test('channel messages + reactions + notes round-trip', () async {
      await db
      ...
      await db
          ...
          .insert(
            ConversationsTableCompanion.insert(id: 'c-1', channelId: 'c-1'),
          );
      await db
      ...
      await db
          .into(db.channelNotesTable)
          .insert(
            ChannelNotesTableCompanion.insert(
              id: 'note-1',
              workspaceId: 'ws',
              channelId: 'c-1',
      ...
      final note = await (db.select(
        db.channelNotesTable,
      )..where((t) => t.id.equals('note-1'))).getSingle();
      expect(note.channelId, 'c-1');
    });

    test('channel autonomy round-trip', () async {
      await db
          .into(db.channelsTable)
          .insert(ChannelsTableCompanion.insert(id: 'c-2', name: 'c-2'));
      await db
          .into(db.channelAutonomyTable)
          .insert(
  ...
  group('Decision / merge generated tables', () {
    test('remembered decisions + worktree merge log round-trip', () async {
      await db
          .into(db.rememberedDecisionsTable)
          .insert(
            RememberedDecisionsTableCompanion.insert(
              id: 'rd-1',
              workspaceId: 'ws',
              fingerprint: 'fp-1',
              decision: 'approved',
      ...
      await db
          .into(db.worktreeMergeLogTable)
          .insert(
            WorktreeMergeLogTableCompanion.insert(
              id: 'wml-1',
              workspaceId: 'ws',
      ...
      final dec = await (db.select(
        db.rememberedDecisionsTable,
      )..where((t) => t.id.equals('rd-1'))).getSingle();
      expect(dec.decision, 'approved');
      final merge = await (db.select(
        db.worktreeMergeLogTable,
      )..where((t) => t.id.equals('wml-1'))).getSingle();
      expect(merge.sourceBranch, 'feature');
    });
  });
}
