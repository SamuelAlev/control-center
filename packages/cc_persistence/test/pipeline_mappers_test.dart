...
void main() {
  group('pipelineRunToCompanion / pipelineRunFromRow', () {
    ...
    test(
      'pipelineRunToCompanion maps all fields',
    ...
    test(
      'pipelineRunToCompanion uses absent for null triggerPayload',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'pipelineRunFromRow maps all fields',
      timeout: const Timeout.factor(2),
      () {
        final row = row0(
          status: 'failed',
    ...
    test(
      'pipelineRunFromRow handles empty stateJson',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'pipelineRunFromRow handles null triggerPayloadJson',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'round-trip: run to companion reconstructs correctly',
      timeout: const Timeout.factor(2),
      () {
        final original = run0(
          state: {'key': 'value'},
          triggerPayload: {'pr_number': 42},
          dedupKey: 'dk',
          finishedAt: finished,
        ).copyWith(attemptStartedAt: DateTime(2025, 6, 1, 17));
        ...
        final row = PipelineRunsTableData(
          ...
          attemptStartedAt: companion.attemptStartedAt.value,
          attemptCount: companion.attemptCount.value,
          activeMs: companion.activeMs.value,
          lastResumedAt: companion.lastResumedAt.value,
        ...
        // A rerun's attempt stamp has to survive the round-trip, or the run
        // page falls back to the original start and the fix is invisible.
        expect(restored.attemptStartedAt, original.attemptStartedAt);
  ...
  group('stepRunToCompanion / stepRunFromRow', () {
    ...
    test(
      'stepRunToCompanion maps fields',
      timeout: const Timeout.factor(2),
      () {
    ...
    test('stepRunFromRow maps fields', timeout: const Timeout.factor(2), () {
      final row = PipelineStepRunsTableData(
        ...
        branchIndex: null,
        attemptCount: 3,
        attemptHistory: jsonEncode([
          {
            'status': 'failed',
            'started_at': '2024-12-31T23:00:00.000',
            'finished_at': '2025-01-01T00:00:00.000',
      ...
      expect(stepRun.priorAttempts, hasLength(1));
      expect(stepRun.priorAttempts.single.status, PipelineStepStatus.failed);
      expect(stepRun.priorAttempts.single.errorMessage, 'earlier boom');
    });

    test(
      'stepRunFromRow treats a malformed attempt archive as empty',
      timeout: const Timeout.factor(2),
      () {
        final row = PipelineStepRunsTableData(
          id: 'sr-1',
          ...
          branchIndex: null,
          attemptCount: 0,
          attemptHistory: '{not a list',
          startedAt: now,
          finishedAt: now,
        );
        expect(stepRunFromRow(row).priorAttempts, isEmpty);
      },
    );
  });
}
