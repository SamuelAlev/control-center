...
void main() {
  ...
  group('workspace isolation', () {
    ...
    test('a workspace with no settings reads empty, not the neighbour\'s', () async {
      await repo.set('ws-1', 'branch_template', 'one');
      expect(await repo.getAll('ws-2'), isEmpty);
    });

    test('the repository does not pin the first workspace it resolved', () async {
  ...
  group('quotas', () {
    test('accepts a value at the byte limit', () async {
      final atLimit =
          'x' * DaoWorkspaceSettingsRepository.maxValueBytes;
    ...
    test('rejects a value over the byte limit', () async {
      final tooBig =
          'x' * (DaoWorkspaceSettingsRepository.maxValueBytes + 1);
      await expectLater(
