...
void main() {
  ...
  group('ProjectMapper', () {
    test(
      'fromRow maps all fields correctly',
    ...
    test(
      'fromRow maps nullable description when present',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'toCompanion round-trips through fromRow',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'fromRow maps all ProjectColor values',
      timeout: const Timeout.factor(2),
      () async {
        for (final color in ProjectColor.values) {
          ...
          expect(
            project.color,
            color,
            reason: 'Failed for color ${color.name}',
          );
        }
      },
    );
    ...
    test(
      'fromRow maps all ProjectStatus values',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'fromRow handles unknown color gracefully (defaults to gray)',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'fromRow handles unknown status gracefully (defaults to active)',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'toCompanion preserves null description',
      timeout: const Timeout.factor(2),
      () async {
        final original = Project(
          id: 'proj-null',
    ...
    test(
      'toCompanion serializes color and status to storage strings',
      timeout: const Timeout.factor(2),
      () async {
