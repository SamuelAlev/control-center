...
void main() {
  ...
  setUp(() async {
    ...
    // Channel row + its main conversation (channel_messages references both).
    await db
  ...
  test('paginates newest-first windows with a working cursor', () async {
    ...
    final page1 = await repo.getMessagePage('w-1', 'c', 'c', limit: 80);
  ...
  test('short history returns a single complete page', () async {
    await seed(10);
    final page = await repo.getMessagePage('w-1', 'c', 'c', limit: 80);
  ...
  test('a page is bound to its channel, not just its conversation id', () async {
    ...
    // any one channel read every conversation in the workspace file. The
    // predicate is authorization, not a filter — for a legitimate caller
    // (a conversation belongs to exactly one channel) it changes nothing.
    await db
        .into(db.channelsTable)
        .insert(
          const ChannelsTableCompanion(id: Value('other'), name: Value('o')),
        );
    await db
        .into(db.conversationsTable)
        .insert(
    ...
    // The rightful channel still reads it.
