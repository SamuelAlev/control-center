...
PipelineDefinition _makeSimpleDef({String id = 'tpl-1', String ws = 'ws-1'}) {
  final steps = [
...
void main() {
  ...
  group('_kindToString (via pipelineDefinitionToCompanion)', () {
    test('converts each StepKind to correct string', () {
      ...
      for (final entry in cases.entries) {
        final steps = [
          PipelineStepDefinition(id: 'n', kind: entry.key, bodyKey: 'body'),
        ];
    ...
    test(
      'round-trip: kindToString → kindFromString for all StepKind values',
      () {
        for (final kind in StepKind.values) {
          ...
          final nodes = jsonDecode(companion.nodesJson.value) as List<dynamic>;
          final kindStr =
              (nodes.first as Map<String, dynamic>)['kind'] as String;
