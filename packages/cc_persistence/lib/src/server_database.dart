...
/// Switches a freshly-opened connection to WAL **before** drift runs its
/// migration, which the `beforeOpen` pragmas cannot do — they run *after*
/// `onCreate`.
///
/// That ordering is what makes creating a workspace pathological on Windows.
/// The baseline builds ~480 schema objects and, until WAL is on, every one of
/// them is its own rollback-journal transaction: create `workspace.db-journal`,
/// sync, write, sync, unlink, sync the directory. On APFS/ext4 that is
/// sub-millisecond noise (the whole create takes ~330ms); on NTFS it is ~480
/// rounds of file-create/unlink metadata work, each one a fresh file for the
/// virus scanner to open, and the create overran a 30s test timeout. In WAL the
/// same statements append to one already-open file.
///
/// Sent to the database isolate, so this must stay a top-level function that
/// captures nothing.
void _enableWal(sqlite3.Database db) {
