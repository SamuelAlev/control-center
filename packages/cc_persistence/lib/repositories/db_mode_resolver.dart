...
///
/// One indexed lookup on `channels.id` per dispatch, in the database file of
...
/// channel (e.g. CLI one-shots) and for a channel id belonging to some other
...
class DbModeResolver implements ModeResolver {
  ...
  Future<Mode> resolveForConversation(
    ...
    final row = await _dao(workspaceId).getChannelById(conversationId);
    if (row == null) {
      return Mode.chat;
    }
