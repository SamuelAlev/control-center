...
class DaoCalendarRepository implements CalendarRepository {
  ...
  Stream<List<CalendarSource>> watchSources(
    ...
    String accountId,
      ...
      _dao.watchSources(workspaceId, accountId).map(
            (rows) => rows.map(_mapper.sourceToDomain).toList(growable: false),
          );
  ...
  Future<void> upsertSources({
    ...
    return _dao.replaceSources(workspaceId, accountId, companions);
  }
