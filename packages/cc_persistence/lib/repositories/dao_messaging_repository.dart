...
class DaoMessagingRepository implements MessagingRepository {
  ...
  ///
  /// Drift invalidation is TABLE-granular: any `channel_messages` write —
  /// e.g. another channel's streaming flush — re-runs EVERY message watch and
  /// re-emits its (unchanged) result, which then costs entity mapping, JSON
  /// encoding, a socket push, and a client decode per subscription. Comparing
  /// row lists here (DataClass/record value equality), before any of that,
  /// absorbs the fan-out: a write to channel A no longer produces emissions
  /// for channels B..Z. Per-call listener state, so each subscription dedups
  /// independently.
