...
import 'package:cc_persistence/src/log/cc_persistence_log.dart';
...
class AnalyticsRepositoryImpl implements AnalyticsRepository {
  ...
  Stream<List<AgentDailyStats>> watchByAgentDateRange(
    ...
    DateTime end,
      ...
      _analyticsDao
          .watchByAgentAndDateRange(workspaceId, agentId, start, end)
          .map(_mappers.toDomainList);
  ...
  Stream<List<AgentDailyStats>> watchAllByDateRange(
    ...
    DateTime end,
      ...
      _analyticsDao
          .watchAllByDateRange(workspaceId, start, end)
          .map(_mappers.toDomainList);
  ...
  Future<AgentScorecard?> getAgentScorecard(
    ...
    // A foreign agent (or unknown id) yields no scorecard — the workspace scope
    // is the isolation boundary, not the id.
    if (agent == null || agent.workspaceId != workspaceId) {
      return null;
    }
    final allStats = await _analyticsDao.getByAgent(workspaceId, agentId);
    if (allStats.isEmpty) {
  ...
  Future<List<AgentScorecard>> getAllAgentScorecards(String workspaceId) async {
    // Workspace-scoped: only this workspace's agents (never getAll()).
    final agents = await _agentDao.watchByWorkspace(workspaceId).first;
    final results = <AgentScorecard>[];
    for (final agent in agents) {
      final card = await getAgentScorecard(workspaceId, agent.id);
      if (card != null) {
        results.add(card);
      }
    }
  ...
  Future<List<LeaderboardEntry>> getLeaderboard(
    ...
    // Stats are workspace-scoped via the DAO's Agents JOIN; names come from the
    // same workspace's agents only.
    final stats = await _analyticsDao.getAllByDateRange(workspaceId, start, end);
    final agents = await _agentDao.watchByWorkspace(workspaceId).first;
  ...
  Future<WorkspaceHealth?> getWorkspaceHealth(String workspaceId) async {
    ...
    try {
      ...
      CcPersistenceLog.warning(
        'AnalyticsRepositoryImpl: Failed to watch all logs for workspace health',
      );
    }

    final activeAgents = agentIds.length;
    // Workspace-scoped agent count (never getAll(), which spans every workspace).
    final totalAgents = (await _agentDao.watchByWorkspace(workspaceId).first).length;

    // PRs merged this week from this workspace's daily stats (scoped via the
    // DAO's Agents JOIN — previously this was a cross-workspace global sum).
    int prsMergedThisWeek = 0;
    try {
      final allStats =
          await _analyticsDao.getAllByDateRange(workspaceId, weekAgo, now);
      prsMergedThisWeek = allStats.fold<int>(0, (s, e) => s + e.prsMerged);
    } catch (_) {
      CcPersistenceLog.warning(
        'AnalyticsRepositoryImpl: Failed to get PR merge stats for date range',
      );
    }
    ...
    try {
      ...
      CcPersistenceLog.warning(
        'AnalyticsRepositoryImpl: Failed to watch PRs for workspace health',
      );
    }
  ...
  // intentionally aggregates every workspace's health. Each entry is built by
  // the workspace-scoped getWorkspaceHealth(ws.id), so no single entry leaks
  // another workspace's data. For one workspace's health call
  // getWorkspaceHealth(workspaceId) directly.
  @override
  ...
  // CROSS-WORKSPACE BY DESIGN: startup/maintenance reconciler that rebuilds
  // today's daily-stats row for EVERY agent in EVERY workspace from its run
  // logs. It writes each agent's own row (keyed by agent.id, which carries the
  // agent's workspace), so it materializes per-workspace data without crossing
  // workspaces. Not a UI read path.
  @override
  Future<void> rebuildDailyStats() async {
    ...
    for (final agent in agents) {
      final runs =
          await _agentDao.watchLogsByAgent(agent.workspaceId, agent.id).first;
  ...
  // CROSS-WORKSPACE BY DESIGN: maintenance reconciler that backfills historical
  // daily-stats rows for EVERY agent in EVERY workspace from its run logs. Each
  // row is keyed by agent.id (which carries the agent's workspace), so it never
  // crosses workspaces. Not a UI read path.
  @override
