...
class DaoReviewRunSnapshotRepository implements ReviewRunSnapshotRepository {
  ...
  Future<void> record(String workspaceId, ReviewRunSnapshot snapshot) =>
      _dao(workspaceId).insertRunSnapshot(
        ReviewRunSnapshotsTableCompanion(
          ...
          spaceId: Value(snapshot.spaceId),
  ...
  Future<List<String>> dismissedFindingTitles(
    ...
    final rows = await _dao(workspaceId).runSnapshotsForWorkspace(workspaceId);
    // Newest first: a workspace's taste changes, and the most recent
    // dismissals describe it best.
    final sorted = [...rows]
      ..sort((a, b) => b.finalizedAt.compareTo(a.finalizedAt));
    final titles = <String>[];
    final seen = <String>{};
    for (final row in sorted) {
      for (final f in _decodeFingerprints(row.fingerprintsJson)) {
        if (f.status != ReviewNodeStatus.dismissed) {
          continue;
        }
        final title = f.title.trim();
        // Deduped: the same finding re-flagged and re-dismissed across three
        // passes is one opinion, not three, and counting it three times would
        // let a single stubborn finding suppress a whole class on its own.
        if (title.isEmpty || !seen.add(title.toLowerCase())) {
          continue;
        }
        titles.add(title);
  ...
  Future<int> applyFindingStatus(
    ...
    final rows = await dao.runSnapshotsForPr(workspaceId, prExternalId);
    var rewritten = 0;
    for (final row in rows) {
      final fingerprints = _decodeFingerprints(row.fingerprintsJson);
      if (!fingerprints.any((f) => f.messageId == nodeMessageId)) {
        continue;
      }
      final updated = [
        for (final f in fingerprints)
          if (f.messageId != nodeMessageId)
            f
          else
            FindingFingerprint(
      ...
      // second press on the same finding cannot drift the counters.
      // `newCount` is a property of the pass, not of anyone's later decision.
      final previous = _decodeStats(row.statsJson);
      final next = ReviewRunStats(
        findingsTotal: updated.length,
        resolved: updated
            .where((f) => f.status == ReviewNodeStatus.resolved)
            .length,
        dismissed: updated
            .where((f) => f.status == ReviewNodeStatus.dismissed)
            .length,
        stillOpen: updated.where((f) => f.isOpen).length,
        newCount: previous.newCount,
      );
      await dao.updateRunSnapshotOutcome(
        row.id,
        fingerprintsJson: jsonEncode([for (final f in updated) f.toJson()]),
        statsJson: jsonEncode(next.toJson()),
      );
      rewritten++;
    }
    return rewritten;
