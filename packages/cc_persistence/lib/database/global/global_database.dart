...
class GlobalDatabase extends _$GlobalDatabase {
  ...
  List<MigrationStep> get _migrationSteps => <MigrationStep>[
    ...
    // v3: SSO. The connection rows are CROSS-WORKSPACE BY DESIGN — one IdP
    // authenticates every workspace's humans (membership stays gated by the
    // auto-member policy). The users columns pin an SSO provision to the
    // provider's immutable subject id so a reused/changed email cannot take
    // over an account; the partial unique index makes the pin one account
    // per (issuer, subject). `m.createTable` does not create indexes, so
    // the partial index is issued explicitly here.
    MigrationStep(2, 3, (Migrator m) async {
      await m.createTable(ssoConnectionsTable);
      await m.addColumn(usersTable, usersTable.ssoSubject);
      await m.addColumn(usersTable, usersTable.ssoIssuer);
      await m.addColumn(usersTable, usersTable.deactivatedAt);
      // Spelled out rather than `m.createIndex(...)` (no partial-index
      // support); idempotent because beforeOpen re-declares it too.
    ...
    // v4: the newsfeed became per-user. `rss_feeds` gains the owning
    // `user_id` (FK to users, cascade delete); `rss_articles` needs no change
    // — its rows hang off a feed, so scoping the feed scopes the article.
    // SQLite cannot ADD COLUMN NOT NULL without a default, and cannot add an
    // FK constraint via ALTER at all, so the column is added with a sentinel
    // owner: feeds registered before the split belonged to no user and are
    // deliberately orphaned (invisible to everyone) rather than reassigned —
    // a breaking change by design. Fresh databases get the real FK from
    // `onCreate`.
    ...
    // v5: opt-in auto-publish for completed reviews. A workspace-level
    // decision (mirrors `review_concurrency`): when on, the review hub
    // publishes the finalized review to GitHub without waiting for the
    // "Publish to GitHub" button. Defaults to false — publishing stays
    // user-gated unless the operator explicitly opts in.
    MigrationStep(4, 5, (Migrator m) async {
      await m.addColumn(workspacesTable, workspacesTable.autoPublishReview);
    }),
  ];
