...
class WorkspaceDatabase extends _$WorkspaceDatabase {
  ...
  List<MigrationStep> get _migrationSteps => <MigrationStep>[
    ...
    MigrationStep(5, 6, (Migrator m) async {
      ...
      // Spelled out rather than `m.createIndex(...)`, which generates a bare
      // `CREATE INDEX` and therefore throws if the index is already there.
      // `createTable` above is `CREATE TABLE IF NOT EXISTS`, so the rest of the
      // step is re-runnable; the index has to be too, or a database that
      // already carries it (a restored backup, an imported workspace file, the
      // wind-back fixtures the migration tests build) fails to open outright.
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_notification_feed_workspace_created '
        'ON notification_feed (workspace_id, created_at)',
      );
    }),
    // v7: repos gain a forge, and every GitHub-named identity column loses the
    // vendor from its name.
    //
    // Control Center used to be a GitHub client that happened to store repos;
    // it is now a multi-forge client where the forge is a property of each repo
    // (one workspace may hold GitHub, GitLab and Bitbucket repos side by side).
    // `github_owner`/`github_repo_name` become `remote_owner`/`remote_name`
    // because a GitLab namespace is not an "owner" and may nest
    // (`group/subgroup`), and `pr_node_id` becomes `pr_external_id` because
    // only GitHub calls its opaque PR identity a node id — the column still
    // holds exactly what it held (existing GitHub node ids stay valid), it just
    // no longer promises which forge minted it.
    //
    // Renames, not add-and-backfill: the data is unchanged, so `ALTER TABLE …
    // RENAME COLUMN` carries every existing row across untouched. SQLite
    // rewrites dependent index definitions automatically, but keeps their
    // names, so the four `pr_node_id` indexes are dropped and recreated under
    // their new names to match what `onCreate` builds for a fresh database.
    MigrationStep(6, 7, (Migrator m) async {
      if (await _columnExists('repos', 'github_owner')) {
        await m.renameColumn(reposTable, 'github_owner', reposTable.remoteOwner);
      }
      if (await _columnExists('repos', 'github_repo_name')) {
        await m.renameColumn(reposTable, 'github_repo_name', reposTable.remoteName);
      }
      if (!await _columnExists('repos', 'forge')) {
        await m.addColumn(reposTable, reposTable.forge);
      }

      // NB: the SQL names below are drift's generated ones, which keep the
      // `_table` suffix wherever the Dart table class does not override
      // `tableName`. Guessing the "obvious" name here silently skips the
      // rename, which is why `workspace_baseline_schema_test` asserts the
      // resulting columns rather than trusting this list.
      if (await _columnExists('pull_requests_table', 'github_pr_url')) {
        await m.renameColumn(
          pullRequestsTable,
          'github_pr_url',
          pullRequestsTable.prUrl,
        );
      }
      if (await _columnExists('pull_requests_table', 'github_pr_number')) {
        await m.renameColumn(
          pullRequestsTable,
          'github_pr_number',
          pullRequestsTable.prNumber,
        );
      }

      // `(table, legacy index name, new index name)`. Every step is guarded on
      // the table actually existing: `onUpgrade` replays each step for any
      // database older than its target, and the wind-back fixtures the
      // migration tests build start from versions where these tables were
      // never created. An unguarded `CREATE INDEX` there fails the whole open.
      const prExternalIdRenames = <(String, String, String)>[
        (
          'review_channels',
          'idx_review_channels_prNodeId',
          'idx_review_channels_prExternalId',
        ),
        (
          'review_cohorts_table',
          'idx_review_cohorts_pr',
          'idx_review_cohorts_pr',
        ),
        (
          'api_contract_snapshots_table',
          'idx_api_contract_snapshots_pr',
          'idx_api_contract_snapshots_pr',
        ),
        (
          'visual_diff_snapshots_table',
          'idx_visual_diff_snapshots_pr',
          'idx_visual_diff_snapshots_pr',
        ),
        (
          'review_axis_results_table',
          'idx_review_axis_results_pr',
          'idx_review_axis_results_pr',
        ),
      ];
      for (final (table, legacyIndex, newIndex) in prExternalIdRenames) {
        if (!await _tableExists(table)) {
          continue;
        }
        if (await _columnExists(table, 'pr_node_id')) {
          await customStatement('DROP INDEX IF EXISTS $legacyIndex');
          await customStatement(
            'ALTER TABLE $table RENAME COLUMN pr_node_id TO pr_external_id',
          );
        }
        if (await _columnExists(table, 'pr_external_id')) {
          await customStatement(
            'CREATE INDEX IF NOT EXISTS $newIndex ON $table (pr_external_id)',
          );
        }
      }
    }),
    // v8: enclosures (rigs) — disposable VM/microVM sessions an agent or a
    // human drives, plus the attributed log of every input event sent to one.
    //
    // The session row outlives the machine on purpose: the VM is destroyed and
    // its disk overlay discarded, so without a durable row a reaped rig is
    // indistinguishable from one that never existed. The action log is the
    // only record that a human ever took the wheel — watch-lane frames are
    // never persisted.
    MigrationStep(7, 8, (Migrator m) async {
      await m.createTable(rigSessionsTable);
      await m.createTable(rigActionLogTable);
      // Spelled out rather than `m.createIndex(...)`, which emits a bare
      // `CREATE INDEX` and throws when the index is already present — a
      // restored backup, an imported workspace file or the migration tests'
      // wind-back fixtures all hit that path.
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_rig_sessions_workspace '
        'ON rig_sessions (workspace_id)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_rig_sessions_phase '
        'ON rig_sessions (workspace_id, phase, last_activity_at)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_rig_sessions_conversation '
        'ON rig_sessions (workspace_id, conversation_id)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_rig_action_log_rig '
        'ON rig_action_log (workspace_id, rig_id, seq)',
      );
    }),
    // v9: the browser rig's address-bar state. The URL the page is on is
    // pushed to watchers off this column rather than polled out of the guest.
    //
    // Guarded: `ALTER TABLE … ADD COLUMN` has no conditional form in SQLite,
    // and the idempotency ratchet replays this step over a database that
    // already carries the column.
    MigrationStep(8, 9, (Migrator m) async {
      if (!await _columnExists('rig_sessions', 'current_url')) {
        await m.addColumn(rigSessionsTable, rigSessionsTable.currentUrl);
      }
    }),
    // v10: deterministic cohort insights (touched symbols, covering tests,
    // which partition the spans came from). Guarded for the same reason as v9.
    MigrationStep(9, 10, (Migrator m) async {
      if (!await _columnExists('review_cohorts_table', 'insights_json')) {
        await m.addColumn(
          reviewCohortsTable,
          reviewCohortsTable.insightsJson,
        );
      }
    }),
    // v11: dependency lockfile diffs for a PR.
    MigrationStep(10, 11, (Migrator m) async {
      if (!await _tableExists('review_dependency_diffs_table')) {
        await m.createTable(reviewDependencyDiffsTable);
      }
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_review_dependency_diffs_workspace '
        'ON review_dependency_diffs_table (workspace_id)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_review_dependency_diffs_pr '
        'ON review_dependency_diffs_table (pr_external_id)',
      );
    }),
    // v12: finalized review passes, so a re-review can report its delta
    // against the previous pass instead of restating every finding.
    MigrationStep(11, 12, (Migrator m) async {
      if (!await _tableExists('review_run_snapshots_table')) {
        await m.createTable(reviewRunSnapshotsTable);
      }
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_review_run_snapshots_workspace '
        'ON review_run_snapshots_table (workspace_id)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_review_run_snapshots_pr '
        'ON review_run_snapshots_table (pr_external_id)',
      );
      await customStatement(
        'CREATE INDEX IF NOT EXISTS idx_review_run_snapshots_pr_time '
        'ON review_run_snapshots_table '
        '(workspace_id, pr_external_id, finalized_at)',
      );
    }),
    // v13: the hot-path index set gained five entries (message history by
    // conversation, run logs by conversation, run logs by `started_at` alone,
    // PRs by workspace+created, and the partial index over messages still
    // awaiting an embedding). `_createHotPathIndexes` is `IF NOT EXISTS`
    // throughout, so re-running the whole helper is the migration.
    MigrationStep(12, 13, (Migrator m) async {
      await _createHotPathIndexes();
      // Two trigger definitions changed, and `CREATE TRIGGER IF NOT EXISTS`
      // (which is what `beforeOpen` reinstalls with) will not replace an
      // existing one. Dropping them here is what makes `beforeOpen` rebuild
      // them with the new guards on an already-deployed database.
      await customStatement('DROP TRIGGER IF EXISTS channel_messages_au');
      await customStatement(
        'DROP TRIGGER IF EXISTS trg_sync_channel_messages_update',
      );
    }),
  ];
  ...
  ///
  /// The v7 renames need it for the same reason [_tableExists] exists: a
  /// migration step runs for every database older than its target version, and
  /// `onUpgrade` re-runs every step from `from`, so a rename must be a no-op
  /// once it has already happened. `ALTER TABLE … RENAME COLUMN` on a missing
  ...
  Future<void> _createSyncTriggers() async {
    ...
    for (final (table, store, wsTemplate, pk, ctxTemplate) in specs) {
      for (final (op, sqlOp, row) in [
        ...
        // batch) and each fired three statements plus a `channels` subselect
        ...
        final embeddingGuard =
            table == 'channel_messages' && op == 'update'
  ...
  Future<void> _createChannelMessagesFts() async {
    ...
    await customStatement('''
      ...
      END
    ''');
    // `OF content, channel_id` — the only two columns this index covers.
    ...
    await customStatement('''
      CREATE TRIGGER IF NOT EXISTS channel_messages_au
      AFTER UPDATE OF content, channel_id ON channel_messages BEGIN
        INSERT INTO channel_messages_fts(channel_messages_fts, rowid, content, channel_id)
        VALUES ('delete', old.rowid, old.content, old.channel_id);
        INSERT INTO channel_messages_fts(rowid, content, channel_id)
        VALUES (new.rowid, new.content, new.channel_id);
  ...
  ///    actually filters on (`conversation_id != channel_id` — the channel
  ...
  Future<void> _createHotPathIndexes() async {
    ...
    await customStatement(
      ...
      'ON agent_run_logs_table (workspace_id, conversation_id, started_at)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS idx_agent_run_logs_started '
      'ON agent_run_logs_table (started_at DESC)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS idx_pull_requests_ws_created '
      'ON pull_requests_table (workspace_id, created_at)',
    );
    ...
    // `workspace_id`: `channel_messages` has no such column (a message belongs
    // to a channel, which belongs to the workspace, and a file now holds
  ...
  /// CC channels. Partial indexes carry a `WHERE`, so it has to be spelled out.
  ///
  ...
  Future<void> _createChatIndexes() async {
    await customStatement(
      ...
      'uq_chat_channel_links_ws_provider_channel_thread '
      'ON chat_channel_links '
      '(workspace_id, provider, external_channel_id, external_thread_id)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS idx_chat_channel_links_ccChannelId '
      'ON chat_channel_links (cc_channel_id)',
    );
    await customStatement(
      'CREATE UNIQUE INDEX IF NOT EXISTS '
