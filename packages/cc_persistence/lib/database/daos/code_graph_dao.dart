...
class CodeGraphDao extends DatabaseAccessor<WorkspaceDatabase>
  ...
  Future<void> setEdgeTargets(
    ...
    return transaction(() async {
      for (final entry in edgeIdToTargetId.entries) {
        await (update(codeEdgesTable)..where(
              (e) => e.id.equals(entry.key) & e.workspaceId.equals(workspaceId),
            ))
            .write(CodeEdgesTableCompanion(targetSymbolId: Value(entry.value)));
      }
    });
  }
  ...
  Future<List<CodeSymbolsTableData>> searchHybrid(
    ...
    final ftsResults = await searchFts(
      workspaceId,
      repoId,
      query,
