...
class MeetingDao extends DatabaseAccessor<WorkspaceDatabase>
    with _$MeetingDaoMixin {
  ...
  /// This workspace's meetings not yet in a terminal state (`recording` or
  /// `processing`). Used by the startup reconciler
  /// (MeetingSummaryReconciler), which visits every workspace, to un-stick
  /// meetings stranded
  ...
  Future<List<MeetingsTableData>> getUnfinalized() => (select(
    meetingsTable,
  )..where((t) => t.status.isIn(['recording', 'processing']))).get();
  ...
  Future<void> deleteMeeting(String workspaceId, String id) => (delete(
    meetingsTable,
  ...
  Future<void> replaceSegments(
    ...
    await (delete(meetingTranscriptSegmentsTable)..where(
          (t) =>
              t.meetingId.equals(meetingId) & t.workspaceId.equals(workspaceId),
        ))
        .go();
    if (segments.isNotEmpty) {
      await batch((b) => b.insertAll(meetingTranscriptSegmentsTable, segments));
    }
  });
  ...
  Stream<Map<String, ({int total, int done})>> watchActionItemStats(
    ...
    final done = meetingActionItemsTable.id.count(
      filter: meetingActionItemsTable.done.equals(true),
    );
  ...
  Future<void> replaceActionItems(
    ...
    final priorAgent =
        await (select(meetingActionItemsTable)..where(
              (t) =>
                  t.meetingId.equals(meetingId) &
                  t.workspaceId.equals(workspaceId) &
  ...
  Future<void> deleteActionItem(String workspaceId, String id) => (delete(
    meetingActionItemsTable,
  )..where((t) => t.id.equals(id) & t.workspaceId.equals(workspaceId))).go();
  ...
  Future<void> deleteDecision(String workspaceId, String id) => (delete(
    meetingDecisionsTable,
  )..where((t) => t.id.equals(id) & t.workspaceId.equals(workspaceId))).go();
  ...
  Future<void> replaceSpeakers(
    ...
    final prior =
        await (select(meetingSpeakersTable)..where(
              (t) =>
                  t.meetingId.equals(meetingId) &
                  t.workspaceId.equals(workspaceId),
            ))
