...
class MessagingDao extends DatabaseAccessor<WorkspaceDatabase>
  ...
  /// Every `channel_messages` column except `embedding`, as a SELECT list.
  ///
  ...
  /// arriving null.
  late final String _messageColumnsSansEmbedding = channelMessagesTable.$columns
  ...
  Stream<List<ChannelMessagesTableData>> watchMessages(String conversationId) =>
      customSelect(
            'SELECT $_messageColumnsSansEmbedding FROM channel_messages '
          ...
          .map(
            (rows) => rows
                .map((r) => channelMessagesTable.map(r.data))
  ...
  ///
  /// Scoped by [channelId] as well as [conversationId], and that is an
  /// AUTHORIZATION predicate, not a filter. A conversation belongs to exactly
  /// one channel, so for a legitimate caller the extra equality changes
  /// nothing — but the caller-supplied conversation id used to be the ONLY
  /// predicate. Callers prove they own a CHANNEL; if the query does not also
  /// bind to that channel, owning any one channel reads every conversation in
  /// the workspace file. Cheap, too: `idx_channel_messages_conversation_created`
  /// still drives the scan and this narrows it.
  Future<List<({ChannelMessagesTableData data, int rowid})>> getMessagePageRows(
    ...
    return customSelect(
          ...
          'WHERE channel_id = ? AND conversation_id = ? AND reverted = 0 '
  ...
  ///
  /// The CHANNEL row goes first and FK `ON DELETE CASCADE` takes the children
  /// with it. Order matters for cost, not correctness: the sync-feed triggers
  /// on the child tables resolve their workspace with
  /// `(SELECT workspace_id FROM channels WHERE id = OLD.channel_id)` and fire
  /// only `WHEN` that is non-null. Deleting children first left the parent in
  /// place, so every one of a 10k-message channel's rows ran three statements
  ...
  /// channel's own delete change and cascade child removal locally.
