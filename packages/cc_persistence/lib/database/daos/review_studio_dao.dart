...
class ReviewStudioDao extends DatabaseAccessor<WorkspaceDatabase>
  ...
  /// Rewrites one recorded pass's fingerprints + counters in place.
  ///
  /// A pass is finalized once but its findings keep living: a person marks one
  /// fixed a day later, and that decision has to reach the row the counters and
  /// the suppression memory are read from, or it reaches nothing at all.
  Future<void> updateRunSnapshotOutcome(
    String snapshotId, {
    required String fingerprintsJson,
    required String statsJson,
  }) =>
      (update(reviewRunSnapshotsTable)..where((t) => t.id.equals(snapshotId)))
          .write(
            ReviewRunSnapshotsTableCompanion(
              fingerprintsJson: Value(fingerprintsJson),
