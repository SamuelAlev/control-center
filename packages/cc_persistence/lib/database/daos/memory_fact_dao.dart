...
class MemoryFactDao extends DatabaseAccessor<WorkspaceDatabase>
    with _$MemoryFactDaoMixin {
  /// Creates a [MemoryFactDao].
  MemoryFactDao(super.attachedDatabase);

  /// Every `memory_facts_table` column except `embedding`, as a SELECT list.
  ///
  /// The embedding is a ~1.5 KB FLOAT32 blob per fact and the domain entity has
  /// no field for it — the mapper drops it on the floor. An unbounded
  /// `SELECT *` watch therefore re-read every fact's blob on every upsert, to
  /// hand back rows the caller cannot see it in. Derived from the table's own
  /// column list so a column added later is included automatically.
  late final String _factColumnsSansEmbedding = memoryFactsTable.$columns
  ...
  Stream<List<MemoryFactsTableData>> watchByWorkspace(String workspaceId) =>
      customSelect(
            'SELECT $_factColumnsSansEmbedding FROM memory_facts_table '
          ...
          .map(
            (rows) => rows
                .map((r) => memoryFactsTable.map(r.data))
  ...
  Future<List<MemoryFactsTableData>> searchFts(
    ...
    return customSelect(
      ...
      'ORDER BY rank '
      'LIMIT 20',
      variables: [Variable<String>(match), Variable<String>(workspaceId)],
      readsFrom: {memoryFactsTable},
  ...
  Future<List<MemoryFactsTableData>> searchHybrid(
    ...
    // Concurrent: the two voices are independent queries and the fusion needs
    // both, so running them in sequence just adds the slower one's latency to
    // the faster one's.
  ...
  Future<void> deleteById(String workspaceId, String id) => (delete(
    memoryFactsTable,
