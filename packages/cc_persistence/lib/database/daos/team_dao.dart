...
class TeamDao extends DatabaseAccessor<WorkspaceDatabase> with _$TeamDaoMixin {
  /// Creates a [TeamDao].
  TeamDao(super.db);
  ...
  Future<List<TeamsTableData>> teamsForWorkspace(String workspaceId) => (select(
    teamsTable,
  ...
  Future<void> addMember(TeamMembersTableCompanion member) =>
      into(teamMembersTable).insert(member, mode: InsertMode.insertOrIgnore);

  /// Removes an agent from a team.
  Future<void> removeMember(String teamId, String agentId) => (delete(
    teamMembersTable,
  )..where((m) => m.teamId.equals(teamId) & m.agentId.equals(agentId))).go();
