...
class RssDao extends DatabaseAccessor<GlobalDatabase> with _$RssDaoMixin {
  ...
  Future<List<RssFeedsTableData>> getEnabledFeeds() =>
      (select(rssFeedsTable)..where((t) => t.enabled.equals(true))).get();

  /// Get feed by url.
  Future<RssFeedsTableData?> getFeedByUrl(String url) => (select(
    rssFeedsTable,
  ...
  Stream<List<RssArticlesTableData>> watchSavedArticles() =>
      (select(rssArticlesTable)
  ...
  /// Get article by id.
  Future<RssArticlesTableData?> getArticleById(String id) => (select(
    rssArticlesTable,
  ...
  Future<void> markAllRead() => update(
    rssArticlesTable,
  ).write(const RssArticlesTableCompanion(read: Value(true)));
}
