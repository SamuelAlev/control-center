...
class CalendarDao extends DatabaseAccessor<WorkspaceDatabase>
  ...
  Future<void> setLastSyncedAt(
    ...
    DateTime at,
      ...
      (update(calendarAccountsTable)..where(
            (t) => t.id.equals(accountId) & t.workspaceId.equals(workspaceId),
          ))
          .write(
  ...
  Future<bool> markNeedsReauth(
    ...
    return transaction(() async {
      ...
      await (update(calendarAccountsTable)..where(
            (t) => t.id.equals(accountId) & t.workspaceId.equals(workspaceId),
          ))
  ...
  Future<void> deleteAccount(String workspaceId, String id) => (delete(
    calendarAccountsTable,
  )..where((t) => t.id.equals(id) & t.workspaceId.equals(workspaceId))).go();

  // ── Sources ──

  /// Watches the synced calendar sources for `(workspaceId, accountId)`
  /// (primary first, then alphabetical by summary). Scoped to both the
  /// workspace and the account so a foreign-workspace or foreign-account id
  /// simply yields nothing.
  Stream<List<CalendarSourcesTableData>> watchSources(
    String workspaceId,
    String accountId,
  ) =>
      (select(calendarSourcesTable)
            ..where(
              (t) =>
                  t.workspaceId.equals(workspaceId) &
            ...
            ..orderBy([
              (t) => OrderingTerm.desc(t.primary),
              (t) => OrderingTerm.asc(t.summary),
            ]))
          .watch();

  /// Replaces the synced calendar sources for `(workspaceId, accountId)` with
  /// [sources] (deleting any the provider no longer lists), atomically. The
  /// provider returns the full current calendar list on every sync, so a
  /// delete-then-insert replace-all is correct and idempotent.
  ///
  /// The per-calendar incremental-sync token columns are **carried over** from
  /// the deleted rows (matched by `calendarId`) — the sync sweep replaces the
  /// source list on every pass and losing the token there would silently
  /// downgrade every pass back to a full re-fetch.
  Future<void> replaceSources(
    String workspaceId,
    String accountId,
    List<CalendarSourcesTableCompanion> sources,
  ) async {
    await transaction(() async {
      final existing =
          await (select(calendarSourcesTable)..where(
                (t) =>
                    t.workspaceId.equals(workspaceId) &
                    t.accountId.equals(accountId),
              ))
              .get();
      final tokensByCalendarId = {
        for (final row in existing)
          if (row.syncToken != null)
            row.calendarId: (
              token: row.syncToken!,
              mintedAt: row.syncTokenMintedAt,
            ),
      };
      await (delete(calendarSourcesTable)..where(
            (t) =>
                t.workspaceId.equals(workspaceId) &
                t.accountId.equals(accountId),
          ))
      ...
      for (final source in sources) {
        final carried = tokensByCalendarId[source.calendarId.value];
        await into(calendarSourcesTable).insert(
          carried == null
              ? source
              : source.copyWith(
                  syncToken: Value(carried.token),
                  syncTokenMintedAt: Value(carried.mintedAt),
                ),
        );
      }
    });
  ...
  /// The incremental-sync token state for one calendar, or null when absent.
  Future<({String token, DateTime? mintedAt})?> getSyncState(
    ...
    final row =
        await (select(calendarSourcesTable)..where(
            ...
            .getSingleOrNull();
    final token = row?.syncToken;
    if (row == null || token == null || token.isEmpty) {
      return null;
    }
    return (token: token, mintedAt: row.syncTokenMintedAt);
  }

  /// Stores (or, with a null [token], clears) one calendar's sync token.
  Future<void> setSyncState(
    ...
    await (update(calendarSourcesTable)..where(
          (t) =>
              t.workspaceId.equals(workspaceId) &
              t.accountId.equals(accountId) &
              t.calendarId.equals(calendarId),
        ...
        .write(
          CalendarSourcesTableCompanion(
            syncToken: Value(token),
            syncTokenMintedAt: Value(mintedAt),
          ),
        );
  }
  ...
  Future<void> upsertEvents(List<CalendarEventsTableCompanion> events) async {
    // One batched `INSERT … ON CONFLICT DO UPDATE` per event, not a SELECT
    // followed by an INSERT or UPDATE: a 500-event sync was 1,000 statements.
    // The conflict target is the existing
    // `uq_calendar_events_account_external` unique index, so SQLite resolves
    // "does this row exist?" from the index it would have used for the probe.
    //
    // `id` and `alertedAt` are excluded from the update set, which is what
    // preserves the original row id and stops a fired "starting soon" alert
    // from being replayed — the same guarantee the read-then-write gave.
    await batch((b) {
      for (final event in events) {
        b.insert(
          ...
          onConflict: DoUpdate(
            ...
            target: [
              calendarEventsTable.accountId,
              calendarEventsTable.externalEventId,
            ],
          ),
        );
      }
  ...
  /// Deletes the events whose provider ids are in [externalIds], scoped to one
  /// calendar. The incremental-sync path's deletion primitive: Google reports
  /// removals as `cancelled` instances, so there is no window to reconcile —
  /// just the explicitly named events.
  Future<int> deleteEventsByExternalIds({
    ...
    return (delete(calendarEventsTable)..where(
          (t) =>
              t.workspaceId.equals(workspaceId) &
              t.accountId.equals(accountId) &
              t.calendarId.equals(calendarId) &
              t.externalEventId.isIn(externalIds),
        ))
        .go();
  }
  ...
  Future<int> deleteEventsMissingFrom({
    ...
    return (delete(calendarEventsTable)..where((t) {
          var predicate =
              t.workspaceId.equals(workspaceId) &
              t.accountId.equals(accountId) &
              t.calendarId.equals(calendarId) &
          ...
          if (keepExternalIds.isNotEmpty) {
            predicate =
                predicate & t.externalEventId.isNotIn(keepExternalIds.toList());
          }
          return predicate;
        }))
        .go();
  ...
  Stream<CalendarEventsTableData?> watchEventById(
    ...
    String eventId,
      ...
      (select(calendarEventsTable)..where(
            (t) => t.id.equals(eventId) & t.workspaceId.equals(workspaceId),
          ))
          .watchSingleOrNull();
  ...
  Future<void> markAlerted(String workspaceId, String eventId, DateTime at) =>
      (update(calendarEventsTable)..where(
            (t) => t.id.equals(eventId) & t.workspaceId.equals(workspaceId),
          ))
  ...
  Future<void> syncLinkedMeetingTitles(String workspaceId) => customStatement(
    '''
        UPDATE meetings
        SET title = (
          SELECT e.title
          FROM meeting_calendar_links l
          JOIN calendar_events e ON e.id = l.calendar_event_id
          WHERE l.meeting_id = meetings.id AND l.workspace_id = ?
        )
        WHERE workspace_id = ?
  ...
  Future<MeetingCalendarLinksTableData?> getLinkForMeeting(
    ...
    String meetingId,
      ...
      (select(meetingCalendarLinksTable)..where(
            (t) =>
                t.meetingId.equals(meetingId) &
                t.workspaceId.equals(workspaceId),
          ))
  ...
  Future<String?> getMeetingIdForEvent(
    ...
    final row =
        await (select(meetingCalendarLinksTable)..where(
              (t) =>
  ...
  Future<CalendarEventsTableData?> getEventForMeeting(
    ...
    return (select(calendarEventsTable)..where(
          (t) =>
