...
class TicketDao extends DatabaseAccessor<WorkspaceDatabase>
  ...
  /// Updates a ticket by id, with optional optimistic concurrency.
  ...
  /// Patches ONLY the provided columns of a ticket, scoped to [workspaceId],
  /// in server receipt order — the per-column last-writer-wins primitive
  /// behind `tickets.patch` (PRD 16 §6). No expected-version gate: two
  /// concurrent field edits both land, each touching only its own columns,
  /// so neither clobbers the other. The version still bumps (inside one
  /// transaction; SQLite's single writer serializes it) so whole-row
  /// optimistic-lock updates observe the change. Returns the number of rows
  /// matched (0 = foreign/unknown ticket).
  Future<int> patchFields(
    String workspaceId,
    String id,
    TicketsTableCompanion changes,
  ) => transaction(() async {
    final row =
        await (select(ticketsTable)..where(
    ...
    return (update(
      ticketsTable,
    )..where((t) => t.id.equals(id) & t.workspaceId.equals(workspaceId))).write(
      changes.copyWith(
  ...
  Future<int> deleteTicket(String id, String workspaceId) => (delete(
    ticketsTable,
  ...
  Future<TicketsTableData?> getByExternalKey(
    ...
    String externalKey,
      ...
      (select(ticketsTable)..where(
            (t) =>
                t.provider.equals(provider) & t.externalKey.equals(externalKey),
          ))
          .getSingleOrNull();

  /// Tickets assigned to an agent, scoped to [workspaceId].
  Future<List<TicketsTableData>> forAgent(String workspaceId, String agentId) =>
      (select(ticketsTable)..where(
            (t) =>
  ...
  /// Adds a collaborator to a ticket, ignoring duplicates.
  ...
  /// Removes a collaborator (agent or user).
  Future<void> removeCollaborator(String ticketId, String principalId) =>
      (delete(ticketCollaboratorsTable)..where(
            (t) =>
                t.ticketId.equals(ticketId) & t.principalId.equals(principalId),
          ))
          .go();
