...
class IdentityMapper {
  ...
  User userToDomain(UsersTableData row) => User(
    ...
    onboardingFinishedAt: row.onboardingFinishedAt,
  ...
  /// which is exactly what happens for a `custom:<id>` role: the base preset
  /// is re-derived from the custom row by the role-definition resolver, and
  /// anything that reads only [WorkspaceMember.role] sees guest until it is.
  /// A custom role can only ever SUBTRACT from its base, so under-reading it
