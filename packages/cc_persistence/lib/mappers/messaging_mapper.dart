...
class MessagingMapper {
  ...
  Channel channelToDomain(ChannelsTableData row) => Channel(
    id: row.id,
    name: row.name,
    ...
    provisioningStep: ChannelProvisioningStep.fromDbValue(row.provisioningStep),
    pipelineRunId: row.pipelineRunId,
    origin: ChannelOrigin.fromWire(row.origin),
  );

  /// Converts a list of database channel rows to domain [Channel]s.
  List<Channel> channelsToDomain(List<ChannelsTableData> rows) =>
      rows.map(channelToDomain).toList(growable: false);
  ...
  ChannelMessage messageToDomain(ChannelMessagesTableData row) {
    ...
    switch (row.messageType) {
      ...
      case 'artifact':
        messageType = ChannelMessageType.artifact;
      case 'compaction':
