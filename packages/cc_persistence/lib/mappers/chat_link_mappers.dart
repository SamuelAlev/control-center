...
class ChatLinkMapper {
  ...
  /// Domain channel link → companion.
  ChatChannelLinksTableCompanion channelLinkToCompanion(ChatChannelLink link) =>
      ChatChannelLinksTableCompanion(
        id: Value(link.id),
        ...
        ccChannelId: Value(link.ccChannelId),
        createdByUserId: Value(link.createdByUserId),
        createdAt: Value(link.createdAt),
        lastActivityAt: Value(link.lastActivityAt),
      );

  /// Row → domain channel link, or null when the row's provider is unknown.
  ChatChannelLink? channelLinkFromRow(ChatChannelLinksTableData row) {
    final provider = ChatProvider.tryFromWire(row.provider);
    if (provider == null) {
      return null;
    }
    return ChatChannelLink(
      id: row.id,
