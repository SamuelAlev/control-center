...
class ActionPolicyMapper {
  ...
  ActionPolicyRule ruleFromRow(ActionPoliciesTableData row) => ActionPolicyRule(
    ...
    constraint: ActionConstraint.decode(row.constraintJson),
    expiresAt: row.expiresAt,
    // NULL reads as `hard` — the pre-column meaning of every stored row, and
    // the fail-closed direction for an unrecognised value.
    enforcement: EnforcementLevel.fromWire(row.enforcement),
  );
  ...
  ActionPoliciesTableCompanion ruleToCompanion(ActionPolicyRule rule) =>
      ActionPoliciesTableCompanion(
        ...
        constraintJson: Value(rule.constraint?.encode()),
        expiresAt: Value(rule.expiresAt),
        enforcement: Value(rule.enforcement.wire),
      );
}
