...
class _WebRootState extends State<_WebRoot> {
  ...
  Future<void> _boot() async {
    ...
    if (urlHints.server.isNotEmpty && (urlHints.psk?.isNotEmpty ?? false)) {
      // A fresh, complete pairing deep link → pair with its own values.
      await _pair(
        server: urlHints.server,
          ...
          child: CcCard(
            ...
            child: Column(
              ...
              children: [
                ...
                // pairing-key pair (same behavior as the desktop server-setup
                // screen): redeeming it mints this browser's own credential.
                _field(
                  t,
