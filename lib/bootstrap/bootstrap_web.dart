...
Future<void> bootstrapAndRun() async {
  ...
  // context menu enabled, which both covers our `showCcMenuAt` menus (channel
...
///    desktop (GitHub auth via keychain PAT or the server-side `gh` probe, plus
///    the server's workspace list over RPC). Connecting to a freshly-minted
...
/// round-trip. sessionStorage (not memory) because the same-tab SSO flow
/// reloads the app when the IdP redirects back — the marker must survive
/// that navigation and it must NOT survive into a new tab (where a forged
/// `#fragment` link could otherwise ride an unrelated marker).
const _ssoInFlightKey = 'cc-sso-inflight';

void _markSsoInFlight() {
  try {
    ...
    // Storage can be disabled (private modes) — worst case the SSO
    // completion falls back to the pre-filled connect form, never a failure.
  }
}

bool _ssoInFlight({Duration window = const Duration(minutes: 10)}) {
  try {
    final raw = web.window.sessionStorage.getItem(_ssoInFlightKey);
...
class _ConnectGateState extends State<_ConnectGate> {
  ...
  Future<AuthProvidersSnapshot?> _probeSso(String origin) async {
    try {
      final response = await web.window
          .fetch('$origin/auth/providers'.toJS)
          .toDart;
      final text = (await response.text().toDart).toDart;
      final snapshot = AuthProvidersSnapshot.tryParse(text);
      if (mounted) {
        setState(() {
          _auth = snapshot;
  ...
  /// wait for the completion page to postMessage the minted credential back
  /// to this tab (origin-validated — the message carries a credential).
  /// Falls back to navigating this tab when the popup is blocked; either
  /// path lands in the same pairing machinery.
  Future<void> _startSso(AuthProviderInfo provider) async {
    ...
    final loginUrl = provider.loginUrl(origin, clientOrigin: Uri.base.origin);
    // The IdP redirect (or popup completion) reloads this tab with the
    // credential fragment; mark the round-trip so `_boot` recognizes the
    // reload as the expected completion rather than an unbidden link.
    _markSsoInFlight();
    final popup = web.window.open(loginUrl, '_blank');
    if (popup == null || popup.closed) {
      // Popup blocked: same-tab round-trip, the original flow.
      web.window.location.assign(loginUrl);
      return;
    }
    _listenForPairMessage(origin);
    if (mounted) {
      setState(() {
  ...
  /// Waits for the completion tab's postMessage. Trusts NOTHING but the
  /// expected [origin] and the message shape; anything else is ignored.
  void _listenForPairMessage(String origin) {
    _removePairListener();
    _pairMessageHandler = ((web.MessageEvent event) {
      ...
      _removePairListener();
      try {
        web.window.sessionStorage.removeItem(_ssoInFlightKey);
      } on Object {
        // Best effort.
      }
      if (!mounted) {
    ...
    web.window.addEventListener('message', _pairMessageHandler!);
  }

  JSFunction? _pairMessageHandler;

  void _removePairListener() {
    final handler = _pairMessageHandler;
    if (handler != null) {
      web.window.removeEventListener('message', handler);
      _pairMessageHandler = null;
    }
  }
