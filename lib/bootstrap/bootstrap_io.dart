...
import 'package:control_center/core/notifications/notification_service_provider.dart';
...
class DesktopServerSwitcher {
  ...
  Future<void> switchTo(String serverId) async {
    ...
    final next = await DesktopBackendSession.build(
      backend: backend,
      prefs: prefs,
      secureStore: secure,
    );
...
Future<void> _handleDeepLink(ProviderContainer container, String rawUrl) async {
  switch (DeepLinkHandler.resolve(rawUrl)) {
    ...
    case ChannelDeepLink(:final workspaceId, :final channelId):
      await _goToWorkspace(
        container,
        workspaceId,
        channelRoute(workspaceId, channelId),
      );
