...
class _MjpegViewState extends State<MjpegView> {
  ...
  /// Rebuilt per connection from what the stream SAYS it carries.
  ///
  /// Not fixed at JPEG: a WebKit rig's lane is PNG (classic WebDriver has no
  /// format parameter), and a reader scanning those bytes for JPEG markers
  /// would find none and paint nothing — while every other signal said the
  /// rig was healthy. The content type is the one place the answer lives.
  RigFrameReader _reader = MjpegFrameReader();
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      // `package:http` lower-cases header names, and this file is web-safe —
      // so no `dart:io` `HttpHeaders` constant here.
      _reader = RigFrameReader.forContentType(response.headers['content-type']);
      if (mounted) {
        setState(() {
          _connecting = false;
          _error = null;
