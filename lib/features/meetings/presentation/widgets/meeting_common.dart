  ...
  Widget build(BuildContext context) {
    switch (status) {
      case MeetingStatus.done:
        return Icon(AppIcons.circleCheck, size: size, color: context.mSuccess);
      case MeetingStatus.processing:
        return MeetingEqualizerBars(
          color: context.mAccent,
          height: size * 0.78,
        );
      case MeetingStatus.recording:
        return Icon(AppIcons.mic, size: size, color: context.mDanger);
      case MeetingStatus.failed:
        return Icon(AppIcons.circleAlert, size: size, color: context.mDanger);
    }
  }
}
...
class _MeetingEqualizerBarsState extends State<MeetingEqualizerBars>
  ...
  Widget build(BuildContext context) {
    ...
    // Under reduced motion the bars render statically — actually stop the
    // controller instead of just painting a constant factor, so no per-frame
    // ticks/rebuilds keep running invisibly.
    if (reduceMotion) {
...
/// Whether a page header's action row must drop its button labels.
///
/// The header lays its actions out in a plain [Row] beside a flexible title, so
/// there is no incoming width to measure with a `LayoutBuilder` — the window is
/// the only thing that can be asked. Below this the record and detail headers
/// switch to tooltipped icon buttons, which is the difference between three
/// labeled buttons fitting and the title being squeezed to nothing.
