...
class _MeetingRecordingHudState extends ConsumerState<MeetingRecordingHud> {
  ...
  Widget build(BuildContext context) {
    ...
    // Hide the in-window HUD when the recording isn't active, when we're already
    // on the record screen, or when the floating toolbar window is out (so the
    // two never show at once).
    final hidden =
        !recorder.isRecording ||
        recorder.meetingId == null ||
        location == meetingsRecordRoute(context.currentWorkspaceId!) ||
        toolbarFloating;
...
class _HudButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (tip != null) {
      button = CcTooltip(
        message: tip,
        child: Semantics(label: tip, button: true, child: button),
      );
    }
    return button;
  }
