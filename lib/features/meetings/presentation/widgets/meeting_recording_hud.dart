...
class _MeetingRecordingHudState extends ConsumerState<MeetingRecordingHud> {
  ...
  /// The elapsed-time ticker only runs while the HUD is actually visible.
  /// This widget is mounted at the shell level for the whole session, so an
  /// unconditional periodic timer would rebuild it every second forever —
  /// even with no recording in progress.
  void _syncTicker({required bool visible}) {
  ...
  Widget build(BuildContext context) {
    ...
    final toolbarFloating = ref.watch(meetingToolbarControllerProvider);
    final location = GoRouterState.of(context).matchedLocation;

    // Hide the in-window HUD when the recording isn't active, when a meetings
    // surface already reports the recording inline (the record screen's pinned
    // header, the list's live strip), or when the floating toolbar window is
    // out — the HUD is for when you have navigated AWAY from meetings, and
    // floating it over a page that already shows the same clock and the same
    // two buttons is just a second copy of them.
    final workspaceId = context.currentWorkspaceId;
    final onMeetingsSurface =
        workspaceId != null &&
        (location == meetingsRecordRoute(workspaceId) ||
            location == meetingsRoute(workspaceId));
    final hidden =
        !recorder.isRecording ||
        recorder.meetingId == null ||
        onMeetingsSurface ||
        toolbarFloating;
    _syncTicker(visible: !hidden);
    if (hidden) {
      return const SizedBox.shrink();
    }

    final meetingId = recorder.meetingId!;
    const hudWidth = 332.0;
...
class _HudButton extends StatelessWidget {
  ...
  /// Button label; when null the button is icon-only (use [tooltip] for the
  /// accessible name and hover affordance).
  final String? label;
  ...
  Widget build(BuildContext context) {
    ...
    final tip = tooltip;
    if (tip != null) {
      button = CcTooltip(message: tip, child: button);
    }
    return button;
  }
}
