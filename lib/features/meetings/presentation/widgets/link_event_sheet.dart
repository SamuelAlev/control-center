...
class _LinkEventSheetState extends ConsumerState<LinkEventSheet> {
  ...
  Future<void> _link(String eventId) async {
    ...
    try {
      await ref
          .read(linkMeetingToEventUseCaseProvider)
          .link(
  ...
  Future<void> _unlink() async {
    ...
    try {
      await ref
          .read(linkMeetingToEventUseCaseProvider)
          .unlink(workspaceId: widget.workspaceId, meetingId: widget.meetingId);
      if (mounted) {
        Navigator.of(context).pop();
      }
    } on Object {
...
class _EventRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CcTile(
      onTap: onTap,
      selected: selected,
      leading: Icon(AppIcons.calendar, size: 15, color: ds.accent),
      title: Text(
        title,
        maxLines: 1,
      ...
      subtitle: AppTimestamp(
        dateTime: startTime,
        child: Text(subtitle, style: meetingMono(context, fontSize: 11)),
      ),
      trailing: selected
          ? Icon(AppIcons.check, size: 16, color: ds.accent)
          : null,
    );
  }
}
