...
class MeetingActionItemsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (actionItems.isEmpty) {
      return MeetingTabEmptyState(
        icon: AppIcons.listChecks,
        message: l10n.meetingActionItemsEmpty,
        actionLabel: l10n.meetingAddActionItem,
    ...
    return showCcDialog<void>(
      ...
      builder: (_) => MeetingActionItemDialog(
        ...
        onSubmit: (content, owner) {
          ref
              .read(meetingRecorderControllerProvider.notifier)
              .updateActionItem(item.id, content: content, owner: owner);
        },
      ),
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          // The design system's own checkbox, not a hand-rolled one: it carries
          // the shared hover/press/focus treatment, keyboard activation and the
          // ≥32px tap target the bespoke 18px box did not.
          Padding(
            padding: const EdgeInsets.only(top: 1),
            child: CcCheckbox(
              value: done,
              onChanged: (_) => _toggleDone(ref),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Icon(AppIcons.user, size: 12, color: ds.muted),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(AppIcons.ticket, size: 12, color: context.mSuccess),
          const SizedBox(width: 5),
          Text(
            label,
            style: meetingMono(context, fontSize: 11, color: context.mSuccess),
          ),
        ],
      ),
    );
