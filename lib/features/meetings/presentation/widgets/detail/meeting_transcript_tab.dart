...
class _MeetingTranscriptTabState extends ConsumerState<MeetingTranscriptTab> {
  ...
  /// The filtered row list, memoized on the inputs it derives from.
  ///
  /// Recomputed in `build`, this ran `segments.where(_matches)` — a
  /// `toLowerCase()` per segment — on EVERY build, including the sub-second
  /// playback ticks. A two-hour meeting is ~2,000 segments.
  List<MeetingSegment>? _rowsMemo;
  List<MeetingSegment>? _rowsMemoSource;
  String? _rowsMemoQuery;
  MeetingSpeakerFilter? _rowsMemoSpeaker;

  List<MeetingSegment> _filteredRows() {
    final cached = _rowsMemo;
    if (cached != null &&
        identical(_rowsMemoSource, widget.segments) &&
        _rowsMemoQuery == _query &&
        _rowsMemoSpeaker == _speaker) {
      return cached;
    }
    final rows = widget.segments.where(_matches).toList(growable: false);
    _rowsMemo = rows;
    _rowsMemoSource = widget.segments;
    _rowsMemoQuery = _query;
    _rowsMemoSpeaker = _speaker;
    return rows;
  }
  ...
  Future<void> _renameSpeaker({
    ...
    if (embedding != null && embedding.isNotEmpty) {
      final profiles = ref
          .read(voiceProfilesProvider(widget.workspaceId))
          .asData
          ?.value;
      if (profiles != null) {
  ...
  Widget build(BuildContext context) {
    ...
    // Sync with audio playback (#8): the line currently playing is highlighted,
    // scrolled into view, and tapping any line seeks the audio there.
    //
    // This build watches only `ready` — a value that changes twice per
    // session. The POSITION is watched per row, through a `select` over a
    // predicate that answers "am I the active line?", so a sub-second tick
    // rebuilds only the two rows whose answer actually flipped instead of
    // rebuilding the whole transcript several times a second.
    final playbackReady = ref.watch(
      meetingPlaybackProvider.select((p) => p.ready),
    );
            ...
            if (rows.isEmpty)
              ...
              for (var i = 0; i < rows.length; i++) ...[
                ...
                () {
                  ...
                  return _PlaybackAwareRow(
                    key: ValueKey(segment.id),
                    segment: segment,
                    // The NEXT line's start is what bounds this one: the active
                    // line is the last whose start has been reached.
                    nextStartMs: i + 1 < rows.length
                        ? rows[i + 1].startMs
                        : null,
                    query: _query,
                    // A per-line override wins over the speaker's group name.
                    speakerName:
                        segment.speakerNameOverride ?? speaker?.displayName,
...
/// One transcript line that tracks its OWN playback state.
///
/// The parent used to watch the whole playback state and rebuild every line on
/// each sub-second tick. This watches a `select` over a predicate that answers
/// only "am I the line currently playing?", so Riverpod rebuilds a row solely
/// when that boolean flips — two rows per line change instead of all of them.
class _PlaybackAwareRow extends ConsumerStatefulWidget {
  const _PlaybackAwareRow({
  ...
  /// The following line's start, or null for the last line. The active line is
  /// the last whose start has been reached, which is exactly
  ...
  @override
  ConsumerState<_PlaybackAwareRow> createState() => _PlaybackAwareRowState();
}

class _PlaybackAwareRowState extends ConsumerState<_PlaybackAwareRow> {
  bool _wasActive = false;

  @override
  Widget build(BuildContext context) {
    final start = widget.segment.startMs;
    final next = widget.nextStartMs;
    final isActive = ref.watch(
      meetingPlaybackProvider.select(
        (p) =>
            p.ready &&
            p.positionMs >= start &&
            (next == null || p.positionMs < next),
      ),
    );

    // Scroll-into-view lives with the row that becomes active, so it needs no
    // GlobalKey and no index bookkeeping in the parent — the row already has
    // the context it wants to reveal.
    ...
    return MeetingTranscriptRow.fromSegment(
      ...
      active: isActive,
      onTap: widget.playbackReady
          ? () => ref
                .read(meetingPlaybackProvider.notifier)
                .seekToMs(widget.segment.startMs)
          : null,
...
class _RenameSpeakerDialogState extends State<_RenameSpeakerDialog> {
  ...
  void _submit() => Navigator.of(
    context,
  ).pop(_RenameSpeakerResult(name: _controller.text, applyToAll: _applyToAll));
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          CcSegmentedToggle<MeetingSpeakerFilter>(
            value: speaker,
            ...
            segments: [
              ...
              CcSegment(
                value: MeetingSpeakerFilter.me,
                label: l10n.meetingSpeakerMe,
              ),
              CcSegment(
