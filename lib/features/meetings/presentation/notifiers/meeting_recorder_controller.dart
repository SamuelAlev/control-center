...
import 'package:control_center/features/meetings/domain/services/meeting_echo_filter.dart';
...
class MeetingRecorderController extends Notifier<MeetingRecorderState> {
  ...
  Future<void> updateTitle(String meetingId, String title) async {
    ...
    await repo.upsert(
      // The user editing the title marks it custom, so a linked calendar event
      // never overwrites it on a later sync.
      meeting.copyWith(
        title: trimmed,
        titleIsCustom: true,
        updatedAt: DateTime.now(),
  ...
  /// Sort-order base for user-added action items / decisions. Agent-extracted
  /// rows use small per-item sort orders (0, 1, 2, …); reserving a high base
  /// keeps user-added rows sorted after the agent's, in creation order (the
  /// list query tie-breaks equal sort orders by `createdAt`) — even after a
  /// re-run regenerates more agent rows than there were before.
  ...
  Future<void> addActionItem(
    ...
    final repo = ref.read(meetingRepositoryProvider);
    // Confirm the meeting belongs to this workspace before inserting, so a
    // stray id can't seed a row into the wrong workspace.
    final meeting = await repo.getById(workspaceId, meetingId);
    if (meeting == null) {
      return;
    }
  ...
  /// Edits an action item's [content] + [owner]. Marks it `isManual` so a
  /// re-run won't overwrite the edit.
