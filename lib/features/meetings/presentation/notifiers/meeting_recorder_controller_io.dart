...
class MeetingRecorderController extends Notifier<MeetingRecorderState> {
  ...
  Future<void> start({
    ...
    try {
      ...
      // reference). In-person mode has no far-end, so no engine (identity
      // passthrough). In remote mode the factory THROWS when the native AEC
      // library can't load — the recording start fails loudly instead of
      // silently streaming an echo-polluted mic.
      final AecEngine? aecEngine = captureSystem
          ? ref.read(aecProcessorFactoryProvider)()
          : null;
      ...
      AppLog.i(
        ...
        aecEngine is AecProcessor
            ...
            : 'AEC inactive (in-person mode — no far-end reference)',
      );

      // mic → log/health on the RAW input (so dead-mic detection sees the true
      // signal) → AEC clean → stream the cleaned mic as `me`. The capture is
      // pinned to the input device selected in You → Audio (falling back to
      // the OS default when unset or unplugged) — the same device the mic test
      // and dictation resolve through, so what you verified is what records.
      final micConfig = await withSelectedInputDevice(
        mic,
        ref.read(audioInputDeviceProvider),
        _micConfig,
      );
      final micRaw = _logFrames(await mic.startStream(micConfig), 'mic/me');
      ...
      if (strandedId != null) {
        try {
          ...
          AppLog.w(
            'MeetingRecorder',
            'cleanup: stop stranded recording failed: $e',
          );
        }
        if (strandedWorkspaceId != null) {
          try {
            ...
            AppLog.w(
              'MeetingRecorder',
              'cleanup: delete stranded meeting failed: $e',
            );
          }
        }
      }
    ...
    if (meetingId != null) {
      try {
        await ref
            ...
            .stopRecording(
              meetingId: meetingId,
              summaryInstructions: ref
                  .read(activeMeetingTemplateProvider)
                  .instructions,
            );
      } catch (e) {
  ...
  Future<void> cancelProcessing(String meetingId) async {
    ...
    if (active != null) {
      await ref.read(pipelineEngineProvider).cancel(workspaceId, active.id);
    }
  }
  ...
  Future<void> addDecision(String meetingId, {required String content}) async {
    final text = content.trim();
    if (text.isEmpty) {
      return;
    }
  ...
  /// overwrite the edit.
  Future<void> updateDecision(String id, {required String content}) async {
    final text = content.trim();
    if (text.isEmpty) {
      return;
    }
    ...
    await ref
        .read(meetingRepositoryProvider)
        .updateDecision(workspaceId: workspaceId, id: id, content: text);
  }
  ...
  Future<void> _teardownCaptures() async {
    ...
    try {
      ...
      AppLog.w('MeetingRecorder', 'teardown: mic stop failed: $e');
    }
    await _mic?.dispose();
    _mic = null;
    try {
      ...
      AppLog.w('MeetingRecorder', 'teardown: system-audio stop failed: $e');
    }
    _system = null;
  }
}
