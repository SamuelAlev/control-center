...
/// Meetings home: the standard page header carrying the record CTA and the
/// "processing now" live pill, then capture stats, the "armed" capture
/// banner, and the day-grouped meeting list with status filters and search.
...
class _MeetingsScreenState extends ConsumerState<MeetingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final processingCount = (meetingsAsync.asData?.value ?? const <Meeting>[])
        .where(
          (m) =>
  ...
  Widget _buildBody(
    ...
    final decisions = decisionCounts.values.fold<int>(0, (sum, c) => sum + c);
    final openActions = actionStats.values.fold<int>(
      0,
            ...
            child: Column(
              ...
              children: [
                ...
                _ListPanel(
                  ...
                  onRecord: _startRecording,
                  onResume: () => context.go(
                    meetingsRecordRoute(context.currentWorkspaceId!),
                  ),
                ),
              ],
            ),
...
class _ListPanel extends StatelessWidget {
  ...
  /// The unfiltered meeting count — when zero, the empty state invites the
  /// user to record their first meeting instead of blaming the filters.
    ...
    if (meetings.isEmpty) {
      ...
      for (final bucket in MeetingDayBucket.values) {
        ...
        for (var i = 0; i < rows.length; i++) {
          if (i > 0) {
            children.add(CcDivider(color: ds.borderSecondary));
          }
          children.add(
            MeetingListRow(
              meeting: rows[i],
...
/// Shown when no meeting has ever been recorded: an inviting pitch plus the
/// record CTA, rather than the "nothing matches" filter state.
      ...
      child: Column(
        children: [
          ...
          ConstrainedBox(
            ...
            child: Text(
              l10n.meetingsEmptyHint,
              textAlign: TextAlign.center,
              style: TextStyle(height: 1.4, color: ds.muted),
            ),
          ),
          const SizedBox(height: AppSpacing.lg),
          _RecordButton(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 6),
          Text(l10n.meetingsNoMatchHint, style: TextStyle(color: ds.muted)),
        ],
      ),
    );
  }
