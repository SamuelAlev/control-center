...
class _TabLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        Text(label),
        if (count != null) ...[const SizedBox(width: 6), _TabCount(count!)],
      ],
    );
  }
}
...
/// A secondary page-header action that keeps its label while there is room for
/// it and falls back to a tooltipped icon button when there is not, so three
/// actions and a title never fight over the same row.
...
/// The page overline: a link back to the meetings list. The detail screen used
/// to have no way back at all except the shell breadcrumb.
class _BackLine extends StatelessWidget {
  const _BackLine({required this.l10n});
  ...
  Widget build(BuildContext context) {
    return Align(
      ...
      child: CcButton(
        ...
        size: CcButtonSize.sm,
        onPressed: () => context.go(meetingsRoute(context.currentWorkspaceId!)),
...
/// The meeting title in the page-header slot, editable in place.
///
/// It renders through [PageHeaderText], so it carries exactly the typography
/// every other page title does rather than the bespoke 38px headline this
/// screen used to set for itself.
class _DetailTitle extends StatelessWidget {
  const _DetailTitle({required this.meeting, required this.onEditTitle});
...
/// The meeting's metadata line: its status (glyph + label, never color alone),
/// when it happened, how long it ran, what it was captured from and the
/// calendar event it belongs to.
class _DetailMeta extends StatelessWidget {
  const _DetailMeta({required this.meeting});
  ...
  Widget build(BuildContext context) {
    ...
    final when =
        '$whenPrefix · ${DateFormat.Hm(locale).format(meeting.startedAt)}';
    ...
    final statusLabel = switch (meeting.status) {
      MeetingStatus.done => l10n.meetingStatusDone,
      MeetingStatus.processing => l10n.meetingStatusProcessing,
      MeetingStatus.recording => l10n.meetingStatusRecording,
      MeetingStatus.failed => l10n.meetingStatusFailed,
    };

    Widget sep() => Text('·', style: meetingMono(context, fontSize: 12));
    ...
    return Wrap(
      ...
      children: [
        MeetingStatusGlyph(status: meeting.status, size: 14),
        Text(statusLabel, style: meetingMono(context, fontSize: 12)),
        sep(),
        AppTimestamp(
          dateTime: meeting.startedAt,
          child: Text(when, style: meetingMono(context, fontSize: 12)),
        ),
        sep(),
        ...
        Consumer(
          builder: (context, ref, _) {
            ...
            return _FromCalendarChip(
              title: event.title,
              onTap: () => context.go(
                calendarDetailRoute(context.currentWorkspaceId!, event.id),
              ),
            );
          },
        ),
...
class _LinkEventButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _HeaderAction(
      ...
      label: linked ? l10n.meetingChangeEvent : l10n.meetingLinkEvent,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(label, style: TextStyle(fontSize: 13, color: ds.fg)),
          ),
        ],
      ),
    );
