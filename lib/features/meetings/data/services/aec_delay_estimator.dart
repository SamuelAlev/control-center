...
class AecDelayEstimator {
  ...
  AecDelayEstimator({
    ...
    if (!(binMs > 0)) {
      throw ArgumentError('invalid argument: binMs > 0');
    }
    if (!(windowMs > 0)) {
      throw ArgumentError('invalid argument: windowMs > 0');
    }
    if (!(maxLagMs > 0)) {
      throw ArgumentError('invalid argument: maxLagMs > 0');
    }
  }
  ...
  /// Minimum near-space envelope standard deviation to attempt an estimate —
  ...
  AecDelayEstimate? estimate() {
    ...
    if (bestCorr <= -2.0) {
      return null; // far space had no variance anywhere
    }
  ...
  /// jittery space may never produce.
  bool get hasRepeatedSupport {
    if (_recent.length < 3) {
      return false;
    }
    final recent5 = _recent.length <= 5
        ? _recent
