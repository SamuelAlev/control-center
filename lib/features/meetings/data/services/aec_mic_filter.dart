...
class AecMicFilter implements MicEchoCanceller {
  ...
  /// omitted, AEC runs with a zero stream delay and no buffering. [_log] receives
  /// human-readable calibration / metrics lines for the recorder's diagnostics.
  AecMicFilter({AecEngine? processor, int Function()? clockNow, this._log})
  ...
  // Reference-availability gate. AEC3 expects one render (far) frame per capture
  // (near) frame; if the loopback stalls, the render buffer would go stale and
  // AEC3 would cancel against an old echo. We track render vs. capture frames
  // fed and zero-pad the reference whenever the far-end falls behind, keeping
  // the two streams aligned. The counters surface in the diagnostics log.
  int _farBlocksFed = 0;
  int _nearBlocksProcessed = 0;
  int _refMatched = 0;
  int _refZeroPadded = 0;
  late final Uint8List _silenceBlock = Uint8List(AecProcessor.blockBytes);
    ...
    return _wrap(micRaw, (chunk, emit) {
      ...
      _near.add(chunk, (block) {
        ...
        while (_nearQueue.length > _nearBufferBlocks) {
          // Reference-availability gate: if the far-end (loopback) has stalled
          // so the render buffer is behind the capture, zero-pad the reference
          // to keep AEC3 aligned instead of cancelling against a stale echo.
          if (_farBlocksFed <= _nearBlocksProcessed) {
            proc.processReverse(_silenceBlock);
            _farBlocksFed++;
            _refZeroPadded++;
          } else {
            _refMatched++;
          ...
          out.add(
            proc.processCapture(_nearQueue.removeFirst(), _streamDelayMs),
          );
          _nearBlocksProcessed++;
        }
      });
      if (out.length > 0) {
        emit(out.takeBytes());
  ...
  void _maybeCalibrate() {
    ...
    final est = estimator.estimateSmoothed();
    if (est != null) {
      // Lock when a single measurement is strongly confident OR when the recent
      // estimates consistently agree (the repeated-support tier) — a jittery /
      // high-latency call may never produce one strong tick but still cluster.
      if (!_locked &&
          (est.confidence >= _minLockConfidence ||
              estimator.hasRepeatedSupport)) {
  ...
  void _logStatus(AecDelayEstimate? est) {
    ...
    _log?.call(
      ...
      '| ref matched $_refMatched zero-padded $_refZeroPadded',
    );
  }

  /// Diagnostics: capture frames processed with a real, matched far-end
  /// reference available.
  int get referenceFramesMatched => _refMatched;

  /// Diagnostics: capture frames whose far-end reference was zero-padded because
  /// the loopback had stalled (the render buffer fell behind the capture).
  int get referenceFramesZeroPadded => _refZeroPadded;
...
class _BlockAccumulator {
  ...
  void add(Uint8List chunk, void Function(Uint8List block) onBlock) {
    ...
    _carry = off == data.length
        ? Uint8List(0)
        : Uint8List.sublistView(data, off);
  }
}
