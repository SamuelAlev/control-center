...
class _RepoIndexButtonState extends ConsumerState<RepoIndexButton> {
  static const String _templateId = IndexCodeTemplate.id;
    ...
    try {
      final run = await ref
          ...
          .start(
            ...
            triggerPayload: {
              'repo_id': widget.repo.id,
              'repo_local_path': widget.repo.path,
            },
      ...
      AppLog.e(
        'RepoIndex',
        'failed to start indexing ${widget.repo.id}',
        e,
        st,
      );
      if (mounted) {
  ...
  Future<void> _cancel() async {
    ...
    await ref
        .read(pipelineEngineProvider)
        .cancel(ref.requireWorkspaceId(), runId);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_starting && _runId == null) {
      return const CcSpinner(size: 14);
    }
    ...
    // Waiting behind the template's concurrency cap. A spinner here would
    // report work that is not happening — the run has not started and there is
    // no progress to show — so it reads as a wait, with the same cancel.
    ...
    if (run == null || !run.isTerminal) {
      final progress = ref
          .watch(repoIndexProgressProvider(runId))
          .asData
          ?.value;
      return Row(
        mainAxisSize: MainAxisSize.min,
    ...
    final summary = run.state['index_summary'];
    final nativeAvailable =
        summary is Map && summary['nativeAvailable'] == true;
