...
({List<TimeBucket> buckets, ObsBucketKind kind}) bucketizeSeries(
  ...
  switch (range) {
    case ObsTimeRange.last24h:
      return (buckets: calc.hourlySeries(runs), kind: ObsBucketKind.hour);
    case ObsTimeRange.last7d:
    case ObsTimeRange.last30d:
      return (buckets: calc.dailySeries(runs), kind: ObsBucketKind.day);
    case ObsTimeRange.all:
      final daily = calc.dailySeries(runs);
      if (daily.length > 62) {
