...
class _WorkerTile extends ConsumerWidget {
  ...
  List<Widget> _actions(
    ...
    final isOnline = status == WorkerStatus.online;
    final isDraining = status == WorkerStatus.draining;
    final isRevoked = status == WorkerStatus.revoked;
    return [
      if (isOnline)
        CcButton(
...
// Exhaustive over the domain enum: a new WorkerStatus is a COMPILE error here
// rather than a silently-neutral badge (which is what the old raw-string
// switches with a `_ =>` fallthrough gave).
CcStatusTone _workerStatusTone(WorkerStatus status) => switch (status) {
  WorkerStatus.online => CcStatusTone.positive,
  WorkerStatus.draining => CcStatusTone.caution,
  WorkerStatus.revoked => CcStatusTone.negative,
  WorkerStatus.offline || WorkerStatus.incompatible => CcStatusTone.neutral,
};

String _workerStatusLabel(AppLocalizations l10n, WorkerStatus status) =>
    switch (status) {
      WorkerStatus.online => l10n.fleetStatusOnline,
      WorkerStatus.draining => l10n.fleetStatusDraining,
      WorkerStatus.offline => l10n.fleetStatusOffline,
      WorkerStatus.incompatible => l10n.fleetStatusIncompatible,
      WorkerStatus.revoked => l10n.fleetStatusRevoked,
    };
