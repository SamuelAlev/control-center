...
/// `debug` — the op, and where the debuggee is stopped.
///
/// A stopped frame is the single most useful thing on the card, so the summary
/// leads with the location rather than the op: "which line is it sitting on"
/// is what someone scanning a transcript wants, and the op is already implied
/// by the arguments beside it.
class DebugRenderer extends ToolRenderer {
  /// Creates a [DebugRenderer].
  const DebugRenderer();

  @override
  Widget summary(BuildContext context, ToolSegment s) {
    final op = _str(s.inputs, ['op', 'action']);
    final file = _str(s.inputs, ['file', 'program']);
    final lines = s.inputs?['lines'];
    final where = lines is List && lines.isNotEmpty
        ? '${file.split('/').last}:${lines.join(',')}'
        : file.split('/').last;
    return Row(
      children: [
        const ToolChip(label: 'debug'),
        const SizedBox(width: AppSpacing.sm),
        Expanded(
          child: _summaryText(
            context,
