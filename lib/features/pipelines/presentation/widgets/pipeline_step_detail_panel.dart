...
class PipelineStepDetailPanel extends ConsumerWidget {
  ...
  /// The step's title with its `{{placeholders}}` already resolved against the
  /// run. Null outside a run, where the raw label is the honest thing to show.
  final String? label;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        if (stepRun.priorAttempts.isNotEmpty)
          _MetaRow(
            label: l10n.pipelineStepAttemptLabel,
            value: '${stepRun.priorAttempts.length + 1}',
            tokens: tokens,
          ),
        if (stepRun.priorAttempts.isNotEmpty) ...[
          const SizedBox(height: AppSpacing.md),
          _PriorAttemptsSection(
            attempts: stepRun.priorAttempts,
            tokens: tokens,
            l10n: l10n,
          ),
        ],
        if (stepRun.spaceId != null) ...[
          const SizedBox(height: AppSpacing.md),
          CcButton(
            ...
            onPressed: () {
              ...
              // point, since these spaces are filtered out of the sidebar.
              // The URL is the source of truth; MessagingScreen mirrors it into
              // the selection provider.
              GoRouter.of(
                context,
              ).go(spaceRoute(context.currentWorkspaceId!, stepRun.spaceId!));
            },
...
/// The step's previous tries, oldest first, archived when a retry (or a
/// crash-resume) re-opened the row. This is the flake trail: each tile keeps
/// the status, timing and error of one superseded firing, so a failure stays
/// visible after the Retry button moved the row past it.
class _PriorAttemptsSection extends StatelessWidget {
  const _PriorAttemptsSection({
    required this.attempts,
    required this.tokens,
    required this.l10n,
  });

  final List<PipelineStepAttempt> attempts;
  final DesignSystemTokens tokens;
  final AppLocalizations l10n;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          l10n.pipelineStepPriorAttempts,
          style: TextStyle(
...
/// One superseded firing: an attempt number, its outcome (status — or
/// "Interrupted" when it never settled) and duration, with the failure it
/// ended in quoted below.
class _PriorAttemptTile extends StatelessWidget {
  const _PriorAttemptTile({
  ...
  final PipelineStepAttempt attempt;
  final int number;
  final DesignSystemTokens tokens;
  final AppLocalizations l10n;
  ...
  (IconData, Color, String) _status() {
    if (attempt.wasInterrupted) {
      return (
        ...
        tokens.textTertiary,
        l10n.pipelineStepAttemptInterrupted,
      );
    }
    final c = pipelineStepStatusColors(attempt.status, tokens);
    return (
      pipelineStepStatusIcon(attempt.status),
      c.foreground,
      pipelineStepStatusLabel(attempt.status, l10n),
    );
  }

  @override
  Widget build(BuildContext context) {
    final (icon, color, statusLabel) = _status();
    final finishedAt = attempt.finishedAt;
    return Container(
        ...
        children: [
          Row(
            children: [
              ...
              Flexible(
                child: Text(
                  '${l10n.pipelineStepAttemptN(number)} · $statusLabel',
                  maxLines: 1,
              ...
              if (finishedAt != null) ...[
                ...
                Text(
                  formatPipelineDuration(
                    finishedAt.difference(attempt.startedAt),
                  ),
                  style: TextStyle(
...
/// matched on the TEMPLATE step id — never the step-RUN row's uuid, which
/// matches nothing and left this silently empty. Surfaces each run's status,
/// summary, duration and cost — the "per-step live logs" observability gap
/// (§199): the step-runs stream already updates status/output live and this
/// adds the agent execution behind it.
class _AgentActivitySection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final logs = (logsAsync.asData?.value ?? const <AgentRunLog>[])
        .where((l) => l.pipelineStepId == stepRun.stepId)
        .toList();
    if (logs.isEmpty) {
      return const SizedBox.shrink();
    }
...
class _JsonBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      // No horizontal scroll view: the tree wraps inside the width it is given
      // (see [JsonTreeView]). Wrapping it in one made every long value — a
      // repo path, a PR title — render clipped at the panel edge with no
      // scrollbar and no mouse-drag to reach the rest.
