...
class NodeConfigEditor extends StatefulWidget {
  ...
  /// Repos in the active workspace (for the channel-clone scope selector
  /// shown on nodes that start a conversation).
...
class _NodeConfigEditorState extends State<NodeConfigEditor> {
  ...
  /// The repos this node's conversation clones when the node starts one.
  /// Mirrors the config's `repoIds` restricted to repos that still exist in
  /// the workspace; an empty config means "all workspace repos", so the set
  /// then defaults to every repo.
  late Set<String> _selectedRepoIds;

  /// Configured `repoIds` entries that are NOT workspace repo ids — template
  /// placeholders like `{{repoId}}` resolved at run time. They are preserved
  /// verbatim across edits (removing one is only possible by editing JSON),
  /// because reclassifying them as a picker selection would silently freeze a
  /// dynamic scope into a static one.
  late List<String> _dynamicRepoEntries;
  ...
  void didUpdateWidget(covariant NodeConfigEditor old) {
    ...
    if (old.step.id != widget.step.id) {
      _disposeControllers();
      _initControllers();
    } else if (old.workspaceRepos.isEmpty && widget.workspaceRepos.isNotEmpty) {
      setState(_reclassifyRepoEntries);
    }
  }
  ...
  /// Splits a configured repo scope into picker-known ids and dynamic
  /// (placeholder) entries, defaulting an empty scope to "all repos selected"
  /// — the same "empty means all" convention as channel creation.
  void _initRepoSelection(List<String> configured) {
    final knownIds = {for (final r in widget.workspaceRepos) r.id};
    _dynamicRepoEntries = [
      for (final entry in configured)
        if (!knownIds.contains(entry)) entry,
    ];
    _selectedRepoIds = configured.isEmpty
        ? knownIds
        : {for (final entry in configured) if (knownIds.contains(entry)) entry};
  }

  /// Repos can arrive after the panel first builds (the provider resolves
  /// asynchronously): entries initially parked as "dynamic" because no repo
  /// list was loaded yet move back into the picker once they resolve to a
  /// real workspace repo.
  void _reclassifyRepoEntries() {
    final knownIds = {for (final r in widget.workspaceRepos) r.id};
    final stillDynamic = <String>[];
    for (final entry in _dynamicRepoEntries) {
      if (knownIds.contains(entry)) {
        _selectedRepoIds.add(entry);
      } else {
        stillDynamic.add(entry);
      }
    }
    _dynamicRepoEntries = stillDynamic;
  }
  ...
  void _emit({String? agentId}) {
    ...
    // Channel-clone scope. "Every workspace repo selected and no dynamic
    // entries" normalizes back to the empty list = the provisioner's default
    // (clone all), so the config only carries real subsets and placeholders.
    final allRepoIds = {for (final r in widget.workspaceRepos) r.id};
    final selectsEverything =
        _selectedRepoIds.length == allRepoIds.length &&
        _selectedRepoIds.containsAll(allRepoIds);
    final List<String> repoIds;
    if (_dynamicRepoEntries.isEmpty && selectsEverything) {
      repoIds = const [];
    } else {
      repoIds = [
        ..._dynamicRepoEntries,
        for (final r in widget.workspaceRepos)
          if (_selectedRepoIds.contains(r.id)) r.id,
      ];
    }
    widget.onChange(
  ...
  Widget build(BuildContext context) {
    ...
    // Bodies that dispatch agents through dispatchConversationStep: when no
    // existing conversation is configured they create a hidden one, which is
    // when the repo scope applies.
    final startsConversation =
              ...
              child: Column(
                ...
                children: [
                  CcMultiSelect<String>(
                    ...
                    onChanged: (next) {
                      setState(() => _selectedRepoIds = next);
                      _emit();
                    },
                  ),
                  if (_dynamicRepoEntries.isNotEmpty) ...[
                    const SizedBox(height: 6),
                    Text(
                      l10n.nodeConfigReposDynamic(
                        _dynamicRepoEntries.join(', '),
                      ),
...
class _Labeled extends StatelessWidget {
  const _Labeled({required this.label, required this.child, this.description});
