...
class _NodeConfigEditorState extends State<NodeConfigEditor> {
  ...
  void _emit({String? agentId}) {
    ...
    if (schemaText.isEmpty) {
      ...
      try {
        ...
        if (decoded is Map<String, dynamic>) outputSchema = decoded;
      } on FormatException {
        // keep previous value
      }
    }
        ...
        children: [
          ...
          if (usesAgent) ...[
            ...
            FSelect<String>.search(
              items: agentItems,
              filter: (query) {
                if (query.isEmpty) return agentItems.values;
  ...
  List<Widget> _routeKeyEditors(
    ...
    final routerEdges = _edges.keys
        ...
        .toList();
    if (routerEdges.isEmpty) return const [];
    return [
      const SizedBox(height: 12),
      Text(
