...
class PipelineGraphLayout {
  ...
  ///
  /// [nodeWidths] carries the rendered width of every step id (the run canvas
  /// measures titles and widens nodes up to a cap). Each column is exactly as
  /// wide as its widest node and column x offsets accumulate, so a long
  /// title widens only its own column instead of the whole grid.
  static Map<String, Offset> compute(
    List<PipelineStepDefinition> steps, {
    required Map<String, double> nodeWidths,
    ...
    // Column x offsets accumulate: a column starts right after the previous
    // column's widest node (+ the inter-column gap).
    var columnX = 0.0;
    for (final col in columnKeys) {
      ...
      for (var i = 0; i < count; i++) {
        final id = ordered[i];
        rowIndex[id] = i;
        positions[id] = Offset(columnX, (i - (count - 1) / 2) * rowPitch);
      }
      columnX += colWidth + columnGap;
    }
