...
class _PipelineCanvasState extends ConsumerState<PipelineCanvas> {
  ...
  Widget build(BuildContext context) {
    ...
    // Node labels are templates (`Code analysis · {{repo_name}}`), so a run's
    // canvas resolves them against that run rather than printing the braces.
    final run = widget.runId == null
        ? null
        : ref.watch(pipelineRunProvider(widget.runId!)).asData?.value;
    ...
    final labels = {
      for (final step in renderable)
        step.id: renderStepLabel(
          step.config.label,
          state: run?.state ?? const {},
