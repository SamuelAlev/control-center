...
class _PipelineCanvasState extends ConsumerState<PipelineCanvas> {
            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  // Read-only run view: derive a layered left-to-right layout
                  // from the graph (trigger leftmost, columns spaced so tiles
                  // never overlap) rather than trusting the stored editor
                  // coordinates.
  ...
  /// Base translation before the user's pan: pin the leftmost (trigger) column
  /// near the left edge and center the graph vertically. We deliberately do
  /// NOT center horizontally — for a wide pipeline the trigger must stay
  /// visible on the left at the start, and the operator pans right to follow
  /// the flow.
