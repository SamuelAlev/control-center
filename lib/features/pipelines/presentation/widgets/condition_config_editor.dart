...
class _ConditionConfigEditorState extends State<ConditionConfigEditor> {
  ...
  void initState() {
    ...
    _baseKeyCtrl = TextEditingController(text: 'repo_local_path');
  ...
  void _hydrate(Map<String, dynamic> extras) {
    ...
    if (predicate is Map) {
      ...
      if (type == 'and') {
        ...
        _baseKeyCtrl.text = (first?['baseKey'] as String?) ?? 'repo_local_path';
      ...
      if (type == 'or') {
        ...
        _pathsCtrl.text = _pathList(
          predicate['paths'],
          predicate['path'],
        ).join('\n');
        _baseKeyCtrl.text =
            (predicate['baseKey'] as String?) ?? 'repo_local_path';
    ...
    if (extras['switchKey'] is String) {
      ...
      _casesCtrl.text = ((extras['cases'] as List?)?.cast<String>() ?? const [])
          .join(', ');
  ...
  List<String> _pathList(Object? paths, Object? singlePath) => [
    ...
    if (singlePath is String && singlePath.trim().isNotEmpty) singlePath.trim(),
  ];
      ...
      children: [
        ...
        if (_mode == _ConditionMode.switchOn) ...[
          ...
          _Labeled(
            ...
            child: CcTextField(controller: _switchKeyCtrl, hintText: 'pr_class'),
          ),
          const SizedBox(height: 12),
          _Labeled(
