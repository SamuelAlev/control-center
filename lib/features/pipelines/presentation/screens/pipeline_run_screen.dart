...
class _PipelineRunScreenState extends ConsumerState<PipelineRunScreen> {
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _PipelineTile(
                      ...
                      onTap: () =>
                          setState(() => _selectedTemplateId = p.templateId),
                    );
                  },
                ),
              ),
  ...
  Future<void> _run({
    ...
    try {
      ...
      final dedupKey = templateId == 'index_code'
          ? values['repo_id'] as String?
          : null;
      final run = await engine.start(
        templateId,
        workspaceId: workspaceId,
        triggerEventType: 'manual',
        triggerPayload: {...values, 'workspace_id': workspaceId},
        dedupKey: dedupKey,
      );
      if (!mounted) {
        return;
    ...
    for (final input in widget.inputs) {
      switch (input.type) {
        ...
        case PipelineInputType.select:
          ...
          _selects[input.key] = (def != null && input.options.contains(def))
              ? def
              : null;
        case PipelineInputType.repo:
          // Pre-select when the workspace has exactly one repo.
          _repos[input.key] = widget.repos.length == 1
              ? widget.repos.single.id
              : null;
  ...
  Map<String, Object?> _collect() {
    ...
    for (final input in widget.inputs) {
      switch (input.type) {
        ...
        case PipelineInputType.repo:
          final repo = widget.repos.firstWhereOrNull(
            (r) => r.id == _repos[input.key],
          );
          if (repo != null) {
            ...
            values['repo_local_path'] = repo.path;
            values['repo_full_name'] = repo.fullName;
            if (repo.hasForgeRemote) {
              values['repo_owner'] = repo.remoteOwner;
              values['repo_name'] = repo.remoteName;
            }
          }
        case PipelineInputType.number:
          final t = _text[input.key]?.text.trim() ?? '';
