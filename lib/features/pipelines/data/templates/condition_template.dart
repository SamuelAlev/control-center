  ...
  switch (type) {
    case 'and':
      ...
      for (final child in of) {
        if (!await _evaluate(child, ctx, renderer)) return false;
      }
      return true;
    case 'or':
      final of = _children(node);
      for (final child in of) {
        if (await _evaluate(child, ctx, renderer)) return true;
      }
      return false;
    case 'not':
      final of = _children(node);
...
List<Map<String, dynamic>> _children(Map<String, dynamic> node) {
  final of = node['of'];
  if (of is Map) return [of.cast<String, dynamic>()];
  if (of is List) {
    return of
        .whereType<Map>()
...
bool _existsRecursive(String rootPath, String basename) {
  ...
  while (queue.isNotEmpty) {
    ...
    for (final entry in entries) {
      if (++scanned > maxEntries) return false;
