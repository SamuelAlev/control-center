...
void registerBashScriptBody(
  ...
  registry.registerBody(BuiltInBodyKeys.bashScript, (ctx) async {
    ...
    final rendered = _render(script, ctx.state, ctx.triggerPayload);
    ...
    final runDir = await pipelineRunDir(ctx.pipelineRunId);
    ...
    AppLog.i(
      'bashScript',
      'Running step "${ctx.stepId}" in ${runDir.path}',
    );
    ...
    stepProcessRegistry.register(ctx.stepRunId, () {
      try {
        ...
        AppLog.e('bashScript', 'kill failed', e, st);
      }
    });
...
/// Substitutes `{{key}}` placeholders. State takes precedence over the
/// trigger payload. Missing keys render as empty strings.
...
String _render(
  ...
  return _renderer.render(template, state: state, trigger: triggerPayload).text;
}
