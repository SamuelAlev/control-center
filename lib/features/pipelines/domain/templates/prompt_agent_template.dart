...
void registerPromptAgentBody(
  ...
  registry.registerBody(BuiltInBodyKeys.promptAgent, (ctx) async {
    ...
    if (!renderResult.isComplete) {
      // Surface (don't fail) so a misconfigured `{{repoId}}`-style placeholder
      // doesn't silently render empty and confuse the agent. Some templates use
      // genuinely-optional placeholders, so this stays a warning.
      AppLog.w(
        'promptAgent',
        'Step "${ctx.stepId}" prompt has unresolved placeholders: '
        '${renderResult.unresolved.join(', ')} — they render empty.',
      );
    }
    ...
    final description = '$rendered\n\n'
        ...
        '(`{ "result": "<markdown body>" }`). If the task cannot be completed, '
