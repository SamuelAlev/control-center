...
import 'package:control_center/features/ticketing/domain/services/ticket_resume_listener.dart' show TicketResumeListener;
      ...
      try {
        ...
        if (tickets.isNotEmpty) {
          final payloads = tickets
              ...
              .map((t) => t.outputJson!)
              .toList();
          // When the node declares an output schema the payload shape is the
          // contract — do NOT unwrap a single `{result: ...}` (that legacy
          // convenience only applies to schemaless steps).
          final hasSchema = stepDef.config.outputSchema != null;
