...
class PrCloneProgress {
  ...
  bool get isTerminal => phase == PrClonePhase.ready || phase == PrClonePhase.error;
}
...
class PrCloneManager {
  ...
  /// Ensures the clone exists and the PR head ref is fetched.
  ...
  Stream<PrCloneProgress> _doClone(Directory cloneDir) async* {
    ...
    await for (final line in _git.runStreaming(
      ...
      yield PrCloneProgress(phase: PrClonePhase.cloning, message: _sanitize(line));
    }
  ...
  Stream<PrCloneProgress> _doFetch(
    ...
    await for (final line in _git.runStreaming(
      ...
      yield PrCloneProgress(phase: PrClonePhase.fetching, message: _sanitize(line));
    }
  }
