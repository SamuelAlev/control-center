  ...
  Future<PrDocAgentResult> generateWithAgent({
    ...
    try {
      ...
      final process = await Process.start(
        'claude',
        const ['--bare', '--output-format', 'json'],
        workingDirectory: workingDirectory,
      );
      // VULN-011: the prompt embeds the PR diff (file contents, potentially
      // secrets). Pass it via stdin, never as a `--prompt` argv element — argv
      // is visible to other local users / sandboxed agents via `ps`, stdin is
      // not. `claude --bare` reads the prompt from stdin (as `claude -p` does
      // when no prompt argument is supplied). No `runInShell`: fixed argv.
