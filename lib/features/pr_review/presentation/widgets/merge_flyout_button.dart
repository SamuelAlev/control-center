...
class _MergeFlyoutButtonState extends ConsumerState<MergeFlyoutButton> {
  ...
  bool get _hasPendingCheck => widget.checks.any(
    (c) => c.status != CheckRunStatus.completed,
  );
  ...
  Future<void> _merge() async {
    ...
    try {
      await repo.mergePullRequest(
        ...
        commitTitle:
            _method != _MergeMethod.rebase ? _titleCtrl.text : null,
        commitMessage:
            _method != _MergeMethod.rebase ? _descCtrl.text : null,
      );
      _close();
      scaffold.showSnackBar(
        SnackBar(content: Text(l10n.pullRequestMerged)),
      );
    } on Exception catch (e) {
      scaffold.showSnackBar(
        SnackBar(content: Text(l10n.failedToMergePr('$e'))),
      );
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (!widget.pr.canMerge) return const SizedBox.shrink();

    final l10n = AppLocalizations.of(context);

    final variant = switch (_readiness) {
      _MergeReadiness.ready => FButtonVariant.primary,
      _MergeReadiness.pending => FButtonVariant.secondary,
      _MergeReadiness.blocked => FButtonVariant.destructive,
    };
    ...
    return OverlayPortal(
      ...
      child: FButton(
        ...
        variant: variant,
        prefix: const Icon(LucideIcons.gitMerge, size: 16),
  ...
  Widget _buildOverlay(BuildContext overlayCtx) {
    ...
    if (!_allChecksPass) warnings.add(l10n.checksFailing);
    if (!_allReviewsApproved) warnings.add(l10n.reviewsPending);
