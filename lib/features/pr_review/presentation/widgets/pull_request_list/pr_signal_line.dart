          ...
          Flexible(
            child: Text(
              ...
              style: style?.copyWith(
                ...
                color: tokens?.textTertiary ?? muted,
              ),
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    ...
    final (IconData icon, Color fg, Color bg, String label) = switch (status) {
      PrChecksStatus.passing => (
        ...
        tokens?.successSoft ?? const Color(0x2417A34A),
        l10n.checksPassing,
      ),
      PrChecksStatus.failing => (
        LucideIcons.x,
        tokens?.danger ?? const Color(0xFFD92D20),
        tokens?.dangerSoft ?? const Color(0x1FDC2626),
        l10n.checksFailing,
      ),
      PrChecksStatus.pending => (
        LucideIcons.clock,
        tokens?.warn ?? const Color(0xFFCA8504),
        tokens?.warnSoft ?? const Color(0x33EAB308),
        l10n.checksRunning,
      ),
      PrChecksStatus.none => (
        LucideIcons.minus,
...
class _DiffStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.designSystem;
    final add = tokens?.success ?? const Color(0xFF079455);
    final del = tokens?.danger ?? const Color(0xFFD92D20);
...
class PrDraftBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.designSystem;
    final colors = context.theme.colors;
    final fg = tokens?.textTertiary ?? colors.mutedForeground;
    return Container(
