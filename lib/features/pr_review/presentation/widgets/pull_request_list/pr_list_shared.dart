...
/// Notifier that holds the currently-selected PR number in the inbox.
class SelectedPrNumberNotifier extends Notifier<int?> {
  @override
  int? build() => null;

  /// Sets the selected PR number, or clears selection when null.
  void select(int? number) => state = number;
}

/// Provider for the currently-selected PR number in the inbox.
...
/// Returns a [GlobalKey] for a PR row identified by repo and PR number.
typedef PrRowKeyGetter = GlobalKey Function(String repoId, int number);

/// Returns a [FocusNode] for a PR row identified by repo and PR number.
typedef PrRowFocusGetter = FocusNode Function(String repoId, int number);

/// Callback to open a PR in its detail view.
...
/// Navigates to the detail view for [prNumber], activating [repo] if provided.
void openPrInRepo(
  WidgetRef ref,
  BuildContext context,
  Repo? repo,
...
/// Capsule-filters [prs] according to [filters] and [currentLogin].
...
List<PullRequest> visiblePrsFor(
  ...
  if (lane != null) {
    result = result
        .where((pr) => lanesOfPr(pr, currentLogin).contains(lane))
        .toList();
  }
...
/// Truncates a PR body string for compact display.
...
/// A section heading with an optional trailing count badge.
