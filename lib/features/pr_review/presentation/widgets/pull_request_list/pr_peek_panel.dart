          ...
          builder: (context, constraints) {
            if (constraints.maxWidth >= _sideBySideThreshold) {
              return Row(
                ...
                children: [
                  Expanded(
                    child: _SummaryColumn(pr: pr, onOpen: onOpen),
                  ),
                  Container(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _SummaryBody(repoFullName: pr.repoFullName, prNumber: pr.number),
          const SizedBox(height: AppSpacing.md),
          Wrap(
            ...
            children: [
              CcButton(
                ...
                icon: AppIcons.gitPullRequestArrow,
...
class _SummaryBody extends ConsumerStatefulWidget {
  const _SummaryBody({required this.repoFullName, required this.prNumber});
...
class _SummaryBodyState extends ConsumerState<_SummaryBody> {
  ...
  Widget build(BuildContext context) {
    ...
    final contentKey = (owner: owner, repo: repo, number: widget.prNumber);
    ...
    final markdown = GitHubMarkdownBody(
      ...
      repoOwner: owner,
      repoName: repo,
      styleSheet: _peekMarkdownStyleSheet(context, codeFontFamily: codeFont),
...
class _SummaryLoadingPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.designSystem ?? DesignSystemTokens.light();
    final bar = tokens.borderSecondary.withValues(alpha: 0.6);
    ...
    Widget line(double widthFactor) => FractionallySizedBox(
      ...
      child: Container(
        height: 10,
        decoration: BoxDecoration(color: bar, borderRadius: AppRadii.brSm),
      ),
    );
...
/// A flattened variant of [githubMarkdownStyleSheet] tuned for the peek panel:
...
MarkdownStyleSheet _peekMarkdownStyleSheet(
  BuildContext context, {
  String? codeFontFamily,
}) {
  final base = githubMarkdownStyleSheet(
    context,
    compact: true,
