...
class PrListMergeButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return FButton(
      onPress: () => _openDialog(context, ref),
      mainAxisSize: MainAxisSize.min,
      size: FButtonSizeVariant.sm,
      variant: FButtonVariant.primary,
      prefix: const Icon(LucideIcons.gitMerge, size: 14),
      child: Text(l10n.merge),
    );
  }

  Future<void> _openDialog(BuildContext context, WidgetRef ref) async {
    final merged = await showFDialog<bool>(
      context: context,
      builder: (ctx, style, animation) =>
          _MergeDialog(style: style, animation: animation, pr: pr, repo: repo),
    );
    if (merged == true) {
...
class _MergeDialogState extends ConsumerState<_MergeDialog> {
  ...
  Future<void> _merge() async {
    ...
    try {
      ...
      navigator.pop(true);
      messenger.showSnackBar(SnackBar(content: Text(l10n.pullRequestMerged)));
  ...
  Widget build(BuildContext context) {
    ...
    return FDialog(
      ...
      actions: [
        FButton(
          onPress: _merging ? null : () => Navigator.of(context).pop(false),
