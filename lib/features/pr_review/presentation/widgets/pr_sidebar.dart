...
class PrSidebar extends ConsumerWidget {
  ...
  /// Whether the checks stream has produced an answer yet. Passed in because
  /// the Overview tab already watches it for the rest of the page.
  final bool checksPending;

  /// Whether the page is rendering the list-row SEED rather than a fetched PR.
  ///
  /// Assignees are the one rail read straight off the entity instead of a
  /// stream, and the list query does not fetch them — so on a seeded page an
  /// assigned PR would confidently report "No assignees" and then fill in.
  final bool detailPending;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context);
    // Each rail tracks whether its stream has ANSWERED, separately from what
    // the answer was. "No reviewers assigned" is a claim about the PR; while
    // the request is still in flight the honest rendering is a skeleton, not
    // the confident opposite of what usually arrives a moment later.
    ...
    final editState = canEdit ? ref.watch(prEditProvider(prRef)) : null;
        ...
        CollapsibleSidebarSection(
          ...
          child: _sectionBody(
            ...
            child: Wrap(
              ...
              children: [
                PrStatusBadge(pr: pr),
                if (hasShipShowAsk) ShipShowAskBadge(prRef: prRef),
        ...
        CollapsibleSidebarSection(
          ...
          child: _sectionBody(
            child: pr.assignees.isEmpty
                ? (detailPending
                      ? const PrSidebarSectionSkeleton(rows: 1)
        ...
        CollapsibleSidebarSection(
          ...
          child: _sectionBody(
            child: sortedFiles.isEmpty
                ? (filesPending
                      ? const PrSidebarSectionSkeleton(rows: 3, people: false)
  ...
  void _removeReviewer(WidgetRef ref, PrReviewer r) {
    final notifier = ref.read(prEditProvider(prRef).notifier);
    switch (r) {
      case PrUserReviewer():
...
class _ChecksRailRow extends StatelessWidget {
  ...
  /// True while the check-runs stream has not answered. "No checks have run
  /// yet" is a verdict on the PR; unknown is not the same as none.
  final bool pending;
  ...
  Widget build(BuildContext context) {
    ...
    final unknown = empty && pending;
    final status = empty ? null : _rollup;
    final verdict = switch (status) {
      null => unknown ? l10n.loadingChecks : l10n.noChecksYet,
          ...
          Expanded(
            child: Row(
              ...
              children: [
                ...
                // While unknown the rail shows a bar rather than words: the
                // verdict is one short phrase that would otherwise swap from
                // "No checks have run yet" to "Passed" a moment later, which
                // reads as the checks having changed rather than as us having
                // found out. The phrase stays in the semantic label so a screen
                // reader is told what the bar means.
                if (unknown)
                  const PrSkeletonBar(width: 64)
                else
                  Flexible(
                    child: Text(
                      verdict,
