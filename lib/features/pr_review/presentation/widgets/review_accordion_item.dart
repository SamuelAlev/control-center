...
class _ReviewAccordionItemState extends ConsumerState<ReviewAccordionItem> {
  ...
  Widget _buildExpandedBody(DesignSystemTokens tokens) {
    ...
    final thread = asyncMessages.maybeWhen(
      data: (msgs) =>
          msgs.where((m) => m.parentMessageId == msg.id).toList(),
          ...
          children: [
            if (selectionMode)
              Padding(
                ...
                child: FCheckbox(
                  value: isSelected,
                  onChange: (_) => onToggleSelect(!isSelected),
                ),
              ),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                GitHubUserAvatar(
                    login: agentName, size: 12, showHoverCard: false),
                const SizedBox(width: 4),
                Text(
                  agentName,
                  style: Theme.of(context).textTheme.labelSmall?.copyWith(
    ...
    return Row(
      children: [
        ...
        if (!canComment)
          FTooltip(
            tipBuilder: (_, _) =>
                Text(AppLocalizations.of(context).noFileAnchor),
            child: FButton(
              size: FButtonSizeVariant.sm,
