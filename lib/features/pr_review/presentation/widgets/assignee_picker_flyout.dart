...
class _AssigneePickerHeaderState extends ConsumerState<AssigneePickerHeader> {
  ...
  Future<void> _apply() async {
    ...
    final add =
        _selected.where((l) => !_ci(current, l)).toList(growable: false);
    ...
    if (error != null && mounted) {
      messenger.showSnackBar(
        SnackBar(content: Text(l10n.failedToUpdateAssignees(error))),
      );
    }
  }
          ...
          return _AssigneeFlyoutRow(
            ...
            onTap: () => setState(() {
              if (_ci(_selected, u.login)) {
                _selected
                    .removeWhere((l) => l.toLowerCase() == u.login.toLowerCase());
              } else {
...
class _AssigneeFlyoutRow extends StatefulWidget {
  ...
  @override
  State<_AssigneeFlyoutRow> createState() => _AssigneeFlyoutRowState();
}

class _AssigneeFlyoutRowState extends State<_AssigneeFlyoutRow> {
  bool _hovered = false;
  ...
  Widget build(BuildContext context) {
    ...
    return FTappable(
      ...
      child: Container(
        color: _hovered ? t.bgPrimaryHover : Colors.transparent,
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
        child: Row(
          children: [
            PickerCheck(selected: widget.selected),
