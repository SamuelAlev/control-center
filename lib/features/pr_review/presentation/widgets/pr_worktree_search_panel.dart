...
class PrWorktreeSearchPanel extends ConsumerStatefulWidget {
  ...
  /// The PR space whose worktree is searched.
...
class _PrWorktreeSearchPanelState extends ConsumerState<PrWorktreeSearchPanel> {
  ...
  void initState() {
    super.initState();
    // Restore this space's last query/options: the panel unmounts on every
    // tree↔search toggle, so its ephemeral state lives in a per-space
    // provider (see [prWorktreeSearchUiStateProvider]) and is written through
    // on every change.
    final saved = ref.read(prWorktreeSearchUiStateProvider(widget.spaceId));
  ...
  void _persist() {
    ref.read(prWorktreeSearchUiStateProvider(widget.spaceId).notifier).state = (
