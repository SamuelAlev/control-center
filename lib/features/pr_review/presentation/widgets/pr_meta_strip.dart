...
class PrMetaStrip extends ConsumerWidget {
  ...
  const PrMetaStrip({super.key, required this.pr, required this.prRef});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (author != null && author.login.isNotEmpty) {
      // The author takes a run of its own. Its width is the one thing in this
      // strip that changes AFTER first paint — a display name resolves from a
      // separate request and roughly doubles the badge — and in a shared run
      // that pushed the branch chips onto a second line, moving the entire PR
      // description down the page a beat after it rendered. On its own run the
      // name can only grow into empty space beside itself.
      children.add(
        SizedBox(
          width: double.infinity,
          child: _AuthorBadge(author: author, tokens: tokens),
        ),
      );
    }
    ...
    children.add(PrStackBadge(pr: pr, prRef: prRef));
...
class _AuthorBadge extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // Three sources, cheapest first, so the name is on screen from frame one
    // wherever it can be. The forge sometimes ships it with the PR itself
    // (GitLab and the GraphQL lanes do; GitHub's REST pull payload does not),
    // and anything this session already resolved is readable synchronously —
    // both of which skip the round trip that used to resize this row.
    final displayName =
        _nonEmpty(author.name) ??
        _nonEmpty(cachedGitHubUserProfile(author.login)?.name) ??
        ref
            .watch(githubUserProfileProvider(author.login))
            .maybeWhen(data: (p) => _nonEmpty(p?.name), orElse: () => null);
...
/// [value] with surrounding whitespace removed, or null when it is blank —
/// a display name the forge returned as `""` is the same as no name.
