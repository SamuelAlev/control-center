...
class _ReviewOverlayButtonState extends ConsumerState<ReviewOverlayButton> {
  ...
  Future<void> _approve() async {
    ...
    ref.read(prOptimisticReviewStateProvider.notifier).set(
          widget.pr.number,
    ...
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).pullRequestApproved)),
      );
    } on Exception catch (e) {
      if (!mounted) {
        return;
      }
      ref.read(prOptimisticReviewStateProvider.notifier).set(widget.pr.number, null);
      scaffold.showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).failedToSubmitReview('$e'))),
      );
    }
  }
  ...
  Future<void> _requestChanges() async {
    ...
    try {
      ...
      unawaited(_clearDraft());
      scaffold.showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).changesRequested)),
      );
    } on Exception catch (e) {
      if (!mounted) {
        return;
  ...
  Future<void> _attachFile() async {
    ...
    for (final file in files) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).failedToUpload(file.name, '$e'))),
        );
      }
    }
  }
  ...
  PrReviewSubmissionState? get _myReviewState {
    final optimistic =
        ref.read(prOptimisticReviewStateProvider)[widget.pr.number];
    if (optimistic != null) {
      final reviews =
          ref.read(prReviewsProvider(widget.pr.number)).asData?.value;
      if (reviews != null) {
  ...
  Widget build(BuildContext context) {
    ...
    final label = switch (myState) {
      ...
      PrReviewSubmissionState.changesRequested => AppLocalizations.of(context).requestedChanges,
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                FTooltip(
                                  tipBuilder: (_, _) =>
                                      Text(_showPreview ? AppLocalizations.of(context).writeLabel : AppLocalizations.of(context).previewLabel),
                                  child: FButton.icon(
                                    onPress: () {
                                      setState(
