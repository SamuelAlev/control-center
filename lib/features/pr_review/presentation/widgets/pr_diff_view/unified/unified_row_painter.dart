...
/// other boundary lands on a HALF physical pixel — and two antialiased rects
/// that each cover that pixel 50% composite to ~75%, letting the surface show
/// through as a thin band every second row. That is the "weird row gap": not a
/// layout error (the rows tile exactly) but a rasterisation one. Aliased fills
/// snap to whole pixels with a consistent tie-break, so the rects meet exactly.
/// Nothing painted with these is curved or rotated, so there is no
/// antialiasing worth keeping.
...
class UnifiedRowPainter {
  ...
  UnifiedRowPainter({
    ...
    required this.commentHighlightActiveColor,
       ...
       _dragPaint = _fill(
         DiffPalette.forBrightness(brightness).dragSelectionBg,
       ),
       _commentPaint = _fill(commentHighlightColor),
       _commentActivePaint = _fill(commentHighlightActiveColor),
       _searchPaint = _fill(
    ...
    _indentGuidePaint = _fill(
      dark ? const Color(0xFF2D333B) : const Color(0xFFEFF2F5),
    );
    _hoverBgPaint = _fill(
      dark ? const Color(0xFF1C2128) : const Color(0xFFF6F8FA),
    );
    ...
    final palette = DiffPalette.forBrightness(brightness);
    _gutterAddBgPaint = _fill(palette.additionGutterBg);
    _gutterDelBgPaint = _fill(palette.deletionGutterBg);
    _gutterAddStyle = _gutterStyle.copyWith(color: palette.additionGutterFg);
    _gutterDelStyle = _gutterStyle.copyWith(color: palette.deletionGutterFg);
    _gutterBgPaint = _fill(gutterBgColor);
  ...
  /// Every tiling background fill this painter uses.
  ///
  /// Exposed so a test can pin that they stay ALIASED: a `Paint()` default is
  /// antialiased, and one antialiased row background is enough to bring the
  /// half-pixel seam back (see [_fill]).
  @visibleForTesting
  List<Paint> get debugFillPaints => [
    _addBgPaint,
    _delBgPaint,
    _hunkBgPaint,
    _hoverBgPaint,
  ...
  late final TextStyle _gutterAddStyle;
  late final TextStyle _gutterDelStyle;
  late final Paint _gutterBgPaint;
  late final Paint _gutterAddBgPaint;
  late final Paint _gutterDelBgPaint;
  ...
  void paintRow({
    ...
    // GitHub-style number column: added/removed rows tint the gutter one step
    // stronger than the row background and pick up the matching number color.
    ...
    _paintGutter(
      ...
      style: kind == DiffLineKind.addition
          ? _gutterAddStyle
  ...
  void _paintGutter({
    ...
    if (!hideOldGutter) {
      final tp = _gutterPainter(oldLine?.toString() ?? '', style)
    ...
    if (!hideNewGutter) {
      final tp = _gutterPainter(newLine?.toString() ?? '', style)
        ..layout(maxWidth: colWidth - 6);
