...
class UnifiedRowPainter {
  ...
  UnifiedRowPainter({
    ...
    required this.commentHighlightActiveColor,
        ...
        _searchPaint = Paint()
          ..color = DiffPalette.forBrightness(brightness)
              .currentSearchMatchBg
    ...
    final gutterColor =
        dark ? const Color(0xFF6E7681) : const Color(0xFF8C959F);
  ...
  void _paintColSpan({
    ...
    if (overflowMode != DiffOverflowMode.wrap || visualRows <= 1) {
      ...
      final double right =
          endCol == null ? width : codeStartX + endCol * _monoAdvance;
      if (right > left) {
        canvas.drawRect(
    ...
    for (var row = firstRow; row <= lastRow; row++) {
      ...
      final int rowEndCol =
          row == lastRow ? ((end - 1) % colsPerRow) + 1 : colsPerRow;
  ...
  void _paintGutter({
    ...
    if (!hideOldGutter) {
      ...
      tp.paint(
        canvas,
        Offset(baseX + colWidth - 6 - tp.width, y + (kDiffLineHeight - tp.height) / 2),
      );
    }
    if (!hideNewGutter) {
      ...
      tp.paint(
        canvas,
        Offset(colX + colWidth - 6 - tp.width, y + (kDiffLineHeight - tp.height) / 2),
      );
    }
  }
  ...
  List<TextSpan> _buildRawSpans(String rawContent, int wrapCols) =>
      [TextSpan(text: _expandAndWrap(rawContent, 0, wrapCols).$1, style: baseStyle)];
  ...
  static TextStyle _styleForToken(DiffToken t, TextStyle base) {
    ...
    return base.copyWith(
      ...
      backgroundColor:
          t.backgroundColorValue == null ? null : Color(t.backgroundColorValue!),
    );
  }
  ...
  void _paintLeadingWhitespaceMarkers({
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      final caret =
          linePainter.getOffsetForCaret(TextPosition(offset: col), Rect.zero);
