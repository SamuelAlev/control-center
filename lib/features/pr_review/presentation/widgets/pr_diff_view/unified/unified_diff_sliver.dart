...
class RenderUnifiedDiffSliver extends RenderSliverMultiBoxAdaptor {
  ...
  late final TapGestureRecognizer _tapRecognizer =
      TapGestureRecognizer()..onTap = _handleGutterTap;
  double? _downMain;
  ...
  /// scrollable.
  late final PanGestureRecognizer _selectRecognizer = PanGestureRecognizer(
  ...
  double _codeWidthFor(double crossAxisExtent) {
    if (_config.splitMode) {
      ...
      return math.max(
          0.0, halfW - kDiffSplitGutterWidth - kDiffCodePadLeft - kDiffCodePadRight);
    }
    return math.max(0.0,
  ...
  double get maxHorizontalScrollExtent {
    ...
    return math.max(
        0.0, contentWidth - _codeViewportWidthFor(_lastCrossAxisExtent));
  }
  ...
  String? copySelectionText() {
    ...
    final text = _document.copyTextBetween(
      a.$1,
      a.$2,
      a.$3,
      f.$1,
  ...
  (int, int, int)? _cellAt(double mainAxisPosition, double crossAxisPosition) {
    ...
    final int subRow =
        ((yLocal - lineTop) / kDiffLineHeight).floor().clamp(0, 1 << 20);
  ...
  int _columnAt(double crossAxisPosition, int file, int line, int subRow) {
    ...
    final int base =
        _config.overflowMode == DiffOverflowMode.wrap ? subRow * _colsPerRow : 0;
  ...
  void _onSelectUpdate(DragUpdateDetails details) {
    ...
    _selMoved = true;
    _selFocus =
        _cellAt(_selDownMain + _selAccumDy, _selDownCross + _selAccumDx);
    markNeedsPaint();
    onSelectionChanged?.call();
  }
  ...
  /// row indices so stale `(file, line)` entries can't be reused.
  void clearLineCache() => lineCache.clear();
  ...
  void performLayout() {
    ...
    final double cacheStart =
        math.max(0, constraints.scrollOffset + constraints.cacheOrigin);
    final double cacheEnd =
        math.min(total, cacheStart + constraints.remainingCacheExtent);
    ...
    final int lastFile =
        _document.fileAtOffset(math.min(total - 0.0001, math.max(0, cacheEnd)));
  ...
  void _layoutSlotRange(int first, int last, double crossAxisExtent) {
    ...
    if (firstChild == null) {
      ...
      firstChild!.layout(_constraintsFor(first, crossAxisExtent),
          parentUsesSize: true);
  ...
  double _stickyMainAxis(double originY) {
    ...
    final double nextScreenY = _stickyFile + 1 < _document.fileCount
        ? originY +
            (_document.offsetOfFile(_stickyFile + 1) - scrollOffset)
        : double.infinity;
    final double pinnedScreenY =
        math.min(topInset, nextScreenY - _document.headerHeight);
    _stickyPinned = true;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_config.splitMode) {
      ...
      _paintCode(canvas, (_) => leftPainter, scrollOffset, bandTop, bandBottom,
          halfW,
      ...
      _paintCode(canvas, (_) => rightPainter, scrollOffset, bandTop, bandBottom,
          halfW,
      ...
      canvas
        ...
        ..translate(offset.dx, offset.dy);
      _paintCode(canvas, painterFor, scrollOffset, bandTop, bandBottom,
          crossAxisExtent);
      canvas.restore();
      fullPainter.dispose();
    ...
    final double originY =
        selfLayer is OffsetLayer ? selfLayer.offset.dy : offset.dy;
    ...
    if (stickyChild != null) {
      ...
      if (_stickyPinned) {
        ...
        context.pushClipRect(
          ...
          offset,
          Rect.fromLTWH(
              0, pos + 1, crossAxisExtent, paintExtentOf(sc) - 1),
    ...
    while (f < _document.fileCount) {
      ...
      if (_document.isExpanded(f)) {
        ...
        if (raw != null && raw.length > 0) {
          ...
          if (segBottom > segTop) {
            final int firstLine =
                _document.lineAtFileLocalY(f, segTop - fileTop);
            final int lastLine =
                _document.lineAtFileLocalY(f, segBottom - fileTop);
            final int displayCount = _document.lineCountOf(f);
            final tokens = _store.tokensOf(f);
            for (var displayLine = firstLine;
              ...
              final bool isSearchHit = _config.searchFile == f &&
                  _config.searchRawIndex == rawIndex;
