...
const Set<String> _dependencyLockFileNames = {
  'bun.lock',
  'bun.lockb',
  'cargo.lock',
  'composer.lock',
...
bool _isDependencyLockFile(String path) {
  final normalized = path.replaceAll(r'\', '/');
  final slash = normalized.lastIndexOf('/');
  final basename = slash < 0 ? normalized : normalized.substring(slash + 1);
  return _dependencyLockFileNames.contains(basename.toLowerCase());
}
...
class PrDiffDocument {
  ...
  List<int> setFiles(List<PrFile> next) {
    ...
    // filename. A FIFO queue per filename, NOT a single slot: the same
    // filename can legitimately appear twice with different patches (the
    // Source control tab renders staged + unstaged as one list and a
    // partially staged file is in both). A single slot would hand BOTH new
    // entries the same _FileLayout object, so one file's parsed structure —
    // its content and line numbers — would render under the other's header.
    // Matching duplicates in order is stable because the combined list always
    // keeps its group order (staged first).
    final prior = <String, List<(PrFile, _FileLayout)>>{};
    for (var i = 0; i < _files.length; i++) {
      prior.putIfAbsent(_files[i].filename, () => []).add((
        _files[i],
        _layouts[i],
      ));
    }
    ...
    for (var i = 0; i < next.length; i++) {
      final f = next[i];
      final queue = prior[f.filename];
      if (queue != null && queue.isNotEmpty) {
        final (old, existing) = queue.removeAt(0);
        if (old.patch != f.patch) {
  ...
  int displayLineOfRaw(int i, int rawIndex) {
    // Binary search: `displayToRaw` is strictly increasing (display rows are
    // a filtered, order-preserving projection of the structure), so a linear
    // scan was O(file lines) per gutter tap for no reason.
    final dtr = _layouts[i].displayToRaw;
    var lo = 0;
    var hi = dtr.length - 1;
    while (lo <= hi) {
    ...
    // Not displayed (a filtered hunk header, say).
    return -1;
  }
