...
> "show N lines" gaps, comment threads) are real widgets hosted as _sparse
> children_ of one custom sliver.
...
canvas and derives an _exact_ extent from a Fenwick tree. The expensive part
...
| `diff_structure_store.dart`   | `DiffStructureStore` — **structure** (pass-1 parse) is produced _synchronously on demand_ on the main isolate and retained, so the painter always has plain text + line numbers (no loading state). **Tokens** (pass-2 syntax colour) are fetched lazily off-thread from the worker pool for the visible window, cached in an LRU and surfaced via a `repaint` notifier so colour fades in.                                                                   |
...
| `measured_inline_thread.dart` | `MeasuredInlineThread` — wraps `PrInlineThreadBlock` in `HeightReporter` so every layout (including later growth like opening the reply composer) reports height back and the document can reserve the exact gap under the anchor row. A one-shot post-frame measure on the wrapper's `build` misses inner `setState`s.                                                                                                                                       |

Shared, outside this directory:

- `../../../utils/diff_isolate_worker.dart` — `DiffWorkerPool`: a process-singleton pool of long-lived isolates running the tokenize pipeline (`parseUnifiedDiff`, then **per-hunk** shiki tokenization — grammar state carries across lines, so multi-line constructs color correctly — using the CC theme compiled into the worker), with a theme-revision-keyed LRU cache. Structure is never emitted by the worker; `buildDiffRawLines` is the synchronous main-isolate builder.
...
- **Tokens are keyed by raw line index**, `displayToRaw` maps display→raw and a
  trailing EOF gap is appended _after_ all real rows so existing indices — and
...
- **Text selection + copy** (character-precise): mouse-drag in the code area
  ...
  the _real_ whitespace (tabs expanded to spaces so it stays tab-width and
...
- **Persistent comment highlights** are painted on the **canvas**
  ...
  A highlight covers a conversation's **whole line range**, and every row of it
  carries the same `groupId` — which is what makes a seven-line comment read as
  one mark, light up together on hover and click back open when collapsed.
  Hover and the click both stay **paint-only** (`hoveredCommentGroup` /
  `onCommentTap` on the render object): recomputing every line's highlight per
  pointer-move is unaffordable at 3000 files.
- **Commenter avatars** down the left rail are real `GitHubUserAvatar` widgets
  in the overlay (one per visible thread anchor); tapping collapses/expands the
  thread.
- **Conversation assembly is shared.** `../../utils/server_review_threads.dart`
  turns the forge's flat comment list into conversations (a reply carries
  `in_reply_to_id` pointing at the thread's ROOT) and maps them to
  `PrInlineEntry`s. This renderer and the Overview tab's timeline both go
  through it, so one discussion cannot acquire two renderings. Replying goes
  through `PrInlineCommentsController.replyTo`, which routes a draft locally
  and a forge thread to `replyToReviewComment` by its root comment id.
- **Suggestions** render as a mini-diff (`SuggestionAwareMarkdown`) inside the
  thread block, with **Accept & resolve** / **Dismiss** actions on drafts.
  A server suggestion is diffed against the **real source lines** its range
  covers (`_originalCodeForSpan`) — without them an empty ```suggestion fence
  (GitHub's "delete these lines") renders as a header over an empty box.
- **Collapse and resolve.** Every thread has a one-line collapsed form
  (`_CollapsedThreadRow`: author, preview, reply count, state) that still
  reserves a row, so a settled conversation is out of the way but never
  invisible. A conversation the forge reports **resolved** arrives collapsed;
  `_threadOpen` holds only deliberate overrides. Resolving a server thread
  writes through (`setReviewThreadResolved`) behind an optimistic state that is
  dropped once the comment stream agrees.
- **Batched review comments.** The composer offers *add single comment* (posts
  now) and *start a review* / *add to review* (queues it,
  `PrInlineSyncState.pendingReview`). The queue is submitted as ONE review with
  its verdict from the header's Review button, which shows the pending count.

Coordinate spaces: the overlay maps document space → **global** screen space in
the _view_ (which has a `BuildContext`), via the enclosing scrollable's box
...
- **Comments inbox + "hide resolved" toggle + detached-comment indicator** in
  the toolbar. Resolve, collapse and accept/dismiss work per-thread and a
  resolved conversation self-collapses, but the cross-file inbox and a
  hide-resolved-entirely toggle are not yet built. (An OUTDATED conversation —
  one whose line is gone, which this renderer can only surface in the file
  header's popover — is readable in full, with replies, in the Overview tab's
  conversation timeline.)
- **Reactions on the selection toolbar.** Synced entries render reaction
  chips with counts plus an add-reaction picker wired to the GitHub reactions
  API (`toggleReviewCommentReaction`, optimistic via `ReactionBar`); the
  toolbar "react" still posts a one-emoji comment instead.
- **True suggestion apply.** "Accept & resolve" records local applied state
  (forge suggestions are comments, not patches); dismiss removes drafts
  locally. Resolve/reopen DOES reach the forge now (GitHub only —
  `commentThreadResolution`); GitLab and Bitbucket keep it local.
