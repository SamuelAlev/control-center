...
| `diff_slot.dart` | `DiffSlot` / `DiffSlotKind` — an offset-ordered descriptor of a sparse child (`header` \| `gap` \| `comment` \| `composer`). Code rows are **not** slots (they're painted); only the rare interactive rows are. |
