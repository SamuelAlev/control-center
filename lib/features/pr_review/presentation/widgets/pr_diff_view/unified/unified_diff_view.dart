...
class UnifiedDiffView extends ConsumerStatefulWidget {
  ...
  /// Whether the diff sits directly under a bordered toolbar. The first file
  /// header then drops its top hairline so the two borders don't stack.
  final bool flushTop;

  /// ⌘F handler: opens the sidebar's "search in files" mode instead of the
  /// floating in-diff search overlay. Returns true when it took the action (the
  /// sidebar is available and now shown); false → the floating overlay is used
  /// as a fallback (e.g. a narrow window or an embedded diff with no sidebar).
  /// Null → always use the floating overlay.
  final bool Function()? onRequestSidebarSearch;

  /// Switches the sidebar back to the file tree (the `t` key). Null hides it.
...
class UnifiedDiffViewState extends ConsumerState<UnifiedDiffView> {
  ...
  /// Explicit per-thread open/closed overrides, keyed by thread id.
  ///
  /// The DEFAULT is derived (`open == !resolved`), so a conversation the author
  /// settled on the forge arrives collapsed without anyone pressing anything,
  /// and a thread the reader collapses to see the code stays collapsed. An
  /// entry here is only ever written by a deliberate press.
  final Map<String, bool> _threadOpen = {};

  /// Optimistic resolve state for server threads, keyed by thread id.
  ///
  /// A resolve is a round trip to the forge and back down the comment stream;
  /// without this the card sits in its old state for a second and reads as a
  /// press that did nothing. An entry is dropped as soon as the server agrees.
  final Map<String, bool> _resolvedOverride = {};

  /// Thread ids whose resolve call is in flight (their toggle is disabled).
  final Set<String> _resolveInFlight = {};

  /// Drops optimistic resolve state the forge has caught up with.
  ///
  /// Kept until then rather than cleared on every refresh: the comment stream
  /// re-yields the CACHED list before the fresh one lands, so clearing on any
  /// update would flip a just-resolved conversation back open for a beat.
  void _reconcileResolvedOverrides() {
    if (_resolvedOverride.isEmpty) {
      return;
    }
    final serverState = <String, bool>{};
    for (final c in widget.serverComments) {
      serverState['server-${c.id}'] = c.isResolved;
    }
    _resolvedOverride.removeWhere(
      (id, value) => serverState[id] != null && serverState[id] == value,
    );
  }

  /// Whether [thread] renders expanded.
  bool _isThreadOpen(PrInlineThread thread) =>
  ...
  /// The file whose header is currently docked (pinned), fed post-frame by the
  /// sliver. The header builder listens and drops the docked header's own top
  /// border — same flush contract as the first file under a bordered toolbar —
  ...
  List<DiffSlot> _buildSlots(Map<String, PrInlineThread> threadOut) {
    ...
    for (var f = 0; f < _document.fileCount; f++) {
      ...
      for (var d = 0; d < count; d++) {
        ...
        final thread = _threadAnchoredAt(raw, r, f, d, ctl, filename);
        if (thread != null) {
          // A collapsed thread still gets a row — a resolved conversation that
          // vanishes entirely is one nobody can find again, and the whole point
          // of collapsing is to read the code WITH the discussion still marked.
      ...
      for (var d = 0; d < count; d++) {
        ...
        var below = _document.offsetOfLine(f, d) + anchorRows * kDiffLineHeight;
  ...
  /// First-frame height reservation for the open composer, replaced by the
  /// measured height. Both shapes gained an actions row (post now / queue for
  ...
  void _invalidateServerSpansIfStale() {
    if (identical(_serverSpansMemoSource, widget.serverComments)) {
      return;
    }
    _serverSpansMemo.clear();
    _serverSpanByLineMemo.clear();
    _serverSpansMemoSource = widget.serverComments;
  }
  ...
  /// ("outdated"). Surfaced in a collapsed group in the file header instead of
  /// being dropped from the review.
  List<PrCodeReviewComment> _outdatedServerComments(String filename) =>
      partitionServerCommentsForFile(widget.serverComments, filename).outdated;

  /// The thread anchored at raw row [rawIndex] (draft preferred, else a
  /// synthesised server thread), or null. A thread anchors at the LAST line of
  /// its range, which is where the forge shows it too.
  PrInlineThread? _threadAnchoredAt(
    ...
    final span = _spanCovering(filename, side, lineNo);
    if (span != null && span.endLine == lineNo) {
      return _synthesizeServerThread(span, filename, fileIndex, displayLine);
    }
    return null;
  }
  ...
  PrInlineThread _synthesizeServerThread(
    ...
    return PrInlineThread(
      ...
      side: span.side,
      // Deliberately `comment` even when the body carries a ```suggestion
      // fence: the renderer detects the fence and draws the mini-diff either
      // way, while `suggestion` would surface the local accept/dismiss/edit
      // actions — none of which can act on a forge-owned comment.
      kind: PrInlineThreadKind.comment,
      // Without the source lines a suggestion has nothing to diff AGAINST, so
      // a "delete these lines" suggestion (an empty fence — legal on GitHub)
      // renders as an empty box with a header and no rows. Read off the
      // rendered rows first (full fidelity, including any expanded context)
      // and fall back to the forge's own hunk if they yield nothing.
      originalCode: _originalCodeForServerThread(
        span,
        fileIndex,
        anchorDisplayLine,
      ),
      ...
      resolved: _isSpanResolved(span),
      syncState: PrInlineSyncState.synced,
      // Shared with the conversation timeline, which renders the same
      // conversations off its own grouping — two mappings would be two
      // renderings of one discussion that nobody compares.
      entries: inlineEntriesFromServerComments(
        sorted,
        unknownAuthorLabel: AppLocalizations.of(context).unknownAuthor,
      ),
    );
  }

  /// The source lines [span] is about: the rendered diff rows when they are
  ...
  String _originalCodeForServerThread(
    ...
    final fromRows = _originalCodeForSpan(fileIndex, anchorDisplayLine, span);
    return fromRows.isNotEmpty
        ? fromRows
        : originalCodeForServerThread(ServerReviewThread(span.comments));
  }

  /// The diff's own source lines for [span], read backwards from its anchor
  /// row. Rows on the other side (and gap/hunk rows) are skipped, so a range
  /// that straddles a deletion block still collects only its own side.
  String _originalCodeForSpan(
    ...
    final lines = <String>[];
    for (var d = anchorDisplayLine; d >= 0; d--) {
      final r = _document.rawIndexOf(fileIndex, d);
      if (!_isCodeRow(raw, r)) {
        continue;
      }
      final n = span.side == 'LEFT' ? raw.oldLines[r] : raw.newLines[r];
      if (n == null || n > span.endLine) {
        continue;
      }
      if (n < span.startLine) {
        break;
      }
      lines.add(raw.contents[r]);
    }
    return lines.reversed.join('\n');
  }

  double _estimateThreadHeight(PrInlineThread thread) => _isThreadOpen(thread)
      ? 52 + thread.entries.length * 64 + 44 + 16
      : kCollapsedThreadHeight + 20;
  ...
  void _openSearch() {
    // Prefer the sidebar's "search in files" mode (a full worktree content
    // search grouped per file). It declines (returns false) when there's no
    // sidebar to show — a narrow window or an embedded diff (e.g. the Source
    // Control tab) — in which case we fall back to the floating in-diff search.
    if (widget.onRequestSidebarSearch?.call() ?? false) {
      return;
    }
    _search.openSearch();
    ...
    return Positioned(
      ...
      child: Material(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Icon(AppIcons.search, size: 16, color: tokens.textTertiary),
  ...
  void didUpdateWidget(covariant UnifiedDiffView oldWidget) {
    ...
    if (oldWidget.inlineCommentsController?.pr !=
            widget.inlineCommentsController?.pr ||
        !identical(
  ...
  /// Memoised THREAD highlights (drafts + server comments), rebuilt only when
  /// something they depend on changes.
  ///
  /// This is the expensive half: O(files x displayed lines), and each line
  /// calls `ctl.forAnchor` — a linear scan over the draft threads. It used to
  /// run in `build` with no memo at all, so a 200-file PR with 20k expanded
  /// lines and 30 drafts paid ~600k iterations on EVERY build — including the
  /// `setState` fired per pointer-move while dragging the gutter pill.
  Map<int, Map<int, DiffCommentHighlight>> _threadHighlights = const {};
  int _threadHighlightsRevision = -1;
  Object? _threadHighlightsComments;
  String? _threadHighlightsFocused;

  Map<int, Map<int, DiffCommentHighlight>> _ensureThreadHighlights() {
    if (_threadHighlightsRevision == _revision &&
        identical(_threadHighlightsComments, widget.serverComments) &&
        _threadHighlightsFocused == _focusedThreadId) {
      return _threadHighlights;
    }
    _threadHighlights = _computeThreadHighlights();
    _threadHighlightsRevision = _revision;
    _threadHighlightsComments = widget.serverComments;
    _threadHighlightsFocused = _focusedThreadId;
    return _threadHighlights;
  }

  Map<int, Map<int, DiffCommentHighlight>> _computeCommentHighlights() {
    final base = _ensureThreadHighlights();
    final req = _activeComposer;
    final drag = _pillDrag;
    if (req == null && drag == null) {
      return base;
    }

    // Copy-on-write, and only while a composer is open or a pill drag is
    // live: those are the two things that change per frame, and they touch a
    // handful of rows rather than every line of every file.
    final out = <int, Map<int, DiffCommentHighlight>>{
      for (final entry in base.entries)
        entry.key: Map<int, DiffCommentHighlight>.of(entry.value),
    };
  ...
  Map<int, Map<int, DiffCommentHighlight>> _computeThreadHighlights() {
    final ctl = widget.inlineCommentsController;
    final out = <int, Map<int, DiffCommentHighlight>>{};
    ...
    for (var f = 0; f < _document.fileCount; f++) {
      ...
      for (var d = 0; d < count; d++) {
        ...
        // A resolved or collapsed thread keeps its highlight: it is the handle
        // that opens the conversation back up, so removing it would strand the
        // discussion behind a card nobody can see.
        if (draft != null) {
          put(
            f,
            d,
        ...
        if (span != null) {
          put(
            ...
            DiffCommentHighlight(
              startCol: 0,
              active: span.id == _focusedThreadId,
              groupId: span.id,
            ),
          );
        }
  ...
  /// The conversation the pointer is currently over, or null.
  ///
  /// Held here only so the value survives an overlay rebuild; the render object
  /// owns the visual effect and repaints itself, so changing it never rebuilds
  /// the view (recomputing every line's highlight per pointer-move is what made
  /// a naive hover unaffordable on a 3000-file diff).
  String? _hoveredCommentGroup;

  void _setHoveredCommentGroup(String? group) {
    if (_hoveredCommentGroup == group) {
      return;
    }
    _hoveredCommentGroup = group;
    _sliver?.hoveredCommentGroup = group;
    // The pill/avatar layer reads it for the click cursor.
  ...
  /// Opens [id]'s card and focuses it. Used by the gutter avatar and by a click
  /// on the comment highlight — the two ways back into a collapsed thread.
  ...
  /// Flips a thread between the one-line summary row and the full card.
  ///
  /// Its measured height is dropped so the reserved gap is re-estimated for the
  /// new shape instead of snapping from the previous one's measurement.
  void _toggleThreadOpen(PrInlineThread thread) {
    final open = _isThreadOpen(thread);
    setState(() {
      _threadOpen[thread.id] = !open;
      _commentHeights.remove(thread.id);
      if (open && _focusedThreadId == thread.id) {
        _focusedThreadId = null;
      } else if (!open) {
  ...
  /// A click on a commented row's highlight. Opens the thread when it is
  /// collapsed; otherwise just focuses it, so the highlight and its card agree
  /// about which conversation you are looking at.
  void _onCommentHighlightTap(String threadId) {
    final thread = _threadBySlotKey['thread:$threadId'];
    if (thread != null && _isThreadOpen(thread)) {
      _focusThread(threadId);
      return;
    }
    _openThread(threadId);
  }

  /// Whether resolving [thread] would actually change anything.
  ///
  /// A draft is ours, so a local resolve is real. A forge-owned conversation
  /// needs a thread id to write to; without one (a forge that models no
  /// resolvable thread) the affordance is hidden rather than offered as a
  /// press that quietly does nothing.
  bool _canResolveThread(PrInlineThread thread) =>
      thread.threadId != null ||
      widget.inlineCommentsController?.byId(thread.id) != null;

  /// Marks a thread resolved / reopened.
  ///
  /// A draft is local, so the controller owns it. A server thread is the
  /// forge's, so this writes through and holds an optimistic state until the
  /// comment stream agrees — and on failure puts the card back and says so,
  /// rather than leaving a conversation that looks settled to one person only.
  Future<void> _setThreadResolved(PrInlineThread thread, bool resolved) async {
    final ctl = widget.inlineCommentsController;
    final forgeThreadId = thread.threadId;
    if (forgeThreadId == null) {
      ctl?.toggleResolved(thread.id);
      return;
    }
    if (_resolveInFlight.contains(thread.id) || ctl == null) {
      return;
    }
    setState(() {
      _resolveInFlight.add(thread.id);
      _resolvedOverride[thread.id] = resolved;
      // Resolving collapses, reopening expands — the press says what you want
      // to look at next, so it should not need a second one.
      _threadOpen[thread.id] = !resolved;
      _commentHeights.remove(thread.id);
      _revision++;
    });
    try {
  ...
  /// Whether this diff body's editor tab is currently the visible one.
  ///
  /// The workbench keeps hidden tab bodies alive (laid out, with real
  /// geometry) inside an `IndexedStack` and the review / scrollbar / search
  /// layers live in the ROOT overlay — so without this gate they keep
  /// painting and hit-testing over whichever tab IS visible (e.g. the hover
  ...
  /// marked dirty after the frame, never mid-build.
  void _syncTabVisibility(bool visible) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _reviewOverlay?.markNeedsBuild();
      _hScrollbarOverlay?.markNeedsBuild();
      _search.searchOverlay?.markNeedsBuild();
    });
  }
  ...
  Widget _buildReviewOverlay(BuildContext overlayContext) {
    ...
    // When a file header is docked at the top, its rows scroll up behind it —
    // but this overlay is a ROOT overlay painted above the sliver, so an avatar
    // or pill anchored on one of those hidden rows would paint over the pinned
    // header. Cull anything above the pinned header's bottom edge.
    final double affordanceTop = ro.stickyHeaderPinned
        ? rect.top + _document.headerHeight
        : rect.top;
    ...
    (int, int)? rowAtGlobalY(double gy) {
      ...
      final line = _document.lineAtFileLocalY(f, yLocal);
      // lineAtFileLocalY resolves a Y inside a comment card to the card's
      // anchor line — but hovering the card itself must not grow a "+" pill
      // on that row, so reject Ys outside the line's own (possibly wrapped)
      // row span.
      final lineTop = _document.lineTopInFile(f, line);
      final lineRows = _document.visualRowsOf(f, line);
      if (yLocal < lineTop || yLocal >= lineTop + lineRows * kDiffLineHeight) {
        return null;
      }
      // Only real code rows are commentable: the expand affordances
      // ("Show N lines" / "Show end of file") and hunk headers must not
      // grow a "+" pill and a range drag may not extend onto them.
      final raw = _document.structureOf(f);
      if (raw == null || !_isCodeRow(raw, _document.rawIndexOf(f, line))) {
        return null;
      }
      return (f, line);
    }
      ...
      Positioned.fill(
        child: ValueListenableBuilder<(int, int)?>(
          ...
          builder: (_, hover, _) {
              ...
              items.add(
                Positioned(
                  ...
                  child: GestureDetector(
                    onTap: () => _toggleThreadOpen(thread),
            ...
            // Clip to the diff's visible area: this layer paints in the ROOT
            // overlay where no sliver clip reaches, so without it an avatar
      ...
      Positioned.fromRect(
        ...
        child: MouseRegion(
          ...
          cursor: _pillDrag != null
              ...
              : MouseCursor.defer,
          onHover: (e) {
            // This region outlives the State by up to a frame: `dispose` runs
            // during the build phase, so `OverlayEntry.remove` defers its
            // `_markDirty` to a post-frame callback and the MouseRegion is only
            // detached on the NEXT frame. Every pointer move in that window
            // lands here with `_hoverRow` already disposed.
            if (!mounted) {
              return;
            }
            if (_pillDrag != null) {
            ...
            // dialog) covers the diff in a lower overlay. Hit-test down to
            // the diff sliver itself — if something opaque intercepts first,
            // the pointer never "reaches" the diff and no pill may surface
            // through (and paint over) the open panel.
            ...
            // Same row resolution feeds the comment-highlight hover, so the
            // pill and the highlight can never disagree about which row the
            // pointer is on.
          ...
          onExit: (_) {
            ...
            // a hovered MouseRegion is exactly what makes the tracker fire an
            // exit, post-frame — i.e. always after `dispose`.
            if (!mounted) {
              return;
            }
            if (_pillDrag == null) {
  ...
  /// Opens [threadId]'s card, focuses it and scrolls it into view.
  ///
  /// The entry point for a comment permalink. Opening + focusing is exactly
  /// [_openThread], which is also what gives the highlight for free:
  /// `_focusedThreadId` already drives the focused comment colour. What is
  /// added here is the scroll and the fade — a permalink's highlight is a
  /// "here it is" flash, and left set it would read as a selection the reader
  /// has to dismiss.
  Future<void> revealThread(
    ...
    _openThread(threadId);
    // After the open: the card changes the document's line offsets, so the
    // target must be computed once the re-layout it triggers has happened.
    ...
    if (renderObject is RenderUnifiedDiffSliver &&
      ...
      final target = renderObject
          .revealOffsetForLine(fileIndex, displayLine)
    ...
    _threadHighlightFade?.cancel();
    _threadHighlightFade = Timer(const Duration(seconds: 3), () {
      if (mounted && _focusedThreadId == threadId) {
        setState(() {
          _focusedThreadId = null;
          _revision++;
        });
  ...
  /// Clears a permalink's highlight after a beat. Held so a second permalink
  ...
  TextStyle _baseStyle(String codeFont, {required bool ligatures}) {
    ...
    return AppFonts.codeDynamic(
      ...
      textStyle: CcTypography.body.copyWith(
        ...
        fontFeatures: AppFonts.codeFontFeatures(ligatures: ligatures),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (inlineCtl != null) {
      // Keyed off the controller's own (repo, number) identity.
      ref.listen(prInlineCommentsControllerProvider(inlineCtl.pr), (_, _) {
        if (mounted) {
          setState(() => _revision++);
        }
      });
    }
    final ctlForComposer = widget.inlineCommentsController;
    // Batching is only offered when a comment could actually be posted: with no
    // PR connected the "start a review" path has nothing to submit to, and a
    // queue nobody can flush is worse than posting nothing.
    final canBatch = ctlForComposer != null && ctlForComposer.canPost;
    final reviewInProgress =
        canBatch && ctlForComposer.pendingReviewThreads.isNotEmpty;
    final tokens =
        context.designSystem ??
    ...
    final config = UnifiedDiffPaintConfig(
      ...
      baseStyle: _baseStyle(codeFont, ligatures: codeLigatures),
    ...
    return UnifiedDiffSliver(
      ...
      hoveredCommentGroup: _hoveredCommentGroup,
      onCommentTap: widget.inlineCommentsController != null
          ? _onCommentHighlightTap
          : null,
