...
class UnifiedDiffViewState extends ConsumerState<UnifiedDiffView> {
  ...
  /// Per-file server-comment partitions, memoized against the comment list.
  ///
  ...
  final Map<String, Map<String, List<PrCodeReviewComment>>>
  _serverByAnchorMemo = {};
  Object? _serverByAnchorMemoSource;

  /// Server review comments for [filename] indexed by `"<side>-<anchorLine>"`.
  /// Comments the current diff can no longer place (no anchor line) are NOT
  /// dropped here — they are surfaced separately via [_outdatedServerComments].

  Map<String, List<PrCodeReviewComment>> _serverByAnchor(String filename) {
    if (!identical(_serverByAnchorMemoSource, widget.serverComments)) {
      _serverByAnchorMemo.clear();
      _serverByAnchorMemoSource = widget.serverComments;
    }
    return _serverByAnchorMemo[filename] ??=
        partitionServerCommentsForFile(widget.serverComments, filename).anchored;
  }
