...
class PrDiffKeyboardHandler {
  ...
  Future<void> stepToFile(int delta) async {
    ...
    final target = _resolvedFileTop(focusedFileIndex);
  ...
  /// Returns the scroll offset that places file [i]'s top at the top of the
  /// viewport. Uses the actually-rendered render object when the file has
  /// been laid out (via its [GlobalKey]); falls back to the estimate
  /// otherwise.
  double _resolvedFileTop(int i) {
    if (i < 0 || i >= files.length) {
      return estimatedFileTopGetter(i.clamp(0, files.length - 1));
    }
    final ctx = fileStateKeys[files[i].filename]?.currentContext;
    ...
    return estimatedFileTopGetter(i);
  }

  int _findFileAtOffset(double offset) {
    // Walk backwards: the focused file is the last one whose top is at or
    // above the current scroll offset. Resolved tops use actual rendered
    // positions where available, which avoids the off-by-one we hit when
    // estimated tops drift from real ones.
    const tolerance = 4.0;
    for (var i = files.length - 1; i >= 0; i--) {
      if (_resolvedFileTop(i) <= offset + tolerance) {
        return i;
      }
    }
    return 0;
