...
class PrActivityTimeline extends ConsumerWidget {
  ...
  /// Called with a changed file's tree-order index when a review row's
  /// "N code comments" button is pressed, so the detail screen can focus the
  /// Diff tab and jump to the first commented file.
  final ValueChanged<int>? onOpenFileInDiff;

  /// Focuses the Diff tab on the file carrying the review's first code
  /// comment (falls back to the top of the diff when the file left the diff,
  /// e.g. an outdated comment). [orderedFiles] is the tree-order file list
  /// the Diff tab's jump indices are defined over.
  void _openCodeComments(
    List<PrFile> orderedFiles,
    List<PrCodeReviewComment> comments,
  ) {
    var index = 0;
    for (final c in comments) {
      final i = orderedFiles.indexWhere((f) => f.filename == c.path);
      if (i >= 0) {
        index = i;
        break;
      }
    }
    onOpenFileInDiff?.call(index);
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final reviews =
        ref.watch(prReviewsProvider(prNumber)).value ??
        const <PrReviewSubmission>[];
    final comments =
        ref.watch(prIssueCommentsProvider(prNumber)).value ??
        const <IssueComment>[];
    final commits =
        ref.watch(prCommitsProvider(prNumber)).value ?? const <PrCommit>[];
    final events =
        ref.watch(prTimelineEventsProvider(prNumber)).value ?? const [];
    final codeComments =
        ref.watch(prReviewCommentsProvider(prNumber)).value ??
        const <PrCodeReviewComment>[];
    final orderedFiles = sortFilesByTreeOrder(
      ref.watch(prFilesProvider(prNumber)).value ?? const <PrFile>[],
    );

    // A review's inline comments, keyed by the review id they were submitted
    // with — drives the "N code comments" jump-to-diff row.
    final commentsByReview = <int, List<PrCodeReviewComment>>{};
    for (final c in codeComments) {
      final reviewId = c.reviewId;
      if (reviewId != null) {
        commentsByReview.putIfAbsent(reviewId, () => []).add(c);
      }
    }
          ...
          _TimelineTile(
            ...
            child: switch (entry) {
              ...
              PrReviewEntry() when entry.review.body.trim().isEmpty =>
                _ReviewVerdictRow(
                  entry: entry,
                  codeComments: commentsByReview[entry.review.id] ?? const [],
                  onOpenCodeComments: (c) => _openCodeComments(orderedFiles, c),
                ),
...
class _EventSentence extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rel = formatRelative(timestamp);
...
class _ReviewVerdictRow extends StatelessWidget {
  const _ReviewVerdictRow({
    required this.entry,
    required this.codeComments,
    required this.onOpenCodeComments,
  });

  final PrReviewEntry entry;
  final List<PrCodeReviewComment> codeComments;
  final ValueChanged<List<PrCodeReviewComment>> onOpenCodeComments;
    ...
    return Column(
      ...
      children: [
        ...
        _CodeCommentsButton(
          ...
          onPressed: () => onOpenCodeComments(codeComments),
        ),
      ],
    );
  }
...
/// "N code comments" under a review row — jumps to the Diff tab (the comments
/// themselves stay inline on their diff lines).
class _CodeCommentsButton extends StatelessWidget {
  const _CodeCommentsButton({required this.comments, required this.onPressed});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // A min-size Row hands the button unbounded width so it hugs its label
      // instead of stretching to the timeline width.
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          CcButton(
            ...
            icon: AppIcons.reply,
            child: Text(l10n.prTimelineCodeComments(comments.length)),
          ),
        ],
      ),
    );
