...
class PrComplexityBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final estimatedMinutes =
        ((totalLoc / 100) * _minutesPer100Loc).round().clamp(5, 240);

    return FTooltip(
      tipAnchor: Alignment.topCenter,
      childAnchor: Alignment.bottomCenter,
      tipBuilder: (_, _) => _buildTooltip(context, estimatedMinutes),
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Icon(LucideIcons.fileText, size: 11, color: color),
  ...
  Widget _buildTooltip(BuildContext context, int estimatedMinutes) {
    final label = totalLoc >= _blockLoc
