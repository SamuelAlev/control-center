...
///
/// Reads the channel's finalized summary through [reviewHubSummaryProvider] and
/// renders its verdict. Renders nothing when no finalized summary exists yet.
///
/// It used to watch the raw message list and re-derive the latest summary here,
/// which meant a rebuild on EVERY message in the thread — a verdict that has
/// not moved since the review finalized, re-scanned and re-painted for each
/// chat line. The derived provider has value equality, so this now rebuilds
/// only when the verdict itself changes. (It also stops this widget and the
/// review hub from parsing the same metadata two different ways.)
class ReviewVerdictBanner extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final verdict = ref.watch(
      reviewHubSummaryProvider(channelId).select((s) => s?.verdict),
    );
    return verdict == null
        ? const SizedBox.shrink()
        : _Banner(verdict: verdict);
  }
}
