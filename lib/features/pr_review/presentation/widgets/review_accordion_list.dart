...
/// Lets a host drive the accordion's batch mode from outside.
///
/// The area deep dive owns the "fix everything in this area" affordance but
/// not the selection state, and duplicating the selection logic there would be
/// a second implementation of batch mode that drifts from this one.
class ReviewAccordionController extends ChangeNotifier {
  /// Puts the list into selection mode with every open, in-scope finding
  /// selected.
  void selectAllOpen() => notifyListeners();
}
...
class ReviewAccordionList extends ConsumerStatefulWidget {
  ...
  /// Optional scope applied before the UI filters — the Review Hub's area deep
  /// dive passes the findings routed into the selected cohort; null renders
  /// every finding of the channel.
...
class _ReviewAccordionListState extends ConsumerState<ReviewAccordionList> {
  ...
  bool _pendingSelectAll = false;

  @override
  void initState() {
    super.initState();
    widget.controller?.addListener(_onSelectAllRequested);
  }
  ...
  void didUpdateWidget(ReviewAccordionList oldWidget) {
    ...
    if (oldWidget.controller != widget.controller) {
      oldWidget.controller?.removeListener(_onSelectAllRequested);
      widget.controller?.addListener(_onSelectAllRequested);
    }
  }
  ...
  @override
  void dispose() {
    widget.controller?.removeListener(_onSelectAllRequested);
    super.dispose();
  }

  /// The findings are derived in `build` from the message stream, so the
  /// request is recorded here and applied there — resolving it now would race
  /// the stream.
  void _onSelectAllRequested() {
    if (!mounted) {
      return;
    }
    setState(() => _pendingSelectAll = true);
  }
    ...
    return asyncMessages.when(
      ...
      data: (messages) {
        ...
        if (_pendingSelectAll) {
          ...
          if (open.isNotEmpty) {
            // Applied after this frame: mutating selection state during build
            // would be a setState-in-build.
  ...
  Future<void> _handleFix(List<ReviewFinding> findings) async {
    ...
    try {
      ...
      // Branch the fix into a fresh parenthesis (N3) so it stays off the main
  ...
  Widget _buildEmpty(BuildContext context) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(AppIcons.share2, size: 48, color: tokens.textTertiary),
      ...
      child: Row(
        children: [
          ...
          CcButton(
            ...
            onPressed: selected.isNotEmpty ? () => _handleFix(selected) : null,
          ...
          if (!someAnchored)
            ...
            CcTooltip(
              ...
              child: CcButton(
                size: CcButtonSize.sm,
                variant: CcButtonVariant.secondary,
                onPressed: () => _handleComment(selected),
                icon: AppIcons.messageSquarePlus,
                child: Text(l10n.commentSelected),
              ),
            )
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: CcTypography.caption.copyWith(
            ...
            fontWeight: active ? FontWeight.w600 : CcTypography.regularWeight,
          ),
        ),
      ),
    );
