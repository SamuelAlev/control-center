...
class _ReviewAccordionListState extends ConsumerState<ReviewAccordionList> {
    ...
    if (_kindFilters.isEmpty && _statusFilters.isEmpty)
 {
    ...
    return findings.where((f) {
      if (_kindFilters.isNotEmpty &&
          !_kindFilters.contains(f.payload.kind)) {
        return false;
      }
        ...
        if (_selectedIds.isEmpty)
 {
  ...
  List<ReviewFinding> _selectedFindings(List<ReviewFinding> visible) {
    return visible
        .where((f) => _selectedIds.contains(f.message.id))
        .toList();
  }
    ...
    if (findings.isEmpty)
 {
    ...
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context).sentFindingsToAgent(findings.length)),
        ),
      );
    } catch (e) {
      scaffold.showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).failedToDispatch('$e'))),
      );
    }
  }
    ...
    for (final f in findings) {
      ...
      try {
        await useCase.postComment(
          ...
          startLine:
              p.anchor.lineEnd != null ? p.anchor.lineNumber : null,
        );
        posted++;
      } catch (e) {
        ...
        children: [
          ...
          if (_selectionMode) ...[
            FButton(
              ...
              onPress: () {
                final visible = _applyDismissedToggle(
                  _applyFilters(all),
                );
          ...
          if (!someAnchored)
            FTooltip(
              ...
              child: FButton(
                ...
                onPress: null,
                prefix:
                    const Icon(LucideIcons.messageSquarePlus, size: 14),
