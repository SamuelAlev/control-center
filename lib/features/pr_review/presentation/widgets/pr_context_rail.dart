        ...
        child: ListView(
          ...
          children: [
            if (ctx.authorRecentPrs.isNotEmpty) ...[
              ...
              for (final p in ctx.authorRecentPrs)
                _PrChip(pr: p),
            ...
            if (ctx.relatedMessages.isNotEmpty) ...[
              ...
              for (final r in ctx.relatedMessages)
                _MessageChip(channel: r.channel.name, content: r.message.content),
              const SizedBox(height: 12),
            ],
            if (ctx.authorRecentPrs.isEmpty &&
                ctx.relatedMessages.isEmpty)
              _EmptyState(),
          ],
        ),
      ),
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon, size: 11, color: context.theme.colors.mutedForeground),
