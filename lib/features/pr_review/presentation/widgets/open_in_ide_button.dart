...
class _OpenInIdeButtonState extends ConsumerState<OpenInIdeButton> {
  ...
  Future<void> _open(IdeEditor editor) async {
    ...
    try {
      // Ask the host to resolve the PR's CHANNEL worktree (the same tree the
                ...
                child: _preparing
                    ? SizedBox(
                        ...
                        child: Padding(
                          ...
                          child: CcSpinner(size: 14, color: tokens.fgSecondary),
                        ),
                      )
                    : _buildLogo(
                        effective.id,
...
/// theme; full-color logos keep their own palette. Falls back to an [AppIcons]
/// glyph
