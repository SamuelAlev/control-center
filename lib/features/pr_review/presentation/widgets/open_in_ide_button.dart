...
class _OpenInIdeButtonState extends ConsumerState<OpenInIdeButton> {
  ...
  Future<void> _open(IdeEditor editor) async {
    ...
    try {
      ...
      messenger.showSnackBar(
        SnackBar(
          content: Text(l10n.failedToOpenInIde(editor.displayName, e.message)),
        ),
      );
    } on Object catch (e) {
      messenger.showSnackBar(
        SnackBar(
          content: Text(l10n.failedToOpenInIde(editor.displayName, '$e')),
        ),
      );
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final splitButton = SizedBox(
      key: _anchorKey,
      // Matches the height of the primary forui `sm` buttons in this action row
    ...
    final row = Padding(
      ...
      child: Row(
        children: [
          ...
          if (selected)
            Icon(LucideIcons.check, size: 15, color: tokens.accent),
        ],
      ),
    );
    if (!enabled) {
