...
class _OpenInIdeButtonState extends ConsumerState<OpenInIdeButton> {
  ...
  Future<void> _open(IdeEditor editor) async {
    ...
    try {
      // Ask the host to lazily check out the PR's branch into a worktree (created
      // on demand, GC'd on merge/close) over RPC, then open that directory in a
      // LOCAL editor. The desktop is a thin client: the server owns the repo
      // checkout + worktree, but the editor launch stays here (a GUI-attached
      // process the headless host can't be). In the default self-serve setup the
      // host is this machine, so the returned path is local.
  ...
  Widget build(BuildContext context) {
    ...
    final splitButton = SizedBox(
      ...
      // (Review / Merge): 18px content padding + the UI font's ~20px line box at
      // the sm font size = 38px under this theme.
      height: 36,
      child: DecoratedBox(
    ...
    final row = Padding(
      ...
      child: Row(
        children: [
          ...
          if (selected) Icon(AppIcons.check, size: 15, color: tokens.accent),
        ],
      ),
    );
    if (!enabled) {
