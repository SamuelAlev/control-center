  ...
  Stream<DiffEvent> enqueue({
    ...
    final job = _ActiveJob(
      ...
      cacheKey: cacheKey == null
          ? null
          : _cacheKey(cacheKey, isDark, language),
    );
    _active[fileId] = job;
    _scheduleNotify();
    ...
    unawaited(() async {
      ...
      _enqueueOrDispatch(_JobRequest(
        fileId: fileId,
        generation: generation,
        patch: patch,
        language: language,
...
void _runJob(_JobRequest job, SendPort eventPort) {
  try {
    // ── Pass 1: parse structure ──────────────────────────────────────────
    final parsed = parseUnifiedDiff(job.patch);
    eventPort.send(_WorkerRawLinesEvent(
      fileId: job.fileId,
    ...
    if (parsed.isEmpty) {
      eventPort.send(_WorkerDoneEvent(
        fileId: job.fileId,
        generation: job.generation,
      ));
      return;
    ...
    void flushChunk() {
      ...
      eventPort.send(_WorkerTokensChunkEvent(
        fileId: job.fileId,
        generation: job.generation,
    ...
    eventPort.send(_WorkerErrorEvent(
      fileId: job.fileId,
      generation: job.generation,
      message: e.toString(),
    ));
