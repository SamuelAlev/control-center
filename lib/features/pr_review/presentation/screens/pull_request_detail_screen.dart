    ...
    return prAsync.when(
      data: (pr) {
        ...
        return PageWrapper(
          ...
          breadcrumbActions: [
            _PrBreadcrumbActions(pr: pr, prNumber: prNumber),
          ],
      ...
      error: (e, _) =>
          PageWrapper(child: _ErrorState(prNumber: prNumber, error: e)),
    );
  }
}
...
class _PrBreadcrumbActions extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isAuthor = currentLogin.isNotEmpty &&
        pr.author?.login.toLowerCase() == currentLogin;

    // Check repo permission for merge/close
    final permissionAsync = ref.watch(repoPermissionProvider(
      (owner: owner, repo: repoName),
    ));
    final hasWriteAccess = permissionAsync.whenOrNull(
...
class _PrMoreActionsMenuState extends ConsumerState<_PrMoreActionsMenu>
  ...
  Future<void> _startAiReview() async {
    ...
    if (workspace == null || repo == null) {
      scaffold.showSnackBar(
        SnackBar(content: Text(l10n.noActiveWorkspace)),
      );
      return;
    }
  ...
  Future<void> _closePr() async {
    ...
    try {
      ...
      scaffold.showSnackBar(
        SnackBar(content: Text(l10n.pullRequestClosed)),
      );
    } on Exception catch (e) {
      if (!mounted) {
        return;
...
class _PrDetailBodyState extends ConsumerState<_PrDetailBody>
  ...
  final GlobalKey _prHeaderKey = GlobalKey();
  ...
  void _scheduleHeaderMeasure() {
    // Always re-schedule a measurement on every layout. The sidebar (the
    // right column of [PrHeaderSection]) grows asynchronously as checks
    // and reviews load, so the header's true height isn't known at first
    // frame. The `< 0.5` diff check in [_measureHeader] gates setState,
    // so steady-state layouts cost nothing.
  ...
  void _measureHeader() {
    final ctx = _prHeaderKey.currentContext;
    if (ctx == null) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ScopedShortcuts(
      ...
      child: Column(
        ...
        final showTree = _activeTab == 0 && constraints.maxWidth >= 1024;
        return Stack(
          children: [
            StickyHeaderInset(
              top: _stickyTopInset,
                    ...
                    slivers: [
                      SliverToBoxAdapter(
                        child: NotificationListener<SizeChangedLayoutNotification>(
                          onNotification: (_) {
                            // PrSidebar grows asynchronously as checks and
                            // reviews stream in — re-measure so the tree
                            // overlay re-aligns with the new bottom edge.
            ...
            if (showTree && _prHeaderHeight > 0)
              AnimatedBuilder(
                ...
                builder: (context, child) {
                  // Only read `.offset` when exactly one scroll position is
                  // attached. During a route transition or when ReadyAutoScroll
                  // re-parents the scrollable, the controller transiently holds
                  // two positions and `.offset` would assert. Reuse the last
                  // good offset for that frame to avoid the crash and a jump.
