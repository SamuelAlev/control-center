...
class PullRequestDetailScreen extends ConsumerStatefulWidget {
  ...
  /// key, so every PR-keyed read resolves in the route's workspace.
...
class _PullRequestDetailScreenState
  ...
  /// This screen's PR identity, built purely from the route params. Every
  /// PR-keyed provider below is keyed by it, so the repo binding can never
  /// race a navigation or fall back to another repo — the route IS the PR.
  ///
  /// A getter, not a field: go_router reuses this State across PR→PR hops (the
  /// page key is the route *pattern*, not the resolved params), so the params
  /// change under a live State.
  PrRef get _prRef => (
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(prDetailProvider(prRef), (_, next) {
      if (next is AsyncData && !next.isLoading) {
        ref
            .read(lastCheckedProvider.notifier)
            .stamp('pr-detail:${prRef.repoFullName}#$prNumber');
      }
    });
    // The list row the operator just clicked stands in for the detail until it
    // lands, so the page opens with its real title, author, branches, state and
    // counts instead of a skeleton waiting on the forge. Everything a seed
    // cannot answer for — the description — gates on `prDetailPendingProvider`.
    final pr = ref.watch(prDetailOrSeedProvider(prRef));
    if (pr != null) {
  ...
  Widget _buildDetail(PullRequest pr, PrRef prRef) {
    ...
    return PageWrapper(
      // Key by the PR's full identity (`owner/repo#number`) so navigating
      // between PRs gets a fresh [_PrDetailBodyState] — without this, the
      // diff view's [GlobalKey]s for individual files (keyed by path) leak
      // across PRs, causing e.g. PR1's `package.json` content to bleed into
      // PR2's `package.json` view because the old [PrFileDiffState] (with
      // its cached `_fileLinesFuture`) gets reused. The number alone would
      // collide for same-numbered PRs in sibling repos.
      child: PrMentionAvatarScope(
        pr: prRef,
        child: _PrDetailBody(
          key: ValueKey('pr-detail-${prRef.repoFullName}#${prRef.number}'),
          pr: pr,
          prRef: prRef,
...
class _PrDetailBodyState extends ConsumerState<_PrDetailBody> {
  ...
  /// A comment the Diff tab should reveal (select its file, expand its thread,
  /// scroll it into view). Seeded from the URL's `?comment=` and cleared by the
  /// tab once applied, so it is a one-shot: a rebuild must not re-scroll a
  /// reader who has since scrolled away.
  late final ValueNotifier<int?> _pendingCommentId = ValueNotifier<int?>(
    widget.pendingCommentId,
  );
  ...
  void didUpdateWidget(covariant _PrDetailBody oldWidget) {
    ...
    // A second notification for a different comment on the SAME pull request
    // re-arms the reveal; the tab is already mounted, so nothing else would.
    if (oldWidget.pendingCommentId != widget.pendingCommentId &&
        widget.pendingCommentId != null) {
      _pendingCommentId.value = widget.pendingCommentId;
    }
  ...
  /// A `+`-menu row that reopens a single-instance PR tab of [kind] in the
  /// clicked leaf — so a closed Overview/Diff/Actions/Review tab can be
  /// brought back.
  ...
  /// The REOPEN group: the fixed PR views that are NOT currently open.
  ///
  /// Listing all six unconditionally made the `+` menu's first six rows a
  /// duplicate of the tab strip directly above it — in the normal state every
  /// one of them is already a tab, so the row's only effect was to focus it,
  /// which is what clicking the tab does. The group earns its place only when
  /// something is genuinely missing.
  ///
  /// It sits LAST for a reason: it is the one group whose length changes as
  /// you work, and a variable-length group anywhere else would shift the rows
  /// below it between two openings of the same menu. Muscle memory needs the
  /// fixed groups to keep a fixed offset from the top.
  List<CcMenuItem> _reopenGroup(String leafId, AppLocalizations l10n) {
    final open = {for (final t in _layout.allTabs()) t.kind};
    final closed = <(String kind, String label)>[
      (PrTabKinds.overview, l10n.overview),
      (PrTabKinds.diff, l10n.diff),
      (PrTabKinds.actions, l10n.actions),
      (PrTabKinds.reviewArtifact, l10n.prReviewArtifactTab),
      (PrTabKinds.sourceControl, l10n.sourceControl),
      (PrTabKinds.chat, l10n.chat),
    ].where((e) => !open.contains(e.$1)).toList();
    if (closed.isEmpty) {
      return const [];
    }
    return [
      const CcMenuItem.divider(),
      CcMenuItem.section(l10n.ideMenuSectionReopen),
      for (final (kind, label) in closed) _reopenTabItem(leafId, kind, label),
    ];
  }
  ...
  /// Fallback for a permalinked comment the diff cannot anchor: an outdated
  /// thread (its line is gone) or a file outside the diff's scope.
  ///
  /// The Overview timeline renders those conversations in full, so the reader
  /// still gets to the comment. Doing nothing would be worse than landing on
  /// the wrong tab — a notification whose link visibly does nothing reads as a
  /// broken app.
  void _revealCommentOnOverview(int commentId) {
    if (!mounted) {
      return;
    }
    _focusKind(PrTabKinds.overview);
  }
  ...
  /// The PR space's isolated worktree root for [tab]'s repo on the server, or
  /// null when it can't be resolved (no repo/workspace/space yet).
  Future<String?> _worktreeBaseFor(EditorTab tab) async {
    ...
    final isolated = await ref
        ...
        .forUnitRepo(workspaceId, spaceId, repoId);
    ...
    switch (tab.kind) {
      ...
      case PrTabKinds.diff:
        return PrDiffTab(
          ...
          pendingCommentAnchor: _pendingCommentId,
          onCommentNotInDiff: _revealCommentOnOverview,
      ...
      case PrTabKinds.actions:
        final checksAsync = ref.watch(prCheckRunsProvider(widget.prRef));
      ...
      case PrTabKinds.rig:
        // Scoped to the PR's own space, the same one the terminal tab uses,
      ...
      // Legacy review kinds fold into the artifact tab (decode-compat): a
      // standing dashboard tab, and before it a split Findings / Studio pair.
      case PrTabKinds.review:
      case PrTabKinds.aiReview:
      case PrTabKinds.reviewStudio:
        return PrReviewArtifactTab(pr: widget.pr);
      default:
  ...
  ///
  /// Empty when the probe has not answered or the server is too old to report
  /// engines; [RigTabSurfaces.targets] reads that as Chromium, which is what
  /// such a server runs.
  ...
  Widget build(BuildContext context) {
    ...
    final pollingState = ref.watch(prDetailPollingProvider(widget.prRef));
    ...
    ref.listen(prPreviewDeploymentsProvider(widget.prRef), (_, next) {
      _reconcilePreviewTabs(next);
    });

    ref.listen(prDetailProvider(widget.prRef), (prev, next) {
