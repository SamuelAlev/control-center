...
class _PullRequestListScreenState extends ConsumerState<PullRequestListScreen> {
  ...
  void initState() {
    super.initState();
    _bodyScrollController.addListener(_onScroll);
  ...
  /// How close to the bottom (px) the operator must scroll before the next page
  /// auto-loads — sized so paging starts before the very last row is reached.
  static const double _autoLoadThreshold = 600;

  /// Pages in the next batch for every repo that still has more, once the
  /// operator scrolls near the bottom — the infinite-scroll companion to the
  /// per-repo "load more" buttons. Search results are a single server page, so
  /// auto-loading is suppressed there (matching the suppressed buttons).
  void _onScroll() {
    if (!_bodyScrollController.hasClients) {
      return;
    }
    final pos = _bodyScrollController.position;
    if (pos.pixels < pos.maxScrollExtent - _autoLoadThreshold) {
      return;
    }
    if (ref.read(prSearchQueryProvider).isActive) {
      return;
    }
    ...
    final notifier = ref.read(prsByRepoProvider.notifier);
    for (final entry in state.hasMore.entries) {
      final repoId = entry.key;
      if (entry.value != true ||
          state.loadingMore[repoId] == true ||
          collapsed.contains(repoId)) {
        continue;
      }
      // `loadMore` re-guards (hasMore / not-already-loading) and flips its
      // loading flag synchronously, so repeated scroll ticks coalesce.
      notifier.loadMore(repoId);
    }
  }
  ...
  Future<void> _mergeCursor(PrListData data, String login) async {
    ...
    if (!lanesOfPr(pair.pr, login).contains(DecisionLane.ready)) {
      CcToastScope.of(context).show(
        AppLocalizations.of(context).laneReadyHint,
  ...
  Widget build(BuildContext context) {
    ...
    // The thin client holds no GitHub token; auth is the SERVER's (reported by
    // the PR-list RPC op). Optimistic while loading so the "connect GitHub" gate
    // never flashes before the first server response.
    final isAuthed = ref
        .watch(prsByRepoProvider)
        .maybeWhen(data: (s) => s.authenticated, orElse: () => true);
  ...
  Widget _buildBody({
    ...
    return Stack(
      children: [
        // The queue scrolls through a CustomScrollView so the rows render via a
        // lazy SliverList (one [RepoPrSectionSliver] per repo) instead of an
        // eager Column — only the on-screen rows are built and laid out. The
        // centred `maxWidth` of the old `Center`+`ConstrainedBox` is preserved
        // with symmetric sliver padding derived from the viewport width.
        LayoutBuilder(
          builder: (context, constraints) {
            final side = math.max(
              AppSpacing.xl,
              (constraints.maxWidth - kPrListMaxWidth) / 2,
            );
            return ReadyAutoScroll(
              controller: _bodyScrollController,
              child: CustomScrollView(
                controller: _bodyScrollController,
                // The default cache extent keeps a few rows built beyond the
                // viewport, so the keyboard cursor's single-step moves (the
                // common case) still land on a built row for
                // `Scrollable.ensureVisible`. Far jumps to an unbuilt row no
                // longer auto-scroll — an accepted trade for virtualization.
                slivers: [
                  SliverPadding(
                    padding: EdgeInsets.fromLTRB(side, AppSpacing.lg, side, 0),
                    sliver: SliverToBoxAdapter(
                      child: DecisionLanesRail(counts: counts),
                    ),
                  ),
                  SliverPadding(
                    padding: EdgeInsets.fromLTRB(side, AppSpacing.lg, side, 0),
                    sliver: _queuePanelSliver(
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: PrKeyboardHints.queue(l10n),
                    ),
                  ),
                ],
              ),
  ...
  /// The bordered queue panel as a sliver: the toolbar over the repo-group
  /// slivers, read as one flat instrument table. A [DecoratedSliver] paints the
  /// panel fill + hairline border + radius around the whole group (replacing
  /// the old `DecoratedBox` + `ClipRRect`); the toolbar is transparent, so the
  /// rounded corners read through it.
  Widget _queuePanelSliver({
  ...
  /// The repo-group slivers inside the queue panel: a loading skeleton, an
  /// empty state, or one lazily-built [RepoPrSectionSliver] per repo.
  List<Widget> _buildQueueBodySlivers({
    required PrListData? data,
    required String login,
    required List<Repo> sortedRepos,
    required bool isSearching,
    ...
    if (data == null || data.byRepo.isEmpty) {
      return const [SliverToBoxAdapter(child: _QueueEmpty())];
    }
