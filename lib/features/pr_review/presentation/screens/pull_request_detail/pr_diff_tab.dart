...
class PrDiffTab extends ConsumerStatefulWidget {
  ...
  /// A comment (REST id) a permalink asked to reveal: this tab resolves it to
  /// a thread, selects its file, expands it, scrolls it into view, then clears
  /// the signal. Null when the Diff tab is used standalone.
  final ValueNotifier<int?>? pendingCommentAnchor;

  /// Called when a pending comment cannot be shown in the diff — it is
  /// outdated (its line is gone) or its file is outside the current scope.
  /// The host re-routes to the Overview timeline, where the conversation can
  /// still be read in full. Never a silent no-op: a permalink that does
  /// nothing looks like a broken notification.
  final void Function(int commentId)? onCommentNotInDiff;
...
class _PrDiffTabState extends ConsumerState<PrDiffTab> {
  ...
  void dispose() {
    ...
    widget.pendingCommentAnchor?.removeListener(_maybeRevealPendingComment);
  ...
  /// Consumes a pending comment permalink: resolve the comment to its thread,
  /// open that thread's file and scroll the thread into view.
  ///
  /// Retries across frames like [_maybeJumpToPendingFile], because two things
  /// arrive asynchronously and neither is guaranteed on the frame the URL is
  /// applied: the comment list (an RPC stream) and the mounted diff state.
  void _maybeRevealPendingComment() {
    final commentId = widget.pendingCommentAnchor?.value;
  ...
  ///
  /// A draft the viewer can publish takes the same slot with a Ready-for-review
  /// button instead, again mirroring the Overview cluster: reading the diff is
  /// exactly when you decide a draft is finished.
  List<Widget> _buildReviewButton() {
    if (!widget.pr.isOpen) {
      return const [];
    }
    if (widget.pr.isDraft && canToggleDraft(ref, widget.pr, widget.prRef)) {
      return [MarkReadyForReviewButton(pr: widget.pr, prRef: widget.prRef)];
    }
    final currentLogin = ref.watch(currentUserLoginForPrProvider(widget.prRef));
    final isAuthor =
    ...
    final repo = ref.watch(prRepoRowProvider(widget.prRef));
    return [
      ReviewOverlayButton(
        pr: widget.pr,
        prRef: widget.prRef,
