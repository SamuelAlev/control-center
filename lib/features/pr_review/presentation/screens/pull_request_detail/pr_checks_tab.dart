...
class ChecksTab extends ConsumerStatefulWidget {
  ...
  /// The PR's identity key (repo coords + number) — the repo the checks'
  /// workflow/job detail fetches bind to.
  final PrRef prRef;
  ...
  String? _timeLabelFor(BuildContext context, WorkflowGroup w) {
    ...
    if (w.status == WorkflowStatus.running) {
      ...
      return earliest == null
          ? null
          : l10n.workflowRunStartedAgo(formatRelativeTime(context, earliest));
    }
    DateTime? latest;
    for (final j in w.jobs) {
    ...
    return latest == null
        ? null
        : l10n.workflowRunCompletedAgo(formatRelativeTime(context, latest));
  }
}
...
class _WorkflowRunBody extends ConsumerStatefulWidget {
  const _WorkflowRunBody({required this.prRef, required this.workflow});
...
class _WorkflowRunBodyState extends ConsumerState<_WorkflowRunBody> {
  ...
  Widget build(BuildContext context) {
    ...
    final graphAsync = ref.watch(
      prWorkflowGraphProvider((pr: widget.prRef, workflowRunId: runId)),
    );
  ...
  Widget _selectionDetail(
    ...
    final children = byNodeId[externalId] ?? const <CheckRun>[];
  ...
  Widget _childDetail(CheckRun c) {
    if (c.jobId != null) {
      return JobStepsAccordion(prRef: widget.prRef, checkRun: c);
    }
    return _JobTile(prRef: widget.prRef, job: c, isFirst: true, isLast: true);
  }
