...
class _PrSourceControlTabState extends ConsumerState<PrSourceControlTab> {
  ...
  Future<void> _stage(String spaceId, String repoId, List<String> paths) async {
  ...
  Future<void> _syncToPrHead(String spaceId, String repoId) async {
  ...
  Future<void> _runCommit(
    ...
    if (workspaceId == null ||
        (message.isEmpty && action != ScmCommitAction.amend)) {
      return;
    }
    final push =
  ...
  Widget build(BuildContext context) {
    ...
    return spaceAsync.when(
      ...
      data: (spaceId) {
        ...
        // Stopped counts as failed here: with no checkout there is no working
        // tree to diff, so showing "no changes" would be a lie.
        if (status.isStopped) {
              ...
              children: [
                SizedBox(
                  ...
                  child: _ChangesList(
                    ...
                    onSyncToPrHead: () => _syncToPrHead(spaceId, repoId),
                    onStage: (p) => _stage(spaceId, repoId, [p]),
                    onUnstage: (p) => _unstage(spaceId, repoId, [p]),
                    onDiscard: (p) => _discard(spaceId, repoId, [p]),
