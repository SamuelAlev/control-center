...
class TreeOverlay extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final rawFiles = ref.watch(prFilesProvider(pr.number)).value ?? const [];
    if (rawFiles.isEmpty) {
...
/// Resolves the PR channel/worktree ids (which provisions the PR-head worktree
/// on first open — acceptable here since the user explicitly opened search) and
/// hosts the [PrWorktreeSearchPanel]. A missing workspace/repo/channel shows a
...
class _SearchHost extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final prFiles = ref.watch(prFilesProvider(pr.number)).value;
    ...
    return channelAsync.when(
      ...
      data: (channelId) => PrWorktreeSearchPanel(
