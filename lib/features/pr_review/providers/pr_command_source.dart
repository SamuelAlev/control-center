...
class PrCommandSource implements CommandSource {
  ...
  CommandItem _prToCommandItem(
    ...
    final desc =
        authorLogin.isNotEmpty ? '${pr.repoFullName} · $authorLogin' : pr.repoFullName;
  ...
  IconData _iconForState(PullRequest pr) {
    if (pr.isDraft) return LucideIcons.filePen;
    if (pr.isMerged) return LucideIcons.gitMerge;
    if (pr.isClosed) return LucideIcons.gitPullRequestClosed;
