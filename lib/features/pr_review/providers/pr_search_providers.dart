...
/// Binds the PR search port to the RPC adapter — the search runs SERVER-SIDE on
/// the host's gh-authenticated client (the thin client holds no token).
final prSearchPortProvider = Provider<PrSearchPort>((ref) {
  return RpcPrSearchPort(ref.watch(rpcClientProvider));
});
