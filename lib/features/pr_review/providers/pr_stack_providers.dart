...
/// The stack containing the open PR detail's `prNumber`, or null. Resolved
/// against the pinned detail repo via the PR-scoped review repository and
/// re-fetched when the PR's head/base refs move (a restack rewires both).
final currentPrStackProvider = FutureProvider.autoDispose.family<PrStack?, int>(
  (ref, prNumber) async {
    ref.watch(
      prDetailProvider(
        prNumber,
    ...
    try {
      final stacks = await repository.listStacks(prNumber: prNumber);
