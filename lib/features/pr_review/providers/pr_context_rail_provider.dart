...
final prContextRailProvider =
    FutureProvider.autoDispose.family<PrContextData, int>((ref, prNumber) async {
  final pr = await ref.watch(prDetailProvider(prNumber).future);
  if (pr == null) {
    return const PrContextData(
      authorRecentPrs: [],
      relatedMessages: [],
    );
  }
...
List<String> _extractKeywords(String title) {
  const stopWords = {
    'update', 'fixes', 'added', 'removes', 'refactor', 'chore',
    'tests', 'merge', 'branch', 'initial', 'cleanup', 'feature',
    'bumps', 'change', 'changes', 'improve', 'revert',
  };
  return title
      .toLowerCase()
      .split(RegExp(r'[\s/\-_:()[\]{}.,]+'))
