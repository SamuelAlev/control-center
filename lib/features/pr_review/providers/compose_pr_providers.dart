...
/// repo, or null when the conversation owns no worktree for it (or no channel is
...
final worktreeBranchProvider = FutureProvider.autoDispose
    .family<String?, String>((ref, channelId) async {
      ...
      final isolated = await ref
          ...
          .forUnitRepo(workspaceId, channelId, repo.id);
...
class ComposePrNotifier extends Notifier<ComposePrState> {
  ...
  /// Pushes [channelId]'s worktree branch to `origin` so it can serve as the PR
  ...
  /// asked for.
  Future<WorktreePublishResult?> publishHead({
    required String channelId,
  }) async {
    final repo = ref.read(activeRepoProvider);
    final workspaceId = ref.read(activeWorkspaceIdProvider);
    if (channelId.isEmpty || repo == null || workspaceId == null) {
      return null;
    }
