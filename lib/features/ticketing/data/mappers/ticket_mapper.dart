...
class TicketMapper {
  ...
  TicketsTableCompanion toCompanion(Ticket t) {
    return TicketsTableCompanion(
      ...
      mode: Value(t.mode.toDbValue()),
      pipelineRunId: Value(t.pipelineRunId),
      pipelineStepId: Value(t.pipelineStepId),
      expectedOutputSchema: Value(
        t.expectedOutputSchema != null
            ? jsonEncode(t.expectedOutputSchema)
            : null,
      ),
      outputContractMode: Value(t.outputContractMode.toStorageString()),
      outputJson:
          Value(t.outputJson != null ? jsonEncode(t.outputJson) : null),
      ...
      checkoutRunId: Value(t.checkoutRunId),
      executionLockedAt: Value(t.executionLockedAt),
      checkoutAgentId: Value(t.checkoutAgentId),
      executionPolicyJson: Value(t.executionPolicyJson),
      executionStateJson: Value(t.executionStateJson),
      recoveryActionsJson: Value(t.recoveryActionsJson),
  ...
  Ticket fromRow(
    ...
    return Ticket(
      ...
      expectedOutputSchema: row.expectedOutputSchema != null
          ...
          : null,
      outputContractMode:
          OutputContractMode.fromStorage(row.outputContractMode),
      ...
      checkoutAgentId: row.checkoutAgentId,
      executionPolicyJson: row.executionPolicyJson,
      executionStateJson: row.executionStateJson,
      recoveryActionsJson: row.recoveryActionsJson,
