import 'package:control_center/features/ticketing/data/providers/linear/linear_graphql_client.dart';
...
class LinearTicketAdapter implements TicketProviderPort {
  ...
  Future<RemoteTicket> update(String externalId, RemoteTicketPatch patch) async {
    ...
    final issue = await _client.getIssue(externalId);
    if (issue == null) throw StateError('Issue $externalId not found.');
    return _toRemote(issue);
  }
  ...
  static TicketStatus _normalizeStatus(String stateName) {
    ...
    if (s.contains('review')) return TicketStatus.inReview;
    ...
    if (s.contains('block')) return TicketStatus.blocked;
    if (s.contains('backlog')) return TicketStatus.backlog;
  ...
  ({String id, String name})? _pickState(
    ...
    for (final state in states) {
      if (_normalizeStatus(state.name) == target) return state;
    }
    return null;
  }
}
