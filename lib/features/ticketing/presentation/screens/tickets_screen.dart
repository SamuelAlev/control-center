...
/// board, switchable via a persisted view toggle. The header exposes a "New
/// ticket" action beside the view toggle.
    ...
    return ColoredBox(
      ...
      child: Column(
        ...
        children: [
          _Header(
            count:
                boardAsync.asData?.value.values.fold<int>(
                  0,
                  (s, l) => s + l.length,
                ) ??
                0,
            ...
            child: Stack(
              children: [
                    ...
                    data: (board) {
                      ...
                      return LayoutBuilder(
                        builder: (context, constraints) {
                          ...
                          final masterW = (total * 0.30).clamp(240.0, 360.0);
                          return CcResizable(
                ...
                Positioned(
                  ...
                  bottom: 24,
                  child: Center(
                    child: _BulkActionBar(workspaceId: workspaceId),
                  ),
                ),
              ],
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          segment(TicketViewMode.list, AppIcons.list, l10n.ticketViewList),
...
class _TicketListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // FLATTENED: one item per header AND one per ticket, so the list
    // virtualizes per ROW.
    //
    // Building one item per GROUP meant the group's whole `Column` of rows was
    // constructed and laid out the moment the group entered the cache area —
    // the "done" column folds failed and cancelled, so a workspace with 2,000
    // closed tickets built 2,000 rows to show a handful.
    final items = <_BoardItem>[
      for (final (status, tickets) in groups) ...[
        _BoardHeader(status, tickets.length),
        for (final ticket in tickets) _BoardRow(ticket),
      ],
    ];
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final item = items[i];
        return switch (item) {
          _BoardHeader(:final status, :final count) => _TicketGroupHeader(
            status: status,
            count: count,
          ),
          // Keyed by ticket id: without it, reordering (a status change moves
          // a ticket between groups) reuses one row's State for a DIFFERENT
          // ticket, because the default key is the index.
          _BoardRow(:final ticket) => _TicketListRow(
...
/// One row of the flattened board list: a status header or a ticket.
sealed class _BoardItem {
  const _BoardItem();
}

final class _BoardHeader extends _BoardItem {
  const _BoardHeader(this.status, this.count);

  final TicketStatus status;
  final int count;
}

final class _BoardRow extends _BoardItem {
  const _BoardRow(this.ticket);

  final Ticket ticket;
}

class _TicketGroupHeader extends StatelessWidget {
  const _TicketGroupHeader({required this.status, required this.count});
  ...
  Widget build(BuildContext context) {
    ...
    // the grouped list reads calm and breathable rather than as stacked gray strips.
    return Padding(
...
class _TicketListRowState extends ConsumerState<_TicketListRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final date = MaterialLocalizations.of(
      context,
    ).formatShortMonthDay(ticket.updatedAt);

    return CcTappable(
      onPressed: () =>
          context.go(ticketDetailRoute(widget.workspaceId, ticket.id)),
...
class _BulkActionBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final l10n = AppLocalizations.of(context);
    final t = context.designSystem ?? DesignSystemTokens.light();
    // Lazy: the write-path service (and its RPC client) is only resolved when
    // the operator actually taps a bulk action, so the bar renders without a
    // connected server (e.g. in widget tests / offline).
    late final workflow = ref.read(ticketWorkflowServiceProvider);
    final ids = selection.toList();
    final agents =
        ref.watch(workspaceAgentsProvider(workspaceId)).asData?.value ??
        const <Agent>[];
    // The selected tickets (for label union math) + the workspace's label
    // universe (existing labels are what a bulk pick offers).
    final allTickets =
        ref.watch(workspaceTicketsProvider(workspaceId)).asData?.value ??
        const <Ticket>[];
    final selectedTickets = allTickets
        .where((tk) => selection.contains(tk.id))
        .toList();
    final labelUniverse = <String>{
      for (final tk in allTickets) ...tk.labels,
    }.toList()..sort();
        ...
        children: [
                      ...
                      onTap: () {
                        for (final id in ids) {
                          patchTicketOptimistic(
                            ...
                            workspaceId: workspaceId,
                            ticketId: id,
                            fields: {'priority': p.toStorageInt()},
                          );
                        }
                        toggle();
                      },
          ...
          TicketPropertyPicker(
            trigger: (context, toggle) => TicketTriggerChip(
              bordered: true,
              onTap: toggle,
              child: _BarChipLabel(icon: AppIcons.user, label: l10n.assignee),
            ),
            menu: (context, toggle) => Padding(
              padding: const EdgeInsets.symmetric(vertical: 4),
              child: Column(
                ...
                children: [
                  CcTile(
                    ...
                    onTap: () {
                      for (final id in ids) {
                        workflow.assign(id, workspaceId: workspaceId);
                      }
                      toggle();
                    },
                  ),
                    ...
                    onTap: () {
                      ...
                      for (final id in ids) {
                        workflow.assign(
                          ...
                          workspaceId: workspaceId,
                          assigneeId: me,
                          assigneeType: PrincipalType.user,
                        );
                      }
                      toggle();
                    },
          ...
          TicketPropertyPicker(
            trigger: (context, toggle) => TicketTriggerChip(
              bordered: true,
              onTap: toggle,
              child: _BarChipLabel(icon: AppIcons.tag, label: l10n.labels),
            ),
            menu: (context, toggle) => Padding(
              padding: const EdgeInsets.symmetric(vertical: 4),
              child: Column(
                ...
                children: [
                  if (labelUniverse.isEmpty)
                    ...
                    for (final label in labelUniverse)
                      CcTile(
                        leading: Icon(
                          AppIcons.tag,
                          size: 14,
                        ...
                        title: label,
                        // Adds the label to every selected ticket that lacks it
                        // (union — never wipes labels the tickets already carry).
                        onTap: () {
                  ...
                  if (selectedTickets.any((tk) => tk.labels.isNotEmpty)) ...[
                    const CcDivider(),
                    CcTile(
                      leading: Icon(
                        AppIcons.x,
...
class _TicketCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final t = context.designSystem ?? DesignSystemTokens.light();
    final isUser = ticket.isAssignedToUser;
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onSecondaryTapDown: (details) => showTicketContextMenu(
        context: context,
