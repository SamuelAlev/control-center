...
class _LinkedPrRowState extends State<_LinkedPrRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final label = resolved == null
        ? widget.nodeId
        : '#${resolved.pr.number}  ${resolved.pr.title}';
...
class _AddLinkedPrButtonState extends ConsumerState<_AddLinkedPrButton>
  ...
  Widget build(BuildContext context) {
    ...
    return FPopoverMenu.tiles(
      ...
      menu: reposWithPrs.isEmpty
          ? [
              FTileGroup(
                children: [
                  FTile(
                    enabled: false,
                    title: Text(l10n.noOpenPullRequests),
                  ),
                ],
              ),
            ]
          : [
              for (final rp in reposWithPrs)
                FTileGroup(
                  label: Text(rp.repo.fullName),
                  children: [
                    for (final pr in rp.prs)
                      FTile(
                        ...
                        suffix: widget.ticket.linkedPrIds.contains(pr.nodeId)
                            ? const Icon(LucideIcons.check, size: 16)
                            : null,
                        onPress: () => _toggleLink(pr),
                      ),
                  ],
                ),
            ],
