...
/// `ticket_pr_link` MCP tool uses — so in-app links and agent-made links
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final byNodeId = <String, _ResolvedPr>{
      for (final rp in repos)
        for (final pr in rp.prs) pr.externalId: (pr: pr, repo: rp.repo),
    };
      ...
      child: Column(
        ...
        children: [
          ...
          if (ticket.linkedPrIds.isEmpty)
            ...
            for (final externalId in ticket.linkedPrIds)
              _LinkedPrRow(
                externalId: externalId,
                resolved: byNodeId[externalId],
                onOpen: byNodeId[externalId] == null
                    ? null
                    : () => openPrInRepo(
                        ref,
                        context,
                        byNodeId[externalId]!.repo,
                        byNodeId[externalId]!.pr.number,
                      ),
                onRemove: () => ref
      ...
      child: GestureDetector(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              Icon(
                status?.icon ?? AppIcons.gitPullRequest,
                size: 14,
...
class _AddLinkedPrButtonState extends ConsumerState<_AddLinkedPrButton> {
  ...
  void _toggleLink(PullRequest pr) {
    ...
    if (widget.ticket.linkedPrIds.contains(pr.externalId)) {
  ...
  Widget build(BuildContext context) {
    ...
    final reposWithPrs = widget.repos.where((r) => r.prs.isNotEmpty).toList();
    ...
    return CcPopover(
      ...
      target: CcTappable(
        ...
        builder: (context, states) => Padding(
          ...
          child: Icon(AppIcons.plus, size: 16, color: t.fgTertiary),
        ),
      ),
                  ...
                  : Column(
                      ...
                      children: [
                        for (var i = 0; i < reposWithPrs.length; i++) ...[
                          ...
                          _MenuSectionLabel(
                            label: reposWithPrs[i].repo.fullName,
                          ),
                          for (final pr in reposWithPrs[i].prs)
                            CcTile(
                              ...
                              trailing:
                                  widget.ticket.linkedPrIds.contains(
                                    pr.externalId,
                                  )
