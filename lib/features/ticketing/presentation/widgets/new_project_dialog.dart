...
/// project is created; otherwise it edits in place. The [workspaceId] the
/// project belongs to is supplied by the caller. Returns the project id, or
/// null if cancelled.
Future<String?> showProjectDialog(
  ...
  return showCcDialog<String>(
    context: context,
    builder: (ctx) => _ProjectDialog(existing: existing, workspaceId: workspaceId),
  );
}

class _ProjectDialog extends ConsumerStatefulWidget {
  const _ProjectDialog({required this.workspaceId, this.existing});
