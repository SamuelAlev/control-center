...
class ProjectGlyph extends StatelessWidget {
  ...
  /// Whether the swatch rides a solid brand fill — a selected `CcSidebarItem`
  /// row. The 16%-alpha wash is transparent, so on that fill it becomes a
  /// faintly tinted orange square and the project hue loses its contrast. The
  /// swatch is IDENTITY, not chrome (recoloring it to `accentOn` like the
  /// row's ink would erase which project this is), so instead it gets an
  /// opaque `accentOn` backing and keeps its own hue, reading on white
  /// exactly as it does everywhere else.
  final bool onBrandFill;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: onBrandFill
            ? Color.alphaBlend(c.withValues(alpha: 0.16), t.accentOn)
...
/// [AppIcons] glyph for a relation kind (paired with the group label).
