...
class _NewTicketDialogState extends ConsumerState<_NewTicketDialog> {
  ...
  /// Whether [_assignedAgentId] names the signed-in user (vs an agent).
  bool _assigneeIsUser = false;
  String? _projectId;
  ...
  Widget build(BuildContext context) {
    ...
    // The fixed 540pt width clipped the dialog (and anything anchored to its
    // right side) on windows narrower than 540 — clamp to the viewport.
    final dialogWidth = math.min(540.0, MediaQuery.widthOf(context) - 32);
          ...
          children: [
            ...
            if (agents.isNotEmpty) ...[
              ...
              for (final a in agents)
                CcTile(
                  selected: !_assigneeIsUser && _assignedAgentId == a.id,
                  leading: TicketAssigneeAvatar(name: a.name, size: 20),
                  title: a.name,
                  trailing: !_assigneeIsUser && _assignedAgentId == a.id
                      ? const Icon(AppIcons.check, size: 16)
                      : null,
                  onTap: () {
                    setState(() {
                      _assignedAgentId = a.id;
                      _assigneeIsUser = false;
                    });
                    toggle();
                  },
