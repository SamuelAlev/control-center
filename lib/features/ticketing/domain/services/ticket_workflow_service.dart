...
class TicketWorkflowService implements TicketWorkflowPort {
  ...
  /// Validates structured output against a ticket's `expectedOutputSchema` at
  /// the `complete_ticket` boundary. Optional so existing call sites and tests
  /// that don't exercise contracts need no change; when null, contracts are
  /// not enforced (schemaless behaviour).
  ...
  /// How many schema-violating `complete_ticket` calls are tolerated before the
  /// ticket is failed (prevents an agent burning unbounded tokens re-trying).
  static const _maxContractRejections = 3;
  ...
  Future<void> completeTicket(
    ...
    // Enforce the output contract at the single chokepoint (so MCP, UI, and
    // the auto-completer all hit it). A strict violation is thrown back to the
    // caller — for the MCP tool, that error reaches the still-running agent,
    // which fixes its payload and re-calls.
    if (!force) {
      await _enforceOutputContract(ticketId, workspaceId, output);
    }
    final warnings = !force
        ? await _permissiveContractWarnings(ticketId, workspaceId, output)
        : const <String>[];
    await _mutate(
      ticketId,
      workspaceId: workspaceId,
      mutate: (ticket) {
        ...
        final now = DateTime.now();
        final metadata = warnings.isEmpty
            ? ticket.metadata
            : {
                ...ticket.metadata,
                'outputContractWarnings': warnings,
              };
        return ticket.copyWith(
          version: ticket.version + 1,
  ...
  /// Strict-mode contract gate. Loads the ticket, validates [output] against
  /// its `expectedOutputSchema`, and on violation: records the rejection count
  /// (persisted, so it survives crashes), throws
  /// [OutputContractViolationException]; once the rejection cap is hit, fails
  /// the ticket and throws with `terminal: true`. No-op when there is no schema
  /// validator, no declared schema, or the ticket is in permissive mode.
  Future<void> _enforceOutputContract(
    String ticketId,
    String workspaceId,
    Map<String, dynamic>? output,
  ) async {
    ...
    if (ticket == null) {
      return; // _mutate will no-op on a missing ticket.
    }
    _assertWorkspace(ticket.id, ticket.workspaceId, workspaceId);
    final schema = ticket.expectedOutputSchema;
    if (schema == null ||
        ticket.outputContractMode == OutputContractMode.permissive) {
      return;
    }
    final violations = _contractViolations(validator, schema, output);
    if (violations.isEmpty) {
      return;
    }

    final rejections = ((ticket.metadata['contractRejections'] as num?)
                ?.toInt() ??
            0) +
        1;
    final terminal = rejections >= _maxContractRejections;
    final detail = violations.map((v) => '- $v').join('\n');

    if (terminal) {
      await failTicket(
        ticketId,
        'Output failed the expected output schema $rejections times:\n$detail',
        workspaceId: workspaceId,
      );
      throw OutputContractViolationException(
        "Output does not match the ticket's expected output schema, and the "
        'rejection limit was reached. The ticket has been marked failed.\n'
        '$detail',
    ...
    // Persist the incremented rejection count without changing status.
    await _mutate(
      ticketId,
      workspaceId: workspaceId,
      mutate: (current) => current.copyWith(
        version: current.version + 1,
        metadata: {...current.metadata, 'contractRejections': rejections},
        updatedAt: DateTime.now(),
      ),
    );
    throw OutputContractViolationException(
      "Output does not match the ticket's expected output schema:\n$detail\n"
      'Fix the payload and call complete_ticket again.',
      violations: violations,
    );
  }

  /// Permissive-mode contract check: returns the (non-fatal) violation list to
  /// record under `metadata['outputContractWarnings']`. Empty when there is no
  /// validator/schema or the ticket is strict (strict is handled by
  /// [_enforceOutputContract]).
  Future<List<String>> _permissiveContractWarnings(
    String ticketId,
    String workspaceId,
    Map<String, dynamic>? output,
  ) async {
    ...
    return _contractViolations(validator, schema, output);
  }

  List<String> _contractViolations(
    SchemaValidatorPort validator,
    Map<String, dynamic> schema,
    Map<String, dynamic>? output,
  ) {
    if (output == null) {
      return ['output is required: this ticket declares an expected output schema'];
    }
