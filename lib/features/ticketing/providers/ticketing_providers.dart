...
final ticketBoardProvider = Provider.family<
  ...
  return ref.watch(workspaceTicketsProvider(workspaceId)).whenData((tickets) {
    final board = <TicketStatus, List<Ticket>>{
      for (final column in TicketBoardColumns.all) column: <Ticket>[],
    };
    for (final ticket in tickets) {
      final column = TicketBoardColumns.columnFor(ticket.status);
...
/// The ordered board columns and the status→column folding used by the UI.
class TicketBoardColumns {
  /// The columns shown on the board, in order.
  static const List<TicketStatus> all = [
  ...
  static TicketStatus columnFor(TicketStatus status) => switch (status) {
