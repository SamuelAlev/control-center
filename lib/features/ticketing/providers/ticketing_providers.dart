...
/// The ticketing vendor this WORKSPACE works in.
///
/// Workspace-scoped, not per-user: where a workspace's tickets live is a
/// property of the workspace, and two members looking at the same board have to
/// be looking at the same board. It was briefly a per-user preference, which
/// would have let one member file into Linear while another filed locally and
/// neither could see the other's work.
///
/// The CREDENTIAL for that vendor stays per-user (server-side, keyed by user
/// id) — everyone authenticates as themself against the one vendor the
/// workspace chose. See `ticketing_connection_providers.dart`.
final activeTicketProviderProvider = Provider<TicketProvider>(
  (ref) => TicketProvider.fromStorage(
    ref.watch(workspaceSettingProvider(ticketingProviderSettingKey)),
  ),
);

/// Switches the workspace's ticketing vendor.
///
/// Server-gated on the admin role, like every workspace setting: a member's
/// attempt is denied there even if the calling UI forgot to hide the control.
Future<void> setActiveTicketProvider(WidgetRef ref, TicketProvider provider) =>
    setWorkspaceSetting(
      ref,
      ticketingProviderSettingKey,
      provider.toStorageString(),
    );
