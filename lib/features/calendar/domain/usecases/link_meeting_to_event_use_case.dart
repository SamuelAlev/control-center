...
class LinkMeetingToEventUseCase {
  ...
  Future<void> link({
    ...
    // A narrow title-only write (not the recorder-owned `upsert`) so this works
    // over RPC on a thin client; `titleIsCustom` stays false (auto-adopted).
    await _meetings.updateTitle(
      workspaceId: workspaceId,
      meetingId: meetingId,
      title: eventTitle,
    );
