...
class CalendarOverflowButton extends StatelessWidget {
  ...
  /// Whether this row *is* the day's whole story rather than the remainder of
  /// it — true in a collapsed all-day strip, where no tile is shown above the
  /// button and "N more" would be counting from a visible zero.
  final bool summarise;

  /// The height this button claims, so a strip that ends in an overflow row
  /// can reserve the row before the button is built. Kept a touch under a full
  /// tile row: the button is a footnote to the tiles, not another one of them.
  ...
  Widget build(BuildContext context) {
    ...
    final label = summarise
        ? l10n.calendarAllDayEventCount(hiddenCount)
        : l10n.calendarMoreEvents(hiddenCount);
