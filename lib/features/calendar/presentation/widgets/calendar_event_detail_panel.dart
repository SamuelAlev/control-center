...
import 'package:control_center/features/calendar/presentation/widgets/link_meeting_sheet.dart';
...
class CalendarEventDetailPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final linkedMeetingId = ref
        .watch(
          meetingIdForEventProvider((workspaceId: workspaceId, eventId: id)),
        )
        .asData
        ?.value;
  ...
  String? _accountEmail(WidgetRef ref, String accountId) {
    final accounts =
        ref.watch(googleAccountsProvider).asData?.value ??
  ...
  List<Widget> _metaRows(CalendarEvent event, AppLocalizations l10n) {
    ...
    if (event.location != null && event.location!.trim().isNotEmpty) {
      rows.add(_MetaRow(icon: AppIcons.mapPin, text: event.location!.trim()));
    }
    if (event.recurringEventId != null) {
      rows.add(_MetaRow(icon: AppIcons.repeat, text: l10n.calendarRecurring));
    }
    if (!event.isAllDay) {
      rows.add(
        _MetaRow(
      ...
      child: Row(
        children: [
          ...
          CcIconButton(
            ...
            onPressed: () =>
                context.go(calendarRoute(context.currentWorkspaceId!)),
          ),
        ],
      ),
    );
...
class _WhenBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      primary = Row(
        children: [
          ...
          Padding(
            ...
            child: Icon(AppIcons.arrowRight, size: 13, color: t.textTertiary),
          ),
          Flexible(
            child: AppTimestamp(
              dateTime: end,
    ...
    // A single-day (or all-day) label is one concrete instant, so it carries
    // the timestamp card; a multi-day range spans two instants and stays bare.
    final Widget dateDisplay = event.isAllDay || sameDay
        ? AppTimestamp(dateTime: start, child: dateText)
        : dateText;
    ...
    return Row(
      ...
      children: [
        Padding(
          ...
          child: Icon(AppIcons.clock, size: 16, color: t.textTertiary),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [primary, const SizedBox(height: 3), dateDisplay],
          ),
        ),
      ],
    );
  ...
  TextStyle _timeStyle(DesignSystemTokens t) =>
      AppFonts.codeStyle(fontSize: 14, color: t.textPrimary);
}
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _hovered ? t.bgSecondaryHover : t.panel,
            borderRadius: AppRadii.brSm,
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(AppIcons.video, size: 15, color: t.textSecondary),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      _conferenceLabel(url, widget.l10n),
                      style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w500,
                        color: t.textPrimary,
              ...
              const SizedBox(width: 8),
              Icon(AppIcons.arrowUpRight, size: 15, color: t.textTertiary),
            ],
          ),
        ),
      ),
      ...
      children: [
        Row(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.only(top: 1),
              child: Icon(AppIcons.users, size: 15, color: t.textTertiary),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
...
class _ParticipantRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          CcAvatar(size: 26, initials: _initials(attendee)),
          const SizedBox(width: 10),
          Expanded(
            child: Row(
              children: [
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: t.surface, borderRadius: AppRadii.brSm),
      child: Text(
        label.toUpperCase(),
        style: AppFonts.codeStyle(
          fontSize: 9,
  ...
  Widget build(BuildContext context) {
    if (linkedMeetingId != null) {
      return CcButton(
        ...
        onPressed: () => context.go(
          meetingDetailRoute(context.currentWorkspaceId!, linkedMeetingId!),
        ),
    ...
    return Column(
      ...
      children: [
        CcButton(
          size: CcButtonSize.sm,
          onPressed: () => onStartRecording(event),
        ...
        CcButton(
          ...
          icon: AppIcons.link,
          child: Text(l10n.calendarLinkExistingMeeting),
        ),
      ],
    );
  }
...
class _RsvpControlsState extends ConsumerState<_RsvpControls> {
  ...
  Future<void> _respond(RsvpResponse response) async {
    ...
    try {
      await ref
          .read(calendarRsvpServiceProvider)
          .respond(widget.event, response);
    } on Object {
      toaster.show(l10n.calendarRsvpFailed, variant: CcToastVariant.danger);
    } finally {
      if (mounted) {
              ...
              child: Opacity(
                ...
                child: SegmentedToggle<String>(
                  ...
                  onChanged: (response) {
                    switch (response) {
                      case 'accepted':
                        _respond(RsvpResponse.accepted);
                      case 'declined':
                        _respond(RsvpResponse.declined);
                      case 'tentative':
                        _respond(RsvpResponse.tentative);
                    }
                  },
                  segments: [
                    (value: 'accepted', label: l10n.calendarRsvpYes),
                    (value: 'declined', label: l10n.calendarRsvpNo),
                    (value: 'tentative', label: l10n.calendarRsvpMaybe),
                  ],
                ),
              ),
            ),
            ...
            child: InkWell(
              ...
              child: Row(
                ...
                children: [
                  Padding(
                    padding: const EdgeInsets.only(top: 1),
                    child: Icon(AppIcons.link, size: 14, color: t.textTertiary),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: CcLinkText(
                      ...
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                      // decoration is ignored (custom-painted skip-ink
                      // underline); withLinkUnderline supplies the softened
                      // decorationColor.
                      style: TextStyle(
                        fontSize: 12,
                        color: t.textSecondary,
...
({IconData icon, Color color}) _statusVisual(
  ...
  switch (responseStatus) {
    case 'accepted':
      return (icon: AppIcons.circleCheck, color: t.success);
    case 'declined':
      return (icon: AppIcons.circleX, color: t.textTertiary);
    case 'tentative':
      return (icon: AppIcons.circleHelp, color: t.fgWarningPrimary);
    default:
      return (icon: AppIcons.circleDashed, color: t.idle);
  }
}
