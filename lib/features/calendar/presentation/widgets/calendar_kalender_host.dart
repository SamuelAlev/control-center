...
class _CalendarKalenderHostState extends State<CalendarKalenderHost> {
  ...
  k.ViewConfiguration _buildConfiguration() => switch (widget.mode) {
  ...
  Widget build(BuildContext context) {
    ...
    return k.CalendarView(
      ...
      body: k.CalendarBody(
        ...
        // Overlapping events cascade (later/shorter tiles sit on top, offset to
        // the right) rather than being squeezed side by side, and every tile
        // gets a minimum height so short events stay readable.
  ...
  Widget _tile(
    ...
    if (cancelled) {
      ...
      // Opaque so an overlapping tile cleanly occludes whatever sits behind it
      // — kalender cascades overlapping events (later/shorter tiles on top), and
      // a translucent fill let the back tile's text bleed through. Alpha-blending
      // the accent over the body background reproduces the soft tint while
      // staying fully opaque. Hover deepens the tint.
    ...
    // The 1px / 0.5px inset that previously lived on the Container's margin; kept
    // as outer padding so the dashed painter aligns with the box's rounded rect.
    final tile = Padding(
