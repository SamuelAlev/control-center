  ...
  void performLayout(Size size) {
    ...
    for (final group in groups) {
      ...
      for (var i = 0; i < members.length; i++) {
        ...
        final leftOverlaps = members
            .take(i)
            .where((e) => e.overlaps(member))
            .toList();
        ...
        final hasRightOverlap = members
            .skip(i + 1)
            .any((e) => e.overlaps(member));
        final width = hasRightOverlap ? columnWidth : size.width - xOffset;

        final datum = render[member.id]!;
        // kalender builds a CustomMultiChildLayout child only for the
        // on-screen (visible-index) events, yet passes *every* event to this
        // delegate so overlap math stays correct when a partner is culled.
        // Lay out / position a child only when it was actually built; the
        // x-offset + width bookkeeping below still runs for every event so
        // on-screen tiles stay aligned with off-screen overlapping partners.
        // Mirrors kalender's own SideBySideLayoutDelegate.
        if (hasChild(datum.id)) {
          layoutChild(
            datum.id,
