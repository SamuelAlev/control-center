...
class CalendarViewModeNotifier extends Notifier<CalendarViewMode> {
  ...
  void setMode(CalendarViewMode mode) {
    ...
    ref.read(appPreferencesProvider).setString(
          calendarViewModeKey,
...
/// the sidebar. A presentation projection of the synced calendar sources
/// (carrying the owning account's email and a parsed accent [Color], neither
/// of which belong in the Flutter-free domain entity).
class CalendarSourceView {
  /// Creates a [CalendarSourceView].
...
/// One account's calendars (primary first, then alphabetical), as synced by
/// the host. Read from the repository's `watchSources` — the host owns the
...
final calendarListProvider = StreamProvider.family<List<CalendarSourceView>,
    CalendarAccountListRef>((ref, account) {
  final workspaceId = ref.watch(activeWorkspaceIdProvider);
  if (workspaceId == null) {
    return Stream.value(const <CalendarSourceView>[]);
  }
  return ref
      .watch(calendarRepositoryProvider)
      .watchSources(workspaceId, account.accountId)
      .map(
        (sources) => sources
            .map(
              (e) => CalendarSourceView(
...
final allCalendarsProvider = Provider<List<CalendarSourceView>>((ref) {
  final accounts = ref.watch(googleAccountsProvider).asData?.value ?? const [];
  final all = <CalendarSourceView>[];
...
class HiddenCalendarsNotifier extends Notifier<Set<String>> {
  ...
  void toggle(String calendarId) {
    ...
    ref.read(appPreferencesProvider).setStringList(_key(ws), next.toList());
    state = next;
  }
}
