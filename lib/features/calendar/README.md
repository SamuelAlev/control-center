...
The connection, OAuth tokens and sync are **owned by `cc_server`** (spawned
locally by desktop self-serve, or a remote instance). Clients are thin: they
drive the connect flow and read synced events over RPC — they never hold a
Google token.
...
│   └── remote_calendar_connect.dart         drives calendar.connectInfo / beginConnect / pollConnect / disconnect
└── features/calendar/
...
```
Connect (GUI):  connect form → ConnectGoogleCalendarNotifier.connect(useBuiltin | clientId, secret)
...
Tokens live **only on the host**, in `FileGoogleCredentialsStore` — `google_*`
keys in the server's shared `secrets.json` (data dir / the desktop's app-support
dir), keyed by `accountId` (`google:<workspaceId>:<email>`). The
...
credentials). The server only _syncs_ — it never writes an RSVP — so it omits
...
The OAuth **client id + secret** come from one of two places:

- **The server's own client** (`use_builtin`), resolved from
  `--google-client-id` / `GOOGLE_OAUTH_CLIENT_ID`, else the pair baked into a
  release build (`builtin_credentials.dart`). `calendar.connectInfo` reports only
  _whether_ one exists; neither the id nor the secret is ever sent to a client.
  An account connected this way stores
  `GoogleServerCredentials.usesBuiltinClient` — a **marker, not a copy** — and
  `ServerGoogleTokenManager` re-resolves the pair from the server's current
  config on every refresh. That is what lets the secret be rotated by shipping a
  new build instead of stranding every connected account.
- **The user's own client**, typed into the connect form or passed to the CLI,
  stored literally per account (a workspace may hold accounts from different
  Google projects).

Google documents a device-code client's secret as non-confidential precisely
because it ships in installed apps: extracting it buys quota abuse and
consent-screen impersonation, never access to anyone's calendar, since every
refresh token stays in that user's own server. It is still kept host-side and out
of the public repo, so a leak has to be worked for.
...
1. Asks `calendar.connectInfo` whether the host has a Google app of its own. If
   it does, "use Control Center's Google app" leads and the client id/secret
   boxes only appear once the user picks "use my own Google Cloud client"; if it
   does not, the form is exactly the two boxes it always was.
2. Calls `calendar.beginConnect` (with `use_builtin`, or with the typed pair);
   shows the **user code** + an "open verification page" button.
...
(The client id/secret may instead come from `GOOGLE_OAUTH_CLIENT_ID` /
`GOOGLE_OAUTH_CLIENT_SECRET`.) It prints a code + URL, blocks until you
...
None of these values is confidential to a _client_ — the secret lives on the
host. It is safe to keep server-side.

> **Scope caveat.** Google's device-code (TV & limited-input) client type only
> grants an _allow-listed_ set of scopes. If Calendar is not permitted for that
