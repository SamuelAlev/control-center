...
- `GoogleCredentialsRepository` suffixes every keychain key with `__<accountId>`,
  and account ids embed the workspace, so one workspace's tokens are
  structurally unreadable from another.
