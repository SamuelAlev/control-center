...
class GoogleTokenManager {
  ...
  Future<GoogleCredentials?> _doRefresh(String accountId) async {
    ...
    try {
      ...
      final refreshedCreds = current.copyWithAccessToken(
        accessToken: refreshed.accessToken,
        expiresAt: refreshed.expiresAt,
      );
      await _repository.save(accountId, refreshedCreds);
  ...
  return GoogleTokenManager(
    ...
    onRefreshTokenInvalid: (accountId) async {
      ...
      if (workspaceId == null) {
        AppLog.w(
          'GoogleAuth',
          'Cannot flag reauth: malformed accountId $accountId',
        );
        return;
      }
      ...
      try {
        transitioned = await repo.markNeedsReauth(workspaceId, accountId, now);
      } on UnsupportedError {
        // Thin client (RPC): reauth state is host-owned and the host detects
        // the invalid grant on its own sync, so there is nothing to flag or
        // notify from here.
        return;
      }
      if (!transitioned) {
      ...
      final email =
          (await repo.getAccounts(workspaceId))
