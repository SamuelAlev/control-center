...
/// Compact avatar strip for the channel header: one chip per participant
/// (human or agent) whose presence currently targets this channel — viewing
...
/// nobody relevant is on this specific channel.
class WhosHereStrip extends ConsumerWidget {
  /// Creates a [WhosHereStrip] for [channelId].
  const WhosHereStrip({super.key, required this.channelId});

  /// The channel this header belongs to.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final here = [
      for (final p in others)
        if (_targetsChannel(p, channelId)) p,
    ];
    if (here.isEmpty) {
      return const SizedBox.shrink();
    }
      ...
      children: [
        for (final p in here)
          Padding(
            ...
            child: PresenceAvatarChip(participant: p, channelId: channelId),
          ),
      ],
    );
  }

  static bool _targetsChannel(ParticipantPresence p, String channelId) {
    final locus = p.locus;
    return (locus is ChannelLocus && locus.channelId == channelId) ||
        p.typingInChannelId == channelId;
  }
}
