...
/// participant (human or agent) is spotlighting this space (PRD 16 §5).
...
class SpotlightBanner extends ConsumerWidget {
  /// Creates a [SpotlightBanner] for [spaceId].
  const SpotlightBanner({super.key, required this.spaceId});

  /// The space this banner watches.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final p in roster) {
      ...
      if (p.spotlightSpaceId != spaceId) {
        continue;
      }
      if (dismissed.contains(
        DismissedSpotlightsNotifier.keyFor(p.principal, spaceId),
      )) {
        continue;
      }
      presenter = p;
