...
String? routeForLocus(String workspaceId, PresenceLocus locus) {
  switch (locus) {
    case ChannelLocus(:final channelId):
      return channelRoute(workspaceId, channelId);
...
/// Session-scoped (never persisted) set of `(principal, channel)` spotlight
/// instances the user has dismissed, keyed by `"<principal.wire>|<channelId>"`
/// (PRD 16 §5).
class DismissedSpotlightsNotifier extends Notifier<Set<String>> {
  @override
  Set<String> build() => const {};

  /// The dedup key for a `(principal, channelId)` spotlight instance.
  static String keyFor(Principal principal, String channelId) =>
      '${principal.wire}|$channelId';

  /// Whether this exact spotlight instance has been dismissed.
  bool isDismissed(Principal principal, String channelId) =>
      state.contains(keyFor(principal, channelId));

  /// Dismisses this spotlight instance (hides its banner; does not navigate).
  void dismiss(Principal principal, String channelId) {
    state = {...state, keyFor(principal, channelId)};
  }
}
...
/// Auto-navigates to a teammate's or agent's spotlighted channel the first
/// time it appears on the roster (PRD 16 §5) — once per `(principal,
/// channel)` instance and never for a pair the user has dismissed. The
...
final spotlightSyncProvider = Provider<void>((ref) {
  ...
  ref.listen(presenceRosterProvider(workspaceId), (previous, next) {
    ...
    for (final p in roster) {
      ...
      final channelId = p.spotlightChannelId;
      if (channelId == null) {
        continue;
      }
      final key = DismissedSpotlightsNotifier.keyFor(p.principal, channelId);
      if (navigated.contains(key) || dismissed.contains(key)) {
        continue;
      }
      navigated.add(key);
      ref.read(routerProvider).go(channelRoute(workspaceId, channelId));
    }
