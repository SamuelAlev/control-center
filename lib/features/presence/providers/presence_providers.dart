...
class MyPresenceState {
  ...
  /// The space this client is typing in right now, or null.
  final String? typingSpaceId;

  /// The space this client is spotlighting (presenting), or null.
  ...
  /// for [locus]/[typingSpaceId]/[spotlightSpaceId] to clear that field;
  /// omitting the parameter leaves it unchanged.
  MyPresenceState copyWith({
    bool? dnd,
    Object? locus = _unset,
    Object? typingSpaceId = _unset,
    Object? spotlightSpaceId = _unset,
    List<SoftClaim>? claims,
  }) => MyPresenceState(
    dnd: dnd ?? this.dnd,
    locus: identical(locus, _unset) ? this.locus : locus as PresenceLocus?,
    typingSpaceId: identical(typingSpaceId, _unset)
        ? this.typingSpaceId
        : typingSpaceId as String?,
    spotlightSpaceId: identical(spotlightSpaceId, _unset)
        ? this.spotlightSpaceId
        : spotlightSpaceId as String?,
...
class MyPresenceNotifier extends Notifier<MyPresenceState> {
  ...
  /// Sets the space this client is typing in (`null` to clear). Clears
  /// itself after 5s of inactivity — call again on every keystroke to keep it
  /// alive.
  void setTyping(String? spaceId) {
    _typingClearTimer?.cancel();
    final changed = state.typingSpaceId != spaceId;
    if (changed) {
      state = state.copyWith(typingSpaceId: spaceId);
    }
    if (spaceId != null) {
  ...
  /// Sets (or clears) the space this client is spotlighting/presenting.
  void setSpotlight(String? spaceId) {
    if (state.spotlightSpaceId == spaceId) {
      return;
    }
    state = state.copyWith(spotlightSpaceId: spaceId);
    _publish(force: true);
  }
  ...
  Future<void> _publishNow() async {
    ...
    final wire = <String, dynamic>{
      ...
      'ty': ?state.typingSpaceId,
      'sp': ?state.spotlightSpaceId,
...
PresenceLocus? presenceLocusForPath(String path) {
  final space = RegExp(r'^/workspaces/[^/]+/spaces/([^/]+)$').firstMatch(path);
  if (space != null) {
    return SpaceLocus(spaceId: space.group(1)!);
  }
