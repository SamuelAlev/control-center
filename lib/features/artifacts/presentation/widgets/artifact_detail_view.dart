...
///
/// A host that already scrolls — the PR review tab keeps the report and its
/// findings in ONE outer scroll — sets [ownsScroll] to false and the body
/// shrink-wraps into that scroll instead of expanding a viewport of its own.
class ArtifactDetailView extends ConsumerStatefulWidget {
  ...
  /// Whether the view scrolls its own body (the default, for hosts that give it
  /// a bounded pane). A host whose own scrollable already covers the body — a
  /// sliver offers unbounded height, which an `Expanded` child cannot survive —
  /// MUST pass false so the body shrink-wraps.
        ...
        return revisions.when(
          ...
          data: (list) {
            ...
            return Column(
              ...
              // Without a scroll of its own the column must shrink-wrap: the
              // host's sliver offers unbounded height, which a flex child
              // cannot take.
              mainAxisSize: widget.ownsScroll
                  ? MainAxisSize.max
                  : MainAxisSize.min,
              children: [
