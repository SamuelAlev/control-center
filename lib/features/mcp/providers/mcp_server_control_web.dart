// Web/thin-client binding for the MCP-server control surface.
//
// The web client hosts no MCP server of its own — it asks the connected SERVER
// (which owns the in-process MCP HTTP server) to start/stop/reconfigure it over
// the `mcp.*` RPC ops. When the connected server doesn't expose them (a host
// that wired no McpServerControl), the status provider resolves to `null` and
// the section degrades to "MCP not available on this server".
library;
...
/// The MCP-server control the settings section drives. On web this is the
