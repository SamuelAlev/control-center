...
// (`TerminalSessionView`) are disposable — unmounting one (e.g. on a channel
  ...
  void _attach(RemoteTerminalRepository repo, String sessionId) {
    ...
    _outputSub = repo
        ...
        .listen(
          ...
          onDone: () {
            if (_disposed || _sessionLost) {
              // A lost session already reported itself above and is respawning;
              // its stream closing is not a shell exit.
              return;
            }
            terminal.write('\r\n[shell exited]\r\n');
            onShellExit?.call();
