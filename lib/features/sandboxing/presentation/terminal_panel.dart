...
class TerminalPanel extends ConsumerStatefulWidget {
  ...
  /// Background color for the panel surface. When null, falls back to
  /// `bgPrimaryAlt` (the default terminal surface). Pass a token from the host
  /// to match the surrounding surface (e.g. `bgPrimary` in the IDE editor).
...
class _TerminalPanelState extends ConsumerState<TerminalPanel> {
  ...
  Future<void> _reset() async {
    ...
    // The awaits above can outlive this widget (the panel may be torn down while
    // a shell restart is in flight). Touching `ref` after that throws, so bail
    // out — mirrors the guard already in _boot().
    if (!mounted) {
      return;
    }
