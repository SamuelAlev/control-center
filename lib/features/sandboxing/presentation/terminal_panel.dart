...
class TerminalSession {
  ...
  @override
  int get hashCode =>
      Object.hash(sessionId, channelId, workspaceId, agentId, backend);
}
...
class _TerminalSessionViewState extends ConsumerState<TerminalSessionView> {
  ...
  RigClipboardBridge? get _bridge {
    ...
    if (client == null || !_isEnclosed || session.channelId.isEmpty) {
      return null;
    }
    final rig = ref.read(
              ...
              Expanded(
                child: Stack(
                  children: [
                    ...
                    if (isVmTerminal && controller.session.channelId.isNotEmpty)
                      Positioned(
                        top: 2,
                        right: 6,
                        child: RigPortsButton(
                          workspaceId: controller.session.workspaceId,
                          conversationId: controller.session.channelId,
                        ),
                      ),
                  ],
                ),
