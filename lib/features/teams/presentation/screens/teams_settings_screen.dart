...
/// Settings → Teams: group agents into teams and route work through a leader.
///
/// A master-detail surface — the roster of teams on the left, the selected
/// team's editor (name, leader, instructions, members) on the right. Below a
/// breakpoint it collapses to a single column that swaps the list for the
/// detail when a team is selected.
class TeamsSettingsScreen extends ConsumerStatefulWidget {
  /// Creates a [TeamsSettingsScreen].
  const TeamsSettingsScreen({super.key});

  @override
  ConsumerState<TeamsSettingsScreen> createState() =>
      _TeamsSettingsScreenState();
}

class _TeamsSettingsScreenState extends ConsumerState<TeamsSettingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return PageWrapper(
      ...
      subtitle: l10n.teamsManageSubtitle,
      actions: [
        if (workspaceId != null)
          CcButton(
            onPressed: () => _createTeam(workspaceId),
      ...
      child: workspaceId == null
          ...
          : _Body(
              ...
              onCreate: () => _createTeam(workspaceId),
            ),
    );
  }

  Future<void> _createTeam(String workspaceId) async {
...
class _Body extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context);
    final teamsAsync = ref.watch(teamsForWorkspaceProvider(workspaceId));
    final agents =
    ...
    return teamsAsync.when(
      ...
      data: (teams) {
        ...
        final selected =
            ...
            (selectedTeamId == null ? null : teams.first);

        return LayoutBuilder(
          builder: (context, constraints) {
            final isWide =
                constraints.maxWidth >=
                _TeamsSettingsScreenState._wideBreakpoint;
            ...
            return Row(
              ...
              children: [
                SizedBox(
                  width: _TeamsSettingsScreenState._rosterWidth,
                  child: roster,
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: CcEmptyState(
        ...
        message: l10n.noActiveWorkspaceCreate,
        description: l10n.createOrSelectWorkspace,
      ),
    );
  }
}
