...
class OrchestrationProposalBubble extends ConsumerWidget {
  ...
  /// The orchestration-proposal space message.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final async = ref.watch(
      orchestrationProvider((workspaceId: workspaceId, id: orchestrationId)),
    );

    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      // `Container`, not `DecoratedBox`: a decoration paints behind the child,
      // so any opaque full-bleed child (a tinted header, a filled footer) covers
      // the 1px side border. `Container` insets the child by the border's
      // dimensions, which keeps the card's edge intact whatever it holds.
      child: Container(
...
class _Card extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifier = ref.watch(orchestrationProposalNotifierProvider.notifier);
    final busy =
        ref.watch(orchestrationProposalNotifierProvider).busyId ==
        orchestration.id;
              ...
              if (orchestration.estimatedCostCents != null)
                _metaRow(
                  ...
                  l10n.orchestrationEstimatedCost(
                    (orchestration.estimatedCostCents! / 100).toStringAsFixed(
                      2,
                    ),
                  ),
                ),
            ...
            child: Wrap(
              ...
              children: [
                CcButton(
                  ...
                  // The design-system button owns the busy affordance: it swaps
                  // the leading icon for an inline spinner at the same size, so
                  // the button never resizes when it flips state.
                  loading: busy,
                  icon: AppIcons.play,
                  child: Text(l10n.orchestrationApprove),
                ),
                CcButton(
                  onPressed: busy
                      ? null
                  ...
                  icon: AppIcons.x,
                  child: Text(l10n.orchestrationReject),
                ),
                // Edit / estimate / partial-approve the plan as a DAG
                // (PRD 17). The bubble stays the quick approve/reject; the
                // Studio is the full surface.
                CcButton(
                  onPressed: () => openPlanStudio(
                    context,
                    workspaceId: workspaceId,
                    kind: PlanTabKind.orchestration,
                    id: orchestration.id,
                    title: p.goal,
                  ),
