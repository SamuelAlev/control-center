...
/// chosen OUTPUT device: ambience is the one audio surface you plausibly want
/// on different hardware than meetings (headphones vs desk speakers), and the
/// selection in You → Audio is applied here. Web has no output routing (the
/// browser decides), so the device application is native-only.
...
class _SoundscapeAudioHostState extends ConsumerState<SoundscapeAudioHost> {
  ...
  Future<void> _applyState(SoundscapeState s) async {
    ...
    try {
      ...
      // media_kit's volume is 0–100; the soundscape state stores 0..1.
      await player.setVolume(s.volume * 100.0);
      if (_disposed) {
        return;
      }
      if (url != _currentUrl) {
