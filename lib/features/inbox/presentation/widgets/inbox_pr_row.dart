...
/// The stack-membership chip next to the PR title: a layers glyph plus the
/// PR's position inside its stack ("2/4"), with a tooltip spelling it out —
/// the shape carries the meaning, never colour alone. Renders nothing when
/// the PR isn't stacked (the common case gets no chrome).
class _StackChip extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final membership = ref.watch(
      prStackMembershipProvider((repo: item.repo, prNumber: item.pr.number)),
    );
    if (membership == null || membership.total < 2) {
      ...
      child: CcTooltip(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              '${membership.position}/${membership.total}',
...
class _UpdatedCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.designSystem ?? DesignSystemTokens.light();
    final style = CcTypography.caption.copyWith(color: tokens.textTertiary);
