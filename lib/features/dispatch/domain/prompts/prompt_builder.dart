...
import 'package:control_center/features/dispatch/domain/prompts/uncertainty_protocol.dart';
...
class PromptBuilder {
  ...
  PromptBuilder executionProcedure({ConversationMode mode = ConversationMode.chat}) {
    ...
    steps.add('**Record what you learned** — before finishing, save durable facts with `propose_fact` and private notes with `record_observation`, and promote normative rules to a policy with `propose_policy`. Do not wait to be asked.');
    ...
    steps.add('**If you need help**, use `consult_agent` to bring in a specialist, or `delegate_ticket` to split off sub-work.');
