...
2. Identify findings. Focus on NEW code (lines starting with '+'). Only flag
   issues introduced by this PR.
3. For each finding, choose a priority and a confidence score in [0.0, 1.0]:
...
4. Confidence is your honest self-assessment, not a sales pitch. A P2 at
   0.95 is more useful than a P0 at 0.40 — the verdict computation reads
   both axes.
5. Do NOT flag style nits, hypothetical concerns, or "the codebase could be
   cleaner in general." Every finding must be discrete, actionable, and
   tied to a specific file:line.
6. Be thorough on bugs and security. Be conservative on lower-priority items.
7. When you finish, call `finalize_review`. The CEO will compute the
   overall ship/hold/block verdict from your findings' priority and
   confidence scores. If you also want to submit your own verdict (optional),
   call `submit_reviewer_verdict`.
