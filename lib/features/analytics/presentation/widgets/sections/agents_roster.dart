  ...
  Widget build(BuildContext context) {
    final tokens =
        ...
        (Theme.of(context).brightness == Brightness.dark
            ? DesignSystemTokens.dark()
            : DesignSystemTokens.light());
    final color = active ? tokens.textPrimary : tokens.textTertiary;
    return InkWell(
        ...
        child: Row(
          ...
          children: [
            ...
            if (active) ...[
              ...
              Icon(AppIcons.arrowDown, size: 12, color: color),
            ],
          ],
        ),
      ),
...
class _AgentRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () => context
          .go(analyticsAgentRoute(context.currentWorkspaceId!, card.agentId)),
      child: Container(
        decoration: BoxDecoration(
          border: Border(
            bottom: BorderSide(
              color: isLast
                  ? Colors.transparent
                  : tokens.borderSecondary.withValues(alpha: 0.6),
            ),
          ),
        ),
