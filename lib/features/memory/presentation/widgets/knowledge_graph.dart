...
class _KnowledgeGraphState extends ConsumerState<KnowledgeGraph> {
  ...
  void _rebuildGraph(
    ...
    // Single-pass group-bys, replacing a `facts.where(...)` scan per domain
    // and another per topic — each of which walked the whole list.
    final activeFactsByDomain = <String, List<MemoryFact>>{};
    final factObjectsByTopic = <String, List<MemoryFact>>{};
    for (final f in facts) {
      factObjectsByTopic.putIfAbsent(f.topic, () => []).add(f);
      if (!f.isSuperseded) {
        activeFactsByDomain.putIfAbsent(f.domain, () => []).add(f);
      }
    }
    final policyObjectsByDomain = <String, List<MemoryPolicy>>{};
    for (final p in policies) {
      policyObjectsByDomain.putIfAbsent(p.domain, () => []).add(p);
    }
    ...
    for (final slug in domainNames) {
      final domainFacts = activeFactsByDomain[slug] ?? const <MemoryFact>[];
      final domainPolicies =
          policyObjectsByDomain[slug] ?? const <MemoryPolicy>[];
    ...
    // Create topic nodes
    for (final entry in factObjectsByTopic.entries) {
      final topic = entry.key;
      final topicFacts = entry.value;
    ...
    for (final fact in facts) {
      ...
      final supersededChain = _resolveSupersededChain(fact, bySuperseder);
    ...
    for (final policy in policies) {
      ...
      _nodeData[key] = NodeData(type: NodeType.policy, policy: policy);
  ...
  Map<String, Offset> _computeLayout(
    ...
    for (final domainKey in domainKeys) {
      ...
      final policiesWidth = policyKeys.isNotEmpty
          ? _policyNodeSize.width + _domainGap
          : 0.0;
    ...
    for (final topicKey in topicsByDomain.values.expand((v) => v)) {
      ...
      for (final factKey in factKeys) {
        positions[factKey] = Offset(
          factX + _factNodeSize.width / 2,
          _layerFactY,
        );
