...
import 'package:control_center/features/memory/presentation/widgets/memory_scope_widgets.dart';
...
class _FactsTabState extends ConsumerState<FactsTab> {
  ...
  /// Selected scope: [kAllMemoryScopes], null for workspace-wide, or a repo
  /// slug. Starts at "all" so nothing is hidden before the person asks.
  String? _scope = kAllMemoryScopes;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        _buildHeader(context, factsAsync.asData?.value ?? const []),
        Expanded(
  ...
  Widget _buildHeader(BuildContext context, List<MemoryFact> facts) {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              MemoryScopeFilter(
                domainSlugs: facts.map((f) => f.domain),
                value: _scope,
  ...
  Widget _buildFactList(BuildContext context, List<MemoryFact> facts) {
    ...
    filtered = filtered
        .where((f) => matchesMemoryScope(f.domain, _scope))
        .toList();
            ...
            children: [
              Row(
                children: [
                  ...
                  if (MemoryDomainScope.repoSlugOf(fact.domain) != null) ...[
                    const SizedBox(width: AppSpacing.sm),
                    MemoryScopeChip(domainSlug: fact.domain),
                  ],
                  if (fact.memoryType != MemoryType.fact) ...[
                    const SizedBox(width: AppSpacing.sm),
                    MemoryMetaChip(label: fact.memoryType.label),
                  ],
