...
class DashboardGreetingHero extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Deferred to a post-frame callback: the provider can settle (loading→data)
    // synchronously during this build, and stamping mutates lastCheckedProvider,
    // which would mark the ProviderScope dirty mid-build ("setState() called
    // during build"). Writing after the frame keeps the mutation off the build.
    ...
    return DecoratedBox(
      ...
      // Full-bleed: the panel spans the cockpit's full width and bleeds to the
      // top edge, so the only chrome is a single bottom border. Content stays
      // aligned with the centered 1080 dashboard column below.
      child: Stack(
        children: [
          Positioned.fill(
            child: IgnorePointer(
          ...
          Center(
            child: ConstrainedBox(
              ...
              child: Padding(
                // Top inset absorbs the cockpit padding that used to sit above
                // the hero (AppSpacing.xl), so the greeting text holds its
                // position now that the panel bleeds to the top edge.
    ...
    final actions = Row(
      ...
      children: [
        DashboardButton(
          ...
          onTap: () =>
              showNewTicketDialog(context, workspaceId: context.currentWorkspaceId!),
        ),
      ],
    );
