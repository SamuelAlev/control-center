...
class _PlanCanvasState extends State<PlanCanvas> {
  ...
  /// The transform as it was just BEFORE the interaction the viewer is applying
  /// right now — see [_onPointerSignal].
  ...
  /// Mouse wheel pans; ⌘/Ctrl + wheel zooms.
  ///
  ...
  /// why the trackpad already feels right and the wheel does not. A pointer
  /// signal is delivered directly to every [Listener] on the hit-test path (it is
  /// not arena-resolved), so this handler cannot pre-empt the viewer: it runs
  /// after it and rewrites the result. `onInteractionStart` fires before the
  /// viewer mutates the matrix, so [_beforeInteraction] is the pre-zoom
  /// transform; a plain wheel restores it and translates instead. Both writes
  /// happen inside the same event dispatch, so one frame is painted and nothing
  /// flickers.
  ///
  /// Everything else is left to the viewer: a trackpad pan, a trackpad/touch
  /// pinch (a `PointerScaleEvent`), and ⌘/Ctrl + wheel, whose zoom-about-the-
  /// pointer is precisely the wanted behaviour.
  void _onPointerSignal(
    ...
    // Same leash as the viewer's own drag clamp: the viewport stays inside the
    // canvas plus its boundary margin, and an axis with nothing to pan into
    // (a graph smaller than the viewport) holds still.
    const margin = _boundaryMargin;
  ...
  /// it, the step is *not* snapped back: a graph shorter than the viewport starts
  /// outside the leash, and snapping would make the first wheel tick jump the
  /// canvas hundreds of pixels the wrong way. Only a step that reduces the
            ...
            LayoutBuilder(
              ...
              builder: (context, constraints) => Listener(
                ...
                child: InteractiveViewer(
                  ...
                  // Snapshots the transform before the viewer changes it — the wheel
                  // rewrite in [_onPointerSignal] needs the pre-zoom matrix.
