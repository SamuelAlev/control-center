...
typedef ChannelKey = ({String workspaceId, String channelId});
...
class ChannelPlanExecution {
  /// Creates a [ChannelPlanExecution].
  const ChannelPlanExecution({
...
/// workspace's orchestrations (matched on `channelId`), the pipeline's step runs,
/// and the conversation's agent runs (matched on `pipelineStepId`) — so the
/// channel can show per-node progress and attribution without a new RPC.
///
...
final channelPlanExecutionProvider = Provider.autoDispose
    .family<ChannelPlanExecution?, ChannelKey>((ref, key) {
      ...
      final mine = [
        for (final o in orchestrations)
          if (o.channelId == key.channelId) o,
      ];
      if (mine.isEmpty) {
        return null;
      }
      ...
      // The conversation's runs carry the template step id (`pipelineStepRunId`,
      ...
      return ChannelPlanExecution(orchestration: orchestration, nodes: nodes);
    });
