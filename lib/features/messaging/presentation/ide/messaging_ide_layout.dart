...
class _MessagingIdeLayoutState extends ConsumerState<MessagingIdeLayout> {
  ...
  EditorTab _newTerminalTab({String? backend}) => EditorTab(
    ...
    label: 'Terminal',
    icon: MessagingTabKinds.iconFor(MessagingTabKinds.terminal),
    // Stable id for the keep-alive registry claim (see _seedLayout). The
    // backend rides in args so a restored layout reopens the shell where it
    // was, not silently on the host.
    args: {
  ...
  void _switchConversation(String? from, String? to) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (from != null) {
        ref
            .read(channelTerminalsProvider(from).notifier)
  ...
  void _reconcileBodyHost({bool killOrphanedTerminals = false}) {
    ...
    // A browser rig tab owns its machine: the panel has no stop button, so the
    // tab's × IS the shutdown. The same close-vs-swap discipline as terminals
    // applies — a conversation switch swaps the whole layout and the rig must
    // survive it, so only a live-layout close destroys.
    final rigTabs = live.where(_isBrowserRigTab).toSet();
    if (killOrphanedTerminals) {
      for (final closed in _browserRigTabs.difference(rigTabs)) {
        _destroyBrowserRig(closed);
      }
    }
    _browserRigTabs = rigTabs;
  ...
  /// Browser-rig tabs in the live layout as of the last reconcile — the diff
  /// input that tells a user close (destroy the machine) from a layout swap
  /// (re-seed only).
  Set<EditorTab> _browserRigTabs = {};

  static bool _isBrowserRigTab(EditorTab tab) =>
      tab.kind == MessagingTabKinds.rig &&
      tab.args['surface'] == RigTabSurfaces.browser;

  /// Destroys the live (or still-booting) browser rig of the conversation a
  /// just-closed rig tab belonged to. Rig tabs are per-conversation, so the
  /// selected channel at close time is the owning conversation.
  void _destroyBrowserRig(EditorTab tab) {
    final workspaceId = _workspaceId;
    final channelId = widget.selectedChannelId;
    ...
    final rig =
        ...
        // A close mid-boot cancels the boot, not just the tab.
    ...
    switch (tab.kind) {
      ...
      case MessagingTabKinds.rig:
        return RigTabPane(
          ...
          conversationId: widget.selectedChannelId,
  ...
  EditorChrome _chrome(AppLocalizations l10n) => EditorChrome(
    ...
    newTabMenuItems: (leafId) => [
      ...
      if (_serverHostsVmTerminals)
        CcMenuItem(
          ...
          onSelected: () => _openTabNextFrame(
            leafId,
            _newTerminalTab(backend: 'microvm'),
          ),
        ),
      CcMenuItem(
      ...
      // One entry per machine an agent can drive. Scoped to this conversation,
      // so the tab and the agent's `computer_use`/`browser_use`/`mobile_use`
      // calls address the SAME machine rather than three copies of it.
        ...
        CcMenuItem(
          ...
          onSelected: () => _openTabNextFrame(
            ...
            EditorTab(
              ...
              // One tab per surface per conversation: a second "Browser (VM)"
              // would show the same machine twice.
              dedupKey: '${MessagingTabKinds.rig}:$surface',
            ),
          ),
        ),
      CcMenuItem(
...
/// in-app webview sitting next to it in the same menu.
