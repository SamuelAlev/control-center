...
import 'package:control_center/features/messaging/providers/space_notes_provider.dart';
...
/// The "Notes" IDE-sidebar panel: the shared per-space handoff doc (PRD 16
...
class NotesPanel extends ConsumerWidget {
  ...
  const NotesPanel({super.key, required this.workspaceId, this.spaceId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _NoteEditor(workspaceId: workspaceId, spaceId: spaceId);
  }
}

class _NoteEditor extends ConsumerStatefulWidget {
  const _NoteEditor({required this.workspaceId, required this.spaceId});
...
class _NoteEditorState extends ConsumerState<_NoteEditor> {
  ...
  void _claimClaim() {
    _presence.addClaim(SoftClaim(entityType: 'note', entityId: widget.spaceId));
  }

  void _releaseClaim() {
    _presence.removeClaim(entityType: 'note', entityId: widget.spaceId);
  }
  ...
  /// this space, or null (PRD 16 §14 — visibility, not a lock).
  ParticipantPresence? _otherEditor() {
    ...
    for (final p in roster) {
      ...
      if (p.claims.any(
        (c) => c.entityType == 'note' && c.entityId == widget.spaceId,
      )) {
        return p;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final noteAsync = ref.watch(spaceNoteProvider(widget.spaceId));
  ...
  Widget _buildView(BuildContext context, SpaceNote note) {
    final l10n = AppLocalizations.of(context);
    final t = context.designSystem ?? DesignSystemTokens.light();
