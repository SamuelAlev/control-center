...
enum PrSidebarStatus {
  ...
  /// Collapses a space's PRs into one representative status, or `null` when
...
class PrStatusBadge extends StatelessWidget {
  ...
  /// Overrides the status colour — for a glyph sitting on a surface where the
  /// status colours fail contrast (the selected sidebar row's solid brand
  /// fill, which recolours all its content to `accentOn`). The glyph shape
  /// keeps the state differentiation.
  final Color? color;

  @override
  Widget build(BuildContext context) {
    return Icon(status.icon, size: size, color: color ?? status.color);
  }
}
