...
import 'package:control_center/features/messaging/presentation/widgets/bubbles/bubble_body.dart';
...
class UserBubble extends ConsumerWidget {
  ...
  /// Hover-rail "start thread" affordance (opens a conversation anchored to
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final l10n = AppLocalizations.of(context);
    // What the person attached, read once: the strip draws the cards and the
    // body's inline references resolve against the same list.
                ...
                Flexible(
                  child: FocusableBubble(
                    ...
                    onDelete: deleted || !isOwn
                        ? null
                        : () => _delete(context, ref),
                    // The width cap wraps the BUBBLE, not the whole
                    // [FocusableBubble]. The action rail underneath is a row of
                    // seven icons; inside this `IntrinsicWidth` it would set the
                    // floor, and a two-word message would be stretched to it.
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: deleted
                                  ...
                                  : FileRefScope(
                                      // Resolves each `@[file:…]` token in the
                                      // body to the file the message actually
                                      // carried, so the word in the sentence
                                      // and the card under it open the same
                                      // thing. Empty for a message with no
                                      // attachments — its references then draw
                                      // as plain words, which is the truth.
                                      attachments: messageAttachmentsByRefName(
                                        context,
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          BubbleBody(
                                            // The stored token, verbatim: the
                                            // chat markdown register parses
                                            // `@[file:…]` itself and draws the
                                            // composer's accent pill, so a sent
                                            // message reads the way it looked
                                          ...
                                          // openable after the send. Renders
                                          // nothing when there is none, so an
