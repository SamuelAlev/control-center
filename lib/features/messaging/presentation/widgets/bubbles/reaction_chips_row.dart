...
/// single space-wide [messageReactionsProvider] (no per-message
...
class ReactionChipsRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final chips = ref.watch(
      messageReactionsProvider((spaceId: spaceId, messageId: messageId)),
    );
    if (chips.isEmpty) {
      ...
      child: Wrap(
        ...
        children: [
          for (final chip in chips)
            _ReactionChip(spaceId: spaceId, messageId: messageId, chip: chip),
        ],
      ),
    );
  }
