...
class _EntityRefChip extends ConsumerWidget {
  ...
  Widget _buildPr(BuildContext context, WidgetRef ref, AppLocalizations l10n) {
    ...
    // Prefer the forge-supplied html_url — it is authoritative and correct for
    // whichever forge answered. Only when it is absent do we build one from the
    // coordinate, and then in that repo's own forge vocabulary rather than
    // assuming github.com.
    final String? webUrl = (preview?.htmlUrl.isNotEmpty ?? false)
        ...
        : (hasRepo && number != null
              ? ref
                    ...
                    .pullRequest(parts[0], parts[1], number)
              : null);
    return _chip(
      context,
      ...
      text: number != null
          ? '#$number'
          : (reference.label ?? l10n.entityRefPrFallback),
