...
class _QuestionBubbleState extends ConsumerState<QuestionBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // so [_header]'s opaque tint would cover the 1px side border along the
      // header band. `Container` insets the child by the border's dimensions.
      child: Container(
  ...
  Widget _header(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            answered ? AppIcons.circleCheck : AppIcons.messageCircleQuestion,
            size: 14,
            color: accent,
          ),
      ...
      children: [
        ...
        Align(
          ...
          child: CcButton(
            ...
            // The design-system button owns the busy affordance: it swaps the
            // leading icon for an inline spinner at the same size, so the
            // button never resizes when it flips state.
            loading: _submitting,
            icon: AppIcons.send,
            child: Text(l10n.agentQuestionSubmit),
          ),
        ),
      ],
    );
