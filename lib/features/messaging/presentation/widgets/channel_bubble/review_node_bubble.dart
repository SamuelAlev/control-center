...
class _ReviewNodeBubbleState extends ConsumerState<ReviewNodeBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    final agentAsync = ref.watch(agentDetailProvider(widget.message.senderId));
    final agentName = agentAsync.value?.name ?? widget.message.senderId;
  ...
  void _snack(String action) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$action — no handler bound')),
    );
  }
}
        ...
        children: [
          if (message.content.isNotEmpty)
            GitHubMarkdownBody(
              ...
              styleSheet: prMarkdownStyleSheet(context, compact: true),
            ),
          ...
          Row(
            ...
            children: [
              ...
              if (!canComment)
                FTooltip(
                  tipBuilder: (_, _) => Text(l10n.noFileAnchor),
                  child: FButton(
                    size: FButtonSizeVariant.sm,
