...
/// Resolved colour, glyph and label for a space's overall state, so the
/// flyout's pill pairs colour with a shape AND a word (never colour alone).
///
/// A sibling of [AgentStatusVisual], not a reuse of it: a space has two states
/// an agent does not (`needsInput`, `provisioning`) and those are exactly the
/// two the operator most needs to see from the sidebar.
class SpaceStatusVisual {
  /// Creates a [SpaceStatusVisual].
  const SpaceStatusVisual({
    required this.label,
  ...
  /// Resolves the treatment for a space, most-urgent state first: a setup that
  /// failed or was stopped blocks everything, an unanswered question is the one
  /// thing only the operator can clear, provisioning explains why nothing is
  /// moving and a live run is the ordinary busy state.
  factory SpaceStatusVisual.resolve({
    required SpaceProvisioningStatus status,
    required bool needsInput,
    required bool isLive,
    required DesignSystemTokens tokens,
    required AppLocalizations l10n,
  }) {
    // Stopped is as blocking as failed — no worktree either way — but it is
    // not an error, so it reads quiet instead of red.
    if (status == SpaceProvisioningStatus.cancelled) {
      return SpaceStatusVisual(
        label: l10n.spaceFlyoutSetupStopped,
    ...
    if (status == SpaceProvisioningStatus.failed) {
      return SpaceStatusVisual(
        label: l10n.spaceFlyoutSetupFailed,
    ...
    if (needsInput) {
      return SpaceStatusVisual(
        label: l10n.spaceFlyoutNeedsInput,
    ...
    if (status == SpaceProvisioningStatus.provisioning) {
      return SpaceStatusVisual(
        label: l10n.spaceFlyoutPreparing,
        dotColor: tokens.fgQuaternary,
        textColor: tokens.textTertiary,
        fillColor: tokens.bgSecondary,
        isLive: false,
...
/// The pill form of [SpaceStatusVisual] shown in the flyout header.
class SpaceStatusPill extends StatelessWidget {
  /// Creates a [SpaceStatusPill].
  const SpaceStatusPill({super.key, required this.visual});
...
class SpaceRunRow extends StatelessWidget {
  /// Creates a [SpaceRunRow].
  const SpaceRunRow({
    super.key,
    required this.run,
    required this.label,
...
/// A live context-window gauge for one agent in a space: `145k / 200k` over a
...
class SpaceContextMeter extends ConsumerWidget {
  /// Creates a [SpaceContextMeter].
  const SpaceContextMeter({
    super.key,
    required this.spaceId,
    required this.agentId,
  });

  /// The space whose messages fill the window.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final usage = ref.watch(
      conversationContextUsageProvider((spaceId: spaceId, agentId: agentId)),
    );
    ...
    return Padding(
      ...
      child: Semantics(
        label: l10n.spaceFlyoutContextUsage(label, fmtPercent(usage.fraction)),
        child: Row(
          children: [
            Expanded(
              child: SizedBox(
