...
import 'package:control_center/features/messaging/presentation/widgets/space_hover_rows.dart';
...
/// content column.
const double kSpaceHoverCardWidth = 300;

/// The floating panel revealed by hovering (or focusing) a space row in the
/// global sidebar: what is running in that space right now, which agents and
/// subagents are doing it, how long they have been at it, how much of their
/// context window is gone and what the space has cost.
///
...
class SpaceHoverCard extends ConsumerWidget {
  /// Creates a [SpaceHoverCard].
  const SpaceHoverCard({
    super.key,
    required this.space,
  ...
  /// The space being described.
  final Space space;

  /// The active workspace — the isolation scope for every read below.
  final String workspaceId;

  /// Opens the space (a root agent row, or the header).
  final VoidCallback onOpenSpace;

  /// Opens one subagent run's activity.
  final void Function(SpaceLiveRun run, String label) onOpenRun;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final needsInput = ref.watch(spaceNeedsInputProvider(space.id));
    final visual = SpaceStatusVisual.resolve(
              ...
              child: SingleChildScrollView(
                child: Padding(
                  ...
                  child: summary.isLive
                      ...
                      : _IdleBody(spaceId: space.id, workspaceId: workspaceId),
                ),
              ),
            ),
            CcDivider(color: t.borderSecondary),
            _Footer(spaceId: space.id, summary: summary),
          ],
        ),
      ),
    );
...
/// Space name, live status pill and the one-line "what is going on" summary.
class _Header extends StatelessWidget {
  ...
  final Space space;
  final SpaceStatusVisual visual;
  final SpaceActivitySummary summary;
  final VoidCallback onOpenSpace;
  ...
  Widget build(BuildContext context) {
    final t = context.designSystem ?? DesignSystemTokens.light();
    final l10n = AppLocalizations.of(context);
    final name = space.name.isNotEmpty ? space.name : l10n.spaceLabel;
          ...
          children: [
            Row(
              ...
              children: [
                ...
                SpaceStatusPill(visual: visual),
              ],
            ),
            const SizedBox(height: 5),
            _HeaderMeta(space: space, summary: summary),
          ],
        ),
      ),
    );
...
/// The line under the space name: how many agents are working and for how
/// long, or (when idle) when the space last saw activity.
class _HeaderMeta extends StatelessWidget {
  const _HeaderMeta({required this.space, required this.summary});
  ...
  Widget build(BuildContext context) {
    ...
    if (space.provisioningStatus == SpaceProvisioningStatus.provisioning) {
      return Text(
        provisioningStepLabel(l10n, space.provisioningStep),
        maxLines: 2,
        overflow: TextOverflow.ellipsis,
        style: style,
      );
...
class _LiveBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    String nameFor(SpaceLiveRun run) {
    ...
    for (final run in summary.liveRuns) {
      ...
      // The window is a property of the agent, not the space, so each live
      ...
      rows.add(SpaceContextMeter(spaceId: spaceId, agentId: run.agentId));
...
/// What an idle space shows: no work in flight, plus the roster that would do
/// the work if there were.
class _IdleBody extends ConsumerWidget {
  const _IdleBody({required this.spaceId, required this.workspaceId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final l10n = AppLocalizations.of(context);
    final participants =
        ref.watch(spaceParticipantsProvider(spaceId)).asData?.value ?? const [];
    final agents =
        ref.watch(workspaceAgentsProvider(workspaceId)).asData?.value ??
        const <Agent>[];
    final agentsById = {for (final a in agents) a.id: a};
...
/// The spend the space has accumulated, plus its open pull requests.
class _Footer extends ConsumerWidget {
  const _Footer({required this.spaceId, required this.summary});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context);
    final openPrs = ref
        .watch(spacePrsProvider(spaceId))
        .where((pr) => pr.isOpen);
