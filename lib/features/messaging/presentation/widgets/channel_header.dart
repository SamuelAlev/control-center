...
class ChannelHeader extends ConsumerWidget {
  ...
  /// Callback to open / close the changes (diff) side panel.
  final VoidCallback? onToggleChanges;

  /// Whether the changes panel is currently visible.
      ...
      child: Row(
        children: [
          if (channel.isDm && agents.isNotEmpty)
            AgentAvatar(agentId: agents.first.agentId, name: title, size: 28)
          else
            Icon(
              AppIcons.users,
              size: 20,
              color: tokens?.textTertiary ?? theme.colorScheme.onSurfaceVariant,
            ),
          ...
          if (onToggleTerminal != null)
            CcTooltip(
              ...
              child: CcIconButton(
                icon: terminalOpen ? AppIcons.x : AppIcons.terminal,
                onPressed: onToggleTerminal,
              ),
            ),
          if (onToggleChanges != null) ...[
            CcTooltip(
              targetAnchor: Alignment.bottomCenter,
              followerAnchor: Alignment.topCenter,
              message: changesOpen
                  ? l10n.hideConversationChanges
                  : l10n.showConversationChanges,
              child: CcIconButton(
                icon: changesOpen ? AppIcons.x : AppIcons.fileDiff,
          ...
          if (channel.isDm && agents.isNotEmpty)
            CcTooltip(
              targetAnchor: Alignment.bottomCenter,
              followerAnchor: Alignment.topCenter,
              message: l10n.createTicketFromConversation,
              child: CcIconButton(
                icon: AppIcons.ticket,
                onPressed: () => _createTicketFromChannel(context, ref, title),
              ),
            ),
          const SizedBox(width: 4),
          CcTooltip(
            ...
            child: CcIconButton(icon: AppIcons.users, onPressed: onManage),
          ),
          const SizedBox(width: 4),
          CcTooltip(
            targetAnchor: Alignment.bottomCenter,
            ...
            child: CcIconButton(icon: AppIcons.trash2, onPressed: onDelete),
          ),
        ],
      ),
    );
  ...
  /// Spins a tracking ticket out of this conversation, linked to the channel,
  /// then posts a chip back into the conversation. Conversation-first: the chat
  /// is the work; the ticket is a derived artifact, not a precondition. Left
  /// unassigned so it doesn't trigger a fresh dispatch (the agents are already
  /// here).
  Future<void> _createTicketFromChannel(
      ...
      child: Stack(
        children: [
          for (var i = 0; i < shown.length; i++)
            Positioned(
              left: i * overlap,
              child: AgentAvatar(agentId: shown[i].agentId, size: size),
            ),
        ],
      ),
    );
...
class _ManageChannelDialogState extends ConsumerState<ManageChannelDialog> {
  ...
  Future<void> _inviteAgent(String agentId) async {
    if (widget.isDm) {
      final l10n = AppLocalizations.of(context);
      final result = await showCcDialog<String>(
        context: context,
        builder: (ctx) => CcDialog(
          title: l10n.convertToGroup,
          content: Text(l10n.convertToGroupBody),
          actions: [
            CcButton(
              variant: CcButtonVariant.secondary,
...
class _ParticipantRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          AgentAvatar(agentId: participant.agentId, name: name, size: 24),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
          ...
          CcTooltip(
            message: l10n.remove,
            child: CcIconButton(icon: AppIcons.x, onPressed: onRemove),
          ),
        ],
      ),
    );
