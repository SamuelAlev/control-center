...
/// demonstrably saw (see [_ChannelMessageFeedState._advanceReadFrontier]).
...
List<_FeedItem> _buildFeedItems(
  ...
  bool isUnread(ChannelMessage m) =>
      readFrontier != null &&
...
class ChannelMessageFeed extends ConsumerStatefulWidget {
  ...
  /// The conversation (stream) inside the channel to display. `main`'s id
  /// equals the channel id.
...
class _ChannelMessageFeedState extends ConsumerState<ChannelMessageFeed> {
  ...
  /// Precalculates every row's real extent so the scroll extent is exact and
  /// the scrollbar thumb stops resizing. Returns true while any row still has
  /// an estimated extent; SuperListView spreads the work across frames via its
  /// layout budget, so this never blocks a frame even for a large window.
  final _precalcPolicy = _AlwaysPrecalculate();
  ...
  ({List<ChannelMessage> messages, bool hasMore})? _frozenWindow;
  ...
  /// Consumes a pending permalink target (`?m=<id>`) matching this channel:
  ...
  void _maybeConsumePendingFocus() {
    ...
    if (pending == null || pending.channelId != widget.channelId) {
      return;
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Still ours? A second feed (another conversation of the same channel,
  ...
  Future<bool> scrollToMessage(String messageId) async {
    ...
    // Verify the message exists and belongs to this channel before growing.
    ...
    if (found == null || found.channelId != widget.channelId) {
      return false;
    }

    _follow.loadingOlder = true;
    try {
      final windowProvider = channelFeedWindowProvider(widget.conversationId);
      while (ref.read(windowProvider) < kChannelFeedMaxWindow) {
  ...
  /// Stamp the user's read cursor for this channel (debounced) — called when
  ...
  /// A null frontier stays null: a never-opened channel shows no divider at all,
  ...
  /// and a channel whose content fits the viewport never fires [_onScroll] to
  ...
  ///
  /// Reaching the live edge means everything currently in the channel has been
  ...
  void _landOnLiveEdge() {
    ...
    // programmatic movement and a channel that fits the viewport never scrolls.
  ...
  void _onWindowChanged(({List<ChannelMessage> messages, bool hasMore})? next) {
    ...
    if (newest.isUser) {
      ...
      if (_follow.mode == FeedFollowMode.following ||
        ...
        // simply sitting on it, which is where a short channel always is), or
        ...
        // the channel the reader is sitting in.
        _advanceReadFrontier(newest.createdAt);
        _stampCursor();
      } else {
  ...
  void _onScroll() {
    ...
    if (_hasMore &&
        ref.read(channelFeedWindowProvider(widget.conversationId)) <
            kChannelFeedMaxWindow &&
        pos.maxScrollExtent - pos.pixels < _loadMoreThreshold) {
      _follow.loadingOlder = true;
      ref
          .read(channelFeedWindowProvider(widget.conversationId).notifier)
          .loadMore();
    }
  }
  ...
  void _maybeAnnounce(List<ChannelMessage> messages) {
  ...
  Widget build(BuildContext context) {
    ...
    // unsubscribe and server work for this channel stops — and render the
    ...
    final ({List<ChannelMessage> messages, bool hasMore}) window;
    final feedRef = (
      channelId: widget.channelId,
      conversationId: widget.conversationId,
    );
    if (isVisible) {
      // Live turn relay: folds `messaging.watchChannelTurns` into the
      ...
      ref.watch(channelTurnRelayProvider(widget.channelId));
      final windowAsync = ref.watch(channelFeedWindowedProvider(feedRef));
      ref.listen(channelFeedWindowedProvider(feedRef), (_, next) {
      ...
      builder: (context) {
        ...
        if (isVisible && !_cursorSnapshotTaken) {
          final liveCursorAsync = ref.watch(
            channelUserLastReadAtProvider(widget.channelId),
          );
                  ...
                  SelectionArea(
                    child: CcSelectionScope(
                      child: SuperListView.builder(
                        ...
                        // Precalculate real extents so the scroll extent is exact
                        // and the scrollbar thumb stops resizing/jumping as tall
                        // transcripts enter and leave the built set.
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (item is _DayItem) {
                            ...
                            final bubble = ChannelMessageBubble(
                              message: m.message,
...
/// Precalculation policy that measures every row's real extent (spread across
/// frames by SuperListView's layout budget), stopping once no row's extent is
/// estimated. Exact extents give a deterministic scroll extent, which is what
/// keeps the scrollbar thumb stable and makes the follow physics' growth
/// delta reflect real content rather than estimation noise.
class _AlwaysPrecalculate extends ExtentPrecalculationPolicy {
  @override
