...
// The desktop opens no local database and runs no in-process dispatch engine — it is a
// thin client; `databaseProvider` throws and `rpcClientProvider` is the connected
// (spawned) `cc_server`.
// So both services run SERVER-SIDE over RPC, identical to the web client:
// RPC messaging repository — submitting an answer marks the persisted question message
// answered server-side (the asking agent resumes there).
library;
