...
final channelMessagesProvider = StreamProvider.autoDispose
    .family<List<ChannelMessage>, String>((ref, channelId) {
      return ref
          ...
          // `channel_messages` — a reaction, a read cursor, a message in another
          // conversation of the same channel — and re-sends the whole list even
          // when nothing in it changed. Every widget watching this then
          // rebuilt for a snapshot identical to the one it already had; in a
          // review channel that is the verdict banner, the accordion and every
          // expanded finding, several times a minute, for nothing.
          //
          // `ChannelMessage` has full value equality (content and metadata
