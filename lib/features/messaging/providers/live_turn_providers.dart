...
/// Live turn relay fold for one channel.
///
/// Watched by the conversation feed while it is visible: subscribes to
/// `messaging.watchChannelTurns` and folds the seed + update frames into the
...
final channelTurnRelayProvider = Provider.autoDispose.family<void, String>((
  ref,
  channelId,
) {
  final registry = ref.watch(activeStreamRegistryProvider);
  final relay = ref.watch(channelTurnRelayPortProvider);
  final cache = ref.watch(transcriptCacheProvider);
  ...
  final sub = relay.watchChannelTurns(channelId).listen((event) {
    switch (event) {
      case TurnRelaySeed(:final turns):
        ...
        for (final t in turns) {
          ...
          registry.seed(t.messageId, t.segments, channelId: channelId);
      ...
      case TurnRelayUpdates(:final messageId, :final updates):
        for (final update in updates) {
          if (!registry.isActive(messageId)) {
            ...
            registry.register(messageId, channelId: channelId);
            live.add(messageId);
          }
