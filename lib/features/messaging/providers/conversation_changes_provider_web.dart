...
/// computed on the SERVER (which owns the checkouts) via the `conversation.changes`
/// op and returned as the same `List<PrFile>` the desktop produces locally.
///
/// When the connected server doesn't expose the op (e.g. a remote headless
/// server that never created these worktrees), this resolves to an empty list —
/// "no changes" — rather than surfacing an error to the panel.
final conversationChangesProvider = FutureProvider.autoDispose
    .family<List<PrFile>, ConversationChangesArgs>((ref, args) async {
      try {
