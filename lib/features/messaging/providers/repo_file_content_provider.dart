...
/// Identifies a file-read request: the repo, the repo-relative path and the
/// conversation whose isolated CoW worktree holds the copy to read (null → the
/// shared linked checkout).
...
final repoFileContentProvider = FutureProvider.autoDispose
    .family<({String content, bool binary}), RepoFileContentArgs>((
      ref,
      args,
    ) async {
      try {
        final data = await ref.watch(rpcClientProvider).call('repos.readFile', {
