...
/// One reaction row from `reactions.watchForChannel`.
...
/// ONE channel-wide subscription backing every message's reaction chips — no
/// per-message subscriptions (PRD 16 §15 "keep it lightweight").
final channelReactionsProvider = StreamProvider.autoDispose
    .family<List<MessageReaction>, String>((ref, channelId) {
      final client = ref.watch(rpcClientProvider);
      return client
          .subscribe('reactions.watchForChannel', {'channel_id': channelId})
          .map((data) {
...
/// Identifies one message's reactions within a channel.
typedef MessageReactionsArgs = ({String channelId, String messageId});

/// Per-message aggregated reaction chips, derived client-side from the single
/// [channelReactionsProvider] subscription for the channel. Ordered by the
...
final messageReactionsProvider = Provider.autoDispose
    .family<List<ReactionChip>, MessageReactionsArgs>((ref, args) {
      final all =
          ref.watch(channelReactionsProvider(args.channelId)).value ??
          const <MessageReaction>[];
