...
class CodeServerSessionRequest {
  ...
  @override
  int get hashCode => Object.hash(spaceId, repoId, path, line);
}
...
/// space id (reported by the bridge extension via `codeServer.watchOpenRequests`). The
...
final codeServerOpenRequestsProvider = StreamProvider.autoDispose
    .family<CodeServerOpenEvent, String>((ref, spaceId) {
      final client = ref.watch(rpcClientProvider);
      return client
          .subscribe('codeServer.watchOpenRequests', {'space_id': spaceId})
          .map(
...
/// space (reported by the bridge extension via `codeServer.watchDirtyState`).
...
final codeServerDirtyStateProvider = StreamProvider.autoDispose
    .family<CodeServerDirtyEvent, String>((ref, spaceId) {
      final client = ref.watch(rpcClientProvider);
      return client
          .subscribe('codeServer.watchDirtyState', {'space_id': spaceId})
          .map(
