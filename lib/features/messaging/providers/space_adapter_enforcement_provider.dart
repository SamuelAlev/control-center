...
/// The adapter a space's agents will actually run on, plus what Control
/// Center can enforce for it.
class SpaceAdapterEnforcement {
  /// Creates a [SpaceAdapterEnforcement].
  const SpaceAdapterEnforcement({
    required this.adapter,
    required this.enforcement,
  });

  /// The resolved adapter. When a space's agents disagree, this is the one
  /// whose enforcement is weakest — see [spaceAdapterEnforcementProvider].
  ...
  bool operator ==(Object other) =>
      ...
      other is SpaceAdapterEnforcement &&
...
final spaceAdapterEnforcementProvider = Provider.autoDispose
    .family<SpaceAdapterEnforcement?, String>((ref, spaceId) {
      ...
      // Loading is NOT "nothing configured": the fallback below would report
      // the built-in loop's enforcement for a space whose agents are all on
      // Claude Code, for as long as the roster takes to arrive. An absent badge
      // says "unknown"; a wrong one says "fine".
      final participantsAsync = ref.watch(spaceParticipantsProvider(spaceId));
      final agentsAsync = ref.watch(workspaceAgentsProvider(workspaceId));
      if (!participantsAsync.hasValue || !agentsAsync.hasValue) {
        return null;
      }
      final participants = participantsAsync.value ?? const [];
      final agents = agentsAsync.value ?? const <Agent>[];
      ...
      // Nothing named an adapter, or nothing it named exists. Dispatch runs
      // those on the built-in loop, so the badge reports the built-in loop —
      // the roster is loaded by here, so this is a real answer, not a gap.
      if (resolved.isEmpty) {
        resolved.add(builtInAdapter);
      }
      ...
      return SpaceAdapterEnforcement(
        adapter: weakest,
