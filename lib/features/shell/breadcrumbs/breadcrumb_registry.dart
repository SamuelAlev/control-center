...
final Map<String, BreadcrumbBuilder> breadcrumbRegistry = {
  ...
  channelsRoute(workspaceIdParam): _channelsCrumbs,
  '${channelsRoute(workspaceIdParam)}/:channelId': _channelsCrumbs,
  ...
  // from. Pipelines and teams keep bespoke builders: they are foreign features
  // that merely live under `/settings`, and pipelines has a child route.
  for (final group in kSettingsNav)
    for (final entry in group.items)
      if (entry.id != 'workspace.pipelines' && entry.id != 'workspace.teams')
...
List<CcBreadcrumbItem> _channelsCrumbs(
  WidgetRef ref,
  BuildContext context,
  GoRouterState state,
  AppLocalizations l10n,
  ...
  final selectedId = state.pathParameters['channelId'];
  if (selectedId == null) {
    return [
  ...
  return [
    CcBreadcrumbItem(
      onPress: () => context.go(channelsRoute(w)),
