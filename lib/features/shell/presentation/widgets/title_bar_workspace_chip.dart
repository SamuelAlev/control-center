...
class _ChipButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (iconOnly) {
      // The rail keeps the avatar at its full expanded size and position: the
      // same 4px vertical / 1px horizontal padding the expanded chip uses
      // puts the avatar at x=9/y=4 from the header's content edge in every
      // state (the 38px-wide result fills the rail content exactly, so
      // centering changes nothing); the workspace name moves to a tooltip.
      return CcTooltip(
    ...
    return _HoverSurface(
      ...
      onTap: onTap,
      child: Padding(
        // 1px horizontal: puts the avatar's center on the x=27 line every
        // sidebar icon shares with the collapsed rail (27 − 8 sidebar inset −
        // 18 half-avatar = 1), so toggling never moves the logo.
        padding: const EdgeInsets.symmetric(vertical: 4, horizontal: 1),
