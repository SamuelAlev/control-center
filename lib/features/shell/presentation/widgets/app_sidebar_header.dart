...
class AppSidebarHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (transitioning) {
      ...
      // chip's 1px horizontal padding puts the avatar at the exact x it
      // occupies in BOTH settled states (x=9 from the sidebar edge), so the
      // logo never moves while the name and search are out of the way (they
      // would overflow the narrowing row).
    ...
    // One row: workspace chip (left, ellipsizing) then search. No leading
    // inset so the chip's hover pill aligns with the nav item pills below
    // (both start at the sidebar content's left edge).
    return Padding(
