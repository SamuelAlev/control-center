...
class HeartbeatRecoveryService {
  ...
  Future<void> evaluateAndSchedule(AgentRunLog run) async {
    if (run.errorFamily == RunErrorFamily.transientUpstream &&
      ...
      AppLog.i('HeartbeatRecovery', 'Scheduling retry ${run.retry.attempt + 1} '
    ...
    if (run.liveness == RunLiveness.stalled) {
      if (!shouldContinue(run)) {
        AppLog.w('HeartbeatRecovery', 'Run ${run.id} exceeded max continuation '
            'attempts (${run.retry.attempt}/$maxContinuationAttempts)');
        await _failBackingTicket(run, 'max continuations exceeded');
        return;
      }

      AppLog.i('HeartbeatRecovery', 'Scheduling continuation wake for '
    ...
    if (run.liveness == RunLiveness.dead) {
      AppLog.w('HeartbeatRecovery', 'Run ${run.id} classified as dead — '
          'failing ticket');
  ...
  Future<void> _scheduleRetry(AgentRunLog run, Duration delay) async {
    try {
      ...
      AppLog.e('HeartbeatRecovery', 'Failed to schedule retry for ${run.id}', e, st);
    }
  }
  ...
  Future<void> _scheduleContinuation(AgentRunLog run) async {
    try {
      ...
      AppLog.e('HeartbeatRecovery', 'Failed to schedule continuation for ${run.id}', e, st);
    }
  }
  ...
  Future<void> _failBackingTicket(AgentRunLog run, String reason) async {
    ...
    if (ticketId == null || workspaceId == null) return;
    ...
    try {
      ...
      AppLog.e('HeartbeatRecovery', 'Failed to fail ticket $ticketId', e, st);
    }
  }
}
