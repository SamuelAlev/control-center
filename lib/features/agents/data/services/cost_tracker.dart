...
class CostTracker {
  ...
  ///
  /// [workspaceId] is the run's workspace: run logs live in their workspace's
  /// own database, so the id is what locates [runLogId] at all.
  Future<void> recordUsage({
    ...
    final log = await _runLogRepo.getById(workspaceId, runLogId);
    if (log == null) {
      return;
    }
  ...
  Future<int> getMonthlySpendForAgent(String agentId) async {
    ...
    return logs
        .where((l) => l.agentId == agentId && l.startedAt.isAfter(monthStart))
