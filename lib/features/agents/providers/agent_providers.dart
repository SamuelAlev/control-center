...
/// when no agent is currently working in the space/ticket.
...
/// Identifies a space within a workspace, for space-scoped run queries.
typedef SpaceRunsKey = ({String workspaceId, String spaceId});

/// Watches the active run logs of a whole SPACE — every conversation in it,
/// threads included. Empty when no agent is working anywhere in the space.
///
/// The space-level signal (the sidebar's running dot). The composer wants the
/// narrower [conversationActiveRunsProvider]: stopping a run from a thread
/// must not stop somebody else's turn in the parent stream.
final spaceActiveRunsProvider = StreamProvider.autoDispose
