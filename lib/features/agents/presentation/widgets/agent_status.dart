...
class AgentStatusVisual {
  ...
  factory AgentStatusVisual.resolve(
    ...
    switch (state) {
      ...
      case AgentLiveState.queued:
        // Dispatched but not executing: the same neutral grey as idle, because
        // nothing is happening yet. It is `isLive: false` on purpose — a
        // breathing dot would claim work in flight.
        return AgentStatusVisual(
          label: l10n.queued,
      ...
      case AgentLiveState.succeeded:
        // A finished run is a positive outcome, so it reads green rather than
        // sharing idle's neutral grey. The icon keeps it from being green-vs-red
        // by colour alone (DESIGN.md status rule) in the badge form; the dot form
        // carries the label through its parent row's semantics.
        return AgentStatusVisual(
          label: l10n.succeeded,
          dotColor: tokens.fgSuccessSecondary,
          textColor: tokens.textSuccessPrimary,
          fillColor: tokens.bgSuccessPrimary,
          isLive: false,
