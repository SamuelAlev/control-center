...
class AgentSettingsTab {
  ...
  /// Whether this tab applies to [Agent] at all. Null means always.
  ///
  /// A tab whose body can only ever say "nothing here" is chrome that costs a
  /// click to disprove, and the reader cannot tell "not applicable to this
  /// agent" from "applicable and empty". Deciding it needs a [WidgetRef]
  /// because the answer usually depends on async server state (which accounts
  /// exist, which runners are installed) rather than on the entity alone —
  /// which is also why it is a predicate here and not a filter in
  /// `di/settings_registry.dart`, where the contribution list is `const`.
  ///
  /// Taking `WidgetRef` keeps this contract-only: riverpod is the app's state
  /// vocabulary, not settings UI, so declaring a tab still pulls in no screen.
  final bool Function(WidgetRef ref, Agent agent)? visible;
}
...
/// Applies [AgentSettingsTab.visible] to a tab list.
extension AgentSettingsTabVisibility on List<AgentSettingsTab> {
  /// The tabs that apply to [agent], in the order they were given.
  ///
  /// A tab that does not apply is DROPPED rather than rendered empty: the
  /// reader cannot tell "not applicable to this agent" from "applicable and
  /// currently empty", and disproving it costs a click either way. Callers
  /// clamp their selected index, so a tab disappearing mid-session is safe.
  List<AgentSettingsTab> visibleFor(WidgetRef ref, Agent agent) => [
    for (final tab in this)
      if (tab.visible?.call(ref, agent) ?? true) tab,
  ];
}
