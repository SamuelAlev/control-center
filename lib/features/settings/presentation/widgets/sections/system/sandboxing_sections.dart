...
/// The sandboxing configuration rendered as a stack of section cards, so it
/// composes into the Security & privacy settings page alongside the command
/// rules and privacy sections. Watches the same providers the old standalone
/// page did.
...
class _MasterToggleSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SectionCard(
      ...
      child: SettingsRow(
        icon: enabled ? AppIcons.shieldCheck : AppIcons.shieldAlert,
...
class _InstallHintSection extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // The hint describes how to enable the native sandbox on the HOST that runs
    // agents, so it branches on the detected platform (the local machine on
    // desktop self-serve; the connected cc_server over RPC on a thin/web
    // client) — never `dart:io` `Platform`, which throws on web. Hidden until
    // detection resolves so the card never flashes a wrong platform's copy.
    final platform = ref
        .watch(sandboxDetectionProvider)
        .maybeWhen(data: (r) => r.platform, orElse: () => null);
    if (platform == null) {
      ...
      child: Padding(
        ...
        child: Text(
          ...
          style: TextStyle(
            ...
            fontFamily: isLinux ? 'monospace' : null,
          ),
        ),
      ),
    );
...
class _BackendOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bg = selected ? primary.withValues(alpha: 0.08) : Colors.transparent;
            ...
            children: [
              Icon(
                ...
                color: interactive
                    ...
                    : (tokens?.fgTertiary ?? mutedFallback).withValues(
                        alpha: 0.5,
                      ),
              ),
              ...
              const SizedBox(width: 12),
              if (selected)
                Icon(AppIcons.circleCheck, size: 18, color: primary)
              else if (!available)
                Text(
      ...
      child: SettingsRow(
        ...
        trailing: CcButton(
          onPressed: () {
            CcToastScope.of(
              context,
            ).show(l10n.allSessionsReset, variant: CcToastVariant.success);
          },
