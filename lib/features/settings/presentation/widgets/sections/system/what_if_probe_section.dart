...
/// command, an agent/space scope and a [Mode]) and see exactly how the live
...
class _WhatIfProbeSectionState extends ConsumerState<WhatIfProbeSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Agent> agents = workspaceId == null
        ...
        : ref.watch(workspaceAgentsProvider(workspaceId)).asData?.value ??
              const <Agent>[];
    final List<Space> spaces = workspaceId == null
        ? const <Space>[]
    ...
    return SectionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.md),
          CcSelect<String>(
            label: l10n.guardrailProbeSpaceLabel,
            options: [
              CcSelectOption(value: _none, label: l10n.guardrailProbeNone),
            ...
            child: CcSegmentedToggle<Mode>(
              ...
              segments: [
                for (final m in Mode.values)
                  CcSegment(value: m, label: _modeLabel(l10n, m)),
              ],
            ),
          ),
  ...
  PolicyResolution _probe(List<ActionPolicyRule> rules, String command) {
    ...
    final spaceId = _spaceId == _none ? null : _spaceId;
