// Voice-section extras: the ASR model picker (over RPC) + the shared
...
// `models.voiceCatalog` / `models.selectVoice` ops. The audio-input row
// (device picker + mic test) is genuine local hardware access — `package:
// record` ships both native and `record_web` backends, so the same
// [AudioInputRow] runs against the desktop mic or the browser mic.
library;
...
/// The voice-section extras below the status row: the shared audio-input
/// device picker + mic test, plus the composer dictation push-to-talk mode.
class VoiceSectionExtras extends ConsumerWidget {
