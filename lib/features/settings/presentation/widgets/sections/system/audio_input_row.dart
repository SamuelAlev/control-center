...
class _AudioInputRowState extends ConsumerState<AudioInputRow> {
  ...
  Future<void> _startTest() async {
    ...
    try {
      ...
      if (!await recorder.hasPermission()) {
        setState(
          () => _testError = AppLocalizations.of(
            context,
      ...
      final device = await resolveSelectedInputDevice(
        recorder,
        ref.read(audioInputDeviceProvider),
      );
      final config = device == null
          ? _testConfig
          : _testConfig.copyWith(device: (value: device));
  ...
  Future<void> _stopTest() async {
    ...
    try {
      ...
      // Recorder may already be stopped; teardown is best-effort.
    }
    try {
      await _recorder?.dispose();
    } catch (_) {
      // Recorder may already be disposed; teardown is best-effort.
    }
    _recorder = null;
    if (mounted) {
      setState(() {
              ...
              children: [
                Flexible(
                  child: devicesAsync.when(
                    ...
                    loading: () => const CcSpinner(size: 16),
        ...
        children: [
          Expanded(
            child: ClipRRect(
              ...
              child: CcProgressBar(value: level.clamp(0.0, 1.0)),
            ),
          ),
          const SizedBox(width: 8),
          SizedBox(
