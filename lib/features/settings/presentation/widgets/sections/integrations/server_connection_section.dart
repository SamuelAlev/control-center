...
import 'package:control_center/features/settings/presentation/widgets/sections/integrations/add_server_dialog.dart';
...
class _ServerConnectionSectionState
        ...
        children: [
          if (!kIsWeb) ...[
            _ServerRow(
              ...
              subtitle: l10n.serverModeLocalDescription,
              facts: [if (serverBuild?.version != null) serverBuild!.version!],
          ...
          for (final entry in state.entries) ...[
            _ServerRow(
              icon: AppIcons.cloud,
              title: entry.name,
              subtitle: _entryUrl(entry),
              facts: [_entryFingerprint(entry)],
  ...
  /// The address this server is reached at, or null when none is recorded.
  String? _entryUrl(ServerEntry entry) =>
      entry.descriptor.paths.firstOrNull?.rpcUri?.toString();

  /// The pinned identity fingerprint, truncated for the row. It is machine
  /// truth, so it renders in the mono face beside the row rather than inside
  /// the sentence describing the server.
  String _entryFingerprint(ServerEntry entry) {
  ...
  Future<void> _showAddDialog({String prefillUrl = ''}) async {
    ...
    final result = await showCcDialog<AddServerCredentials>(
      context: context,
      builder: (dialogContext) =>
          AddServerDialog(prefillUrl: prefillUrl, excludeServerIds: pairedIds),
    );
    if (result == null || !mounted) {
      // Dismissed, or an SSO round-trip is finishing in the browser: on the
      // desktop its credential comes back on the `control-center://pair`
      // channel and the app switches itself, so there is nothing to do here.
      return;
    }
    await _run(
      () => ref
...
class _ServerRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SettingsEntityRow(
      ...
      title: title,
      icon: icon,
      // Active is the state that matters here: which server this client is
      // actually talking to. Everything else is "reachable, not in use".
      tone: isActive ? CcStatusTone.positive : CcStatusTone.neutral,
      statusLabel: isActive ? l10n.serverListActive : null,
      subtitle: subtitle,
      meta: [
        for (final fact in facts) SettingsMetaFact(value: fact),
        if (insecure)
          CcBadge(
        ...
        children: [
          ...
          if (onRemove != null) ...[
            ...
            CcIconButton(
              ...
              tooltip: removeHint ?? l10n.remove,
