...
class MembersRosterSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Invites are an admin surface; the server rejects the watch anyway, so it
    // is only watched (and rendered) for admins.
      ...
      child: CcSelect<String>(
        ...
        options: [
          for (final role in const ['admin', 'member', 'viewer', 'guest'])
            CcSelectOption(value: role, label: workspaceRoleLabel(l10n, role)),
        ],
