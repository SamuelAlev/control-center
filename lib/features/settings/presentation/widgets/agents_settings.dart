...
class _AgentsSettingsState extends ConsumerState<AgentsSettings> {
  ...
  // When true the detail pane shows the workspace-wide org chart instead of the
  // selected agent — a one-glance, workspace-level surface (the chart is not
  // tied to any single agent).
  ...
  Widget build(BuildContext context) {
    ...
    return SettingsShortcuts(
      ...
      child: PageWrapper(
        ...
        actions: [
          CcButton(
            onPressed: () => setState(() => _showOrgChart = !_showOrgChart),
            variant: _showOrgChart
                ? CcButtonVariant.accent
                : CcButtonVariant.secondary,
            size: CcButtonSize.sm,
            icon: AppIcons.network,
            child: Text(l10n.orgChart),
          ),
          CcButton(
      ...
      builder: (context, constraints) {
        ...
        final roster = AgentRosterList(
          ...
          onSelect: (id) => setState(() {
            _selectedAgentId = id;
            // Selecting an agent leaves the org-chart overview.
            _showOrgChart = false;
          }),
        ...
        Widget? detail() {
          if (_showOrgChart && workspaceId != null) {
            return _OrgChartPane(
              workspaceId: workspaceId,
              onClose: wide
                  ? null
                  : () => setState(() => _showOrgChart = false),
            );
          }
...
/// The workspace-wide agent org chart, shown in the detail area when the
/// header's "Org chart" toggle is active. It is a workspace-level surface (the
/// reporting tree spans every agent), so it lives here rather than buried under
/// any single agent's detail tabs.
class _OrgChartPane extends StatelessWidget {
  const _OrgChartPane({required this.workspaceId, this.onClose});
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Icon(AppIcons.network, size: 18, color: tokens.fgBrandPrimary),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    l10n.orgChart,
...
class _RunViewerDialogState extends State<_RunViewerDialog> {
  ...
  Future<List<Map<String, dynamic>>> _load() async {
    final file = File(_resolveLogPath(widget.logPath));
  ...
  String _resolveLogPath(String raw) {
    if (p.isAbsolute(raw)) {
      return raw;
    }
    var rel = p.normalize(raw);
    if (rel.startsWith('data/')) {
      rel = rel.substring('data/'.length);
    }
    final rooted = p.join(appCcPaths.appSupportRoot, rel);
    if (File(rooted).existsSync()) {
      return rooted;
    }
    return raw;
  }
