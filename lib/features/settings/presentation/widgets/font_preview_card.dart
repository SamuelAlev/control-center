...
///
/// For system fonts this widget loads the font file into Flutter's engine the
/// first time it is shown, through the platform seam
/// ([loadSystemFontFromFile]) — inert on web, which has no local font files.
...
class _FontPreviewCardState extends State<FontPreviewCard> {
  ...
  Future<void> _maybeLoadSystemFont() async {
    if (widget.font.source == FontSource.system &&
      ...
      // Through the conditional-import seam, never `dart:io` directly: this
      // widget also builds on web, where `File(...)` throws.
      await loadSystemFontFromFile(widget.font.family, widget.font.filePath!);
      if (mounted) {
        setState(() {
          _isLoading = false;
          _loaded = true;
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Container(
        ...
        child: const Center(child: CcSpinner(size: 20)),
      );
    }
