    ...
    return CcScrollArea(
      child: ListView(
        ...
        children: [
          ...
          if (_showGoogle) ...[
            if (catalogLoading && googleFonts.isEmpty)
              const Padding(
...
/// One family in the list, previewed in its own face.
///
/// The preview is DEFERRED by a short delay: a downloadable family's bytes are
/// fetched the first time it is rendered in its own font and the catalogue runs
/// to ~2000 entries, so previewing on sight would fire a request for every row
/// that flickers past during a fast scroll. Waiting until a row has settled means
/// only the rows a user actually stops on cost anything and the timer dies with
/// the row when it scrolls off. Already-loaded families re-render instantly, so
/// the delay is invisible on a second pass.
...
class _FontRowState extends State<_FontRow> {
  ...
  void initState() {
    super.initState();
    if (widget.source == FontSource.google) {
