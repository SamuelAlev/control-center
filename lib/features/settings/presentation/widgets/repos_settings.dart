...
import 'package:control_center/features/repos/presentation/widgets/add_repo_form.dart'
    if (dart.library.js_interop) 'package:control_center/features/repos/presentation/widgets/add_repo_form_web.dart';
...
class ReposSettings extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Source the workspace straight from the route — the URL's `:workspaceId`
    // is the single source of truth. Reading the synced `activeWorkspaceIdProvider`
    // mirror here could link the repo to a stale/lagging workspace.
...
Future<void> _addRepo(
  ...
  final repoId = await showAddRepoDialog(context, ref, workspaceId);
  if (repoId == null) {
    return;
  }
  await ref
