...
class _AgentSettingsFormState extends ConsumerState<AgentSettingsForm> {
  ...
  String? _effort;
  /// True once the user has manually set the context size, so model-driven
  /// auto-prefill never clobbers an explicit value.
  bool _contextSizeUserEdited = false;
  ...
  void _onContextSizeChanged() {
    ...
    _contextSizeUserEdited = _contextSizeCtl.text.trim().isNotEmpty;
  ...
  AcpModel? _resolveSelectedModel() {
    ...
    final models = ref.read(adapterModelsProvider(adapterId)).value ??
        const <AcpModel>[];
  ...
  /// prefill the context window (unless the user already set one).
  void _onModelChanged(String? modelId) {
    ...
    if (!_contextSizeUserEdited &&
        model.contextWindow != null &&
        (_contextSize == null || _contextSize == 0)) {
      _contextSizeCtl.text = model.contextWindow.toString();
      _contextSize = model.contextWindow;
    }
  }
