...
/// The `workspace_settings` key holding the ticketing vendor tickets live in.
///
/// Workspace-scoped because it names WHERE the shared board is; the credential
/// for that vendor is per-user and lives server-side.
const String ticketingProviderSettingKey = 'ticketing_provider';
...
/// The workspace's default AI-review level.
///
/// Workspace-scoped because it governs how much of a shared repository's
/// reviews every member reads, not one person's taste. An unset or
/// unrecognized value reads as [ReviewLevel.defaultLevel], which is what every
/// review ran as before levels existed.
final workspaceReviewLevelProvider = Provider<ReviewLevel>(
  (ref) =>
      ReviewLevel.fromWire(
        ref.watch(workspaceSettingProvider(kReviewLevelSettingKey)),
      ) ??
