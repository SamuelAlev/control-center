...
/// `skills.scanInstalled` wire shape.
...
class RpcSkillSecurityControl {
  ...
  /// Lists the skills the space's checked-out repos ship, with the repo each
  /// came from. Server-side these are already behind the scan gate, so every
  /// name returned is one a slash command can actually load.
  Future<List<RepoSkillDto>> repoSkills(String spaceId) async {
    final data = await _client.call('skills.repoSkills', {
      'space_id': spaceId,
    });
    final list = (data['skills'] as List?) ?? const [];
    return [
      for (final e in list)
        if (e is Map) RepoSkillDto.fromJson(e.cast<String, dynamic>()),
    ];
  }
}

/// One skill contributed by a repo checked out in the current space.
@immutable
class RepoSkillDto {
  /// Creates a [RepoSkillDto].
  const RepoSkillDto({
    required this.repo,
    required this.slug,
    required this.name,
  ...
  /// Parses from the `skills.repoSkills` wire shape.
  factory RepoSkillDto.fromJson(Map<String, dynamic> json) => RepoSkillDto(
  ...
  /// The repo directory name — shown as the provenance chip.
  final String repo;
  ...
  /// `<repo>:<name>` — what a slash command uses, so two repos may each ship
  /// a `testing` without one shadowing the other.
...
/// The skills every repo in a space contributes, for the composer's slash
/// palette.
///
/// Unlike an agent's own skill index — scoped to the one repo it is working in,
/// because that index costs context on every turn — this lists them all: a
/// human naming a skill pays no such cost, and a space can hold several agents
/// with different active repos, so there is no single "current" repo here.
///
/// An older host without the op returns nothing rather than failing the
/// composer.
final repoSkillListProvider =
    FutureProvider.family<List<RepoSkillDto>, String>((ref, spaceId) async {
      if (spaceId.isEmpty) {
        return const [];
      }
      try {
        return await ref.watch(skillSecurityControlProvider).repoSkills(spaceId);
