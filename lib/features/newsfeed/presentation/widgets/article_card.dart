...
class _MetaRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final tokens = context.designSystem;
    final muted = tokens?.textTertiary ?? theme.colorScheme.onSurfaceVariant;
        ...
        if (feed != null)
          Flexible(
            child: Text(
              ...
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: CcTypography.caption.copyWith(color: muted),
            ),
          ),
        if (feed != null && time != null)
          Text('  ·  ', style: CcTypography.caption.copyWith(color: muted)),
        if (time != null)
          AppTimestamp(
...
class _NetworkThumbnail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ProxiedImage(
      ...
      // The error widget fades in over prior content, so a failed load settles
      // onto the source-favicon fallback instead of flashing.
      errorBuilder: (_, _) => _PlaceholderThumbnail(feed: feed),
    );
  }
}
