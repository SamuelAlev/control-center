...
class _ArticleCardState extends State<ArticleCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final bg = _hovered
        ? (tokens?.bgPrimaryHover ?? fColors.muted)
        : (tokens?.bgPrimary ?? fColors.card);
    final border = tokens?.borderSecondary ?? fColors.border;
...
class _NetworkThumbnail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Image.network(
      url,
      fit: BoxFit.cover,
      // Keep the previous frame on rebuild so scrolling the grid doesn't blink
      // back to a blank box before the cached image repaints.
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            tokens?.bgSecondary ?? fColors.muted,
            tokens?.bgTertiary ?? fColors.muted,
          ],
        ),
      ),
      child: Center(
