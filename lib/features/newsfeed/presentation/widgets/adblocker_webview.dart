...
import 'package:control_center/features/newsfeed/domain/helpers/scriptlet_library.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return InAppWebView(
      ...
      initialSettings: InAppWebViewSettings(
        ...
        // Use a non-persistent (`WKWebsiteDataStore.nonPersistent()`) store
        // so cookies/cache/IndexedDB live in memory only. The reader is
        // ephemeral — open an article, read it, close it — so there is
        // nothing worth persisting. Critically, this stops WebKit from
        // writing its per-bundle "WebCrypto master key" to the macOS login
        // keychain when a page uses `crypto.subtle`; that keychain item
        // otherwise triggers a "Control Center wants to access … WebCrypto
        // vault" password prompt on every access under our ad-hoc/self-
        // signed builds.
        incognito: true,
      ),
      onWebViewCreated: (c) {
