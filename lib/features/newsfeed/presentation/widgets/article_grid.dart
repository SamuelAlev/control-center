...
class ArticleActionsNotifier extends Notifier<void> {
  ...
  Future<void> openArticle(BuildContext context, RssArticle article) async {
    ...
    // On web there is no in-app reader (the browser IS the reader), so always
    // open the article in a new tab instead of routing to the placeholder.
    if (kIsWeb || mode == ArticleOpenMode.externalBrowser) {
...
class _ArticleGridState extends ConsumerState<ArticleGrid> {
  ...
  /// One-shot guard set before a pointer-driven (click) focus change. The
  /// provider listener fires synchronously on `.set`, consumes this and skips
  /// the scroll-into-view so a tapped item — already on screen — doesn't jump
  /// toward the center. Keyboard focus changes (j/k, Tab) leave it false and
  /// keep scrolling the focused row into view.
  bool _suppressNextFocusScroll = false;
  ...
  void _focusArticle(String? id) {
    ...
    // Click-driven focus changes are already on screen; don't recenter them.
    // j/k and Tab still scroll the focused row into view.
    final suppressScroll = _suppressNextFocusScroll;
    _suppressNextFocusScroll = false;
  ...
  void _open(BuildContext context, RssArticle article) {
    // The user tapped a visible item; don't recenter the list under it.
    _suppressNextFocusScroll = true;
