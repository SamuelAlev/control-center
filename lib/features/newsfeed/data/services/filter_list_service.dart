...
class FilterListService {
  ...
  FilterListService(this._dio, this._prefs);
  ...
  static const _sources = [
    _AbpSource(
      ...
      category: _Category.ads,
    ),
    _AbpSource(
      name: 'idcac',
      url: 'https://www.i-dont-care-about-cookies.eu/abp/',
      category: _Category.cookies,
    ),
    _AbpSource(
      name: 'fanboy_cookie',
  ...
  Future<FilterListUpdateState> manualRefresh() async {
    ...
    final ruleList = <Map<String, dynamic>>[
      ...networkBlocks,
      ..._buildCssDisplayNoneRules(adsSelectors),
      ..._buildCssDisplayNoneRules(cookiesSelectors),
      ..._buildDomainScopedHideRules(domainHides),
      ..._buildScriptletRules(scriptlets),
    ];
  ...
  /// Reads the cached set from SharedPreferences, falling back to the
  ...
  void _parseSource(
    ...
    if (source.category == _Category.ads) {
  ...
  static List<Map<String, dynamic>> _buildScriptletRules(
    List<ScriptletInjection> scriptlets,
  ) {
  ...
  static List<Map<String, dynamic>> _buildCssDisplayNoneRules(
    List<String> selectors,
  ) {
    final rules = <Map<String, dynamic>>[];
    for (var i = 0; i < selectors.length; i += _cssChunkSize) {
      final end = (i + _cssChunkSize < selectors.length)
          ? i + _cssChunkSize
          : selectors.length;
      rules.add(<String, dynamic>{
  ...
  /// past its rule cap.
  static List<Map<String, dynamic>> _buildDomainScopedHideRules(
