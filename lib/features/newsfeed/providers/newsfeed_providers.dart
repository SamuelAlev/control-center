...
/// Provider for the newsfeed repository the UI reads through — the cc_data
/// RPC adapter over the desktop's in-process RPC server (the composition
/// flip). The newsfeed is PER-USER: the surface is session-scoped (the server
/// resolves the user from the RPC session), so unlike the server-side
/// `NewsfeedRepository` port it carries no user ids. Server-side EXECUTION
/// uses the DAO repository in `cc_server`.
final newsfeedRepositoryProvider = Provider<RpcNewsfeedRepository>((ref) {
  return RpcNewsfeedRepository(ref.watch(rpcClientProvider));
});
...
enum NewsfeedLayout {
  /// Magazine card grid with larger thumbnails — the default.
  grid,
...
///
/// Defaults to [NewsfeedLayout.grid]. An explicit `'list'` preference is
/// honored; anything else (unset, `'grid'`) is the magazine grid.
class NewsfeedLayoutController extends Notifier<NewsfeedLayout> {
  @override
  NewsfeedLayout build() {
    final prefs = ref.watch(appPreferencesProvider);
    return prefs.getString(_kNewsfeedLayoutKey) == 'list'
        ? NewsfeedLayout.list
        : NewsfeedLayout.grid;
  }
...
/// The active newsfeed layout (grid / list).
...
class NewsfeedRefreshController extends Notifier<bool> {
  ...
  Future<void> refreshAll() async {
    ...
    try {
      ...
      // Thin client (RPC): feed fetching runs host-side and the client reads
      // the synced articles, so there is nothing to refresh from here.
    } finally {
      state = false;
    }
  }
  ...
  Future<void> refreshFeed(String feedId) async {
    ...
    try {
      ...
      // See [refreshAll]: host-side only over the thin client.
    } finally {
      state = false;
    }
  }
...
class ContentBlockingController extends Notifier<bool> {
  ...
  Future<void> set({required bool enabled}) async {
    final prefs = ref.read(appPreferencesProvider);
    await prefs.setBool(_kBlockContentKey, value: enabled);
    state = enabled;
  }
}
...
class FilterListUpdateController extends Notifier<FilterListUpdateState> {
  ...
  Future<void> autoUpdate() async {
    state = await autoUpdateFilterList(ref);
  }
