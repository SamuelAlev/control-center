...
/// True when GitHub auth is available — either a client-side PAT or the gh CLI
/// authenticated on the server. The gh CLI probe runs on the host and
/// deliberately never ships its token to the client, so we check the
/// `isAuthenticated` flag rather than the (always empty) token.
final isGitHubAuthenticatedProvider = Provider<bool>((ref) {
  ...
  final cli = ref
      .watch(githubCliStatusProvider)
      .maybeWhen(data: (c) => c, orElse: () => null);
  return cli?.isAuthenticated ?? false;
});
