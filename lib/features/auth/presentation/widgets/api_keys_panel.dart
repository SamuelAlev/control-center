...
class ApiKeysPanel extends ConsumerWidget {
    ...
    return Column(
      ...
      children: [
        ...
        SectionCard(
          ...
          subtitle: Text(
            _patSubtitle(l10n, cliAsync, credentials.hasGitHubToken),
          ),
          child: Row(
            children: [
              CcButton(
  ...
  String _patSubtitle(
    ...
    AsyncValue<GitHubCliStatus> cliAsync,
    bool hasPat,
  ) {
    if (hasPat) {
...
class _GitHubCliCard extends StatelessWidget {
  ...
  Widget _body(BuildContext context) {
    ...
    return statusAsync.when(
      ...
      data: (s) {
        if (!s.isInstalled) {
          return Text(l10n.installGhCliBody);
        }
        if (!s.isAuthenticated) {
          return Text(l10n.runGhAuthLoginBody);
        }
        if (hasPatOverride) {
          return Text(l10n.ghCliAuthButPatOverrideBody);
        }
