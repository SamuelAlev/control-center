...
import 'package:control_center/core/domain/services/domain_event_audit_bridge.dart';
...
import 'package:control_center/core/infrastructure/observability/activity_log_persister.dart';
...
/// so presentation never touches the database layer.
final entityActivityProvider = StreamProvider.autoDispose.family<
  ...
  return ref
      .watch(activityLogDaoProvider)
      .watchForEntity(args.workspaceId, args.entityType, args.entityId)
      .map((rows) => [
            for (final r in rows)
              ActivityEntry(
                id: r.id,
