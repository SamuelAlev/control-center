...
/// An icon package exposes each style as a class of ~1530 `static const IconData` fields;
/// on Flutter web the dev compiler (DDC) lazily links a library the first time one of its
/// members is accessed and linking that giant library overflows the JS stack and crashes
/// with a `StackOverflowError` before the first frame renders (so `flutter run -d chrome`
/// breaks even though a dart2js release build is fine).
/// Declaring only the glyphs the app actually uses also keeps the icon tree-shaker
/// effective: the shipped font is a ~52 KB subset of the 489 KB original.
@staticIconProvider
