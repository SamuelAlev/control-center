...
class _EditorTabGroupState extends State<EditorTabGroup> {
  ...
  /// submenu. The tab is already selected by the strip before this fires.
  void _showTabMenu(AppLocalizations l10n, int index, Offset position) {
    final tabs = _c.tabs;
    if (index < 0 || index >= tabs.length) {
      return;
    }
    ...
    void split(DropEdge edge) =>
        widget.layout.splitActiveTabToward(widget.leafId, edge);

    showCcMenuAt(
      context: context,
      position: position,
      items: [
        CcMenuItem(label: l10n.close, onSelected: () => _closeTab(index)),
        CcMenuItem(
  ...
  List<Widget> _buildInlineActions(AppLocalizations l10n) {
    ...
    return [
      ...
      CcMenu(
        target: _MenuTrigger(AppIcons.plus, tooltip: l10n.ideNewTab),
        items: items,
      ),
    ];
  }
