...
class EditorLayoutController extends ChangeNotifier {
  ...
  /// The split-button behaviour: duplicate the active tab of [leafId] into a new
  /// leaf on the [edge] side. Duplication makes a fresh tab instance
  /// (independent body) so both panes show live, separate content. A
  /// [DropEdge.center] edge is a no-op (there is nothing to split toward).
  void splitActiveTabToward(String leafId, DropEdge edge) {
    ...
    if (tabs.isNotEmpty) {
      final active = tabs[leaf.controller.selectedIndex];
      newCtrl.insert(
        0,
        EditorTab(
          kind: active.kind,
