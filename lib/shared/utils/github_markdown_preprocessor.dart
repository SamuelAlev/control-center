...
class ImageDimensionHint {
  ...
  bool get isEmpty => width == null && height == null && widthPercent == null;
}
...
String rewriteUserAttachmentUrls(
  ...
  for (final line in lines) {
    ...
    result.add(
      line.replaceAllMapped(pattern, (m) {
...
String? _parseDimension(String? raw) {
  ...
  final percent = _percentSizeRe.firstMatch(trimmed);
  if (percent != null) {
    return '${percent.group(1)}p';
  }

  final px = _pxSizeRe.firstMatch(trimmed);
  if (px != null) {
    return px.group(1);
  }
  return null;
...
String preprocessGitHubReferences(
  ...
  for (final line in lines) {
    ...
    // Skip issue/PR shorthand on heading lines (`# `, `## `, …) so ATX
    // markers aren't eaten as `#123`. Mentions still rewrite — a heading
    // like `## Thanks @octocat` is a real mention.
    if (_headingRe.hasMatch(line)) {
      result.add(_rewriteMentions(line));
      continue;
    }
...
String _processLine(
  ...
  final skip = [
    ..._findInlineCodeRegions(line),
    ..._findMarkdownLinkRegions(line),
  ];
  final rewrites = <(RegExp, String Function(Match))>[];
  ...
  if (owner.isNotEmpty && repo.isNotEmpty) {
    ...
    rewrites.add((
      _crossRepoRefRe,
      (m) =>
          '[${m.group(0)}](control-center://pr/${m.group(1)}/${m.group(2)}/${m.group(3)})',
    ));
  ...
  rewrites.addAll(_mentionRewrites);
  return _applyRewrites(line, skip: skip, rewrites: rewrites);
}

String _rewriteMentions(String line) {
  final skip = [
    ..._findInlineCodeRegions(line),
    ..._findMarkdownLinkRegions(line),
  ];
  return _applyRewrites(line, skip: skip, rewrites: _mentionRewrites);
}

final List<(RegExp, String Function(Match))> _mentionRewrites = [
  // Teams first so `@org/team` is not truncated to `@org`.
  (
    RegExp('(?<![@\\w/])@($kGitHubLoginPattern)/($kGitHubLoginPattern)\\b'),
    (m) => '[${m.group(0)}](control-center://team/${m.group(1)}/${m.group(2)})',
  ),
  (
    RegExp('(?<![@\\w/])@($kGitHubLoginPattern)\\b(?!/)'),
    (m) => '[${m.group(0)}](control-center://user/${m.group(1)})',
  ),
];

/// Collects matches of [rewrites] on a copy of [line] whose [skip] ranges
/// are blanked out (so indices stay aligned), then applies non-overlapping
/// replacements from the end so earlier hits don't shift later ones.
String _applyRewrites(
  String line, {
  required List<(int, int)> skip,
  required List<(RegExp, String Function(Match))> rewrites,
}) {
  if (rewrites.isEmpty) {
    return line;
  }
  final masked = _maskRanges(line, skip);
  final hits = <({int start, int end, String text})>[];
  for (final (pattern, replacer) in rewrites) {
    for (final match in pattern.allMatches(masked)) {
      hits.add((start: match.start, end: match.end, text: replacer(match)));
    }
  }
  if (hits.isEmpty) {
    return line;
  ...
  var result = line;
  for (final hit in kept.reversed) {
    result = result.replaceRange(hit.start, hit.end, hit.text);
  }
  return result;
}

String _maskRanges(String line, List<(int, int)> ranges) {
  if (ranges.isEmpty) {
    return line;
  }
  ...
  for (final (start, end) in sorted) {
    ...
    final from = start < i ? i : start;
    if (from > i) {
      buf.write(line.substring(i, from));
    }
    buf.write(' ' * (end - from));
    i = end;
  }
...
/// `[label](url)` / `![alt](url)` spans so we don't wrap already-linked text.
List<(int, int)> _findMarkdownLinkRegions(String line) {
  final regions = <(int, int)>[];
  for (final match in _mdLinkInlineRe.allMatches(line)) {
...
String _trimUrlPunctuation(String url) =>
    url.replaceAll(_urlTrailingPunctRe, '');

/// Hoisted from the per-line loops below.
///
/// Dart does NOT cache `RegExp` construction, so building one inside a loop
/// compiles the pattern once per LINE. A 500-line PR body was thousands of
/// compilations per preprocessing pass, and the chain runs several passes.
final RegExp _fenceOpenRe = RegExp(r'^(\s*)```');
final RegExp _headingRe = RegExp(r'^#{1,6} ');
final RegExp _crossRepoRefRe = RegExp(
  r'(?<![.\w/&])([\w-]+)/([\w-]+)#(\d{1,7})\b',
);
final RegExp _issueRefRe = RegExp(r'(?<![.\w/&])#(\d{1,7})\b');
final RegExp _urlTrailingPunctRe = RegExp(r'''[.,;:!?)\]}'"]+$''');
final RegExp _percentSizeRe = RegExp(r'^(\d+(?:\.\d+)?)\s*%$');
final RegExp _pxSizeRe = RegExp(r'^(\d+(?:\.\d+)?)\s*(?:px)?$');
