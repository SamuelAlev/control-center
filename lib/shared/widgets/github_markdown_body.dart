...
/// Renders GitHub-flavoured markdown into Flutter widgets, supporting
/// `<details>` blocks, image attachments, and inline video embeds.
class GitHubMarkdownBody extends ConsumerWidget {
  ...
  /// Optional stylesheet for customising markdown rendering.
  final MarkdownStyleSheet? styleSheet;

  /// Optional custom builders for markdown elements.
  final Map<String, MarkdownElementBuilder>? builders;

  /// Optional builder for rendering checkboxes.
  final MarkdownCheckboxBuilder? checkboxBuilder;

  /// Whether the markdown body should shrink-wrap its content height.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // recurse naturally.
    final segments = _parseDetailsSegments(
      cleaned,
      defaultSummary: AppLocalizations.of(context).detailsLabel,
    );
  ...
  Widget _renderMarkdown({
    ...
    final processed = hasRepoContext
        ? preprocessGitHubReferences(embedded, owner: owner, repo: repo)
        : embedded;
...
List<_BodySegment> _parseDetailsSegments(
  String markdown, {
  String defaultSummary = 'Details',
}) {
  if (markdown.isEmpty) {
