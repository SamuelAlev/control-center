...
List<List<InlineSpan>> highlightCodeLines({
  ...
  final tokenLines = _tokenize(code: code, languageId: languageId, dark: dark);
  if (tokenLines == null) {
    return _plainLines(code);
  }
...
Future<List<InlineSpan>> highlightCodeSpansAsync({
  ...
  final tokenLines = await CcShikiTokenizer.instance.tokenizeAsync(
    code,
    langId: languageId,
    dark: dark,
  );
...
List<List<ThemedToken>>? _tokenize({
  ...
  debugHighlightParseCount++;
  return CcShikiTokenizer.instance.tokenizeSync(
    code,
    langId: languageId,
    dark: dark,
  );
