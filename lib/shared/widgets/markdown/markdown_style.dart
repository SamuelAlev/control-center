...
CcMarkdownStyle appMarkdownStyle(
  ...
  // The emoji font is APPENDED to whatever fallback the resolution produced,
  // never substituted for it: a user-selected family resolves with the bundled
  // Manrope behind it (what renders while its bytes are in flight), and
  // replacing that list leaves the emoji font as the only real family in the
  // chain — which is the same blown-open word gaps by another route.
  TextStyle uiWithEmoji(TextStyle base) {
    final style = ui(base);
    return style.copyWith(
      fontFamilyFallback: [...?style.fontFamilyFallback, ...emojiFallback],
    );
  }

  final body = uiWithEmoji(CcTypography.body).copyWith(
  ...
  final built = CcMarkdownStyle(
    ...
    listBullet: body,
    tableHead: uiWithEmoji(
      CcTypography.body,
    ).copyWith(fontSize: bodyFontSize, fontWeight: FontWeight.w600, color: fg),
    tableBody: uiWithEmoji(
      CcTypography.body,
