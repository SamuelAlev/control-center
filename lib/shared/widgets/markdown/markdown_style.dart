...
CcMarkdownStyle appMarkdownStyle(
  ...
  final uiFamily = context.ccTheme?.fontFamily;
...
class _SharedCodeBlockState extends State<_SharedCodeBlock> {
  ...
  List<InlineSpan> _spansFor(
    ...
    if (!widget.cache) {
      ...
      final lines = displayCode.split('\n');
      final head = lines.take(budget).join('\n');
      final tail = lines.skip(budget).join('\n');
      return [
        ...highlightCodeSpans(
          code: head,
          languageId: languageId,
  ...
  Widget build(BuildContext context) {
    ...
    if (oversized) {
      ...
      if (lines.length > kSharedCodeBlockMaxLines) {
        truncatedLines = kSharedCodeBlockMaxLines;
        displayCode = lines.take(kSharedCodeBlockMaxLines).join('\n');
      }
    } else {
      final lines = code.split('\n');
      if (lines.length > kSharedCodeBlockCollapseThreshold) {
        collapsedLines = lines.length - kSharedCodeBlockCollapsedLines;
        if (!_expanded) {
          displayCode = lines.take(kSharedCodeBlockCollapsedLines).join('\n');
        }
      }
    }
