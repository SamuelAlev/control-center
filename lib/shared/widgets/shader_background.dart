...
/// [Theme.of] brightness — light theme gets the golden-hour cloudscape, dark
...
class _ShaderBackgroundState extends State<ShaderBackground>
  ...
  // Freeze while scrolled out of view. The inbox hosts this inside a
  // scroll view, so without this check the full-bleed shader keeps burning
  // GPU/CPU at 30 FPS after the hero has scrolled away. Visibility is
  // re-checked at most every [_visibilityCheckInterval]; while hidden the
  // uniform stops updating, so nothing repaints and the shader costs nothing
  // beyond the (cheap) ticker callback.
  static const Duration _visibilityCheckInterval = Duration(milliseconds: 500);
  Duration _lastVisibilityCheck = Duration.zero;
  bool _visible = true;
  ...
  void _onTick(Duration elapsed) {
    ...
    _lastTick = elapsed;
    if (elapsed - _lastVisibilityCheck >= _visibilityCheckInterval) {
      _lastVisibilityCheck = elapsed;
      _visible = _isOnScreen();
    }
    if (!_visible) {
      return;
    }
  ...
  /// Whether any part of this widget currently intersects the screen.
  /// Defaults to true when geometry isn't available yet (first frame).
  bool _isOnScreen() {
    if (!mounted) {
      return false;
    }
