...
/// ([highlightCodeLines]) — not a parse per line — so rebuilds are cheap and
...
/// Hard tokenize caps for the preview: past either, the body renders plain.
/// The old highlighter had no work cap at all here — a 200KB Read output was
/// tokenized synchronously inside build.
...
class _CodePreviewState extends State<CodePreview> {
  ...
  /// Per-line spans for [displayCode]: synchronous within the grammar's
  /// line budget, otherwise async-with-plain-first (the tokenize lands in
  /// the highlight LRU; the rebuild picks it up via peek).
  List<List<InlineSpan>> _linesFor(String displayCode, {required bool dark}) {
    final lineCount = 1 + '\n'.allMatches(displayCode).length;
    final languageId = widget.languageId;
    if (languageId == null ||
        displayCode.length > _maxHighlightChars ||
    ...
    return highlightCodeLines(code: displayCode, languageId: null, dark: dark);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (lines.isNotEmpty && lines.last.isEmpty && displayCode.endsWith('\n')) {
      lines.removeLast();
    }
