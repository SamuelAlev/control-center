...
ToolPresentation resolveToolPresentation(ToolSegment seg) {
  ...
  // The harness file tools (`read`/`write`/`edit`) call it `path`; Claude
  // Code calls it `file_path`. Batched edits may carry the path per item.
  String? filePath() {
    final top = str('file_path') ?? str('path') ?? str('filename');
    if (top != null) {
      return top;
    }
    ...
    if (batch is List) {
      for (final item in batch) {
        if (item is Map) {
          final p = item['path'] ?? item['file_path'];
  ...
  switch (name) {
    case 'read':
      return ToolPresentation(
        ...
        subtitle: _withOffset(shortenPath(filePath()), inputs),
      );
    case 'edit':
    case 'multiedit':
    case 'str_replace':
    ...
    // `todowrite` is Claude Code's built-in; `todo_write`/`todo_read` are ours
    // (the MCP prefix is stripped upstream, the underscore is not).
    case 'todowrite':
    case 'todo_write':
    case 'todo_read':
