...
class GitHubUserProfileHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget avatarDisc = profile.avatarUrl.isNotEmpty
        ? CcAvatar(
            image: DiskCachedNetworkImage(
        ...
        : CcAvatar(size: avatarSize, initials: initial);

    // The status rides the avatar as a badge instead of sitting beside the
    // name: a long status overflowed the header row on the profile page.
    final status = profile.status;
    final avatar = status != null && statusHasContent(status)
        ? GitHubUserStatusAvatarBadge(
            status: status,
    ...
    final infoBlock = Column(
      ...
      children: [
        ...
        if (infoFooter != null) ...[const SizedBox(height: 10), infoFooter!],
      ],
    );
    ...
    return Column(
      ...
      children: [
        ...
        if (heatmap != null) ...[const SizedBox(height: 14), heatmap],
      ],
    );
  }
