...
class WorkspaceAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (trimmed.isEmpty) {
      final bg = ds?.bgTertiary ?? fTheme.colors.muted;
      final fg = ds?.fgTertiary ?? fTheme.colors.mutedForeground;
      return Container(
        width: size,
        height: size,
    ...
    final ink = ds?.fg ?? const Color(0xFF1F1F1F);
    ...
    return Container(
      ...
      decoration: ShapeDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            ds?.sunshine500 ?? const Color(0xFFFFB83E),
            ds?.sunshine900 ?? const Color(0xFFFF8A00),
            ds?.accent ?? const Color(0xFFFA520F),
            ds?.blockEdge ?? const Color(0xFFC0400F),
          ],
        ),
