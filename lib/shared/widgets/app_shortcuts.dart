...
/// like ⌘K or ⌘, fire from anywhere — they no longer depend on this subtree
...
class AppShortcuts extends ConsumerStatefulWidget {
  ...
  const AppShortcuts({
    ...
    required this.readActiveWorkspaceId,
  ...
  /// Reads the active workspace id at the moment a nav shortcut fires.
  /// Workspace-scoped destinations need it; the app root sources it from its
  /// synced provider (the URL is the source of truth) so shared/ stays free
  /// of feature imports.
  final String? Function() readActiveWorkspaceId;
...
class _AppShortcutsState extends ConsumerState<AppShortcuts> {
  ...
  Map<String, VoidCallback> _buildHandlers() {
    ...
    // Workspace-scoped destinations need the active workspace id; the app
    // root feeds it in (the URL is the source of truth, mirrored by its
    // synced provider). With no active workspace, fall back to the picker.
    String nav(String Function(String) build) {
      final w = widget.readActiveWorkspaceId();
