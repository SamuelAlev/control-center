...
/// Immutable snapshot of the navigation history state.
class NavigationHistoryState {
  /// Creates a [NavigationHistoryState].
...
/// Controller that tracks browser-style navigation history for mouse buttons.
class NavigationHistoryController extends Notifier<NavigationHistoryState> {
  ...
  /// Triggers the registered back-navigation callback.
  void goBack() => _onGoBack?.call();

  /// Triggers the registered forward-navigation callback.
  void goForward() => _onGoForward?.call();

  /// Updates the navigation state and callbacks.
  void update({
...
/// Riverpod provider for the [NavigationHistoryController].
...
/// Listens to mouse button 4/5 (back/forward) events and delegates them to a [NavigationHistoryController].
class MouseNavigationHandler extends StatefulWidget {
  /// Creates a [MouseNavigationHandler].
  ...
  /// The child widget to wrap with mouse navigation handling.
  final Widget child;

  /// Optional external controller for navigation history state.
...
class _MouseNavigationHandlerState extends State<MouseNavigationHandler> {
  ...
  Widget build(BuildContext context) {
    // Track the full URI (path + query), not matchedLocation: query-driven
    // state — a `?tab=` editor-tab switch on the channel / PR detail pages,
    // a `?m=` permalink — is a real navigation the back/forward stack must
    // see. matchedLocation strips the query and would skip those entries.
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    // On the web the browser natively handles the mouse back/forward buttons as
    // history navigation. Intercepting them here too double-navigates: the
    // browser steps back while this handler also navigates from the new
    // position — which shows up as a flash of the previous page that snaps back
    // to the current one. Let the browser own them on web; the in-app history
    // stack (and the title-bar back/forward buttons it powers) still work.
  ...
  void _goBack() {
    ...
    if (_backStack.isNotEmpty) {
      ...
      // Fall back to the active workspace's inbox (the in-shell home).
      final wsId = GoRouterState.of(context).pathParameters['workspaceId'];
      final home = wsId == null ? workspaceListRoute : inboxRoute(wsId);
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: (tokens?.fg ?? const Color(0xFF1F1F1F)).withValues(
                    alpha: 0.5,
                  ),
                  border: Border.all(
                ...
                child: const Icon(AppIcons.move, color: Colors.white, size: 16),
              ),
            ),
          ],
        ),
