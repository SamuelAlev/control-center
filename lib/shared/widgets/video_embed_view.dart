                  ...
                  children: [
                    InAppWebView(
                      ...
                      initialUrlRequest: URLRequest(
                        url: WebUri.uri(widget.embedUrl),
                      ),
                      initialSettings: InAppWebViewSettings(
                        // Non-persistent data store: WebKit otherwise stashes a
                        // per-bundle WebCrypto master key in the login keychain
                        // and prompts when an embed touches crypto.subtle. A
                        // trusted signature does NOT suppress it (verified). This
                        // is a throwaway embed, so losing persistence is free.
                        // Don't remove without re-verifying no keychain prompt.
                        incognito: true,
                    ...
                    if (!_loaded)
                      ColoredBox(
                        ...
                        child: const Center(child: CcSpinner(size: 24)),
                      ),
                  ],
                ),
              ),
...
class _VideoEmbedFallbackCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8),
      child: InkWell(
        onTap: () => openExternalUrl(sourceUrl.toString()),
        borderRadius: BorderRadius.circular(8),
        child: Container(
          height: 140,
          ...
          child: Column(
            ...
            children: [
              Icon(AppIcons.circlePlay, size: 28, color: hint),
