...
Future<void> handleGitHubLink({
  ...
  if (refInfo is GitHubPrReference) {
    ...
    if (isSameRepo) {
      // The URL is the source of truth for the active workspace (every
      // surface hosting GitHub markdown sits under a workspace-prefixed
      // route), so read the id from the route rather than a feature provider.
      final wsId = context.currentWorkspaceId;
...
Future<_WorkspaceRepo?> _findWorkspaceForRepo(
  ...
  for (final ws in await wsRepo.getAll()) {
    final match = (await repoRepo.getAll(ws.id)).firstWhereOrNull(
      (r) =>
          r.remoteOwner.toLowerCase() == ownerLower &&
          r.remoteName.toLowerCase() == repoLower,
    );
    if (match != null) {
