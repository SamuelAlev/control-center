...
class ScratchpadMentionSource extends SyncMentionSource {
  ...
  String? sectionLabel(BuildContext context) =>
      AppLocalizations.of(context).taskMentionSection;
  ...
  List<MentionSuggestion> suggestSync(MentionQuery query) {
    ...
    final matchesAlias =
        partial.isEmpty || aliases.any((a) => a.startsWith(partial));
    if (!matchesAlias) {
      return const [];
    }
    return [
      MentionSuggestion(
        ...
        replacement: '@task ',
        payload: {'scratchpadId': workspaceId, 'workspaceId': workspaceId},
      ),
    ];
  }
}
