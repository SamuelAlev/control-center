...
///
/// When [codeRunsAsText] is set, code runs degrade to backgrounded monospace
/// [TextSpan]s instead of [WidgetSpan] chips. This is REQUIRED whenever the
/// title ellipsizes: `RenderParagraph` lays every inline child out with the
/// line's FULL max width (a long unbreakable code run inflates its chip to
/// the whole cap) and drops every placeholder past the ellipsis truncation
/// point (`positionInlineChildren` leaves it a null offset, so it paints
/// nowhere) — the visible result is "prefix …" followed by dead space. Pure
/// text spans ellipsize per grapheme, so nothing can be dropped.
List<InlineSpan> buildInlineCodeSpans(
  BuildContext context,
  String text, {
  required TextStyle baseStyle,
  bool codeRunsAsText = false,
}) {
  ...
  for (final match in _inlineCodeRegex.allMatches(text)) {
    ...
    if (codeRunsAsText) {
      // No padding/radius is possible on a text-run background — the wash
      // hugs the glyphs. That is the accepted trade-off for a title that
      // must ellipsize (see the doc above).
      spans.add(
        TextSpan(
          text: match.group(1)!,
          style: codeStyle.copyWith(
            backgroundColor: inlineCodeChipColor(context),
          ),
        ),
      );
    } else {
      spans.add(
        WidgetSpan(
          // Baseline, not middle: `middle` centres the (shorter) chip box on
          // the text's vertical midpoint, which lands the code glyphs ~1px
          // above the surrounding baseline and reads as a floating title.
          // Sitting the code on the same alphabetic baseline is what inline
          // code does everywhere else (see [AppInlineCodeBuilder] in the
          // markdown registries).
  ...
  Widget build(BuildContext context) {
    ...
    return Text.rich(
      TextSpan(
        ...
        children: [
          ...?leading,
          // An ellipsizing line must stay all-text: a WidgetSpan past the
          // truncation point is dropped by the paragraph (painted nowhere,
          // leaving dead space after the ellipsis), so code runs degrade to
          // backgrounded monospace text runs when ellipsis is requested.
          ...buildInlineCodeSpans(
            context,
            title,
            baseStyle: base,
            codeRunsAsText: overflow == TextOverflow.ellipsis,
          ),
        ],
      ),
      maxLines: maxLines,
