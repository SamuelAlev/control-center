...
/// The absolute-timestamp format used by the hover card and the semantics
/// label.
///
/// Hoisted to a `final`: constructing a `DateFormat` builds an ICU pattern and
/// pulls locale data, and this ran on every build of every timestamp — of
/// which a list surface renders one per row.
...
String _absoluteLabel(DateTime dt) {
  ...
  return '${_gmtLabel(local)}  ${_absoluteFormat.format(local)}';
}
    ...
    return Column(
      ...
      children: [
        _row(
          ...
          value: _absoluteFormat.format(local),
