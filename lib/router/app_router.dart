      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            path: calendarRoute,
            pageBuilder: (context, state) => NoTransitionPage(
              key: const ValueKey(calendarRoute),
              child: _absorb(const CalendarScreen()),
            ),
          ),
          // Sibling (not nested) so the event-detail location replaces the
          // calendar page in place (shared key → same master-detail screen).
          GoRoute(
            path: '$calendarRoute/:eventId',
            pageBuilder: (context, state) => NoTransitionPage(
              key: const ValueKey(calendarRoute),
              child: _absorb(
                CalendarScreen(selectedEventId: state.pathParameters['eventId']),
              ),
            ),
          ),
        ],
