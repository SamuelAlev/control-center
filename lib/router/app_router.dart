...
import 'package:control_center/features/messaging/presentation/screens/channels_home_screen.dart';
...
import 'package:control_center/features/settings/presentation/screens/server_sharing_settings_screen.dart';
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  const channelsPageKey = ValueKey('channels_screen');
  const channelsHomePageKey = ValueKey('channels_home_screen');
      ...
      ShellRoute(
        ...
        routes: [
                  ...
                  return buildPage(
                    ...
                    _absorb(
                      PullRequestDetailScreen(
                        ...
                        // convention as the channel page).
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) => buildPage(
              state,
              _absorb(const ChannelsHomeScreen()),
              key: channelsHomePageKey,
            ),
          ),
          // SIBLING (not nested) with its own page key ([channelsPageKey]) so
          // detail↔detail locations reuse the one MessagingScreen instance in
          // place — the selected channel id comes from the URL (source of
          // truth), same pattern as the tickets list↔detail routes.
          GoRoute(
            path: '${channelsRoute(workspaceIdParam)}/:channelId',
            pageBuilder: (context, state) => buildPage(
              state,
              _absorb(
                MessagingScreen(
          ...
          GoRoute(
            path: settingsServerSharingRoute(workspaceIdParam),
            pageBuilder: (context, state) => buildPage(
              state,
              _absorb(const ServerSharingSettingsScreen()),
            ),
          ),
          GoRoute(
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) =>
                buildPage(state, _absorb(const TeamsSettingsScreen())),
          ),
          GoRoute(
