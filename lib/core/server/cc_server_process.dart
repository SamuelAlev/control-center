...
///
/// Lifecycle: [start] → [endpoint]/[exitCode] → [stop]. A handle is SINGLE-USE:
/// [start] throws once this instance has been started, stopped or has become
/// ready, because the readiness completer cannot be re-armed. The supervisor
/// does not auto-restart; callers that want resilience watch [exitCode] and
/// construct a FRESH instance (a dead child means a new endpoint).
class CcServerProcess {
  ...
  Future<CcServerEndpoint> start({
    ...
    // A handle is single-use. `stop()`/`_kill()` null `_process`, so guarding on
    // that alone let a reused handle spawn a SECOND child whose ready line could
    // never complete the already-completed `_ready` — the caller then waited the
    // full timeout and SIGKILLed a perfectly healthy server. Refuse instead:
    // construct a fresh CcServerProcess per launch (what every caller does).
    if (_process != null || _ready.isCompleted || _stopRequested) {
      throw StateError(
        'CcServerProcess is single-use — construct a new one to restart',
      );
    }
...
class CcServerLauncher {
  ...
  }) => ['--data-dir', dataDir, '--port', '$port', '--bind', bind];
  ...
  static CcServerProcess? resolve({
    ...
    // Dev fallback: run from source with a Dart SDK.
    final dart = devDartPath(root);
    final entry = _devEntryPath(root);
    if (dart != null && File(entry).existsSync()) {
      return devRun(
        dartExecutable: dart,
        repoRoot: root,
        dataDir: dataDir,
  ...
  /// The Dart executable the dev fallback runs `cc_server` with, or null when
  /// this machine has none.
  ///
  /// The repo's fvm-pinned SDK is probed FIRST and that ordering is the whole
  /// point: the pinned SDK is the one the lockfile resolved against, so a
  /// developer with fvm keeps getting exactly it. The fallbacks exist because
  /// "no `.fvm/` checkout" is not the same as "no Dart", and treating them as
  /// the same meant this resolver returned null — no server at all — on any
  /// machine that installed Flutter the ordinary way, CI runners included.
  /// That is what kept the desktop's thin-client boot path
  /// (`test/integration/fork_a_spawn_connect_test.dart`) skipped in CI
  /// forever.
  @visibleForTesting
  static String? devDartPath(String root) {
    final flutterRoot = Platform.environment['FLUTTER_ROOT'];
    final exeDir = File(Platform.resolvedExecutable).parent.path;
    final suffix = Platform.isWindows ? '.exe' : '';
    for (final candidate in [
      '$root/.fvm/flutter_sdk/bin/dart$suffix',
      if (flutterRoot != null && flutterRoot.isNotEmpty) ...[
        '$flutterRoot/bin/dart$suffix',
        '$flutterRoot/bin/cache/dart-sdk/bin/dart$suffix',
      ],
      // A plain `dart test` run: the process IS the Dart VM, so its own
      // directory holds the executable we want.
      '$exeDir/dart$suffix',
    ]) {
  ...
  static String describeSearchedLocations({String? repoRoot}) {
    ...
    final dart = devDartPath(root);
    final entry = _devEntryPath(root);
    final devOk = dart != null && File(entry).existsSync();
    lines.add(
      line('dev run: ${dart ?? '<no Dart SDK found>'} $entry', exists: devOk),
    );
    return lines.join('\n');
  }
  ...
  static CcServerProcess devRun({
    ...
    return CcServerProcess(
      ...
      args: [
        'run',
        entry,
        ...serverArgs(dataDir: dataDir, port: port, bind: bind),
      ],
