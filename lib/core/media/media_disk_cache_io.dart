...
class MediaDiskCache {
  ...
  /// render.
  Future<Uint8List?> get(String url, {String? cacheKey}) {
    final key = _keyFor(cacheKey ?? url);
  ...
  /// nothing else — but wrong for a caller that needs the response's
  /// `Content-Type` (to tell an SVG from a raster) or wants to read the pixel
  /// dimensions out of the header mid-download. Those callers pair [peek] with
  /// [put] and keep their own fetch.
  Future<Uint8List?> peek(String url, {String? cacheKey}) async {
    final key = _keyFor(cacheKey ?? url);
  ...
  /// Stores [bytes] for [url] (or [cacheKey]) without fetching anything.
  ///
  /// Honours the same per-entry ceiling as the fetching path: one oversized
  /// body must not evict the working set.
  Future<void> put(String url, Uint8List bytes, {String? cacheKey}) async {
    if (bytes.isEmpty || bytes.length > maxEntryBytes) {
      return;
    }
    await _write(_keyFor(cacheKey ?? url), bytes);
  }
  ...
  File _bodyFile(String key) => File(
