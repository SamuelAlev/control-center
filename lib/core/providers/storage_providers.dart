...
///
/// macOS uses the **legacy login keychain** (`usesDataProtectionKeychain: false`)
/// rather than the iOS-style data-protection keychain. The data-protection
/// keychain requires a team-resolved `keychain-access-groups` entitlement, which
/// an ad-hoc / unsigned build (no Apple Developer team) cannot provide — writes
/// fail with `errSecMissingEntitlement` (-34018). The login keychain needs no
/// such entitlement, so secure storage works for both local dev and the
/// ad-hoc-unsigned release without any Apple account. (The App Sandbox is also
/// off — see the Runner entitlements — so the login keychain is reachable.)
