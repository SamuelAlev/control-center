...
enum AgentEffort {
  /// Minimal effort — fast, cheap responses.
  low,
  /// Balanced effort.
  medium,
  /// Maximum effort — thorough, expensive responses.
  high;

  /// Parses a string into an [AgentEffort], returning `null` for unrecognized
  /// values.
  static AgentEffort? tryParse(String? value) {
    if (value == null) {
      return null;
    }
  ...
  /// Parses a string into an [AgentEffort], defaulting to [medium] for
  /// unrecognized values.
  static AgentEffort? fromNullable(String? value) {
    if (value == null) {
      return null;
    }
    return tryParse(value) ?? AgentEffort.medium;
  }
}
