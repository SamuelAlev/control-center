...
class KeyStroke {
  ...
  /// (e.g. a `cmd` binding and a `ctrl` binding on Windows, where [cmd]
  /// resolves to the Control key).
  String canonical(TargetPlatform platform) {
    final isMac = platform == TargetPlatform.macOS;
    final names = <String>{
      if (cmd) isMac ? 'meta' : 'control',
      if (ctrl) 'control',
      if (shift) 'shift',
      if (alt) 'alt',
    }.toList()..sort();
...
class KeyChord {
  ...
  /// The first stroke — the one the dispatcher matches key events against.
