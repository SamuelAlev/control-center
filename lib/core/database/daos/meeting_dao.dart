...
class MeetingDao extends DatabaseAccessor<AppDatabase> with _$MeetingDaoMixin {
  ...
  /// Replaces a meeting's action items wholesale, but carries the user's triage
  /// state forward: a re-run regenerates rows from the agent's output, so the
  /// persisted `done` flag + `ticketId` link are preserved for any item whose
  /// `content` matches a prior row (matched by content because ids are fresh).
  Future<void> replaceActionItems(
    ...
    List<MeetingActionItemsTableCompanion> items,
      ...
      transaction(() async {
        final prior = await (select(meetingActionItemsTable)..where(
              (t) =>
                  t.meetingId.equals(meetingId) &
                  t.workspaceId.equals(workspaceId),
            ))
        ...
        final byContent = <String, MeetingActionItemsTableData>{
          for (final row in prior) row.content: row,
        };
  ...
  /// Replaces a meeting's decisions wholesale (idempotent re-run).
