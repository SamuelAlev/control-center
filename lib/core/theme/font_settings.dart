...
class FontSettingsNotifier extends Notifier<FontSettings> {
  ...
  Future<void> setCodeFontLigatures({required bool enabled}) async {
    await _prefs.setBool(codeFontLigaturesKey, value: enabled);
  ...
  Future<void> loadSystemFont(FontSelection selection) async {
    ...
    final loaded = await loadSystemFontFromFile(
      selection.family,
      selection.filePath!,
    );
    if (loaded) {
