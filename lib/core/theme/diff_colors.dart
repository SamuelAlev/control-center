...
class DiffColors {
  ...
  /// Diff colors distilled from an imported editor theme (e.g. a VS Code
  /// theme), so the embedded diff matches the user's IDE. Such themes only
  /// carry line-level diff backgrounds, so the word emphasis is a translucent
  /// accent that composites over whatever line tint the theme chose and the
  /// gutter reuses the line tint with the theme's own line-number color.
  factory DiffColors.fromEditorTheme({
    required Brightness brightness,
    required Color addedBackground,
    required Color removedBackground,
    ...
    return DiffColors(
      additionBg: addedBackground,
      deletionBg: removedBackground,
      additionWordBg: green.withValues(alpha: 0.4),
      deletionWordBg: red.withValues(alpha: 0.4),
      additionGutterBg: addedBackground,
      deletionGutterBg: removedBackground,
      additionGutterFg: lineNumberColor,
      deletionGutterFg: lineNumberColor,
      additionAccent: green,
      deletionAccent: red,
      contextFg: foreground,
      gutterFg: lineNumberColor,
      hunkBg: foreground.withValues(alpha: dark ? 0.06 : 0.04),
    );
  }
