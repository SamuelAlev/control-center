...
fragment PrChecksFields on PullRequest {
  ...
  Future<List<Map<String, dynamic>>> _searchOpenPullRequestNodes({
    ...
    for (var start = 0; start < repos.length; start += _reviewSearchChunkSize) {
      ...
      nodes.addAll(
        searchNodes.whereType<Map<String, dynamic>>().where((n) => n.isNotEmpty),
      );
    }
    return nodes;
  }
  ...
  Future<GitHubPrReviewState> getPullRequestReviewState({
    ...
    final reviewRequests = pullRequest['reviewRequests'] as Map<String, dynamic>?;
    for (final rr in (reviewRequests?['nodes'] as List? ?? const [])
        .whereType<Map<String, dynamic>>()) {
      ...
      if (reviewer['__typename'] == 'Team') {
        ...
        pendingTeams.add(GitHubPendingTeamRequest(
        ...
        pendingUsers.add(GitHubPendingUserRequest(
          login: login,
    ...
    for (final r in (latestReviews?['nodes'] as List? ?? const [])
        .whereType<Map<String, dynamic>>()) {
      ...
      for (final t in (obo?['nodes'] as List? ?? const [])
          .whereType<Map<String, dynamic>>()) {
        final slug = t['slug'] as String? ?? '';
        if (slug.isEmpty) {
          continue;
        }
        onBehalfOf.add(GitHubReviewTeamRef(
      ...
      completed.add(GitHubCompletedReview(
        authorLogin: login,
