...
//
// These are BASE secure-storage keys. The Google account is connected
// per-workspace (workspace-isolation invariant), so `GoogleCredentialsRepository`
// suffixes each key with `__<workspaceId>` — a token written for one workspace
// is never readable from another.

/// Base secure-storage key for the Google OAuth access token.
const String googleAccessTokenKey = 'google_access_token';

/// Base secure-storage key for the Google OAuth refresh token.
const String googleRefreshTokenKey = 'google_refresh_token';

/// Base secure-storage key for the Google OAuth access-token expiry (ISO-8601).
const String googleTokenExpiryKey = 'google_token_expiry';

/// Base secure-storage key for the connected Google account email.
const String googleAccountEmailKey = 'google_account_email';

/// Base secure-storage key for the granted Google OAuth scope string.
const String googleScopeKey = 'google_scope';
