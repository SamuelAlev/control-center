...
class SharedPreferencesNotificationPreferences
  ...
  static const _mutedReposKey = 'notifications_muted_repos';
  ...
  /// The notification preference keys that follow the USER across devices.
  ///
  /// Quiet hours, the enabled categories and the batch policy describe the
  /// human, not the machine: they have to hold on the phone and the laptop at
  /// once, or `cc_remote` buzzes at 3am because that device was never
  /// configured. Exposed as one list so the synced-preference registry cannot
  /// drift from the keys this class actually reads.
  ///
  /// [_volumeKey] is deliberately EXCLUDED — laptop speakers and phone speakers
  /// want different volumes, so it stays device-local.
  static List<String> get syncedKeys => [
    _globalKey,
    for (final category in NotificationCategory.values)
      '$_categoryPrefix${category.name}',
    _batchPolicyKey,
    _quietEnabledKey,
    _quietStartHourKey,
    _quietStartMinKey,
  ...
  Future<void> setGlobalEnabled({required bool enabled}) async =>
      _prefs.setBool(_globalKey, value: enabled);
  @override
  ...
  Future<void> setCategoryEnabled(
    ...
    required bool enabled,
      ...
      _prefs.setBool('$_categoryPrefix${category.name}', value: enabled);
  @override
  Future<Set<String>> getMutedRepos() async => _decodeMutedRepos(
    _prefs.getString(_mutedReposKey),
  );
  ...
  Future<void> setRepoMuted(
    ...
    final key = repoFullName.trim().toLowerCase();
    if (key.isEmpty) {
      return;
    }
    final current = _decodeMutedRepos(_prefs.getString(_mutedReposKey));
    if (muted ? !current.add(key) : !current.remove(key)) {
      return;
    }
    // Sorted so two devices that muted the same repos converge on identical
    // bytes — the preference sync compares values, and set iteration order
    // would otherwise look like a change on every promotion pass.
    final sorted = current.toList()..sort();
    await _prefs.setString(_mutedReposKey, jsonEncode(sorted));
  }

  /// Tolerant of anything that is not a JSON list of strings: a corrupt or
  /// hand-edited value mutes nothing rather than throwing on every render.
  static Set<String> _decodeMutedRepos(String? raw) {
  ...
  Future<void> setQuietHours(QuietHoursConfig config) async {
    await _prefs.setBool(_quietEnabledKey, value: config.enabled);
  ...
  @override
  Future<double> getVolume() async => _prefs.getDouble(_volumeKey) ?? 1.0;
