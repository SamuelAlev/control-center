...
import 'package:control_center/core/infrastructure/audio/aec/aec_ffi_bindings.dart' show AecFfiBindings;
...
/// shared library (`libaec_ffi`), tried in order by [AecFfiBindings.tryLoad]:
/// an explicit `AEC_FFI_DYLIB` override, the app-support install, the dev build
/// under the project's `macos/Frameworks/`, and the native staging dir
/// (`build/natives/`, where `scripts/build_aec.sh` writes it). The bundled-app
/// `@executable_path/../Frameworks/...` default is appended by [AecFfiBindings]
/// itself. The app degrades to the text echo filter when none resolve.
List<String> aecFfiDylibCandidatePaths() {
  final name = Platform.isWindows
      ? 'aec_ffi.dll'
      : Platform.isMacOS
          ? 'libaec_ffi.dylib'
          : 'libaec_ffi.so';
  final root = FontCachePathProvider.realAppSupportDir.path;
  final env = Platform.environment['AEC_FFI_DYLIB'];
  return [
    if (env != null && env.isNotEmpty) env,
    p.join(root, name),
    p.join(Directory.current.path, 'macos', 'Frameworks', name),
    p.join(Directory.current.path, 'build', 'natives', name),
  ];
}
