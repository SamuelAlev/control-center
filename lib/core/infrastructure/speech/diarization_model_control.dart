...
/// The current diarization-model snapshot as a LIVE stream, or `null` when
/// the connected server exposes no model control (`models.diarizationStatus`
/// / `models.watchDiarization` absent → `opUnknown`). The section renders
/// the "managed on the server host" placeholder for the null case.
final diarizationModelStatusSnapshotProvider =
    StreamProvider<ModelStatusSnapshot?>(
      (ref) => modelStatusStream(ref.watch(diarizationModelControlProvider)),
    );
