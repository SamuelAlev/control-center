...
/// A live handler registration (see [setDesktopUpdaterHandlers]).
///
/// Handler registration used to be raw last-caller-wins statics: a second
/// caller silently stole the first's callbacks and neither could tell. A handle
/// makes ownership explicit — [release] only clears the handlers while this
/// registration is still the active one, so a stale handle can never
/// unregister its successor.
class DesktopUpdaterHandlerRegistration {
  DesktopUpdaterHandlerRegistration._();

  /// An inert registration that owns nothing (fakes / off-desktop parity).
  const DesktopUpdaterHandlerRegistration.inert();

  /// Whether these handlers are the ones the updater currently fires.
  bool get isActive => identical(_activeRegistration, this);

  /// Releases the handlers if still active; a no-op once superseded.
  void release() {
    if (!isActive) {
      return;
    }
    ...
    _onAvailable = null;
    _onNotAvailable = null;
    _onError = null;
  }
}

DesktopUpdaterHandlerRegistration? _activeRegistration;

/// Registers the outcome handlers (available / not-available / error) the
/// native updater events forward to, superseding any previous registration.
DesktopUpdaterHandlerRegistration setDesktopUpdaterHandlers({
  void Function(String? version, String? notes)? onAvailable,
  void Function()? onNotAvailable,
  void Function(String message)? onError,
}) {
  if (_activeRegistration != null) {
    // Not fatal (a server switch rebuilds the controller), but silent theft is
    // exactly the bug this shape exists to make visible.
    AppLog.w(
      'DesktopUpdater',
      'update handlers replaced while a previous registration was still '
          'active — the old controller will stop receiving outcomes',
    );
  }
  _onAvailable = onAvailable;
  _onNotAvailable = onNotAvailable;
  _onError = onError;
  return _activeRegistration = DesktopUpdaterHandlerRegistration._();
}
