...
Agents run inside OS-native sandboxes, collaborate over messaging channels, review pull requests, execute DAG-based pipelines, and share a workspace-scoped knowledge memory — all behind a native GUI with GitHub and Linear integration.
...
│   ├── mcp/              # MCP server + 70 typed tools (uses application/ instead of presentation/)
...
| Networking | dio (GitHub REST/GraphQL, Linear GraphQL) + json_serializable models |
| UI components | forui (FTheme, FSidebar) + Material 3, google_fonts, fl_chart, flutter_flow_chart |
...
| On-device ML | sherpa_onnx + onnxruntime_v2 (speech), sqlite_vector + dart_wordpiece (embeddings) |
| Audio & video | record, audioplayers, video_player + chewie |
...
| Code generation | build_runner, json_serializable, drift_dev |
...
| `/pull-requests` · `/pull-requests/:prNumber` | PR list · PR detail with diff viewer |
