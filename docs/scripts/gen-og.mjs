...
function sceneColorLight(ux, uy) {
  ...
  const canvasWhite = [0.988, 0.984, 0.976];
  const hazeIvory = [0.963, 0.941, 0.902];
  const warmSand = [0.929, 0.882, 0.812];
  const horizonTaupe = [0.855, 0.769, 0.671];
  const emberCore = [0.98, 0.314, 0.059];
  const sunlitWisp = [1.0, 0.976, 0.918];
  const cloudShade = [0.792, 0.616, 0.537];

  let color = mix3(canvasWhite, hazeIvory, smoothstep(0.05, 0.6, uy));
  color = mix3(color, warmSand, smoothstep(0.45, 0.92, uy));
  color = mix3(color, horizonTaupe, smoothstep(0.68, 1.02, uy) * 0.75);
  color = mix3(color, emberCore, smoothstep(0.9, 1.14, uy) * 0.22);

  const cloud = smoothstep(0.35, 0.75, n);
  const band = smoothstep(0.15, 0.55, uy) * smoothstep(0.95, 0.45, uy);
  const glow = cloud * band;

  color = mix3(color, sunlitWisp, glow * 0.5);
  color = mix3(color, sunlitWisp, Math.pow(glow, 3) * 0.4);

  const wisp = smoothstep(0.45, 0.15, n) * smoothstep(0.85, 0.05, uy);
  color = mix3(color, cloudShade, wisp * 0.45);
  return color;
}
function sceneColorDark(ux, uy) {
  ...
  color = mix3(
    color,
    [nearBlack[0] * 0.7, nearBlack[1] * 0.7, nearBlack[2] * 0.7],
    wisp * 0.35,
  );
  return color;
}
...
const overlay = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <rect x="72" y="232" width="20" height="2" fill="#fb6224"/>
...
await sharpFn(buf, { raw: { width: W, height: H, channels: 3 } })
  ...
  // Indexed-palette PNG with dithering. The card is a smooth ember gradient
  // plus crisp high-contrast text — it quantizes to 256 colors cleanly and
  // drops from ~800 KB to ~200 KB. Social cards don't need 24-bit color and
  // a lighter card means faster unfurls. compression/effort maxed for size.
  .png({
    palette: true,
    quality: 90,
    colours: 256,
...
console.log(
  "[gen-og] wrote public/og.png (1200x630) — shader cloudscape background",
);
