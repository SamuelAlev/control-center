...
function sceneColorLight(ux, uy) {
  ...
  const hazeGold = [1.0, 0.898, 0.663];
  const sunGold = [1.0, 0.816, 0.416];
  const horizonAmber = [1.0, 0.722, 0.243];
  const emberCore = [0.98, 0.322, 0.059];
  const sunlitWisp = [1.0, 0.976, 0.918];
  const cloudShade = [0.792, 0.616, 0.537];

  let color = mix3(canvasWhite, hazeGold, smoothstep(0.05, 0.6, uy));
  color = mix3(color, sunGold, smoothstep(0.45, 0.92, uy));
  color = mix3(color, horizonAmber, smoothstep(0.68, 1.02, uy) * 0.75);
  color = mix3(color, emberCore, smoothstep(0.9, 1.14, uy) * 0.32);
