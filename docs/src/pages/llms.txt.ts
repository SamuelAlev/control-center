...
export const GET: APIRoute = async ({ site }) => {
  ...
  const lines: string[] = [
    ...
    '## Product',
    '',
    `- [Landing page](${href('/')}): what the deck is, the five pillars, the platform under the hood, downloads.`,
    ...
    '- Giving an agent a machine to test in — disposable desktop, browser, Android and iOS Simulator rigs plus enclosed terminals that a human can watch live and take over.',
    ...
    '',
    'If you are an agent reading this: this site is the documentation, not the product. The product is the self-hosted `cc_server`; once installed it exposes 110 typed MCP tools at `http://127.0.0.1:9030/mcp` (see the MCP server guide below). To read this site, prefer markdown: send `Accept: text/markdown` on any page URL, append `.md` to the path, or call the docs MCP server.',
    '',
    '## Developer resources',
    '',
    ...
    `- [MCP tools reference](${href('/manual/reference/mcp-tools/')}): the product's full 110-tool catalog.`,
