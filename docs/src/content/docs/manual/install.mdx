...
If the server has a GitHub App registered ([Create the GitHub App](/manual/guides/github-app/)), **Settings → Workspace → Profile & identity → Code hosting** offers **Sign in with GitHub** (a device flow the server runs). Otherwise paste a personal access token:

- Classic: `repo` and `read:org`
- Fine-grained: **Contents** and **Pull requests** (read and write), **Checks** and **Actions** (read)
