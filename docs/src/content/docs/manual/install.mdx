...
- **macOS**: Download the latest `.dmg` from [GitHub Releases](https://github.com/SamuelAlev/control-center/releases), open it, and drag Control Center to Applications. The build is signed with a Developer ID and notarized by Apple, so it opens normally.
...
Control Center dispatches agents through **adapters**. The **built-in harness** adapter runs agents in-process with no external CLI — you authenticate providers in-app (browser OAuth for Anthropic, OpenAI, and Kimi Code, or API keys) under **Settings → Adapters**.

Install an external CLI only if you want to use its adapter:
...
Model choice can be governed by per-workspace provider policies built on the [models.dev](https://models.dev) catalog — see [Manage adapters and models](/manual/guides/adapters/).
...
2. Open **Settings → Adapters** to verify the built-in harness is ready and any agent CLIs were detected.
