...
| Measure                                                                   | Count |
| ------------------------------------------------------------------------- | ----- |
| Tools registered by `cc_server`                                           | 110   |
| From `packages/cc_mcp`                                                    | 108   |
| From `packages/cc_mcp_client` (tool discovery)                            | 2     |
| Registered in the base registry (`server_mcp_registry.dart`)              | 63    |
| Registered post-boot as their services come up (`cc_server_runtime.dart`) | 47    |
| Of the base registry's 63, conditional on a code-graph repository         | 5     |

The five code-graph tools are registered only when the host passes a code-graph repository to `buildServerMcpRegistry`. `cc_server` always passes one, so all 110 are live in the shipped binary.

The full registry is advertised in `tools/list` with no discovery gating. Ticket edits are typed tools.
...
:::note[Every tool class is registered]
A source file under `packages/cc_mcp/lib/src/tools/` is a tool an agent can
call. `packages/cc_server_core/test/mcp_tool_registration_ratchet_test.dart`
fails the build if a new tool class is constructed by nothing.
:::
...
**Required** on every tool that touches workspace-scoped data. A missing or invalid `workspace_id` returns an explicit error and a value that does not match the target entity's workspace is rejected rather than silently ignored. Omit it on the genuinely global tools: `list_workspaces` and the six newsfeed tools (`list_feeds`, `list_articles`, `get_article`, `set_article_read`, `set_article_saved`, `refresh_feeds`), which ride the server owner's per-user feed list. Discovery tools (`search_tool_bm25`, `list_my_tools`) take it optionally so they can classify callability by conversation mode. When an agent is dispatched by the server, the workspace is forced server-side, so the agent cannot reach another workspace by passing a foreign id.
...
Registered only when the host supplies a code-graph repository. The index covers the shipped grammar set (Dart, JS/TS, PHP, Python, Rust, Zig, C/C++, Go, Java, Ruby, C#, Swift, Kotlin, R, Assembly, MATLAB, Ada); symbols in any other language are not present.
...
Drive disposable machines — an enclosed browser or Linux desktop, an Android
emulator, or an iOS Simulator — from inside a run. Registered post-boot,
unconditionally: whether a machine can actually boot is a probe question the
tools answer per call. Repeated calls in one conversation reuse one machine;
screenshots return as image content with a one-image budget per result, and
extracted page/UI content is fenced as untrusted data. The four use tools plus
`rig_close` declare the `enclosureControl` [action class](/manual/concepts/guardrails/)
(the use tools also declare `networkEgress` and `processSpawn`; `mobile_use` and
`ios_use` also declare `packageInstall`), so read-only modes deny them. `rig_list`
declares none — it only reads workspace state. The full verb vocabulary per surface is in the
...
| `mobile_use`   | Drive Android over ADB: inspect/input, screenshots, install/start/stop/reset/uninstall apps, deep links and argv-shaped device commands |
| `ios_use`      | Drive iOS Simulator through WebDriverAgent/CoreSimulator: inspect/input, screenshots, install/start/stop/uninstall apps, deep links and argv-shaped simulator processes |
| `rig_list`     | List a workspace's rigs with surface, status, display size and controller |
| `rig_close`    | Destroy a rig and discard its owned machine state |
...
Gating is enforced twice from one table (`ModeToolPolicy`): once in the MCP dispatcher for the Claude Code adapter and once in the built-in harness registry. A blocked call returns the reason.
