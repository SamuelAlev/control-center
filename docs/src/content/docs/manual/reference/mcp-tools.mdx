...
extracted page/UI content is fenced as untrusted data. All six declare the
`enclosureControl` [action class](/manual/concepts/guardrails/) (the use tools
also declare `networkEgress` and `processSpawn`), so read-only modes deny them.
