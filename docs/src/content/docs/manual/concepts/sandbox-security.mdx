...
per agent.** Two agents in the same channel get distinct overlay working
...
confirmation in a channel — approve this push, approve this delete — it was
...
entry point today: there is no doctor command in the client and the `doctor`
MCP tool is defined but never registered, so it does not appear in `tools/list`.
