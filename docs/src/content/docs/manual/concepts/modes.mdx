...
The allow-lists are not an inventory of what exists. They constrain only write-tier-and-above tools, so the built-in harness's read tier reaches tools no list names; and on the external-CLI path, where the allow-list is the whole enforcement, a tool it omits is refused whether or not the server has it.
:::
