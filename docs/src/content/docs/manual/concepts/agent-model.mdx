...
| `adapterId`             | Which runner the agent dispatches to. **Null falls back to the `pi` CLI**             |
...
Skills fetched from outside pass a fail-closed supply-chain scan between fetch and write — no skill content reaches disk or a prompt without a verdict. Manage them at Settings → Workspace → Skills.
...
An agent with **no** adapter falls back to the `pi` CLI. Every seeded agent is created without one, so a fresh workspace's five agents all dispatch to `pi` until you set an adapter and model per agent. The onboarding "default adapter" choice is a client-side preference the server never reads and it does not retro-apply to already-created agents.
...
`hire_agent`, `fire_agent`, `propose_hire` and `update_agent` exist as MCP tool implementations but are not registered on the server's tool surface, so an agent cannot call them. Asking the CEO to hire someone will not create an agent. Likewise, importing agents by scanning `AGENTS.md` files found on disk exists as a provider with no screen behind it.
:::
