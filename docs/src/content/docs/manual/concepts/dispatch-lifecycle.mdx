...
- **Mode prompt**: a mode-specific system prompt (chat, plan, review, or orchestrate)
- **Channel context**: recent messages from the channel, compacted to fit context limits
...
Each channel gets its own working root. The source checkout is never mutated.
...
- Observability aggregation (activity log, budget ledger)
...
- Carries over the channel context so the agent doesn't start from scratch
...
- [Modes](/manual/concepts/modes/): how mode affects the prompt
