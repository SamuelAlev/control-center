...
| `status` | Lifecycle state (`backlog`, `open`, `inProgress`, `blocked`, `inReview`, `done`, `failed`, `cancelled`) |
...
failed     — reachable from inProgress or inReview
cancelled  — reachable from any non-terminal state

done / failed / cancelled are terminal — no outgoing transitions.
```
...
Terminal states are dead ends: a finished ticket is never reopened in place.
...
5. Transitions the ticket from `open` to `inProgress`
