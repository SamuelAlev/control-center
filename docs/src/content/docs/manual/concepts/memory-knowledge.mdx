...
Memory is the workspace-scoped knowledge store that gives agents persistent context across runs. Without memory, every agent channel starts from zero, and the agent doesn't remember conventions, decisions, or past work.
...
2. Searches for relevant facts based on the channel context
