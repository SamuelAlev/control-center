...
Memory is the workspace-scoped knowledge store that gives agents persistent context across runs. Without memory, every agent conversation starts from zero, and the agent doesn't remember conventions, decisions, or past work.
