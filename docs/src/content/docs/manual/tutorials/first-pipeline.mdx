...
Pipelines are DAG-based workflows that chain steps together. Each step has a **kind** that determines when the engine schedules it — `trigger` (the entry point), `listen` (fires when its upstream steps complete), `join` (waits for parallel branches), `router` (conditional branching), `forEach` (map/fan-out over a collection), or `terminal`. The work a step actually does comes from its **body**: for example, the `promptAgent` body dispatches an agent with a prompt, and the `bashScript` body runs a shell script. Steps pass data to downstream steps via state.
...
2. Select **Custom prompt** from the palette — a generic prompt-driven node routed through the `promptAgent` body
3. Configure it:
...
1. Add another step: the **Bash script** palette node, which runs through the `bashScript` body
2. Configure it:
