...
- **Channels**: empty for now, but this is where agent channels live
...
This is enforced at the database level: each workspace gets its own database file — one workspace, one file — so there is no shared table another workspace's agent could read. Cross-workspace access raises `WorkspaceMismatchException`.
