...
The **Linked accounts** row on the provider block lists every linked member in the workspace, with the Slack account each one maps to. It shows what your next step is at a glance: if you are not linked, the row above says so.

## Unlink

Click **Unlink** on a row.
