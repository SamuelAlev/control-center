...
- To take or list an **install snapshot**: the install's **owner**. The same gate applies to downloading the whole snapshot as a zip.
- For per-workspace actions: **admin** of that workspace to export it, **owner** to import or delete it.
...
There is no CLI subcommand and the snapshot RPC ops need a paired full client that is the install owner, so the practical
...
| "The server refused"                                                     | Role. Taking or listing an install snapshot needs the install's operator. Downloading a workspace needs admin, restoring one needs owner, a whole snapshot zip needs the install's operator |
