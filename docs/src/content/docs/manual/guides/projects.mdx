...
A `Project` is a workspace-scoped grouping of tickets toward a shared goal. Projects are Control Center-only, so they never sync to external providers like Linear.
