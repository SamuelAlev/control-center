...
There is no screen for access grants — the memory screen has Facts, Policies and Graph tabs and nothing else. The repository is bound on the client, but no widget reads it, so grants are reachable only over the `memory_access_grant.*` RPC ops. You cannot restrict who reads a memory domain from the app today.
:::
