...
1. Open **Settings → Workspace → Profile & identity** (`/settings/workspace/profile`).
...
| ------------- | --------------------------------------------------------- | --------------------------------------------------------- |
| **GitHub**    | Classic: `repo` and `read:org`. Fine-grained: Contents + Pull requests (read/write), Checks + Actions (read) | github.com → Settings → Developer settings → Tokens |
...
You have no credential for that forge. Sign in or paste a token in **Settings → Workspace → Profile & identity → Code hosting**. Other forges' repos are unaffected.
