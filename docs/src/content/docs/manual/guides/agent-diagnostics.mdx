...
Earlier documentation described an "Agent doctor" run from an agent's context menu. There is no such menu item, no diagnostics screen for an agent and no `doctor` MCP tool — the tool class was deleted, so an agent calling it gets "unknown tool". Nothing in the product produces a per-agent diagnostic report. Work through the checks below instead.
:::
