    ...
    changes: [
      {
        ...
        items: [
          ...
          "<b>OS-native sandboxing.</b> Agents run under macOS Seatbelt or Linux bubblewrap with filesystem allow-lists and network egress controls and per-channel capabilities (git push, GitHub API, ticketing, network) gate credential injection so a token is minted only when you explicitly enable it. Where neither sandbox backend is available, agents run unsandboxed.",
          "<b>Workspace isolation.</b> Workspaces are hard-boundary tenants that auto-seed a CEO agent plus specialists; registered repos provision copy-on-write worktrees per channel via rift FFI (with git-worktree fallback) and every workspace-scoped query is filtered so one tenant's data never surfaces in another.",
          ...
          "<b>Autonomy dial and unified guardrails.</b> Every channel carries a named autonomy profile — propose-only, act-with-approval or act-freely — over one policy store covering ~12 action classes (git push, PR create, network egress, secret access, package install, …). Resolution is most-specific-scope-wins (channel > agent > workspace > mode preset) and a prompt with no approver connected is denied, fail-closed.",
          "<b>Agent-to-agent collaboration.</b> Agents talk over the same durable channels you do — never a separate bus — with <code>send_to_agent</code>, <code>ask_agent</code> (request/reply with a mandatory timeout and pairwise cycle detection), <code>delegate_task</code> (child tickets guarded by depth cap, cycle detection, budget-envelope inheritance and an autonomy ceiling) and <code>todo_read</code>. Agent-to-agent channels are muted by default and never bump your unread badge.",
