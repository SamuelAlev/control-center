...
- **`WorkspaceDatabase`** (`lib/database/workspace/workspace_database.dart`) → `<dataDir>/workspaces/<workspaceId>.db`. Holds *everything else* — agents, channels, tickets, memory, pipelines, meetings, the code graph, reviews, **repos**. One file per workspace, opened lazily on first touch.
...
- **Backup is a directory, not a file:** `backups/<ts>/{manifest.json, global.db, workspaces/<id>.db}`, each written with `VACUUM INTO`. `workspace.export` / `workspace.import` hand a single workspace around as one file.
