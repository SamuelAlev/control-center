...
if [ -z "$TEAM_ID" ]; then
  # Resolve the Team ID from a signing cert's OU field — NOT the name in
  # parentheses. For "Apple Development"/"Apple Distribution" certs the
  # parenthetical is your personal developer id (e.g. 6WP3K6F5YJ), which is
  # NOT a team; the team always lives in the certificate's OU (e.g. L3C7R68G6X).
  # Reading the parenthetical is the classic foot-gun here.
  team_of() {
    local cn
    cn="$(security find-identity -v -p codesigning 2>/dev/null \
          | sed -nE "s/^[[:space:]]*[0-9]+\) [0-9A-Fa-f]+ \"($1[^\"]*)\".*/\1/p" | head -1)"
    [ -n "$cn" ] || return 0
    security find-certificate -c "$cn" -p 2>/dev/null \
      | openssl x509 -noout -subject 2>/dev/null \
      | sed -nE 's/.*OU[[:space:]]*=[[:space:]]*([A-Z0-9]{10}).*/\1/p' | head -1
  }
