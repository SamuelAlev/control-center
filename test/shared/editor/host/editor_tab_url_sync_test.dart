...
void main() {
  ...
  group('locationWithEditorTab', () {
    test('sets the tab param on a bare location', () {
      expect(
        locationWithEditorTab(
          Uri.parse('/workspaces/w1/channels/c1'),
          'chat:c1',
        ),
        '/workspaces/w1/channels/c1?tab=chat%3Ac1',
      );
    });
    ...
    test('replaces an existing tab param and preserves the others', () {
      expect(
        locationWithEditorTab(
          Uri.parse('/workspaces/w1/channels/c1?m=m9&tab=chat%3Ac1'),
          'terminal',
        ),
        '/workspaces/w1/channels/c1?m=m9&tab=terminal',
      );
    });
    ...
    test('removes the tab param (null key) without a dangling ?', () {
      expect(
        locationWithEditorTab(
          Uri.parse('/workspaces/w1/channels/c1?tab=chat%3Ac1'),
          null,
        ),
        '/workspaces/w1/channels/c1',
      );
    });
    ...
    test('keeps other params when removing the tab param', () {
      expect(
        ...
        '/workspaces/w1/channels/c1?m=m9',
      );
    });
  ...
  group('EditorTabUrlTracker', () {
    ...
    test('force apply(null) resyncs to a replaced tab set', () {
      // Conversation switch: the tracker still holds channel A's key while
      // the layout now focuses channel B's chat tab.
