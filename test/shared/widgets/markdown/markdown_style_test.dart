...
Widget _buildTestApp(WidgetBuilder builder, {String? fontFamily}) {
  return MaterialApp(
    ...
    home: CcTheme(
      data: CcThemeData.light(fontFamily: fontFamily),
...
void main() {
  ...
  group('appMarkdownStyle', () {
    ...
    // Regression: the app ALWAYS names a UI family on the theme — the font
    // setting defaults to the bundled Manrope rather than to null — so this,
    // not the bare `CcThemeData.light()` above, is the path chat renders on.
    // Resolving the bundled family through `CcFontRegistry` named a per-weight
    // variant no `FontLoader` ever registers, and the emoji fallback below
    // replaced the list that still held the real family. Nothing in the chain
    // resolved, so spaces and digits (which Apple Color Emoji covers, for
    // keycap sequences) shaped in the emoji font at emoji advances and letters
    // fell through to the system default: mixed typefaces, gaping word gaps.
    testWidgets('the theme font family resolves to a registered family', (
      tester,
    ) async {
      await tester.pumpWidget(
        _buildTestApp(fontFamily: AppFonts.uiFamily, (context) {
          final style = appMarkdownStyle(context);
          expect(style.paragraph?.fontFamily, AppFonts.uiFamily);
          expect(style.tableBody?.fontFamily, AppFonts.uiFamily);
          expect(style.tableHead?.fontFamily, AppFonts.uiFamily);
          expect(style.h1?.fontFamily, AppFonts.uiFamily);
          return const SizedBox.shrink();
        }),
      );
    });

    testWidgets('the emoji font is appended to the fallback, not swapped in', (
      tester,
    ) async {
      await tester.pumpWidget(
        ...
        _buildTestApp(fontFamily: 'Inter', (context) {
          ...
          expect(
            fallback,
            contains(AppFonts.uiFamily),
            reason: 'the bundled family renders while the selection loads',
          );
          expect(
            fallback!.indexOf(AppFonts.uiFamily),
            lessThan(fallback.indexOf(AppFonts.emojiFallback.first)),
            reason: 'a real text family must be reached before the emoji font',
          );
          return const SizedBox.shrink();
        }),
      );
