  ...
  group('WorkspaceAvatar', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        // `cacheWidth` wraps the provider in a [ResizeImage], which is the
        // point: `width`/`height` bind the PAINT size, so without it a 1024px
        // logo is decoded and cached at full size to be drawn into a 48px
        // disc. Assert the cap is applied, then unwrap to check the URL.
        expect(provider, isA<ResizeImage>());
        final resize = provider as ResizeImage;
        expect(
          resize.width,
          isNotNull,
          reason: 'the decode must be capped, not just the paint',
        );
        expect(
          resize.width,
          lessThanOrEqualTo(48 * 4),
          reason: 'the cap is the laid-out size x DPR, not the source size',
        );

        final networkImage = resize.imageProvider;
