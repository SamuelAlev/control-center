...
void main() {
  group('AgentResponderResolver.resolveDefault', () {
    test('returns null for empty agents list', () {
      expect(AgentResponderResolver.resolveDefault(agents: []), isNull);
    });

    group('single-agent channel', () {
      test('returns the sole agent participant', () {
        final result = AgentResponderResolver.resolveDefault(agents: [_ceo]);
