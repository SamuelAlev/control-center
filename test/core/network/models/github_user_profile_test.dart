...
void main() {
  group('GitHubContributionDay', () {
    ...
    test('toJson round-trip', timeout: const Timeout.factor(2), () {
  ...
  group('GitHubOrganization', () {
    ...
    test('fromJson handles null fields', timeout: const Timeout.factor(2), () {
  ...
  group('GitHubUserProfile', () {
        ...
        'nodes': [
          {
            ...
            // Mirrors the real selection:
            // `teams(first: 10, userLogins: [$login]) { nodes { name, slug } }`
            // (see GitHubGraphqlClient). Team membership is only reachable
            // through this nested query, never from the REST profile.
            'teams': {
              'nodes': [
                {'name': 'core', 'slug': 'core'},
                {'name': 'reviewers', 'slug': 'reviewers'},
              ],
            },
          },
          {
            ...
            'teams': {
              'nodes': [
                // Deliberately repeats `core` from the org above — orgTeams is
                // de-duplicated across organizations.
                {'name': 'core', 'slug': 'core'},
                {'name': 'maintainers', 'slug': 'maintainers'},
              ],
            },
          },
        ],
    ...
    test('fromJson parses status', timeout: const Timeout.factor(2), () {
      final profile = GitHubUserProfile.fromJson(fullJson);
    ...
    test('fromJson parses organizations', timeout: const Timeout.factor(2), () {
      final profile = GitHubUserProfile.fromJson(fullJson);
    ...
    test(
      'fromJson flattens + de-duplicates team names across organizations',
      timeout: const Timeout.factor(2),
      () {
        final profile = GitHubUserProfile.fromJson(fullJson);

        // Team NAMES (not org logins), flattened across both orgs, with the
        // duplicate `core` collapsed.
        expect(profile.orgTeams, ['core', 'reviewers', 'maintainers']);
      },
    );

    test(
      'fromJson yields no teams when the orgs carry no teams selection',
      timeout: const Timeout.factor(2),
      () {
        // A token without `read:org` gets organizations but no `teams` node —
        // that must read as "no teams", never as an error.
        final json = Map<String, dynamic>.of(fullJson);
        json['organizations'] = {
          'nodes': [
            {'login': 'github', 'name': 'GitHub', 'avatarUrl': '', 'url': ''},
          ],
        };

        final profile = GitHubUserProfile.fromJson(json);

        expect(profile.organizations.single.login, 'github');
