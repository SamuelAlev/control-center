...
void main() {
  group('GitHubPrReviewState', () {
    test('constructor holds all fields', timeout: const Timeout.factor(2), () {
      const state = GitHubPrReviewState(
        ...
        completedReviews: [
          GitHubCompletedReview(
            ...
            state: 'APPROVED',
            onBehalfOf: [GitHubReviewTeamRef(name: 'Backend', slug: 'backend')],
          ),
        ],
      );
    ...
    test('defaults to empty lists', timeout: const Timeout.factor(2), () {
      const state = GitHubPrReviewState();
    ...
    test('holds isDraft', timeout: const Timeout.factor(2), () {
      const state = GitHubPrReviewState(isDraft: true, prState: 'OPEN');
  ...
  group('GitHubPendingUserRequest', () {
    test('holds all fields', timeout: const Timeout.factor(2), () {
    ...
    test('asCodeOwner can be true', timeout: const Timeout.factor(2), () {
      const req = GitHubPendingUserRequest(
        login: 'user1',
        avatarUrl: 'https://example.com/avatar.png',
        asCodeOwner: true,
  ...
  group('GitHubReviewTeamRef', () {
    test('holds name and slug', timeout: const Timeout.factor(2), () {
      const ref = GitHubReviewTeamRef(name: 'Design', slug: 'design');
      expect(ref.name, 'Design');
      expect(ref.slug, 'design');
    });

    test('supports const construction', timeout: const Timeout.factor(2), () {
