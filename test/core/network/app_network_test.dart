...
void main() {
  group('createDio', () {
    ...
    test('sets default headers', () {
      final dio = createDio();
      // Accept is set, but Content-Type is intentionally NOT defaulted:
      // forcing `application/json` onto bodiless GETs makes strict servers
      // (e.g. Cloudflare-fronted RSS feeds) reject with 415. Dio's transformer
      // still sets it automatically for POST/PUT/PATCH with a Map/List body.
      expect(dio.options.headers['Accept'], 'application/json');
      expect(dio.options.headers['Content-Type'], isNull);
    });
