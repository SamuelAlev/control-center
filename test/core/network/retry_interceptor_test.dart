...
void main() {
  group('RetryInterceptor', () {
    ...
    test('does not retry on 404', timeout: const Timeout.factor(2), () async {
      final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
      dio.interceptors.add(
        RetryInterceptor(dio: dio, maxRetries: 3, baseDelay: Duration.zero),
      );
    ...
    test('does not retry on 403', timeout: const Timeout.factor(2), () async {
      final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
      dio.interceptors.add(
        RetryInterceptor(dio: dio, maxRetries: 3, baseDelay: Duration.zero),
      );
    ...
    test(
      ...
      () async {
        ...
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.interceptors.add(
          RetryInterceptor(dio: dio, maxRetries: 1, baseDelay: Duration.zero),
        );
        dio.httpClientAdapter = _Adapter((options) {
          attemptCount++;
