...
void main() {
  ...
  const allowedUndoable = <String, String>{
    ...
    // A review finding's status. The op returns `previous_status`, so the
    // inverse restores what was actually there rather than assuming `open` —
    // undoing a reopen must not silently mark the finding fixed. Recorded by
    // `review_accordion_item.dart`, which is what makes `reversible` a fact
    // rather than a label.
    'pr_review.setFindingStatus': 'reversible',
  };
  ...
  test(
    ...
    () {
      ...
      final externalPattern = RegExp(
        r'(\.publish|mergePullRequest|submitReview|\.syncNow)',
      );
      for (final e in declared.entries) {
