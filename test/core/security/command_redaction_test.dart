...
void main() {
  group('redactSecrets', () {
    test(
      ...
      () {
        const input =
            'curl --api-key=sk_live_abcdef1234567890 https://api.example.com';
    ...
    test('redacts --token flag', timeout: const Timeout.factor(2), () {
    ...
    test('redacts --key flag', timeout: const Timeout.factor(2), () {
    ...
    test(
      'redacts GitHub App / OAuth / refresh tokens (ghs_, ghu_, ghr_)',
      timeout: const Timeout.factor(2),
      () {
        const tokens = [
        ...
        for (final token in tokens) {
          final result = redactSecrets('Authorization: Bearer $token');
          expect(result, isNot(contains(token)), reason: token);
          expect(result, contains('***REDACTED***'));
        }
      },
    );
    ...
    test(
      'redacts space-separated --token VALUE (VULN-012)',
      timeout: const Timeout.factor(2),
      () {
        const input =
        ...
        expect(
          result,
          isNot(contains('ghp_1234567890abcdef1234567890abcdef1234')),
        );
      },
    );

    test(
      'redacts a PAT embedded in a git https URL (VULN-010)',
      timeout: const Timeout.factor(2),
      () {
        const line =
            'fatal: unable to access '
            "'https://x-access-token:ghp_1234567890abcdef1234567890abcdef1234"
            "@github.com/owner/repo.git': Could not resolve host";
        final result = redactSecrets(line);
        expect(result, contains('***REDACTED***'));
        expect(
          result,
          isNot(contains('ghp_1234567890abcdef1234567890abcdef1234')),
        );
        // The clean host / path survive the redaction.
        expect(result, contains('github.com/owner/repo.git'));
      },
    );
    ...
    test(
      ...
      () {
        const input =
            'git clone https://github.com/owner/repo.git --branch main';
    ...
    test('handles empty string', timeout: const Timeout.factor(2), () {
    ...
    test(
      ...
      timeout: const Timeout.factor(2),
      () {
        expect(redactSecrets('--API_KEY=secret'), contains('***REDACTED***'));
        expect(redactSecrets('--Token=abc'), contains('***REDACTED***'));
      },
    );
  });
  ...
  group('redactSecretsFromJson', () {
    ...
    test('redacts token keys', timeout: const Timeout.factor(2), () {
    ...
    test('redacts secret keys', timeout: const Timeout.factor(2), () {
    ...
    test('redacts password keys', timeout: const Timeout.factor(2), () {
    ...
    test('redacts credential keys', timeout: const Timeout.factor(2), () {
    ...
    test('redacts auth keys', timeout: const Timeout.factor(2), () {
    ...
    test('handles nested JSON objects', timeout: const Timeout.factor(2), () {
    ...
    test('handles arrays in JSON', timeout: const Timeout.factor(2), () {
