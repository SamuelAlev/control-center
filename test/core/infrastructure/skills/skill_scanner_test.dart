...
void main() {
  ...
  group('SkillScanner', () {
    ...
    test('scan returns empty list for non-existent directory', () async {
      ...
      final results = await scanner.scan('${tmpRoot.path}/does-not-exist');
    ...
    test('scanned skill has correct framework', () async {
      ...
      final claudeSkills = results
          .where((s) => s.framework == 'claude')
          .toList();
      final cursorSkills = results
          .where((s) => s.framework == 'cursor')
          .toList();
    ...
    test('scanned skill parses quoted frontmatter values', () async {
      ...
      final quoted = results.firstWhere((s) => s.slug == 'quoted-skill');
    ...
    test(
      'files directly in scan root are ignored (must be subdirectory)',
      () async {
    ...
    test(
      'skips skill directory where SKILL.md has no closing frontmatter',
      () async {
        final dir = Directory.systemTemp.createTempSync(
          'skill_scanner_unclosed_',
        );
        try {
          final skillsDir = Directory(
            '${dir.path}/.claude/skills/broken-skill',
          );
          skillsDir.createSync(recursive: true);
    ...
    test(
      'frontmatter line with only a colon yields empty key and value',
      () async {
        final dir = Directory.systemTemp.createTempSync(
          'skill_scanner_barecolon_',
        );
        try {
