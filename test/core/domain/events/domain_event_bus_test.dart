  ...
  group('DomainEventBus', () {
    ...
    test(
      ...
      () async {
        ...
        bus.on<MemoryEvent>().listen(received.add);
        ...
        expect(received[0], isA<MemoryFactRecorded>());
        expect(received[1], isA<MemoryFactUpdated>());
      },
    );
    ...
    test(
      ...
      timeout: const Timeout.factor(2),
      () async {
        final exact = <MemoryFactRecorded>[];
        final base = <MemoryEvent>[];
        final all = <DomainEvent>[];
        bus.on<MemoryFactRecorded>().listen(exact.add);
        bus.on<MemoryEvent>().listen(base.add);
