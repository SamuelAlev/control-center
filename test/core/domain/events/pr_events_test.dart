...
void main() {
  ...
  group('PullRequestStatusChanged', () {
    ...
    test(
      'carries a workspace; the forge coordinates stay optional',
      timeout: const Timeout.factor(2),
      () {
        // `workspaceId` moved from optional-and-nullable to required-and-
        // non-null: `pull_requests` rows live in a per-workspace database
        // file, so a status change that could not name its workspace was one
        // no listener could route. The forge coordinates are genuinely
        // optional — a status change detected from a webhook may carry the
        // internal draft id, the `owner/name` + number pair, or neither.
        final event = PullRequestStatusChanged(
          status: 'closed',
          occurredAt: now,
          workspaceId: 'ws-1',
  ...
  group('PR events on bus', () {
    test(
      'each PR event type filters independently',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'PrMerged does not appear on PullRequestPublished stream',
      timeout: const Timeout.factor(2),
      () async {
        final bus = DomainEventBus();
        addTearDown(bus.dispose);
