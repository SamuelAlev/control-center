...
void main() {
  ...
  group('ChannelDeleted', () {
    test('constructs with all fields', timeout: const Timeout.factor(2), () {
      final now = DateTime(2026, 5, 18);
      final event = ChannelDeleted(
        channelId: 'ch-1',
        workspaceId: 'ws-1',
        occurredAt: now,
      );
    ...
    test(
      'carries the workspace that owned the channel',
      timeout: const Timeout.factor(2),
      () {
        ...
        // channel-id lookup ACROSS workspaces, and both publishers always had
        // the id in hand.
        final event = ChannelDeleted(
          channelId: 'ch-1',
          workspaceId: 'ws-1',
    ...
    test('type filtering on bus', timeout: const Timeout.factor(2), () async {
      ...
      final received = <ChannelDeleted>[];
      bus.on<ChannelDeleted>().listen(received.add);
