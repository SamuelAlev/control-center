...
void main() {
  ...
  group('TicketAssigned', () {
    ...
    test('supports all nullable fields', timeout: const Timeout.factor(2), () {
      ...
      expect(event.assignedTeamId, isNull);
      // The workspace is the scope every listener reads the ticket back in, so
      // it is required: an unscoped assignment would be undispatchable.
      expect(event.workspaceId, 'ws-1');
    });
  ...
  group('TicketReassigned', () {
    ...
    test(
      'supports nullable agent fields',
  ...
  group('TicketCollaboratorAdded', () {
    ...
    test(
      'user as collaborator uses literal string',
      timeout: const Timeout.factor(2),
      () {
        final event = TicketCollaboratorAdded(
          ticketId: 't-1',
  ...
  group('Ticketing events on bus', () {
    test(
      'terminal events filter independently',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'TicketCreated does not appear on TicketCompleted stream',
      timeout: const Timeout.factor(2),
      () async {
        final bus = DomainEventBus();
        addTearDown(bus.dispose);
        ...
        bus.publish(
          TicketCreated(ticketId: 't-1', workspaceId: 'ws-1', occurredAt: now),
        );
    ...
    test(
      'TicketAssigned carries all payload fields through bus',
      timeout: const Timeout.factor(2),
      () async {
        final bus = DomainEventBus();
        addTearDown(bus.dispose);
    ...
    test(
      'TicketStatusChanged tracks before/after',
      timeout: const Timeout.factor(2),
      () async {
        final bus = DomainEventBus();
        addTearDown(bus.dispose);
