...
void main() {
  group('AgentSkills', () {
    group('constructor', () {
      ...
      test('modifying source list does not affect instance', () {
        final source = <String>['coding', 'reviewing'];
        final skills = AgentSkills(source);
        source.add('testing');
        source.clear();
        expect(skills.toList(), ['coding', 'reviewing']);
      });
    });
    ...
    group('== and hashCode', () {
      ...
      test(
        'hashCode differs for different order (order-dependent implementation)',
        () {
          final s1 = AgentSkills(['coding', 'reviewing']);
          final s2 = AgentSkills(['reviewing', 'coding']);
          expect(s1, equals(s2));
