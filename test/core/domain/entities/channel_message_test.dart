...
void main() {
  ...
  group('ChannelMessage', () {
    ...
    group('convenience getters', () {
      ...
      test('isThinking', () {
        final msg = createMessage(messageType: ChannelMessageType.thinking);
    ...
    group('thinkingEvents', () {
      test('returns empty list when metadata is null', () {
        final msg = createMessage();
        expect(msg.thinkingEvents, isEmpty);
      });

      test('returns empty list when events is not a list', () {
        final msg = createMessage(metadata: {'events': 'not a list'});
        expect(msg.thinkingEvents, isEmpty);
      });

      test('decodes thinking events from metadata', () {
        ...
        expect(msg.thinkingEvents, hasLength(1));
        expect(msg.thinkingEvents.first.kind, ThinkingEventKind.reasoning);
      });

      test('skips non-map entries in events list', () {
        final msg = createMessage(metadata: {
          'events': ['not a map', 42],
        });
        expect(msg.thinkingEvents, isEmpty);
