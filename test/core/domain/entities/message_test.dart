...
void main() {
  ...
  group('MessageMention', () {
    ...
    group('principalType (PRD 16 §15)', () {
      test('defaults to agent', () {
        const m = MessageMention(agentId: 'a1', raw: '@agent-1');
        expect(m.principalType, PrincipalType.agent);
        expect(m.principalId, 'a1');
      });

      test('toJson omits principalType when it is the default agent', () {
        const m = MessageMention(agentId: 'a1', raw: '@agent-1');
        expect(m.toJson().containsKey('principalType'), isFalse);
      });

      test('a human mention round-trips through toJson/fromJson', () {
        const m = MessageMention(
          agentId: 'u1',
          raw: '@ada',
        ...
        final json = m.toJson();
        expect(json['principalType'], 'user');
        final decoded = MessageMention.fromJson(json);
        expect(decoded.principalType, PrincipalType.user);
        expect(decoded.principalId, 'u1');
        expect(decoded, m);
      });

      test('fromJson is tolerant of a frame with no principalType (defaults '
          'to agent)', () {
        final m = MessageMention.fromJson({'agentId': 'a1', 'raw': '@agent-1'});
        expect(m.principalType, PrincipalType.agent);
      });

      test('fromJson defaults to agent for an unknown principalType value', () {
        final m = MessageMention.fromJson({
          'agentId': 'a1',
          'raw': '@agent-1',
          'principalType': 'robot',
        });
      ...
      test('mentions with different principalType are not equal', () {
        const a = MessageMention(agentId: 'x1', raw: '@x');
        const b = MessageMention(
          agentId: 'x1',
          raw: '@x',
          principalType: PrincipalType.user,
        );
  ...
  group('SenderType', () {
    test('has user and agent', () {
      expect(
        SenderType.values,
        containsAll([SenderType.user, SenderType.agent]),
      );
    });
  });
  ...
  group('Message', () {
    group('constructor', () {
      test('creates with required fields', () {
        ...
        expect(msg.senderType, SenderType.user);
      ...
      test('creates with all optional fields', () {
        final msg = createMessage(metadata: {'key': 'value'}, compacted: true);
    ...
    group('convenience getters', () {
      ...
      test('isReviewNode', () {
        final msg = createMessage(messageType: MessageType.reviewNode);
        expect(msg.isReviewNode, isTrue);
      });

      test('isHireProposal', () {
        final msg = createMessage(messageType: MessageType.hireProposal);
      ...
      test('isPlan', () {
        final msg = createMessage(messageType: MessageType.plan);
        expect(msg.isPlan, isTrue);
      });

      test('isUserQuestion', () {
        final msg = createMessage(messageType: MessageType.userQuestion);
        expect(msg.isUserQuestion, isTrue);
      });

      test(
        'isQuestionAnswered returns true when metadata answered is true',
        () {
    ...
    group('== and hashCode', () {
      ...
      test('== returns false for different spaceId', () {
        final a = createMessage(spaceId: 'ch-1');
        final b = createMessage(spaceId: 'ch-2');
        expect(a, isNot(equals(b)));
      });
      ...
      test('== returns false for different senderType', () {
        final a = createMessage(senderType: SenderType.user);
        final b = createMessage(senderType: SenderType.agent);
        expect(a, isNot(equals(b)));
      });
      ...
      test('== returns false for different messageType', () {
        final a = createMessage(messageType: MessageType.text);
        final b = createMessage(messageType: MessageType.system);
        expect(a, isNot(equals(b)));
      });
      ...
      test('== returns false for different conversationId', () {
        final a = createMessage(conversationId: 'cv1');
        final b = createMessage(conversationId: 'cv2');
        expect(a, isNot(equals(b)));
      });
      ...
      test('== returns false for non-Message', () {
        final msg = createMessage();
    ...
    group('copyWith', () {
      ...
      test('updates id', () {
        ...
        expect(copy.spaceId, msg.spaceId);
      });

      test('updates spaceId', () {
        final msg = createMessage();
        final copy = msg.copyWith(spaceId: 'new-ch');
      ...
      test('updates senderType', () {
        final msg = createMessage();
        final copy = msg.copyWith(senderType: SenderType.agent);
        expect(copy.senderType, SenderType.agent);
      });
      ...
      test('updates messageType', () {
        final msg = createMessage();
        final copy = msg.copyWith(messageType: MessageType.system);
        expect(copy.messageType, MessageType.system);
      });
      ...
      test('updates conversationId', () {
        final msg = createMessage();
        final copy = msg.copyWith(conversationId: 'cv-2');
        expect(copy.conversationId, 'cv-2');
      });
      ...
      test('chaining copyWith calls', () {
        final msg = createMessage();
        final copy = msg.copyWith(content: 'Updated').copyWith(compacted: true);
        expect(copy.content, 'Updated');
        expect(copy.compacted, isTrue);
      });
    });
