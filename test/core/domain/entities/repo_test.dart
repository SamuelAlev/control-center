...
void main() {
  ...
  Repo createRepo({
    ...
    String remoteOwner = 'my-org',
    String remoteName = 'my-repo',
    DateTime? createdAt,
    DateTime? updatedAt,
  }) {
    return Repo(
  ...
  group('Repo', () {
    group('constructor', () {
      test('creates repo with required fields', () {
        ...
        expect(repo.remoteOwner, 'org');
        expect(repo.remoteName, 'repo');
        expect(repo.createdAt, testCreatedAt);
        expect(repo.updatedAt, testUpdatedAt);
      });

      test('constructor rejects name is not empty', () {
        expect(
          () => Repo(
            id: 'r',
            name: '',
      ...
      test('constructor rejects path is not empty', () {
        expect(
          () => Repo(
            id: 'r',
            name: 'org/repo',
    ...
    group('getters', () {
      test('hasForgeRemote returns true when owner and repo are present', () {
        final repo = createRepo(remoteOwner: 'org', remoteName: 'repo');
        expect(repo.hasForgeRemote, isTrue);
      });

      test('hasForgeRemote returns false when remoteOwner is empty', () {
        final repo = createRepo(remoteOwner: '', remoteName: 'repo');
        expect(repo.hasForgeRemote, isFalse);
      });

      test('hasForgeRemote returns false when remoteName is empty', () {
        final repo = createRepo(remoteOwner: 'org', remoteName: '');
        expect(repo.hasForgeRemote, isFalse);
      });

      test('hasForgeRemote returns false when both are empty', () {
        final repo = createRepo(remoteOwner: '', remoteName: '');
        expect(repo.hasForgeRemote, isFalse);
      });
      ...
      test('fullName returns path when remoteOwner is missing', () {
        final repo = createRepo(
          remoteOwner: '',
          remoteName: 'repo',
          path: '/local/repo',
      ...
      test('fullName returns path when remoteName is missing', () {
        final repo = createRepo(
          remoteOwner: 'org',
          remoteName: '',
          path: '/local/repo',
    ...
    group('== and hashCode', () {
      ...
      test('== returns false for different id', () {
        expect(createRepo(id: 'a') == createRepo(id: 'b'), isFalse);
      });
    ...
    group('copyWith', () {
      test('returns identical copy with no arguments', () {
        ...
        expect(copy.remoteOwner, repo.remoteOwner);
        expect(copy.remoteName, repo.remoteName);
      ...
      test('updates remoteOwner', () {
        final copy = createRepo().copyWith(remoteOwner: 'new-org');
        expect(copy.remoteOwner, 'new-org');
      });

      test('updates remoteName', () {
        final copy = createRepo().copyWith(remoteName: 'new-repo');
        expect(copy.remoteName, 'new-repo');
      });
      ...
      test('copyWith preserves other fields unchanged', () {
        final repo = createRepo(id: 'repo-x', name: 'name-x', path: '/path-x');
