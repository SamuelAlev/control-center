...
void main() {
  ...
  group('ReviewChannelRepository', () {
    ...
    test('contract — watchByChannel', () async {
      ...
      final stream = repo.watchByChannel('ws-1', 'ch-2');
      final found = await stream.first;
      expect(found, isNotNull);
    ...
    test(
      'contract — watchAllByChannel returns every PR for a channel',
      () async {
        final repo = _TestReviewChannelRepository();
        await repo.create(
          channelId: 'ch-multi',
        ...
        // A different channel must not bleed in.
        await repo.create(
          channelId: 'ch-other',
          workspaceId: 'ws-1',
          prNodeId: 'pr-node-3',
        ...
        final list = await repo.watchAllByChannel('ws-1', 'ch-multi').first;
        expect(list.length, 2);
        expect(list.map((a) => a.prNodeId).toSet(), {'pr-node-1', 'pr-node-2'});
      },
    );

    test('contract — watchAllByChannel is workspace-scoped', () async {
      final repo = _TestReviewChannelRepository();
      await repo.create(
        channelId: 'ch-x',
        workspaceId: 'ws-1',
      ...
      // Same channel id, different workspace — must not be returned.
      final list = await repo.watchAllByChannel('ws-2', 'ch-x').first;
      expect(list, isEmpty);
    });
    ...
    test('contract — updateStatus', () async {
      ...
      expect(assoc.status, ReviewChannelStatus.requested);
      await repo.updateStatus('ws-1', assoc.id, ReviewChannelStatus.completed);

      final stream = repo.watchByChannel('ws-1', 'ch-3');
...
class _TestReviewChannelRepository implements ReviewChannelRepository {
  ...
  Stream<ReviewChannelAssociation?> watchByChannel(
    ...
    final found = _assocs.values.where(
      (a) => a.workspaceId == workspaceId && a.channelId == channelId,
    );
    return Stream.value(found.isNotEmpty ? found.first : null);
  }
