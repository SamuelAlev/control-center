...
void main() {
  ...
  group('AgentQuestionAnswer', () {
    ...
    test('toJson excludes empty string freeText', () {
      const answer = AgentQuestionAnswer(selectedLabels: ['B'], freeText: '');
      final json = answer.toJson();
      expect(json.containsKey('freeText'), isFalse);
    });
    ...
    test('toPromptString with only freeText', () {
      ...
      expect(
        ...
        'Additional input: use the feature branch',
      );
    });
  ...
  group('Port interface contracts', () {
    ...
    test('ModeResolver contract is valid', () async {
      final port = _TestModeResolver();
      final mode = await port.resolveForConversation('ws1', 'conv-1');
    ...
    test('RepoWorkspaceProvisionerPort contract is valid', () async {
      ...
      final dir = await port.ensureSpaceWorkspace(
        workspaceId: 'ws-1',
        spaceId: 'ch-1',
        agentSlug: 'dev',
...
class _TestModeResolver implements ModeResolver {
  @override
...
class _TestNotificationPreferencesPort implements NotificationPreferencesPort {
  @override
...
class _TestRepoIsolationPort implements RepoIsolationPort {
  ...
  Future<RepoIsolationResult> provision({
    ...
    bool pristine = false,
    CancellationToken? cancel,
  }) async => const RepoIsolationResult(
    path: '/tmp/isolated',
    backend: RepoIsolationBackend.rift,
  );
...
class _TestRepoWorkspaceProvisionerPort
  ...
  Future<String> ensureSpaceWorkspace({
    ...
    // Accepted to match the port; the contract fake runs no setup scripts.
    void Function(String repoName)? onRepoSetupScript,
    CancellationToken? cancel,
  }) async => '/tmp/conversation';
