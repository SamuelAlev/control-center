...
void main() {
  ...
  ToolSegment tool(
    String name, {
    ToolSegmentStatus status = ToolSegmentStatus.running,
  }) => ToolSegment(
    toolName: name,
    toolCallId: 'c',
    status: status,
  ...
  group('normalizeToolName', () {
    test('lowercases and strips mcp prefix', () {
      ...
      expect(
        normalizeToolName('mcp__control-center__create_ticket'),
        'create_ticket',
      );
  ...
  group('statusLineFor', () {
    ...
    test('running edit -> making edits', () {
      expect(
        statusLineFor([tool('Edit')])?.kind,
        TranscriptStatusKind.makingEdits,
      );
      expect(
        statusLineFor([tool('Write')])?.kind,
        TranscriptStatusKind.makingEdits,
      );
    });
    ...
    test('running read -> reading files', () {
      expect(
        statusLineFor([tool('Read')])?.kind,
        TranscriptStatusKind.readingFiles,
      );
    });

    test('running grep/glob/ls -> searching', () {
      expect(
        statusLineFor([tool('Grep')])?.kind,
        TranscriptStatusKind.searching,
      );
      expect(
        statusLineFor([tool('Glob')])?.kind,
        TranscriptStatusKind.searching,
      );
    ...
    test('completed tool -> null', () {
      expect(
        statusLineFor([tool('Read', status: ToolSegmentStatus.ok)]),
        isNull,
      );
    });
