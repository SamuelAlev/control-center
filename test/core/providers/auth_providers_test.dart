...
void main() {
  ...
  group('CredentialsNotifier', () {
    ...
    test('clearGitHubToken removes token from state', () async {
      ...
      expect(container2.read(credentialsProvider).requireValue.githubToken, '');
  ...
  group('isGitHubAuthenticatedProvider', () {
    ...
    test(
      'returns true when gh CLI is authenticated but token is empty',
      () async {
        // Mirrors the production RPC path: RpcGitHubCliPort.probe() returns
        // isAuthenticated: true but an empty token (the server never ships its
        // gh token to the client). The compose-PR gate must still see authed.
        final repo = _InMemoryCredentialsRepository();
        final container = ProviderContainer(
          overrides: [
            credentialsRepositoryProvider.overrideWithValue(repo),
