...
void main() {
  ...
  group('CalendarDao sources', () {
    ...
    test('replaceSources is a scoped replace-all for (workspace, account)',
        () async {
      await db.calendarDao.replaceSources('ws-A', 'acc-A', [
        source(id: 's1', workspaceId: 'ws-A', accountId: 'acc-A', calendarId: 'primary', summary: 'Primary', primary: true),
        source(id: 's2', workspaceId: 'ws-A', accountId: 'acc-A', calendarId: 'team@group', summary: 'Team'),
      ]);

      var rows = await db.calendarDao.watchSources('ws-A', 'acc-A').first;
      expect(rows.map((s) => s.calendarId), ['primary', 'team@group']); // primary first

      // A re-sync drops the now-absent calendar and keeps only the new set.
      await db.calendarDao.replaceSources('ws-A', 'acc-A', [
        source(id: 's3', workspaceId: 'ws-A', accountId: 'acc-A', calendarId: 'family', summary: 'Family'),
      ]);
    ...
    test('watchSources is scoped to (workspace, account)', () async {
      ...
      await db.calendarDao.replaceSources('ws-A', 'acc-A', [
        source(id: 's1', workspaceId: 'ws-A', accountId: 'acc-A', calendarId: 'primary', summary: 'A primary'),
      ]);
      // Same workspace, different account — tests account scoping.
      await db.calendarDao.replaceSources('ws-A', 'acc-B', [
        source(id: 's2', workspaceId: 'ws-A', accountId: 'acc-B', calendarId: 'primary', summary: 'B primary'),
      ]);
      // Different workspace, different account — must never leak into ws-A.
      await db.calendarDao.replaceSources('ws-B', 'acc-C', [
        source(id: 's3', workspaceId: 'ws-B', accountId: 'acc-C', calendarId: 'primary', summary: 'other workspace'),
      ]);

      expect(
        (await db.calendarDao.watchSources('ws-A', 'acc-A').first)
            .map((s) => s.summary),
        ['A primary'],
      );
      expect(
        (await db.calendarDao.watchSources('ws-A', 'acc-B').first)
            .map((s) => s.summary),
        ['B primary'],
      );
