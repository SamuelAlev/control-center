...
void main() {
  ...
  group('MessagingDao — messages', () {
    ...
    test(
        'messages inserted in the same second keep insertion order '
      ...
      final Value<DateTime> sameSecond = Value(DateTime.utc(2024, 1, 1, 12, 0, 0));
      for (final entry in [
        ('ccc', 'user', 'user', 'first'),
      ...
      final window = await db.messagingDao
          .watchTopLevelMessagesWindow('ch-order', limit: 50)
          .first;
