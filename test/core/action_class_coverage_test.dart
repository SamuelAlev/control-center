...
void main() {
  ...
  const expectedHarness = <String, Set<String>>{
    ...
    'checkpoint': <String>{},
    'rewind': <String>{},
    // Commits a change another tool staged. Write tier and a plain file write:
    // it rewrites whole files but never removes one, so `fileDelete` would
    // overstate it.
    'resolve': {'fileWriteOutsideWorktree'},
    // Vibe mode's director verbs. Read tier and no effect class, for the same
    // reason `task` has none: they start an in-process subagent LOOP, not a
    // process, and the gate belongs on the worker's own tools where the edit
    // actually happens.
    'vibe_spawn': <String>{},
    'vibe_send': <String>{},
    'vibe_wait': <String>{},
  ...
  const expectedMcp = <String, Set<String>>{
    ...
    'ticket_relation': {
      'vendorSyncWrite',
    }, // relation writes sync to the vendor
    'assign_ticket': {
      'vendorSyncWrite',
      'processSpawn',
    ...
    // one system message to its space. Both are local-DB writes and it only
    ...
    // space message. No filesystem, no process, no external system — the
    // same effect profile as `send_space_message`, which is why they are
    // allowed in every conversation mode.
    'get_artifact': <String>{},
    'get_messages': <String>{},
    'get_space_notes': <String>{}, // read: returns a space's notes
    'get_my_notes': <String>{},
    ...
    'ticket_pr_link':
        <String>{}, // local ticket.linkedPrIds mutate, no vendor event
    'todo_read': <String>{},
    'todo_write': <String>{},
    'update_goal_progress': <String>{},
    'update_space_notes': <String>{}, // edits a space's free-text notes field
  ...
  const expectedRepoOps = <String, Set<String>>{
    ...
    // The rest of the on-disk layout surface. Every one of these writes on the
    // HOST filesystem, outside any conversation worktree — which is exactly
    // what `fileWriteOutsideWorktree` names.
    'fs.ensureAgentDir': {'fileWriteOutsideWorktree'},
    'fs.ensureDir': {'fileWriteOutsideWorktree'},
    'fs.ensureSpaceDir': {'fileWriteOutsideWorktree'},
    'fs.ensureWorkspaceDirs': {'fileWriteOutsideWorktree'},
    'fs.persistLogo': {'fileWriteOutsideWorktree'},
    'fs.persistLogoBytes': {'fileWriteOutsideWorktree'},
    'fs.writeString': {'fileWriteOutsideWorktree'},
    'fs.ensureMcpSymlink': {'fileWriteOutsideWorktree'},
    'fs.syncAgentSkillLinks': {'fileWriteOutsideWorktree'},
    // A PTY on the host. `write` is not a lesser act than `spawn`: writing to
    // a live shell IS executing commands in it.
    ...
    // Skill sources: an install/update fetches from GitHub and writes skill
    // content into the workspace's skills dir (same family as the MCP
    // install_skill tool).
    'skills.sourceInstall': {'packageInstall', 'networkEgress'},
    'skills.updateSkill': {'packageInstall', 'networkEgress'},
    // Forge writes.
    'pr_lifecycle.publish': {'prCreate'},
    'pr_review.publishReview': {'prPublish'},
    'pr_review.commentFindings': {'prPublish'},
    // Marking a draft ready notifies its reviewers on the forge; the reverse
    // direction rides the same op.
    ...
    // spawn by proxy.
    'repos.setScripts': {'processSpawn'},
    // Test-run of a script DRAFT in a throwaway clone: executes the body.
    'repos.testScript': {'processSpawn'},
  };
  ...
  const effectFamilies = <String>[
    'rig.',
    'worktree.',
    // Added with the demo lockdown: these are the families a public host must
    // be able to refuse WHOLESALE, and an undeclared member is invisible to
    // the guardrails. `terminal.spawn` in particular declared nothing, which
    // is why the demo's boundary had to be a name allowlist rather than an
    // ActionClass denylist — this is the other half of that fix.
    'terminal.',
    'fs.',
    'codeServer.',
  ];
  ...
  const noEffectExemptions = <String, String>{
    ...
    'terminal.resize':
        ...
        'be a false claim, and a taxonomy people stop believing stops being '
        'read.',
    'codeServer.saveFile':
        'Saves a file INSIDE the conversation worktree (the handler confines '
