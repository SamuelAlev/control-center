...
void main() {
  ...
  group('NotificationEventMapper', () {
    ...
    test(
      'maps MeetingStartingSoon to a meetingStartsSoon notification',
      () async {
        eventBus.publish(
          MeetingStartingSoon(
            workspaceId: 'w1',
        ...
        expect(port.shown.first.route, '/workspaces/w1/calendar/evt-1');
        expect(port.shown.first.workspaceId, 'w1');
      },
    );

    test(
      'maps CalendarAuthExpired to a calendarAuthExpired notification',
      () async {
        eventBus.publish(
          CalendarAuthExpired(
            workspaceId: 'w1',
        ...
        expect(port.shown.first.route, '/workspaces/w1/calendar');
        expect(port.shown.first.workspaceId, 'w1');
      },
    );
    ...
    test(
      'does not notify on ExternalPrDetected (broad poller is pipeline-only)',
      () async {
        // A new PR on a linked repo must NOT raise a notification on its own —
        // only relevance-gated signals (PrReviewRequested / PrMentioned) ping
        // the user, so they are not spammed for PRs that do not concern them.
        eventBus.publish(
          ExternalPrDetected(
            repoOwner: 'acme',
            repoName: 'lib',
    ...
    test('maps PrMentioned to notification', () async {
      eventBus.publish(
        PrMentioned(
          workspaceId: 'w1',
          repoOwner: 'acme',
      ...
      expect(port.shown, hasLength(1));
      expect(port.shown.first.category, NotificationCategory.prMentioned);
      expect(port.shown.first.title, 'Mentioned in pull request');
      expect(port.shown.first.workspaceId, 'w1');
    });
  });
