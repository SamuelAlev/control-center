...
void main() {
  ...
  const rules = <String, List<String>>{
    ...
    'cc_server_core': ['package:flutter', 'dart:ui', 'package:control_center'],
    ...
    // the on-device-inference impls (ASR/VAD/diarization + embeddings) that
    // moved here out of the Flutter app. dart:io and dart:ffi are its whole job.
    // It must stay Flutter-free so it links into the server binary without
    // dragging Flutter (this guard makes the de-Fluttering permanent — the
    // `package:flutter/foundation` -> `package:meta` swap for `@immutable`).
    // Inference is the in-repo Rust crate `native/inference/`, reached over
    // dart:ffi, so this purity is structural — there is no pub dependency that
    // could drag Flutter in. cc_domain IS
  ...
  for (final entry in rules.entries) {
    ...
    test(
      '$pkg source imports nothing forbidden (Flutter-free server rail)',
      () {
  ...
  for (final entry in forbiddenDeps.entries) {
    ...
    test(
      '$pkg/pubspec depends on nothing forbidden (Flutter-free server rail)',
      () {
