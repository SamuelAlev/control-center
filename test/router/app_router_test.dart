...
GoRouter buildFullRouter({OnboardingGate gate = OnboardingGate.complete}) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) =>
        onboardingGuard(context, state, gateNotifier, () => 'ws-1'),
    routes: [
      GoRoute(path: splashRoute, builder: (_, _) => const SizedBox()),
      GoRoute(path: onboardingRoute, builder: (_, _) => const SizedBox()),
      GoRoute(path: workspaceListRoute, builder: (_, _) => const SizedBox()),
      GoRoute(
        path: inboxRoute(workspaceIdParam),
        builder: (_, _) => const SizedBox(),
...
void main() {
  group('OnboardingGuard via router', () {
    ...
    testWidgets('loading gate: onboarding redirects to splash', (tester) async {
      final router = buildFullRouter(gate: OnboardingGate.loading);
      addTearDown(router.dispose);
    ...
    testWidgets('complete gate: splash redirects to inbox', (tester) async {
      ...
      final location = router.routerDelegate.currentConfiguration.uri
          .toString();
      expect(location, inboxRoute('ws-1'));
    });

    testWidgets('complete gate: inbox stays on inbox', (tester) async {
      final router = buildFullRouter(gate: OnboardingGate.complete);
      addTearDown(router.dispose);
    ...
    testWidgets('complete gate: onboarding stays on onboarding', (
      ...
      // onboarding screen navigates to the inbox itself once the user
    ...
    testWidgets('incomplete gate: inbox redirects to onboarding', (
      tester,
    ) async {
      final router = buildFullRouter(gate: OnboardingGate.incomplete);
      addTearDown(router.dispose);
  ...
  group('OnboardingGate enum', () {
    test('has four values', () {
      expect(OnboardingGate.values, hasLength(4));
    });

    test('values are loading, complete, incomplete, signedOut', () {
      ...
      // A lapsed credential is its own state, not a flavour of `incomplete`:
      // it routes to the re-auth screen instead of the first-run flow.
      expect(OnboardingGate.values, contains(OnboardingGate.signedOut));
    });
  });
  ...
  group('Workspace deep-link routing', () {
    ...
    testWidgets('/spaces navigates correctly', (tester) async {
      ...
      router.go(spacesRoute('ws-1'));
      await tester.pump();

      final location = router.routerDelegate.currentConfiguration.uri
          .toString();
      expect(location, contains(spacesRoute('ws-1')));
    });
  ...
  group('onboardingGuard edge cases', () {
    ...
    testWidgets('gate transitions from loading to complete', (tester) async {
      ...
      var location = router.routerDelegate.currentConfiguration.uri.toString();
      expect(location, splashRoute);
    ...
    testWidgets('gate transitions from loading to incomplete', (tester) async {
      final gateNotifier = ValueNotifier<OnboardingGate>(
        OnboardingGate.loading,
      );
      final router = GoRouter(
  ...
  group('deep linking edge cases', () {
    ...
    testWidgets('/pull-requests/ empty path navigates to list', (tester) async {
      final router = buildRouter();
      addTearDown(router.dispose);
  ...
  group('route configuration structure', () {
    ...
    testWidgets('newsfeed settings route navigates correctly', (tester) async {
      final router = buildRouter();
      addTearDown(router.dispose);
      ...
      router.go(settingsNewsfeedRoute('ws-1'));
      await tester.pump();
