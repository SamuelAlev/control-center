...
GoRouter buildRouter({OnboardingGate gate = OnboardingGate.complete}) {
  ...
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        path: channelsRoute(workspaceIdParam),
        builder: (_, _) => const SizedBox(),
      ),
      GoRoute(
...
void main() {
  ...
  group('Workspace deep-link routing', () {
    ...
    testWidgets('/channels navigates correctly', (tester) async {
      final router = buildRouter();
      addTearDown(router.dispose);
      ...
      router.go(channelsRoute('ws-1'));
      await tester.pump();

      final location = router.routerDelegate.currentConfiguration.uri
          .toString();
      expect(location, contains(channelsRoute('ws-1')));
    });
