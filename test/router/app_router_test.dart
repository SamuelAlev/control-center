...
void main() {
  ...
  group('Workspace deep-link routing', () {
    ...
    testWidgets('/settings/general navigates correctly', (tester) async {
      final router = buildRouter();
      addTearDown(router.dispose);
      ...
      router.go(settingsGeneralRoute);
      await tester.pump();

      final location =
          router.routerDelegate.currentConfiguration.uri.toString();
      expect(location, contains(settingsGeneralRoute));
    });
  ...
  group('deep linking edge cases', () {
    ...
    testWidgets('/settings redirects to /settings/general', (tester) async {
      final router = GoRouter(
        navigatorKey: GlobalKey<NavigatorState>(),
        initialLocation: dashboardRoute,
        routes: [
          ...
          GoRoute(
            path: settingsRoute,
            redirect: (_, _) => settingsGeneralRoute,
          ),
          GoRoute(
