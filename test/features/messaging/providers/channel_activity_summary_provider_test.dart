...
/// Exercises [summarizeChannelActivity] — the pure shaping the sidebar's channel
...
void main() {
  ...
  group('summarizeChannelActivity', () {
    test('reports nothing for a conversation with no runs', () {
      expect(summarizeChannelActivity(const []), ChannelActivitySummary.empty);
    });

    test('an idle conversation has no live runs but keeps its totals', () {
      final summary = summarizeChannelActivity([
        run(
          id: 'r-1',
    ...
    test('a pending run counts as live but reports queued, not running', () {
      ...
      // Still live: the channel has work in flight, which is what `isLive`
