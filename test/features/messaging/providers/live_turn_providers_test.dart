...
class _FakeRelay implements SpaceTurnRelayPort {
  final StreamController<SpaceTurnEvent> controller =
      StreamController<SpaceTurnEvent>.broadcast();

  @override
  Stream<SpaceTurnEvent> watchSpaceTurns(String spaceId) => controller.stream;
...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [spaceTurnRelayPortProvider.overrideWithValue(relay)],
    );
  });
  ...
  group('spaceTurnRelayProvider fold', () {
    test('seed frame registers active turns with their snapshots', () async {
      final sub = container.listen(spaceTurnRelayProvider('c1'), (_, _) {});
