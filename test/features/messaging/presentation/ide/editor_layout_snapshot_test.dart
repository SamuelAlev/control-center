...
EditorTab _chat(String channelId) => EditorTab(
...
void main() {
  group('round-trip', () {
    test(
      'preserves a split tree, tab kinds/args, selection and active leaf',
      () {
        ...
        expect(first.first.args['channelId'], 'chan-1');

        // Second pane is the moved browser, and is the active leaf.
        final second = _tabsOf(split.children[1]);
        expect(second.single.kind, MessagingTabKinds.browser);
        expect(
          restored.activeLeaf.controller.tabs.single.kind,
    ...
    test('preserves per-leaf selection', () {
      final ctl = _single([
        _chat('c'),
      ...
      final restored = decodeEditorLayout(encodeEditorLayout(ctl))!;
      expect((restored.root as EditorLeafNode).controller.selectedIndex, 2);
    });
  });
  ...
  group('robust decode', () {
    ...
    test('a chat tab missing its channelId is dropped', () {
      const json =
          '{"v":1,"root":{"t":"leaf","active":true,"sel":0,"tabs":['
  ...
  group('agentActivity restoration', () {
    ...
    test('a tab missing its runId is dropped', () {
      const json =
          ...
          '{"workspaceId":"ws-1","channelId":"c-1"}}]}}';
    ...
    test('a tab missing its workspaceId is dropped', () {
      const json =
          '{"v":1,"root":{"t":"leaf","active":true,"sel":0,"tabs":['
          '{"kind":"agentActivity","label":"scout","args":'
          '{"channelId":"c-1","runId":"run-1"}}]}}';

      expect(decodeEditorLayout(json), isNull);
    });

    test('a tab missing its channelId is dropped', () {
      const json =
          '{"v":1,"root":{"t":"leaf","active":true,"sel":0,"tabs":['
          '{"kind":"agentActivity","label":"scout","args":'
