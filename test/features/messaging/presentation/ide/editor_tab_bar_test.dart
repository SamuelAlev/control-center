...
Widget _harness({
  ...
  Future<void> Function(int index, Offset globalPosition)? onTabContextMenu,
...
void main() {
  ...
  testWidgets('right-clicking a tab opens its menu without selecting it', (
    tester,
  ) async {
    // The whole point of the tab menu is acting on a tab you are not in:
    // selecting first would load its body (boot a rig, spawn a PTY) only to
    // close it a click later.
    int? selected;
    int? menuIndex;
    ...
    await tester.tap(_label('C'), buttons: kSecondaryButton);
    await tester.pumpAndSettle();

    expect(menuIndex, 2);
    expect(selected, isNull);

    // While the menu stands open the pointer is over the overlay, so the tab
    // carries the "menu target" wash in hover's place — visible here as C's
    // close button appearing alongside the selected A's.
  ...
  testWidgets('a leading widget replaces the icon glyph and follows the label '
      'color', (tester) async {
    await tester.pumpWidget(
      _harness(
        onTabSelected: (_) {},
