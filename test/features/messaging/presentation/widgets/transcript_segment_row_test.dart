...
void main() {
  ...
  group('TranscriptSegmentRow', () {
    ...
    testWidgets('diff stats and duration share one baseline', (tester) async {
      // Regression: an unstyled ' ' spacer inside the stats Text.rich used to
      // inherit the ambient DefaultTextStyle, inflate the rich text's line
      // box and lift "+N −N" off the duration's baseline.
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ToolSegment(
      ...
      final statsCenter = tester.getCenter(find.textContaining('+2'));
      final durationCenter = tester.getCenter(find.text('<1s'));
      expect(
        (statsCenter.dy - durationCenter.dy).abs(),
        lessThan(1.5),
        reason: 'stats and duration must sit on the same baseline',
      );
    });

    testWidgets('error segment is expanded by default with error icon', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ErrorSegment(
              message:
                  'something went wrong with a long enough message to expand',
              startedAt: ts,
            ),
            codeFont: 'monospace',
          ),
      ...
      expect(find.byIcon(AppIcons.circleX), findsOneWidget);
    ...
    testWidgets('edit tool is expanded by default with its diff', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ToolSegment(
              ...
              inputs: const {
                'file_path': 'x.dart',
                'old_string': 'final removed = 1;',
                'new_string': 'final added = 2;',
              },
              status: ToolSegmentStatus.ok,
              startedAt: ts,
              durationMs: 500,
      ...
      // No tap: an edit's body IS the information, so it opens expanded.
      expect(find.text('Modified'), findsOneWidget);
      expect(find.textContaining('final removed = 1;'), findsWidgets);
      expect(find.textContaining('final added = 2;'), findsWidgets);
    });

    testWidgets('edit tool can still be collapsed', (tester) async {
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ToolSegment(
      ...
      await tester.tap(find.textContaining('Edit').first);
      await tester.pumpAndSettle();
      expect(find.text('Modified'), findsNothing);
      expect(find.textContaining('final removed = 1;'), findsNothing);
    });

    testWidgets('a still-streaming edit does not open onto nothing', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ToolSegment(
              toolName: 'Edit',
              toolCallId: 'c',
              // Arguments are still arriving: no old/new strings yet.
    ...
    testWidgets('write tool is expanded by default with its contents', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
          TranscriptSegmentRow(
            segment: ToolSegment(
              ...
              inputs: const {
                'file_path': 'x.dart',
                'content': 'final written = 3;',
              },
              outputs: 'File created successfully at: x.dart',
              status: ToolSegmentStatus.ok,
              startedAt: ts,
              durationMs: 500,
            ),
      ...
      expect(find.textContaining('final written = 3;'), findsWidgets);
    });
