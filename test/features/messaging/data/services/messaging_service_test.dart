...
void main() {
  ...
  group('stopRuns', () {
    test('stops given run log ids', () async {
      ...
      await service.stopRuns([runLog!.id]);
  ...
  group('dispatchAgent', () {
    ...
    test('sends thinking message and dispatches', () async {
      ...
      final msgs = await repo.getMessages(channel.id);
      expect(
        msgs.any((m) => m.messageType == ChannelMessageType.thinking),
        isTrue,
      );
    });
    ...
    test('uses agent name and directory from agent repo', () async {
      ...
      final thinking = msgs.firstWhere(
        (m) => m.messageType == ChannelMessageType.thinking,
      );
    ...
    test('registers stream ids in the registry', () async {
      ...
      expect(streamRegistry.isActive('${runLog.id}_resp'), isTrue);
    });
  });
  ...
  group('stopRuns multiple', () {
    test('stops multiple run log ids', () async {
      ...
      await service.stopRuns([runLog1!.id, runLog2!.id]);
    });
  });
