...
void main() {
  ...
  group('start with params', () {
    test(
      'accepts optional parameters including ticketId and wakeContext',
      () async {
        final handle = ds.start(
          cliName: 'pi',
          prompt: 'test',
  ...
  group('stream error handling', () {
    test('stream errors are propagated through error channel', () async {
      ...
      final errors = <Object>[];
      handle.events.listen((_) {}, onError: errors.add, cancelOnError: false);
  ...
  group('handlePiEvent — "event" type routing', () {
    ...
    setUp(() {
      ...
      collected = [];
      // Use the test-only controller seam rather than start(): start() probes
      // the host for the `pi` binary and, when it is absent (as on the CI Linux
      // runner), pushes a ProcessException onto a stream with no onError
      // handler — an unhandled async error that fails the test. These tests
      // drive handlePiEvent in isolation, so no real dispatch is needed.
    ...
    test('unknown event type emits a parse-error marker', () async {
      testDs.handlePiEvent({
        'type': 'event',
      ...
      expect(collected, hasLength(1));
      expect(collected.single, isA<DebugEvent>());
    });

    test('unknown top-level type emits a parse-error marker', () async {
  ...
  group('coalescing — thinking/text buffering', () {
    ...
    test('real-world sequence produces correct events', () async {
      ...
      testDs.handlePiEvent({
        'type': 'event',
        'eventType': 'debug',
        'content': '[sandbox] pi running (pid 123)',
      });
      testDs.handlePiEvent({
        'type': 'event',
        'eventType': 'error',
  ...
  group('handlePiEvent — PI native format', () {
    ...
    test(
      'tool_execution_start with full payload emits toolCall with metadata',
      () async {
        testDs.handlePiEvent({
    ...
    test(
      'tool_execution_end with full payload emits toolResult with outputs',
      () async {
        testDs.handlePiEvent({
    ...
    test(
      'tool_execution_update emits toolResult with partial outputs',
      () async {
        testDs.handlePiEvent({
    ...
    test('message_update with null assistantMessageEvent is ignored', () async {
      testDs.handlePiEvent({'type': 'message_update'});
