...
void main() {
  group('MeetingFormat.clock', () {
    test('formats seconds as MM:SS', () {
      ...
      expect(
        MeetingFormat.clock(const Duration(minutes: 3, seconds: 7)),
        '03:07',
      );
      expect(
        MeetingFormat.clock(const Duration(minutes: 59, seconds: 59)),
        '59:59',
      );
    });
    ...
    test('formats durations >= 1 hour as H:MM:SS', () {
      ...
      expect(
        MeetingFormat.clock(const Duration(hours: 1, minutes: 5, seconds: 3)),
        '1:05:03',
      );
      expect(
        MeetingFormat.clock(
          const Duration(hours: 10, minutes: 42, seconds: 17),
        ),
        '10:42:17',
      );
    });
  ...
  group('MeetingFormat.totalLabel', () {
    ...
    test('formats durations >= 1 hour with padded minutes', () {
      ...
      expect(
        MeetingFormat.totalLabel(const Duration(hours: 2, minutes: 5)),
        '2h 05m',
      );
      expect(
        MeetingFormat.totalLabel(const Duration(hours: 3, minutes: 42)),
        '3h 42m',
      );
    });
  ...
  group('MeetingFormat.duration', () {
    test('returns difference when endedAt is provided', () {
      ...
      expect(
        MeetingFormat.duration(start, end, now),
    ...
    test('uses now when endedAt is null (recording still in progress)', () {
      ...
      expect(
        MeetingFormat.duration(start, null, now),
