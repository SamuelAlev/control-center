...
void main() {
  group('MeetingSignalPill', () {
    testWidgets('renders icon and label', (tester) async {
      await tester.pumpWidget(
        testWrap(
          const MeetingSignalPill(icon: AppIcons.check, label: '3 decisions'),
        ),
      );
  ...
  group('MeetingStatusGlyph', () {
    testWidgets('done status shows check icon', (tester) async {
      await tester.pumpWidget(
        testWrap(const MeetingStatusGlyph(status: MeetingStatus.done)),
      );
    ...
    testWidgets('recording status shows mic icon', (tester) async {
      await tester.pumpWidget(
        testWrap(const MeetingStatusGlyph(status: MeetingStatus.recording)),
      );
    ...
    testWidgets('failed status shows alert icon', (tester) async {
      await tester.pumpWidget(
        testWrap(const MeetingStatusGlyph(status: MeetingStatus.failed)),
      );
    ...
    testWidgets('processing status shows equalizer bars', (tester) async {
      await tester.pumpWidget(
        testWrap(const MeetingStatusGlyph(status: MeetingStatus.processing)),
      );
    ...
    testWidgets('respects custom size', (tester) async {
      ...
      final icon = tester.widget<Icon>(find.byIcon(AppIcons.circleCheck));
      expect(icon.size, 24);
    });
  });
  ...
  group('MeetingEyebrow', () {
    testWidgets('renders text in uppercase', (tester) async {
      await tester.pumpWidget(testWrap(const MeetingEyebrow('summary')));
