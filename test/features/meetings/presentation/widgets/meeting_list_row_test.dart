...
void main() {
  group('MeetingListRow', () {
    ...
    testWidgets('renders processing tag for processing meeting', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
    ...
    testWidgets('renders signal pills for enhanced meeting', (tester) async {
          ...
          ProviderScope(
            overrides: [
              meetingActionItemStatsProvider.overrideWith(
                (ref, _) => Stream.value(const <String, MeetingActionItemStats>{
                  'm2': (total: 2, done: 0),
                }),
              ),
      ...
      expect(find.byIcon(AppIcons.listChecks), findsOneWidget);
    });

    testWidgets('renders the duration column instead of an Open button', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
      ...
      // The whole row is the affordance; a per-row button would duplicate it.
      expect(find.text('Open'), findsNothing);
      // 10:00 → 11:00 reads as one hour, in tabular mono.
      expect(find.text('1:00:00'), findsOneWidget);
      expect(find.byIcon(AppIcons.chevronRight), findsOneWidget);
    });

    testWidgets('replaces the duration with Stop while summarizing', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
    ...
    testWidgets('reserves no pill row when nothing was extracted', (
      tester,
    ) async {
      await tester.pumpWidget(
        testWrap(
      ...
      expect(find.byType(MeetingSignalPill), findsNothing);
    });
