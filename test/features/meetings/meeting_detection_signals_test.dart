...
void main() {
  ...
  group('ProcessMeetingSignalCollector', () {
    ...
    test(
      'emits one conferencingApp signal per per-meeting app, deduped',
      () async {
        ...
        expect(signals.map((s) => s.label).toSet(), {'Zoom', 'Webex'});
        expect(
    ...
    test(
      'returns nothing (no throw) when the process list command fails',
      () async {
        final collector = ProcessMeetingSignalCollector(
  ...
  group('CalendarMeetingSignalCollector', () {
    ...
    test(
      'also emits a browserMeeting signal when a known join URL is present',
      () async {
        final signals = await sample([
          event(
            start: now.subtract(const Duration(minutes: 5)),
        ...
        expect(signals.map((s) => s.kind).toSet(), {
          MeetingSignalKind.calendarEvent,
          MeetingSignalKind.browserMeeting,
        });
      },
    );
    ...
    test(
      'ignores all-day, declined, cancelled, solo, and not-yet events',
      () async {
        final live = {
          'allDay': event(
