...
void main() {
  ...
  group('MeetingDetectionStateMachine', () {
    MeetingCandidate cand({String? label, int sinceSec = 0}) =>
        MeetingCandidate(
          confidence: 0.9,
    ...
    test('debounces: prompts only after minPresence', () {
      ...
      expect(
        m.update(
          candidate: cand(label: 'a', sinceSec: 9),
          now: later,
        ),
      ...
      expect(
        m.update(
          candidate: cand(label: 'a', sinceSec: 10),
          now: later,
        ),
        MeetingDetectionAction.none,
      );
    ...
    test(
      'suggests auto-stop after a sustained no-signal gap while recording',
      () {
        final m = MeetingDetectionStateMachine(
