...
void main() {
  ...
  test(
    ...
    'until the late them cancels it',
    () async {
      final f = makeFilter();
      ...
      f.add(
        cand(MeetingSpeaker.them, themLine, 1120),
      ); // within band, cancels it
      await settle();
  ...
  test(
    'a "me" emitted while the remote is quiet commits promptly (idle hold)',
    () async {
      final f = makeFilter();
  ...
  test(
    'genuine me over the remote is kept (held long, then committed)',
    () async {
      final f = makeFilter();
      f.noteSystemActivity(1000); // remote playing → echo-possible → long hold
      f.add(
        cand(MeetingSpeaker.me, 'i strongly disagree with that plan', 1010),
      );
      await betweenHolds();
  ...
  test(
    'no bleed: interleaved non-matching windows all commit exactly once',
    () async {
      final f = makeFilter();
  ...
  test(
    'short me window (below minTokens) is never matched as an echo',
    () async {
      final f = makeFilter();
  ...
  test('a matching text outside the time band is kept (not an echo)', () async {
    final f = makeFilter();
    f.add(
      cand(MeetingSpeaker.them, 'the quarterly report is finally ready', 1000),
    );
    await Future<void>.delayed(const Duration(milliseconds: 5));
