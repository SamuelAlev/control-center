...
void main() {
  group('formatMeetingTranscript', () {
    ...
    test('uses the diarization label, overridden by the displayNames map', () {
      final segments = [
        _seg(speaker: MeetingSpeaker.them, speakerLabel: 'Person 1', text: 'Hi'),
      ];
      expect(formatMeetingTranscript(segments), '[00:00] Person 1: Hi');
      expect(
        formatMeetingTranscript(segments, displayNames: {'Person 1': 'Alice'}),
        '[00:00] Alice: Hi',
      );
    });

    test('a per-segment override wins over both label and displayNames', () {
      final result = formatMeetingTranscript(
        [
          _seg(
            speaker: MeetingSpeaker.them,
        ...
        displayNames: {'Person 1': 'Alice'},
      );
      // The block was individually renamed to Bob — that wins.
      expect(result, '[00:00] Bob: Hi');
    });
  });
}
