...
class FakePipelineRunRepository implements PipelineRunRepository {
  ...
  @override
  Stream<PipelineRun?> watchRun(String id) => const Stream.empty();
...
void main() {
  ...
  group('start() and dispose()', () {
    test('start() subscribes to event bus and runs _reconcileStale', () {
      reconciler.start();
  ...
  group('_onEvent filtering', () {
    ...
    test('ignores events for wrong templateId', () async {
      final meeting = _meeting();
      meetingRepo.addMeeting(meeting);
      runRepo.addRun(_pipelineRun(triggerPayload: {'meeting_id': meeting.id}));
      ...
      expect(
        meetingRepo.lastUpserted,
        isNull,
        reason: 'Should not finalize for wrong templateId',
      );
    }, timeout: const Timeout.factor(2));

    test('ignores events where run not found', () async {
      final meeting = _meeting();
      meetingRepo.addMeeting(meeting);
    ...
    test('ignores events where triggerPayload has no meetingId', () async {
      runRepo.addRun(_pipelineRun(triggerPayload: {'other_key': 'otherValue'}));
    ...
    test('ignores events where triggerPayload is null', () async {
      runRepo.addRun(_pipelineRun(triggerPayload: null));
    ...
    test('ignores events where meeting is not processing status', () async {
    ...
    test('ignores events where meeting not found', () async {
      // meeting is never added to repo
      runRepo.addRun(
        _pipelineRun(triggerPayload: {'meeting_id': 'nonexistent'}),
      );
  ...
  group('finalization on pipeline events', () {
    ...
    test('uses transcript as fallback when enhanced notes are empty', () async {
      ...
      final expectedTranscript = formatMeetingTranscript(segments);
      expect(result.enhancedNotes, expectedTranscript);
    }, timeout: const Timeout.factor(2));

    test('leaves enhanced notes untouched when they exist', () async {
  ...
  group('_reconcileStale', () {
    test('finalizes stuck processing meetings with no active run', () async {
    ...
    test('skips meetings that have an active run', () async {
      final meeting = _meeting();
      meetingRepo.addMeeting(meeting);
    ...
    test('handles multiple stuck meetings (some with active runs)', () async {
      ...
      runRepo._activeByDedup[('meeting_summary', m2.workspaceId, m2.id)] =
          _pipelineRun();
    ...
    test('does nothing when getUnfinalized returns empty', () async {
  ...
  group('_finalize edge cases', () {
    ...
    test('emits correct transcript format via fallback', () async {
      final meeting = _meeting();
      final segments = [
        MeetingSegment(
          id: 'seg-me',
    ...
    test('preserves non-status fields on finalize', () async {
  ...
  group('finalization on PipelineRunCancelled', () {
    ...
    test('ignores cancellation for a wrong templateId', () async {
      final meeting = _meeting();
      meetingRepo.addMeeting(meeting);
      runRepo.addRun(_pipelineRun(triggerPayload: {'meeting_id': meeting.id}));
  ...
  group('_reconcileStale recovers stranded recordings', () {
    test('a recording with a transcript moves to processing and re-announces '
        'MeetingRecordingStopped so the summary pipeline re-runs', () async {
      ...
      final sub = eventBus.on<MeetingRecordingStopped>().listen(announced.add);
    ...
    test('a recording with no transcript is finalized straight to done '
        'without announcing a summary run', () async {
      final meeting = _meeting(status: MeetingStatus.recording);
      meetingRepo.addMeeting(meeting);
      meetingRepo.addSegments(meeting.id, []);
