...
void main() {
  ...
  group('AecMicFilter.cleanMic', () {
    test(
      're-blocks mismatched chunk sizes into exact blocks, drops the tail',
      () async {
        ...
        final reInverted = Uint8List.fromList(
          out.map((b) => b ^ 0xFF).toList(),
        );
    ...
    test(
      'null processor → identity passthrough (same stream, same bytes)',
      () async {
    ...
    test(
      'zero-pads the far reference when the loopback never feeds (stall gate)',
      () async {
        final fake = _FakeAecEngine();
        final filter = AecMicFilter(processor: fake);
        // cleanMic only, no referenceTap → the render buffer is always behind, so
        // every capture block gets a zero-padded silence reference (AEC3 stays
        // aligned instead of cancelling against a stale/empty render buffer).
        await filter.cleanMic(Stream.fromIterable([_ramp(block * 3)])).toList();

        expect(fake.captureBlocks, hasLength(3));
        expect(filter.referenceFramesZeroPadded, 3);
        expect(filter.referenceFramesMatched, 0);
        expect(fake.reverseBlocks, hasLength(3));
        for (final b in fake.reverseBlocks) {
          expect(
            b.every((x) => x == 0),
            isTrue,
            reason: 'reference is silence',
          );
        }
      },
    );
    ...
    test(
      'uses the real reference when the far-end leads (matched, no zero-pad)',
      () async {
        final fake = _FakeAecEngine();
        final filter = AecMicFilter(processor: fake);
        // Drain 5 far blocks first so the render buffer leads the capture, then
        // process 3 capture blocks — all matched, none zero-padded.
        await filter
            .referenceTap(Stream.fromIterable([_ramp(block * 5)]))
            .toList();
        await filter.cleanMic(Stream.fromIterable([_ramp(block * 3)])).toList();

        expect(filter.referenceFramesMatched, 3);
        expect(filter.referenceFramesZeroPadded, 0);
        // Only the 5 real far blocks were fed as references (no silence padding).
        expect(fake.reverseBlocks, hasLength(5));
      },
    );
  });
  ...
  group('AecMicFilter.referenceTap', () {
    test('feeds far-end blocks and re-emits "them" chunks UNCHANGED', () async {
      ...
      final out = await filter
          .referenceTap(Stream.fromIterable(chunks))
          .toList();
