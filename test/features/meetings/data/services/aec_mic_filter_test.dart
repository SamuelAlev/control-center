...
void main() {
  ...
  group('AecMicFilter.cleanMic', () {
    ...
    test('zero-pads the far reference when the loopback never feeds (stall gate)',
        () async {
      final fake = _FakeAecEngine();
      final filter = AecMicFilter(processor: fake);
      ...
      for (final b in fake.reverseBlocks) {
        expect(b.every((x) => x == 0), isTrue, reason: 'reference is silence');
      }
    });

    test('uses the real reference when the far-end leads (matched, no zero-pad)',
        () async {
      final fake = _FakeAecEngine();
      final filter = AecMicFilter(processor: fake);
