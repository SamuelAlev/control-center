...
void main() {
  ...
  group('MeetingMapper.toDomain', () {
    test(
      'maps all fields correctly with non-null optional values',
    ...
    test(
      'maps all nullable fields as null when absent',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'MeetingStatus.fromStorage is used for status field',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
    ...
    test(
      'MeetingStatus.fromStorage defaults to done for unknown status',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
    ...
    test(
      'defaults userNotes to empty string when mapped',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
    ...
    test(
      'preserves timestamp fields exactly',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('MeetingMapper.segmentToDomain', () {
    ...
    test(
      'MeetingSpeaker.fromStorage is used for speaker field',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
    ...
    test(
      'MeetingSpeaker.fromStorage defaults to them for unknown speaker',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
        ...
        expect(mapper.segmentToDomain(row).speaker, MeetingSpeaker.them);
      },
    );
    ...
    test(
      'preserves millisecond offsets exactly',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime(2026, 6, 11);
        ...
        for (final (start, end) in edgeCases) {
          ...
          final segment = mapper.segmentToDomain(row);
          expect(segment.startMs, start, reason: 'startMs should be $start');
    ...
    test(
      'preserves createdAt timestamp exactly',
      timeout: const Timeout.factor(2),
      () {
