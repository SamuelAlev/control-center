...
class _FakeMeetingRepository implements MeetingRepository {
  ...
  Stream<Map<String, MeetingActionItemStats>> watchActionItemStats(String w) =>
      const Stream.empty();
  @override
  Stream<Map<String, int>> watchDecisionCounts(String w) =>
      const Stream.empty();
  @override
...
PipelineContext _ctx({
  ...
  return PipelineContext(
    ...
    stepRunId: 'sr1',
    workspaceId: workspaceId,
    state: {'meeting_id': meetingId, 'meeting_outcome': outcome},
  );
}
...
void main() {
  ...
  test('meeting.addActionItems writes ordered rows with owners', () async {
    ...
    expect(
      items.every((i) => i.workspaceId == 'w1' && i.meetingId == 'm1'),
      isTrue,
    );
  });
  ...
  test(
    'a foreign-workspace meeting fails the step (no leak, no write)',
    () async {
      repo.seed(_processing('m1', 'w1'));
      final result = await registry.body(BuiltInBodyKeys.meetingSaveNotes)(
  ...
  test(
    'a structured outcome with empty lists DOES clear (legitimate replace)',
    () async {
      repo.seed(_processing('m1', 'w1'));
  ...
  test(
    'meeting.updateTranscript labels + merges the transcript into turns',
    () async {
      ...
      final ctx = PipelineContext(
        ...
        state: {
          ...
          'diarization_spans': encodeDiarizedSpans(const [
      ...
      final result = await registry.body(
  ...
  test(
    'meeting.updateTranscript is a no-op when diarization did not run',
    () async {
      repo.seed(_processing('m1', 'w1'));
      const ctx = PipelineContext(
        ...
        stepRunId: 'sr1',
        workspaceId: 'w1',
        state: {'meeting_id': 'm1', 'meeting_diarized': false},
      );
  ...
  group('meeting.assemblePlayback', () {
    test('mixes the retained WAVs into mixed.wav', () async {
      ...
      expect(result.errorMessage, isNull);
      expect(result.mutatedState?['playback_assembled'], isTrue);
    ...
    test('is a no-op when the meeting kept no audio', () async {
      ...
      expect(result.errorMessage, isNull);
      expect(result.mutatedState?['playback_assembled'], isFalse);
    });
  ...
  group('meeting.identifySpeakers (voiceprint recognition)', () {
    test('auto-applies a confident voice-profile match', () async {
      ...
      expect(result.errorMessage, isNull);
      expect(result.mutatedState?['meeting_speakers_recognized'], 1);
    ...
    test(
      'leaves "Person N" when the best match is below the auto threshold',
      () async {
        repo.seed(_processing('m1', 'w1'));
        ...
        expect(result.mutatedState?['meeting_speakers_recognized'], 0);
        expect(repo.renames, isEmpty);
      },
    );

    test(
      'never overrides a speaker that already has a name (user/prior wins)',
      () async {
        repo.seed(_processing('m1', 'w1'));
        repo.seedSpeakers('m1', [
          _speaker(
  ...
  group('meeting.saveNotes (LLM speaker-name inference)', () {
    ...
    test(
      'ignores inferred names for labels with no matching speaker',
      () async {
        repo.seed(_processing('m1', 'w1'));
