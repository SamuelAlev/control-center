...
void main() {
  ...
  group('AndSpeechActivityDetector', () {
    test('fires only when every wrapped detector agrees', () {
      ...
      expect(
        AndSpeechActivityDetector([yes, yes]).isSpeech(_pcm(1, 0)),
        isTrue,
      );
      expect(
        AndSpeechActivityDetector([yes, no]).isSpeech(_pcm(1, 0)),
        isFalse,
      );
    ...
    test(
      'evaluates every detector each chunk (stateful VADs must see audio)',
      () {
    ...
    test('Silero(true) AND RMS-floor gates out quiet residual echo', () {
      ...
      expect(
        gate.isSpeech(_pcm(100, 12000)),
        isTrue,
      ); // loud near speech → kept
    });
