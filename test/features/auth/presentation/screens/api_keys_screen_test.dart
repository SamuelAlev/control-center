...
void main() {
  ...
  group('ApiKeysScreen', () {
    ...
    testWidgets('renders the ticketing Update key button', (tester) async {
      await tester.pumpWidget(wrap(const ApiKeysScreen()));
      await tester.pumpAndSettle(const Duration(seconds: 5));

      // Only the ticketing API key remains on this screen; the z.ai key (the
      // former second button) moved to the harness provider panel, where
      // per-provider credentials now live.
