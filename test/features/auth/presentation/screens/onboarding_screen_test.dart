...
void main() {
  ...
  Future<void> pumpOnboarding(
    ...
    // When true, neither the gh probe nor the derived auth flag is
    // overridden: the test supplies the probe via [extraOverrides] and the
    // REAL derivation answers isGitHubAuthenticatedProvider.
      ...
      ProviderScope(
        overrides: [
          ...
          if (!deriveAuth) ...[
            ...
            isGitHubAuthenticatedProvider.overrideWith((ref) => authenticated),
          ],
          ...extraOverrides,
        ],
        child: MaterialApp(
  ...
  testWidgets('gh auth settled before the first frame still shows step 1', (
    ...
    // through the REAL derivation (cli probe → isGitHubAuthenticatedProvider)
    // rather than an override of the derived flag.
