...
void main() {
  group('SubscriptionManager', () {
    ...
    test(
      'rejects a workspace-scoped subscribe when args lack workspace_id',
      () {
        final r = manager.subscribe(
          id: 1,
    ...
    test('unknown query is default-denied', () {
      final r = manager.subscribe(id: 1, params: const {'query': 'nope'});
      expect((r['error'] as Map)['code'], RpcErrorCodes.opUnknown);
    });

    test('enforces the per-session subscription cap', () {
      manager.subscribe(id: 1, params: const {'query': 'feeds.watch'});
      manager.subscribe(id: 2, params: const {'query': 'feeds.watch'});
