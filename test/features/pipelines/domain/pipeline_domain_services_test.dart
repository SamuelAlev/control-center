...
void main() {
  group('TemplateRenderer', () {
    ...
    test('substitutes bare, state and trigger-scoped refs', () {
      final out = r.render(
        'PR #{{pr_number}} by {{\$trigger.author}} at {{\$state.path}}',
  ...
  group('PipelineValidator', () {
    ...
    test('flags missing trigger and terminal', () {
      ...
      expect(
        issues.any((i) => i.isError && i.message.contains('trigger')),
        isTrue,
      );
      expect(
    ...
    test('switch router with no cases and no default is an error', () {
      final issues = validator.validate(
        def([
          ...
          PipelineStepDefinition(
            ...
            config: const PipelineNodeConfig(
              extras: {'switchKey': 'x', 'cases': <String>[]},
            ),
          ),
          PipelineStepDefinition(
            id: 'a',
      ...
      expect(
        issues.any(
          (i) => i.isError && i.message.contains('switch with no cases'),
        ),
        isTrue,
      );
    });
    ...
    test(
      'and predicate with a single-map child is valid (matches runtime)',
      () {
        final issues = validator.validate(
          def([
            PipelineStepDefinition(
    ...
    test('all built-in templates are structurally valid (no errors)', () {
      ...
      for (final seed in seeds) {
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Template "${seed.templateId}" has errors: '
