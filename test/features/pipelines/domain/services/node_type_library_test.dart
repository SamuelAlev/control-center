...
void main() {
  ...
  group('NodeTypeLibrary', () {
    ...
    test(
      'byId returns null for missing id',
  ...
  group('defaultNodeTypeLibrary', () {
    test(
      'returns a library with entries',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'contains expected built-in node types',
      timeout: const Timeout.factor(2),
      () {
        final lib = defaultNodeTypeLibrary();
        ...
        expect(lib.byId('messaging.postSpace'), isNotNull);
    ...
    test(
      'all entries have non-empty id, displayName, description, bodyKey',
      timeout: const Timeout.factor(2),
      () {
        final lib = defaultNodeTypeLibrary();
        for (final type in lib.types) {
          expect(type.id.isNotEmpty, isTrue, reason: 'Empty id');
          expect(
          ...
          expect(
            type.defaultBodyKey.isNotEmpty,
            isTrue,
    ...
    test(
      'bash.script has correct defaults',
      timeout: const Timeout.factor(2),
      () {
        final lib = defaultNodeTypeLibrary();
    ...
    test('bash.clonePr has correct defaults', () {
      ...
      expect(
        clone.defaultConfig.inputKeys,
        containsAll(['repo_full_name', 'pr_number']),
      );
      expect(clone.defaultConfig.outputKey, 'repo_local_path');
    });
    ...
    test('prompt.reviewer has correct defaults', () {
      ...
      expect(
        reviewer.defaultConfig.inputKeys,
        containsAll(['repo_local_path', 'pr_title', 'pr_number']),
      );
    ...
    test('prompt.join has join kind', timeout: const Timeout.factor(2), () {
      ...
      expect(join.defaultConfig.outputKey, 'consolidated_findings');
    });
    ...
    test('messaging.postSpace has correct defaults', () {
      final lib = defaultNodeTypeLibrary();
      final space = lib.byId('messaging.postSpace')!;
      expect(space.defaultKind, StepKind.listen);
      expect(
        space.defaultConfig.inputKeys,
        containsAll(['space_id', 'content']),
      );
      expect(space.defaultConfig.outputKey, 'posted_space_id');
    });

    test(
      'pipeline.condition has router kind',
      timeout: const Timeout.factor(2),
      () {
        final lib = defaultNodeTypeLibrary();
    ...
    test('human.gate has listen kind', () {
      ...
      expect(gate.defaultConfig.outputKey, 'approval_decision');
    });
    ...
    test('flow.callPipeline has listen kind and templateId extras', () {
      ...
      expect(call.defaultConfig.extras.containsKey('templateId'), isTrue);
    });
  });
}
