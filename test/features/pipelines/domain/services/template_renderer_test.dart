...
void main() {
  group('TemplateRenderer', () {
    ...
    test(
      'render substitutes bare key from state',
    ...
    test(
      'render substitutes bare key from trigger when not in state',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          '{{author}}',
    ...
    test(
      'state takes precedence over trigger for bare keys',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          '{{k}}',
    ...
    test(
      r'render substitutes $state.key',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          r'{{$state.path}}',
    ...
    test(
      r'render substitutes $trigger.key',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          r'{{$trigger.author}}',
    ...
    test(
      ...
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render('a={{a}} b={{b}}', state: {'a': 1});
    ...
    test(
      'multiple unresolved placeholders tracked',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render('{{x}} {{y}} {{z}}', state: {});
        expect(result.unresolved, {'x', 'y', 'z'});
      },
    );

    test(
      'placeholders with whitespace inside braces',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render('{{ name }}', state: {'name': 'Alice'});
        expect(result.text, 'Alice');
      },
    );

    test(
      'no placeholders returns original text',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render('no placeholders here', state: {});
    ...
    test(
      r'$state. prefix with missing key produces unresolved',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(r'Value: {{$state.missing}}', state: {});
    ...
    test(
      r'$trigger. prefix with trigger=null produces unresolved',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
    ...
    test(
      'dotted bare key treated as flat lookup (not nested)',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'renders numeric and boolean values',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
    ...
    test(
      'renders special characters in values',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          'msg={{msg}}',
    ...
    test(
      'complex template with mixed refs',
      timeout: const Timeout.factor(2),
      () {
        final result = renderer.render(
          r'PR #{{pr_number}} by {{$trigger.author}} at {{$state.path}}',
          state: {'pr_number': 42, 'path': '/repo'},
          trigger: {'author': 'octocat'},
        );
    ...
    test(
      'isComplete true only when no unresolved',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'escape is applied to every resolved value (shell-injection guard)',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'placeholders() extracts all refs',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'placeholders() with no placeholders returns empty',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'resolve returns null for missing keys',
      timeout: const Timeout.factor(2),
      () {
        expect(renderer.resolve('missing', state: {}, trigger: {}), isNull);
      },
    );

    test(
      r'resolve handles $state prefix',
      timeout: const Timeout.factor(2),
      () {
        expect(renderer.resolve(r'$state.key', state: {'key': 'val'}), 'val');
      },
    );

    test(
      r'resolve handles $trigger prefix',
      timeout: const Timeout.factor(2),
      () {
        expect(
    ...
    test(
      r'$state. prefix with missing key returns null',
      timeout: const Timeout.factor(2),
      () {
        expect(renderer.resolve(r'$state.nope', state: {}), isNull);
      },
    );

    test(
      r'$trigger. prefix with trigger=null returns null',
      timeout: const Timeout.factor(2),
      () {
        expect(renderer.resolve(r'$trigger.nope', state: {}), isNull);
      },
    );

    test(
      'bare key resolves from trigger when state absent',
      timeout: const Timeout.factor(2),
      () {
        expect(
    ...
    test(
      r'isTriggerScoped identifies $trigger refs',
      timeout: const Timeout.factor(2),
      () {
    ...
    test('stateKeyOf extracts state key', timeout: const Timeout.factor(2), () {
  ...
  group('RenderResult', () {
    test(
      'isComplete is true when unresolved is empty',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'isComplete is false when unresolved has entries',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'text and unresolved fields are accessible',
      timeout: const Timeout.factor(2),
      () {
