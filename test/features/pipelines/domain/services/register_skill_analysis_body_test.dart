...
void main() {
  ...
  test('analyzes the event payload slug', () async {
    ...
    final summary = result.mutatedState!['skillScanSummary']
        as Map<String, dynamic>;
    expect(summary['pass'], 0);
  });

  test('manual form skill_slug scopes the scan; empty scans everything', () async {
  ...
  test('records the tallies in state', () async {
    analysis.outcome = SkillAnalysisOutcome(
      results: [
        ...
        SkillAnalysisSkillResult(slug: 'b', verdict: SkillScanVerdict.quarantine),
      ],
    );
    final result = await registry.body(BuiltInBodyKeys.skillAnalysis)(
      ctx(payload: {}),
    );
    final summary = result.mutatedState!['skillScanSummary'] as Map;
  ...
  test('the seeded template wires the body key and output key', () {
    ...
    expect(scan.config.outputKey, 'skillScanSummary');
