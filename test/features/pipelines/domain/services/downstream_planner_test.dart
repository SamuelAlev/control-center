...
void main() {
  group('DownstreamPlan', () {
    test(
      'stores toSkip, toRun, terminalReached',
  ...
  group('planDownstream', () {
    test(
      'nothing completed, nothing skipped returns empty plan',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
    ...
    test(
      'trigger completed → listeners run',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'completed step + completed listener → terminal reached',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'router routes false → true branch skipped, false branch runs',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'skip propagates to descendants',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
          ...
          _step(
            ...
            StepKind.terminal,
            triggers: const [
              StepTrigger(sourceStepIds: ['a_child']),
            ],
          ),
        ]);
    ...
    test(
      'join fires when all waitForStepIds are completed or skipped',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'join is never killed by skip propagation',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'terminal reached only via completed branch, not skipped',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'existing steps are neither skipped nor run',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
    ...
    test(
      'multi-source trigger requires all sources completed',
      timeout: const Timeout.factor(2),
      () {
        final def = _def([
          _step('t', StepKind.trigger),
