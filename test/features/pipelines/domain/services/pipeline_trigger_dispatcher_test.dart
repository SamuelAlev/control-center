...
class _FakePipelineEngine implements PipelineEngine {
  ...
  @override
  AgentRunLogRepository get agentRunLogRepository => _stub();
  @override
  ...
  @override
  Set<String> get projectionTriggerEventTypes => _stub();
  ...
  @override
  Future<void> cancel(String workspaceId, String pipelineRunId) => _stub();
  @override
  ...
  @override
  Future<void> killStep(String workspaceId, String stepRunId) => _stub();
  @override
  Future<void> retry(String workspaceId, String pipelineRunId) => _stub();
  @override
...
void main() {
  ...
  group('PipelineTriggerDispatcher', () {
    ...
    test(
      'ignores events whose typeName is not in knownEventTypes',
    ...
    test(
      'calls triggerRepository.enabledForEvent with correct typeName',
      timeout: const Timeout.factor(2),
      () async {
        String? capturedType;
        triggerRepo = FakePipelineTriggerRepository(
    ...
    test(
      'filters triggers by workspaceId in payload (skips different workspace)',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'workspace-scoped filtering skips all triggers when workspaceId mismatches',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'calls trigger.matches(payload) and respects the result',
      timeout: const Timeout.factor(2),
      () async {
        // Trigger that matches only when status == 'done'
        final trigger = _trigger(match: {'status': 'done'});
    ...
    test(
      'trigger.matches returns true when match filter is empty',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'trigger.matches with list value respects contains check',
      timeout: const Timeout.factor(2),
      () async {
        // TicketAssigned always has 'ticket_title' in payload.
        // match looks for ticketTitle in a list.
        final trigger = _trigger(
          match: {
            'ticket_title': ['Test', 'Other'],
          },
        );
    ...
    test(
      ...
      () async {
        ...
        expect(
          engine.calls.map((c) => c.templateId),
    ...
    test(
      'passes correct parameters to engine.start',
      timeout: const Timeout.factor(2),
      () async {
        ...
        expect(payload['ticket_id'], 't-42');
        expect(payload['ticket_title'], 'Fix Bug');
        expect(payload['ticket_body'], 'body text');
        expect(payload['ticket_url'], 'https://example.com/t/42');
    ...
    test(
      'includes triggerPayload workspaceId overriding event workspaceId',
      timeout: const Timeout.factor(2),
      () async {
        ...
        // Actually: fullPayload = {...payload, 'workspace_id': trigger.workspaceId}
    ...
    test(
      'error in handler is caught and does not crash the stream',
      timeout: const Timeout.factor(2),
      () async {
        triggerRepo = FakePipelineTriggerRepository(
    ...
    test(
      'start() subscribes to DomainEventBus; events are dispatched',
      timeout: const Timeout.factor(2),
      () async {
        final trigger = _trigger();
    ...
    test(
      'dispose is safe to call without start',
      timeout: const Timeout.factor(2),
      () {
        // Should not throw.
        dispatcher.dispose();
