...
void main() {
  group('PipelineTrigger', () {
    ...
    test(
      'constructor asserts non-empty eventType and templateId',
    ...
    test(
      ...
      () {
        ...
        expect(
          trigger.createdAt.isAfter(
            before.subtract(const Duration(seconds: 1)),
          ),
          isTrue,
        );
        expect(
          trigger.createdAt.isBefore(after.add(const Duration(seconds: 1))),
          isTrue,
        );
      },
    );
    ...
    test(
      'matches returns true for empty filter',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'matches returns true when payload matches scalar filter',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'matches returns true when payload value is in list filter',
      timeout: const Timeout.factor(2),
      () {
        final trigger = trigger0(
          match: {
    ...
    test(
      'matches returns false when payload key is missing',
      timeout: const Timeout.factor(2),
      () {
        final trigger = trigger0(match: {'status': 'merged'});
    ...
    test(
      'matches with multiple filters — all must match',
      timeout: const Timeout.factor(2),
      () {
        final trigger = trigger0(
          match: {'status': 'merged', 'repo': 'acme/app'},
        );
        expect(
          trigger.matches({'status': 'merged', 'repo': 'acme/app'}),
          isTrue,
        );
        expect(trigger.matches({'status': 'merged', 'repo': 'other'}), isFalse);
      },
    );

    test(
      'intervalSeconds parses every:<seconds>',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'intervalSeconds returns null for non-interval expressions',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'equality compares id, eventType, templateId, workspaceId, enabled',
      timeout: const Timeout.factor(2),
      () {
        final a = trigger0();
        final b = trigger0();
    ...
    test(
      'equality ignores cronExpression, match, lastFiredAt',
      timeout: const Timeout.factor(2),
      () {
