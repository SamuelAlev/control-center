...
void main() {
  group('PipelineRunStatus', () {
    test('has seven values', timeout: const Timeout.factor(2), () {
      expect(PipelineRunStatus.values.length, 7);
      expect(
    ...
    test(
      ...
      () {
        ...
        expect(PipelineRunStatus.queued.isTerminal, isFalse);
    ...
    test(
      ...
      () {
        expect(
          PipelineRunStatus.fromString('pending'),
          PipelineRunStatus.pending,
        );
        expect(
          PipelineRunStatus.fromString('queued'),
          PipelineRunStatus.queued,
        );
        expect(
        ...
        expect(
          PipelineRunStatus.fromString('suspended'),
          PipelineRunStatus.suspended,
        );
        expect(
          PipelineRunStatus.fromString('completed'),
          PipelineRunStatus.completed,
        );
        expect(
          PipelineRunStatus.fromString('failed'),
          PipelineRunStatus.failed,
        );
        expect(
          PipelineRunStatus.fromString('cancelled'),
          PipelineRunStatus.cancelled,
        );
      },
    );
    ...
    test(
      'fromString defaults to pending for unknown values',
      timeout: const Timeout.factor(2),
      () {
        expect(
          PipelineRunStatus.fromString('unknown'),
          PipelineRunStatus.pending,
        );
    ...
    test(
      'fromString/toStorageString round-trip',
      timeout: const Timeout.factor(2),
      () {
        for (final status in PipelineRunStatus.values) {
          expect(
