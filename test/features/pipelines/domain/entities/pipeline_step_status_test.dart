...
void main() {
  group('PipelineStepStatus', () {
    ...
    test(
      'isTerminal is true for completed, failed, skipped, cancelled',
    ...
    test(
      'fromString parses all known values',
      timeout: const Timeout.factor(2),
      () {
        expect(
          PipelineStepStatus.fromString('pending'),
          PipelineStepStatus.pending,
        );
        expect(
          PipelineStepStatus.fromString('running'),
          PipelineStepStatus.running,
        );
        expect(
          PipelineStepStatus.fromString('suspended'),
          PipelineStepStatus.suspended,
        );
        expect(
          PipelineStepStatus.fromString('completed'),
          PipelineStepStatus.completed,
        );
        expect(
          PipelineStepStatus.fromString('failed'),
          PipelineStepStatus.failed,
        );
        expect(
          PipelineStepStatus.fromString('skipped'),
          PipelineStepStatus.skipped,
        );
        expect(
          PipelineStepStatus.fromString('cancelled'),
          PipelineStepStatus.cancelled,
        );
      },
    );
    ...
    test(
      'fromString defaults to pending for unknown',
      timeout: const Timeout.factor(2),
      () {
        expect(
          PipelineStepStatus.fromString('bogus'),
          PipelineStepStatus.pending,
        );
    ...
    test(
      'round-trip fromString/toStorageString',
      timeout: const Timeout.factor(2),
      () {
        for (final s in PipelineStepStatus.values) {
