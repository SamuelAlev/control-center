...
void main() {
  group('PipelineNodeConfig', () {
    test(
      'empty constant has all defaults',
    ...
    test(
      'toJson omits null/default fields',
      timeout: const Timeout.factor(2),
      () {
    ...
    group('copyWith', () {
      ...
      test(
        'preserves fields not overridden',
        timeout: const Timeout.factor(2),
        () {
          const config = PipelineNodeConfig(
            prompt: 'p',
    ...
    test(
      'equality compares all fields including deep collections',
      timeout: const Timeout.factor(2),
      () {
        const a = PipelineNodeConfig(inputKeys: ['a', 'b'], extras: {'x': 1});
        const b = PipelineNodeConfig(inputKeys: ['a', 'b'], extras: {'x': 1});
        expect(a, equals(b));
  ...
  group('StepRetryPolicy', () {
    ...
    test(
      'delayForAttempt with exponential backoff',
      timeout: const Timeout.factor(2),
      () {
        const policy = StepRetryPolicy(
          maxAttempts: 5,
    ...
    test(
      'delayForAttempt with linear backoff',
      timeout: const Timeout.factor(2),
      () {
        const policy = StepRetryPolicy(
          maxAttempts: 3,
