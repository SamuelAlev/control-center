...
void main() {
  group('PipelineInputType', () {
    ...
    test(
      'fromName defaults to text for null or unknown',
  ...
  group('PipelineInput', () {
    test(
      'constructor requires non-empty key',
      timeout: const Timeout.factor(2),
      () {
        expect(() => PipelineInput(key: ''), throwsA(isA<ArgumentError>()));
      },
    );

    test(
      'label defaults to key when not provided',
      timeout: const Timeout.factor(2),
      () {
        final input = PipelineInput(key: 'repo_full_name');
        expect(input.label, 'repo_full_name');
      },
    );

    test(
      'label defaults to key when empty string',
      timeout: const Timeout.factor(2),
      () {
        final input = PipelineInput(key: 'repo_full_name', label: '');
        expect(input.label, 'repo_full_name');
      },
    );

    test(
      'label uses provided value when non-empty',
      timeout: const Timeout.factor(2),
      () {
        final input = PipelineInput(key: 'repo_full_name', label: 'Repository');
