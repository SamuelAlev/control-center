...
void main() {
  group('planDownstream — routing & skip propagation', () {
    test(
      'router routes false → the "true" branch is skipped, "false" runs',
      () {
        final def = _def([
    ...
    test(
      'skip propagates to an unconditional descendant of a skipped node',
      () {
        final def = _def([
          _step('t', StepKind.trigger),
          _step(
    ...
    test('a join fires once every wait-for source is completed OR skipped', () {
      final def = _def([
        ...
        _step(
          ...
          StepKind.terminal,
          triggers: const [
            StepTrigger(sourceStepIds: ['consolidate']),
          ],
        ),
      ]);
    ...
    test('terminal is reached only via a branch that actually completed', () {
      ...
      final def = _def([
        ...
        _step(
          ...
          StepKind.listen,
          triggers: const [
            StepTrigger(sourceStepIds: ['docs']),
          ],
        ),
        _step(
          'sec_c',
