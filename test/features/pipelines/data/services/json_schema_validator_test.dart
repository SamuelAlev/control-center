  ...
  group('JsonSchemaValidator', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          errs.any((e) => e.contains('required property missing')),
          isTrue,
        );
      },
    );
    ...
    test('wrong type is reported', timeout: const Timeout.factor(2), () {
      final errs = validator.validate('hello', {'type': 'object'});
      expect(errs, isNotEmpty);
      expect(errs.any((e) => e.contains('expected object')), isTrue);
    });

    test(
      'type mismatch on nested property',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          {'count': 'not-a-number'},
    ...
    test('valid integer passes', timeout: const Timeout.factor(2), () {
      expect(validator.validate(42, {'type': 'integer'}), isEmpty);
    });
    ...
    test('valid array passes', timeout: const Timeout.factor(2), () {
      expect(validator.validate([1, 2, 3], {'type': 'array'}), isEmpty);
    });

    test(
      'enum constraint passes for valid value',
      timeout: const Timeout.factor(2),
      () {
        expect(
    ...
    test(
      'enum constraint fails for invalid value',
      timeout: const Timeout.factor(2),
      () {
    ...
    test('minLength constraint', timeout: const Timeout.factor(2), () {
      expect(
        validator.validate('ab', {'type': 'string', 'minLength': 3}),
        isNotEmpty,
      );
      expect(
        validator.validate('abc', {'type': 'string', 'minLength': 3}),
        isEmpty,
      );
    });
    ...
    test('minItems constraint', timeout: const Timeout.factor(2), () {
      expect(
        validator.validate([1], {'type': 'array', 'minItems': 2}),
        isNotEmpty,
      );
      expect(
        validator.validate([1, 2], {'type': 'array', 'minItems': 2}),
        isEmpty,
      );
    });
    ...
    test('nullable allows null', timeout: const Timeout.factor(2), () {
      expect(
        validator.validate(null, {'type': 'string', 'nullable': true}),
        isEmpty,
      );
    });

    test('non-nullable rejects null', timeout: const Timeout.factor(2), () {
      final errs = validator.validate(null, {'type': 'string'});
      expect(errs, isNotEmpty);
      expect(errs.any((e) => e.contains('got null')), isTrue);
    });

    test(
      'null without type constraint is still rejected (no nullable flag)',
      timeout: const Timeout.factor(2),
      () {
        expect(validator.validate(null, {}), isNotEmpty);
      },
    );

    test(
      'null type in schema matches everything',
      timeout: const Timeout.factor(2),
      () {
        expect(validator.validate('anything', {'type': 'null'}), isNotEmpty);
      },
    );
    ...
    test(
      'value without schema type passes',
      timeout: const Timeout.factor(2),
      () {
        expect(validator.validate('anything', <String, dynamic>{}), isEmpty);
      },
    );

    test(
      'handles schema error gracefully',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'required property with null value is reported',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          {'name': null},
    ...
    test(
      'empty object with no required passes',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'empty array with minItems fails',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate([], {'type': 'array', 'minItems': 1});
        expect(errs, isNotEmpty);
        expect(
          errs.any((e) => e.contains('expected at least 1 items')),
          isTrue,
        );
      },
    );
    ...
    test(
      'nullable: true without type allows null',
      timeout: const Timeout.factor(2),
      () {
    ...
    test('minLength exact match passes', timeout: const Timeout.factor(2), () {
      final errs = validator.validate('ab', {'type': 'string', 'minLength': 2});
      expect(errs, isEmpty);
    });

    test('minItems exact match passes', timeout: const Timeout.factor(2), () {
      final errs = validator.validate([1, 2], {'type': 'array', 'minItems': 2});
      expect(errs, isEmpty);
    });

    test(
      'nullable on nested property allows null',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          {'nested': null},
    ...
    test(
      'nullable on non-null value has no effect',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          {'x': 'hello'},
    ...
    test(
      'enum with non-list value is ignored',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'unknown type string passes any value',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'nested array with item validation',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          [
    ...
    test(
      'object with extra properties not in schema passes',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
    ...
    test(
      'nullable without type constraint allows null',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'multiple nested violations accumulate path correctly',
      timeout: const Timeout.factor(2),
      () {
        final errs = validator.validate(
          {
