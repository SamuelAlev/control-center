...
void main() {
  ...
  final credsRepo = _FakeCredsRepo(
    const ApiCredentials(githubToken: 'gh_token'),
  );
  ...
  group('validation', () {
    test('fails when step config is missing', () async {
      registry = PipelineBodyRegistry();
      final templateRepo = _FakeTemplateRepo(
        _def('step-1', const PipelineNodeConfig(script: 'echo ok')),
      );
      registerBashScriptBody(
        registry,
        templateRepository: templateRepo,
      ...
      final body = registry.body(BuiltInBodyKeys.bashScript);
      final result = await body(_ctx(stepId: 'nonexistent', state: {}));
      expect(result.isFailed, isTrue);
      expect(result.errorMessage, contains('missing config'));
    });

    test('fails when script is null', () async {
      final result = await runWith(config: const PipelineNodeConfig());
      expect(result.isFailed, isTrue);
  ...
  group('dry run', () {
    ...
    test('returns ok with no mutatedState when outputKey is empty', () async {
      final result = await runWith(
        config: const PipelineNodeConfig(script: 'echo hello', outputKey: ''),
        dryRun: true,
      );
      expect(result.isFailed, isFalse);
      expect(result.isSuspended, isFalse);
    ...
    test(
      'unresolved placeholders render as empty and dry-run still succeeds',
      () async {
        final result = await runWith(
          config: const PipelineNodeConfig(
