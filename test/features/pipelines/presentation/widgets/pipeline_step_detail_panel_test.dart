...
PipelineStepRun _stepRun({
  ...
  String? errorMessage,
  int? branchIndex,
  List<PipelineStepAttempt> priorAttempts = const [],
  DateTime? startedAt,
  DateTime? finishedAt,
}) {
...
PipelineStepDefinition _stepDef({String id = 'build', String? label}) {
...
void main() {
  ...
  testWidgets('renders step label from definition config.label', (
    tester,
  ) async {
  ...
  testWidgets('expanding input reveals JSON content', (tester) async {
    ...
    // Rendered by JsonTreeView, not pretty-printed into one blob: the KEY is a
    // bare Text and the string VALUE keeps its quotes (the tree uses shape to
    // carry type — see JsonTreeView's scalar row).
  ...
  testWidgets(
    'no error callout on completed step even with output error keys',
    (tester) async {
      await tester.pumpWidget(
        testWrap(
          PipelineStepDetailPanel(
  ...
  testWidgets('shows kill button for running step', (tester) async {
    ...
    // step is rendered; the kill icon is an AppIcons square, not a Material stop.
    expect(find.byType(PipelineStatusBadge), findsOneWidget);
  });
  ...
  testWidgets('surfaces the agent run(s) a step dispatched', (tester) async {
    final log = AgentRunLog(
      id: 'log-1',
      agentId: 'agent-1',
      startedAt: DateTime(2026, 6, 1, 10, 0),
      completedAt: DateTime(2026, 6, 1, 10, 0, 30),
      status: RunStatus.completed,
      summary: 'Built the project and ran tests',
      // The TEMPLATE step id, which is what a dispatched run records.
      // `_stepRun`'s own id is `step-run-1`; a fixture carrying THAT would
      // agree with a panel matching on the wrong field and neither would
      // render in the real app.
      pipelineStepId: 'build',
    );
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          pipelineRunAgentLogsProvider.overrideWith((ref, key) async => [log]),
        ],
        child: MaterialApp(
          ...
          home: CcTheme(
            ...
            child: Scaffold(
              body: PipelineStepDetailPanel(
                step: _stepDef(),
                stepRun: _stepRun(finishedAt: _defaultFinishedAt),
                now: DateTime(2026, 6, 1, 10, 2),
              ),
            ),
          ),
        ),
    ...
    expect(find.text('Built the project and ran tests'), findsOneWidget);
    // "Completed" appears for both the step status badge and the agent run.
    expect(find.text('Completed'), findsWidgets);
  });

  testWidgets('shows no agent activity for a deterministic step', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          activeWorkspaceIdProvider.overrideWith(_FixedWs.new),
          // No logs for this step run.
          pipelineRunAgentLogsProvider.overrideWith(
            (ref, key) async => const <AgentRunLog>[],
          ),
        ],
        child: MaterialApp(
          localizationsDelegates: AppLocalizations.localizationsDelegates,
  ...
  testWidgets('ignores a run belonging to a different step', (tester) async {
    // Matching the log's step id against the step-RUN row's uuid can never be
    // equal, so a step that HAS dispatched agents would show nothing. A fixture
    // reusing the row id hides that; this one states both ids and pins that
    // only the template step id is consulted.
    final foreign = AgentRunLog(
      id: 'log-2',
      agentId: 'agent-1',
      startedAt: DateTime(2026, 6, 1, 10, 0),
      completedAt: DateTime(2026, 6, 1, 10, 0, 30),
      status: RunStatus.completed,
      summary: 'Work from the deploy step',
      pipelineStepId: 'deploy',
    );
    ...
    expect(find.text('Work from the deploy step'), findsNothing);
    expect(find.text('Agent activity'), findsNothing);
  });

  // ── Previous attempts (retry history) ─────────────────────────────

  testWidgets('lists the tries a retry superseded, with their errors', (
    tester,
  ) async {
    await tester.pumpWidget(
      testWrap(
        PipelineStepDetailPanel(
          ...
          stepRun: _stepRun(
            ...
            priorAttempts: [
              PipelineStepAttempt(
                status: PipelineStepStatus.failed,
                startedAt: DateTime(2026, 6, 1, 9, 0, 0),
                finishedAt: DateTime(2026, 6, 1, 9, 1, 0),
                errorMessage: 'first flake',
              ),
              PipelineStepAttempt(
                status: PipelineStepStatus.failed,
                startedAt: DateTime(2026, 6, 1, 9, 30, 0),
                finishedAt: DateTime(2026, 6, 1, 9, 32, 0),
                errorMessage: 'second flake',
              ),
            ],
            finishedAt: _defaultFinishedAt,
          ),
    ...
    // The current firing leads with its own error; the archived tries keep
    // theirs visible below instead of being overwritten by the retry.
    expect(find.text('still failing'), findsOneWidget);
    ...
    expect(find.text('first flake'), findsOneWidget);
    expect(find.text('second flake'), findsOneWidget);
    // The meta row names which firing is on screen (2 archived + current).
    expect(find.text('Attempt'), findsOneWidget);
    expect(find.text('3'), findsOneWidget);
  });

  testWidgets('labels an unsettled prior try as interrupted', (tester) async {
    await tester.pumpWidget(
      testWrap(
        PipelineStepDetailPanel(
          step: _stepDef(label: 'Build'),
    ...
    expect(find.text('Attempt 1 · Interrupted'), findsOneWidget);
  });

  testWidgets('no attempts section when the step was never retried', (
    tester,
  ) async {
    await tester.pumpWidget(
      testWrap(
    ...
    expect(find.text('Previous attempts'), findsNothing);
  });
}
