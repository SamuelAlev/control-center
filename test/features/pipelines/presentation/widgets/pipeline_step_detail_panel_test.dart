...
void main() {
  ...
  testWidgets('no kill button for terminal completed step', (tester) async {
    ...
    expect(find.byType(PipelineStatusBadge), findsOneWidget);
    // FButton appears only for close and kill. With onClose null and non-live,
    // zero FButton widgets.
  ...
  testWidgets('no kill button for terminal failed step', (tester) async {
    ...
    // With onClose null and non-live status, no FButton icons at all.
    expect(find.byType(FButton), findsNothing);
  });
  ...
  testWidgets('close button fires onClose callback', (tester) async {
    ...
    // The close button wraps an FButton.icon which contains an FTappable.
    // The FTappable starts a 100ms timer on tap; we advance past it.
