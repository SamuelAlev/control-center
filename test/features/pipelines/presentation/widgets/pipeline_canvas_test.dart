...
void main() {
  ...
  group('empty canvas', () {
    testWidgets('shows placeholder when definition has no steps', (
      tester,
    ) async {
    ...
    testWidgets('shows placeholder when all steps are terminal', (
      tester,
    ) async {
      final def = _definition([_step(id: 'end', kind: StepKind.terminal)]);
      await pumpCanvas(tester, def);
      expect(find.text(l10n.pipelinesNoSteps), findsOneWidget);
    });

    testWidgets('shows placeholder when only terminal steps exist', (
      tester,
    ) async {
      final def = _definition([
  ...
  group('nodes', () {
    ...
    testWidgets('renders node label from config, falling back to id', (
      tester,
    ) async {
      final def = _definition([
    ...
    testWidgets('nodes use Positioned with coordinates when provided', (
      tester,
    ) async {
      final def = _definition([
    ...
    testWidgets('trigger node uses brand colors', (tester) async {
      final def = _definition([_step(id: 'trigger', kind: StepKind.trigger)]);
      await pumpCanvas(tester, def);
  ...
  group('connections', () {
    testWidgets('renders CustomPaint for edge/background layers', (
      tester,
    ) async {
      final def = _definition([
  ...
  group('interaction without runId', () {
    testWidgets('tapping a node does not open detail panel', (tester) async {
      ...
      expect(find.byType(PipelineStepDetailPanel), findsNothing);
    });

    testWidgets('detail panel sidebar is absent when runId is null', (
      tester,
    ) async {
      final def = _definition([_step(id: 'start', kind: StepKind.trigger)]);
      await pumpCanvas(tester, def);
      expect(find.byType(PipelineStepDetailPanel), findsNothing);
      // Without a panel there is no seam to drag, so the graph is not split.
  ...
  group('interaction with runId', () {
    testWidgets('tapping a node selects it and shows detail panel', (
      tester,
    ) async {
      final def = _definition([
        _step(id: 'start', kind: StepKind.trigger, label: 'Start'),
      ...
      await pumpCanvas(
        ...
        runId: 'run-1',
        overrides: [
          pipelineStepRunsForRunProvider(
            'run-1',
          ).overrideWith((_) => Stream.value(const <PipelineStepRun>[])),
        ],
      );
      ...
      expect(find.byType(PipelineStepDetailPanel), findsOneWidget);
      // The panel docks as a resizable region, not a fixed-width slab.
    ...
    testWidgets('nodes show status colors when step runs are present', (
      tester,
    ) async {
      const stepId = 'stepA';
      final def = _definition([
    ...
    testWidgets('running step shows spinner', (tester) async {
      ...
      // A running step renders a CcSpinner as its glyph
    ...
    testWidgets('resets selected step when runId changes', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: testWrap(PipelineCanvas(definition: def, runId: 'run-2')),
        ),
      );
      await tester.pump(const Duration(milliseconds: 200));
  ...
  group('semantics', () {
    testWidgets('nodes have button semantics when runId is set', (
      tester,
    ) async {
      final def = _definition([
        _step(id: 'start', kind: StepKind.trigger, label: 'Start'),
    ...
    testWidgets('selected node has selected=true semantics', (tester) async {
      ...
      // The selected node and potentially its CcTappable wrapper both have selected=true
  ...
  group('focus and keyboard', () {
    testWidgets('Focus widget is present with autofocus', (tester) async {
      ...
      final canvasFocus = find.byWidgetPredicate(
        (w) =>
            w is Focus && w.autofocus == true && w.child is CallbackShortcuts,
      );
  ...
  group('node labels', () {
    testWidgets('resolves a label template against the run', (tester) async {
      // Node labels are authored as templates. A run's canvas has the values,
      // so it shows the step's name — not the braces.
      final def = _definition([
        _step(id: 'start', kind: StepKind.trigger, label: 'Trigger'),
        _step(
          id: 'space',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            pipelineRunProvider('run-1').overrideWith(
              (_) => Stream.value(
                PipelineRun(
                  id: 'run-1',
                  ...
                  state: const {'repo_name': 'web-app'},
                  startedAt: DateTime(2026),
                ),
              ),
            ),
          ],
          child: testWrap(PipelineCanvas(definition: def, runId: 'run-1')),
        ),
      );
      await tester.pump(const Duration(milliseconds: 200));

      expect(find.text('Code analysis · web-app'), findsOneWidget);
      expect(find.textContaining('{{'), findsNothing);
    });

    testWidgets('drops the separator when the value is absent', (tester) async {
      final def = _definition([
        _step(id: 'start', kind: StepKind.trigger, label: 'Trigger'),
        _step(
          id: 'space',
