...
PipelineStepDefinition _step({
  ...
  return PipelineStepDefinition(
    ...
    kind: kind,
    bodyKey: 'body_$id',
    triggers: after.isEmpty ? const [] : [StepTrigger(sourceStepIds: after)],
...
Map<String, Offset> _layout(List<PipelineStepDefinition> steps) =>
    PipelineGraphLayout.compute(
      steps,
      // Uniform width: adaptive-width behaviour has its own group below.
      nodeWidths: {for (final s in steps) s.id: _nodeWidth},
...
void _expectNoOverlaps(Map<String, Offset> positions) {
  ...
  for (var i = 0; i < entries.length; i++) {
    for (var j = i + 1; j < entries.length; j++) {
      ...
      final overlaps =
          intersection.width > 0.001 && intersection.height > 0.001;
      expect(
        overlaps,
        isFalse,
...
void main() {
  group('PipelineGraphLayout', () {
    ...
    test(
      'ignores overlapping stored coordinates and recomputes a clean layout',
      () {
    ...
    test('a wide node widens only its own column', () {
      const wideWidth = 360.0;
      final positions = PipelineGraphLayout.compute(
        [
          _step(id: 'trigger', kind: StepKind.trigger),
          _step(id: 'wide', kind: StepKind.listen, after: ['trigger']),
          _step(id: 'tail', kind: StepKind.listen, after: ['wide']),
        ],
        nodeWidths: const {
          'trigger': _nodeWidth,
          'wide': wideWidth,
      ...
      // Column x offsets accumulate: the next column starts right after the
      // widest tile of the previous one (+ the inter-column gap), so the wide
      // node pushes only its own column's successors out.
      expect(positions['trigger']!.dx, 0);
      expect(positions['wide']!.dx, _nodeWidth + PipelineGraphLayout.columnGap);
      expect(
        positions['tail']!.dx,
        _nodeWidth +
            PipelineGraphLayout.columnGap +
            wideWidth +
            PipelineGraphLayout.columnGap,
      );
    });
