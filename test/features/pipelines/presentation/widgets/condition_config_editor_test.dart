...
/// Enters text into the field whose label contains [labelText] by locating its
/// inner [EditableText] and using the test IME.
///
/// The editor lays each field out as a `[label Text, control]` sibling column
/// (see `_Labeled`), so the control is *not* an ancestor/descendant of the
/// label. We instead resolve the label's nearest ancestor [Column] — the
/// per-field group — and grab the single [EditableText] it contains. This works
/// uniformly for both `CcTextField` and `CcTextArea` controls.
...
void main() {
  group('ConditionConfigEditor', () {
    ...
    testWidgets('filesAny emits correct predicate shape', (tester) async {
      ...
      expect(p['baseKey'], 'repo_local_path');
    });
    ...
    testWidgets('legacy comparison (top-level left/op/right)', (tester) async {
      await _pumpEditor(
        tester,
        extras: {'left': '{{value}}', 'op': 'lt', 'right': '50'},
      );
      expect(find.textContaining('Left value'), findsOneWidget);
      expect(find.textContaining('Right value'), findsOneWidget);
    });
