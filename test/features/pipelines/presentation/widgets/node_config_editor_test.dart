...
void main() {
  group('NodeConfigEditor', () {
    ...
    testWidgets('repo selector renders for agent nodes, hidden for bash', (
      tester,
    ) async {
      final repos = [_repo('r-1'), _repo('r-2')];
      final agentStep = _step(bodyKey: 'pipeline.promptAgent');
      await _pumpEditor(
        tester,
        NodeConfigEditor(
          step: agentStep,
    ...
    testWidgets('placeholder entries are preserved and shown as dynamic', (
      ...
      PipelineStepDefinition? emitted;
      final step = _step(
        config: const PipelineNodeConfig(repoIds: ['{{repoId}}']),
      );
      await _pumpEditor(
        tester,
        NodeConfigEditor(
      ...
      expect(find.textContaining('{{repoId}}'), findsOneWidget);

      // Editing the picker keeps the placeholder in the emitted scope.
      select.onChanged({'r-1'});
      expect(emitted!.config.repoIds, ['{{repoId}}', 'r-1']);
    });
  });
}
