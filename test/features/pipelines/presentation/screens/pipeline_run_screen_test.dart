...
Widget _wrap({
  ...
  final overrides = [
    manuallyRunnablePipelinesProvider('ws-1').overrideWith((ref) => pipelines),
  ...
  group('PipelineRunScreen', () {
    ...
    testWidgets('shows a loading indicator while data is loading', (
      ...
      await tester.pumpWidget(
        _wrap(pipelines: const AsyncValue.loading(), workspaceId: 'ws-1'),
      );
    ...
    testWidgets('shows no-active-workspace message when workspaceId is null', (
      ...
      await tester.pumpWidget(_wrap(pipelines: const AsyncValue.loading()));
      await tester.pump();
    ...
    testWidgets('shows empty state when no pipelines are runnable', (
      ...
      await tester.pumpWidget(
        _wrap(pipelines: const AsyncValue.data([]), workspaceId: 'ws-1'),
      );
    ...
    testWidgets('renders pipeline list, run form, and run button', (
      ...
      await tester.pumpWidget(
        _wrap(pipelines: AsyncValue.data([_pipeline()]), workspaceId: 'ws-1'),
      );
    ...
    testWidgets('renders multiple pipelines in the sidebar', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(pipelines: AsyncValue.data(pipelines), workspaceId: 'ws-1'),
      );
    ...
    testWidgets(
      'pre-selects first pipeline when initialTemplateId is unknown',
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            pipelines: AsyncValue.data([
    ...
    testWidgets('shows singular input count', (tester) async {
      await tester.pumpWidget(
        _wrap(
          pipelines: AsyncValue.data([
            _pipeline(
              inputs: [PipelineInput(key: 'x', label: 'X')],
            ),
          ]),
          workspaceId: 'ws-1',
        ),
    ...
    testWidgets('shows pipeline description in sidebar tile only', (
      ...
      await tester.pumpWidget(
        _wrap(
          pipelines: AsyncValue.data([
            _pipeline(name: 'My Pipe', description: 'Does something useful'),
          ]),
          workspaceId: 'ws-1',
        ),
      );
    ...
    testWidgets('Run button enables for negative number input', (tester) async {
      await tester.pumpWidget(
        _wrap(
          pipelines: AsyncValue.data([
            _pipeline(
    ...
    testWidgets('Run button enables for decimal number input', (tester) async {
      await tester.pumpWidget(
        _wrap(
          pipelines: AsyncValue.data([
            _pipeline(
    ...
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          _wrap(pipelines: AsyncValue.data([_pipeline()])),
        );
        await tester.pump();
