...
void main() {
  group('PipelineRunScreen', () {
    ...
    testWidgets('Run button enabled state — required field empty vs filled',
      ...
      final runFinder = find.widgetWithText(FButton, 'Run pipeline');
      expect(tester.widget<FButton>(runFinder).onPress, isNull);
      ...
      // Now enabled.
      expect(tester.widget<FButton>(runFinder).onPress, isNotNull);
    });
    ...
    testWidgets('renders input placeholder text', (tester) async {
      ...
      expect(field.decoration?.hintText, 'user@example.com');
    });
    ...
    testWidgets('renders and validates form with mixed input types',
      ...
      // Two TextFields (text + number) rendered.
    ...
    testWidgets('select dropdown shows all options', (tester) async {
      ...
      // Tap the FSelect to open the dropdown.
      await tester.tap(find.byWidgetPredicate(
        (w) => '${w.runtimeType}'.startsWith('_BasicSelect'),
      ));
