...
void main() {
  group('PipelineTemplatesSettingsScreen', () {
    ...
    testWidgets('the new-template dialog offers the run-concurrency cap', (
      tester,
    ) async {
      // Capping at creation is what lets a pipeline that must not fan out (an
      // indexer, a deploy) exist capped instead of being capped only after its
      // first uncapped runs.
      await tester.pumpWidget(
      ...
      expect(
        find.text(
          'Leave empty for unlimited. Extra runs wait in a queue and start as '
          'slots free up.',
        ),
        findsOneWidget,
      );
