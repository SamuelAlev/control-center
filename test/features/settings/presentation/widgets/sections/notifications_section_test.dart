...
class _FakeNotificationPreferences implements NotificationPreferencesPort {
  ...
  final Set<String> mutedRepos = {};

  @override
  Future<Set<String>> getMutedRepos() async => mutedRepos;
  ...
  Future<void> setRepoMuted(
    ...
    if (muted) {
      mutedRepos.add(repoFullName.toLowerCase());
    } else {
      mutedRepos.remove(repoFullName.toLowerCase());
    }
  }
...
void main() {
  group('NotificationsSection rendering', () {
    ...
    testWidgets('renders category rows when enabled', (tester) async {
      ...
      expect(find.byIcon(AppIcons.bot), findsOneWidget);
    ...
    testWidgets('renders one switch per notification category', (tester) async {
      await tester.pumpWidget(_buildSection());
      await tester.pump();

      // Derived from the enum rather than hardcoded: adding a category should
      // not require editing a magic number here (this assertion drifted once
      // already). The two extras are the global enable and quiet-hours toggles.
      const extras = 2;
      expect(
        find.byType(CcSwitch),
        findsNWidgets(NotificationCategory.values.length + extras),
      );
    });
    ...
    testWidgets('category titles render', (tester) async {
      ...
      // Every category must render a title — no silent blank row when one is
      // added. ('External PRs' used to be asserted here but no such category
      // exists; `triggerEventExternalPr` is a pipeline-trigger label.)
      ...
      expect(find.text('Mentioned in pull request'), findsOneWidget);
      expect(find.text('Review requested'), findsOneWidget);
      expect(find.text('Meeting starting soon'), findsOneWidget);
      expect(find.text('Calendar disconnected'), findsOneWidget);
      // Title and subtitle share a key for these two, hence findsWidgets.
  ...
  group('Toggle interactions', () {
    ...
    testWidgets('toggling quiet hours on calls setQuietHours', (tester) async {
