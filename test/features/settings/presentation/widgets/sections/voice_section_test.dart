...
/// [ModelControl] whose [watch] never emits, so the section must render from
/// [status] or it stays on the spinner forever.
class _HangingWatchControl implements SelectableModelControl {
  _HangingWatchControl(this._snapshot, this._watch);

  final ModelStatusSnapshot _snapshot;
  final Stream<ModelStatusSnapshot> _watch;

  @override
  Future<ModelStatusSnapshot> status() async => _snapshot;

  @override
  Stream<ModelStatusSnapshot> watch() => _watch;
  ...
  @override
  Future<ModelStatusSnapshot> select(String modelId) async => _snapshot;
}
...
void main() {
  group('VoiceSection', () {
    ...
    group('status seed', () {
      testWidgets(
        'shows not-installed when watch never emits (no infinite spinner)',
        (tester) async {
          final hanging = StreamController<ModelStatusSnapshot>();
          addTearDown(hanging.close);
          final control = _HangingWatchControl(
            const ModelStatusSnapshot(
              status: ModelLifecycleStatus.notInstalled,
            ),
            hanging.stream,
    ...
    group('AudioInputRow', () {
      // The row lives on You → Audio now (the server voice page is
      // models-only), inside the merged audio-devices panel.
      Widget audioSectionWithDevices(
        ...
        return ProviderScope(
          ...
          child: testWrap(const AudioInputRow()),
        );
      }
