...
/// A RemoteRpcClient stand-in that answers nothing — the fake control below
/// overrides every method that would dial it.
class _NoopRpcClient implements RemoteRpcClient {
  @override
  dynamic noSuchMethod(Invocation invocation) => super.noSuchMethod(invocation);
}

/// Configurable stand-in for the installed-skills antivirus control: tests set
/// the fields, the UI drives the overrides. When [filesystem] is set, a
/// 'saved' saveLocal ALSO writes through to it so the CRUD tests can keep
/// asserting file content (mirroring what the real server does).
class FakeSkillSecurityControl extends RpcSkillSecurityControl {
  FakeSkillSecurityControl() : super(_NoopRpcClient());

  /// Optional write-through target for successful saves.
  FakeFilesystemPort? filesystem;

  /// Queued results for saveLocal, popped per call (last one repeats).
  final List<SkillSaveResult> saveResults = [];
  SkillSaveResult lastSaveResult = const SkillSaveResult(
    status: 'saved',
    slug: '',
  );
  bool? lastAllowQuarantineOverride;

  /// Per-slug scan reports for scanInstalled (default: a clean pass).
  final Map<String, SkillScanReport> scanReports = {};
  SkillScanReport fallbackReport = const SkillScanReport(
  ...
  /// The result the batch analyze call returns (default: nothing scanned).
  SkillAnalyzeSummary analyzeSummary = const SkillAnalyzeSummary(
    passCount: 0,
    warnCount: 0,
    quarantineCount: 0,
    results: [],
  ...
  Future<SkillScanReport> scanInstalled(
    ...
    String slug, {
    bool? llmReview,
  }) async => scanReports[slug] ?? fallbackReport;
  ...
  Future<SkillSaveResult> saveLocal(
    ...
    if (saveResults.isNotEmpty) {
      lastSaveResult = saveResults.length == 1
          ? saveResults.first
          : saveResults.removeAt(0);
    }
    if (lastSaveResult.status == 'saved') {
      await filesystem?.writeSkillFile(workspaceId, slug, content);
    }
...
late FakeSkillSecurityControl securityControl;

void main() {
  setUp(() async {
    TestWidgetsFlutterBinding.ensureInitialized();
    prefs = AppPreferences.inMemory();
    securityControl = FakeSkillSecurityControl();
  });
  ...
  List baseOverrides() {
    return [
      ...
      skillSecurityControlProvider.overrideWithValue(securityControl),
    ];
  }
    ...
    return ProviderScope(
      ...
      child: CcTheme(
        ...
        child: MaterialApp(
          ...
          home: CcToastScope(child: Scaffold(body: child)),
        ),
      ),
    );
  }
  ...
  group('SkillsSettings rendering', () {
    testWidgets('renders no workspace selected state', (tester) async {
      setView(tester);
      await tester.pumpWidget(wrap(const SkillsSettings()));
      await tester.pump();
        ...
        ProviderScope(
          overrides: [
            ...
            skillListProvider.overrideWith(
              (ref, workspaceId) => completer.future,
            ),
    ...
    testWidgets('New button is tappable', (tester) async {
      ...
      final newBtn = find.text('New skill');
  ...
  group('SkillsSettings with mocked agents', () {
    ...
    testWidgets('renders editor area when isNew', (tester) async {
      ...
      final newButton = find.widgetWithText(CcButton, 'New skill');
      expect(newButton, findsOneWidget);
      await tester.tap(newButton);
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));

      // Two now: the header's action and the heading of the pane it opened.
      expect(find.text('New skill'), findsNWidgets(2));
    });
  });
  ...
  group('SkillsSettings with skill data', () {
    ...
    testWidgets('filter skills by name', (tester) async {
      ...
      // Type filter text
      await tester.enterText(find.byType(CcTextField).first, 'test');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('filter skills by description', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).first, 'review');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('filter with no matches shows "No matches."', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).first, 'nonexistent');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('selecting a different skill switches editor', (tester) async {
      ...
      expect(find.text('Delete'), findsOneWidget);
      // The editor has filter + name + description = 3 CcTextFields, plus the
      // body which is a CcTextArea.
      expect(find.byType(CcTextField), findsNWidgets(3));
    ...
    testWidgets('New button creates editor with cleared form', (tester) async {
      ...
      final newBtn = find.widgetWithText(CcButton, 'New skill');
      await tester.tap(newBtn);
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      // The header's action plus the heading of the pane it opened.
      expect(find.text('New skill'), findsNWidgets(2));
    ...
    testWidgets('save button disabled when form not dirty', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(1), 'modified');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
    ...
    testWidgets('save validates empty name', (tester) async {
      ...
      // Type something in body field (the body is rendered as a CcTextArea).
      final bodyField = find.byType(CcTextArea);
      await tester.enterText(bodyField, 'body content');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      // Try to save with empty name
      final saveBtn = find.widgetWithText(CcButton, 'Save');
    ...
    testWidgets('no agents shows placeholder message in editor', (
      tester,
    ) async {
      setView(tester);
      await tester.pumpWidget(
    ...
    testWidgets('save produces success snackbar', (tester) async {
      ...
      // New skill
      await tester.tap(find.widgetWithText(CcButton, 'New skill'));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      // Fill name (index 1, after filter at 0)
      await tester.enterText(find.byType(CcTextField).at(1), 'my-skill');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
  ...
  group('SkillsSettings CRUD operations', () {
    ...
    testWidgets('delete shows confirmation dialog', (tester) async {
      ...
      // Tap delete
      final deleteBtn = find.widgetWithText(CcButton, 'Delete');
    ...
    testWidgets('cancel in delete dialog does not delete', (tester) async {
      ...
      await tester.tap(find.widgetWithText(CcButton, 'Delete'));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
    ...
    testWidgets('save creates skill then shows success snackbar', (
      ...
      await tester.enterText(find.byType(CcTextField).at(1), 'my-new-skill');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
  ...
  group('_SkillsBodyState operations', () {
    ...
    testWidgets('save with description updates frontmatter', (tester) async {
      ...
      final content = fs.files['/fake/ws-1/skills/my-skill/SKILL.md'];
  ...
  group('SkillsSettings CRUD - delete confirm', () {
    ...
    testWidgets('confirming delete dialog removes skill from filesystem', (
      ...
      final deleteButtons = find.widgetWithText(CcButton, 'Delete');
  ...
  group('SkillsSettings CRUD - update description', () {
    ...
    testWidgets('editing description saves new description to file', (
      ...
      final content = fs.files['/fake/ws-1/skills/review/SKILL.md'];
      expect(content, isNotNull);
  ...
  group('SkillsSettings CRUD - update body content', () {
    ...
    testWidgets('editing body saves new markdown content to file', (
      ...
      // Edit body field (the body is rendered as a CcTextArea).
      await tester.enterText(
        find.byType(CcTextArea),
  ...
  group('SkillsSettings CRUD - very long skill name', () {
    testWidgets('saving a 100+ char name succeeds and creates file', (
      ...
      const slugified =
          'a-very-long-skill-name-that-exceeds-one-hundred-characters-to-test-the-save-handling-of-unusually-long-input';
      expect(
        fs.files.containsKey('/fake/ws-1/skills/$slugified/SKILL.md'),
        isTrue,
      );
    });
  });
  ...
  group('SkillsSettings CRUD - very long body content', () {
    testWidgets('pasting large markdown body saves successfully', (
      ...
      await tester.enterText(find.byType(CcTextField).at(1), 'large-skill');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      await tester.enterText(find.byType(CcTextArea), largeBody);
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      // The detail pane scrolls; after entering a large body the Save button
      // at the top may be off-screen. Scroll the detail's SingleChildScrollView
      // back to the top so the Save button is visible.
      final saveButton = find.widgetWithText(CcButton, 'Save');
      await tester.drag(
      ...
      expect(find.textContaining('saved'), findsOneWidget);
      final content = fs.files['/fake/ws-1/skills/large-skill/SKILL.md'];
      expect(content, isNotNull);
  ...
  group('SkillsSettings filter - case insensitive', () {
    ...
    testWidgets('uppercase filter matches lowercase skill names', (
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'CODE');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
  ...
  group('SkillsSettings filter - clear filter', () {
    ...
    testWidgets('clearing filter shows all skills again', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'test');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
      ...
      // Clear the filter
      await tester.enterText(find.byType(CcTextField).at(0), '');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
  ...
  group('SkillsSettings antivirus surface', () {
    Widget harness(
      WidgetTester tester,
      List overrides, {
      FakeSkillSecurityControl? control,
    }) {
      setView(tester);
      return ProviderScope(
        overrides: [
          ...
          // Exactly ONE override of the control provider (Riverpod forbids
          // duplicates): the test-specific control or the shared default.
          skillSecurityControlProvider.overrideWithValue(
    ...
    List standardOverrides(List<SkillInfo> skills) {
      return [
        skillListProvider.overrideWith((ref, workspaceId) async => skills),
        activeWorkspaceIdProvider.overrideWith(
          () => _TestActiveWorkspaceNotifier('ws-1'),
    ...
    testWidgets('tile shows the scan verdict badge and provenance meta', (
      tester,
    ) async {
      await tester.pumpWidget(
        harness(
          ...
          standardOverrides([
            ...
            const SkillInfo(
              ...
              lockState: InstalledSkillLockState.drifted,
            ...
            const SkillInfo(
              ...
              lockState: InstalledSkillLockState.unmanaged,
            ),
          ]),
        ),
      );
      ...
      // Unscanned skill: no badge, but the provenance meta says so (the meta
      // renders as one joined line, so match by substring).
      expect(find.textContaining('Not scanned'), findsOneWidget);
      expect(find.textContaining('Modified since install'), findsOneWidget);
    });

    testWidgets('scan button scans the selected skill and shows the report', (
      tester,
    ) async {
      final control = FakeSkillSecurityControl();
      control.scanReports['target'] = const SkillScanReport(
        verdict: SkillScanVerdict.warn,
        llmReviewed: true,
        capabilities: ['Bash'],
        requiredActionClasses: ['processSpawn'],
        findings: [
          SkillScanFinding(
            ruleId: 'chmod_exec',
            verdict: SkillScanVerdict.warn,
            message: 'Makes a file executable',
            file: 'SKILL.md',
            line: 4,
          ),
        ],
      ...
      await tester.tap(find.widgetWithText(CcButton, 'Scan'));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));

      expect(find.text('Warning'), findsWidgets);
      expect(find.text('chmod_exec'), findsOneWidget);
      expect(find.text('Makes a file executable'), findsOneWidget);
    });

    testWidgets('blocked save shows findings + override, retry saves', (
      tester,
    ) async {
      final control = FakeSkillSecurityControl();
      control.saveResults.addAll([
        ...
        const SkillSaveResult(status: 'saved', slug: 'risky'),
      ]);
      await tester.pumpWidget(
        harness(tester, standardOverrides(const []), control: control),
      );
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));

      // New skill, fill the name, save.
      await tester.tap(find.widgetWithText(CcButton, 'New skill'));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 200));
      await tester.enterText(find.byType(CcTextField).at(1), 'risky');
      await tester.pump();
      ...
      // The block dialog: title, findings, disabled destructive retry. The
      // dialog's Save is identified by its destructive variant (the editor's
      // Save matches the same text finder).
      expect(find.text('Save blocked by the scan gate'), findsOneWidget);
      expect(find.text('curl_pipe_shell'), findsOneWidget);
      CcButton destructiveSave() => tester
          .widgetList<CcButton>(find.widgetWithText(CcButton, 'Save'))
          .firstWhere((b) => b.variant == CcButtonVariant.destructive);
      expect(destructiveSave().onPressed, isNull);

      // Tick the explicit override — the retry arms.
      await tester.tap(find.byType(CcCheckbox));
      await tester.pump();
      final armed = destructiveSave();
      expect(armed.onPressed, isNotNull);
      await tester.tap(find.byWidget(armed));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));

      // The retry went out with the override and the save succeeded.
      expect(control.lastAllowQuarantineOverride, isTrue);
      expect(find.textContaining('saved'), findsOneWidget);
    });

    testWidgets('scan all runs one analyze op and toasts the summary', (
      tester,
    ) async {
      final control = FakeSkillSecurityControl();
      control.analyzeSummary = const SkillAnalyzeSummary(
        passCount: 1,
        warnCount: 0,
        quarantineCount: 1,
        results: [],
      ...
      await tester.pumpWidget(
        harness(
          tester,
          standardOverrides([
            const SkillInfo(name: 'a', content: '# a', description: 'A'),
            const SkillInfo(name: 'b', content: '# b', description: 'B'),
          ]),
          control: control,
        ),
      );
      ...
      await tester.tap(find.byIcon(AppIcons.scanSearch));
      await tester.pump();
      await tester.pump(const Duration(seconds: 1));

      expect(
        find.text('1 passed · 0 warnings · 1 quarantined'),
        findsOneWidget,
      );
      // One server-side batch pass, not a per-skill op loop.
      expect(control.analyzeCalls, 1);
    });
  });
}
