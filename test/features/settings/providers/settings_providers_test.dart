...
/// Detection runs in the background off the RPC-backed adapter repository, which
/// the composition root only installs after a live server handshake. In tests
/// there is no server, so we bind a fake whose probes never resolve — this keeps
/// detection fire-and-forget while leaving every adapter in its initial
/// [DetectionStatus.checking] state for the assertions below.
class _NeverResolvingAdapterRepository implements AdapterRepository {
  @override
  Future<DetectedAdapter> detectOne(Adapter adapter) =>
      Completer<DetectedAdapter>().future;

  @override
  Future<List<DetectedAdapter>> detectAll(List<Adapter> adapters) =>
      Completer<List<DetectedAdapter>>().future;
}

/// A detection notifier that reports a fixed result and never probes, so the
/// derived providers can be exercised against a settled outcome.
class _FixedDetectionNotifier extends AdapterDetectionNotifier {
  _FixedDetectionNotifier(this._detected);

  final List<DetectedAdapter> _detected;

  @override
  List<DetectedAdapter> build() => _detected;
}

/// Overrides the adapter repository so `AdapterDetectionNotifier` can build
/// without a connected RPC client.
final _adapterOverrides = [
  adapterRepositoryProvider.overrideWithValue(
    _NeverResolvingAdapterRepository(),
  ),
];
...
void main() {
  ...
  group('availableAdaptersProvider', () {
    test('offers nothing while every probe is still checking', () {
      final container = ProviderContainer(overrides: _adapterOverrides);
      addTearDown(container.dispose);

      expect(container.read(availableAdaptersProvider), isEmpty);
      expect(container.read(adapterDetectionPendingProvider), isTrue);
    });

    test('offers only the adapters that were found', () {
      final found = predefinedAdapters.first;
      final missing = predefinedAdapters.last;
      final container = ProviderContainer(
        overrides: [
          detectedAdaptersProvider.overrideWith(
            () => _FixedDetectionNotifier([
              DetectedAdapter(adapter: found, status: DetectionStatus.found),
              DetectedAdapter(
                adapter: missing,
      ...
      expect(container.read(availableAdaptersProvider), [found]);
      expect(container.read(adapterDetectionPendingProvider), isFalse);
    });
  });
