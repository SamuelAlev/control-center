...
/// Status notifier that resolves immediately to [_status].
...
Widget _wrap({
  ...
  overrides: [
    githubStatusProvider.overrideWith(
      () => _StatusNotifier(_status(indicator, incidents: incidents)),
    ),
...
void main() {
  group('resolveInboxEmptyCaveat', () {
    ...
    test('an unloaded or unreachable status page is not evidence by itself', () {
      expect(
        resolveInboxEmptyCaveat(
          indicator: null,
          viewerLogins: const {ForgeHost.github: 'octocat'},
  ...
  group('InboxEmptyState', () {
    ...
    testWidgets('reports a possible outage instead of a clear queue', (
      ...
      expect(
        find.textContaining('Outage'),
        findsOneWidget,
      );
      expect(
        find.textContaining('Incident with GitHub.com'),
        findsOneWidget,
      );
    ...
    testWidgets('explains an unresolved identity rather than an empty queue', (
      ...
      expect(find.text("You're all caught up"), findsNothing);
      expect(
        find.text("Couldn't confirm your GitHub account"),
        findsOneWidget,
      );
      expect(
    ...
    testWidgets('during an outage it still names the identity as the blocker', (
      ...
      expect(find.text('GitHub might be down'), findsNothing);
      expect(find.textContaining('GitHub status: Major issues.'), findsOneWidget);
