...
/// GitHub notifier that starts at a snapshot but flips to loading (no value)
/// whenever refreshed — mimicking the real `refresh()`, which clears state.
class _GithubRefreshClearsNotifier extends GitHubStatusNotifier {
  _GithubRefreshClearsNotifier(this._status);
  final GitHubServiceStatus _status;
...
/// Wraps the button in a [ProviderScope] with all four status providers
...
Widget _wrap({
  ...
  return ProviderScope(
    ...
    child: testWrap(const GitHubStatusButton()),
  );
}

void main() {
  group('GitHubStatusButton chip', () {
    testWidgets('hides the tag when every provider is healthy', (
      tester,
    ) async {
      ...
      expect(tester.getSize(find.byType(GitHubStatusButton)), Size.zero);
    });

    testWidgets('stays hidden before the first snapshot lands', (
      tester,
    ) async {
      ...
      // No "Unknown" flash at boot — the tag waits for real data.
      expect(find.text('Unknown'), findsNothing);
      expect(tester.getSize(find.byType(GitHubStatusButton)), Size.zero);
    });

    testWidgets('stays hidden while the loaded providers are all healthy', (
      tester,
    ) async {
    ...
    testWidgets('appears once a provider degrades', (tester) async {
  ...
  group('GitHubStatusButton flyout', () {
    testWidgets('keeps the tag mounted while its flyout is open', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
          github: _GithubRefreshClearsNotifier(
            _status(GitHubStatusIndicator.minor),
          ),
        ),
      );
      await tester.pumpAndSettle();

      await tester.tap(find.text('Minor issues'));
      // No pumpAndSettle: GitHub's cleared state renders a spinner in the
      // flyout, which animates forever.
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 300));

      // Opening refreshed every provider; GitHub's refresh cleared its value,
      // so the headline is now clean — but the open flyout keeps the tag (and
      // its popover anchor) mounted.
      expect(find.text('Service status'), findsOneWidget);
      expect(tester.getSize(find.byType(GitHubStatusButton)).height, 24);
    });
    ...
    testWidgets('shows a spinner for a provider still loading', (tester) async {
      ...
      // GitHub loaded degraded so the tag is up; Claude is still fetching, so
