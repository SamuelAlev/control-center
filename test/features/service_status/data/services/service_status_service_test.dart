...
Map<String, dynamic> _baseResponse({
  ...
  final statusBlock = <String, dynamic>{'description': description};
  if (indicator != null) {
...
void main() {
  late _FakeDio dio;
  late ServiceStatusService service;
  ...
  setUp(() {
    dio = _FakeDio();
    service = ServiceStatusService(dio);
  });
  ...
  group('fetch (status polling)', () {
    test('returns parsed status from full API response', () async {
      dio.stubResponse(
        response(
          _baseResponse(
            ...
            incidents: [_incident(id: 'inc1', name: 'Slow API')],
          ),
        ),
      );
      ...
      expect(
        ...
        'https://githubstatus.com/incidents/1',
      );
    ...
    test(
      'returns minimal status when response has no components or incidents',
      () async {
        dio.stubResponse(
          response(_baseResponse(indicator: 'none', description: 'OK')),
        );
  ...
  group('parse responses', () {
    group('indicator parsing', () {
      for (final entry in <MapEntry<String, GitHubStatusIndicator>>[
        ...
        test('maps "$entry.key" → ${entry.value}', () async {
          dio.stubResponse(
            response(_baseResponse(indicator: entry.key, description: '')),
          );
    ...
    test(
      'unknown indicator string falls back to GitHubStatusIndicator.unknown',
      () async {
        dio.stubResponse(
          response(_baseResponse(indicator: 'bogus_value', description: '')),
        );
    ...
    test(
      'null indicator string falls back to GitHubStatusIndicator.unknown',
      () async {
        dio.stubResponse(
          response(_baseResponse(indicator: null, description: '')),
        );
    ...
    group('component parsing', () {
      ...
      test('handles missing fields in component', () async {
        dio.stubResponse(
          response(_baseResponse(components: [<String, dynamic>{}])),
        );
    ...
    group('incident parsing', () {
      ...
      test('handles missing fields in incident', () async {
        dio.stubResponse(
          response(_baseResponse(incidents: [<String, dynamic>{}])),
        );
    ...
    group('date parsing', () {
      test('parses valid ISO 8601 date strings', () async {
        ...
        expect(
          result.incidents[0].createdAt,
          DateTime.utc(2024, 6, 1, 12, 0, 0),
        );
        expect(
          result.incidents[0].updatedAt,
  ...
  // ── Vendor-aware Statuspage parsing (page.url) ─────────────────────────

  group('page-aware parsing', () {
    test('filters the dummy component of the page named in page.url', () {
      final result = GitHubServiceStatus.fromSummaryJson({
        ...
        'components': [
          _component(id: 'c1', name: 'claude.ai'),
          _component(
            id: 'c2',
            name: 'Visit status.claude.com for more information',
          ),
        ],
        'incidents': const [],
      });

      expect(result.components.map((c) => c.name), ['claude.ai']);
    });

    test(
      'falls back to the githubstatus.com host when no page block exists',
      () {
        final result = GitHubServiceStatus.fromSummaryJson(
          _baseResponse(
            components: [
              _component(id: 'c1', name: 'Git Operations'),
              _component(
                id: 'c2',
                name: 'Visit www.githubstatus.com for more information',
              ),
        ...
        expect(result.components.map((c) => c.name), ['Git Operations']);
      },
    );

    test('incident shortlink falls back to the page.url', () {
      final result = GitHubServiceStatus.fromSummaryJson({
        'page': {'url': 'https://status.claude.com'},
      ...
      expect(result.incidents.single.shortlink, 'https://status.claude.com');
    });
  });
}
