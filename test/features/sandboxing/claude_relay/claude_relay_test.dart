...
void main() {
  ...
  group('buildClaudeArgs further', () {
    ...
    test('loads the MCP server explicitly when a config path is given', () {
      final args = ClaudeRelay.buildClaudeArgs(
        modelId: 'm',
        mcpConfigPath: '/data/mcp.json',
      );
      // Project `.mcp.json` is not auto-loaded; the server must be passed
      // explicitly, and `--strict-mcp-config` avoids a duplicate of the same
      // server picked up from project discovery.
      expect(
        args,
        containsAllInOrder(['--mcp-config', '/data/mcp.json']),
      );
      expect(args, contains('--strict-mcp-config'));
    });

    test('omits --mcp-config when path is null or empty', () {
      expect(
        ClaudeRelay.buildClaudeArgs(modelId: 'm'),
        isNot(contains('--mcp-config')),
      );
      expect(
        ClaudeRelay.buildClaudeArgs(modelId: 'm', mcpConfigPath: ''),
        isNot(contains('--mcp-config')),
      );
    });
  });
