...
void main() {
  group('AgentExecutionEnvironment.prepare', () {
    ...
    test('prepare does NOT write SKILLS.md when agent has no skills', () async {
    ...
    test(
      ...
      () async {
        ...
        expect(fs.written['$rd/MEMORY.md'], equals('Remember the key: 42'));
      },
    );

    test(
      'prepare does NOT write MEMORY.md when memoryContext is null',
      () async {
        final agent = _agent();
        final rd = _runDir();
    ...
    test(
      'prepare does NOT write MEMORY.md when memoryContext is empty',
      () async {
        final agent = _agent();
        final rd = _runDir();
    ...
    test(
      'prepare with chat mode writes prompt including memory instructions',
      () async {
        final agent = _agent();
        final rd = _runDir();
