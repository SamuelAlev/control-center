...
class _FakeChannel implements RemoteRpcChannelPort {
  ...
  /// How many `terminal.spawn` calls this channel served.
  int spawnCount = 0;

  /// How many `terminal.kill` calls this channel served.
  int killCount = 0;
  ...
  Future<void> send(Map<String, dynamic> frame) async {
    ...
    if (method == 'sub/unsubscribe') {
      _incoming.add({'jsonrpc': '2.0', 'id': id, 'result': <String, dynamic>{}});
    }
  }
...
void main() {
  TerminalSession sessionFor(String id) =>
      TerminalSession(sessionId: id, channelId: 'chan-1', workspaceId: 'ws-1');

  ({_FakeChannel channel, ProviderContainer container, TerminalRegistryNotifier notifier})
  setup() {
  ...
  test('obtain returns the same controller for a session id and touches it', () {
    final (:container, :notifier, channel: _) = setup();
    ...
    expect(
      container.read(terminalRegistryProvider).controllers.keys,
      ['s2', 's1'],
    );
  });

  test('never evicts a claimed (on-screen) shell past the cap', () async {
    final (:channel, :container, :notifier) = setup();
    ...
    // A 7th tab in the same channel: every entry is claimed, so the cap
  ...
  test(
    'past the cap, the least-recently-touched UNCLAIMED shells are evicted '
    'and their PTYs killed',
    () async {
      final (:channel, :container, :notifier) = setup();
      ...
      // The user navigated away from every channel: no claims left. The next
      ...
      await pumpEventQueue();
      expect(channel.killCount, 2);
    },
  );

  test('kill removes and disposes the session; a second kill is a no-op', () async {
    final (:channel, :container, :notifier) = setup();
