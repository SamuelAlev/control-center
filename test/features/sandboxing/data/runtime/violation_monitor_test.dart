...
void main() {
  group('parseLogLine', () {
    ...
    test('parses mach-lookup actions', () {
      ...
      expect(result.violation.target, 'com.apple.CoreDisplay.Notification');
    });
  ...
  group('isNoise', () {
    ...
    group('process-name filtering', () {
      ...
      for (final proc in SandboxViolationMonitor.agentProcesses) {
        test('agent process "$proc" with benign action → NOT noise', () {
          ...
          expect(
            SandboxViolationMonitor.isNoise(p),
            isFalse,
    ...
    group('non-file-read actions on noisy paths are not filtered', () {
      test(
        'file-write-create on /System/ → NOT noise (action is not file-read)',
        () {
          final p = makeParsed(
            processName: 'bash',
      ...
      test(
        'network-outbound on any target → NOT noise (for agent process)',
        () {
          final p = makeParsed(
            processName: 'curl',
    ...
    group('combined filters', () {
      ...
      test(
        'agent process with mach-lookup on user path → still noise (action wins)',
        () {
          final p = makeParsed(
            processName: 'bash',
            action: 'mach-lookup',
  ...
  group('noisyReadPaths', () {
    ...
    test('contains fsevents noise', () {
      expect(SandboxViolationMonitor.noisyReadPaths, contains('/.fseventsd'));
    });
  });
}
