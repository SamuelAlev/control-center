...
Future<ServerSocket> startEchoServer() async {
  ...
  server.listen((s) {
    s.listen((data) => s.add(data), onDone: () => s.close());
  });
  return server;
}
...
void main() {
  group('SandboxSocksProxy', () {
    ...
    test('closes connection on non-SOCKS5 version', () async {
      ...
      try {
        await socket.expand((x) => x).first.timeout(const Duration(seconds: 2));
    ...
    test('isBlocked denies everything', () async {
      proxy.updateConfig(
        network: const NetworkConfig(allowAll: false, allowedDomains: []),
      );
      final conn = await _connect(proxy);
      final reply = await _socksConnect(
        conn.reader,
