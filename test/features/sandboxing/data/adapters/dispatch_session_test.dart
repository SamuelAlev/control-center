...
class FakeAgentRepository implements AgentRepository {
  ...
  @override
  Future<Agent?> getById(String workspaceId, String id) async => agentToReturn;
...
class ControllableSandboxPort extends FakeSandboxPort {
  ...
  @override
  Stream<SandboxEvent> events(SandboxHandle handle) => _eventController.stream;
...
DispatchSession _makeSession({
  ...
  return DispatchSession(
    deps: deps ?? _makeDeps(),
    onResolveHandle:
        ({required sessionId, required spec, required emit}) async =>
    ...
    runLogId: null,
    mode: mode,
    resolveBinary: _fakeResolveBinary,
  );
}
...
void main() {
  ...
  group('emittedDone guard', () {
    ...
    test(
      'controller closes after stop() and no more events accepted',
      () async {
        final session = _makeSession();
        expect(session.controller.isClosed, isFalse);
  ...
  group('terminate', () {
    ...
    test('emits DebugEvent about termination', () async {
      ...
      final terminationMsg = debugEvents.any(
        (e) => e.content.contains('terminated by request'),
      );
    ...
    test('emits ErrorEvent with failure message', () async {
      ...
      final failMsg = errorEvents.any(
        (e) => e.content == 'Terminated by user request',
      );
  ...
  // -----------------------------------------------------------------------
  // Per-agent bind-mount isolation (Task 5: cwd rw, convRoot/repos rw,
  // agentConfigDir ro; sibling agent folders never mounted)
  // -----------------------------------------------------------------------
  group('bind mounts', () {
    test(
      'fallback dispatch (no agentConfigDir) mounts a single writable cwd',
      () async {
        SandboxSpec? captured;
        final session = DispatchSession(
          deps: _makeDeps(),
          onResolveHandle:
              ({required sessionId, required spec, required emit}) async {
        ...
        // Let run() reach onResolveHandle (mint + backend switch + binary resolve).
        await Future<void>.delayed(const Duration(milliseconds: 100));

        expect(captured, isNotNull);
        expect(captured!.bindMounts, [
          const SandboxBindMount(
            hostPath: '/tmp/agent',
            guestPath: '/tmp/agent',
          ),
        ]);
        expect(captured!.guestWorkdir, '/tmp/agent');
      },
    );

    test(
      'per-agent overlay mounts cwd rw, agentConfigDir ro, shared repos rw',
      () async {
        final tempDir = await Directory.systemTemp.createTemp(
          'dispatch_mounts_',
        );
        try {
          final convRoot = '${tempDir.path}/ws/conversations/ch';
          final cwd = '$convRoot/agents/dev';
          final agentConfigDir = '${tempDir.path}/ws/agents/dev';
          await Directory(cwd).create(recursive: true);
          await Directory(agentConfigDir).create(recursive: true);
          // Shared conversation repos (must exist to be mounted).
          await Directory('$convRoot/repos').create(recursive: true);
          // A sibling agent folder that must NEVER be reachable.
          await Directory('$convRoot/agents/other').create(recursive: true);
          ...
          expect(captured, isNotNull);
          final mounts = captured!.bindMounts;
          expect(captured!.guestWorkdir, cwd);

          final cwdMount = mounts.firstWhere((m) => m.hostPath == cwd);
          expect(cwdMount.readOnly, isFalse);

          final cfgMount = mounts.firstWhere(
            (m) => m.hostPath == agentConfigDir,
          );
          expect(cfgMount.readOnly, isTrue);

          final reposMount = mounts.firstWhere(
            (m) => m.hostPath == '$convRoot/repos',
          );
          expect(reposMount.readOnly, isFalse);

          // Sibling agent folder is invisible to this agent's namespace.
          expect(
            mounts.any((m) => m.hostPath.contains('agents/other')),
            isFalse,
          );
        } finally {
  ...
  // -----------------------------------------------------------------------
  // Mode integration with constructor
  // -----------------------------------------------------------------------
  group('Mode', () {
    test('stores plan mode', () {
      final session = _makeSession(mode: Mode.plan);
      expect(session.mode, Mode.plan);
    });

    test('stores review mode', () {
      final session = _makeSession(mode: Mode.review);
      expect(session.mode, Mode.review);
    });

    test('stores chat mode', () {
      final session = _makeSession(mode: Mode.chat);
      expect(session.mode, Mode.chat);
    });
  });
  ...
  group('controller lifecycle edge cases', () {
    ...
    test('controller is single-subscription by default', () {
      ...
      expect(() => session.controller.stream.listen((_) {}), throwsStateError);
    });
  });
  ...
  group('forwardSandboxEvent', () {
    test('stdout non-JSON line → TextEvent', () async {
      ...
      expect(
        textEvents.any((e) => e.content == 'plain text output from agent'),
        isTrue,
      );
    });
    ...
    test(
      'killed event without content → ErrorEvent with default message',
      () async {
        final sandbox = ControllableSandboxPort();
        final session = _makeSession(deps: _makeDeps(sandbox: sandbox));
        final events = <AgentProcessEvent>[];
        ...
        sandbox.addSandboxEvent(
          const SandboxEvent(type: SandboxEventType.killed),
        );
        ...
        final errorEvents = events.whereType<ErrorEvent>().toList();
        expect(errorEvents.any((e) => e.content == '[sandbox] killed'), isTrue);
      },
    );
    ...
    test('starting event → DebugEvent', () async {
      ...
      sandbox.addSandboxEvent(
        const SandboxEvent(type: SandboxEventType.starting),
      );
      ...
      final debugEvents = events.whereType<DebugEvent>().toList();
      expect(
        debugEvents.any((e) => e.content.contains('booting sandbox session')),
        isTrue,
      );
    });
    ...
    test('ready event is a no-op (no event emitted)', () async {
      ...
      sandbox.addSandboxEvent(const SandboxEvent(type: SandboxEventType.ready));
    ...
    test(
      'violation event without violation data → generic deny message',
      () async {
        final sandbox = ControllableSandboxPort();
        final session = _makeSession(deps: _makeDeps(sandbox: sandbox));
        final events = <AgentProcessEvent>[];
        ...
        sandbox.addSandboxEvent(
          const SandboxEvent(type: SandboxEventType.violation),
        );
  ...
  group('run exec arguments', () {
    ...
    test('non-zero exit code emits error event', () async {
      ...
      final errorEvents = events.whereType<ErrorEvent>().toList();
      expect(
        errorEvents.any((e) => e.content.contains('exited with code 1')),
        isTrue,
      );
    });
    ...
    test('non-zero exit after stderr does not repeat as an error', () async {
      ...
      sandbox.addSandboxEvent(
        const SandboxEvent(
          type: SandboxEventType.stderr,
          content: 'Error: Unknown option: --nope',
        ),
      );
      await Future<void>.delayed(Duration.zero);
    ...
    test('exit code 127 emits binary-not-found error', () async {
      ...
      final errorEvents = events.whereType<ErrorEvent>().toList();
      expect(
        errorEvents.any((e) => e.content.contains('not found on PATH')),
        isTrue,
      );
    });
    ...
    test('zero exit code emits debug message', () async {
      ...
      final debugEvents = events.whereType<DebugEvent>().toList();
      expect(
        debugEvents.any((e) => e.content.contains('exited cleanly (code 0)')),
        isTrue,
      );
    });
  ...
  group('event bus publishing', () {
    test(
      '_completeRun publishes AgentRunCompleted when agentId is set',
      () async {
        final eventBus = DomainEventBus();
