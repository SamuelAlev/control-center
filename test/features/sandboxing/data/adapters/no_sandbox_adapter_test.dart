...
SandboxSpec _spec({
  ...
  return SandboxSpec(
    ...
    workspaceId: 'ws',
    bindMounts: [
      const SandboxBindMount(hostPath: '/tmp/work', guestPath: '/tmp/work'),
    ],
    agentId: agentId,
...
void main() {
  group('NoSandboxAdapter', () {
    ...
    test('launch with empty bind mounts sets null working directory', () async {
      const spec = SandboxSpec(
        sessionId: 's3',
        workspaceId: 'ws',
        bindMounts: [],
    ...
    test('isAlive is true for active state (state management only)', () async {
      final handle = await adapter.launch(_spec());
    ...
    test(
      'events returns broadcast stream reused for same session id',
      () async {
        final handle = await adapter.launch(_spec());
        final stream = adapter.events(handle);
    ...
    test('exec throws ArgumentError when argv is empty', () async {
      ...
      expect(() => adapter.exec(handle, []), throwsA(isA<ArgumentError>()));
    });
    ...
    // -----------------------------------------------------------------------
    test(
      'destroy removes handle from internal map and sets destroyed state',
      () async {
        final handle = await adapter.launch(_spec());
