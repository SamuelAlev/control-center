...
Future<String> _getThroughProxy(HttpClient client, String url) async {
  ...
  if (resp.statusCode != 200) {
    ...
    throw HttpException(
      'Proxy returned ${resp.statusCode}: $body',
      uri: Uri.parse(url),
    );
  }
...
void main() {
  ...
  group('proxy rule construction', () {
    test(
      'allows requests to any domain when allowAll is true (default)',
      () async {
    ...
    test('denies requests to denied domains', () async {
      ...
      addTearDown(proxy.close);
      proxy.updateConfig(
        network: const NetworkConfig(deniedDomains: ['127.0.0.1']),
      );
    ...
    test(
      'blocks requests to domains not in allowlist when restricted',
      () async {
        final upstream = await _FakeUpstream.start(body: 'should-not-reach');
        addTearDown(upstream.close);
    ...
    test(
      'deniedDomains takes precedence over allowAll and allowedDomains',
      () async {
        final upstream = await _FakeUpstream.start(body: 'should-not-reach');
        addTearDown(upstream.close);
