...
void main() {
  ...
  Future<RunLogWriter> openWriter({
    ...
    final writer = RunLogWriter(
      coalesceableLogTypes: coalesceableLogTypes ?? const {'thinking', 'text'},
  ...
  group('RunLogWriter', () {
    ...
    group('logEvent', () {
      test('writes non-coalesceable event immediately', () async {
        final writer = await openWriter();
        writer.logEvent(ToolCallEvent(toolName: 'bash', toolCallId: 'tc1'));
        final lines = await closeAndRead(writer);
      ...
      test('coalesces consecutive coalesceable events of same type', () async {
        final writer = await openWriter(
      ...
      test('redacts secrets in content', () async {
        final writer = await openWriter();
        writer.logEvent(TextEvent(content: 'key=sk-abc12345678901234567890ab'));
        final lines = await closeAndRead(writer);
    ...
    group('coalescing edge cases', () {
      test(
        'first coalesceable event populates buffer without writing',
        () async {
          final writer = await openWriter(
            logCoalesceWindow: const Duration(seconds: 10),
          );
