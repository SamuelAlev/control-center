...
void main() {
  group('SandboxBackendDetector', () {
    test('recommends native when native probe succeeds', () async {
      ...
      expect(result.capabilities[SandboxBackend.native]!.available, isTrue);
    ...
    test(
      'multiple adapters: picks first available in priority order',
      () async {
    ...
    test('multiple adapters: native throws but another available '
    ...
    test('multiple adapters: native throws and no adapter provides '
    ...
    test(
      'throwing adapter with non-String error captures exception text',
      () async {
        final detector = SandboxBackendDetector([
