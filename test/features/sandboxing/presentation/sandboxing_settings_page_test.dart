...
void main() {
  ...
  group('SandboxingSections rendering', () {
    ...
    testWidgets('renders all section labels when detection succeeds', (
      tester,
    ) async {
      final overrides = await _baseOverrides();
      await _pumpScreen(tester, overrides: overrides);
      ...
      expect(
        find.text('DEFAULT CAPABILITIES \u00b7 NEW CHANNELS'),
        findsOneWidget,
      );
    ...
    testWidgets('renders reset section with destructive button', (
      tester,
    ) async {
      final overrides = await _baseOverrides();
      await _pumpScreen(tester, overrides: overrides);
