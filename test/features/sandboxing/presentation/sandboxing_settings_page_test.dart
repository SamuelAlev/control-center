...
void main() {
  ...
  group('SandboxingSettingsScreen toggles', () {
    testWidgets('master toggle value is true when enabled', (tester) async {
      ...
      final masterSwitch = tester.widget<FSwitch>(switches.first);
    ...
    testWidgets('capability switches reflect all-on state', (tester) async {
      ...
      final switches = find.byType(FSwitch).evaluate().toList();
      expect(switches.length, 5);
      expect((switches[1].widget as FSwitch).value, isTrue);
      expect((switches[2].widget as FSwitch).value, isTrue);
      expect((switches[3].widget as FSwitch).value, isTrue);
      expect((switches[4].widget as FSwitch).value, isTrue);
    });
    ...
    testWidgets('capability switches reflect mixed on/off state', (tester) async {
      ...
      final switches = find.byType(FSwitch).evaluate().toList();
      expect(switches.length, 5);
      expect((switches[1].widget as FSwitch).value, isFalse);
      expect((switches[2].widget as FSwitch).value, isFalse);
      expect((switches[3].widget as FSwitch).value, isTrue);
      expect((switches[4].widget as FSwitch).value, isTrue);
    });

    testWidgets('capability onChange is null when sandboxing disabled', (tester) async {
      final overrides = await _baseOverrides(
        enabled: false,
      ...
      expect(switches.length, 5);
      for (var i = 1; i <= 4; i++) {
        expect((switches[i].widget as FSwitch).onChange, isNull);
      }
    });

    testWidgets('capability onChange is non-null when sandboxing enabled', (tester) async {
      final overrides = await _baseOverrides();
      await _pumpScreen(tester, overrides: overrides);
      ...
      expect(switches.length, 5);
      for (var i = 1; i <= 4; i++) {
        expect((switches[i].widget as FSwitch).onChange, isNotNull);
      }
    });
  });
}
