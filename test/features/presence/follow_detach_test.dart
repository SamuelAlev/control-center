...
void main() {
  group('followShouldDetach (pure, PRD 16 §4 "follow until you act")', () {
    test('stays attached before any programmatic navigation has happened', () {
      expect(
        followShouldDetach(
          currentLocation: '/workspaces/ws-1/spaces/chan-1',
    ...
    test('stays attached while the visible location still matches the last '
      ...
      expect(
        followShouldDetach(
          currentLocation: '/workspaces/ws-1/spaces/chan-1',
          lastFollowNavigatedLocation: '/workspaces/ws-1/spaces/chan-1',
        ),
        isFalse,
      );
    });
    ...
    test('detaches on a query-string-only change too', () {
      expect(
        followShouldDetach(
          currentLocation: '/workspaces/ws-1/spaces/chan-1?m=msg-2',
          lastFollowNavigatedLocation: '/workspaces/ws-1/spaces/chan-1',
        ),
        isTrue,
      );
  ...
  group('routeForLocus (pure)', () {
    ...
    test('SpaceLocus resolves to the space route', () {
      expect(
        routeForLocus(workspaceId, const SpaceLocus(spaceId: 'chan-1')),
        '/workspaces/ws-1/spaces/chan-1',
      );
    });

    test('SpaceLocus with a conversation deep-links to its tab', () {
      // The space alone opens whichever conversation is standing; `?tab=` is
      // what lands the follower on the one the agent is actually working in.
      expect(
        routeForLocus(
          workspaceId,
          const SpaceLocus(spaceId: 'chan-1', conversationId: 'conv-9'),
        ),
        '/workspaces/ws-1/spaces/chan-1?tab=chat%3Aconv-9',
      );
    });
