...
void main() {
  group('ResolveOrCreateDomainUseCase', () {
    ...
    test(
      'returns existing domain when slug matches',
    ...
    test(
      'returns existing domain regardless of input casing',
      timeout: const Timeout.factor(2),
      () async {
        final existing = MemoryDomain(
          id: 'd1',
    ...
    test(
      'creates new domain when none exists',
      timeout: const Timeout.factor(2),
      () async {
        final result = await useCase.execute(
          workspaceId: 'ws1',
    ...
    test(
      'slugifies domain input for name',
      timeout: const Timeout.factor(2),
      () async {
        final result = await useCase.execute(
          workspaceId: 'ws1',
    ...
    test(
      'uses domain input as label when no label provided',
      timeout: const Timeout.factor(2),
      () async {
        final result = await useCase.execute(
          workspaceId: 'ws1',
    ...
    test(
      'persists new domain to repository',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'seeds access grants for all roles on creation',
      timeout: const Timeout.factor(2),
      () async {
        await useCase.execute(
          workspaceId: 'ws1',
        ...
        // Workspace memory is collaborative: every role is seeded WRITE so any
        // agent can contribute facts and policies (the access-matrix editor is
        // the place to tighten a sensitive domain afterwards).
    ...
    test(
      'does not seed grants when domain already exists',
      timeout: const Timeout.factor(2),
      () async {
        domainRepo.seed([
          MemoryDomain(
