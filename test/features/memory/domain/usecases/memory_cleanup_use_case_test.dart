...
void main() {
  group('MemoryCleanupUseCase', () {
    ...
    group('_archiveStaleFacts', () {
      test(
        'archives stale fact with low confidence',
      ...
      test(
        'does not archive stale fact with high confidence',
        timeout: const Timeout.factor(2),
        () async {
          final oldDate = DateTime.now().subtract(const Duration(days: 31));
          final fact = createFact(
      ...
      test(
        'does not archive fresh fact with low confidence',
        timeout: const Timeout.factor(2),
        () async {
      ...
      test(
        'skips already superseded facts',
        timeout: const Timeout.factor(2),
        () async {
      ...
      test(
        'only processes facts from the given workspace',
        timeout: const Timeout.factor(2),
        () async {
          final oldDate = DateTime.now().subtract(const Duration(days: 31));
          factRepo.seed([
          ...
          expect(
            (await factRepo.getById('ws1', 'f-ws1'))?.supersededBy,
            'system:cleanup',
          );
          expect(
            (await factRepo.getById('ws2', 'f-ws2'))?.supersededBy,
            isNull,
          );
        },
      );
    ...
    group('_truncateWorkingMemory', () {
      test(
        'truncates bloated working memory older than 14 days',
        timeout: const Timeout.factor(2),
        () async {
          ...
          final updated =
              (await workingMemoryRepo.watchByWorkspace('ws1').first).first;
          expect(updated.content.length, 2000);
        },
      );

      test(
        'does not truncate bloated working memory younger than 14 days',
        timeout: const Timeout.factor(2),
        () async {
      ...
      test(
        'does not truncate working memory under max chars',
        timeout: const Timeout.factor(2),
        () async {
      ...
      test(
        'truncated content preserves the last 2000 characters',
        timeout: const Timeout.factor(2),
        () async {
          final oldDate = DateTime.now().subtract(const Duration(days: 20));
