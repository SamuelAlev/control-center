...
void main() {
  group('PromoteFactsToPolicyUseCase', () {
    ...
    group('execute', () {
      test(
        'creates policy when author has write access',
      ...
      test(
        'throws when author lacks write access',
        timeout: const Timeout.factor(2),
        () async {
          grantRepo.seed([
            MemoryAccessGrant(
      ...
      test(
        'throws when no grant exists for author',
        timeout: const Timeout.factor(2),
        () async {
          // No grants seeded
          expect(
    ...
    group('autoPromote', () {
      test(
        'creates policy for domains with >= 3 active facts',
        timeout: const Timeout.factor(2),
        () async {
          factRepo.seed([
      ...
      test(
        'does not promote domains with < 3 facts',
        timeout: const Timeout.factor(2),
        () async {
          factRepo.seed([
            createFact(id: 'f1', domain: 'codebase', content: 'Uses Riverpod'),
      ...
      test(
        'skips superseded facts when counting',
        timeout: const Timeout.factor(2),
        () async {
          factRepo.seed([
            createFact(id: 'f1', domain: 'codebase', content: 'Uses Riverpod'),
      ...
      test(
        'does not create duplicate policies for same fact set',
        timeout: const Timeout.factor(2),
        () async {
          factRepo.seed([
            createFact(id: 'f1', domain: 'codebase', content: 'Uses Riverpod'),
      ...
      test(
        'promotes multiple domains independently',
        timeout: const Timeout.factor(2),
        () async {
          factRepo.seed([
            ...
            createFact(
              id: 'b2',
              domain: 'preferences',
              content: 'Pref vim keys',
            ),
            createFact(
              id: 'b3',
              domain: 'preferences',
              content: 'Pref monospace',
            ),
          ]);
