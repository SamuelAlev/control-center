...
void main() {
  group('Weibull decay (phase 4.2)', () {
    ...
    test('AC: a 30-day-old request ranks below a fresh fact', () {
      ...
      final staleRequest = weibullBoost(
        old,
        now: now,
        memoryType: MemoryType.request,
      );
      final freshFact = weibullBoost(
        now,
        now: now,
  ...
  group('Conflict detection (phase 4.1)', () {
    test(
      'AC: contradictory content on same topic is a conflict; old loses',
      () {
        final older = _fact(
          id: 'old',
  ...
  group('Query intent (phase 4.3 / feature #6)', () {
    test(
      'AC: "what did we decide last week" leans temporal (high FTS bias)',
      () {
    ...
    test('suppresses near-duplicate paraphrases', () {
      final items = [
        ...
        const MmrItem(
          value: 'c',
          score: 0.8,
          content: 'deploys happen every friday afternoon',
        ),
      ];
  ...
  group('SHMR harmonization (phase 4.6 / feature #11)', () {
    test(
      'AC: two agents asserting contradictory facts → contradiction flagged',
      () {
        final items = [
          const ShmrItem(
            factId: 'a',
    ...
    test('corroborating facts from two agents emit a belief', () {
      ...
      expect(
        result.beliefs.first.provenanceAgentIds,
  ...
  group('Heuristic extraction (phase 4.4 / feature #9)', () {
    test('AC: conversation text yields facts without explicit remember', () {
      final result = heuristicExtract(
        'my name is Sam and I work at UseCtrl. I prefer dark mode.',
      );
      expect(result.isEmpty, isFalse);
      final all = result.toTypedItems().map((i) => i.content).join(' | ');
      expect(all.toLowerCase(), contains('sam'));
      expect(all.toLowerCase(), contains('usectrl'));
