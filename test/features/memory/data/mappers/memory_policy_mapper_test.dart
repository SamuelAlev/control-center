...
void main() {
  group('MemoryPolicyMapper', () {
    ...
    test(
      'parses source fact ids from JSON array',
      timeout: const Timeout.factor(2),
      () {
        final row = createRow(sourceFactIds: jsonEncode(['f1', 'f2', 'f3']));
    ...
    test(
      'returns empty list when source fact ids is empty JSON array',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'returns empty list when source fact ids is non-list JSON',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'parses requiredRole to AgentRole',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'returns null requiredRole when not set',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'returns null requiredRole for unknown role string',
      timeout: const Timeout.factor(2),
      () {
