...
void main() {
  group('MemoryFactMapper', () {
    ...
    test(
      'parses source observation ids from JSON array',
    ...
    test(
      'returns empty list when source observation ids is empty string',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'returns empty list when source observation ids is non-JSON',
      timeout: const Timeout.factor(2),
      () {
        ...
        final rowNonList = createRow(
          sourceObservationIds: jsonEncode('a-string'),
        );
    ...
    test(
      'maps nullable authoredByAgentId',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'parses authoredByRole to AgentRole',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'returns null authoredByRole when not set',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'handles unknown authoredByRole gracefully',
      timeout: const Timeout.factor(2),
      () {
