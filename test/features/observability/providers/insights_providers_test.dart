...
void main() {
  ...
  group('bucketizeSeries', () {
    ...
    test('last 24h buckets hourly, padded to 24 slots', () {
      ...
      expect(result.buckets, hasLength(24));
      expect(result.buckets.first.bucketStart, DateTime(2026, 7, 27, 16));
      expect(result.buckets.last.bucketStart, DateTime(2026, 7, 28, 15));
      expect(result.buckets.last.runs, 1);
      // The empty hours are present and zeroed, not absent.
      expect(result.buckets.first.runs, 0);
    });

    test('last 7d buckets daily, padded to 7 slots', () {
      final result = bucketizeSeries(
        [_run(id: 'r1', agentId: 'a', startedAt: now)],
        ObsTimeRange.last7d,
        calc,
      ...
      expect(result.buckets.first.bucketStart, DateTime(2026, 7, 22));
      expect(result.buckets.last.bucketStart, DateTime(2026, 7, 28));
      expect(result.buckets.map((b) => b.runs), [0, 0, 0, 0, 0, 0, 1]);
    });

    test('last 30d buckets daily, padded to 30 slots', () {
      final result = bucketizeSeries(
        [_run(id: 'r1', agentId: 'a', startedAt: now)],
      ...
      expect(result.kind, ObsBucketKind.day);
      expect(result.buckets, hasLength(30));
      expect(result.buckets.first.bucketStart, DateTime(2026, 6, 29));
    });

    test('a run older than the nominal window is never dropped', () {
      // The 7d filter admits runs back to `now - 7d`, which floors to an
      // eighth calendar day; padding must widen to it rather than cut it.
      final result = bucketizeSeries(
        [
          _run(
            id: 'r1',
      ...
      expect(result.buckets, hasLength(8));
      expect(result.buckets.first.bucketStart, DateTime(2026, 7, 21));
      expect(result.buckets.first.runs, 1);
    });

    test('gaps between two active days are zero-filled', () {
      final result = bucketizeSeries(
        [
          _run(
            id: 'r1',
          ...
          _run(id: 'r2', agentId: 'a', startedAt: now),
        ],
        ObsTimeRange.last7d,
        calc,
        now: now,
      ...
      expect(result.buckets, hasLength(7));
      expect(result.buckets.map((b) => b.runs), [0, 0, 0, 1, 0, 0, 1]);
      expect(result.buckets.map((b) => b.costCents), [0, 0, 0, 100, 0, 0, 0]);
    });
    ...
    test('all time folds to weekly beyond 62 days, preserving totals', () {
      ...
      final result = bucketizeSeries(runs, ObsTimeRange.all, calc, now: now);
