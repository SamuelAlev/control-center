...
void main() {
  ...
  group('a required verb is always reachable', () {
    ...
    test('interaction verbs are always pinned', () {
      // `ask_user` IS how a run asks the operator anything — gating it would
      // deadlock a run that needs to ask something.
      for (final mode in Mode.values) {
        expect(
    ...
    test('every interaction verb names a real harness tool', () {
      ...
      // tool classes (`ask_user_question`, `request_confirmation`) while the
      // one tool that actually asks the operator went unpinned — and how the
      // plan-mode prompt came to instruct agents to call a tool that did not
      // exist. Pin the names against the registry's own vocabulary.
      const harnessInteractionTools = {'ask_user'};
      expect(
        ModeCapabilityProfile.interactionVerbs,
        everyElement(isIn(harnessInteractionTools)),
        reason:
            'an interaction verb that no harness tool answers to is inert — '
            'it pins nothing and silently protects nothing',
      );
    });

    test('the plan-mode clarify verb is a tool the run can actually call', () {
      // The prompt tells a plan agent to use this verb by name. If it names a
      // tool the surface does not carry, an ambiguous brief is planned against
      // the model's own assumptions instead of paused for the operator — the
      // exact class of prompt/surface disagreement `plan_mode_contract.dart`
      // was written to prevent.
      expect(
        ModeCapabilityProfile.interactionVerbs,
        contains(planModeClarifyVerb),
      );
      expect(profileFor(Mode.plan).pinnedVerbs, contains(planModeClarifyVerb));
    });
  });
  ...
  group('prompt / tool-surface parity', () {
    ...
    test(
      'a deferred surface advertises names without claiming completeness',
      () {
        final preamble = buildCapabilityPreamble(
          profileFor(Mode.plan),
          materializedToolNames: const ['read', 'submit_plan'],
          deferredToolNames: const ['create_ticket', 'search_memory'],
        );
        // "exactly N tools, these and nothing else" becomes false the moment
        // anything is callable but unlisted, so it must not survive deferral.
        expect(preamble, isNot(contains('nothing else')));
        expect(preamble, contains('2 tools are loaded'));
        expect(preamble, contains('2 more tools'));
        // The index is names under headings, never schemas — that is the whole
        // saving.
        expect(preamble, contains('`create_ticket`'));
        expect(preamble, contains('Tickets and projects'));
        expect(preamble, contains('Memory and knowledge'));
        // Both affordances stated: call it directly, or search for it.
        expect(preamble, contains('directly by name'));
        expect(preamble, contains('`search_tools`'));
      },
    );

    test('a merely-deferred required verb is not reported as missing', () {
      // "Not loaded" and "not available" are different facts, and confusing
      // them would tell an agent its deliverable is impossible.
      final preamble = buildCapabilityPreamble(
        profileFor(Mode.plan),
        materializedToolNames: const ['read'],
        deferredToolNames: const ['submit_plan'],
      );
      expect(preamble, isNot(contains('WARNING')));
    });

    test('a forbidden verb stays forbidden only if it is truly absent', () {
      final profile = profileFor(Mode.plan);
      final forbidden = profile.forbiddenVerbs.first;
      final preamble = buildCapabilityPreamble(
        profile,
        materializedToolNames: const ['read'],
        deferredToolNames: [forbidden],
      );
      // Telling a model it does not have a tool it can call by name would be
      // the same class of lie the generated preamble exists to prevent.
  ...
  group('staffing guidance stays inside the mode', () {
    ...
    test('chat mode gets the roster, the ids and the delegation verbs', () {
      ...
      // No hiring verb is offered: the agent-lifecycle tools (`hire_agent`,
      // `propose_hire`, …) were deleted from the MCP surface, and guidance
      // must never name a tool the run cannot call.
    ...
    test('a lone agent gets no staffing section at all', () {
      // With no roster and no hiring verb left to offer, there is nothing to
      // say — and a "Your team" header over an empty list reads as a bug.
      final brief = PromptBuilder()
          .team(const [], mode: Mode.chat)
          .buildPersistentBrief();

      expect(brief, isNot(contains('Your team')));
      expect(brief, isNot(contains('Staffing is YOUR decision')));
    });
    ...
    test('read-only modes fall back to the verbs they actually have', () {
      ...
      // Plan mode falls back to `consult_agent` (it has no `ask_agent`), and
      // names no delegation or hiring verb it cannot call.
  ...
  group('completion contracts', () {
    ...
    test('the plan contract requires exactly the plan-mode output verb', () {
      ...
      // space setting, so a plain question can arrive inside it.
...
const Map<String, ToolApprovalTier> _builtinTiers = {
  ...
  // Read tier: asking a question mutates nothing, which is what lets a
  // read-only mode keep it.
  'ask_user': ToolApprovalTier.read,
};

/// Every name that could plausibly be a tool reference in a prompt.
///
/// The parity check above filters the prompt's backticked tokens through this
/// set, so a verb MISSING here is silently never checked. That is how the plan
/// prompt came to instruct agents to call `ask_user_question`, a tool that does
/// not exist, while a test called "every tool a mode prompt names is reachable"
/// passed: the name was not recognized, so it was skipped rather than failed.
/// Anything the mode contracts declare therefore has to be in here by
