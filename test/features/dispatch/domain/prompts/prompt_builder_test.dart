...
void main() {
  group('PromptBuilder', () {
    ...
    test('identity section includes agent_id, agent_name, workspace_id', () {
      final result = PromptBuilder().identity(_agent()).buildPersistentBrief();
    ...
    test('systemPrompt with null returns this (no crash)', () {
      final result = PromptBuilder().systemPrompt(null).buildPersistentBrief();

      expect(result, isEmpty);
    });

    test('systemPrompt with empty string returns this (no addition)', () {
      final result = PromptBuilder().systemPrompt('').buildPersistentBrief();
    ...
    test(
      'persona with null persona and role still works (just role sections)',
      () {
        final result = PromptBuilder()
    ...
    test('executionContract includes all rules', () {
      final result = PromptBuilder().executionContract().buildPersistentBrief();
    ...
    test('executionContract for chat includes memory instruction (rule 7)', () {
      final result = PromptBuilder()
          .executionContract(mode: Mode.chat)
          .buildPersistentBrief();
    ...
    test(
      'executionContract for review/plan omits memory-write instruction',
      () {
        final result = PromptBuilder()
            .executionContract(mode: Mode.review)
            .buildPersistentBrief();
    ...
    test('locale with english returns this (no change)', () {
      final base = PromptBuilder().identity(_agent()).buildPersistentBrief();
    ...
    test('locale with null returns this', () {
      final result = PromptBuilder().locale(null).buildPersistentBrief();
    ...
    test('memoryContext with null returns this', () {
      final result = PromptBuilder().memoryContext(null).buildPersistentBrief();

      expect(result, isEmpty);
    });

    test('wakeContext includes runId, ticketId, wakeReason, spaceId', () {
      ...
      final result = PromptBuilder().wakeContext(wc).buildPersistentBrief();
    ...
    test('wakeContext with null returns this', () {
      final result = PromptBuilder().wakeContext(null).buildPersistentBrief();

      expect(result, isEmpty);
    });

    test('mentions includes summonedBy and space roster', () {
    ...
    test('build() wraps content in <context> tags and appends prompt', () {
      final result = PromptBuilder().identity(_agent()).build('Do the thing.');
    ...
    test(
      ...
      () {
        final result = PromptBuilder()
            .resourceProtocols(mode: Mode.chat)
            .buildPersistentBrief();
    ...
    test(
      'resourceProtocols for review now includes memory + uncertainty protocol',
      () {
        final result = PromptBuilder()
            .resourceProtocols(mode: Mode.review)
            .buildPersistentBrief();

        expect(result, contains('Reading files and resources'));
        expect(result, contains('Search discipline'));
        // Memory contribution is now expected in every mode (knowledge writes
        // are allowed in review/plan/orchestrate), plus the uncertainty protocol.
        expect(result, contains('Memory Management'));
        expect(result, contains('Uncertainty protocol'));
      },
    );
  });
}
