...
void main() {
  group('RetryDecision', () {
    test(
      'retry singleton has shouldRetry=true',
  ...
  group('RunRetryPolicy', () {
    ...
    test(
      'retries when classification is retryable and under max attempts',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'retries on second attempt when maxAttempts=3',
      timeout: const Timeout.factor(2),
      () {
        const classification = RunFailureClassification(
          family: RunErrorFamily.transientUpstream,
    ...
    test(
      'suppresses when attempt limit reached',
      timeout: const Timeout.factor(2),
      () {
        const classification = RunFailureClassification(
          family: RunErrorFamily.transientUpstream,
    ...
    test(
      'suppresses non-retryable classification',
      timeout: const Timeout.factor(2),
      () {
        const classification = RunFailureClassification(
    ...
    test(
      'non-retryable checked before attempt limit',
      timeout: const Timeout.factor(2),
      () {
        const classification = RunFailureClassification(
    ...
    test(
      'with maxAttempts=1, never retries',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      ...
      () {
        ...
        expect(
          p.decide(classification: classification, attempt: 0).shouldRetry,
          isTrue,
        );
        // attempt 1 == maxAttempts-1 → no more
        expect(
          p.decide(classification: classification, attempt: 1).shouldRetry,
          isFalse,
        );
      },
    );
    ...
    test(
      'with maxAttempts=0, never retries',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'classification without userAction',
      timeout: const Timeout.factor(2),
      () {
        const classification = RunFailureClassification(
          family: RunErrorFamily.transientUpstream,
    ...
    test(
      'all FailureStage values produce a decision without error',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'all RunErrorFamily values handled',
      timeout: const Timeout.factor(2),
      () {
