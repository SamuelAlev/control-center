...
/// Workspace [_testAgent] belongs to by default and the one every dispatch in
/// this suite is executed in.
const _ws = 'ws-1';
...
class _FakeAgentRepository implements AgentRepository {
  ...
  Future<Agent?> getById(String workspaceId, String id) async {
    ...
    // Scoped, not id-only: an agent id belonging to another workspace lives in
    // another database file and must not resolve here.
...
class _FakeConversationContextUseCase
  ...
  /// Workspace of the most recent call — the history read must be scoped to the
  /// workspace the dispatch runs in, not to the space id alone.
...
class _FakeModeResolver implements ModeResolver {
  Future<Mode> Function(String? conversationId)? onResolve;

  /// Workspace of the most recent resolution — the mode lives in the
  /// workspace's own database file, so the lookup must carry it.
  String? lastWorkspaceId;
...
void main() {
  ...
  group('successful dispatch', () {
    test('resolves agent, builds prompt, returns PreparedDispatch', () async {
      ...
      expect(result.mode, Mode.chat);
      // An agent with no adapter runs on the built-in loop, never on an
      // external CLI the host may not have.
      expect(result.cliName, 'cc-harness');
      expect(result.resolvedAdapterId, 'cc-harness');
    });
    ...
    test('falls back to spaceId when conversationId is null', () async {
      agentRepo.addAgent(_testAgent());
    ...
    test(
      'resolves agent adapterId when no explicit adapterId is given',
      () async {
    ...
    test('an unknown adapter id resolves to the built-in loop', () async {
      ...
      // Reported as what actually ran, not as what was asked for: the run log
      // records this field, and naming an adapter that never ran is a lie the
      // transcript cannot be checked against.
      expect(result.resolvedAdapterId, 'cc-harness');
      expect(result.cliName, 'cc-harness');
    });
    ...
    test('resolves plan mode when modeResolver returns plan', () async {
      agentRepo.addAgent(_testAgent());
      modeResolver.onResolve = (_) async => Mode.plan;
      ...
      expect(result.mode, Mode.plan);
      // Plan mode injects ModePromptContext with the goal and the block names
      // the one verb that delivers a plan.
      expect(result.effectivePrompt, contains('Design a system'));
      expect(result.effectivePrompt, contains('submit_plan'));
      // The raw user text is carried separately so slash commands still parse
      // after the prompt is wrapped in a `<context>` block.
      expect(result.rawUserText, 'Design a system');
    });
  ...
  group('memory context', () {
    ...
    test('skips memory when the agent belongs to another workspace', () async {
      final agent = _testAgent(workspaceId: 'ws-5', id: 'agt-5');
      agentRepo.addAgent(agent);
      memoryUseCase.onExecute =
          ({
      ...
      // The id alone is not an access boundary: `agt-5` lives in ws-5's
      // database file, so a ws-1 dispatch resolves no agent and layers no
      // memory from it.
      expect(result.agent, isNull);
      expect(result.effectivePrompt, isNot(contains('CONTAMINATION')));
    });
  ...
  group('conversation context', () {
    ...
    test('skips when spaceId is null', () async {
      agentRepo.addAgent(_testAgent());
    ...
    test(
      'injects conversation context when space and agent are available',
      () async {
        ...
        // The history read is scoped to the dispatching workspace; a space id
        // on its own addresses no database file.
        expect(conversationUseCase.lastWorkspaceId, _ws);
      },
    );
  ...
  group('mode resolution', () {
    ...
    test('resolves review mode via resolver', () async {
      agentRepo.addAgent(_testAgent());
      modeResolver.onResolve = (_) async => Mode.review;
      ...
      expect(result.mode, Mode.review);
      // The conversation's mode row lives in the workspace's own database file.
      expect(modeResolver.lastWorkspaceId, _ws);
    });
  });
  ...
  group('agent not found', () {
    test('returns PreparedDispatch with null agent', () async {
      ...
      expect(result.agent, isNull);
      // No agent and no explicit adapter: still the built-in loop.
      expect(result.resolvedAdapterId, 'cc-harness');
      expect(result.cliName, 'cc-harness');
    ...
    test(
      'still resolves explicit adapterId even when agent not found',
      () async {
        final useCase = createUseCase();
        final result = await useCase.execute(
          workspaceId: _ws,
  ...
  group('agent repo throws', () {
    test('propagates exception from agent repository', () async {
      ...
      await expectLater(
        () => useCase.execute(workspaceId: _ws, agentId: 'any', prompt: 'test'),
        throwsA(
