...
void main() {
  ...
  group('execute', () {
    test(
      'returns raw prompt when agent is null',
    ...
    test(
      'returns non-empty result when agent is provided',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
        final result = useCase.execute(prompt: 'Do something', agent: agent);
    ...
    test(
      'includes agent identity in result',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'includes persona when provided',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'includes systemPrompt when provided',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'works with different conversation modes',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
    ...
    test(
      'includes conversation context when provided',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
        final result = useCase.execute(
    ...
    test(
      'result without optional fields still contains prompt',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
        final result = useCase.execute(prompt: 'What is 2+2?', agent: agent);
  ...
  group('buildPersistentBrief', () {
    test(
      'returns non-empty string for valid agent',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
    ...
    test(
      'changes with different conversation modes',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
  ...
  group('buildPerTurnPrompt', () {
    ...
    test('works with minimal arguments', timeout: const Timeout.factor(2), () {
      final agent = _testAgent();
      final result = useCase.buildPerTurnPrompt(prompt: 'Hello', agent: agent);
      expect(result, isNotEmpty);
    });
  });
  ...
  group('consistency', () {
    test(
      'execute with agent produces result that contains prompt text',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
    ...
    test(
      'persistent brief + per-turn produce content for same agent',
      timeout: const Timeout.factor(2),
      () {
        final agent = _testAgent();
        final brief = useCase.buildPersistentBrief(agent: agent);
