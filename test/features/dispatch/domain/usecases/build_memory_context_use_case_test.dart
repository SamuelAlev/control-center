...
class _FakePolicyRepo implements MemoryPolicyRepository {
  ...
  @override
  Future<MemoryPolicy?> getById(String workspaceId, String id) async => policies
...
void main() {
  group('BuildMemoryContextUseCase', () {
    test(
      'returns empty string when no policies and no working memory',
      () async {
    ...
    test(
      'returns empty string when working memory is null and no policies',
      () async {
        final useCase = BuildMemoryContextUseCase(
          policyRepository: _FakePolicyRepo(),
    ...
    test(
      'returns empty string when working memory content is whitespace',
      () async {
        final useCase = BuildMemoryContextUseCase(
          policyRepository: _FakePolicyRepo(),
          workingMemoryRepository: _FakeWorkingMemoryRepo()
    ...
    test('includes both policies and working memory', () async {
      final useCase = BuildMemoryContextUseCase(
        policyRepository: _FakePolicyRepo(
          policies: [_policy('p1', 'ws1', 'general', 'Be concise')],
        ),
        workingMemoryRepository: _FakeWorkingMemoryRepo()
    ...
    test(
      'survives policy load failure and still returns working memory',
      () async {
        final useCase = BuildMemoryContextUseCase(
    ...
    test(
      'survives working memory load failure and still returns policies',
      () async {
        final useCase = BuildMemoryContextUseCase(
          policyRepository: _FakePolicyRepo(
            policies: [_policy('p1', 'ws1', 'general', 'Be polite')],
          ),
    ...
    test('output includes search_memory hint', () async {
      final useCase = BuildMemoryContextUseCase(
        policyRepository: _FakePolicyRepo(
          policies: [_policy('p1', 'ws1', 'test', 'Rule')],
        ),
        workingMemoryRepository: _FakeWorkingMemoryRepo(),
      );
