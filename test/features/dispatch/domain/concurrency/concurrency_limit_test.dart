...
void main() {
  group('mapWithConcurrencyLimit', () {
    test('preserves input order in results', () async {
      ...
      final result = await mapWithConcurrencyLimit<int, String>(items, 4, (
        item,
        index,
        signal,
      ) async {
      ...
      expect(result.results, <String>[for (final i in items) 'v$i']);
    });
    ...
    test('peak concurrency never exceeds the limit', () async {
      ...
      final result = await mapWithConcurrencyLimit<int, int>(items, limit, (
        item,
        index,
        signal,
      ) async {
    ...
    test('clamps concurrency to items.length when limit is huge', () async {
      ...
      await mapWithConcurrencyLimit<int, int>(items, 1000, (
        item,
        index,
        signal,
      ) async {
    ...
    test('fails fast and rethrows the first error', () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<StateError>().having((e) => e.message, 'message', 'boom on 2'),
        ),
      );
    ...
    test(
      'external-signal abort returns aborted:true with partial results',
      () async {
        final items = List<int>.generate(8, (i) => i);
        final source = CancellationTokenSource();
        var completed = 0;

        final future = mapWithConcurrencyLimit<int, int>(items, 2, (
          item,
          index,
          signal,
        ) async {
