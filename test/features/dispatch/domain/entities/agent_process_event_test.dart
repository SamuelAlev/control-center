...
void main() {
  ...
  group('AgentProcessEventTypeExtension', () {
    test(
      'name returns correct string for each type',
      ...
      () {
        ...
        expect(
          AgentProcessEventType.sandboxViolation.name,
          'sandbox_violation',
        );
    ...
    group('fromString', () {
      test('parses known values', timeout: const Timeout.factor(2), () {
        ...
        expect(
          AgentProcessEventTypeExtension.fromString('sandbox_violation'),
          AgentProcessEventType.sandboxViolation,
        );
        expect(
          AgentProcessEventTypeExtension.fromString('debug'),
  ...
  group('AgentProcessEvent equality', () {
    test(
      'events with same type and content are equal',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'events of different subtypes are not equal even with same content',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('ToolCallEvent', () {
    ...
    test(
      'metadata omits inputs when null',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('ToolResultEvent', () {
    test(
      'has correct type and content is outputs',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'metadata includes error and partial flags',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'metadata omits toolName when null',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'metadata omits partial when false',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('UsageEvent', () {
    test(
      'has correct type and empty content',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'metadata omits zero-valued optional fields',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('timestamp', () {
    test(
      ...
      () {
        ...
        expect(
          e.timestamp.isAfter(before.subtract(const Duration(milliseconds: 1))),
          isTrue,
        );
        expect(
          e.timestamp.isBefore(after.add(const Duration(milliseconds: 1))),
          isTrue,
        );
      },
    );
