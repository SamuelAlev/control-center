...
void main() {
  ...
  group('SteeringMessage', () {
    test('rejects empty content', () {
      // Was `throwsA(isA<AssertionError>())`. `assert` is stripped in release,
      // so in the shipped server binary an empty steering message was accepted
      // and injected as an empty user turn — a turn that tells the model
      // nothing and costs a round trip. It throws now.
      expect(
        () => SteeringMessage(
          content: '',
  ...
  group('SteeringQueue drain', () {
    test('drain returns FIFO order', () {
      ...
      expect(drained.map((m) => m.content).toList(), <String>[
        'first',
        'second',
        'third',
      ]);
