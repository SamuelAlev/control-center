...
void main() {
  ...
  group('register', () {
    ...
    test('re-registering a known agent preserves createdAt and emits '
  ...
  group('setStatus', () {
    test(
      'running -> idle clears activity and dispatch, emits change',
      () async {
  ...
  group('setActivity', () {
    test(
      'records a normalized gist while running, without emitting an event',
      () async {
        registry.register(_input('a1'));
        final events = <RegistryEvent>[];
        final sub = registry.changes.listen(events.add);
  ...
  group('workspace isolation', () {
    test('listForWorkspace returns only that workspace\'s agents', () {
      ...
      expect(registry.listForWorkspace('ws-1').map((r) => r.id).toSet(), {
        'a1',
        'a2',
      });
      expect(registry.listForWorkspace('ws-2').map((r) => r.id).toSet(), {
        'b1',
      });
  ...
  group('watchWorkspaceRoster', () {
    test(
      'emits the current snapshot immediately, then on every change',
      () async {
        registry.register(_input('a1', workspaceId: 'ws-1'));

        final snapshots = <List<AgentRef>>[];
        final sub = registry.watchWorkspaceRoster('ws-1').listen(snapshots.add);
        await pumpEventQueue();
    ...
    test(
      'does NOT re-emit when another workspace changes (isolation)',
      () async {
        registry.register(_input('a1', workspaceId: 'ws-1'));
