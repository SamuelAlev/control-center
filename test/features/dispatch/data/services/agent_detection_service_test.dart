...
void main() {
  ...
  group('AdapterDetectionResult', () {
    test(
      'constructs with required fields only',
    ...
    test(
      'constructs with all fields set',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'constructs with notAuthenticated and authError',
      timeout: const Timeout.factor(2),
      () {
        final now = DateTime.now();
        final result = AdapterDetectionResult(
    ...
    test(
      'configDir can be set to non-null',
      timeout: const Timeout.factor(2),
      () {
        const result = AdapterDetectionResult(
          cliName: 'claude',
    ...
    test(
      'unknown status represents undetected state',
      timeout: const Timeout.factor(2),
      () {
        const result = AdapterDetectionResult(
  ...
  group('AgentDetectionService', () {
    test(
      'can be constructed with default cacheTtl',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'can be constructed with custom cacheTtl',
      timeout: const Timeout.factor(2),
      () {
        final service = AgentDetectionService(
    ...
    test(
      'detect returns a map keyed by cli names',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
    ...
    test(
      'detect result entries have correct cliName',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'detect result status is one of valid enum values',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'detect sets binaryPath for installed adapters',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'detect sets notInstalled when binary is absent',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'getCached returns detect when cache is empty',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
    ...
    test(
      'getCached returns cached result on second call',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService(
    ...
    test(
      'authError is null for ready status',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'cacheTtl can be set to very short duration',
      timeout: const Timeout.factor(2),
      () {
        final service = AgentDetectionService(
    ...
    test(
      'getCached returns unmodifiable map',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService(
          cacheTtl: const Duration(hours: 1),
    ...
    test(
      'detect result has correct number of adapters',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'consecutive detect calls return fresh results',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final first = await service.detect();
    ...
    test(
      'getCached with zero TTL always re-detects',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService(cacheTtl: Duration.zero);
        final first = await service.getCached();
    ...
    test(
      'detect returns mutable map (not unmodifiable)',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'configDir is populated for claude when directory exists',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'detect result fields are consistent with status',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService();
        final results = await service.detect();
    ...
    test(
      'getCached after detect returns same statuses',
      timeout: const Timeout.factor(2),
      () async {
        final service = AgentDetectionService(
          cacheTtl: const Duration(hours: 1),
