...
class _FakeAgentDispatchPort implements AgentDispatchPort {
  ...
  @override
  Future<bool> pauseDispatch(String dispatchId) async => false;
...
class _FakeRunLogRepository implements AgentRunLogRepository {
  ...
  @override
  Future<AgentRunLog?> getById(String workspaceId, String id) async => logs[id];
  @override
  ...
  @override
  Future<List<AgentRunLog>> forPipelineRun(String w, String r) async =>
      const [];
  @override
  ...
  @override
  Stream<List<AgentRunLog>> watchActiveBySpace(String w, String c) =>
      Stream.value([]);
}
...
void main() {
  ...
  test(
    'dispatch registers the agent as running with display name + dispatch',
    () async {
  ...
  test('a workspace-less dispatch is not tracked in the registry', () async {
    // The roster is per-workspace and an empty workspace id names none, so the
    // run joins no roster.
    await service(agent: _agent()).dispatch(
      agentId: 'a1',
      prompt: 'do work',
  ...
  test(
    'tool-call events become the agent activity as the stream is consumed',
    () async {
  ...
  test(
    'a DoneEvent flips the agent to idle (clearing activity + dispatch)',
    () async {
      final result = await service(agent: _agent()).dispatch(
        agentId: 'a1',
        prompt: 'do work',
