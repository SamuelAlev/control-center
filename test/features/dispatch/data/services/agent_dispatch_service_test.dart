...
class _FakeAgentDispatchPort implements AgentDispatchPort {
  ...
  /// What the last `start` was handed about Claude Code accounts — the two
  ...
  DispatchHandle start({
    ...
    startCallCount++;
    lastClaudeConfigDir = claudeConfigDir;
    lastClaudeAccountsSpent = claudeAccountsSpent;
...
class _FakeRepoProvisioner implements RepoWorkspaceProvisionerPort {
  ...
  Future<String> ensureSpaceWorkspace({
    ...
    // Accepted to match the port; the fake never triggers a setup script.
...
void main() {
  ...
  group('AgentDispatchService', () {
    ...
    // The Claude lane's gate lives in the SERVICE, not the session, because the
    // account plan feeds the sandbox profile: a run parked deeper down and
    // resumed on a different account would hold a directory its own sandbox
    // denies. These pin that the plan is genuinely re-resolved.
    group('the credential gate', () {
      /// A resolver that refuses until [fixedAfter] calls, then succeeds.
      ({
        Future<ClaudeAccountPlan?> Function({
          String? workspaceId,
          String? conversationId,
      ...
      resolverThatHealsAfter(int fixedAfter) {
        final calls = <int>[];
        Future<ClaudeAccountPlan?> resolve({
          String? workspaceId,
          String? conversationId,
          String? agentId,
          String? accountId,
          ...
          if (calls.length <= fixedAfter) {
            return const ClaudeAccountPlan(
              accounts: [],
              refusal: (
          ...
          return const ClaudeAccountPlan(
            accounts: [(accountId: 'work', configDir: '/tmp/claude/work')],
          );
        }
      ...
      test('a resolved block re-resolves the plan and runs on it', () async {
        final resolver = resolverThatHealsAfter(1);
        final gate = _FakeGate([RunCredentialOutcome.resolved]);
        final service = createService(
          resolveClaudeConfigDir: resolver.resolve,
          credentialGate: gate,
        );
        ...
        expect(gate.requests.single.reason, RunCredentialReason.planSpent);
        expect(
          dispatchPort.lastClaudeConfigDir,
          '/tmp/claude/work',
          reason: 'the run starts on what the operator just fixed',
        );
        expect(
          dispatchPort.lastClaudeAccountsSpent,
          isNull,
          reason: 'a resumed run carries no refusal',
        );
      });

      test(
        'a plan that still refuses re-parks rather than losing the turn',
        () async {
          // Pressing "check again" too early, or a second account expiring while
          // the first was being fixed. The registry's deadline bounds the wait,
          // so this cannot spin forever.
          final resolver = resolverThatHealsAfter(2);
          ...
          expect(dispatchPort.lastClaudeConfigDir, '/tmp/claude/work');
        },
      );

      test('a cancelled block hands the refusal on, unchanged', () async {
        final resolver = resolverThatHealsAfter(99);
        ...
        expect(
          dispatchPort.lastClaudeAccountsSpent?.reason,
          RunCredentialReason.planSpent,
          reason: 'the session reports it exactly as it did before the gate',
        );
      });

      test('with no gate wired the refusal goes straight through', () async {
        final resolver = resolverThatHealsAfter(99);
        final service = createService(resolveClaudeConfigDir: resolver.resolve);
        ...
        expect(resolver.calls, hasLength(1), reason: 'resolved once, no loop');
        expect(
          dispatchPort.lastClaudeAccountsSpent?.reason,
          RunCredentialReason.planSpent,
        );
    ...
    test(
      // Post-cutover contract: a conversation id is not a space id. A run
      // dispatched with a conversation but NO space cannot provision a
      // space-owned CoW worktree, so it honestly falls back to the agent dir.
      'dispatch with no space falls back to the agent dir without provisioning',
      timeout: const Timeout.factor(2),
      () async {
        final provisioner = _FakeRepoProvisioner('/provisioned/work');
        final service = createService(repoProvisioner: provisioner);
...
/// A gate that answers with a scripted sequence of outcomes, one per block.
class _FakeGate implements RunCredentialGatePort {
  _FakeGate(this._outcomes);

  final List<RunCredentialOutcome> _outcomes;
  ...
  Future<RunCredentialOutcome> awaitCredentials(
    ...
    return requests.length <= _outcomes.length
        ? _outcomes[requests.length - 1]
