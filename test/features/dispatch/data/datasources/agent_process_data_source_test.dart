...
void main() {
  group('AgentProcessDataSource', () {
    ...
    group('handlePiEvent', () {
      ...
      test(
        'handles "event" type with thinking eventType',
      ...
      test(
        'handles "event" type with text eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "event" type with error eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "event" type with done eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "event" type with tool_call eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "event" type with tool_result eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          expect(
            (events.first as ToolResultEvent).outputs,
            'file contents here',
          );
        },
      );
      ...
      test(
        'handles "event" type with sandbox_violation eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          expect(
            (events.first as SandboxViolationEvent).content,
            'Access denied',
          );
        },
      );
      ...
      test(
        'handles "message_update" with text_delta',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "message_update" with thinking_delta',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "message_update" with empty delta — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "message_update" with null assistantMessageEvent — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          dataSource.handlePiEvent({'type': 'message_update'});
      ...
      test(
        'handles "tool_execution_end" with error',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "agent_end" as DoneEvent',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          dataSource.handlePiEvent({'type': 'agent_end'});
      ...
      test('ignores "start" type', timeout: const Timeout.factor(2), () async {
        ...
        dataSource.handlePiEvent({'type': 'start'});
      ...
      test('ignores "end" type', timeout: const Timeout.factor(2), () async {
        ...
        dataSource.handlePiEvent({'type': 'end'});
      ...
      test(
        ...
        () async {
          ...
          dataSource.handlePiEvent({'type': 'session'});
      ...
      test(
        'emits a parse-error marker for an unknown event type',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          // Hardening: unknown event types surface a grep-able debug marker
          // instead of being silently dropped.
          expect(events, hasLength(1));
          expect(events.single, isA<DebugEvent>());
          expect(
            (events.single as DebugEvent).content,
            contains('ndjson-parse-error'),
          );
        },
      );

      test(
        'emits a parse-error marker for a missing type key',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'emits a parse-error marker for "event" with missing eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "message_update" with unknown subtype — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          dataSource.handlePiEvent({
            'type': 'message_update',
            'assistantMessageEvent': {'type': 'tool_use', 'delta': 'some args'},
          });
      ...
      test(
        'handles "message_update" with empty assistantMessageEvent map — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "tool_execution_end" with null result — empty outputs',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "tool_execution_update" with non-map partialResult — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handles "tool_execution_update" with non-list content — no event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
          ...
          dataSource.handlePiEvent({
            'type': 'tool_execution_update',
            'toolCallId': 'call-6',
            'partialResult': {'content': 'not a list'},
          });
      ...
      test(
        'flushes buffered text before emitting non-buffered error event',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'handlePiEvent does not crash when no controller set',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('event coalescing', () {
      test(
        'coalesces consecutive text events',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'flushes buffered event when type changes',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'coalesces three or more events of same type',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'flushBufferedEvent when nothing buffered — no events',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'empty content does not set bufferedType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
    ...
    group('tool_execution_update', () {
      test(
        'handles partial result with text content',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'ignores partial result with no text blocks',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'ignores partial result with empty text',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'ignores tool_execution_update with no partialResult',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
    ...
    group('debug event', () {
      test(
        'handles "event" type with debug eventType',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
    ...
    group('initTestController', () {
      ...
      test(
        'multiple calls replace the controller',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('smoke test', () {
      ...
      test(
        'can be instantiated with an event bus',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('controller lifecycle', () {
      test(
        'stop() closes controller — new events not emitted',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'stop() flushes buffered events before closing',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'stopDispatch() closes controller',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'stopAllForAgent() closes controller',
        timeout: const Timeout.factor(2),
        () async {
          final stream = dataSource.initTestController();
          final events = <AgentProcessEvent>[];
      ...
      test(
        'multiple stop calls do not throw',
        timeout: const Timeout.factor(2),
        () async {
          dataSource.initTestController();
