...
void main() {
  group('AgentProcessDataSource', () {
    ...
    group('handlePiEvent', () {
      ...
      test('emits a parse-error marker for an unknown event type', timeout: const Timeout.factor(2), () async {
        final stream = dataSource.initTestController();
        final events = <AgentProcessEvent>[];
        stream.listen(events.add);
        ...
        expect((events.single as DebugEvent).content,
            contains('ndjson-parse-error'));
      });

      test('emits a parse-error marker for a missing type key', timeout: const Timeout.factor(2), () async {
        final stream = dataSource.initTestController();
        final events = <AgentProcessEvent>[];
        stream.listen(events.add);
      ...
      test('emits a parse-error marker for "event" with missing eventType', timeout: const Timeout.factor(2), () async {
        final stream = dataSource.initTestController();
        final events = <AgentProcessEvent>[];
        stream.listen(events.add);
