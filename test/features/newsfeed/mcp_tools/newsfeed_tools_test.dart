...
class _FakeNewsfeedRepository implements NewsfeedRepository {
  ...
  @override
  Stream<List<RssFeed>> watchFeeds(String userId) => Stream.value(feeds);
  ...
  @override
  Stream<List<RssArticle>> watchSavedArticles(String userId) => Stream.value(
    articles.where((a) => savedOverrides[a.id] ?? a.saved).toList(),
  );
  ...
  dynamic noSuchMethod(Invocation invocation) => throw UnimplementedError(
    'Not used in newsfeed tool tests: ${invocation.memberName}',
  );
}
...
void main() {
  group('ListFeedsTool', () {
    test('lists all feeds', () async {
      final repo = _FakeNewsfeedRepository(
        feeds: [_feed('a'), _feed('b', enabled: false)],
      );
      final tool = ListFeedsTool(repository: repo, userId: 'owner');
  ...
  group('ListArticlesTool', () {
    test('filters by feed_id', () async {
      ...
      final tool = ListArticlesTool(repository: repo, userId: 'owner');
    ...
    test('unread_only returns only unread', () async {
      final repo = _FakeNewsfeedRepository(
        articles: [_article('1', 'f1', read: true), _article('2', 'f1')],
      );
      final tool = ListArticlesTool(repository: repo, userId: 'owner');
      final res = await tool.run({'unread_only': true});
      final body = jsonDecode(res.content.first.text) as Map<String, dynamic>;
      expect(body['count'], 1);
      expect(((body['articles'] as List).first as Map)['id'], '2');
    });

    test('saved_only overrides unread_only', () async {
      final repo = _FakeNewsfeedRepository(
        articles: [_article('1', 'f1', saved: true, read: true)],
      );
      final tool = ListArticlesTool(repository: repo, userId: 'owner');
  ...
  group('GetArticleTool', () {
    test('returns article json', () async {
      ...
      final tool = GetArticleTool(repository: repo, userId: 'owner');
    ...
    test('errors on missing article_id', () async {
      final tool = GetArticleTool(
        repository: _FakeNewsfeedRepository(),
        userId: 'owner',
      );
      final res = await tool.run({});
  ...
  group('SetArticleReadTool', () {
    test('marks read by default', () async {
      final repo = _FakeNewsfeedRepository();
      final tool = SetArticleReadTool(repository: repo, userId: 'owner');
      final res = await tool.run({'article_id': '1'});
      final body = jsonDecode(res.content.first.text) as Map<String, dynamic>;
  ...
  group('SetArticleSavedTool', () {
    test('saves by default', () async {
      final repo = _FakeNewsfeedRepository();
      final tool = SetArticleSavedTool(repository: repo, userId: 'owner');
  ...
  group('RefreshFeedsTool', () {
    test('calls refreshAll', () async {
      final repo = _FakeNewsfeedRepository();
      final tool = RefreshFeedsTool(repository: repo, userId: 'owner');
      final res = await tool.run({});
