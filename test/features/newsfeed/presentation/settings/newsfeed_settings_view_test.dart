...
Future<List> _defaultOverrides({
  ...
  final sp = AppPreferences.inMemory(prefs ?? {});
  return [
...
Future<List> _inlineOverrides() async {
  final sp = AppPreferences.inMemory({});
  return [
    appPreferencesProvider.overrideWithValue(sp),
    articleOpenModeProvider.overrideWith(ArticleOpenModeController.new),
    contentBlockingProvider.overrideWith(ContentBlockingController.new),
...
Future<List> _blockingOverrides({
  ...
  final sp = AppPreferences.inMemory({'newsfeed.blockContent': true});
  return [
    appPreferencesProvider.overrideWithValue(sp),
    feedsProvider.overrideWith((ref) => Stream.value(feeds ?? <RssFeed>[])),
    articleOpenModeProvider.overrideWith(ArticleOpenModeController.new),
    ...
    siteAllowlistProvider.overrideWith((ref) => Stream.value(trustedSites)),
  ];
}
...
Future<void> _pumpScreen(WidgetTester tester, {required List overrides}) async {
  await tester.pumpWidget(
    ProviderScope(
      overrides: [...overrides],
      child: _wrap(const NewsfeedSettingsView()),
    ),
  );
  await tester.pump();
...
void main() {
  ...
  group('NewsfeedSettingsView', () {
    ...
    testWidgets('renders Refresh all button', (tester) async {
      ...
      await _pumpScreen(
        tester,
        overrides: await _defaultOverrides(feeds: [_makeFeed()]),
      );
    ...
    testWidgets('renders feed URL as subtitle when no description', (
      ...
      await _pumpScreen(
        ...
        overrides: await _defaultOverrides(
          feeds: [
            _makeFeed(name: 'My Feed', url: 'https://rss.example.com/atom'),
          ],
        ),
      );
    ...
    testWidgets('renders error message on feed with error', (tester) async {
      ...
      await _pumpScreen(
        ...
        overrides: await _defaultOverrides(
          feeds: [
            _makeFeed(name: 'Broken Feed', lastError: 'Connection timed out'),
          ],
        ),
      );
    ...
    testWidgets('does NOT treat empty lastError as an error', (tester) async {
      _setupViewport(tester);
      final feed = RssFeed(
        id: 'f1',
        name: 'Clean Feed',
      ...
      await _pumpScreen(
        tester,
        overrides: await _defaultOverrides(feeds: [feed]),
      );
    ...
    testWidgets('renders refresh button per feed', (tester) async {
      ...
      expect(find.byIcon(AppIcons.refreshCw), findsWidgets);
    });
    ...
    testWidgets('renders delete button per feed', (tester) async {
      ...
      expect(find.byIcon(AppIcons.trash2), findsWidgets);
    });
    ...
    testWidgets('renders relative updated label for days ago', (tester) async {
      _setupViewport(tester);
    ...
    testWidgets('multiple feeds render all rows', (tester) async {
      ...
      expect(find.byIcon(AppIcons.trash2), findsAtLeast(2));
    });
    ...
    testWidgets('Refresh all button shows progress while refreshing', (
      ...
      await _pumpScreen(
        ...
        overrides: [
          appPreferencesProvider.overrideWithValue(sp),
          feedsProvider.overrideWith((ref) => Stream.value([_makeFeed()])),
          articleOpenModeProvider.overrideWith(ArticleOpenModeController.new),
          contentBlockingProvider.overrideWith(ContentBlockingController.new),
          newsfeedRefreshControllerProvider.overrideWith(
      ...
      await tester.pump(const Duration(milliseconds: 300));
      expect(find.text('Refresh all'), findsOneWidget);
      // The refresh button renders its progress via CcButton(loading: true)
      // (which shows CcButton's own inline spinner), not a bare CcSpinner.
      final refreshButton = tester.widget<CcButton>(
        find.ancestor(
          of: find.text('Refresh all'),
          matching: find.byType(CcButton),
        ),
      );
      expect(refreshButton.loading, isTrue);
    });

    // ── Settings toggles ────────────────────────────────────────────────

    testWidgets(
      'toggling content blocking off hides filter + trusted sections',
      (tester) async {
        _setupViewport(tester);
    ...
    testWidgets('shows bundled defaults label when never updated', (
      ...
      await _pumpScreen(
        tester,
        overrides: await _blockingOverrides(
      ...
      expect(
        find.text('Bundled defaults \u2014 never updated'),
        findsOneWidget,
      );
    });
      ...
      await _pumpScreen(
        ...
        overrides: await _blockingOverrides(
          filterState: const FilterListUpdateState(
            isUpdating: false,
            errors: [],
            cookieHidingRules: 12,
            adHidingRules: 34,
            networkBlockRules: 56,
            removeParamsCount: 78,
          ),
        ),
      );
    ...
    testWidgets('shows updating state with progress indicator', (tester) async {
      _setupViewport(tester);
      await _pumpScreen(
        tester,
        overrides: await _blockingOverrides(
      ...
      expect(find.text('Check for updates'), findsOneWidget);
      // The update button renders its progress via CcButton(loading: true)
      // (CcButton's own inline spinner), not a bare CcSpinner.
      final updateButton = tester.widget<CcButton>(
        find.ancestor(
          of: find.text('Check for updates'),
          matching: find.byType(CcButton),
        ),
      );
      expect(updateButton.loading, isTrue);
    });
    ...
    testWidgets('shows empty state when no trusted sites', (tester) async {
      ...
      await _pumpScreen(
        tester,
        overrides: await _blockingOverrides(trustedSites: {}),
      );
    ...
    testWidgets('each trusted site row has a remove button', (tester) async {
      ...
      await _pumpScreen(
        tester,
        overrides: await _blockingOverrides(trustedSites: {'example.com'}),
      );
    ...
    testWidgets('feed with long name renders', (tester) async {
      ...
      await _pumpScreen(
        tester,
        overrides: await _defaultOverrides(feeds: [_makeFeed(name: longName)]),
      );
