...
/// The shared RPC-adapter fake, plus a canned saved-articles stream (the
/// screen's saved view reads it separately from the recent window).
class _FakeNewsfeedRepository extends FakeRpcNewsfeedRepository {
  ...
  @override
  Stream<List<RssArticle>> watchSavedArticles() => Stream.value(savedArticles);
}
...
Future<Override> _sharedPrefsOverride() async {
  ...
  return appPreferencesProvider.overrideWithValue(sp);
}
