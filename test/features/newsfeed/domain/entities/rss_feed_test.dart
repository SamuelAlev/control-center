...
void main() {
  ...
  group('RssFeed', () {
    group('constructor', () {
      ...
      test('rejects an empty name', timeout: const Timeout.factor(2), () {
        expect(
          () => RssFeed(
            id: 'f',
            name: '',
      ...
      test('rejects an empty url', timeout: const Timeout.factor(2), () {
        expect(
          () => RssFeed(
            id: 'f',
            name: 'n',
    ...
    group('hasError', () {
      test(
        'returns false when lastError is null',
        timeout: const Timeout.factor(2),
        () {
          final f = createFeed();
      ...
      test(
        'returns false when lastError is empty',
        timeout: const Timeout.factor(2),
        () {
      ...
      test(
        'returns true when lastError is non-empty',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('copyWith', () {
      test(
        'returns identical feed with no arguments',
        timeout: const Timeout.factor(2),
        () {
      ...
      test(
        'clears lastError by passing null',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('== and hashCode', () {
      ...
      test(
        'hashCode matches for equal feeds',
        timeout: const Timeout.factor(2),
        () {
          final f1 = createFeed();
          final f2 = createFeed();
      ...
      test(
        'hashCode differs for different feeds',
        timeout: const Timeout.factor(2),
        () {
          final f1 = createFeed(id: 'feed-1');
          final f2 = createFeed(id: 'feed-2');
