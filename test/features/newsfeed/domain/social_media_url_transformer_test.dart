  ...
  group('transformSocialMediaUrl', () {
    ...
    group('no match', () {
      test('returns url unchanged for non-social domains', () {
        final result = transformSocialMediaUrl('https://example.com/article');
        expect(result, 'https://example.com/article');
      });

      test(
        'returns url unchanged for subdomains that are not social media',
        () {
          final result = transformSocialMediaUrl(
    ...
    group('edge cases', () {
      ...
      test('handles http scheme', () {
        final result = transformSocialMediaUrl('http://x.com/user/status/789');
        expect(result, 'http://xcancel.com/user/status/789');
      });

      test('is case-insensitive for domain', () {
        final result = transformSocialMediaUrl('https://X.COM/user/status');
