...
void main() {
  ...
  group('add / remove / read / watch', () {
    test('add persists across instances', () async {
      final prefs = AppPreferences.inMemory(<String, Object>{});
      final writer = SharedPrefsSiteAllowlistRepository(prefs);
      await writer.add('example.com');
      // A separate repository instance over the same backing store reads the
      // persisted entry (each AppPreferences.inMemory() is its own store, so
      // persistence is scoped to the shared prefs, not to the repo object).
      final fresh = SharedPrefsSiteAllowlistRepository(prefs);
