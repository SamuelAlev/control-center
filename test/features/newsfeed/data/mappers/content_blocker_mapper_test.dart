...
void main() {
  group('buildContentBlockers', () {
    ...
    test('builds a block action from a valid entry', () async {
      ...
      expect(blockers.first.action.type, ContentBlockerActionType.BLOCK);
      expect(blockers.first.trigger.urlFilter, '.*ads\\.example\\.com.*');
    });
    ...
    test('skips css-display-none with null selector', () async {
      final blockers = buildContentBlockers([
        {
          'trigger': {'url-filter': '.*'},
          'action': {'type': 'css-display-none', 'selector': null},
        },
      ]);
      expect(blockers, isEmpty);
    });
    ...
    test('passes valid if-domain entries', () async {
      ...
      expect(blockers.length, 1);
      expect(blockers.first.trigger.ifDomain, [
        'example.com',
        '*sub.example.com',
      ]);
    });
    ...
    test('ignores unknown resource types', () async {
      ...
      expect(blockers.length, 1);
      expect(blockers.first.trigger.resourceType, [
        ContentBlockerTriggerResourceType.IMAGE,
      ]);
    });
      ...
      final blockers = buildContentBlockers([
        {
          'trigger': {
            'url-filter': '.*',
            'resource-type': ['image', 'media', 'script', 'stylesheet', 'font'],
          },
          'action': {'type': 'block'},
        },
      ]);
