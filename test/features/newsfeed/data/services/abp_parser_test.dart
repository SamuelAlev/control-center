...
''';
    ...
    group('network rules', () {
      test('parses simple domain block', () {
        ...
        expect((entry['trigger'] as Map<String, dynamic>)['if-domain'], [
          '*doubleclick.net',
        ]);
...
''';
      ...
      expect(
        result.blocklist.any(
          (b) => ((b['trigger'] as Map<String, dynamic>)['if-domain'] as List)
              .contains('*tracking.com'),
        ),
        isTrue,
      );
      expect(
    ...
    test('parses multi-domain scriptlet rules', () {
      const raw = 'a.com,b.org##+js(no-setInterval-if, /ad-poll/)';
      final result = AbpParser.parse(raw);
      expect(result.scriptlets, hasLength(1));
      final s = result.scriptlets.first;
...
''';
    ...
    test('scriptlet args strip surrounding double/single/backtick quotes', () {
      for (final raw in <String>[
        ...
        expect(result.scriptlets.first.args, [
          'foo',
          'bar baz, qux',
        ], reason: raw);
      }
