...
const _rssGarbageXml =
    '''This is not XML at all, just random text. No tags here. & < > everything broken.''';

const _rssMissingSpace = '''<?xml version="1.0"?>
<rss version="2.0">
...
<dc:creator>Charlie</dc:creator></item>
  ...
  group('HTTP error responses', () {
    test('throws DioException on 404 Not Found', () async {
      ...
      expect(
        () =>
            service.fetchAndParse(feedId: 'f', url: 'https://example.com/feed'),
        throwsA(isA<DioException>()),
      );
    });
  ...
  group('enclosure handling', () {
    ...
    test(
      'image enclosure takes priority over HTML img in description',
      () async {
  ...
  group('CDATA edge cases', () {
    ...
    test('treats XML-like content inside CDATA as plain text', () async {
      ...
      expect(articles.single.summary, 'Not realnope');
    });
  });
  ...
  group('date parsing variants', () {
    ...
    test('parses ISO 8601 date without timezone', () async {
      ...
      // Without a timezone suffix the timestamp is interpreted as local time,
      // so its UTC value depends on the runner's zone. Assert the local-time
      // interpretation instead — machine-independent while still proving the
      // no-zone date parsed to the correct wall-clock instant.
      expect(articles.single.publishedAt, isNotNull);
      expect(
        articles.single.publishedAt!.toUtc(),
        DateTime(2025, 6, 2, 12, 0, 0).toUtc(),
      );
    });
  ...
  group('malformed XML extras', () {
    ...
    test('throws on RSS with missing channel element', () async {
      adapter.respondWith(_rssMissingSpace);

      // rss_dart fails to find <space>, throws ArgumentError which
  ...
  group('summary edge cases', () {
    test('Atom entry uses summary when content is missing', () async {
      ...
      expect(
        articles.single.summary,
        'Plain text summary when no content element',
      );
    });
  });
  ...
  group('Atom edge cases', () {
    ...
    test(
      'Atom entry with empty author name yields empty author string',
      () async {
        const xml =
            '<?xml version="1.0"?>'
            '<feed xmlns="http://www.w3.org/2005/Atom">'
  ...
  group('format detection fallback', () {
    test(
      'parses RSS when Atom detection falsely triggers (Atom→RSS fallback)',
      () async {
    ...
    test(
      'social media transform and UTM stripping both applied to same URL',
      () async {
