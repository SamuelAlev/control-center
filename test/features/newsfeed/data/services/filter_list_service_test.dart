...
void main() {
  ...
  group('FilterListService.buildCssDisplayNoneRules', () {
    ...
    test('url-filter is always wildcard', () {
      ...
      expect(
        (rules.single['trigger'] as Map<String, dynamic>)['url-filter'],
        '.*',
      );
    });
    ...
    test('action type is css-display-none', () {
      ...
      expect(
        (rules.single['action'] as Map<String, dynamic>)['type'],
        'css-display-none',
      );
    });

    test('exactly chunkSize selectors produce one rule', () {
      final selectors = List.generate(
        FilterListService.cssChunkSize,
        (i) => '.sel-$i',
      );
      expect(
        FilterListService.buildCssDisplayNoneRules(selectors),
        hasLength(1),
      );
    });

    test('chunkSize + 1 selectors produce two rules', () {
      final selectors = List.generate(
    ...
    test('first chunk has chunkSize selectors, second has remainder', () {
      final selectors = List.generate(
      ...
      final firstSelector =
          (rules[0]['action'] as Map<String, dynamic>)['selector'] as String;
      final secondSelector =
          (rules[1]['action'] as Map<String, dynamic>)['selector'] as String;
      expect(
        firstSelector.split(', '),
    ...
    test('large list produces correct number of chunks', () {
      const total = 100;
  ...
  group('FilterListService.buildDomainScopedHideRules', () {
    ...
    test('two hides with same domain set are grouped in one rule', () {
      ...
      expect((rules.single['trigger'] as Map<String, dynamic>)['if-domain'], [
        '*a.com',
      ]);
    });
    ...
    test('domain order invariance: same set in different order → grouped', () {
      final rules = FilterListService.buildDomainScopedHideRules([
    ...
    test('multi-domain hide rule has sorted if-domain list', () {
      final rules = FilterListService.buildDomainScopedHideRules([
    ...
    test('multiple domain groups each chunk independently', () {
    ...
    test('selector order preserved within each domain group', () {
      final rules = FilterListService.buildDomainScopedHideRules([
  ...
  group('FilterListService.buildScriptletRules', () {
    ...
    test('universal scriptlet (no domains) has url-filter only', () {
      final rules = FilterListService.buildScriptletRules([
        const ScriptletInjection(domains: [], name: 'noeval', args: []),
      ]);
      expect(rules, hasLength(1));
    ...
    test('action type is scriptlet', () {
      final rules = FilterListService.buildScriptletRules([
        const ScriptletInjection(domains: [], name: 'noop', args: []),
      ]);
      expect(
        (rules.single['action'] as Map<String, dynamic>)['type'],
        'scriptlet',
    ...
    test('multiple scriptlets produce multiple rules', () {
      final rules = FilterListService.buildScriptletRules([
        const ScriptletInjection(domains: ['*a.com'], name: 's1', args: []),
        const ScriptletInjection(domains: ['*b.com'], name: 's2', args: ['a']),
      ]);
      expect(rules, hasLength(2));
    ...
    test('scriptlet with multi-domain if-domain preserves domain list', () {
      final rules = FilterListService.buildScriptletRules([
        const ScriptletInjection(
          domains: ['*a.com', '*b.com'],
    ...
    test('url-filter is always wildcard for scriptlets', () {
      final rules = FilterListService.buildScriptletRules([
        const ScriptletInjection(domains: ['*x.com'], name: 'test', args: []),
      ]);
      expect(
        (rules.single['trigger'] as Map<String, dynamic>)['url-filter'],
        '.*',
  ...
  group('FilterListService.parseRemoveParams', () {
    ...
    test('skips domain-specific rules', () {
      final result = svc.parseRemoveParams('example.com\$removeparam=tracking');
      expect(result, isEmpty);
    });

    test('skips regex patterns', () {
      final result = svc.parseRemoveParams('*\$removeparam=/utm_.*/');
      expect(result, isEmpty);
    });
...
''');
    ...
    test('allows wildcard prefix before removeparam', () {
      final result = svc.parseRemoveParams('*\$removeparam=fbclid');
      expect(result, contains('fbclid'));
    });
    ...
    test('multi-param values are trimmed after split', () {
      final result = svc.parseRemoveParams('*\$removeparam= a | b | c ');
    ...
    test('wildcard domain followed by domain-specific comma', () {
      ...
      final result = svc.parseRemoveParams('*\$removeparam=foo,~script');
  ...
  group('FilterListService.parseSource', () {
    ...
    test('ads source routes selectors to ads list and blocks to network', () {
      ...
      svc.parseSource(
        raw,
        const AbpSource(name: 'test', url: '', category: FilterCategory.ads),
        adsSelectors,
        cookiesSelectors,
        domainHides,
        networkBlocks,
      ...
      expect(
        (networkBlocks.single['action'] as Map<String, dynamic>)['type'],
        'block',
      );
    });
  ...
  group('Exception / whitelist rules', () {
    ...
    test('exception rules do not affect adjacent active rules', () {
      ...
      final trigger =
          result.blocklist.single['trigger'] as Map<String, dynamic>;
  ...
  group('Network block rule shape', () {
    ...
    test('subdomain block prefix is normalized', () {
      ...
      final result = AbpParser.parse(raw);
      expect(
        (result.blocklist.single['trigger']
    ...
    test('network block output merges correctly with hide rules', () {
      ...
      expect(
        (merged[1]['action'] as Map<String, dynamic>)['type'],
        'css-display-none',
      );
    });
  });
  ...
  group('Edge cases', () {
    ...
    test(
      'buildDomainScopedHideRules with empty domains (should not happen)',
      () {
    ...
    test('parseRemoveParams handles DOS-style line endings', () {
      final result = _service.parseRemoveParams(
        '*\$removeparam=foo\r\n*\$removeparam=bar',
      );
      expect(result, contains('foo'));
    ...
    test('rule list merging preserves order: blocks before hides', () {
      ...
      final cssRules = FilterListService.buildCssDisplayNoneRules(['.ad']);
    ...
    test('all filter content types coexist in single rule list', () {
      final networkBlocks = AbpParser.parse('||blocked.com^\n').blocklist;
      final cssRules = FilterListService.buildCssDisplayNoneRules([
        '.ad-banner',
      ]);
      final domainRules = FilterListService.buildDomainScopedHideRules([
        const DomainHide(domains: ['*example.com'], selector: '.cookie'),
      ]);
      ...
      final types = merged
          .map((r) => (r['action'] as Map<String, dynamic>)['type'] as String)
          .toList();
      expect(types, [
        'block',
        'css-display-none',
  ...
  group('parseRemoveParams additional edge cases', () {
    ...
    test('wildcard domain with dot is NOT filtered out', () {
      ...
      final result = svc.parseRemoveParams('*example.com\$removeparam=foo');
      expect(result, contains('foo'));
    });

    test('bare wildcard domain with dot is NOT filtered out', () {
      final result = svc.parseRemoveParams('*.example.com\$removeparam=bar');
      expect(result, contains('bar'));
    });
  ...
  group('buildCssDisplayNoneRules chunking boundaries', () {
    ...
    test(
      'each chunk has exactly chunkSize selectors when total is multiple',
      () {
        final selectors = List.generate(
          FilterListService.cssChunkSize * 3,
          (i) => '.sel-$i',
        ...
        for (final rule in rules) {
          final selector =
              (rule['action'] as Map<String, dynamic>)['selector'] as String;
          expect(
            selector.split(', '),
            hasLength(FilterListService.cssChunkSize),
          );
  ...
  group('buildDomainScopedHideRules advanced grouping', () {
    ...
    test('single domain group spanning 2 chunks', () {
      ...
      expect((rules[0]['trigger'] as Map<String, dynamic>)['if-domain'], [
        '*a.com',
      ]);
      expect((rules[1]['trigger'] as Map<String, dynamic>)['if-domain'], [
        '*a.com',
      ]);
    });
  ...
  group('parseSource advanced', () {
    ...
    test('cookies source: network blocks are excluded but hiding/scriptlets '
        'are kept', () {
      final cookiesSelectors = <String>[];
      final domainHides = <DomainHide>[];
      final networkBlocks = <Map<String, dynamic>>[];
