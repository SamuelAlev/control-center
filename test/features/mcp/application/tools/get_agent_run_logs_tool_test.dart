...
class _FakeAgentRunLogRepository implements AgentRunLogRepository {
  ...
  @override
  Future<AgentRunLog?> getById(String workspaceId, String id) async => _logs
      // Scoped, not id-only: a run-log id from another workspace lives in a
      // different database file and must not resolve here.
      .where((l) => l.id == id && l.workspaceId == workspaceId)
      .firstOrNull;
...
void main() {
  group('GetAgentRunLogsTool', () {
    ...
    test('returns run logs for agent', () async {
      ...
      expect(data['count'], 2);
      expect(
        ((data['run_logs'] as List<dynamic>)[0]
      ...
      expect(
        ((data['run_logs'] as List<dynamic>)[1]
            as Map<String, dynamic>)['adapter'],
        'claude',
      );
    });
