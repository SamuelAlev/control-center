...
void main() {
  ...
  group('validation errors', () {
    ...
    test(
      'repo_full_name with only slash at start → accepted (split produces 2 parts)',
      () async {
    ...
    test(
      'repo_full_name with only slash at end → accepted (split produces 2 parts)',
      () async {
        engine.stub(_makeRun());
        final result = await _call(
          tool,
  ...
  group('edge value acceptance', () {
    ...
    test('empty string pr_node_id is passed through', () async {
      ...
      expect(result.isError, isFalse);
      expect(engine.lastTriggerPayload?['prNodeId'], '');
    });
  ...
  group('success', () {
    test(
      'engine returns run → response has pipeline_run_id and status=started',
      () async {
    ...
    test(
      'engine returns null (duplicate) → status=duplicate with message',
      () async {
        engine.stub(null);
        final result = await _call(
          tool,
        ...
        expect(body['message'], contains('already active'));
      },
    );
  });
}
