...
void main() {
  group('ListPullRequestsTool', () {
    ...
    setUp(() {
      ...
      tool = ListPullRequestsTool(prRepo: prRepo, workspaceRepo: workspaceRepo);
    });
    ...
    test('has valid inputSchema', () {
      ...
      expect(
        ((schema['properties'] as Map<String, dynamic>)['status']
            as Map<String, dynamic>)['type'],
        'string',
      );
    });
    ...
    test('returns PRs for specific workspace', () async {
      ...
      expect(data['count'], 2);
      expect(
        ((data['pull_requests'] as List<dynamic>)[0]
            as Map<String, dynamic>)['title'],
        'Feature A',
      );
      expect(
        ((data['pull_requests'] as List<dynamic>)[1]
            as Map<String, dynamic>)['title'],
        'Feature B',
      );
    });
    ...
    test('includes PR fields in response', () async {
      ...
      final data =
          jsonDecode(result.content.first.text) as Map<String, dynamic>;
      final pr =
          (data['pull_requests'] as List<dynamic>)[0] as Map<String, dynamic>;
      expect(pr['id'], 'pr-1');
