...
/// mode — the guard must not be escapable by naming a foreign space.
...
void main() {
  group('ModeToolGuard', () {
    ...
    test('null space_id with no agent bypasses guard', () async {
      final guard = ModeToolGuard(_FakeResolver(Mode.review));
      expect(
    ...
    test(
      'omitting space_id does NOT escape mode — resolves via agent run',
      () async {
    ...
    test("a foreign space id does not carry its workspace's mode", () async {
      // The resolver only knows ws-1's spaces; asking for ws-2 falls back to
      // the safe default rather than leaking ws-1's restriction (or exemption).
      final guard = ModeToolGuard(_FakeResolver(Mode.review));
      expect(
        await guard.resolveMode(workspaceId: 'ws-2', spaceId: 'ch-review'),
        Mode.chat,
      );
    });
  });
