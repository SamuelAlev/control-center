...
class FakeAgentWorkingMemoryRepository implements AgentWorkingMemoryRepository {
  ...
  Future<AgentWorkingMemory?> getByAgent(
    String workspaceId,
    String agentId,
  ) async => _store[_key(workspaceId, agentId)];
  ...
  Stream<List<AgentWorkingMemory>> watchByWorkspace(String workspaceId) =>
      Stream.value(
        _store.values.where((m) => m.workspaceId == workspaceId).toList(),
      );
}
...
void main() {
  group('RecordObservationTool', () {
    ...
    test('inputSchema has type=object and required includes all fields', () {
      ...
      expect(
        schema['required'],
        containsAll(['workspace_id', 'agent_id', 'observation']),
      );
    });
    ...
    test(
      'response JSON has status recorded, agent_id, observation_length',
      () async {
        final result = await tool.run({
          'workspace_id': 'ws-1',
          'agent_id': 'a-1',
