...
void main() {
  ...
  test(
    'provision — non-CoW, non-unsafe rift error falls through to worktree',
    () async {
      final rift = _FakeRift(
        createError: const RiftException(
          code: 'internal_error',
  ...
  test('destroy rescues uncommitted work to a rescue branch before GC', () async {
    ...
    // Rescue: add -A, then a commit (with injected identity), then a rescue/ branch.
    expect(git.ran((a) => a.contains('add') && a.contains('-A')), isTrue);
    expect(
      git.ran((a) => a.contains('commit') && a.contains('--no-verify')),
      isTrue,
    );
    expect(
      git.ran(
        (a) =>
            a.length == 2 &&
            a[0] == 'branch' &&
            a[1].startsWith('rescue/cc/conv/123-'),
      ),
      isTrue,
    );
  ...
  test('destroy does NOT rescue when the worktree is clean', () async {
  ...
  test('a failed rescue commit still proceeds to GC (best-effort)', () async {
    final wt = Directory('${tmp.path}/failcommit')..createSync();
    final rift = _FakeRift();
    final git = _FakeGit(
      responses: {
        'status --porcelain': const GitResult(
        ...
        'commit --no-verify': const GitResult(
          exitCode: 1,
          stdout: '',
          stderr: 'nothing staged / identity error',
        ),
      },
    );
    final adapter = _adapter(rift: rift, git: git);
    ...
    // No rescue branch was created (commit failed).
    expect(git.ran((a) => a.length == 2 && a[0] == 'branch'), isFalse);
  });
  ...
  test('on rift path, no git commands target sourcePath', () async {
    ...
    // (symbolic-ref / show-ref) — but we supplied baseRef so it was skipped.
  ...
  test('an undeletable stale marker still degrades to git worktree', () async {
    // Read-only checkout / permissions: the heal cannot run, so the previous
    // behaviour (the worktree backend) is the right outcome — not a failure.
    final rift = _FakeRift(
      createError: const RiftException(
        code: 'unknown_marker',
        message: 'rift marker belongs to an unknown registry entry',
    ...
    () async {
      ...
      final git = _FakeGit(
        responses: {
          'remote get-url': const GitResult(
            exitCode: 0,
            stdout: 'git@github.com:Frontify/app-server.git\n',
            stderr: '',
          ),
        },
      );
      ...
      expect(
        git.ran(
          (a) =>
              a.contains('fetch') &&
              a.contains('git@github.com:Frontify/app-server.git') &&
              a.contains('main') &&
              !a.any((s) => s.contains('refs/remotes/origin/')),
        ),
        isTrue,
  ...
  test('empty origin URL skips fetch and checks out local origin/main', () async {
    final rift = _FakeRift();
    final git = _FakeGit();
    final adapter = _adapter(rift: rift, git: git);
  ...
  test(
    ...
    () async {
      ...
      expect(result.backend, RepoIsolationBackend.rift);
      // Empty headRef follows the channel path (checkout -B), not the PR path.
      expect(
        git.ran((a) => a.contains('checkout') && a.contains('-B')),
        isTrue,
      );
