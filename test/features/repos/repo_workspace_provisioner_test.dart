...
class _FakeRegistry implements IsolatedRepoRepository {
  ...
  Future<List<IsolatedRepo>> forChannelAcrossWorkspaces(
    String channelId,
  ) async => rows.where((r) => r.channelId == channelId).toList();
...
void main() {
  group('RepoWorkspaceProvisioner', () {
    ...
    test(
      'provisions repos into conversation dir and returns the per-agent cwd',
      () async {
        ...
        try {
          ...
          // cwd is the per-agent overlay, NOT the conversation root.
          expect(
            dir,
            '${tempDir.path}/w-1/conversations/ch-12345678/agents/dev',
          );
    ...
    test(
      'releaseConversation removes the conversation dir (mcp.json + overlays)',
      () async {
        final tempDir = Directory.systemTemp.createTempSync(
        ...
        try {
          ...
          final cwd = await p.ensureConversationWorkspace(
            workspaceId: 'w-1',
            channelId: 'ch-12345678',
            agentSlug: 'dev',
            fallbackDir: '/fallback',
          ...
          final convRoot = '${tempDir.path}/w-1/conversations/ch-12345678';
          expect(Directory(convRoot).existsSync(), isTrue);

          await p.releaseConversation(
            workspaceId: 'w-1',
            channelId: 'ch-12345678',
          );

          // Worktrees destroyed AND the whole conversation dir (overlays + the
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final worktreePath =
              '${tempDir.path}/w-1/conversations/ch/repos/owner-repo';
          ...
          expect(dir, '${tempDir.path}/w-1/conversations/ch/agents/dev');
    ...
    test('one repo failing to provision does not block others', () async {
      ...
      try {
        ...
        expect(dir, '${tempDir.path}/w-1/conversations/ch-12/agents/dev');
    ...
    test(
      'ensureConversationWorkspace returns fallbackDir on top-level error',
      () async {
    ...
    test(
      'overlay cwd has a `repos` symlink to the shared conversation repos',
      () async {
        final tempDir = Directory.systemTemp.createTempSync(
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final cwdA = await p.ensureConversationWorkspace(
            workspaceId: 'w-1',
            channelId: 'ch',
            agentSlug: 'alice',
            fallbackDir: '/fallback',
          );
          final cwdB = await p.ensureConversationWorkspace(
            workspaceId: 'w-1',
            channelId: 'ch',
            agentSlug: 'bob',
            fallbackDir: '/fallback',
          ...
          expect(cwdA, '${tempDir.path}/w-1/conversations/ch/agents/alice');
          expect(cwdB, '${tempDir.path}/w-1/conversations/ch/agents/bob');

          // Shared repos: every provision targets the SAME shared repos dir
          // (keyed by workspace+channel+repo, NOT per-agent), and both overlays'
          // `repos` symlinks resolve to the same shared target.
          final sharedReposDir = '${tempDir.path}/w-1/conversations/ch/repos';
          expect(
    ...
    test('overlay build is idempotent across re-dispatches', () async {
      ...
      try {
        ...
        final first = await p.ensureConversationWorkspace(
          workspaceId: 'w-1',
          channelId: 'ch',
          agentSlug: 'dev',
          fallbackDir: '/fallback',
        ...
        final second = await p.ensureConversationWorkspace(
          workspaceId: 'w-1',
          channelId: 'ch',
          agentSlug: 'dev',
          fallbackDir: '/fallback',
    ...
    test(
      'releaseConversation destroys + deletes every worktree for the channel',
      () async {
        final registry = _FakeRegistry()
          ..rows.addAll([
            _row('a', 'w-1', 'ch'),
    ...
    test(
      'releaseConversationAnyWorkspace destroys across all workspaces',
      () async {
        final registry = _FakeRegistry()
          ..rows.addAll([
            _row('a', 'w-1', 'ch'),
