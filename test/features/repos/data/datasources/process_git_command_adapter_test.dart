...
void main() {
  ...
  group('ProcessGitCommandAdapter.run', () {
    ...
    test('captures stderr and non-zero exit code on failure', () async {
      ...
      final result = await adapter.run(['log'], workdir: tempDir.path);
    ...
    test('completes with empty output when command produces none', () async {
      ...
      final result = await adapter.run(['gc', '--auto'], workdir: tempDir.path);
    ...
    test('throws when workdir does not exist', () async {
      expect(
        adapter.run(['status'], workdir: '${tempDir.path}/nonexistent'),
  ...
  group('ProcessGitCommandAdapter onProgress', () {
    ...
    setUp(() {
      ...
      Process.runSync('git', [
        'init',
        '--bare',
      ], workingDirectory: bareRepo.path);
    ...
    test('calls onProgress for stderr lines during clone', () async {
      ...
      expect(
        progressLines.any(
          (l) =>
              l.contains('Receiving') ||
              l.contains('Enumerating') ||
              l.contains('done'),
        ),
        isTrue,
      );
  ...
  group('ProcessGitCommandAdapter.runStreaming', () {
    ...
    test('throws StateError on non-zero exit', () async {
      ...
      final stream = adapter.runStreaming(['log'], workdir: tempDir.path);
    ...
    test('throws when workdir does not exist', () async {
      ...
      expect(stream.toList(), throwsA(isA<ProcessException>()));
    });
  });
