...
void main() {
  ...
  group('CalendarEventCache', () {
    test('overlapping is null before anything is cached', () {
      ...
      expect(cache.overlapping(ws, day, day.add(const Duration(days: 1))), isNull);
    });

    test('replaceRange seeds overlapping, earliest first, out-of-range excluded', () {
      final cache = CalendarEventCache(AppPreferences(InMemoryStorage()));
      ...
      final result = cache.overlapping(ws, day, day.add(const Duration(days: 1)));
      expect(result, [earlier, later]);
      // A wider query still sees the out-of-range event.
      expect(
        cache.overlapping(ws, day, day.add(const Duration(days: 30))),
        [earlier, later, outside],
      );
    });
    ...
    test('events outside the refreshed range survive it', () {
      ...
      cache.replaceRange(ws, day, day.add(const Duration(days: 1)), [event('a')]);
    ...
    test('snapshot persists and reloads through preferences', () async {
      ...
      final result = reloaded.overlapping(ws, day, day.add(const Duration(days: 7)));
