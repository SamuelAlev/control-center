...
void main() {
  group('GoogleCredentialsRepository', () {
    ...
    test(
      'serves later reads from the in-memory cache, not the keychain',
      () async {
        await repo.save('ws-A', creds);
    ...
    test(
      're-saving after a refresh swaps token + expiry, keeps the rest',
      () async {
        await repo.save('ws-A', creds);
    ...
    test(
      'clear evicts the cache so a disconnected account stays gone',
      () async {
        await repo.save('ws-A', creds);
        await repo.load('ws-A'); // prime the cache
    ...
    test('stores one blob under a single workspace-suffixed key', () async {
      ...
      final blob =
          jsonDecode(backing.get('${googleCredentialsKey}__ws-A'))
    ...
    test(
      'load for a different workspace never sees another\'s tokens',
      () async {
        await repo.save('ws-A', creds);
