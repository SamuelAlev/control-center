...
void main() {
  group('GoogleCredentialsRepository', () {
    ...
    test('stores one blob under a single workspace-suffixed key', () async {
      ...
      expect(storage.store.containsKey('${googleCredentialsKey}__ws-A'), isTrue);
      // No un-suffixed (global) key is ever written.
      expect(storage.store.containsKey(googleCredentialsKey), isFalse);
      // The single item is a JSON blob carrying every field.
      final blob = jsonDecode(storage.store['${googleCredentialsKey}__ws-A']!)
