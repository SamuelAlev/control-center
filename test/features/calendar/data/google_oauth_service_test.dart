...
void main() {
  ...
  group('GoogleOAuthService.authenticate', () {
    test('exchanges the redirected code (no client_secret) for tokens', () async {
    ...
    test('rejects a redirect whose state does not match', () async {
      final svc = GoogleOAuthService(
        ...
        tokenDio: Dio(),
        launcher: (_) async => true,
        awaitRedirect: (_) async => Uri.parse(
          'com.googleusercontent.apps.cid:/oauth2redirect?code=c&state=WRONG',
        ),
      );
      await expectLater(
        svc.authenticate(),
  ...
  group('GoogleOAuthService.refresh', () {
    test(
      'posts grant_type=refresh_token and returns the new access token',
      () async {
        final adapter = _CapturingAdapter({
    ...
    test('throws when client id is missing', () async {
      ...
      await expectLater(svc.refresh('x'), throwsA(isA<Exception>()));
    });

    test(
      'classifies a 400 invalid_grant as the terminal invalidGrant kind',
      () async {
        final dio = Dio()
    ...
    test(
      'classifies other token-endpoint errors as transient, body in message',
      () async {
        final dio = Dio()
