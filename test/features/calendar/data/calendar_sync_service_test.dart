...
class _FakeAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    final body = options.uri.path.contains('calendarList') ? calendars : events;
...
void main() {
  group('CalendarSyncService', () {
    ...
    test('syncs events and records lastSyncedAt', () async {
    ...
    test(
      'backfills attendee display names from the organizer/creator',
      () async {
        repo.account = const CalendarAccount(
          id: 'acc-1',
          workspaceId: 'ws-A',
    ...
    test(
      'reconciles deletions: each calendar is swept with its fetched ids',
      () async {
        repo.account = const CalendarAccount(
          id: 'acc-1',
          workspaceId: 'ws-A',
    ...
    test('syncs every calendar, tags events, skips freeBusyReader', () async {
      ...
      // skipped.
      expect(repo.upsertedEvents, hasLength(2));
      expect(repo.upsertedEvents.map((e) => e.calendarId).toSet(), {
        'primary',
        'team@group',
      });
      ...
      expect(persisted.sources.map((s) => s.id).toSet(), {
        'primary',
        'team@group',
        'busy@group',
      });
    ...
    test('syncs every connected account, tagging events per account', () async {
      ...
      expect(repo.upsertedEvents, hasLength(2));
      expect(repo.upsertedEvents.map((e) => e.accountId).toSet(), {
        'acc-1',
        'acc-2',
      });
      expect(repo.lastSynced.map((s) => s.accountId).toSet(), {
        'acc-1',
        'acc-2',
      });
    });
    ...
    test(
      'ensureRangeLoaded fetches an out-of-window range once, then caches',
      () async {
        repo.account = const CalendarAccount(
          id: 'acc-1',
          workspaceId: 'ws-A',
