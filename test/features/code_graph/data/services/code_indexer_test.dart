...
class StubCodeGraphRepository implements CodeGraphRepository {
  ...
  Future<Map<String, String>> fileHashesFor(
    ...
    for (final path in filePaths)
      if (_fileHashes.containsKey(path)) path: _fileHashes[path]!,
  };
  ...
  Future<Map<String, ({String contentHash, DateTime indexedAt})>> fileStatesFor(
    ...
    for (final path in filePaths)
      if (_fileHashes.containsKey(path))
        path: (contentHash: _fileHashes[path]!, indexedAt: DateTime(2000)),
  };
...
class StubSourceFileWalker implements SourceFileWalker {
  ...
  /// The stubbed targeted pass: the walk's files narrowed to [relativePaths].
  /// The real one additionally applies git's ignore rules, which is exactly
  /// what `code_indexer_targeted_test.dart` drives against a real work tree.
  @override
  Future<List<HashedSourceFile>> hashPaths(
    String rootPath,
    List<String> relativePaths,
  ) async {
    final wanted = relativePaths.toSet();
    return [
      for (final file in await walkAndHash(rootPath))
        if (wanted.contains(file.relativePath)) file,
    ];
  }
}
...
void main() {
  ...
  // ---- Progress callback ----
  test('announces the work up front, then once per ingest batch', () async {
    final repoDir = _createTempRepo({
      'lib/a.dart': 'void main() {}',
      'lib/b.dart': 'void b() {}',
    });
