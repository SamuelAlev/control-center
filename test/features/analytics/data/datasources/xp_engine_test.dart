...
class _FakeAnalyticsRepository implements AnalyticsRepository {
  ...
  @override
  Future<AgentScorecard?> getAgentScorecard(String agentId) async => _scorecard;

  @override
  Stream<List<AgentDailyStats>> watchByAgent(String agentId) => const Stream.empty();
...
class _ThrowingAnalyticsRepository implements AnalyticsRepository {
  @override
  Future<AgentScorecard?> getAgentScorecard(String agentId) async => throw Exception('boom');
...
class _FakeAchievementRepository implements AchievementRepository {
  ...
  @override
  Stream<List<Achievement>> watchByAgent(String agentId) => const Stream.empty();

  @override
  Future<List<Achievement>> getByAgent(String agentId) async => [];

  @override
  Future<bool> isUnlocked(String agentId, String badgeKey) async => unlockedKeys.contains(badgeKey);
}

/// In-memory fake for [StreakRepository].
class _FakeStreakRepository implements StreakRepository {
  final List<(String, String, bool)> updates = [];
  ...
  Future<void> updateStreak(
    ...
    updates.add((agentId, streakType, increment));
  }
...
void main() {
  ...
  group('XpEngine', () {
    ...
    test('updates streaks on PrMerged', timeout: const Timeout.factor(2), () async {
      ...
      expect(streakRepo.updates[0], ('agent-1', 'pr_merged', true));
      expect(streakRepo.updates[1], ('agent-1', 'daily_active', true));
    });
