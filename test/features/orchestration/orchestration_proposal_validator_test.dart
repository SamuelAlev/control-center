...
void main() {
  ...
  test('rejects an unknown role reference in a sub-ticket', () {
    final p = _valid().copyWith(
      subTickets: [
        const ProposedSubTicket(key: 'x', title: 'X', roleKey: 'ghost'),
      ],
    );
    expect(
  ...
  test('rejects a dependency cycle', () {
    ...
    expect(validator.validate(p).any((v) => v.contains('cycle')), isTrue);
  });
  ...
  test('rejects a role with both existing agent and hire spec', () {
    ...
    expect(validator.validate(p).any((v) => v.contains('exactly one')), isTrue);
  });
  ...
  test('enforces the sub-ticket count cap', () {
    final p = _valid().copyWith(
      subTickets: [
        for (
          var i = 0;
          i < OrchestrationProposalValidator.maxSubTickets + 1;
          i++
        )
