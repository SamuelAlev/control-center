...
class _TestableService extends WorkspaceFilesystemService {
  _TestableService(this.baseDir) : super(CcPaths(baseDir.path));

  final Directory baseDir;

  @override
  Future<String> workspaceDir(String workspaceId) async {
    return p.join(baseDir.path, workspaceId);
  }
}
...
void main() {
  ...
  group('workspaceDir', () {
    test('returns directory for workspace id', () async {
      expect(await service.workspaceDir('ws-1'), p.join(tempDir.path, 'ws-1'));
    });
  });
  ...
  group('skillsDir', () {
    test('returns skills subdirectory', () async {
      expect(
        await service.skillsDir('ws-1'),
        p.join(tempDir.path, 'ws-1', 'skills'),
      );
    });
  });
  ...
  group('skillDir', () {
    test('returns skill subdirectory', () async {
      expect(
        await service.skillDir('ws-1', 'dart-dev'),
        p.join(tempDir.path, 'ws-1', 'skills', 'dart-dev'),
      );
    });
  });
  ...
  group('skillFilePath', () {
    test('returns SKILL.md path in skill directory', () async {
      ...
      expect(
        path,
        p.join(tempDir.path, 'ws-1', 'skills', 'dart-dev', 'SKILL.md'),
      );
    });
  });
  ...
  group('agentsDir', () {
    test('returns agents subdirectory', () async {
      expect(
        await service.agentsDir('ws-1'),
        p.join(tempDir.path, 'ws-1', 'agents'),
      );
    });
  });
  ...
  group('agentDir', () {
    test('returns agent subdirectory', () async {
      expect(
        await service.agentDir('ws-1', 'ceo'),
        p.join(tempDir.path, 'ws-1', 'agents', 'ceo'),
      );
    });
  });
  ...
  group('agentSkillsLinkDir', () {
    test('returns .agents/skills link directory', () async {
      expect(
        await service.agentSkillsLinkDir('ws-1', 'ceo'),
  ...
  group('ensureAgentDir', () {
    test('creates agent directory', () async {
      ...
      final dir = Directory(
        p.join(tempDir.path, 'ws-1', 'agents', 'developer'),
      );
      expect(dir.existsSync(), isTrue);
    });

    test(
      'does not create an .mcp.json symlink (MCP config is cwd-derived)',
      () async {
        await service.ensureAgentDir('ws-1', 'developer');
        ...
        // No root mcp.json default is materialized either.
        expect(File(p.join(tempDir.path, 'mcp.json')).existsSync(), isFalse);
      },
    );
  ...
  group('deleteAgentDir', () {
    test('deletes agent directory recursively', () async {
      ...
      final dir = Directory(
        p.join(tempDir.path, 'ws-1', 'agents', 'temp-agent'),
      );
      expect(dir.existsSync(), isTrue);
  ...
  group('readSkillFile', () {
    test('returns content when skill file exists', () async {
      await service.writeSkillFile('ws-1', 'dart', '# Dart');

      final content = await service.readSkillFile('ws-1', 'dart');

      expect(content, '# Dart');
    });

    test('returns null when skill file does not exist', () async {
      final content = await service.readSkillFile('ws-1', 'nonexistent');
  ...
  group('deleteSkillDir', () {
    test('deletes skill directory recursively', () async {
      ...
      final dir = Directory(
        p.join(tempDir.path, 'ws-1', 'skills', 'temp-skill'),
      );
      expect(dir.existsSync(), isTrue);
  ...
  group('listSkillSlugs', () {
    ...
    test('excludes directories without SKILL.md', () async {
      ...
      final emptyDir = Directory(
        p.join(tempDir.path, 'ws-1', 'skills', 'empty'),
      );
  ...
  group('spacesDir', () {
    test('returns the spaces subdirectory', () async {
      expect(
        await service.spacesDir('ws-1'),
        p.join(tempDir.path, 'ws-1', 'spaces'),
      );
    });
  });

  group('spaceDir', () {
    test('returns the space subdirectory for an id', () async {
      expect(
        await service.spaceDir('ws-1', 'conv-42'),
        p.join(tempDir.path, 'ws-1', 'spaces', 'conv-42'),
      );
    });
  });

  group('ensureSpaceDir', () {
    test('creates the space directory on first call', () async {
      final dir = await service.ensureSpaceDir('ws-1', 'conv-1');
      expect(Directory(dir).existsSync(), isTrue);
      expect(dir, p.join(tempDir.path, 'ws-1', 'spaces', 'conv-1'));
    });

    test('does not throw on repeated calls', () async {
      await service.ensureSpaceDir('ws-1', 'conv-1');
      await service.ensureSpaceDir('ws-1', 'conv-1');
    });
  });
  ...
  group('prCloneDir', () {
    test('returns pr_clones subdirectory for owner/repo', () async {
      expect(
        await service.prCloneDir('ws-1', 'acme', 'project'),
    ...
    test('sanitizes special characters in owner', () async {
      expect(
        await service.prCloneDir('ws-1', 'evil/../owner', 'repo'),
    ...
    test('sanitizes special characters in repo', () async {
      expect(
        await service.prCloneDir('ws-1', 'owner', 'repo;rm -rf'),
    ...
    test('preserves hyphens and dots', () async {
      expect(
        await service.prCloneDir('ws-1', 'my-org', 'my.repo'),
  ...
  group('persistLogo', () {
    ...
    test('creates workspace directory if it does not already exist', () async {
  ...
  group('persistLogoBytes', () {
    test('writes bytes to workspace as logo with extension', () async {
      final wsDir = Directory(p.join(tempDir.path, 'ws-1'));
      await wsDir.create(recursive: true);

      final result = await service.persistLogoBytes('ws-1', [4, 5, 6], '.png');

      expect(result, p.join(wsDir.path, 'logo.png'));
      final dest = File(result!);
      expect(dest.existsSync(), isTrue);
      expect(dest.readAsBytesSync(), [4, 5, 6]);
    });

    test('lowercases extension', () async {
      final wsDir = Directory(p.join(tempDir.path, 'ws-1'));
      await wsDir.create(recursive: true);

      final result = await service.persistLogoBytes('ws-1', [1], '.PNG');

      expect(result, p.join(wsDir.path, 'logo.png'));
    });

    test('uses bare "logo" when extension is empty', () async {
      final wsDir = Directory(p.join(tempDir.path, 'ws-1'));
      await wsDir.create(recursive: true);

      final result = await service.persistLogoBytes('ws-1', [1], '');

      expect(result, p.join(wsDir.path, 'logo'));
    });

    test('creates workspace directory if it does not already exist', () async {
      await service.persistLogoBytes('ws-fresh', [1, 2], '.png');

      expect(Directory(p.join(tempDir.path, 'ws-fresh')).existsSync(), isTrue);
    });

    test('returns null for empty bytes', () async {
      final result = await service.persistLogoBytes('ws-1', [], '.png');
      expect(result, isNull);
    });
  });
  ...
  group('ensureDir', () {
    ...
    test('creates nested directories recursively', () async {
      ...
      expect(Directory(dirPath).existsSync(), isTrue);
      expect(Directory(p.join(tempDir.path, 'level1')).existsSync(), isTrue);
    });
  });
  ...
  group('syncAgentSkillLinks - extended', () {
    ...
    test(
      'creates symlinks that point to the correct skill directories',
      () async {
        ...
        expect(target, p.join(tempDir.path, 'ws-1', 'skills', 'python'));
      },
    );
