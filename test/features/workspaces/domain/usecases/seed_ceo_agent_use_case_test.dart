...
class _FakeFilesystem implements WorkspaceFilesystemPort {
  ...
  @override
  Future<String> workspaceDir(String workspaceId) async => '/tmp/$workspaceId';

  @override
  Future<String> conversationsDir(String workspaceId) async =>
      '/tmp/$workspaceId/conversations';

  @override
  Future<String> conversationDir(
    String workspaceId,
    String conversationId,
  ) async => '/tmp/$workspaceId/conversations/$conversationId';
  ...
  Future<String> agentSkillsLinkDir(
    String workspaceId,
    String agentSlug,
  ) async => '/tmp/$workspaceId/agents/$agentSlug/skills';
  ...
  Future<String> prCloneDir(
    ...
    String owner,
    String repo,
  ) async => '/tmp/$workspaceId/pr_clones/${owner}__$repo';
...
void main() {
  group('SeedCeoAgentUseCase', () {
    ...
    SeedCeoAgentUseCase useCase() =>
        SeedCeoAgentUseCase(agentRepository: agentRepo, filesystemService: fs);
    ...
    test(
      'when ceo exists without skills, seeds skills and updates agent',
      () async {
    ...
    test('processes multiple workspaces', () async {
      agentRepo = _FakeAgentRepo(agents: [_ceoAgent('ws1'), _ceoAgent('ws2')]);
    ...
    test('ceo with existing full skill set is not updated', () async {
      final fullCeo = _ceoAgent(
        'ws1',
      ).copyWith(skills: AgentSkills(ceoSkillSlugs.toList()));
    ...
    test('ceo with partial skills gets new ones added', () async {
      final partialCeo = _ceoAgent(
        'ws1',
      ).copyWith(skills: AgentSkills([ceoSkillSlugs.first]));
