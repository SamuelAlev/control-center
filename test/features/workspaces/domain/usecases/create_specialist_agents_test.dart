...
class _FakeAgentRepository implements AgentRepository {
  ...
  Stream<List<Agent>> watchByWorkspace(String workspaceId) => Stream.value(
    _agents.values.where((a) => a.workspaceId == workspaceId).toList(),
  );
...
void main() {
  ...
  group('CreateSpecialistAgentsUseCase', () {
    test(
      'seeds all specialist agents into a new workspace',
      timeout: const Timeout.factor(2),
      () async {
        final useCase = createUseCase();
        final agents = await useCase.execute('ws-1', ceoAgentId: 'ceo-1');
    ...
    test(
      'creates agents with correct workspace and reportsTo',
      timeout: const Timeout.factor(2),
      () async {
        final useCase = createUseCase();
        final agents = await useCase.execute('ws-1', ceoAgentId: 'ceo-uuid');
    ...
    test(
      'passes adapterId and modelId to created agents',
      timeout: const Timeout.factor(2),
      () async {
        final useCase = createUseCase();
    ...
    test(
      'does not duplicate existing agents (idempotent)',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'writes agent files for all specialists',
      timeout: const Timeout.factor(2),
      () async {
        final useCase = createUseCase();
        await useCase.execute('ws-1', ceoAgentId: 'ceo-1');
    ...
    test(
      'does not overwrite existing skill files',
      timeout: const Timeout.factor(2),
      () async {
