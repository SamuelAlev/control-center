...
void main() {
  ...
  group('CreateCeoAgentUseCase', () {
    ...
    test('ensures workspace directories are created', () async {
      ...
      expect(filesystem.createdDirs, contains('/fake/ws-1/.agents'));
      expect(filesystem.createdDirs, contains('/fake/ws-1/skills'));
    });

    test('writes agent markdown file', () async {
      await useCase.execute('ws-1');

      final agentFile = filesystem.files['/fake/ws-1/agents/ceo/AGENTS.md'];
    ...
    test('writes skill files', () async {
      ...
      final strategySkill =
          filesystem.files['/fake/ws-1/skills/strategy/SKILL.md'];
      expect(strategySkill, isNotNull);
      expect(strategySkill, contains('# Strategy'));

      final coordinationSkill =
          filesystem.files['/fake/ws-1/skills/coordination/SKILL.md'];
      expect(coordinationSkill, isNotNull);
      expect(coordinationSkill, contains('# Coordination'));
    });

    test('writes all 6 skill files', () async {
      await useCase.execute('ws-1');

      final skillFiles = filesystem.files.keys.where(
        (k) => k.contains('/skills/'),
      );
      expect(skillFiles.length, 6);
    });
    ...
    test('different workspaces create separate CEOs', () async {
      ...
      final ws1File = filesystem.files['/fake/ws-1/agents/ceo/AGENTS.md'];
      final ws2File = filesystem.files['/fake/ws-2/agents/ceo/AGENTS.md'];
