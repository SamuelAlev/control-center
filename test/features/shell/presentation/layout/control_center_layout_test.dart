...
void main() {
  ...
  testWidgets('toggling the rail never moves the sidebar icons', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1400, 900);
    tester.view.devicePixelRatio = 1.0;
    ...
    // The nav icons (and the workspace mark, which sits on the same x line)
    // must hold their horizontal position in every phase of the toggle:
    // settled expanded, mid-collapse, settled rail, mid-expand.
    const icons = [
    ...
    void expectUnmoved(Map<IconData, double> baseline, String phase) {
    ...
    final expandedX = {for (final icon in icons) icon: iconX(icon)};
    ...
    expectUnmoved(expandedX, 'collapsing');
    await tester.pumpAndSettle();
    expectUnmoved(expandedX, 'collapsed');

    // Expand: same assertions on the way back.
    await tester.tap(find.byIcon(AppIcons.panelLeftOpen));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 120));
    expectUnmoved(expandedX, 'expanding');
    await tester.pumpAndSettle();
    expectUnmoved(expandedX, 're-expanded');
