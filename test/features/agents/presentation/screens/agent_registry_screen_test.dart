...
Future<void> _pumpAgentsSettings(WidgetTester tester, List overrides) async {
    ...
    ProviderScope(
      ...
      child: CcTheme(
        ...
        child: MaterialApp(
          ...
          home: const Scaffold(body: AgentRegistryScreen()),
        ),
      ),
    ),
  );
...
void main() {
      ...
      ProviderScope(
        ...
        child: CcTheme(
          ...
          child: const MaterialApp(
            localizationsDelegates: AppLocalizations.localizationsDelegates,
            supportedLocales: AppLocalizations.supportedLocales,
            home: Scaffold(body: AgentRegistryScreen()),
          ),
        ),
      ),
    );
      ...
      ProviderScope(
        overrides: [
          agentsProvider.overrideWith(
            (ref) => Stream<List<Agent>>.error(Exception('Connection refused')),
          ),
          activeWorkspaceIdProvider.overrideWith(
  ...
  testWidgets('renders empty state with no agents', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          agentsProvider.overrideWith((ref) => Stream.value(const <Agent>[])),
          activeWorkspaceIdProvider.overrideWith(
            () => _TestActiveWorkspaceNotifier(null),
          ),
          appPreferencesProvider.overrideWithValue(prefs),
      ...
      ProviderScope(
        overrides: [
          ...
          agentRunLogsProvider((
            workspaceId: 'ws-1',
            agentId: 'a1',
          )).overrideWith((ref) => Stream.value(const <AgentRunLog>[])),
        ],
        child: CcTheme(
          data: CcThemeData.light(),
          child: const MaterialApp(
      ...
      ProviderScope(
        overrides: [
          ...
          agentRunLogsProvider((
            workspaceId: 'ws-1',
            agentId: 'a1',
          )).overrideWith((ref) => Stream.value(logs)),
        ],
        child: CcTheme(
          data: CcThemeData.light(),
          child: const MaterialApp(
  ...
  testWidgets('AgentRegistryScreen header shows agent info', (tester) async {
    tester.view.physicalSize = const Size(1000, 700);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(() {
      tester.view.resetPhysicalSize();
  ...
  testWidgets('AgentRegistryScreen widget accepts key', (tester) async {
    const widget = AgentRegistryScreen(key: ValueKey('agents'));
  ...
  group('AgentRegistryScreen filter section', () {
  ...
  group('AgentRegistryScreen log edge cases', () {
    ...
    testWidgets('empty agents renders empty content', (tester) async {
      ...
      expect(find.text('No agents'), findsOneWidget);
      expect(find.text('New agent'), findsAtLeast(1));
    });
  });

  group('AgentRegistryScreen CRUD', () {
    ...
    testWidgets('deletes an agent with confirmation dialog', (tester) async {
      ...
      // Delete now sits in the danger zone at the end of the form.
  ...
  group('AgentRegistryScreen selection', () {
  ...
  group('AgentRegistryScreen edge cases', () {
  ...
  group('AgentRegistryScreen CRUD - validation', () {
    testWidgets('create agent with empty name shows error snackbar', (
      ...
      // Save is pinned in the pane's footer, so there is nothing to scroll
      // to — but the bar grows as it appears, so let it settle first.
      await tester.pumpAndSettle();
    ...
    testWidgets('create agent with special characters in name', (tester) async {
      final repo = FakeAgentRepository();
      final agent = _testAgent('a1', 'architect', 'Software Architect');
      await repo.upsert(agent);
  ...
  group('AgentRegistryScreen CRUD - rename', () {
  ...
  group('AgentRegistryScreen CRUD - title update', () {
  ...
  group('AgentRegistryScreen CRUD - duplicate handling', () {
  ...
  group('AgentRegistryScreen CRUD - cancel create', () {
    testWidgets('tap Add agent shows form and creates agent', (tester) async {
      final repo = FakeAgentRepository();
      ...
      // Tap Add agent
      await tester.tap(find.text('New agent').first);
      await tester.pumpAndSettle(const Duration(seconds: 2));
  ...
  group('AgentRegistryScreen log edge cases - multiple logs', () {
  ...
  group('AgentRegistryScreen log edge cases - pending status', () {
  ...
  group('AgentRegistryScreen log edge cases - zero duration', () {
    testWidgets(
      'log where startedAt equals completedAt renders without crash',
      (tester) async {
        final agents = [_testAgent('a1', 'architect', 'Software Architect')];
  ...
  group('AgentRegistryScreen agent tile click', () {
    testWidgets(
      'tapping agent tile shows details with settings and logs tabs',
      (tester) async {
        final agents = [
          _testAgent('a1', 'architect', 'Software Architect'),
          _testAgent('a2', 'reviewer', 'Code Reviewer'),
  ...
  group('AgentRegistryScreen multiple workspaces', () {
