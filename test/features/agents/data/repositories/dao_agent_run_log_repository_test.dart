...
import 'package:control_center/core/database/tables/agent_run_logs.dart' show AgentRunLogsTable;
...
void main() {
  ...
  group('watchByAgent', () {
    test('emits empty list when agent has no logs', () async {
      final logs = await repo.watchByAgent('agent-1').first;
      expect(logs, isEmpty);
    });
    ...
    test('emits updated list after update', () async {
      ...
      // update.
      final afterUpdate = await repo.watchByAgent('agent-1').first;
