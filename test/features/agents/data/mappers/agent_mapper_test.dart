...
void main() {
  group('AgentMapper', () {
    ...
    test(
      'splits skills from comma-separated string',
    ...
    test(
      'filters empty skills from trailing comma',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'effort passes the raw level id through (model-driven)',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'toDomainList returns non-growable list',
      timeout: const Timeout.factor(2),
      () {
        final rows = [_makeRow()];
        final agents = mapper.toDomainList(rows);
        expect(
          () => agents.add(mapper.toDomain(_makeRow(id: 'x'))),
