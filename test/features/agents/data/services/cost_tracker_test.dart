...
void main() {
  group('CostTracker', () {
    ...
    setUp(() {
      ...
      tracker = CostTracker(runLogRepo: runLogRepo, eventBus: eventBus);
    });
    ...
    AgentRunLog makeLog({
      ...
      id: id,
      agentId: agentId,
      // The run's workspace selects the database it is read back from, so an
      // unscoped seed would never resolve.
      workspaceId: 'ws1',
    ...
    group('recordUsage', () {
      test(
        'computes cost and updates run log',
        ...
        () async {
          ...
          final updated = await runLogRepo.getById('ws1', 'log-1');
      ...
      test(
        'computes cost with custom token prices',
        timeout: const Timeout.factor(2),
        () async {
          final log = makeLog();
          runLogRepo.seed(log);
      ...
      test(
        'is no-op when run log not found',
        timeout: const Timeout.factor(2),
        () async {
          await tracker.recordUsage(
            workspaceId: 'ws1',
      ...
      test(
        'preserves other fields on the run log',
        timeout: const Timeout.factor(2),
        () async {
          final log = makeLog().copyWith(summary: 'original summary');
          runLogRepo.seed(log);
    ...
    group('getMonthlySpendForAgent', () {
      test(
        'sums cost for agent runs this month',
        timeout: const Timeout.factor(2),
        () async {
          runLogRepo.seed(
            makeLog(id: 'l1', costCents: 100, agentId: 'agent-1'),
          );
          runLogRepo.seed(
            makeLog(id: 'l2', costCents: 200, agentId: 'agent-1'),
          );
          runLogRepo.seed(
            makeLog(id: 'l3', costCents: 300, agentId: 'agent-2'),
          );
      ...
      test(
        'excludes runs from previous months',
        timeout: const Timeout.factor(2),
        () async {
          final now = DateTime.now();
          final lastMonth = DateTime(now.year, now.month - 1, 15);
          runLogRepo.seed(
            makeLog(id: 'l-old', costCents: 500, startedAt: lastMonth),
          );
      ...
      test(
        'returns 0 for agent with no runs',
        timeout: const Timeout.factor(2),
        () async {
          final spend = await tracker.getMonthlySpendForAgent('agent-1');
          expect(spend, 0);
    ...
    group('getMonthlySpendTotal', () {
      test(
        'sums cost across all agents this month',
        timeout: const Timeout.factor(2),
        () async {
    ...
    group('checkBudget', () {
      test(
        'publishes hard stop event at 100%',
        timeout: const Timeout.factor(2),
        () async {
      ...
      test(
        'publishes hard stop event above 100%',
        timeout: const Timeout.factor(2),
        () async {
          final events = <BudgetThresholdCrossed>[];
          eventBus.on<BudgetThresholdCrossed>().listen(events.add);
      ...
      test(
        'does nothing when budgetCents is 0',
        timeout: const Timeout.factor(2),
        () async {
          final events = <BudgetThresholdCrossed>[];
          eventBus.on<BudgetThresholdCrossed>().listen(events.add);
      ...
      test(
        'does nothing when budgetCents is negative',
        timeout: const Timeout.factor(2),
        () async {
          final events = <BudgetThresholdCrossed>[];
          eventBus.on<BudgetThresholdCrossed>().listen(events.add);
      ...
      test(
        'does not publish when eventBus is null',
        timeout: const Timeout.factor(2),
        () {
      ...
      test(
        'only publishes hard stop (not warning) at exactly 100%',
        timeout: const Timeout.factor(2),
        () async {
          final events = <BudgetThresholdCrossed>[];
          eventBus.on<BudgetThresholdCrossed>().listen(events.add);
