...
void main() {
  group('runDiagnostics', () {
    group('sandbox backend check', () {
      test('returns ok when native backend is available', () async {
        final fake = FakeSandboxDetectorPort(
          _detectionResultWith(recommendation: SandboxBackend.native),
        );
      ...
      test('returns error when detection throws', () async {
        final fake = ThrowingSandboxDetectorPort(
          Exception('Sandbox probe failed'),
        );
        final service = DoctorService(sandboxDetector: fake);
    ...
    group('error handling', () {
      test(
        'detection failure does not prevent other checks from running',
        () async {
          ...
          expect(
            report.results.where((r) => r.name == 'Sandbox backend'),
            hasLength(1),
          );
  ...
  group('DoctorReport', () {
    test('allOk returns true when all results are ok', () {
      final results = [
        const DiagnosticResult(name: 'check a', status: DiagnosticStatus.ok),
        const DiagnosticResult(name: 'check b', status: DiagnosticStatus.ok),
        const DiagnosticResult(name: 'check c', status: DiagnosticStatus.ok),
      ];
    ...
    test('hasErrors and hasWarnings with mixed results', () {
      final results = [
        ...
        const DiagnosticResult(name: 'check c', status: DiagnosticStatus.error),
      ];
      final report = DoctorReport(results: results);
