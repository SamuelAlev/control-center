...
void main() {
  group('DiagnosticResult', () {
    ...
    test(
      'isWarning is true for warning status',
    ...
    test(
      'isError is true for error status',
      timeout: const Timeout.factor(2),
      () {
        const result = DiagnosticResult(
          name: 'check',
    ...
    test(
      'canAutoRepair defaults to false',
      timeout: const Timeout.factor(2),
      () {
        const result = DiagnosticResult(
          name: 'check',
    ...
    test('inequality when fields differ', timeout: const Timeout.factor(2), () {
      const a = DiagnosticResult(name: 'check', status: DiagnosticStatus.ok);
      const b = DiagnosticResult(name: 'check', status: DiagnosticStatus.error);
  ...
  group('DoctorReport', () {
    test(
      'allOk is true when every result is ok',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'allOk is false when any result is not ok',
      timeout: const Timeout.factor(2),
      () {
        const report = DoctorReport(
          results: [
    ...
    test(
      'hasErrors counts error results',
      timeout: const Timeout.factor(2),
      () {
        const report = DoctorReport(
          results: [
    ...
    test(
      'hasWarnings counts warning results',
      timeout: const Timeout.factor(2),
      () {
        const report = DoctorReport(
          results: [
    ...
    test(
      'empty report is allOk and has no errors/warnings',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'equality uses deep collection equality',
      timeout: const Timeout.factor(2),
      () {
        const a = DoctorReport(
          results: [DiagnosticResult(name: 'x', status: DiagnosticStatus.ok)],
        );
        const b = DoctorReport(
          results: [DiagnosticResult(name: 'x', status: DiagnosticStatus.ok)],
        );
        expect(a, equals(b));
        expect(a.hashCode, equals(b.hashCode));
    ...
    test(
      'inequality when results differ',
      timeout: const Timeout.factor(2),
      () {
        const a = DoctorReport(
          results: [DiagnosticResult(name: 'x', status: DiagnosticStatus.ok)],
