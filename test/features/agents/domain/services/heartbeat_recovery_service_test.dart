...
AgentRunLog _makeRun({
  ...
  String? ticketId,
  // Nullable so the "no workspace" path stays testable, but scoped by default:
  // the repository refuses to persist a run log without a workspace.
...
void main() {
  group('HeartbeatRecoveryService', () {
    ...
    group('evaluateAndSchedule', () {
      test(
        'schedules retry for transient upstream with attempts remaining',
        ...
        () async {
          ...
          final updated = await runLogRepo.getById('ws-1', run.id);
      ...
      test(
        'does not retry transient upstream when attempts exhausted',
        timeout: const Timeout.factor(2),
        () async {
          final run = _makeRun(
            errorFamily: RunErrorFamily.transientUpstream,
      ...
      test(
        'schedules continuation for stalled run with attempts remaining',
        timeout: const Timeout.factor(2),
        () async {
          final run = _makeRun(liveness: RunLiveness.stalled, retryAttempt: 1);
          runLogRepo.seed(run);
      ...
      test(
        'fails ticket for stalled run exceeding max continuations',
        timeout: const Timeout.factor(2),
        () async {
          final run = _makeRun(
          ...
          expect(
            ticketWorkflow.failedTickets.first.reason,
      ...
      test(
        'skips failBackingTicket when ticketId is null',
        timeout: const Timeout.factor(2),
        () async {
          final run = _makeRun(
            liveness: RunLiveness.dead,
      ...
      test(
        'skips failBackingTicket when workspaceId is null',
        timeout: const Timeout.factor(2),
        () async {
          final run = _makeRun(
            liveness: RunLiveness.dead,
    ...
    group('shouldContinue', () {
      ...
      test(
        'returns false when attempt >= max',
        timeout: const Timeout.factor(2),
        () {
          final run = _makeRun(
            retryAttempt: HeartbeatRecoveryService.maxContinuationAttempts,
    ...
    test(
      'transient upstream takes priority over stalled',
      timeout: const Timeout.factor(2),
      () async {
        final run = _makeRun(
          errorFamily: RunErrorFamily.transientUpstream,
