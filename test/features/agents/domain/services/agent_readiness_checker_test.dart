...
void main() {
  group('AgentReadinessChecker', () {
    ...
    test(
      'returns ready when workspace matches and adapter is set',
    ...
    test(
      'returns ready when workspaceId is null (no workspace filter)',
      timeout: const Timeout.factor(2),
      () async {
        final agent = _makeAgent();
    ...
    test(
      'returns wrongWorkspace when workspace does not match',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'returns noAdapter when adapterId is null',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'returns noAdapter when adapterId is empty string',
      timeout: const Timeout.factor(2),
      () async {
    ...
    test(
      'checkFromId returns noAdapter when agent not found',
      timeout: const Timeout.factor(2),
      () async {
        final result = await checker.checkFromId(
          'missing',
          workspaceId: 'ws-1',
        );
    ...
    test(
      'checkFromId delegates to check when agent exists',
      timeout: const Timeout.factor(2),
      () async {
        final agent = _makeAgent();
    ...
    test(
      'checkFromId cannot reach an agent in another workspace',
      timeout: const Timeout.factor(2),
      () async {
        final agent = _makeAgent(workspaceId: 'ws-1');
        await agentRepo.upsert(agent);
        ...
        // The workspace selects the database the agent is read from, so a
        // foreign id resolves to nothing rather than to a mismatch verdict.
        expect(result.readiness, AgentReadiness.noAdapter);
        expect(result.reason, 'Agent not found');
      },
    );
  ...
  group('AgentReadinessResult', () {
    test(
      'isReady is true only for ready state',
      timeout: const Timeout.factor(2),
      () {
        expect(
          const AgentReadinessResult(AgentReadiness.ready).isReady,
          isTrue,
        );
        expect(
          const AgentReadinessResult(AgentReadiness.noAdapter).isReady,
          isFalse,
        );
        expect(
