...
void main() {
  group('deriveAgentLiveState', () {
    test(
      'returns neverRun for empty logs',
    ...
    test(
      'returns running when any log is running',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'running takes priority over other states',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns failed when latest log has error status',
      timeout: const Timeout.factor(2),
      () {
        final logs = [_makeLog(status: RunStatus.error)];
        expect(deriveAgentLiveState(logs), AgentLiveState.failed);
      },
    );

    test(
      'returns failed when latest log has failed liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns failed when latest log has dead liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns blocked when latest log has blocked liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns blocked when latest log has stalled liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns blocked when latest log has looping liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
    ...
    test(
      'returns idle for completed run with no issues',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
          _makeLog(
    ...
    test(
      'returns idle for completed run with alive liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
          _makeLog(status: RunStatus.completed, liveness: RunLiveness.alive),
        ];
        expect(deriveAgentLiveState(logs), AgentLiveState.idle);
      },
    );
    ...
    test(
      'returns idle for completed run with productive liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
          _makeLog(
    ...
    test(
      'returns idle for completed run with empty liveness',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
          _makeLog(status: RunStatus.completed, liveness: RunLiveness.empty),
        ];
        expect(deriveAgentLiveState(logs), AgentLiveState.idle);
      },
    );
    ...
    test(
      'classifies based on latest (first) log',
      timeout: const Timeout.factor(2),
      () {
        final logs = [
  ...
  group('agentLastActive', () {
    ...
    test(
      'returns lastOutputAt when present',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'falls back to completedAt when lastOutputAt is null',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'falls back to startedAt when both are null',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('AgentLiveState', () {
    test('sortPriority ordering', timeout: const Timeout.factor(2), () {
      ...
      // Per-run states: queued still has work coming, succeeded is settled.
      expect(AgentLiveState.queued.sortPriority, 3);
      expect(AgentLiveState.succeeded.sortPriority, 4);
      expect(AgentLiveState.idle.sortPriority, 5);
      expect(AgentLiveState.neverRun.sortPriority, 6);
    });

    test(
      'sort priority is monotonically increasing',
      timeout: const Timeout.factor(2),
      () {
        ...
        for (var i = 1; i < values.length; i++) {
          expect(
            ...
            isTrue,
            reason:
                '${values[i]}.sortPriority should be > ${values[i - 1]}.sortPriority',
          );
        }
      },
    );
