...
/// Pre-seeds the PR-detail repo scope to the screen's owner/repo. In the app
/// this scope is driven from the URL by `prDetailRouteScopeSyncProvider`; that
/// sync is not running in this widget test, so seeding the notifier directly
/// stands in for it and lets the (number-keyed) PR-review providers resolve the
/// right repo.
class _SeededPrScopeNotifier extends PrDetailRepoScopeNotifier {
  @override
  PrDetailRepoScope? build() => (owner: 'owner', repo: 'repo');
}
...
void main() {
  ...
  List baseOverrides({int prNumber = 42}) {
    return [
      ...
      prDetailRepoScopeProvider.overrideWith(_SeededPrScopeNotifier.new),
