...
Widget _wrap(Widget child) {
  return ProviderScope(
    overrides: [
      ...
      prOptimisticReviewStateProvider.overrideWith(
        _EmptyOptimisticReviewNotifier.new,
      ),
...
void main() {
  group('PrHeaderSection', () {
    testWidgets('renders body', (tester) async {
      final pr = _pr();
      await tester.pumpWidget(_wrap(PrHeaderSection(pr: pr, prNumber: 42)));
    ...
    testWidgets('renders empty body message when body is whitespace', (
      tester,
    ) async {
      final pr = _pr(body: '   \n  ');
      await tester.pumpWidget(_wrap(PrHeaderSection(pr: pr, prNumber: 42)));
  ...
  group('PrTitle', () {
    testWidgets('renders PR number and title', (tester) async {
      ...
      await tester.pumpWidget(_wrap(PrTitle(pr: pr)));
      await tester.pumpAndSettle(const Duration(seconds: 5));
  ...
  group('EditablePrTitle', () {
    testWidgets('leads the title with the PR status icon', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(EditablePrTitle(pr: pr, canEdit: false)),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
