...
void main() {
  ...
  group('review status pill', () {
    // Replaces the old `reviewStatusRingColor` group. That helper returned
    // `borderSecondary` for an open finding, which the pill then drew as text
    // over a 12%-alpha wash of the same color — "Open" was effectively
    // invisible on every row of the list. The pill is a `CcBadge` now, so the
    // contract under test is the variant + glyph pair rather than one color.

    test('open reads neutral so a full list is not flooded with tint', () {
      expect(
        reviewStatusVariant(ReviewNodeStatus.open),
        CcBadgeVariant.neutral,
      );
      expect(reviewStatusVariant(null), CcBadgeVariant.neutral);
    });

    test('consensus and resolved carry their own tint', () {
      expect(
        reviewStatusVariant(ReviewNodeStatus.consensusReady),
        CcBadgeVariant.info,
      );
      expect(
        reviewStatusVariant(ReviewNodeStatus.resolved),
        CcBadgeVariant.success,
      );
    });

    test('every status has a distinct glyph', () {
      final glyphs = {
        for (final s in ReviewNodeStatus.values) reviewStatusIcon(s),
      };
      expect(
        glyphs.length,
        ReviewNodeStatus.values.length,
        reason:
            'open and dismissed share the neutral tint, so only the shape '
            'separates them in greyscale',
      );
    });

    test('a settled finding recedes without dropping under the text floor', () {
      expect(reviewSettledOpacity(ReviewNodeStatus.open), 1);
      expect(
        reviewSettledOpacity(ReviewNodeStatus.resolved),
        greaterThan(reviewSettledOpacity(ReviewNodeStatus.dismissed)),
      );
      expect(
        reviewSettledOpacity(ReviewNodeStatus.dismissed),
        greaterThanOrEqualTo(0.55),
      );
    });
  });

  group('reviewFindingSummary', () {
    test('takes the first non-empty line', () {
      expect(reviewFindingSummary('\n\nFirst line\nSecond'), 'First line');
    });

    test('strips the markdown a one-line row would render literally', () {
      expect(
        reviewFindingSummary('## **Null deref** in `foo`'),
        'Null deref in foo',
      );
      expect(reviewFindingSummary('- A bullet finding'), 'A bullet finding');
    });

    test('is empty when there is nothing to summarize', () {
      expect(reviewFindingSummary('   \n\n'), '');
    });
  });
  ...
  group('parseAndSortFindings', () {
    ...
    test('skips messages whose payload fails validation', () {
      ...
      final invalidConfidence = Message(
        id: 'bad-conf',
        spaceId: 'ch',
        conversationId: 'ch',
        senderId: 'agent-1',
