...
void main() {
  ...
  group('showGifPicker widget', () {
    // The test environment may have KLIPY_APP_KEY set via OS env or .env.
    // We temporarily rename .env and clear the EnvConfig cache so the
    // widget shows the "not configured" card — no HTTP calls, no Dio errors.
    // Run with: KLIPY_APP_KEY= flutter test <file> --timeout 30s
    ...
    setUp(() {
      ...
      if (envFile.existsSync()) {
        ...
        envFile.renameSync('.env.test_bak');
      }
      EnvConfig.clearCache();
    });

    tearDown(() {
      final bak = File('.env.test_bak');
      if (bak.existsSync()) {
        bak.renameSync('.env');
      }
      EnvConfig.clearCache();
