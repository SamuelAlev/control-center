...
void main() {
  final frontend = _repo(id: '1', name: 'uber-cloud-a');
  final appServer = _repo(id: '2', name: 'uber-cloud-b');
  ...
  testWidgets('shows owner/repo names and counts', (tester) async {
    ...
    expect(find.text('Uber/uber-cloud-a'), findsOneWidget);
    expect(find.text('Uber/uber-cloud-b'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final selected = tester.widget<Text>(find.text('Uber/uber-cloud-a'));
      final idle = tester.widget<Text>(find.text('Uber/uber-cloud-b'));
  ...
  testWidgets('selected name is laid out across the rail, not a stub width', (
    ...
    final box = tester.getRect(find.text('Uber/uber-cloud-a'));
    // A premature ellipsis ("Uber...") is ~50px. The 224px rail minus
