...
void main() {
  ...
  group('DiffSummaryCard', () {
    testWidgets('renders empty state with zero changes', (tester) async {
      await tester.pumpWidget(_wrapPS(const Scaffold(body: DiffSummaryCard())));
    ...
    testWidgets('renders with additions and deletions', (tester) async {
      await tester.pumpWidget(
        _wrapPS(
          const Scaffold(
            body: DiffSummaryCard(filesChanged: 3, additions: 25, deletions: 7),
          ),
        ),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
    ...
    testWidgets('renders singular file text', (tester) async {
      await tester.pumpWidget(
        _wrapPS(const Scaffold(body: DiffSummaryCard(filesChanged: 1))),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
    ...
    testWidgets('icons for new, deleted, and modified files', (tester) async {
      const files = [
      ...
      expect(find.byIcon(AppIcons.filePlus), findsOneWidget);
      expect(find.byIcon(AppIcons.trash2), findsOneWidget);
      expect(find.byIcon(AppIcons.fileEdit), findsOneWidget);
    });

    testWidgets('CcBadge variant differences for add/delete', (tester) async {
      await tester.pumpWidget(
        _wrapPS(
          const Scaffold(
            body: DiffSummaryCard(filesChanged: 1, additions: 42, deletions: 7),
          ),
        ),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));

      expect(find.byType(CcBadge), findsNWidgets(2));
