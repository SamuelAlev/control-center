...
PrReviewerCandidate _teamCandidate(String slug) =>
    PrReviewerCandidate(kind: ReviewerKind.team, key: slug, label: slug);

Widget _wrap(Widget child, {_FakeRepo? repo}) {
  return ProviderScope(
    overrides: [
      prReviewRepositoryProvider.overrideWith((ref) => repo ?? _FakeRepo()),
    ],
...
void main() {
  ...
  testWidgets('assignee flyout shows empty state when no users', (
    tester,
  ) async {
    await tester.pumpWidget(_wrap(const ComposePrSidebar()));
  ...
  testWidgets('multiple assignees can be selected', (tester) async {
    final repo = _FakeRepo(
      assignableUsers: [_user('alice'), _user('bob'), _user('carol')],
    );
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
  ...
  testWidgets('assignee search filters candidates', (tester) async {
    final repo = _FakeRepo(
      assignableUsers: [_user('alice'), _user('bob'), _user('ali')],
    );
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
    ...
    await tester.enterText(find.byType(CcTextField), 'ali');
    await tester.pumpAndSettle();
  ...
  testWidgets('assignee search is case-insensitive', (tester) async {
    ...
    await tester.enterText(find.byType(CcTextField), 'alice');
    await tester.pumpAndSettle();

    expect(_rowText('Alice'), findsOneWidget);
  });

  testWidgets('assignee row shows login (name)', (tester) async {
    final repo = _FakeRepo(
      assignableUsers: [
        const PrUser(login: 'alice', avatarUrl: '', name: 'Alice Smith'),
      ],
    );
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
    ...
    expect(_rowText('alice (Alice Smith)'), findsOneWidget);
  });

  testWidgets('assignee search matches display name', (tester) async {
    final repo = _FakeRepo(
      assignableUsers: [
        const PrUser(login: 'jlpicard', avatarUrl: '', name: 'Jean-Luc Picard'),
        _user('worf'),
      ],
    );
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
    ...
    await _openAssigneePicker(tester);
    await tester.enterText(find.byType(CcTextField), 'picard');
    await tester.pumpAndSettle();

    expect(_rowText('jlpicard (Jean-Luc Picard)'), findsOneWidget);
    expect(_rowText('worf'), findsNothing);
  });
  ...
  testWidgets('reviewer flyout shows user candidates and selecting adds chip', (
    ...
    final repo = _FakeRepo(
      requestableReviewers: [_userCandidate('alice'), _userCandidate('bob')],
    );
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
  ...
  testWidgets('reviewer row shows login (name)', (tester) async {
    final repo = _FakeRepo(
      requestableReviewers: [
        PrReviewerCandidate.user(
          const PrUser(login: 'alice', avatarUrl: '', name: 'Alice Smith'),
        ),
      ],
    );
  ...
  testWidgets('reviewer flyout shows team candidates', (tester) async {
    final repo = _FakeRepo(requestableReviewers: [_teamCandidate('eng')]);
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
  ...
  testWidgets('team reviewer chip shows users icon instead of avatar', (
    ...
    // The chip for a team uses AppIcons.users (size 14).
    // The section header also uses AppIcons.users — so at least 2.
    expect(find.byIcon(AppIcons.users), findsAtLeast(2));
  });

  testWidgets('team reviewer chip shows the logo when a URL is known', (
    tester,
  ) async {
    final repo = _FakeRepo(
      requestableReviewers: [
    ...
    expect(find.byType(GitHubUserAvatar), findsWidgets);
  });

  testWidgets('toggling reviewer off removes the chip', (tester) async {
    final repo = _FakeRepo(requestableReviewers: [_userCandidate('alice')]);
    await tester.pumpWidget(_wrap(const ComposePrSidebar(), repo: repo));
  ...
  testWidgets('reviewer search filters candidates', (tester) async {
    ...
    await tester.enterText(find.byType(CcTextField), 'a');
    await tester.pumpAndSettle();
