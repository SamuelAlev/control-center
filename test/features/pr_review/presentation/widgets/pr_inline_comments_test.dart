...
PrInlineCommentsController _makeController(WidgetTester tester) {
  final container = ProviderContainer(
    overrides: [
      activeWorkspaceProvider.overrideWith((ref) => null),
      activeRepoProvider.overrideWith((ref) => null),
      // No RPC server here: the viewer identity stays unresolved, so locally
      // created entries keep the neutral 'You' author and no retry timer
...
void main() {
  group('PrCommentComposer', () {
    ...
    testWidgets('calls onSubmit when text entered and send pressed', (
      ...
      final sendButton = find.byIcon(AppIcons.arrowUp);
  ...
  group('PrCommentComposer', () {
    testWidgets('renders with seeded original code', (tester) async {
      ...
      final field = find.byType(CcTextField);
      expect(
        tester.widget<CcTextField>(field).controller?.text,
        'print("hello world");',
      );
    });
  ...
  group('PrInlineThreadBlock', () {
    ...
    testWidgets('renders thread with multiple entries after reply', (
      ...
      await tester.pumpWidget(
        _wrap(
          PrInlineThreadBlock(thread: updatedThread, controller: controller),
        ),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
    ...
    testWidgets('shows reply field when Reply... tapped', (tester) async {
      ...
      expect(
        find.byWidgetPredicate(
          (w) => w is CcTextField && (w.hintText == 'Reply\u2026'),
        ),
        findsOneWidget,
      );
    });
  ...
  group('PrSelectionToolbar', () {
    testWidgets('renders toolbar with three action icons', (tester) async {
      ...
      expect(find.bySemanticsLabel('Add a comment'), findsOneWidget);
      expect(find.bySemanticsLabel('Add a suggestion'), findsOneWidget);
      expect(find.bySemanticsLabel('Add a reaction'), findsOneWidget);
    });
    ...
    testWidgets('comment icon calls onComment', (tester) async {
      ...
      await tester.tap(find.bySemanticsLabel('Add a comment'));
    ...
    testWidgets('suggestion icon calls onSuggest', (tester) async {
      ...
      await tester.tap(find.bySemanticsLabel('Add a suggestion'));
    ...
    testWidgets('reaction icon calls onReact', (tester) async {
      ...
      await tester.tap(find.bySemanticsLabel('Add a reaction'));
      expect(reacted, true);
    });
  });
  ...
  group('SuggestionAwareMarkdown', () {
    ...
    testWidgets('renders empty body without crash', (tester) async {
      await tester.pumpWidget(
        _wrap(
          const SuggestionAwareMarkdown(prRef: _prRef, body: '', originalCode: 'original'),
        ),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
  ...
  group('PrInlineThreadBlock - edge cases', () {
    ...
    testWidgets('reply button sends reply via controller', (tester) async {
      ...
      final sendButtons = find.bySemanticsLabel('Send');
  ...
  group('PrInlineThreadBlock - thread interaction states', () {
    testWidgets('shows pending sync state indicator', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(
          PrInlineThreadBlock(thread: pendingThread, controller: controller),
        ),
      );
      await tester.pumpAndSettle(const Duration(seconds: 5));
    });
