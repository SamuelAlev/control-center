...
void main() {
  ...
  group('PrInlineThreadBlock', () {
    ...
    testWidgets('resolve button toggles thread state', (tester) async {
      ...
      final resolveButton = find.byTooltip('Resolve');
      await tester.tap(resolveButton);
      await tester.pumpAndSettle(const Duration(seconds: 5));

      expect(controller.threads.first.resolved, true);
      expect(find.byTooltip('Reopen'), findsOneWidget);

      await tester.tap(find.byTooltip('Reopen'));
      await tester.pumpAndSettle(const Duration(seconds: 5));

      expect(controller.threads.first.resolved, false);
      expect(find.byTooltip('Resolve'), findsOneWidget);
    });
  });
  ...
  group('PrInlineThreadBlock - edge cases', () {
    ...
    testWidgets('reply button sends reply via controller', (tester) async {
      ...
      expect(find.text('Nice catch'), findsOneWidget);
      expect(find.text('2 comments'), findsOneWidget);
    });
  });
  ...
  group('PrCommentsInbox - edge cases', () {
    ...
    testWidgets('inbox close button calls onClose', (tester) async {
      var closed = false;
      final controller = PrInlineCommentsController(1);
      ...
      final closeButton = find.byTooltip('Close inbox');
