...
void main() {
  ...
  group('PrDiffSearchController', () {
    ...
    test(
      'a changed patch for the same filename re-parses (no stale cache)',
      () {
        // Line indices come from the parsed patch; the parse cache must key on
        // the patch content, not the filename alone — the Source control tab
        // re-diffs the same file every few seconds while agents edit it.
        final fileList = [
          _testFile(patch: '@@ -1,2 +1,2 @@\n-needle old\n+needle new\n ctx\n'),
        ];
        final ctrl = create(f: fileList);
        ctrl.searchQuery = 'needle';
        ctrl.updateMatchData();
        expect(ctrl.matchLocations.map((m) => m.lineIndex), [1, 2]);

        // Same filename, new patch: the matches now sit two lines lower.
        fileList[0] = _testFile(
          patch: '@@ -1,4 +1,4 @@\n ctx0\n ctx1\n-needle old\n+needle new\n',
        );
        ctrl.updateMatchData();
        expect(ctrl.matchLocations.map((m) => m.lineIndex), [3, 4]);
      },
    );
