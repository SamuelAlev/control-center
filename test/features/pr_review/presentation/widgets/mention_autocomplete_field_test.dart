...
/// Focuses the CcTextField then sends text via the test input connection.
...
void main() {
  group('MentionAutocompleteField @-trigger', () {
    ...
    testWidgets('selecting user by Enter key replaces token', (tester) async {
      await tester.pumpWidget(buildWidget());
      await tester.pump();
  ...
  group('MentionAutocompleteField #-trigger', () {
    ...
    testWidgets('typing #number triggers issue search overlay', (tester) async {
  ...
  group('MentionAutocompleteField edge cases', () {
    ...
    Widget buildWidget({List<PrUser> users = _testUsers}) {
      return ProviderScope(
        overrides: [
          assignableUsersProvider.overrideWith((ref) => Future.value(users)),
        ],
        child: testWrap(
          MentionAutocompleteField(
            controller: controller,
    ...
    testWidgets(
      'selection replaces only the @ token preserving surrounding text',
      (tester) async {
        await tester.pumpWidget(buildWidget());
        await tester.pump();
  ...
  group('MentionAutocompleteField non-trigger text', () {
    ...
    testWidgets('caret at position 0 does not trigger overlay', (tester) async {
      await tester.pumpWidget(buildWidget());
      await tester.pump();
