...
void main() {
  ...
  testWidgets('action bar rendered', (tester) async {
    ...
    await tester.tap(find.text('BUG'));
    await tester.pump();
    expect(find.widgetWithText(FButton, 'Fix'), findsOneWidget);
    expect(find.widgetWithText(FButton, 'Comment'), findsOneWidget);
  });
  ...
  testWidgets('comment disabled without anchor', (tester) async {
    ...
    await tester.pump();
    final commentButton = tester.widget<FButton>(
      find.widgetWithText(FButton, 'Comment'),
    );
    expect(commentButton.onPress, isNull);
  });
