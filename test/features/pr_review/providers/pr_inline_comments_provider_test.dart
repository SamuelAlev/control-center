...
void main() {
  group('PrInlineCommentsController', () {
    ...
    setUp(() {
      ...
      controller = container.read(prInlineCommentsControllerProvider(1));
    });
    ...
    test('reply adds an entry to the thread', () {
      ...
      expect(thread.entries.length, 2);
      expect(thread.entries[1].author, 'Bob');
      expect(thread.entries[1].body, 'Second');
    });
  ...
  group('prInlineCommentsControllerProvider', () {
    test('creates a unique controller per PR number', () {
      ...
      final c1 = container.read(prInlineCommentsControllerProvider(1));
      final c2 = container.read(prInlineCommentsControllerProvider(2));
    ...
    test('returns same controller for same PR number', () {
      ...
      final a = container.read(prInlineCommentsControllerProvider(42));
      final b = container.read(prInlineCommentsControllerProvider(42));
