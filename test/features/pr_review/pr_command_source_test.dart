...
void main() {
  group('PrCommandSource', () {
    ...
    testWidgets(
      ...
      (tester) async {
        final fake = _FakeActiveRepoIdNotifier();
        // The PR belongs to org/test-backend while the active repo elsewhere may
        // be a different repo — the palette must switch to the PR's repo.
        final pr = _pr(
          id: 1,
          number: 42,
          title: 'Fix login',
          repoFullName: 'org/test-backend',
        );
        final state = PrsByRepoState(
          repos: [
            RepoPullRequests(
              repo: _repo(owner: 'org', name: 'test-backend'),
              prs: [pr],
            ),
          ],
          hasMore: const {},
            ...
            MaterialApp.router(
              routerConfig: GoRouter(
                ...
                routes: [
                  ...
                  GoRoute(
                    path: pullRequestDetailRoute('w1', 'org/test-backend', 42),
        ...
        // Active repo switched to the PR's repo (id == 'org/test-backend')...
        expect(fake.setActiveCalls, ['org/test-backend']);
