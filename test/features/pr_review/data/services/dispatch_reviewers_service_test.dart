...
class _FakeMessaging implements MessagingRepository {
  ...
  Future<void> addParticipant(
    ...
    addedParticipants.add(_AddedParticipant(spaceId, principalId));
  }
  ...
  // Unused stubs
  @override
  Stream<List<Space>> watchSpaces() => throw UnimplementedError();
  @override
  Stream<List<SpaceParticipant>> watchParticipants(String _, String _) =>
      throw UnimplementedError();
  @override
  Stream<List<Message>> watchMessages(String _, String _, String _) =>
      throw UnimplementedError();
  @override
  Stream<List<Space>> watchSpacesByWorkspace(String _) =>
      throw UnimplementedError();
  @override
  Future<Message?> getMessageById(String _, String _) =>
      throw UnimplementedError();
  @override
  ...
  @override
  Future<void> setSpaceMode(String _, String _, Mode _) =>
      throw UnimplementedError();
  @override
  ...
  Future<void> markCompacted(String _, List<String> _) =>
      throw UnimplementedError();
  @override
  Future<void> deleteSpace(String _, String _) => throw UnimplementedError();
  @override
  Future<void> updateSpaceName(String _, String _, String _) =>
      throw UnimplementedError();
  @override
  Future<void> clearSpaceMessages(String _, String _) =>
      throw UnimplementedError();
  @override
  ...
  Future<void> updateMessageEmbedding(String _, String _, Uint8List _) =>
      throw UnimplementedError();
  @override
  Future<List<EmbeddedMessage>> getMessagesWithEmbedding(String _, String _) =>
      throw UnimplementedError();
  @override
...
class _FakeReviewSpaces implements ReviewSpaceRepository {
  _FakeReviewSpaces({this._assocs = const []});
  final List<ReviewSpaceAssociation> _assocs;
  final Map<String, ReviewSpaceStatus> _statusUpdates = {};

  Map<String, ReviewSpaceStatus> get statusUpdates =>
  ...
  Stream<ReviewSpaceAssociation?> watchBySpace(
    ...
    _assocs.cast<ReviewSpaceAssociation?>().firstWhere(
      (a) => a?.spaceId == spaceId,
      orElse: () => null,
    ),
  );
  ...
  @override
  Stream<ReviewSpaceAssociation?> watchByPr(String _, String _) =>
      throw UnimplementedError();
  @override
  Stream<List<ReviewSpaceAssociation>> watchByWorkspace(String _) =>
      throw UnimplementedError();
  @override
...
class _FakeMessagingPort implements MessagingPort {
  ...
  @override
  Future<bool> spaceExists(String _, String _) async => true;
  @override
  Future<Space> createSpace(
    String _,
    String _,
  ...
  // Unused-by-this-test surface (sendUserMessage/addAgentToSpace/steerRun/…)
...
class _AddedParticipant {
  const _AddedParticipant(this.spaceId, this.agentId);
  final String spaceId;
...
void main() {
  ...
  group('dispatch logic', () {
    test('adds participant when agent is not already in the space', () async {
      seedWorkspaceAndAgents();
    ...
    test(
      'does NOT add participant when agent is already in the space',
      () async {
        seedWorkspaceAndAgents();
        messaging = _FakeMessaging(
          participants: [
    ...
    test('sends assignment message to the space', () async {
      seedWorkspaceAndAgents();
    ...
    test('dispatches agent via messaging port with brief', () async {
      seedWorkspaceAndAgents();
      reviewSpaces = _FakeReviewSpaces(assocs: [_assoc()]);
  ...
  group('status transitions', () {
    test('transitions requested → inProgress when agents dispatched', () async {
      seedWorkspaceAndAgents();
      reviewSpaces = _FakeReviewSpaces(
        assocs: [_assoc(status: ReviewSpaceStatus.requested)],
      );
      ...
      expect(
        reviewSpaces.statusUpdates['assoc-1'],
    ...
    test('does NOT transition when no agents were dispatched', () async {
      ...
      expect(reviewSpaces.statusUpdates, isEmpty);
    });
    ...
    test('does NOT transition when status is already inProgress', () async {
      seedWorkspaceAndAgents();
      reviewSpaces = _FakeReviewSpaces(
        assocs: [_assoc(status: ReviewSpaceStatus.inProgress)],
      );
    ...
    test('does NOT transition when status is completed', () async {
      seedWorkspaceAndAgents();
      reviewSpaces = _FakeReviewSpaces(
        assocs: [_assoc(status: ReviewSpaceStatus.completed)],
      );
    ...
    test('does NOT transition when status is awaitingApproval', () async {
      seedWorkspaceAndAgents();
      reviewSpaces = _FakeReviewSpaces(
        assocs: [_assoc(status: ReviewSpaceStatus.awaitingApproval)],
      );
  ...
  group('space result', () {
    test('result includes space_id', () async {
      seedWorkspaceAndAgents();
  ...
  group('result structure', () {
    ...
    test('return map includes all expected top-level keys', () async {
      ...
      expect(
        result.keys,
        containsAll(['space_id', 'concurrency', 'dispatched', 'unmatched']),
      );
    });
  ...
  // -----------------------------------------------------------------------
  // Space participant edge cases
  // -----------------------------------------------------------------------

  group('space participant edge cases', () {
    ...
    test('does not re-add participant when called twice', () async {
      ...
      // Seed participant so second call sees agent already in space.
      messaging = _FakeMessaging(
        participants: [
          SpaceParticipant(
            id: 'p1',
...
class _MockConversationDirFilesystemPort extends FakeFilesystemPort {
  ...
  @override
  Future<String> spaceDir(String _, String _) async => _tmpDir.path;
}
...
class _ThrowingFilesystemPort extends FakeFilesystemPort {
  @override
  Future<String> spaceDir(String _, String _) async =>
