...
class _FakeFilesystem extends Fake implements WorkspaceFilesystemPort {
  ...
  Future<String> prCloneDir(
    ...
    String owner,
    String repo,
  ) async => '$baseDir/$workspaceId/pr_clones/${owner}__$repo';
}
...
class _OneThrowFilesystem extends Fake implements WorkspaceFilesystemPort {
  ...
  Future<String> prCloneDir(
    ...
    return '$baseDir/$workspaceId/pr_clones/${owner}__$repo';
  }
}
...
/// Builds the manager under test.
///
/// [missingRiftIsExpected] defaults to TRUE, i.e. the Windows contract: rift is
/// not built there, so an unloadable dylib means "seed via network clone"
/// rather than "broken install". That default lets tests set
/// `_FakeRift(available: false)` to reach the network-clone path they are about.
/// On a platform that ships rift an unloadable dylib must THROW instead — pass
/// `missingRiftIsExpected: false` for that (see the broken-install test).
...
void main() {
  ...
  group('busy flag', () {
    ...
    test('allows sequential calls after first completes', () async {
      ...
      final events1 = await manager
          .ensureCloneAndFetch(prNumber: 1, baseRef: 'main', headSha: 'abc123')
          .toList();
  ...
  group('state machine — fresh clone', () {
    ...
    test('clone uses clean URL as positional arg; token rides in env', () async {
      final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
      addTearDown(() => tmp.deleteSync(recursive: true));
      ...
      // VULN-010: the token is never embedded in the remote URL. The clone URL
      // is the clean HTTPS URL, and the PAT rides in GIT_CONFIG_PARAMETERS as an
      // http.extraHeader Authorization header (env, not argv).
      final cloneArgs = git.runs[0];
      expect(cloneArgs, contains('https://github.com/octocat/hello.git'));
      expect(
        cloneArgs.any((a) => a.contains('x-access-token')),
        isFalse,
        reason: 'token must never appear in git argv',
      );
      // The last arg is the destination path.
      final clonePath = cloneArgs.last;
      expect(clonePath, contains('pr_clones'));

      final cloneEnv = git.runEnvs[0];
      final expectedHeader = base64Encode(utf8.encode('x-access-token:token'));
      expect(
        cloneEnv['GIT_CONFIG_PARAMETERS'],
        contains(
  ...
  group('CoW via rift', () {
    test('uses CoW copy and emits cloning → fetching → ready', () async {
      ...
      final rift = _FakeRift(available: true, createResult: _cloneDirPath(tmp));
      final git = _FakeGit();
      git.enqueueStreamLines(['Fetching...']);
    ...
    test(
      'falls back to network clone when CoW throws generic Exception',
      () async {
        final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
        addTearDown(() => tmp.deleteSync(recursive: true));
    ...
    test('network-clones where rift is never built (Windows)', () async {
      // Windows ships no rift, so an unloadable dylib means "seed over the
      // network" — the correct answer there, not a degradation.
      final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
      addTearDown(() => tmp.deleteSync(recursive: true));
    ...
    test('surfaces an unloadable rift dylib as an error phase', () async {
      // On a platform that ships rift, a load failure is a broken install. It
      // must surface rather than silently network-cloning forever (which would
      // just look like "PR clones are slow").
      final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
      addTearDown(() => tmp.deleteSync(recursive: true));
      ...
      expect(events.last.phase, PrClonePhase.error);
      expect(
        events.last.error,
  ...
  group('empty stream output', () {
    ...
    test(
      'produces phase markers when fetch produces no lines (skip clone)',
      () async {
        final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
        addTearDown(() => tmp.deleteSync(recursive: true));
  ...
  group('auth URL edge cases', () {
    test(
      'clean URL used for clone even with special characters in repo name',
      () async {
        final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
        addTearDown(() => tmp.deleteSync(recursive: true));
        ...
        // VULN-010: clean URL in argv, token in env only.
        final cloneArgs = git.runs[0];
        expect(
          cloneArgs,
          contains('https://github.com/my-org/my-repo.with.dots.git'),
        );
        expect(cloneArgs.any((a) => a.contains('x-access-token')), isFalse);
        final expectedHeader = base64Encode(
          utf8.encode('x-access-token:token'),
        );
        expect(
          git.runEnvs[0]['GIT_CONFIG_PARAMETERS'],
          contains(
            "'http.https://github.com/.extraHeader=Authorization: Basic $expectedHeader'",
          ),
        );
    ...
    test('clean URL after clone has no token for custom owner/repo', () async {
      ...
      expect(
        remoteArgs.last,
        'https://github.com/my-org/my-repo.with.dots.git',
      );
    });
  });
  ...
  group('additional error paths', () {
    ...
    test(
      'busy flag is reset after stream error from fetch (skip clone)',
      () async {
        final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
        addTearDown(() => tmp.deleteSync(recursive: true));
    ...
    test('error phase includes the error object in progress event', () async {
      ...
      final fs = _OneThrowFilesystem(tmp.path, StateError('Disk full'));
      final git = _FakeGit();
      final manager = _manager(tmp: tmp, git: git, filesystem: fs);
  ...
  group('clone directory creation', () {
    test(
      'creates parent directory when it does not exist before clone',
      () async {
        final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
        addTearDown(() => tmp.deleteSync(recursive: true));
  ...
  group('edge cases', () {
    ...
    test('ensureCloneAndFetch with empty baseRef still works', () async {
      ...
      final events = await manager
          .ensureCloneAndFetch(prNumber: 1, baseRef: '', headSha: 'abc123')
          .toList();
    ...
    test('clone with empty github token injects no auth header', () async {
      final tmp = Directory.systemTemp.createTempSync('pr_clone_test_');
      addTearDown(() => tmp.deleteSync(recursive: true));
      ...
      final manager = _manager(tmp: tmp, git: git, githubToken: '');
      ...
      // VULN-010: with an empty token, no extraHeader Authorization is added,
      // and the clean URL is still used for the clone.
      final cloneArgs = git.runs[0];
      expect(cloneArgs, contains('https://github.com/octocat/hello.git'));
      final cloneEnv = git.runEnvs[0];
      final gitConfigParams = cloneEnv['GIT_CONFIG_PARAMETERS'] ?? '';
      expect(gitConfigParams, isNot(contains('extraHeader')));
      expect(gitConfigParams, isNot(contains('x-access-token')));
    });
