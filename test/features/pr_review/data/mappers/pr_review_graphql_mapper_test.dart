...
void main() {
  group('pullRequestFromGraphQlNode', () {
    test('maps the core list fields and metrics', () {
      ...
      expect(pr.externalId, 'PR_node_42');
    ...
    test('maps user and team requested reviewers; ignores empty reviewers', () {
      ...
      expect(pr.requestedReviewers.map((u) => u.login), ['reviewer1']);
      expect(pr.requestedTeamSlugs, ['frontend-platform']);
      expect(pr.isPriority, isTrue);
    });

    test(
      'derives reviewedByMe from latestReviews against the viewer (case-insensitive)',
      () {
