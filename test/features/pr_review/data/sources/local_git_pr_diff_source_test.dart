...
class _TestFilesystemPort extends FakeFilesystemPort {
  ...
  @override
  Future<String> prCloneDir(String ws, String owner, String repo) async =>
      _cloneDir.path;
}
...
({
  Directory dir,
  _FakeGitCommandPort git,
  _TestFilesystemPort fs,
  LocalGitPrDiffSource source,
})
_setup() {
...
void main() {
  ...
  group('_sanitize (via diff progress)', () {
    ...
    test('strips ANSI escape codes from progress lines', () async {
      ...
      git.enqueueRun((a, onProgress) async {
        ...
        return _okWithStderr(
          '1\t0\ta.dart$_nul',
          '${_ansiBrown}Receiving objects: 45%$_ansiReset',
        );
      });
      ...
      final progressMessages = events
          .where(
            (e) =>
                e.clonePhase == ClonePhase.computing &&
                e.cloneMessage.isNotEmpty,
          )
    ...
    test('strips "remote: " prefix', () async {
      ...
      for (final msg in progressMessages) {
        expect(
          msg.startsWith('remote:'),
          isFalse,
          reason: '"remote:" in "$msg"',
        );
      }
    });
    ...
    test('trims whitespace and does not emit empty messages', () async {
      ...
      final computingEvents = events.where(
        (e) => e.clonePhase == ClonePhase.computing,
      );
  ...
  group('_mapPhase (via clone progress)', () {
    ...
    test('ClonePhase.error emitted on merge-base failure', () async {
      ...
      expect(
        errorEvents.first.error.toString(),
  ...
  group('diff output parsing', () {
    ...
    test('handles renamed files (previousFilename populated)', () async {
      ...
      git.enqueueRun(
        (_, _) async =>
            _ok('0\t0\t${_nul}src/old.dart${_nul}src/new.dart$_nul'),
      );
      // name-status rename
      git.enqueueRun(
        (_, _) async => _ok('R100${_nul}src/old.dart${_nul}src/new.dart$_nul'),
      );
      git.enqueueRun(
        (_, _) async => _ok(
    ...
    test(
      'falls back to modified for files without name-status entry',
      () async {
  ...
  group('error handling', () {
    ...
    test('merge-base failure yields error event', () async {
      ...
      expect(errorEvents, isNotEmpty);
      expect(
        errorEvents.last.error.toString(),
        contains('Could not resolve merge base'),
      );
    });
    ...
    test(
      'diff command failure still emits file list without patches',
      () async {
        git.enqueueStream((_) => const Stream.empty());
        git.enqueueRun((_, _) async => _ok('abc\n'));
        git.enqueueRun((_, _) async => _ok('1\t0\ta.dart$_nul'));
  ...
  group('watchCommits', () {
    ...
    test(
      'yields empty list when merge base is empty and rev-parse fails',
      () async {
        git.enqueueRun((_, _) async => _ok(''));
        git.enqueueRun((_, _) async => _fail('no rev-parse'));
    ...
    test(
      'commit messages with embedded newlines are skipped gracefully',
      () async {
    ...
    test('handles unparseable date gracefully', () async {
      ...
      git.enqueueRun(
        (_, _) async =>
            _ok(['sha\x1fmsg\x1fdev\x1fdev@acme.com\x1fnot-a-date'].join('\n')),
      );
  ...
  group('watchCommitFiles', () {
    ...
    test('handles renamed files for a specific commit', () async {
      ...
      git.enqueueRun(
        (_, _) async =>
            _ok('0\t0\t${_nul}old/path.dart${_nul}new/path.dart$_nul'),
      );
      // name-status rename
      git.enqueueRun(
        (_, _) async =>
            _ok('R100${_nul}old/path.dart${_nul}new/path.dart$_nul'),
      );
      // full diff
      git.enqueueRun(
        (_, _) async => _ok(
  ...
  group('watchFiles edge cases', () {
    ...
    test(
      'patch lookup falls back to previousFilename for renamed files',
      () async {
        ...
        git.enqueueRun(
          (_, _) async =>
              _ok('0\t0\t${_nul}old_file.dart${_nul}new_file.dart$_nul'),
        );
        // name-status rename
        git.enqueueRun(
          (_, _) async =>
              _ok('R100${_nul}old_file.dart${_nul}new_file.dart$_nul'),
        );
    ...
    test(
      'numstat record with no tab (less than 3 fields) is skipped',
      () async {
        // parseGitNumstatZ skips records where `tab.length < 3`.
        git.enqueueStream((_) => const Stream.empty());
        git.enqueueRun((_, _) async => _ok('abc\n'));
        git.enqueueRun(
          (_, _) async => _ok('not-enough-fields${_nul}1\t0\tok.dart$_nul'),
        );
