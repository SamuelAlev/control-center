...
void main() {
  group('DiffOverflowMode', () {
    test('has wrap and scroll values', timeout: const Timeout.factor(2), () {
      expect(
        DiffOverflowMode.values,
        containsAll([DiffOverflowMode.wrap, DiffOverflowMode.scroll]),
      );
    });
    ...
    test(
      'fromName defaults to wrap for null',
    ...
    test(
      'fromName defaults to wrap for unknown values',
      timeout: const Timeout.factor(2),
      () {
