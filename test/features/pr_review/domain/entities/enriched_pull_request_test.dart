...
void main() {
  ...
  group('EnrichedPullRequest', () {
    test('repoFullName delegates to repo.fullName', () {
      final pr = createPr();
      final repo = createRepo(remoteOwner: 'acme', remoteName: 'widgets');
      final enriched = NormalPr(pr: pr, repo: repo);
      expect(enriched.repoFullName, 'acme/widgets');
    });

    test('repoOwner delegates to repo.remoteOwner', () {
      final pr = createPr();
      final repo = createRepo(remoteOwner: 'acme');
      final enriched = NormalPr(pr: pr, repo: repo);
      expect(enriched.repoOwner, 'acme');
    });

    test('repoName delegates to repo.remoteName', () {
      final pr = createPr();
      final repo = createRepo(remoteName: 'widgets');
      final enriched = NormalPr(pr: pr, repo: repo);
  ...
  group('RepoPullRequests', () {
    ...
    test('== returns false for different repo', () {
      final a = RepoPullRequests(
        repo: createRepo(id: 'r1'),
        prs: [],
      );
      final b = RepoPullRequests(
        repo: createRepo(id: 'r2'),
        prs: [],
      );
      expect(a, isNot(equals(b)));
    });
