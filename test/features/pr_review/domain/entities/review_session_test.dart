import 'package:cc_domain/features/pr_review/domain/entities/review_session.dart';
...
void main() {
  ...
  group('ReviewSession status getters', () {
    test(
      'isInProgress is true when inProgress',
    ...
    test(
      'isCompleted is true when completed',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'isAbandoned is true when abandoned',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('ReviewSessionStatus.tryParse', () {
    test('parses in_progress', timeout: const Timeout.factor(2), () {
      expect(
        ReviewSessionStatus.tryParse('in_progress'),
    ...
    test('parses completed', timeout: const Timeout.factor(2), () {
      expect(
        ReviewSessionStatus.tryParse('completed'),
    ...
    test('parses abandoned', timeout: const Timeout.factor(2), () {
      expect(
        ReviewSessionStatus.tryParse('abandoned'),
        ReviewSessionStatus.abandoned,
      );
    });

    test(
      'defaults to inProgress for unknown values',
      timeout: const Timeout.factor(2),
      () {
        expect(
          ReviewSessionStatus.tryParse('unknown'),
          ReviewSessionStatus.inProgress,
        );
        expect(
          ReviewSessionStatus.tryParse(''),
          ReviewSessionStatus.inProgress,
        );
      },
    );
  ...
  group('ReviewSessionStatus.serializedName', () {
    test(
      'returns correct serialized names',
      timeout: const Timeout.factor(2),
      () {
