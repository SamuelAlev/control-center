...
void main() {
  ...
  group('PrUser constructor', () {
    ...
    test('displayLabel appends name when it differs from login', () {
      const named = PrUser(login: 'alice', avatarUrl: '', name: 'Alice Smith');
      const same = PrUser(login: 'alice', avatarUrl: '', name: 'Alice');
      const blank = PrUser(login: 'alice', avatarUrl: '', name: '  ');
      expect(named.displayLabel, 'alice (Alice Smith)');
      expect(same.displayLabel, 'alice');
      expect(blank.displayLabel, 'alice');
      expect(createUser().displayLabel, 'dev');
    });

    test('matchesQuery looks at login and name', () {
      const user = PrUser(
        login: 'jlpicard',
        avatarUrl: '',
        name: 'Jean-Luc Picard',
      );
      expect(user.matchesQuery('picard'), isTrue);
      expect(user.matchesQuery('JL'), isTrue);
      expect(user.matchesQuery('worf'), isFalse);
      expect(user.matchesQuery(''), isTrue);
    });
  ...
  group('PrUser == and hashCode', () {
    ...
    test(
      'same login but different avatarUrl are equal (identity by login)',
      () {
