...
void main() {
  ...
  group('PrUserReviewer', () {
    ...
    test(
      'identity is lowercased user login',
    ...
    test(
      'not equal when isCodeOwner differs',
      timeout: const Timeout.factor(2),
      () {
        const a = PrUserReviewer(
          user: user,
  ...
  group('PrTeamReviewer', () {
    ...
    test(
      'identity is lowercased team slug',
      timeout: const Timeout.factor(2),
      () {
        const reviewer = PrTeamReviewer(
          name: 'Backend',
    ...
    test(
      'not equal when reviewedBy differs',
      timeout: const Timeout.factor(2),
      () {
        const a = PrTeamReviewer(
          name: 'Team',
  ...
  group('ReviewerKind', () {
    test('has user and team values', timeout: const Timeout.factor(2), () {
      expect(
        ReviewerKind.values,
        containsAll([ReviewerKind.user, ReviewerKind.team]),
      );
    });
  });
  ...
  group('PrReviewerCandidate', () {
    test(
      'user factory creates user candidate',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'user factory labels with login (name) when name differs',
      timeout: const Timeout.factor(2),
      () {
        final candidate = PrReviewerCandidate.user(
          const PrUser(login: 'alice', avatarUrl: '', name: 'Alice Smith'),
        );
        expect(candidate.key, 'alice');
        expect(candidate.label, 'alice (Alice Smith)');
      },
    );
    ...
    test(
      'equal when kind and key match (case-insensitive)',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'avatarUrl defaults to null for teams',
      timeout: const Timeout.factor(2),
      () {
        const candidate = PrReviewerCandidate(
          kind: ReviewerKind.team,
    ...
    test('team candidate can carry a logo URL', () {
      const candidate = PrReviewerCandidate(
        kind: ReviewerKind.team,
        key: 'eng',
        label: 'Eng',
      ...
      expect(candidate.avatarUrl, 'https://t/eng');
    });
  });
}
