...
void main() {
  ...
  group('CheckRun constructor', () {
    ...
    test('throws assertion error for empty name', () {
      expect(
        () =>
            CheckRun(name: '', status: CheckRunStatus.queued, conclusion: null),
  ...
  group('CheckRun computed properties', () {
    test('isComplete returns true for completed status', () {
      expect(
        createCheckRun(status: CheckRunStatus.completed).isComplete,
        isTrue,
      );
      expect(createCheckRun(status: CheckRunStatus.queued).isComplete, isFalse);
      expect(
        createCheckRun(status: CheckRunStatus.inProgress).isComplete,
        isFalse,
      );
    });
  ...
  group('CheckRunStatus', () {
    ...
    group('fromString', () {
      test('parses queued', () {
        expect(
          CheckRunStatusExtension.fromString('queued').name,
          CheckRunStatus.queued.name,
        );
      });
      test('parses in_progress', () {
        expect(
          CheckRunStatusExtension.fromString('in_progress').name,
          CheckRunStatus.inProgress.name,
        );
      });
      test('parses completed', () {
        expect(
          CheckRunStatusExtension.fromString('completed').name,
          CheckRunStatus.completed.name,
        );
      });
      test('unknown defaults to queued', () {
        expect(
          CheckRunStatusExtension.fromString('bogus').name,
          CheckRunStatus.queued.name,
        );
      });
      test('empty string defaults to queued', () {
        expect(
          CheckRunStatusExtension.fromString('').name,
          CheckRunStatus.queued.name,
        );
      });
    });
  ...
  group('CheckRunConclusion', () {
    ...
    group('fromString', () {
      test('parses success', () {
        expect(
          CheckRunConclusionExtension.fromString('success').name,
          CheckRunConclusion.success.name,
        );
      });
      test('parses failure', () {
        expect(
          CheckRunConclusionExtension.fromString('failure').name,
          CheckRunConclusion.failure.name,
        );
      });
      test('parses neutral', () {
        expect(
          CheckRunConclusionExtension.fromString('neutral').name,
          CheckRunConclusion.neutral.name,
        );
      });
      test('parses cancelled', () {
        expect(
          CheckRunConclusionExtension.fromString('cancelled').name,
          CheckRunConclusion.cancelled.name,
        );
      });
      test('parses skipped', () {
        expect(
          CheckRunConclusionExtension.fromString('skipped').name,
          CheckRunConclusion.skipped.name,
        );
      });
      test('parses timed_out', () {
        expect(
          CheckRunConclusionExtension.fromString('timed_out').name,
          CheckRunConclusion.timedOut.name,
        );
      });
      test('parses action_required', () {
        expect(
          CheckRunConclusionExtension.fromString('action_required').name,
          CheckRunConclusion.actionRequired.name,
        );
      });
      test('parses stale', () {
        expect(
          CheckRunConclusionExtension.fromString('stale').name,
          CheckRunConclusion.stale.name,
        );
      });
      test('unknown defaults to neutral', () {
        expect(
          CheckRunConclusionExtension.fromString('bogus').name,
          CheckRunConclusion.neutral.name,
        );
      });
      test('empty string defaults to neutral', () {
        expect(
          CheckRunConclusionExtension.fromString('').name,
          CheckRunConclusion.neutral.name,
        );
      });
    });
