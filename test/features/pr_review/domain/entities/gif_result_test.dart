...
void main() {
  ...
  group('GifResult.fromJson', () {
    ...
    test(
      'falls back to sm gif when no hd',
      timeout: const Timeout.factor(2),
      () {
        final json = {
          'id': 2,
    ...
    test(
      'falls back to sm webp for preview',
      timeout: const Timeout.factor(2),
      () {
        final json = {
          'id': 3,
    ...
    test(
      'falls back to hd gif url for preview when sm has no gif/webp/jpg',
      timeout: const Timeout.factor(2),
      () {
        final json = {
          'id': 4,
    ...
    test('parses string id as int', timeout: const Timeout.factor(2), () {
      final json = {'id': '99', 'file': <String, dynamic>{}};
      final result = GifResult.fromJson(json);
      expect(result.id, 99);
    });

    test(
      'defaults to 0 for non-parseable id',
      timeout: const Timeout.factor(2),
      () {
        final json = {'id': 'not-a-number', 'file': <String, dynamic>{}};
        final result = GifResult.fromJson(json);
        expect(result.id, 0);
      },
    );
    ...
    test(
      'defaults to empty strings and zero dimensions for missing file data',
      timeout: const Timeout.factor(2),
      () {
        final json = {'id': 5, 'file': <String, dynamic>{}};
        final result = GifResult.fromJson(json);
        expect(result.url, '');
    ...
    test(
      'handles missing file key entirely',
      timeout: const Timeout.factor(2),
      () {
  ...
  group('GifResult == and hashCode', () {
    ...
    test(
      'not equal when previewUrl differs',
      timeout: const Timeout.factor(2),
      () {
        const b = GifResult(
          id: 1,
    ...
    test(
      'not equal when dimensions differ',
      timeout: const Timeout.factor(2),
      () {
        const b = GifResult(
          id: 1,
