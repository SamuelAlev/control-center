...
import 'package:cc_domain/features/pr_review/domain/usecases/invalidate_pr_cache.dart';
import 'package:test/test.dart';

class FakePrReviewRepository implements PrReviewRepository {
  final List<int> invalidatePullRequestCalls = [];

  @override
  Future<void> invalidatePullRequest(int prNumber) async {
    invalidatePullRequestCalls.add(prNumber);
  }
...
void main() {
  group('InvalidatePrCacheUseCase', () {
    late FakePrReviewRepository repo;
    late InvalidatePrCacheUseCase useCase;

    setUp(() {
      repo = FakePrReviewRepository();
      useCase = InvalidatePrCacheUseCase(repository: repo);
    });

    test(
      'delegates to repository.invalidatePullRequest with correct PR number',
      () async {
        await useCase.execute(42);

        expect(repo.invalidatePullRequestCalls, [42]);
      },
    );
  });
}
