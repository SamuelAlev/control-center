  ...
  group('ClassifyShipShowAskUseCase', () {
    group('ask lane', () {
      test('draft PR', () {
        final result = useCase.classify(
          ...
          checks: [_check(name: 'ci', conclusion: CheckRunConclusion.success)],
        );
      ...
      test('CI failing', () {
        final result = useCase.classify(
          pr: _pr(number: 1),
          files: [_file(filename: 'src/app.dart', additions: 5)],
          checks: [_check(name: 'ci', conclusion: CheckRunConclusion.failure)],
        );
      ...
      test('large change — >100 LOC', () {
        final result = useCase.classify(
          pr: _pr(number: 1),
          files: [
            _file(filename: 'src/app.dart', additions: 60, deletions: 41),
          ],
          checks: [_check(name: 'ci', conclusion: CheckRunConclusion.success)],
        );
    ...
    group('show lane', () {
      test('moderate change — 21-100 LOC', () {
        final result = useCase.classify(
          pr: _pr(number: 1),
          files: [_file(filename: 'src/app.dart', additions: 30, deletions: 5)],
          checks: [_check(name: 'ci', conclusion: CheckRunConclusion.success)],
        );
    ...
    group('edge cases', () {
      ...
      test('in-progress CI → treat as passing', () {
        final result = useCase.classify(
          pr: _pr(number: 1),
          files: [_file(filename: 'src/app.dart', additions: 5)],
          checks: [_check(name: 'ci', status: CheckRunStatus.inProgress)],
        );
      ...
      test('queued CI → treat as passing', () {
        final result = useCase.classify(
          pr: _pr(number: 1),
          files: [_file(filename: 'src/app.dart', additions: 5)],
          checks: [_check(name: 'ci', status: CheckRunStatus.queued)],
        );
