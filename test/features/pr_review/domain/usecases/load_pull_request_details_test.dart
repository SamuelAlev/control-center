...
void main() {
  group('LoadPullRequestDetailsUseCase', () {
    ...
    test('stream emits expected PR', () async {
      ...
      // watchPullRequest returns a broadcast stream, so emit only after a
      // listener is attached (stream.first subscribes synchronously).
      final firstEmission = stream.first;
      repo.emitPullRequest(expectedPr);

      final emitted = await firstEmission;
