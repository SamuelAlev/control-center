...
void main() {
  group('FocusModeState', () {
    ...
    group('elapsed', () {
      ...
      test(
        'computes elapsed from sessionStartedAt when active and not paused',
        () {
      ...
      test(
        'freezes elapsed when paused (uses pausedAt instead of DateTime.now)',
        () {
    ...
    group('minutesRemaining', () {
      ...
      test('returns clamped value when elapsed exceeds duration', () {
        ...
        final state = FocusModeState(active: true, sessionStartedAt: startedAt);
      ...
      test('returns full custom duration when elapsed is zero', () {
        const state = FocusModeState(active: false, sessionDurationMinutes: 30);
    ...
    group('withinSession', () {
      ...
      test('boundary case - exactly at session duration', () {
        final startedAt = DateTime.now().subtract(const Duration(minutes: 50));
        final state = FocusModeState(active: true, sessionStartedAt: startedAt);
    ...
    group('copyWith', () {
      ...
      test('replaces sessionStartedAt', () {
        ...
        final copy = const FocusModeState(
          active: true,
        ).copyWith(sessionStartedAt: dt);
      ...
      test('replaces sessionDurationMinutes', () {
        final copy = const FocusModeState(
          active: true,
        ).copyWith(sessionDurationMinutes: 25);
      ...
      test('replaces blockNotifications', () {
        final copy = const FocusModeState(
          active: true,
        ).copyWith(blockNotifications: false);
      ...
      test('clears pausedAt when clearPausedAt is true', () {
        final original = FocusModeState(active: true, pausedAt: DateTime(2026));
      ...
      test('clear flags take precedence over new values', () {
        ...
        final copy = original.copyWith(clearGoal: true, goal: 'rewrite');
        expect(copy.goal, isNull);
      });
    ...
    group('equality', () {
      ...
      test('different pausedAt makes them unequal', () {
        final a = FocusModeState(active: true, pausedAt: DateTime(2026, 6, 10));
        final b = FocusModeState(active: true, pausedAt: DateTime(2026, 6, 11));
        expect(a, isNot(equals(b)));
      });
