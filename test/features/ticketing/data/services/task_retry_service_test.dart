  ...
  group('TaskRetryService.maybeRetry', () {
    ...
    for (final reason in RetryableFailureReason.values) {
      group('RetryableFailureReason.${reason.name}', () {
        test('returns true when attempt=0 < maxAttempts=3', () async {
        ...
        test('returns true when attempt=2 < maxAttempts=3', () async {
          final result = await service.maybeRetry(
            ticketId: 'ticket-2',
            failureReason: reason.name,
            attempt: 2,
        ...
        test('returns true with maxAttempts=1 and attempt=0', () async {
          final result = await service.maybeRetry(
            ticketId: 'ticket-3',
            failureReason: reason.name,
            attempt: 0,
    ...
    group('pipeline tasks', () {
      test(
        'returns false when isPipelineTask is true (valid reason)',
        () async {
          final result = await service.maybeRetry(
            ticketId: 'pipeline-1',
      ...
      test(
        'returns false when isPipelineTask is true (unrecognized reason)',
        () async {
          final result = await service.maybeRetry(
            ticketId: 'pipeline-2',
      ...
      test(
        'returns false when isPipelineTask is true and attempt is already at max',
        () async {
          final result = await service.maybeRetry(
            ticketId: 'pipeline-3',
    ...
    group('attempt at or beyond maxAttempts', () {
      test('returns false when attempt == maxAttempts', () async {
        final result = await service.maybeRetry(
          ticketId: 'ticket-eq',
          failureReason: RetryableFailureReason.timeout.name,
          attempt: 3,
      ...
      test('returns false when maxAttempts=0 and attempt=0', () async {
        final result = await service.maybeRetry(
      ...
      test('returns false when maxAttempts=0 and attempt=1', () async {
        final result = await service.maybeRetry(
    ...
    group('unrecognized failure reasons', () {
      ...
      test('returns false for an arbitrary unrecognized string', () async {
        final result = await service.maybeRetry(
      ...
      test(
        'returns false for a near-miss reason with different casing',
        () async {
          final result = await service.maybeRetry(
      ...
      test('returns false for a null-like string', () async {
        final result = await service.maybeRetry(
    ...
    group('edge cases', () {
      test('attempt=0 with generous maxAttempts returns true', () async {
        final result = await service.maybeRetry(
          ticketId: 'edge-0',
          failureReason: RetryableFailureReason.runtimeOffline.name,
          attempt: 0,
      ...
      test(
        'maxAttempts=1 with attempt=1 returns false (non-retryable boundary)',
        () async {
          final result = await service.maybeRetry(
            ticketId: 'edge-1',
            failureReason: RetryableFailureReason.sandboxViolation.name,
      ...
      test('maxAttempts=1 with attempt=0 is retryable (last chance)', () async {
        final result = await service.maybeRetry(
          ticketId: 'edge-last',
          failureReason: RetryableFailureReason.timeout.name,
          attempt: 0,
    ...
    test('RetryableFailureReason.values contains exactly three reasons', () {
      ...
      expect(
        names,
        containsAll(['runtimeOffline', 'timeout', 'sandboxViolation']),
      );
