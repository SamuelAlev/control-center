...
class _FakeTicketProviderPort implements TicketProviderPort {
  _FakeTicketProviderPort({this.tickets = const [], this.supportsSync = true});
...
void main() {
  group('TicketSyncService', () {
    ...
    test(
      'sync sets createdAt and updatedAt from remote when present',
      () async {
        final created = DateTime(2024, 6, 15);
        final updated = DateTime(2025, 1, 10);
        final port = _FakeTicketProviderPort(
          tickets: [
            _remote(externalId: 'e1', createdAt: created, updatedAt: updated),
          ],
        );
        final repo = _FakeTicketRepository();
        final service = TicketSyncService(port: port, repository: repo);
    ...
    test(
      'sync falls back to DateTime.now for missing createdAt/updatedAt',
      () async {
...
class _ThrowingTicketRepository extends _FakeTicketRepository {
  @override
  Future<void> upsertMirror(Ticket ticket) async => throw Exception('DB error');
}
