...
Widget _wrapRail(Ticket ticket, {List<Agent> agents = const []}) {
  ...
  return ProviderScope(
    overrides: [
      ...
      ticketCollaboratorsProvider(
        ticket.id,
      ).overrideWith((ref) => Stream.value(ticket.collaborators)),
      ticketLinksProvider((
    ...
    child: testWrap(
      TicketPropertiesRail(ticket: ticket, workspaceId: ticket.workspaceId),
    ),
  );
}
  ...
  group('TicketPropertiesRail', () {
    ...
    testWidgets('renders user assignee as You', (tester) async {
      await tester.pumpWidget(
        _wrapRail(
          _ticket(assignedAgentId: 'user-1', assigneeType: PrincipalType.user),
        ),
      );
