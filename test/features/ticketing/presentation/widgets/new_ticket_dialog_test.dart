...
class _FakeTicketWorkflowService extends TicketWorkflowService {
  _FakeTicketWorkflowService()
    : super(repository: _FakeTicketRepository(), eventBus: DomainEventBus());
...
Widget _wrap(_FakeTicketWorkflowService fake, Widget child) {
  return ProviderScope(
    overrides: [
      activeWorkspaceIdProvider.overrideWith(() => _FixedWorkspaceId('ws-1')),
      ...
      activeTicketProviderProvider.overrideWith((ref) => TicketProvider.local),
    ],
    // The dialog raises its toast from the dialog's own context (root navigator
    // overlay), so the scope must sit above the navigator.
    child: testWrapWithToastOverlay(child),
  );
}
...
void main() {
  group('NewTicketDialog rendering', () {
          ...
          Builder(
            builder: (context) => Center(
              child: ElevatedButton(
                onPressed: () =>
                    showNewTicketDialog(context, workspaceId: 'ws-1'),
    ...
    testWidgets('title field is autofocused', (tester) async {
      ...
      final titleField = tester.widget<CcTextField>(
        find.byType(CcTextField).at(0),
      );
  ...
  group('NewTicketDialog submission', () {
    testWidgets('valid title creates ticket with defaults', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Fix login bug');
      await tester.tap(find.text('Create'));
      await tester.pumpAndSettle();
    ...
    testWidgets('submits description when provided', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Fix auth');
    ...
    testWidgets('enter on title field submits', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Quick fix');
    ...
    testWidgets('create more clears fields and keeps dialog open', (
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'First ticket');
      await tester.enterText(
        find.byType(CcTextField).at(1),
        'Some description',
      );
      ...
      expect(
        tester
            .widget<CcTextField>(find.byType(CcTextField).at(0))
            .controller!
            .text,
        isEmpty,
      );
      expect(
        tester
            .widget<CcTextField>(find.byType(CcTextField).at(1))
            .controller!
            .text,
        isEmpty,
      );
    ...
    testWidgets('cancel button closes dialog without creating', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Something');
    ...
    testWidgets('shows snackbar on submission error', (tester) async {
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Will fail');
      await tester.tap(find.text('Create'));
      await tester.pumpAndSettle();

      expect(find.text('Failed: Exception: Network failure'), findsOneWidget);
      // Dialog still open after error.
      expect(find.text('Create'), findsOneWidget);
    });

    testWidgets('submit button shows progress while submitting', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            ticketWorkflowServiceProvider.overrideWith((ref) => fake),
            activeTicketProviderProvider.overrideWith(
              (ref) => TicketProvider.local,
            ),
          ],
          child: testWrap(
            Builder(
      ...
      await tester.enterText(find.byType(CcTextField).at(0), 'Slow');
      await tester.tap(find.text('Create'));
      await tester.pump();
