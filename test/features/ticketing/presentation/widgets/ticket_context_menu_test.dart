...
class _FakeTicketRepository implements TicketRepository {
  ...
  @override
  Stream<List<Ticket>> watchByStatus(String ws, TicketStatus s) =>
      const Stream.empty();
  @override
  Stream<List<Ticket>> watchByAssignee(String ws, String a) =>
      const Stream.empty();
  @override
...
class _FakeTicketLinkRepository implements TicketLinkRepository {
  ...
  @override
  Stream<List<TicketLink>> watchForTicket(String ws, String t) =>
      const Stream.empty();
}
...
void main() {
  ...
  group('state-based visibility', () {
    ...
    testWidgets('menu renders with correct icon set for submenus', (
      ...
      expect(find.byIcon(AppIcons.circleDashed), findsOneWidget); // Status
      expect(find.byIcon(AppIcons.signalHigh), findsOneWidget); // Priority
      expect(find.byIcon(AppIcons.userRound), findsOneWidget); // Assignee
      expect(find.byIcon(AppIcons.box), findsOneWidget); // Project
      expect(
        find.byIcon(AppIcons.gitCompareArrows),
        findsOneWidget,
      ); // Relate to
      expect(find.byIcon(AppIcons.clipboard), findsOneWidget); // Copy ID
      expect(find.byIcon(AppIcons.trash2), findsOneWidget); // Delete
    });
  });
  ...
  group('interactions', () {
    ...
    testWidgets('chevron icons on submenu launchers', (tester) async {
      ...
      expect(find.byIcon(AppIcons.chevronRight), findsNWidgets(5));
    });
  ...
  group('submenu interactions', () {
    ...
    testWidgets('delete row is present with destructive styling', (
      tester,
    ) async {
      _setupView(tester);
      await _openMenu(tester, _ticket());
