...
void main() {
  ...
  group('ProjectStatus', () {
    group('fromStorage', () {
      ...
      test(
        'defaults to active for unknown',
  ...
  group('Project', () {
    ...
    test('rejects an empty name', timeout: const Timeout.factor(2), () {
      expect(() => make(name: ''), throwsA(isA<ArgumentError>()));
    });
    ...
    group('copyWith', () {
      ...
      test(
        'clears description with removeDescription',
        timeout: const Timeout.factor(2),
        () {
      ...
      test(
        'preserves all fields when no args given',
        timeout: const Timeout.factor(2),
        () {
    ...
    group('equality', () {
      test(
        'equal when id, name, description, color, status, updatedAt match',
        timeout: const Timeout.factor(2),
        () {
          final a = make(
            id: 'x',
      ...
      test(
        'createdAt difference does not affect equality',
        timeout: const Timeout.factor(2),
        () {
          final a = Project(
            id: 'p',
