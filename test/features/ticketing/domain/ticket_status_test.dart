...
void main() {
  ...
  group('canTransitionTo', () {
    ...
    test('backlog allowed transitions', () {
      ...
      expect(
        TicketStatus.backlog.canTransitionTo(TicketStatus.blocked),
        isFalse,
      );
      expect(
        TicketStatus.backlog.canTransitionTo(TicketStatus.inReview),
        isFalse,
      );
      expect(TicketStatus.backlog.canTransitionTo(TicketStatus.done), isFalse);
      expect(
        TicketStatus.backlog.canTransitionTo(TicketStatus.failed),
        isFalse,
      );
    });
    ...
    test('inProgress allowed transitions', () {
      ...
      expect(
        TicketStatus.inProgress.canTransitionTo(TicketStatus.backlog),
        isFalse,
      );
      expect(
    ...
    test('blocked allowed transitions', () {
      ...
      expect(
        TicketStatus.blocked.canTransitionTo(TicketStatus.failed),
        isFalse,
      );
      expect(
        TicketStatus.blocked.canTransitionTo(TicketStatus.inReview),
        isFalse,
      );
    });
    ...
    test('inReview allowed transitions', () {
      ...
      expect(
        TicketStatus.inReview.canTransitionTo(TicketStatus.blocked),
        isFalse,
      );
      expect(
        TicketStatus.inReview.canTransitionTo(TicketStatus.backlog),
        isFalse,
      );
    ...
    test('done has no outgoing transitions except self', () {
      for (final s in TicketStatus.values) {
        if (s == TicketStatus.done) {
          ...
          expect(
            TicketStatus.done.canTransitionTo(s),
            isFalse,
            reason: 'done → $s',
          );
        }
    ...
    test('failed has no outgoing transitions except self', () {
      for (final s in TicketStatus.values) {
        if (s == TicketStatus.failed) {
          ...
          expect(
            TicketStatus.failed.canTransitionTo(s),
            isFalse,
            reason: 'failed → $s',
          );
        }
    ...
    test('cancelled has no outgoing transitions except self', () {
      for (final s in TicketStatus.values) {
        if (s == TicketStatus.cancelled) {
          ...
          expect(
            TicketStatus.cancelled.canTransitionTo(s),
            isFalse,
            reason: 'cancelled → $s',
          );
        }
