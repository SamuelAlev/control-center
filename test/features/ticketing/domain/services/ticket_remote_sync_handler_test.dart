...
class _FakeProviderPort implements TicketProviderPort {
  _FakeProviderPort({
    ...
    TicketProviderCapabilities? capabilities,
       ...
       _capabilities =
           capabilities ?? TicketProviderCapabilities.full(provider);
  ...
  Future<RemoteTicket> transitionStatus(
    ...
    _throwIfSet();
    return nextResult ??
        RemoteTicket(externalId: externalId, title: 'ticket', status: target);
  }
...
void main() {
  ...
  group('onCreated sync flow', () {
    test(
      'creates remote ticket and upserts mirror with external key',
      () async {
        _seedTicket(repo);
        final now = DateTime.now();

        bus.publish(
          TicketCreated(ticketId: 't1', occurredAt: now, workspaceId: 'ws1'),
        );
        await _settle();
    ...
    test('draft carries labels from ticket', () async {
      _seedTicket(
        repo,
        (t) => _makeTicket(title: 'Label ticket', labels: ['bug', 'ui']),
      );
  ...
  group('guard: not remote (provider = local)', () {
    ...
    test('_onStatusChanged skips local tickets', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(provider: TicketProvider.local, externalKey: 'K-1'),
      );
      bus.publish(
        TicketStatusChanged(
          ticketId: 't1',
  ...
  group('guard: provider mismatch', () {
    ...
    test('_onStatusChanged skips when ticket provider differs', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(provider: TicketProvider.jira, externalKey: 'K-1'),
      );
      bus.publish(
        TicketStatusChanged(
          ticketId: 't1',
  ...
  group('guard: capability missing', () {
    ...
    test('_onAssigned skips when supportsAssignee is false', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: 'K-1', assignedAgentId: 'a1'),
      );
      providerPort = _FakeProviderPort(
        capabilities: const TicketProviderCapabilities(
          provider: TicketProvider.linear,
  ...
  group('guard: missing externalKey (update/status/assign only)', () {
    ...
    test('_onAssigned skips when externalKey is null', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: null, assignedAgentId: 'a1'),
      );
      bus.publish(
        TicketAssigned(
          ticketId: 't1',
  ...
  group('guard: missing assignedAgentId (assign only)', () {
    test('_onAssigned skips when assignedAgentId is null', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: 'K-1', assignedAgentId: null),
      );
      bus.publish(
        TicketAssigned(
          ticketId: 't1',
  ...
  group('onAssigned sync flow', () {
    test('assigns agent on remote provider', () async {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: 'KEY-1', assignedAgentId: 'agent-42'),
      );
  ...
  group('onReassigned sync flow', () {
    test('delegates to assign with new agent id', () async {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: 'KEY-1', assignedAgentId: 'agent-99'),
      );
  ...
  group('onDetailsUpdated sync flow', () {
    test(
      'patches remote ticket with title, description, and priority',
      () async {
        _seedTicket(
          repo,
          (_) => _makeTicket(
  ...
  group('error handling', () {
    ...
    test('_onAssigned survives provider.assign throwing', () {
      _seedTicket(
        repo,
        (_) => _makeTicket(externalKey: 'KEY-1', assignedAgentId: 'a1'),
      );
    ...
    test('handler survives error and processes next event normally', () async {
      ...
      repo.store['t1'] = _makeTicket(externalKey: 'KEY-1', title: 'New title');
