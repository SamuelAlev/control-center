import 'package:cc_domain/features/ticketing/domain/services/agent_loop_guard.dart';
...
void main() {
  group('AgentLoopGuard', () {
    ...
    test(
      'does not suppress when trigger author differs from target agent',
    ...
    test(
      'suppresses self-trigger (same agent triggers itself)',
      timeout: const Timeout.factor(2),
      () {
        expect(
          guard.shouldSuppress(
    ...
    test(
      'suppresses self-trigger regardless of isAgentAuthor',
      timeout: const Timeout.factor(2),
      () {
        expect(
          guard.shouldSuppress(
    ...
    test(
      'different agents triggering different targets are not suppressed',
      timeout: const Timeout.factor(2),
      () {
        expect(
          guard.shouldSuppress(
    ...
    test(
      'suppresses for identical empty string author and target',
      timeout: const Timeout.factor(2),
      () {
        expect(
          guard.shouldSuppress(
    ...
    test(
      'human author, participants null — not suppressed',
      timeout: const Timeout.factor(2),
      () {
        expect(
          guard.shouldSuppress(
