...
void main() {
  group('ProjectService', () {
    ...
    test('update from another workspace does not resolve the id', () async {
      final created = await service.create(workspaceId: 'ws1', name: 'Proj');

      // A workspace id selects the database file: 'ws1''s project id is absent
      // in 'ws2', so the update finds nothing instead of crossing over.
      expect(
        await service.update(created.id, workspaceId: 'ws2', name: 'X'),
        isNull,
      );
      expect((await repo.getById('ws1', created.id))!.name, 'Proj');
    });
    ...
    test('delete removes the project', () async {
      ...
      expect(await repo.getById('ws1', created.id), isNull);
    });
    ...
    test(
      'delete from another workspace is a no-op and the row survives',
      () async {
        final created = await service.create(workspaceId: 'ws1', name: 'Proj');

        await service.delete(created.id, workspaceId: 'ws2');

        expect(repo.deleteCount, 0);
        expect(await repo.getById('ws1', created.id), isNotNull);
      },
    );
  });
}
