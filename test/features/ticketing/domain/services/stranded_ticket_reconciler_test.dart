...
class FakeTicketRepository implements TicketRepository {
  ...
  Future<List<Ticket>> forAgent(String workspaceId, String agentId) async {
    ...
    return _store.values
        .where(
          (t) => t.workspaceId == workspaceId && t.assignedAgentId == agentId,
        )
        .toList();
  }
...
class _FakeTicketWorkflowService extends TicketWorkflowService {
  _FakeTicketWorkflowService()
    : super(repository: FakeTicketRepository(), eventBus: DomainEventBus());
  ...
  Future<void> transitionStatus(
    ...
    transitionCalls.add(
      _TransitionCall(ticketId, target, workspaceId, force: force),
    );
  }
...
void main() {
  ...
  group('StrandedTicketReconciler.reconcile', () {
    test('skips terminal tickets', () async {
      ...
      expect(await ticketRepo.getById(ws, 't-done'), same(terminalTicket));
      expect(
        await ticketRepo.getById(ws, 't-cancelled'),
        same(cancelledTicket),
      );
      expect(await ticketRepo.getById(ws, 't-failed'), same(failedTicket));
    }, timeout: const Timeout.factor(2));

    test(
      'does not process tickets whose assigned agent is not in the system',
      () async {
        ...
        final sut = StrandedTicketReconciler(
          ...
          agentRepo: FakeAgentRepository([
            makeAgent(id: agentId, workspaceId: ws),
          ]),
          runLogRepo: FakeAgentRunLogRepository([]),
        ...
        final stored = await ticketRepo.getById(ws, 't-1');
    ...
    test(
      're-dispatches (sets status to open) when ticket has no runs',
      () async {
        final ticketRepo = FakeTicketRepository();
        final agent = makeAgent(id: agentId, workspaceId: ws);
        final ticket = makeTicket(id: 't-1', workspaceId: ws, agentId: agentId);
        ticketRepo._put(ticket);
        ...
        final updated = await ticketRepo.getById(ws, 't-1');
        expect(updated!.status, TicketStatus.open);
      },
      timeout: const Timeout.factor(2),
    );

    test('handles failed run with retries < max — re-dispatches '
    ...
    test('escalates to blocked when retries exhausted on failed run', () async {
      final ticketRepo = FakeTicketRepository();
      final workflow = _FakeTicketWorkflowService();
      final agent = makeAgent(id: agentId, workspaceId: ws);
      final ticket = makeTicket(id: 't-1', workspaceId: ws, agentId: agentId);
    ...
    test('in-progress run with retries < max does nothing '
    ...
    test('budget block prevents re-dispatch and escalates to blocked '
        'instead', () async {
      final ticketRepo = FakeTicketRepository();
      final workflow = _FakeTicketWorkflowService();
    ...
    test('budget check returning null allows re-dispatch '
    ...
    test('budget check error does not escalate or re-dispatch '
    ...
    test(
      ...
      () async {
        ...
        final stored2 = await ticketRepo.getById(ws, 't-2');
        expect(stored2!.status, TicketStatus.open);
        final stored1 = await ticketRepo.getById(ws, 't-1');
    ...
    test('does not escalate when canTransitionTo(blocked) '
    ...
    test(
      'reconcile with no tickets for any agent completes without error',
      () async {
        final agent = makeAgent(id: agentId, workspaceId: ws);
