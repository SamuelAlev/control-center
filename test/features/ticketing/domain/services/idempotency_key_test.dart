...
void main() {
  group('IdempotencyKey', () {
    ...
    test('inequality when fields differ', timeout: const Timeout.factor(2), () {
      ...
      expect(
        ...
        isNot(
          equals(
            const IdempotencyKey(ticketId: 't2', agentId: 'a1', source: 's'),
          ),
        ),
      );
      expect(
        ...
        isNot(
          equals(
            const IdempotencyKey(ticketId: 't1', agentId: 'a2', source: 's'),
          ),
        ),
      );
      expect(
        ...
        isNot(
          equals(
            const IdempotencyKey(ticketId: 't1', agentId: 'a1', source: 'x'),
          ),
        ),
      );
    });
    ...
    test(
      'hashCode differs when fields differ',
      timeout: const Timeout.factor(2),
      () {
        const a = IdempotencyKey(ticketId: 't1', agentId: 'a1', source: 's');
  ...
  group('DispatchDedupGuard', () {
    test(
      'isDuplicate returns false for unseen key',
      timeout: const Timeout.factor(2),
      () {
    ...
    test(
      'isDuplicate returns true after recording',
      timeout: const Timeout.factor(2),
      () {
        final guard = DispatchDedupGuard();
        const key = IdempotencyKey(ticketId: 't1', agentId: 'a1', source: 's');
    ...
    test(
      'clear of unrecorded key is a no-op',
      timeout: const Timeout.factor(2),
      () {
        final guard = DispatchDedupGuard();
        const key = IdempotencyKey(ticketId: 't1', agentId: 'a1', source: 's');
    ...
    test('purge removes expired entries', timeout: const Timeout.factor(2), () {
      ...
      guard.record(key);
      expect(
        guard.isDuplicate(key),
        isFalse,
      ); // zero window = immediately expired
    });
    ...
    test(
      'multiple keys: some purged, some survive',
      timeout: const Timeout.factor(2),
      () {
        final guard = DispatchDedupGuard(window: const Duration(hours: 1));
        const freshKey = IdempotencyKey(
          ticketId: 't1',
          agentId: 'a1',
          source: 's',
        );
        const anotherKey = IdempotencyKey(
          ticketId: 't2',
          agentId: 'a2',
          source: 'x',
        );
    ...
    test(
      'recording same key twice is idempotent',
      timeout: const Timeout.factor(2),
      () {
        final guard = DispatchDedupGuard(window: const Duration(hours: 1));
        const key = IdempotencyKey(ticketId: 't1', agentId: 'a1', source: 's');
    ...
    test(
      'does not confuse keys with unrelated string values',
      timeout: const Timeout.factor(2),
      () {
        final guard = DispatchDedupGuard(window: const Duration(hours: 1));
