...
`rift` is the single documented exception: it is not built on Windows (no MSVC copy-on-write backend), where plain `git worktree` is the _backend_ rather than a degradation. The only other fallbacks left are environment-driven, never build-driven: a filesystem without copy-on-write support and semantic search staying FTS-only until the on-device embedding **model** is downloaded (models are the only artifacts fetched at runtime).
