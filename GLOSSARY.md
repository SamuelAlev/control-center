...
Enumerates desktop notification types: `agentRunCompleted`, `pullRequestPublished`, `prMerged`, `newMessage`, `externalPr`, `pipelineCompleted`, `taskCompleted`, `achievementUnlocked`.
...
The `mcp` feature hosts a JSON-RPC 2.0 MCP server exposing **70 typed tools** to external MCP clients. It uses `application/` instead of `presentation/` because tools are use-case logic, not UI. Tools receive their dependencies as typed constructor parameters (no `Ref` injection); the wiring happens once in `mcpToolRegistryProvider`.
...
**Tool families** (registered into one shared registry): agents (list/hire/update/fire/kill, run logs), skills, workspaces, messaging (channels, DMs, thread replies), pull requests & review (`start_ai_review`, review nodes, verdicts, `dispatch_reviewers`, `finalize_review`), memory (facts, policies, domains, observations, `search_memory`), code graph (`search_code`, `code_symbol`, `code_callers`, `code_callees`, `code_impact`), the universal `read` resource tool, user interaction (`ask_user_question`, `request_confirmation`, `suggest_tasks`), `doctor`, and ticketing/projects (CRUD, lifecycle, assignment, links, approval gates). Tools touching workspace-scoped data require `workspace_id`; only genuinely global tools (`list_workspaces`, `create_workspace`) are exempt.
...
- **Schema version: 31**, tracked in `AppDatabase.schemaVersion`, with ordered `MigrationStep(from, to, migrate)` steps in `migration_steps.dart`.
- Workspace-scoped tables carry a `workspaceId` column and MUST be filtered by it on every read (see the workspace-isolation invariant). Code-graph tables are keyed by `(workspaceId, repoId)`.
