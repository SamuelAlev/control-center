...
A user-named, soft-deletable container that groups agents and repositories, the top-level isolation tenant. Each agent, space, ticket and memory fact is scoped to a single workspace.
...
A join entity that decouples PR review from messaging. A review space is a regular space. The review context is established by this association record, not by the space kind. This separation lets spaces own messages independently of review lifecycle.

**Key attributes:** `id`, `spaceId`, `workspaceId`, `prNodeId`, `prNumber`, `repoFullName`, `status` (ReviewSpaceStatus), `createdAt`, `updatedAt`

**Location:** `packages/cc_domain/lib/core/domain/entities/review_space_association.dart`
...
A messaging container scoped to a workspace, with one or more participants. There is no separate DM/group distinction — a one-on-one chat and a multi-participant room are both just spaces, differing only in participant count. Each space carries a `Mode` that gates which tools and sandbox policies apply and a provisioning status. `workspaceId` is nullable: some system/legacy spaces carry no workspace.

**Key attributes:** `id`, `name`, `workspaceId` (String?), `mode` (Mode), `provisioningStatus` (SpaceProvisioningStatus), `provisioningStep` (SpaceProvisioningStep?), `kind` (SpaceKind — `topic`, `pr`, `agentPeer`, `system`, `slack`), `pipelineRunId`, `createdAt`, `updatedAt`

**Location:** `packages/cc_domain/lib/features/messaging/domain/entities/space.dart`

### Conversation

A message stream inside a space. Conversations in a space are flat equals — there is no primary stream — each with its own uuid, message history and agent sessions. A conversation whose `anchorMessageId` is set is a **thread**: anchored to one message of a sibling conversation in the same space, seeded with that message as its agent context; threads never nest.

**Key attributes:** `id` (own uuid), `workspaceId` (String?), `spaceId`, `title`, `status` (ConversationStatus), `anchorMessageId` (String?), `createdByPrincipalId`, `createdAt`, `updatedAt`
...
The live rollup of one thread, projected server-side so the feed can draw a Slack-style "N replies · last reply …" row under the message a thread was branched from. Carries no state of its own: one grouped query per space replaces one message subscription per thread, which is what that row would otherwise cost in a space holding a dozen of them.

**Key attributes:** `threadId`, `anchorMessageId` (what the feed keys on), `title`, `replyCount`, `lastReplyAt` (DateTime?), `participantIds` (distinct senders, newest-first, capped)

**Location:** `packages/cc_domain/lib/features/messaging/domain/value_objects/thread_summary.dart`, streamed by `ConversationRepository.watchThreadSummaries` / `conversation.watchThreadSummaries`

### Message

A single message within a space, always belonging to one `Conversation`. Messages carry metadata (mentions, plan status, stream completion flags), support compaction and revert and are typed by `MessageType` to control rendering.

**Key attributes:** `id`, `spaceId`, `conversationId`, `senderId`, `senderType` (SenderType), `content`, `messageType` (MessageType), `metadata`, `compacted`, `reverted`, `revertedAt`, `createdAt`

**Location:** `packages/cc_domain/lib/core/domain/entities/message.dart` (shared kernel, consumed by messaging, pr_review, pipelines, mcp)

### SpaceParticipant

A membership record linking a principal — an agent or a human user — to a space with a role. Each member is identified by `principalId` plus `participantType`; humans are first-class rows (no sentinel), each keeping their own read cursor.

**Key attributes:** `id`, `spaceId`, `principalId` (agent or user id), `participantType` (PrincipalType), `role`, `joinedAt`, `lastReadAt`

**Location:** `packages/cc_domain/lib/features/messaging/domain/entities/space_participant.dart`

### SpaceRepo

A many-to-many join declaring which repos a space provisions worktrees for. A space with no rows falls back to every workspace repo (back-compat); PR spaces ignore the table (always the PR's repo). Table-only — no domain entity.

**Location:** `packages/cc_persistence/lib/database/tables/space_repos.dart`, DAO `packages/cc_persistence/lib/database/daos/space_repo_dao.dart`
...
**Location:** `packages/cc_domain/lib/core/domain/entities/message.dart`

### MessageReaction

A lightweight emoji reaction on a space message. The reactor is a principal — human OR agent, co-equal — unique per `(workspaceId, messageId, principalId, emoji)` and rendered from a fixed palette (`kReactionPalette`: 👍 👎 🎉 ❤️ 👀 ✅). Distinct from pr_review's `ReactionGroup`, which mirrors GitHub PR reactions.

**Location:** table `packages/cc_persistence/lib/database/tables/message_reactions_table.dart`; client DTO + palette `lib/features/messaging/providers/space_reactions_provider.dart`
...
`DispatchAgentUseCase` resolves the effective prompt, CLI name and mode for a run. `BuildAgentPromptUseCase` assembles the agent's prompt from the mode prompts plus space context (`BuildConversationContextUseCase`) and memory context (`BuildMemoryContextUseCase`).

**Location:** `packages/cc_domain/lib/features/dispatch/domain/usecases/`

### PromptBuilder / mode prompts

The space-mode prompt library (chat / plan / review system-prompt blocks, role personas, protocol docs), formerly the `agent_modes` feature, folded into dispatch.

**Location:** `packages/cc_domain/lib/features/dispatch/domain/prompts/`

### MentionContext / MentionRosterEntry

`MentionContext` describes who summoned an agent plus the full space roster available for mention resolution. `MentionRosterEntry` is one roster row (agent ID, name, whether top-level).
...
**Key attributes:** `id`, `workspaceId`, `spaceId`, `conversationId`, `agentId`, `userText`, `kind` (AgentGoalKind), `status` (AgentGoalStatus), budget walls (`costCapCents`, `deadlineAt`, `maxRuns`), `costCents`, `runCount`, `activeRunId`, `consecutiveFailures`, `requestedByUserId`, `summary`, `createdAt`, `updatedAt`
...
The aggregate root: one "big ask" the system proposed a whole-team plan for. Always workspace-scoped and opened against a parent ticket with one shared discussion space. Carries a monotonic `revision` and the `approvedRevision` the user signed off on, so a mid-flight replan can't silently change what was approved.

**Key attributes:** `id`, `workspaceId`, `proposal`, `parentTicketId`, `spaceId`, `orchestratorAgentId`, `status`, `revision`, `approvedRevision`, `approvedNodeKeys` (List&lt;String&gt;? — the partial/branched-approval surface), `pipelineTemplateId`, `pipelineRunId`, `teamId`, `projectId`, `estimatedCostCents`, `maxCostCents`, `hiredAgentIds`, `errorMessage`, `createdAt`, `updatedAt`, `completedAt`
...
Local, Granola-style meeting notes. Records the microphone ("me") and system-output ("them") spaces concurrently, transcribes both live with on-device Whisper, diarizes the remote space into individual speakers and runs a deterministic summarization pipeline that emits structured notes, action items and decisions. Everything is workspace-scoped and stays on the machine.
...
A diarized speaker identity, one per `(meeting, space, label)` tuple. Carries the auto-assigned label ("Person 1") and an optional user-supplied `displayName`.
...
A built-in inference adapter definition (e.g. Control Center built-in, Claude Code) with the CLI binary name used for detection.
...
Control Center's **built-in agent runtime** (`AdapterTransport.harness`) — an in-process, event-sourced loop that talks to LLM providers directly and runs tools in-process, with no external CLI subprocess. It is the default runner; the only shipped external runner is the Claude Code CLI. Cursor (Ultra/Pro) is an LLM *provider* on this harness. The harness is its own pub-workspace package pair: `packages/cc_harness` is the web-safe kernel and `packages/cc_harness_runtime` is the VM-only batteries (concrete providers, OAuth/PKCE brokering, credential stores, the built-in tool set, the watchdog advisor). CC-coupled adapters that bridge the kernel to server services live in `packages/cc_infra/lib/src/harness/`.

### AgentLoop

The event-sourced turn loop — an abstract interface; `AgentLoopRunner` is the implementation. Emits typed events (text/thinking deltas, tool-call start/result, turn-complete) so the dispatch layer can stream reasoning and render tool-call cards. Supports prompt caching, context compaction (`context/harness_compaction.dart`), reasoning-effort mapping (`provider/effort_mapping.dart`), USD cost accounting, an advisor/steering space (`loop/advisor.dart`), slash commands (`tools/command_runner.dart`) and subagent spawning. The kernel is consumed through a curated facade (`cc_harness.dart`, held at ~40 symbols by a ratchet test) plus topic entrypoints (`loop.dart`, `provider.dart`, `tools.dart`, `context.dart`, `messages.dart`, `slash_command.dart`, `cancellation.dart`).
...
A fourth source is not installed at all: a **repo-scoped skill**, shipped by a checked-out worktree in `.agents/skills` / `.claude/skills` / `.opencode/skills`. An agent carries only the skills of the repo it is currently working in (`ActiveRepoTracker` infers that from tool-call paths; `RepoSkillProjector` materializes them into the overlay `.claude/skills` and composes them into `AGENTS.md` for the built-in harness). `RepoSkillCatalog` is the shared scan-gated discovery behind both that projection and the `skills.repoSkills` op the composer's palette reads. Invocation is namespaced — `/skill:<name>`, `/skill:<repo>:<name>` — so a skill can share a builtin command's name.

**Location:** `packages/cc_domain/lib/features/skills/domain/entities/skill_lock.dart`, `packages/cc_domain/lib/features/skills/domain/ports/skill_bundle_port.dart`, `packages/cc_infra/lib/src/skills/repo_skill_catalog.dart`
...
Push-to-talk voice input. The client captures the mic (16 kHz mono PCM16) and streams frames to the host, which runs the same rolling-window transcriber the meeting recorder uses; `DictationControlPort` is the client-side control space (start/stop/cancel, `DictationPartial` updates).
...
A sealed tagged union of where a participant currently is: `SpaceLocus` / `FileLocus` (repo + path + line) / `PrLocus` / `TicketLocus` / `PlanNodeLocus`. Compact wire keys because cursor-cadence updates ride the ephemeral lane at up to 10 Hz.
...
A per-space per-agent control — `propose-only` / `act-with-approval` / `act-freely` — implemented as a named profile over the [Action guardrails](#action-guardrails-bounded-context) store. Risky actions hit a fail-closed gate regardless of dial position.
...
An event stream + HTTP/LLM cassettes + tool I/O captured for one run, paired with an `AgentConfigHash`. Cassettes are opt-in per agent/space (not ambient surveillance); redaction-before-persistence + a retention service apply.
...
A rule `(scope, ActionClass | commandPrefix) → allow | prompt | deny`. Scope resolution is `space > agent > workspace > mode preset > built-in default` (most-specific wins; within a scope, longest-prefix then most-restrictive). The flat `allow > deny > prompt` precedence was replaced by specificity-then-restrictiveness.
...
Per-space capability flags (push to repo, GitHub API, ticketing, network egress) that the credential broker checks at sandbox launch to gate token/network injection. Serialized as JSON on the agent.
...
`WakeReason` enumerates why an agent was dispatched; `WakeContext` carries the dispatch context (ticket, run, agent, workspace, space, reason, message, pipeline run) injected at dispatch and serialized to environment variables.

**Location:** `packages/cc_domain/lib/core/domain/value_objects/wake_context.dart`

### RepoIsolationBackend

Enum identifying which mechanism produced an isolated repo copy: `rift` copy-on-write clone, or plain `gitWorktree` — provisioned only where no CoW backend exists (Windows), and otherwise seen only on rows persisted before CoW became the sole backend.
...
Surfaces an agent's question (option choices and/or free text) as an interactive inline form in a space and blocks until the user answers. Defines the `AgentQuestionOption` / `Request` / `Answer` payloads.
...
Provisions and tears down isolated copy-on-write worktrees of a local repo (rift) without mutating the source — a CoW failure throws rather than falling back to `git worktree`, which would write into the user's checkout. Defines `RepoIsolationResult`.

**Location:** `packages/cc_domain/lib/core/domain/ports/repo_isolation_port.dart`

### RepoWorkspaceProvisionerPort

Provisions a per-space working root with isolated CoW worktrees of the workspace's repos and tears them down on unit completion (idempotent, no-op-safe by space/space/ticket).

**Location:** `packages/cc_domain/lib/core/domain/ports/repo_workspace_provisioner_port.dart`

### WorkspaceFilesystemPort

Provides filesystem paths and operations for agents, skills, spaces, PR clones and logos within a workspace.
...
Resolves the `Mode` for a given space id, defaulting to `chat` when the id is null or the row is missing.
...
- **MessagingPort**, messaging space operations (send, add agents, create groups, send-and-dispatch, refine plan). `packages/cc_domain/lib/features/messaging/domain/ports/messaging_port.dart`
...
- **DispatchReviewersPort**, dispatches reviewer agents into a review space; shared by the MCP tool and pipeline step bodies. `packages/cc_domain/lib/features/pipelines/domain/ports/dispatch_reviewers_port.dart`
...
- **TicketWorkflowService** (ticketing), pure-domain ticket lifecycle engine with optimistic-concurrency mutation chokepoint and workspace-isolation enforcement; never dispatches or opens spaces itself.
...
The registry keys all tools by name (lookup, definition listing, name enumeration). The dispatcher routes JSON-RPC requests to tools, applying space-mode gating (`ModeToolGuard`) and destructive-action confirmation prompts.
...
The server-side RPC entrypoint (`cc_host`): accepts a transport (WSS socket or in-process space), authenticates the session against a paired-device PSK, binds it to a workspace and dispatches repo-op + subscription frames to the server's repository catalog. Embeds in both the headless `cc_server` binary and (for the in-process path) the desktop.

**Location:** `packages/cc_host/`, composed in `packages/cc_server_core/`

### InProcessRpcChannel

An in-memory back-to-back RPC space (no serialization, no sockets): the desktop-LOCAL "be your own server" data path and the protocol-conformance test harness that drives the full repo-RPC + subscription surface and asserts parity with direct calls.
...
The repository is a single-resolved-lockfile Dart workspace of twenty-one members (6 apps + 15 packages, plus the root app). The **apps** are `cc_server` (headless server binary), `cc_worker` (headless fleet executor binary — lease-plumbing MVP: `agentRun` is the only real runner; other `JobKind`s probe with git), `cc_remote` (phone PWA thin client), `cc_signaling_server` (stateless relay broker), `cc_gallery` (Widgetbook catalogue) and `cc_demo_server` (public demo host); the root `control_center` app is the desktop+web thin client. The **packages** are `cc_ui` (design system), `cc_domain` (pure-Dart shared kernel: entities/value-objects/ports/events/services + every feature's `domain/`), `cc_harness` (web-safe built-in agent-loop kernel) + `cc_harness_runtime` (its VM-only batteries: providers, OAuth/PKCE, tool set), `cc_rpc` (client transports), `cc_host` (server kernel + presence hub), `cc_data` (remote repositories), `cc_persistence` (server-side Drift/SQLite), `cc_server_core` (app-server composition: repo-RPC catalog + MCP registry + LocalRpcServer + identity/presence/fleet/evals runtime), `cc_infra` (server-side VM-only adapters + fleet + tunnels), `cc_mcp` (MCP tool surface), `cc_mcp_client` (bridges external MCP servers in), `cc_markdown` (in-repo typed-AST markdown engine + native mermaid), `cc_natives` (FFI leaf: rift/fff/tree-sitter/watcher/pty/audio/ML) and `system_audio_capture` (audio loopback plugin).

## Database Conventions

- Persistence is **split by workspace** and owned entirely by `cc_persistence`; only `cc_server` opens a database — every client reads/writes over RPC. There is no single app database: **`GlobalDatabase`** (`<dataDir>/global.db`, schema v1, 11 tables — the workspaces registry, `users`, `user_preferences`, `paired_devices`, `rss_feeds`/`rss_articles`, the fleet queue `workers`/`jobs`/`placement_log`, `workspace_routes`, `server_meta`) and one **`WorkspaceDatabase`** file per workspace (`<dataDir>/<workspaceId>/workspace.db`, schema v1 — a squashed baseline with an empty migration chain — holding everything else, including `repos`). Table definitions live in `packages/cc_persistence/lib/database/tables/` (Drift); DAOs in `packages/cc_persistence/lib/database/daos/`.
...
- **Backup is a directory, not a file:** `backups/<ts>/{manifest.json, global.db, <workspaceId>/workspace.db}`, each written with `VACUUM INTO` (safe on a live WAL). `workspace.export` / `workspace.import` hand a single workspace around as one file.
...
- **Full-text search** via FTS5 external-content virtual tables (`memory_facts_fts`, `code_symbols_fts`, `conversation_messages_fts`) with triggers reinstalled per workspace file in `beforeOpen`.
