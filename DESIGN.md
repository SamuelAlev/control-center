...
real system, implemented in Flutter in the in-repo **`cc_ui`** design-system
package (`packages/cc_ui/lib/src/tokens/` + `…/foundation/`) and read via
`context.designSystem` from the nearest `CcTheme` (`lib/core/theme/` keeps thin
re-export shims for back-compat). Every token and component below is browsable,
live, in the **`cc_gallery`** Widgetbook app — see *Implementation & gallery* at
...
This spec is shipped by the **`cc_ui`** package (`packages/cc_ui/`) — a
Material-/Cupertino-free design system built directly on
`flutter/widgets.dart`. Read tokens via `context.designSystem`; never hardcode a
value this document names.

**Spec → code map**

| This spec | Code (`cc_ui`) |
|---|---|
| Colors (`{colors.*}`) | `DesignSystemTokens` — semantic aliases (`canvas`, `surface`, `panel`, `fg`, `muted`, `accent`, `success`, `warn`, `danger`, …) over the full role scale. |
| Typography (one family, weight 400; size + color only) | `CcTypography` — `displayHero`/`display`/`title`/`body`/`bodySm`/`caption`/`label`/`monoNum`; families via `CcFonts`. |
| Spacing (8px base) | `AppSpacing` — `xxs`(2) → `xxxl`(48) + gap widgets. |
| Rounded (2px / 4px / pill) | `AppRadii` — `brSm`/`brMd`(2), `brLg`/`brXl`(4), `pill`(999). |
| Elevation & depth (warm amber shadows) | `AppShadows.golden`/`.soft`, `CcElevation` (z-index scale + `floating`/`raised`). |
| Motion + reduced-motion path | `CcMotion` (`fast`/`normal`/`slow`, `standard`/`emphasized`) with `CcMotion.resolve(context, …)` collapsing to zero under reduce-motion. |
| Focus ring (always-visible 2px accent) | `FocusRing` + `FocusModality` (keyboard-only `:focus-visible`). |
| Components | 30+ `Cc*` widgets — `CcButton`, `CcSelect`, `CcCard`, `CcSidebar`, `CcDialog`, … |

**The living reference.** `apps/cc_gallery` is a [Widgetbook](https://docs.widgetbook.io/)
catalogue with ~130 interactive use-cases across **Components** and
**Foundations** (token specimens + primitives). It is the fastest way to audit
the do's/don'ts above against the real, themed widgets — toggle the Light/Dark
theme addon to check both palettes at once. Before designing or reviewing UI,
open the gallery (`flutter run -d macos` from `apps/cc_gallery`) and read the
relevant component's states. Authoring a new component means adding its states as
`@widgetbook.UseCase` builders there — see `apps/cc_gallery/README.md` and
`packages/cc_ui/README.md`.
