...
// Tuned for a near-white background: shadows tint warm taupe instead of
...
const float GLASS_SHADOW_TINT = 0.22;  // strength of warm taupe shadow tint
...
// Warm morning-mist cloudscape — the near-white canvas deepens through warm
// ivory and soft sand to a taupe-latte horizon, with only a faint ember blush
// low on the canvas (the day counterpart of the dark scene's ember glow).
// Clouds read through sunlit tops and warm taupe-rose bellies — cream and
// stone, never a saturated gold wash.
vec3 sceneColor(vec2 uv) {
	...
	vec3 hazeIvory    = vec3(0.963, 0.941, 0.902);  // warm ivory haze #f6f0e6
	vec3 warmSand     = vec3(0.929, 0.882, 0.812);  // soft sand #ede1cf
	vec3 horizonTaupe = vec3(0.855, 0.769, 0.671);  // taupe latte #dac4ab
	vec3 emberCore    = vec3(0.980, 0.314, 0.059);  // signal orange #fa500f
...
void main() {
	...
	// Shadow: tint *toward* a deeper warm taupe across the body of each
	...
	vec3  shadowHue  = vec3(0.67, 0.60, 0.52);
	...
	// Gentle vignette — a touch deeper so the corners hold the warmth.
	vec2 vp = uv - 0.5;
